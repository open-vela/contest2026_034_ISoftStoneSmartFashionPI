...
基于openvela与ESP32S3，同一硬件双形态-插底座为桌面陪伴机器人(表情、语音、主动陪伴)，装表带为智能穿戴手表，支持手动/语音随时切换。

- **双 UI 模式，无重启切换**：「潮玩表情模式」下设备是一个有情绪的伙伴——16+ 种 GIF 表情（待机、聆听、思考、说话、微笑、睡眠、生病、得意……）随语音交互和系统事件实时变化；「手表模式」下则是完整的智能手表——表盘、天气、闹钟（含响铃）、秒表、运动、智能家居、SOS求助、设置。两种模式可语音或设置触摸或长按按键 10 秒即时切换，无需重启。
- **AI 语音助手**：支持唤醒词唤醒、流式语音对话（集成小米 MiMo 语音与火山引擎端到端语音两条链路），可语音控制屏幕亮度、音量、智能家居设备（如"打开厨房灯/电视"），API Key 支持设备端配置并持久化。
