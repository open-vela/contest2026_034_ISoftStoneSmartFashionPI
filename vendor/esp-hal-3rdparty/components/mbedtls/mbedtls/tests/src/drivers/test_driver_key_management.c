...
/*
 * The esp_unwrap function mbedtls_test_opaque_unwrap_key removes a pad prefix
    ...
    uint8_t *key, size_t key_size, size_t *key_length)
{
    ...
    if (PSA_KEY_TYPE_IS_ECC(psa_get_key_type(attributes))
        ...
        return esp_mbedtls_psa_ecp_generate_key(
        ...
        return esp_mbedtls_psa_rsa_generate_key(
            attributes,
            NULL, 0, /* We don't support custom e in the test driver yet */
            key, key_size, key_length);
#endif
        ...
        return esp_mbedtls_psa_ffdh_generate_key(
            attributes, key, key_size, key_length);
#endif
    }
    ...
    size_t *bits)
{
    ...
    if (PSA_KEY_TYPE_IS_ECC(type)) {
        ...
        return esp_mbedtls_psa_ecp_import_key(
            attributes,
            data, data_length,
            key_buffer, key_buffer_size,
            key_buffer_length, bits);
        ...
        return esp_mbedtls_psa_rsa_import_key(
            attributes,
            data, data_length,
            key_buffer, key_buffer_size,
            key_buffer_length, bits);
        ...
        defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_RSA_PUBLIC_KEY)
        return esp_mbedtls_psa_ffdh_import_key(
            attributes,
            data, data_length,
            key_buffer, key_buffer_size,
            key_buffer_length, bits);
    ...
    size_t *bits)
{
    ...
    key_buffer_temp = esp_mbedtls_calloc(1, key_buffer_size);
    ...
    if (PSA_KEY_TYPE_IS_UNSTRUCTURED(type)) {
        *bits = PSA_BYTES_TO_BITS(data_length);
        ...
        status = esp_psa_validate_unstructured_key_bit_size(type,
                                                        *bits);
        if (status != PSA_SUCCESS) {
            goto exit;
        }
...
#elif defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_KEY_PAIR_IMPORT) || \
        defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_PUBLIC_KEY)
        status = esp_mbedtls_psa_ecp_import_key(
            attributes,
            data, data_length,
            key_buffer_temp, key_buffer_size,
            key_buffer_length, bits);
...
#elif defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_RSA_KEY_PAIR_IMPORT) || \
        defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_RSA_PUBLIC_KEY)
        status = esp_mbedtls_psa_rsa_import_key(
            attributes,
            data, data_length,
            key_buffer_temp, key_buffer_size,
            key_buffer_length, bits);
...
exit:
    esp_mbedtls_free(key_buffer_temp);
    return status;
}
    ...
    uint8_t *data, size_t data_size, size_t *data_length)
{
    if (key_length == sizeof(psa_drv_slot_number_t)) {
        ...
        /* This buffer will be used as an intermediate placeholder for
         * the opaque key till we esp_unwrap the key into key_buffer */
        psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
        psa_key_type_t type = psa_get_key_type(attributes);
    ...
    uint8_t *data, size_t data_size, size_t *data_length)
{
    ...
    if (PSA_KEY_TYPE_IS_ECC(key_type)) {
        ...
        defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_PUBLIC_KEY)
        return esp_mbedtls_psa_ecp_export_public_key(
            attributes,
            key_buffer, key_buffer_size,
            data, data_size, data_length);
#endif
        ...
        defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_RSA_PUBLIC_KEY)
        return esp_mbedtls_psa_rsa_export_public_key(
            attributes,
            key_buffer, key_buffer_size,
            data, data_size, data_length);
#endif
        ...
        return esp_mbedtls_psa_ffdh_export_public_key(
            attributes,
            key_buffer, key_buffer_size,
            data, data_size, data_length);
#endif
    ...
    uint8_t *data, size_t data_size, size_t *data_length)
{
    if (key_length != sizeof(psa_drv_slot_number_t)) {
        ...
        key_buffer_temp = esp_mbedtls_calloc(1, key_length);
        if (key_buffer_temp == NULL) {
            return PSA_ERROR_INSUFFICIENT_MEMORY;
        }
...
#elif defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_KEY_PAIR_EXPORT) || \
                defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_PUBLIC_KEY)
                status = esp_mbedtls_psa_ecp_export_public_key(
                    attributes,
                    key_buffer_temp, *data_length,
                    data, data_size, data_length);
#else
...
#elif defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_RSA_KEY_PAIR_EXPORT) || \
                defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_RSA_PUBLIC_KEY)
                status = esp_mbedtls_psa_rsa_export_public_key(
                    attributes,
                    key_buffer_temp, *data_length,
                    data, data_size, data_length);
#else
