...
#elif defined(MBEDTLS_PSA_BUILTIN_MAC)
        mbedtls_test_driver_mac_hooks.driver_status =
            esp_mbedtls_psa_mac_sign_setup(
...
#elif defined(MBEDTLS_PSA_BUILTIN_MAC)
        mbedtls_test_driver_mac_hooks.driver_status =
            esp_mbedtls_psa_mac_verify_setup(
                operation, attributes, key_buffer, key_buffer_size, alg);
#else
        (void) operation;
        (void) attributes;
...
#elif defined(MBEDTLS_PSA_BUILTIN_MAC)
        mbedtls_test_driver_mac_hooks.driver_status =
            esp_mbedtls_psa_mac_sign_finish(
                operation, mac, mac_size, mac_length);
#else
        (void) operation;
        (void) mac;
...
#elif defined(MBEDTLS_PSA_BUILTIN_MAC)
        mbedtls_test_driver_mac_hooks.driver_status =
            esp_mbedtls_psa_mac_verify_finish(
                operation, mac, mac_length);
#else
        (void) operation;
        (void) mac;
