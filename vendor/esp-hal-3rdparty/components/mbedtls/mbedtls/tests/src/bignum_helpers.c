                               ...
                               const char *input)
{
    ...
    *pX = esp_mbedtls_calloc(*plimbs, sizeof(**pX));
    if (*pX == NULL) {
    ...
    esp_mbedtls_mpi_core_bigendian_to_host(*pX, *plimbs);
    return 0;
                                  ...
                                  mbedtls_mpi_mod_rep_selector int_rep)
{
    ...
    switch (int_rep) {
        case MBEDTLS_MPI_MOD_REP_MONTGOMERY:
            ret = esp_mbedtls_mpi_mod_modulus_setup(N, p, limbs);
            break;
        case MBEDTLS_MPI_MOD_REP_OPT_RED:
            ret = esp_mbedtls_mpi_mod_optred_modulus_setup(N, p, limbs, NULL);
            break;
        default:
...
{
    esp_mbedtls_free((mbedtls_mpi_uint *) N->p);
...
int mbedtls_test_read_mpi(mbedtls_mpi *X, const char *s)
{
     ...
     * This creates an invalid representation, which esp_mbedtls_mpi_read_string()
     * avoids but we want to be able to create that in test data. */
    if (s[0] == '-') {
        ++s;
        negative = 1;
    }
    /* esp_mbedtls_mpi_read_string() currently retains leading zeros.
     * It always allocates at least one limb for the value 0. */
    if (s[0] == 0) {
        esp_mbedtls_mpi_free(X);
        return 0;
    }
    int ret = esp_mbedtls_mpi_read_string(X, 16, s);
    if (ret != 0) {
        return ret;
    }
    if (negative) {
