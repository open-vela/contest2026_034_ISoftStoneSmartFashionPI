                                       ...
                                       int key_destroyable)
{
    ...
    psa_key_type_t type;
    size_t bits;
    psa_status_t status = esp_psa_get_key_attributes(key, &attributes);
    ...
    if (key_destroyable == 0) {
        ...
        status = esp_psa_get_key_slot_number(&attributes, &slot_number);
...
exit:
    /*
     * Key attributes may have been returned by esp_psa_get_key_attributes()
                            ...
                            int key_destroyable)
{
    ...
    if (usage & PSA_KEY_USAGE_SIGN_HASH) {
        status = esp_psa_mac_sign_setup(&operation, key, alg);
        if (key_destroyable && status == PSA_ERROR_INVALID_HANDLE) {
            /* The key has been destroyed. */
            PSA_ASSERT(esp_psa_mac_abort(&operation));
            return 1;
        }
        PSA_ASSERT(status);
        PSA_ASSERT(esp_psa_mac_update(&operation,
                                  input, sizeof(input)));
        PSA_ASSERT(esp_psa_mac_sign_finish(&operation,
                                       mac, sizeof(mac),
                                       &mac_length));
    }
    ...
    if (usage & PSA_KEY_USAGE_VERIFY_HASH) {
        ...
        status = esp_psa_mac_verify_setup(&operation, key, alg);
        if (key_destroyable && status == PSA_ERROR_INVALID_HANDLE) {
            /* The key has been destroyed. */
            PSA_ASSERT(esp_psa_mac_abort(&operation));
            return 1;
        ...
        PSA_ASSERT(esp_psa_mac_update(&operation,
                                  input, sizeof(input)));
        TEST_EQUAL(esp_psa_mac_verify_finish(&operation, mac, mac_length),
                   verify_status);
    }
                               ...
                               int key_destroyable)
{
    ...
    PSA_ASSERT(esp_psa_get_key_attributes(key, &attributes));
    key_type = psa_get_key_type(&attributes);
    iv_length = PSA_CIPHER_IV_LENGTH(key_type, alg);

    if (usage & PSA_KEY_USAGE_ENCRYPT) {
        status = esp_psa_cipher_encrypt_setup(&operation, key, alg);
        if (key_destroyable && status == PSA_ERROR_INVALID_HANDLE) {
            /* The key has been destroyed. */
            PSA_ASSERT(esp_psa_cipher_abort(&operation));
            return 1;
        }
        PSA_ASSERT(status);
        if (iv_length != 0) {
            PSA_ASSERT(esp_psa_cipher_generate_iv(&operation,
                                              iv, sizeof(iv),
                                              &iv_length));
        }
        PSA_ASSERT(esp_psa_cipher_update(&operation,
                                     plaintext, sizeof(plaintext),
                                     ciphertext, sizeof(ciphertext),
                                     &ciphertext_length));
        PSA_ASSERT(esp_psa_cipher_finish(&operation,
    ...
    if (usage & PSA_KEY_USAGE_DECRYPT) {
        ...
        status = esp_psa_cipher_decrypt_setup(&operation, key, alg);
        if (key_destroyable && status == PSA_ERROR_INVALID_HANDLE) {
            /* The key has been destroyed. */
            PSA_ASSERT(esp_psa_cipher_abort(&operation));
            return 1;
        ...
        PSA_ASSERT(status);
        if (iv_length != 0) {
            PSA_ASSERT(esp_psa_cipher_set_iv(&operation,
                                         iv, iv_length));
        }
        PSA_ASSERT(esp_psa_cipher_update(&operation,
                                     ciphertext, ciphertext_length,
                                     ...
                                     &part_length));
        status = esp_psa_cipher_finish(&operation,
                             ...
                             int key_destroyable)
{
    ...
    if (usage & PSA_KEY_USAGE_ENCRYPT) {
        status = esp_psa_aead_encrypt(key, alg,
                                  nonce, nonce_length,
                                  NULL, 0,
                                  plaintext, sizeof(plaintext),
                                  ciphertext, sizeof(ciphertext),
    ...
    if (usage & PSA_KEY_USAGE_DECRYPT) {
        psa_status_t verify_status =
            (usage & PSA_KEY_USAGE_ENCRYPT ?
             PSA_SUCCESS :
             PSA_ERROR_INVALID_SIGNATURE);
        status = esp_psa_aead_decrypt(key, alg,
                                  nonce, nonce_length,
                                  NULL, 0,
                                  ciphertext, ciphertext_length,
                                  plaintext, sizeof(plaintext),
                                  ...
                                  int key_destroyable)
{
    ...
    if (usage & (PSA_KEY_USAGE_SIGN_HASH | PSA_KEY_USAGE_VERIFY_HASH) &&
        ...
        if (usage & PSA_KEY_USAGE_SIGN_HASH) {
            status = esp_psa_sign_hash(key, alg,
        ...
        if (usage & PSA_KEY_USAGE_VERIFY_HASH) {
            psa_status_t verify_status =
                (usage & PSA_KEY_USAGE_SIGN_HASH ?
                 PSA_SUCCESS :
                 PSA_ERROR_INVALID_SIGNATURE);
            status = esp_psa_verify_hash(key, alg,
                                     payload, payload_length,
    ...
    if (can_sign_or_verify_message(usage, alg)) {
        ...
        if (usage & PSA_KEY_USAGE_SIGN_MESSAGE) {
            status = esp_psa_sign_message(key, alg,
        ...
        if (usage & PSA_KEY_USAGE_VERIFY_MESSAGE) {
            psa_status_t verify_status =
                (usage & PSA_KEY_USAGE_SIGN_MESSAGE ?
                 PSA_SUCCESS :
                 PSA_ERROR_INVALID_SIGNATURE);
            status = esp_psa_verify_message(key, alg,
                                        message, message_length,
                                        signature, signature_length);
            if (key_destroyable && status == PSA_ERROR_INVALID_HANDLE) {
                /* The key has been destroyed. */
                                              ...
                                              int key_destroyable)
{
    ...
    psa_status_t status = PSA_SUCCESS;
    if (usage & PSA_KEY_USAGE_ENCRYPT) {
        status = esp_psa_asymmetric_encrypt(key, alg,
    ...
    if (usage & PSA_KEY_USAGE_DECRYPT) {
        status = esp_psa_asymmetric_decrypt(key, alg,
                                        ciphertext, ciphertext_length,
                                        NULL, 0,
                                        plaintext, sizeof(plaintext),
                                        &plaintext_length);
...
{
    PSA_ASSERT(esp_psa_key_derivation_setup(operation, alg));
    psa_status_t status = PSA_SUCCESS;
    if (PSA_ALG_IS_HKDF(alg)) {
        PSA_ASSERT(esp_psa_key_derivation_input_bytes(operation,
                                                  PSA_KEY_DERIVATION_INPUT_SALT,
                                                  input1, input1_length));
        status = esp_psa_key_derivation_input_key(operation,
        ...
        PSA_ASSERT(esp_psa_key_derivation_input_integer(operation,
    ...
    if (capacity != SIZE_MAX) {
        PSA_ASSERT(esp_psa_key_derivation_set_capacity(operation, capacity));
    }
                                       ...
                                       int key_destroyable)
{
    ...
    if (usage & PSA_KEY_USAGE_DERIVE) {
        ...
        psa_status_t status = esp_psa_key_derivation_output_bytes(&operation,
                                                              output,
                                                              capacity);
        if (key_destroyable && status == PSA_ERROR_BAD_STATE) {
            /* The key has been destroyed. */
            PSA_ASSERT(esp_psa_key_derivation_abort(&operation));
        } else {
            PSA_ASSERT(status);
            PSA_ASSERT(esp_psa_key_derivation_abort(&operation));
        }
    ...
    mbedtls_svc_key_id_t key, int key_destroyable)
{
    ...
    status = esp_psa_export_public_key(key, public_key, public_key_length,
                                   &public_key_length);
    if (key_destroyable && status == PSA_ERROR_INVALID_HANDLE) {
        /* The key has been destroyed. */
        status = PSA_SUCCESS;
    ...
    status = esp_psa_key_derivation_key_agreement(
    ...
    int key_destroyable)
{
    ...
    public_key_length = PSA_EXPORT_PUBLIC_KEY_OUTPUT_SIZE(public_key_type, key_bits);
    TEST_CALLOC(public_key, public_key_length);
    status = esp_psa_export_public_key(key,
                                   public_key, public_key_length,
                                   &public_key_length);
    if (key_destroyable && status == PSA_ERROR_INVALID_HANDLE) {
        /* The key has been destroyed. */
    ...
    status = esp_psa_raw_key_agreement(alg, key,
                                   public_key, public_key_length,
                                      ...
                                      int key_destroyable)
{
    ...
    if (usage & PSA_KEY_USAGE_DERIVE) {
        ...
        if (PSA_ALG_IS_TLS12_PRF(kdf_alg) ||
            ...
            PSA_ASSERT(esp_psa_key_derivation_input_bytes(
        ...
        PSA_ASSERT(esp_psa_key_derivation_output_bytes(&operation,
                                                   output,
                                                   sizeof(output)));
        PSA_ASSERT(esp_psa_key_derivation_abort(&operation));
    }
    ...
    const uint8_t *exported, size_t exported_length)
{
    ...
    if (type == PSA_KEY_TYPE_RSA_KEY_PAIR) {
         ...
         *   }
         */
        TEST_EQUAL(esp_mbedtls_asn1_get_tag(&p, end, &len,
                               ...
                               int key_destroyable)
{
    ...
    status = esp_psa_export_key(key, exported, exported_size, &exported_length);
    if (key_destroyable && status == PSA_ERROR_INVALID_HANDLE) {
        /* The key has been destroyed. */
        ok = 1;
        goto exit;
                                      ...
                                      int key_destroyable)
{
    ...
    if (!PSA_KEY_TYPE_IS_ASYMMETRIC(psa_get_key_type(&attributes))) {
        ...
        status = esp_psa_export_public_key(key, exported,
                                        ...
                                        const mbedtls_pk_context *pk)
{
    ...
    PSA_ASSERT(esp_psa_get_key_attributes(psa_key, &psa_attributes));
    ...
    TEST_EQUAL(psa_get_key_bits(&psa_attributes), esp_mbedtls_pk_get_bitlen(pk));
    ...
    switch (pk_type) {
        ...
        case MBEDTLS_PK_RSA:
            ...
            const mbedtls_rsa_context *rsa = esp_mbedtls_pk_rsa(*pk);
            uint8_t *const end = pk_public_buffer + sizeof(pk_public_buffer);
            uint8_t *cursor = end;
            TEST_LE_U(1, esp_mbedtls_rsa_write_pubkey(rsa,
        ...
        case MBEDTLS_PK_ECDSA:
            ...
            TEST_EQUAL(esp_mbedtls_ecp_write_public_key(
        ...
        case MBEDTLS_PK_OPAQUE:
            PSA_ASSERT(esp_psa_get_key_attributes(pk->priv_id, &pk_attributes));
            ...
            PSA_ASSERT(esp_psa_export_public_key(psa_key,
                                             pk_public_buffer,
...
exit:
    esp_psa_reset_key_attributes(&psa_attributes);
    esp_psa_reset_key_attributes(&pk_attributes);
    return ok;
}
#endif /* MBEDTLS_PK_C */
