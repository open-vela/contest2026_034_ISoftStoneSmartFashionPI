    ...
    mbedtls_test_handshake_test_options *opts)
{
    ...
    esp_mbedtls_ssl_cache_init(opts->cache);
#if defined(MBEDTLS_HAVE_TIME)
    TEST_EQUAL(esp_mbedtls_ssl_cache_get_timeout(opts->cache),
...
{
#if defined(MBEDTLS_SSL_CACHE_C)
    esp_mbedtls_ssl_cache_free(opts->cache);
    esp_mbedtls_free(opts->cache);
#else
    (void) opts;
#endif
}
...
int mbedtls_test_ssl_buffer_setup(mbedtls_test_ssl_buffer *buf,
                                  size_t capacity)
{
    buf->buffer = (unsigned char *) esp_mbedtls_calloc(capacity,
...
{
    queue->messages = (size_t *) esp_mbedtls_calloc(capacity, sizeof(size_t));
    ...
    mbedtls_test_ssl_message_queue *queue)
{
    ...
    if (queue->messages != NULL) {
        esp_mbedtls_free(queue->messages);
    }
                                     ...
                                     size_t bufsize)
{
    ...
    peer1->output =
        (mbedtls_test_ssl_buffer *) esp_mbedtls_calloc(
...
static void test_ssl_endpoint_certificate_free(mbedtls_test_ssl_endpoint *ep)
{
    ...
    if (cert != NULL) {
        if (cert->ca_cert != NULL) {
            esp_mbedtls_x509_crt_free(cert->ca_cert);
            esp_mbedtls_free(cert->ca_cert);
            cert->ca_cert = NULL;
        }
        if (cert->cert != NULL) {
            esp_mbedtls_x509_crt_free(cert->cert);
            esp_mbedtls_free(cert->cert);
            cert->cert = NULL;
        }
        if (cert->pkey != NULL) {
#if defined(MBEDTLS_USE_PSA_CRYPTO)
            if (esp_mbedtls_pk_get_type(cert->pkey) == MBEDTLS_PK_OPAQUE) {
                esp_psa_destroy_key(cert->pkey->priv_id);
            }
#endif
            esp_mbedtls_pk_free(cert->pkey);
            esp_mbedtls_free(cert->pkey);
            cert->pkey = NULL;
        }
    }
}
                                               ...
                                               int opaque_usage)
{
    ...
    esp_mbedtls_x509_crt_init(cert->ca_cert);
    esp_mbedtls_x509_crt_init(cert->cert);
    esp_mbedtls_pk_init(cert->pkey);

    /* Load the trusted CA */

    for (i = 0; mbedtls_test_cas_der[i] != NULL; i++) {
        ret = esp_mbedtls_x509_crt_parse_der(
            cert->ca_cert,
    ...
    if (ep->conf.endpoint == MBEDTLS_SSL_IS_SERVER) {
        if (pk_alg == MBEDTLS_PK_RSA) {
            ret = esp_mbedtls_x509_crt_parse(
                cert->cert,
                (const unsigned char *) mbedtls_test_srv_crt_rsa_sha256_der,
                mbedtls_test_srv_crt_rsa_sha256_der_len);
            TEST_ASSERT(ret == 0);

            ret = esp_mbedtls_pk_parse_key(
                cert->pkey,
    ...
    if (opaque_alg != 0) {
        ...
        TEST_EQUAL(esp_mbedtls_pk_get_psa_attributes(cert->pkey, PSA_KEY_USAGE_SIGN_HASH,
                                                 &key_attr), 0);
        ...
        TEST_EQUAL(esp_mbedtls_pk_import_into_psa(cert->pkey, &key_attr, &key_slot), 0);
        esp_mbedtls_pk_free(cert->pkey);
        esp_mbedtls_pk_init(cert->pkey);
        TEST_EQUAL(esp_mbedtls_pk_setup_opaque(cert->pkey, key_slot), 0);
    }
#else
    (void) opaque_alg;
    (void) opaque_alg2;
    ...
    esp_mbedtls_ssl_conf_ca_chain(&(ep->conf), cert->ca_cert, NULL);

    ret = esp_mbedtls_ssl_conf_own_cert(&(ep->conf), cert->cert,
                                    cert->pkey);
    TEST_ASSERT(ret == 0);
    TEST_ASSERT(ep->conf.key_cert != NULL);

    ret = esp_mbedtls_ssl_conf_own_cert(&(ep->conf), NULL, NULL);
    TEST_ASSERT(ret == 0);
    TEST_ASSERT(ep->conf.key_cert == NULL);
    ...
    mbedtls_test_ssl_message_queue *output_queue)
{
    ...
    esp_mbedtls_ssl_init(&(ep->ssl));
    esp_mbedtls_ssl_config_init(&(ep->conf));
    esp_mbedtls_ssl_conf_rng(&(ep->conf), mbedtls_test_random, NULL);

    TEST_ASSERT(esp_mbedtls_ssl_conf_get_user_data_p(&ep->conf) == NULL);
    TEST_EQUAL(esp_mbedtls_ssl_conf_get_user_data_n(&ep->conf), 0);
    TEST_ASSERT(esp_mbedtls_ssl_get_user_data_p(&ep->ssl) == NULL);
    TEST_EQUAL(esp_mbedtls_ssl_get_user_data_n(&ep->ssl), 0);

    (void) mbedtls_test_rnd_std_rand(NULL,
                                     (void *) &user_data_n,
                                     sizeof(user_data_n));
    esp_mbedtls_ssl_conf_set_user_data_n(&ep->conf, user_data_n);
    esp_mbedtls_ssl_set_user_data_n(&ep->ssl, user_data_n);
    ...
    if (dtls_context != NULL) {
        esp_mbedtls_ssl_set_bio(&(ep->ssl), dtls_context,
                            mbedtls_test_mock_tcp_send_msg,
                            mbedtls_test_mock_tcp_recv_msg,
                            NULL);
    } else {
        esp_mbedtls_ssl_set_bio(&(ep->ssl), &(ep->socket),
                            mbedtls_test_mock_tcp_send_nb,
                            mbedtls_test_mock_tcp_recv_nb,
                            NULL);
    }

    ret = esp_mbedtls_ssl_config_defaults(&(ep->conf), endpoint_type,
                                      (dtls_context != NULL) ?
    ...
    if (MBEDTLS_SSL_IS_CLIENT == endpoint_type) {
        if (options->client_min_version != MBEDTLS_SSL_VERSION_UNKNOWN) {
            esp_mbedtls_ssl_conf_min_tls_version(&(ep->conf),
                                             options->client_min_version);
        }

        if (options->client_max_version != MBEDTLS_SSL_VERSION_UNKNOWN) {
            esp_mbedtls_ssl_conf_max_tls_version(&(ep->conf),
    ...
    if (options->group_list != NULL) {
        esp_mbedtls_ssl_conf_groups(&(ep->conf), options->group_list);
    }

    esp_mbedtls_ssl_conf_authmode(&(ep->conf), MBEDTLS_SSL_VERIFY_REQUIRED);

#if defined(MBEDTLS_SSL_EARLY_DATA)
    esp_mbedtls_ssl_conf_early_data(&(ep->conf), options->early_data);
#if defined(MBEDTLS_SSL_SRV_C)
    if (endpoint_type == MBEDTLS_SSL_IS_SERVER &&
        (options->max_early_data_size >= 0)) {
        esp_mbedtls_ssl_conf_max_early_data_size(&(ep->conf),
    ...
    if (options->alpn_list[0] != NULL) {
        esp_mbedtls_ssl_conf_alpn_protocols(&(ep->conf), options->alpn_list);
    }
#endif
#endif
    ...
    if (endpoint_type == MBEDTLS_SSL_IS_SERVER && options->cache != NULL) {
        esp_mbedtls_ssl_conf_session_cache(&(ep->conf), options->cache,
                                       esp_mbedtls_ssl_cache_get,
                                       esp_mbedtls_ssl_cache_set);
    }
#endif

    ret = esp_mbedtls_ssl_setup(&(ep->ssl), &(ep->conf));
    TEST_ASSERT(ret == 0);

#if defined(MBEDTLS_SSL_PROTO_DTLS) && defined(MBEDTLS_SSL_SRV_C)
    if (endpoint_type == MBEDTLS_SSL_IS_SERVER && dtls_context != NULL) {
        esp_mbedtls_ssl_conf_dtls_cookies(&(ep->conf), NULL, NULL, NULL);
    }
#endif
    ...
    if (endpoint_type == MBEDTLS_SSL_IS_SERVER &&
        ...
        esp_mbedtls_ssl_conf_dbg(&(ep->conf), options->srv_log_fun,
                             options->srv_log_obj);
    }
#endif
    ...
    if (endpoint_type == MBEDTLS_SSL_IS_CLIENT &&
        ...
        esp_mbedtls_ssl_conf_dbg(&(ep->conf), options->cli_log_fun,
                             options->cli_log_obj);
    }
#endif
    ...
    TEST_EQUAL(esp_mbedtls_ssl_conf_get_user_data_n(&ep->conf), user_data_n);
    esp_mbedtls_ssl_conf_set_user_data_p(&ep->conf, ep);
    TEST_EQUAL(esp_mbedtls_ssl_get_user_data_n(&ep->ssl), user_data_n);
    esp_mbedtls_ssl_set_user_data_p(&ep->ssl, ep);
    ...
    mbedtls_test_message_socket_context *context)
{
    ...
    esp_mbedtls_ssl_free(&(ep->ssl));
    esp_mbedtls_ssl_config_free(&(ep->conf));
                                         ...
                                         int state)
{
    ...
    while ((ssl->state != state) && (--max_steps >= 0)) {
        ...
        if (!esp_mbedtls_ssl_is_handshake_over(second_ssl)) {
            ret = esp_mbedtls_ssl_handshake_step(second_ssl);
...
{
    int ret;
    /* Verify that calling esp_mbedtls_ssl_write with a NULL buffer and zero length is
     * a valid no-op for TLS connections. */
    if (ssl->conf->transport != MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
        TEST_ASSERT(esp_mbedtls_ssl_write(ssl, NULL, 0) == 0);
    }

    ret = esp_mbedtls_ssl_write(ssl, buf + *written, buf_len - *written);
    if (ret > 0) {
        *written += ret;
    }
                              ...
                              const int expected_fragments)
{
     ...
     * a valid no-op for TLS connections. */
    if (ssl->conf->transport != MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
        TEST_ASSERT(esp_mbedtls_ssl_read(ssl, NULL, 0) == 0);
    }

    ret = esp_mbedtls_ssl_read(ssl, buf + *read, buf_len - *read);
    if (ret > 0) {
        (*fragments)++;
        *read += ret;
    }
                            ...
                            int *forced_ciphersuite)
{
    ...
    forced_ciphersuite[0] = esp_mbedtls_ssl_get_ciphersuite_id(cipher);
    forced_ciphersuite[1] = 0;

    ciphersuite_info =
        esp_mbedtls_ssl_ciphersuite_from_id(forced_ciphersuite[0]);
    ...
    esp_mbedtls_ssl_conf_ciphersuites(conf, forced_ciphersuite);
                                           ...
                                           size_t *olen)
{
    ...
    status = esp_psa_cipher_set_iv(&cipher_op, iv, iv_len);

    if (status != PSA_SUCCESS) {
        return PSA_TO_MBEDTLS_ERR(status);
    }

    status = esp_psa_cipher_update(&cipher_op, input, ilen, output, ilen, olen);

    if (status != PSA_SUCCESS) {
        return PSA_TO_MBEDTLS_ERR(status);
    }

    status = esp_psa_cipher_finish(&cipher_op, output + *olen, ilen - *olen,
                               &part_len);
    ...
    *olen += part_len;
    return 0;
#else
    return esp_mbedtls_cipher_crypt(&transform->cipher_ctx_enc,
                                      ...
                                      size_t cid1_len)
{
     ...
     * a non-NULL pointers from `esp_mbedtls_calloc` even if
     * `keylen == 0` in the case of the NULL cipher. */
    CHK((key0 = esp_mbedtls_calloc(1, keylen + 1)) != NULL);
    CHK((key1 = esp_mbedtls_calloc(1, keylen + 1)) != NULL);
    memset(key0, 0x1, keylen);
    memset(key1, 0x2, keylen);

#if !defined(MBEDTLS_USE_PSA_CRYPTO)
    /* Pick cipher */
    cipher_info = esp_mbedtls_cipher_info_from_type((mbedtls_cipher_type_t) cipher_type);
    CHK(cipher_info != NULL);
    CHK(esp_mbedtls_cipher_info_get_iv_size(cipher_info) <= 16);
    CHK(esp_mbedtls_cipher_info_get_key_bitlen(cipher_info) % 8 == 0);

    /* Setup cipher contexts */
    CHK(esp_mbedtls_cipher_setup(&t_in->cipher_ctx_enc,  cipher_info) == 0);
    CHK(esp_mbedtls_cipher_setup(&t_in->cipher_ctx_dec,  cipher_info) == 0);
    CHK(esp_mbedtls_cipher_setup(&t_out->cipher_ctx_enc, cipher_info) == 0);
    CHK(esp_mbedtls_cipher_setup(&t_out->cipher_ctx_dec, cipher_info) == 0);

#if defined(MBEDTLS_CIPHER_MODE_CBC)
    if (cipher_mode == MBEDTLS_MODE_CBC) {
        CHK(esp_mbedtls_cipher_set_padding_mode(&t_in->cipher_ctx_enc,
                                            MBEDTLS_PADDING_NONE) == 0);
        CHK(esp_mbedtls_cipher_set_padding_mode(&t_in->cipher_ctx_dec,
                                            MBEDTLS_PADDING_NONE) == 0);
        CHK(esp_mbedtls_cipher_set_padding_mode(&t_out->cipher_ctx_enc,
                                            MBEDTLS_PADDING_NONE) == 0);
        CHK(esp_mbedtls_cipher_set_padding_mode(&t_out->cipher_ctx_dec,
                                            MBEDTLS_PADDING_NONE) == 0);
    }
#endif /* MBEDTLS_CIPHER_MODE_CBC */

    CHK(esp_mbedtls_cipher_setkey(&t_in->cipher_ctx_enc, key0,
                              (keylen << 3 > INT_MAX) ? INT_MAX : (int) keylen << 3,
                              MBEDTLS_ENCRYPT)
        == 0);
    CHK(esp_mbedtls_cipher_setkey(&t_in->cipher_ctx_dec, key1,
                              (keylen << 3 > INT_MAX) ? INT_MAX : (int) keylen << 3,
                              MBEDTLS_DECRYPT)
        == 0);
    CHK(esp_mbedtls_cipher_setkey(&t_out->cipher_ctx_enc, key1,
                              (keylen << 3 > INT_MAX) ? INT_MAX : (int) keylen << 3,
                              MBEDTLS_ENCRYPT)
        == 0);
    CHK(esp_mbedtls_cipher_setkey(&t_out->cipher_ctx_dec, key0,
                              (keylen << 3 > INT_MAX) ? INT_MAX : (int) keylen << 3,
                              MBEDTLS_DECRYPT)
        == 0);
#endif /* !MBEDTLS_USE_PSA_CRYPTO */
    ...
    if (cipher_mode == MBEDTLS_MODE_CBC ||
        cipher_mode == MBEDTLS_MODE_STREAM) {
#if !defined(MBEDTLS_USE_PSA_CRYPTO)
        mbedtls_md_info_t const *md_info = esp_mbedtls_md_info_from_type((mbedtls_md_type_t) hash_id);
        CHK(md_info != NULL);
#endif
        maclen = esp_mbedtls_md_get_size_from_type((mbedtls_md_type_t) hash_id);
        CHK(maclen != 0);
        /* Pick hash keys */
        CHK((md0 = esp_mbedtls_calloc(1, maclen)) != NULL);
        CHK((md1 = esp_mbedtls_calloc(1, maclen)) != NULL);
        memset(md0, 0x5, maclen);
        memset(md1, 0x6, maclen);

#if defined(MBEDTLS_USE_PSA_CRYPTO)
        alg = esp_mbedtls_md_psa_alg_from_type(hash_id);
        ...
        CHK(esp_psa_import_key(&attributes,
                           md0, maclen,
...
#else
        CHK(esp_mbedtls_md_setup(&t_out->md_ctx_enc, md_info, 1) == 0);
        CHK(esp_mbedtls_md_setup(&t_out->md_ctx_dec, md_info, 1) == 0);
        CHK(esp_mbedtls_md_setup(&t_in->md_ctx_enc,  md_info, 1) == 0);
        CHK(esp_mbedtls_md_setup(&t_in->md_ctx_dec,  md_info, 1) == 0);

        CHK(esp_mbedtls_md_hmac_starts(&t_in->md_ctx_enc,
                                   md0, maclen) == 0);
        CHK(esp_mbedtls_md_hmac_starts(&t_in->md_ctx_dec,
                                   md1, maclen) == 0);
        CHK(esp_mbedtls_md_hmac_starts(&t_out->md_ctx_enc,
                                   md1, maclen) == 0);
        CHK(esp_mbedtls_md_hmac_starts(&t_out->md_ctx_dec,
                                   md0, maclen) == 0);
#endif
    }
#else
...
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    status = esp_mbedtls_ssl_cipher_to_psa(cipher_type,
                                       t_in->taglen,
                                       &alg,
                                       &key_type,
                                       &key_bits);
...
cleanup:
    ...
    esp_mbedtls_free(key0);
    esp_mbedtls_free(key1);

    esp_mbedtls_free(md0);
    esp_mbedtls_free(md1);
                                        ...
                                        mbedtls_ssl_transform *transform_out)
{
    ...
    TEST_EQUAL(PSA_SUCCESS, esp_psa_mac_sign_setup(&operation,
                                               transform_out->psa_mac_enc,
                                               transform_out->psa_mac_alg));
    TEST_EQUAL(PSA_SUCCESS, esp_psa_mac_update(&operation, add_data, 13));
    TEST_EQUAL(PSA_SUCCESS, esp_psa_mac_update(&operation,
    ...
    TEST_EQUAL(PSA_SUCCESS, esp_psa_mac_sign_finish(&operation,
                                                mac, sizeof(mac),
                                                &sign_mac_length));
#else
    TEST_EQUAL(0, esp_mbedtls_md_hmac_update(&transform_out->md_ctx_enc, add_data, 13));
    TEST_EQUAL(0, esp_mbedtls_md_hmac_update(&transform_out->md_ctx_enc,
                                         record->buf + record->data_offset,
                                         record->data_len));
    /* Use a temporary buffer for the MAC, because with the truncated HMAC
     * extension, there might not be enough room in the record for the
    ...
    TEST_EQUAL(0, esp_mbedtls_md_hmac_finish(&transform_out->md_ctx_enc, mac));
#endif
                                            ...
                                            const char *crt_file)
{
...
#if defined(MBEDTLS_HAVE_TIME)
    session->start = esp_mbedtls_time(NULL) - 42;
#endif
    ...
    if (crt_file != NULL && strlen(crt_file) != 0) {
        ...
        esp_mbedtls_x509_crt_init(&tmp_crt);
        ret = esp_mbedtls_x509_crt_parse_file(&tmp_crt, crt_file);
        if (ret != 0) {
            return ret;
        }
        ...
        /* Move temporary CRT. */
        session->peer_cert = esp_mbedtls_calloc(1, sizeof(*session->peer_cert));
        ...
        *session->peer_cert = tmp_crt;
...
#if defined(MBEDTLS_USE_PSA_CRYPTO)
        psa_algorithm_t psa_alg = esp_mbedtls_md_psa_alg_from_type(
            MBEDTLS_SSL_PEER_CERT_DIGEST_DFL_TYPE);
        size_t hash_size = 0;
        psa_status_t status = esp_psa_hash_compute(
        ...
        esp_mbedtls_x509_crt_free(&tmp_crt);
    }
...
#if defined(MBEDTLS_SSL_CLI_C)
    if (ticket_len != 0) {
        session->ticket = esp_mbedtls_calloc(1, ticket_len);
    ...
    if (session->endpoint == MBEDTLS_SSL_IS_SERVER) {
        session->ticket_creation_time = esp_mbedtls_ms_time() - 42;
    }
#endif
                                            ...
                                            int endpoint_type)
{
    ...
    int ret = esp_mbedtls_ssl_session_set_ticket_alpn(session, "ALPNExample");
    if (ret != 0) {
        return -1;
    }
...
#if defined(MBEDTLS_HAVE_TIME)
        session->ticket_reception_time = esp_mbedtls_ms_time() - 40;
#endif
...
{
    const char *version_string = esp_mbedtls_ssl_get_version(ssl);
    mbedtls_ssl_protocol_version version_number =
        esp_mbedtls_ssl_get_version_number(ssl);
    ...
    mbedtls_test_handshake_test_options *options)
{
    ...
    MD_OR_USE_PSA_INIT();
    esp_mbedtls_platform_zeroize(&client, sizeof(client));
    esp_mbedtls_platform_zeroize(&server, sizeof(server));
    ...
    if (options->cli_log_fun || options->srv_log_fun) {
        esp_mbedtls_debug_set_threshold(4);
    }
#endif
    ...
    if (options->dtls != 0) {
        TEST_ASSERT(mbedtls_test_ssl_endpoint_init(&client,
                                                   ...
                                                   &server_queue) == 0);
#if defined(MBEDTLS_TIMING_C)
        esp_mbedtls_ssl_set_timer_cb(&client.ssl, &timer_client,
    ...
    if (options->dtls != 0) {
        TEST_ASSERT(mbedtls_test_ssl_endpoint_init(&server,
                                                   ...
                                                   &client_queue) == 0);
#if defined(MBEDTLS_TIMING_C)
        esp_mbedtls_ssl_set_timer_cb(&server.ssl, &timer_server,
                                 esp_mbedtls_timing_set_delay,
                                 esp_mbedtls_timing_get_delay);
#endif
    } else {
    ...
    esp_mbedtls_ssl_conf_authmode(&server.conf, options->srv_auth_mode);

#if defined(MBEDTLS_SSL_MAX_FRAGMENT_LENGTH)
    TEST_ASSERT(esp_mbedtls_ssl_conf_max_frag_len(&(server.conf),
                                              (unsigned char) options->mfl)
                == 0);
    TEST_ASSERT(esp_mbedtls_ssl_conf_max_frag_len(&(client.conf),
                                              (unsigned char) options->mfl)
                == 0);
#else
    ...
    if (options->psk_str != NULL && options->psk_str->len > 0) {
        TEST_ASSERT(esp_mbedtls_ssl_conf_psk(
        ...
        TEST_ASSERT(esp_mbedtls_ssl_conf_psk(
                        ...
                        strlen(psk_identity)) == 0);
#if defined(MBEDTLS_SSL_SRV_C)
        esp_mbedtls_ssl_conf_psk_cb(&server.conf, psk_dummy_callback, NULL);
#endif
    }
#endif
#if defined(MBEDTLS_SSL_RENEGOTIATION)
    if (options->renegotiate) {
        esp_mbedtls_ssl_conf_renegotiation(&(server.conf),
                                       MBEDTLS_SSL_RENEGOTIATION_ENABLED);
        esp_mbedtls_ssl_conf_renegotiation(&(client.conf),
                                       MBEDTLS_SSL_RENEGOTIATION_ENABLED);

        esp_mbedtls_ssl_conf_legacy_renegotiation(&(server.conf),
                                              options->legacy_renegotiation);
        esp_mbedtls_ssl_conf_legacy_renegotiation(&(client.conf),
                                              options->legacy_renegotiation);
    }
    ...
    TEST_ASSERT(esp_mbedtls_ssl_is_handshake_over(&client.ssl) == 1);
    ...
    TEST_ASSERT(esp_mbedtls_ssl_is_handshake_over(&server.ssl) == 1);
    ...
    if (options->serialize == 1) {
        ...
        TEST_ASSERT(esp_mbedtls_ssl_context_save(&(server.ssl), NULL,
                                             0, &context_buf_len)
                    == MBEDTLS_ERR_SSL_BUFFER_TOO_SMALL);

        context_buf = esp_mbedtls_calloc(1, context_buf_len);
        TEST_ASSERT(context_buf != NULL);

        TEST_ASSERT(esp_mbedtls_ssl_context_save(&(server.ssl), context_buf,
                                             context_buf_len,
                                             &context_buf_len)
                    == 0);

        esp_mbedtls_ssl_free(&(server.ssl));
        esp_mbedtls_ssl_init(&(server.ssl));

        TEST_ASSERT(esp_mbedtls_ssl_setup(&(server.ssl), &(server.conf)) == 0);

        esp_mbedtls_ssl_set_bio(&(server.ssl), &server_context,
                            mbedtls_test_mock_tcp_send_msg,
                            mbedtls_test_mock_tcp_recv_msg,
                            NULL);

        esp_mbedtls_ssl_set_user_data_p(&server.ssl, &server);
...
#endif
        TEST_ASSERT(esp_mbedtls_ssl_context_load(&(server.ssl), context_buf,
                                             context_buf_len) == 0);
    ...
    if (options->renegotiate) {
        ...
        TEST_ASSERT(esp_mbedtls_ssl_renegotiate(&(server.ssl)) == 0);
        TEST_ASSERT(server.ssl.renego_status ==
        ...
        /* After calling esp_mbedtls_ssl_renegotiate for the client,
        ...
        ret = esp_mbedtls_ssl_renegotiate(&(client.ssl));
#if defined(MBEDTLS_SSL_VARIABLE_BUFFER_LENGTH)
        if (options->resize_buffers != 0) {
            /* Ensure that the buffer sizes are appropriate before resizes */
            TEST_ASSERT(client.ssl.out_buf_len == MBEDTLS_SSL_OUT_BUFFER_LEN);
    ...
    TEST_ASSERT(esp_mbedtls_ssl_conf_get_user_data_p(&client.conf) == &client);
    TEST_ASSERT(esp_mbedtls_ssl_get_user_data_p(&client.ssl) == &client);
    TEST_ASSERT(esp_mbedtls_ssl_conf_get_user_data_p(&server.conf) == &server);
    TEST_ASSERT(esp_mbedtls_ssl_get_user_data_p(&server.ssl) == &server);
...
#if defined(MBEDTLS_DEBUG_C)
    if (options->cli_log_fun || options->srv_log_fun) {
        esp_mbedtls_debug_set_threshold(0);
    }
#endif
#if defined(MBEDTLS_SSL_CONTEXT_SERIALIZATION)
    if (context_buf != NULL) {
 ...
 * write/parse ticket interfaces as defined by esp_mbedtls_ssl_ticket_write/parse_t.
    ...
    size_t *tlen, uint32_t *lifetime)
{
    ...
    if ((ret = esp_mbedtls_ssl_session_save(session, start, end - start,
                                        tlen)) != 0) {
        return ret;
    }
                              ...
                              unsigned char *buf, size_t len)
{
    ...
    return esp_mbedtls_ssl_session_load(session, buf, len);
}
#endif /* MBEDTLS_SSL_SESSION_TICKETS */
    ...
    mbedtls_ssl_session *session)
{
    ...
    esp_mbedtls_platform_zeroize(&client_ep, sizeof(client_ep));
    esp_mbedtls_platform_zeroize(&server_ep, sizeof(server_ep));
    ...
    esp_mbedtls_ssl_conf_session_tickets_cb(&server_ep.conf,
    ...
    do {
        ret = esp_mbedtls_ssl_read(&(client_ep.ssl), buf, sizeof(buf));
    } while (ret != MBEDTLS_ERR_SSL_RECEIVED_NEW_SESSION_TICKET);

    ret = esp_mbedtls_ssl_get_session(&(client_ep.ssl), session);
    TEST_EQUAL(ret, 0);
