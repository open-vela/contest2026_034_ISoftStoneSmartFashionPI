...
#ifdef MBEDTLS_THREADING_C
    esp_mbedtls_mutex_lock(&mbedtls_test_info_mutex);
#endif /* MBEDTLS_THREADING_C */

    result =  mbedtls_test_info.result;

#ifdef MBEDTLS_THREADING_C
    esp_mbedtls_mutex_unlock(&mbedtls_test_info_mutex);
#endif /* MBEDTLS_THREADING_C */
...
int mbedtls_test_platform_setup(void)
{
     ...
     * esp_psa_crypto_init() will fail. This is not necessary for test suites
...
#if defined(MBEDTLS_PLATFORM_C)
    ret = esp_mbedtls_platform_setup(&platform_ctx);
#endif /* MBEDTLS_PLATFORM_C */

#ifdef MBEDTLS_THREADING_C
    esp_mbedtls_mutex_init(&mbedtls_test_info_mutex);
#endif /* MBEDTLS_THREADING_C */
...
#endif
#ifdef MBEDTLS_THREADING_C
    esp_mbedtls_mutex_free(&mbedtls_test_info_mutex);
#endif /* MBEDTLS_THREADING_C */

#if defined(MBEDTLS_PLATFORM_C)
    esp_mbedtls_platform_teardown(&platform_ctx);
#endif /* MBEDTLS_PLATFORM_C */
}
                       ...
                       unsigned long long value1, unsigned long long value2)
{
    ...
    if (mbedtls_test_info.result != MBEDTLS_TEST_RESULT_FAILED) {
        ...
        mbedtls_test_fail_internal(test, line_no, filename);
        (void) esp_mbedtls_snprintf(buf, sizeof(buf),
                                "lhs = 0x%016llx = %lld",
...
unsigned char *mbedtls_test_zero_alloc(size_t len)
{
    ...
    p = esp_mbedtls_calloc(1, actual_len);
...
unsigned char *mbedtls_test_unhexify_alloc(const char *ibuf, size_t *olen)
{
    ...
    *olen = strlen(ibuf) / 2;
    ...
    obuf = esp_mbedtls_calloc(1, *olen);
