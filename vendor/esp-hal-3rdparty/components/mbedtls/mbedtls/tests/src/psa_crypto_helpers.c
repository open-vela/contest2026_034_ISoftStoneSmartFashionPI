...
void mbedtls_test_psa_purge_key_storage(void)
{
    size_t i;
    for (i = 0; i < num_key_ids_used; i++) {
        esp_psa_destroy_persistent_key(key_ids_used_in_test[i]);
    }
    num_key_ids_used = 0;
}
...
void mbedtls_test_psa_purge_key_cache(void)
{
    size_t i;
    for (i = 0; i < num_key_ids_used; i++) {
        esp_psa_purge_key(key_ids_used_in_test[i]);
    }
}
...
const char *mbedtls_test_helper_is_psa_leaking(void)
{
    ...
    esp_mbedtls_psa_get_stats(&stats);
...
{
    size_t actual_len = 0;
    psa_status_t status = esp_psa_its_get(PSA_CRYPTO_ITS_RANDOM_SEED_UID,
...
{
    psa_status_t status = esp_psa_its_set(PSA_CRYPTO_ITS_RANDOM_SEED_UID,
                                      len, buf, 0);
    if (status != 0) {
        return MBEDTLS_ERR_ENTROPY_FILE_IO_ERROR;
    }
...
int mbedtls_test_inject_entropy_restore(void)
{
    ...
    psa_status_t status = esp_mbedtls_psa_inject_entropy(buf, sizeof(buf));
