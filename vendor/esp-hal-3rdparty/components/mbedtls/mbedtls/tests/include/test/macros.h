 ...
 * put `esp_mbedtls_free(pointer)` in the test's cleanup code.
 *
...
#define TEST_CALLOC(pointer, item_count)                    \
    do {                                                    \
        ...
        if ((item_count) != 0) {                            \
            (pointer) = esp_mbedtls_calloc((item_count),        \
                                       sizeof(*(pointer))); \
            TEST_ASSERT((pointer) != NULL);                 \
        }                                                   \
    } while (0)
 ...
 *                   This expression may be evaluated multiple times.
 *
 * Note: if passing size 0, esp_mbedtls_calloc may return NULL. In this case,
...
#define TEST_CALLOC_NONNULL(pointer, item_count)            \
    do {                                                    \
        ...
        if (((pointer) == NULL) && ((item_count) == 0)) {   \
            (pointer) = esp_mbedtls_calloc(1, 1);               \
        }                                                   \
        TEST_ASSERT((pointer) != NULL);                     \
    } while (0)
...
#define TEST_HELPER_ASSERT(a) if (!(a))                          \
    {                                                                   \
        esp_mbedtls_fprintf(stderr, "Assertion Failed at %s:%d - %s\n",    \
                        __FILE__, __LINE__, #a);              \
        esp_mbedtls_exit(1);                                              \
    }
