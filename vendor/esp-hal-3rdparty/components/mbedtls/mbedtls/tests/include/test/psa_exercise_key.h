 ...
 * using esp_psa_raw_key_agreement().
 *
 ...
 * \p esp_psa_export_key correctly rejects the attempt. If the key is
 * asymmetric, also check \p esp_psa_export_public_key.
 *
 ...
 * to 1, while another thread calls esp_psa_destroy_key on the same key;
