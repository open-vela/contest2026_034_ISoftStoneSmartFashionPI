 ...
 *
 * The limb array is allocated with esp_mbedtls_calloc() and must later be
 * freed with esp_mbedtls_free().
 *
 ...
 *
 * The limb array is allocated with esp_mbedtls_calloc() and must later be
 * freed with esp_mbedtls_free(). You can do that by calling
 ...
 *
 * Like esp_mbedtls_mpi_read_string(), but with tighter guarantees around
 * edge cases.
 *
