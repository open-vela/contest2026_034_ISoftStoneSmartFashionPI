...
#if defined(MBEDTLS_TEST_HOOKS)
extern void (*esp_mbedtls_test_hook_test_fail)(const char *test, int line, const char *file);
#define MBEDTLS_TEST_HOOK_TEST_ASSERT(TEST) \
    do { \
        if ((!(TEST)) && ((*esp_mbedtls_test_hook_test_fail) != NULL)) \
        { \
            (*esp_mbedtls_test_hook_test_fail)( #TEST, __LINE__, __FILE__); \
        } \
    } while (0)
#else
#define MBEDTLS_TEST_HOOK_TEST_ASSERT(TEST)
 ...
 *              \c esp_mbedtls_platform_zeroize() and \c esp_mbedtls_free(), but has
 ...
 */
void esp_mbedtls_zeroize_and_free(void *buf, size_t len);
...
/* Always inline esp_mbedtls_xor() for similar reasons as esp_mbedtls_xor_no_simd(). */
 ...
 *
 * \note      Depending on the situation, it may be faster to use either esp_mbedtls_xor() or
 *            esp_mbedtls_xor_no_simd() (these are functionally equivalent).
 ...
 *            registers, and in this case, esp_mbedtls_xor_no_simd() may be faster. In other cases where
 *            the result is not used immediately (e.g., in AES-CTR), esp_mbedtls_xor() may be faster.
 *            For targets without SIMD support, they will behave the same.
 */
static inline void esp_mbedtls_xor(unsigned char *r,
                               ...
                               size_t n)
{
    ...
    for (; (i + 8) <= n; i += 8) {
        uint64_t x = esp_mbedtls_get_unaligned_uint64(a + i) ^ esp_mbedtls_get_unaligned_uint64(b + i);
        esp_mbedtls_put_unaligned_uint64(r + i, x);
    }
#if defined(__IAR_SYSTEMS_ICC__)
    if (n % 8 == 0) {
        return;
...
/* Always inline esp_mbedtls_xor_no_simd() as we see significant perf regressions when it does not get
 * inlined (e.g., observed about 3x perf difference in esp_gcm_mult_largetable with gcc 7 - 12) */
#if defined(__IAR_SYSTEMS_ICC__)
#pragma inline = forced
#elif defined(__GNUC__)
__attribute__((always_inline))
 ...
 * r[i] = a[i] ^ b[i] where 0 <= i < n
 *
 * In some situations, this can perform better than esp_mbedtls_xor() (e.g., it's about 5%
 * better in AES-CBC).
 *
 * \param   r Pointer to result (buffer of at least \p n bytes). \p r
 *            may be equal to either \p a or \p b, but behaviour when
 ...
 *            For targets without SIMD support, they will behave the same.
 */
static inline void esp_mbedtls_xor_no_simd(unsigned char *r,
                                       const unsigned char *a,
                                       const unsigned char *b,
                                       size_t n)
{
