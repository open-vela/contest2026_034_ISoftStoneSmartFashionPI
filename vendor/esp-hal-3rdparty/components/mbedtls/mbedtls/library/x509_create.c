...
static const x509_attr_descriptor_t esp_x509_attrs[] =
{
    { ADD_STRLEN("CN"),
...
static const x509_attr_descriptor_t *esp_x509_attr_descr_from_name(const char *name, size_t name_len)
{
    const x509_attr_descriptor_t *cur;

    for (cur = esp_x509_attrs; cur->name != NULL; cur++) {
...
static int esp_hex_to_int(char c)
{
    return ('0' <= c && c <= '9') ? (c - '0') :
           ('a' <= c && c <= 'f') ? (c - 'a' + 10) :
           ('A' <= c && c <= 'F') ? (c - 'A' + 10) : -1;
...
static int esp_hexpair_to_int(const char *hexpair)
{
    int n1 = esp_hex_to_int(*hexpair);
    int n2 = esp_hex_to_int(*(hexpair + 1));
...
static int esp_parse_attribute_value_string(const char *s,
                                        int len,
                                        unsigned char *data,
                                        size_t *data_len)
{
    ...
    for (c = s; c < end; c++) {
        if (*c == '\\') {
            ...
            if (c + 1 < end && (n = esp_hexpair_to_int(c)) != -1) {
                if (n == 0) {
 ...
 */
static int esp_parse_attribute_value_hex_der_encoded(const char *s,
                                                 size_t len,
                                                 unsigned char *data,
                                                 size_t data_size,
                                                 size_t *data_len,
    ...
    *tag = der[0];
    {
        ...
        if (esp_mbedtls_asn1_get_len(&p, der + der_length, data_len) != 0) {
            goto error;
        }
...
int esp_mbedtls_x509_string_to_names(mbedtls_asn1_named_data **head, const char *name)
{
    ...
    esp_mbedtls_asn1_free_named_data_list(head);

    while (c <= end) {
        if (in_attr_type && *c == '=') {
            if ((attr_descr = esp_x509_attr_descr_from_name(s, (size_t) (c - s))) == NULL) {
                if ((esp_mbedtls_oid_from_numeric_string(&oid, s, (size_t) (c - s))) != 0) {
                    return MBEDTLS_ERR_X509_INVALID_NAME;
                } else {
                    numericoid = 1;
                }
                ...
                oid.p = esp_mbedtls_calloc(1, oid.len);
        ...
        if (!in_attr_type && ((*c == ',' && *(c-1) != '\\') || c == end)) {
            if (s == c) {
                esp_mbedtls_free(oid.p);
                return MBEDTLS_ERR_X509_INVALID_NAME;
            } else if (*s == '#') {
                /* We know that c >= s (loop invariant) and c != s (in this
                 * else branch), hence c - s - 1 >= 0. */
                parse_ret = esp_parse_attribute_value_hex_der_encoded(
                    s + 1, (size_t) (c - s) - 1,
                ...
                if (numericoid) {
                    ...
                    if ((parse_ret =
                             esp_parse_attribute_value_string(s, (int) (c - s), data,
                                                          &data_len)) != 0) {
                        esp_mbedtls_free(oid.p);
                        return parse_ret;
                    }
            ...
            mbedtls_asn1_named_data *cur =
                esp_mbedtls_asn1_store_named_data(head, (char *) oid.p, oid.len,
                                              (unsigned char *) data,
                                              data_len);
            esp_mbedtls_free(oid.p);
            oid.p = NULL;
 ...
 */
int esp_mbedtls_x509_set_extension(mbedtls_asn1_named_data **head, const char *oid, size_t oid_len,
                               int critical, const unsigned char *val, size_t val_len)
{
    ...
    if ((cur = esp_mbedtls_asn1_store_named_data(head, oid, oid_len,
                                             NULL, val_len + 1)) == NULL) {
        return MBEDTLS_ERR_X509_ALLOC_FAILED;
    }
 ...
 */
static int esp_x509_write_name(unsigned char **p,
                           unsigned char *start,
                           mbedtls_asn1_named_data *cur_name)
{
    ...
    MBEDTLS_ASN1_CHK_ADD(len, esp_mbedtls_asn1_write_tagged_string(p, start,
                                                               cur_name->val.tag,
                                                               (const char *) name,
                                                               name_len));
    // Write OID
    //
    MBEDTLS_ASN1_CHK_ADD(len, esp_mbedtls_asn1_write_oid(p, start, oid,
                                                     oid_len));
...
int esp_mbedtls_x509_write_names(unsigned char **p, unsigned char *start,
                             mbedtls_asn1_named_data *first)
{
    ...
    while (cur != NULL) {
        MBEDTLS_ASN1_CHK_ADD(len, esp_x509_write_name(p, start, cur));
        cur = cur->next;
    }
...
int esp_mbedtls_x509_write_sig(unsigned char **p, unsigned char *start,
                           ...
                           mbedtls_pk_type_t pk_alg)
{
    ...
    *--(*p) = 0;
    ...
    MBEDTLS_ASN1_CHK_ADD(len, esp_mbedtls_asn1_write_len(p, start, len));
    MBEDTLS_ASN1_CHK_ADD(len, esp_mbedtls_asn1_write_tag(p, start, MBEDTLS_ASN1_BIT_STRING));
    ...
    MBEDTLS_ASN1_CHK_ADD(len,
                         esp_mbedtls_asn1_write_algorithm_identifier_ext(p, start, oid, oid_len,
                                                                     0, write_null_par));

    return (int) len;
}

static int esp_x509_write_extension(unsigned char **p, unsigned char *start,
                                mbedtls_asn1_named_data *ext)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    size_t len = 0;

    MBEDTLS_ASN1_CHK_ADD(len, esp_mbedtls_asn1_write_raw_buffer(p, start, ext->val.p + 1,
                                                            ext->val.len - 1));
    MBEDTLS_ASN1_CHK_ADD(len, esp_mbedtls_asn1_write_len(p, start, ext->val.len - 1));
    MBEDTLS_ASN1_CHK_ADD(len, esp_mbedtls_asn1_write_tag(p, start, MBEDTLS_ASN1_OCTET_STRING));

    if (ext->val.p[0] != 0) {
        MBEDTLS_ASN1_CHK_ADD(len, esp_mbedtls_asn1_write_bool(p, start, 1));
    }

    MBEDTLS_ASN1_CHK_ADD(len, esp_mbedtls_asn1_write_raw_buffer(p, start, ext->oid.p,
                                                            ext->oid.len));
    MBEDTLS_ASN1_CHK_ADD(len, esp_mbedtls_asn1_write_len(p, start, ext->oid.len));
    MBEDTLS_ASN1_CHK_ADD(len, esp_mbedtls_asn1_write_tag(p, start, MBEDTLS_ASN1_OID));
 ...
 *     }
 */
int esp_mbedtls_x509_write_extensions(unsigned char **p, unsigned char *start,
                                  mbedtls_asn1_named_data *first)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    size_t len = 0;
    ...
    while (cur_ext != NULL) {
        MBEDTLS_ASN1_CHK_ADD(len, esp_x509_write_extension(p, start, cur_ext));
