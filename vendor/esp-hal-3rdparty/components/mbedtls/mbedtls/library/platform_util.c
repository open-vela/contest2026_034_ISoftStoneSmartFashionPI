 ...
 *
 * This implementation for esp_mbedtls_platform_zeroize() was inspired from Colin
 ...
 *
 * if (esp_memset_func != memset)
 *     esp_memset_func(buf, 0, len);
 *
 * Note that it is extremely difficult to guarantee that
 * the memset() call will not be optimized out by aggressive compilers
 * in a portable way. For this reason, Mbed TLS also provides the configuration
 * option MBEDTLS_PLATFORM_ZEROIZE_ALT, which allows users to configure
 * esp_mbedtls_platform_zeroize() to use a suitable implementation for their
 * platform and needs.
 */
...
void esp_mbedtls_platform_zeroize(void *buf, size_t len)
{
    if (len > 0) {
...
{
    if (buf != NULL) {
        esp_mbedtls_platform_zeroize(buf, len);
    }
...
struct tm *esp_mbedtls_platform_gmtime_r(const mbedtls_time_t *tt,
                                     struct tm *tm_buf)
{
    ...
    if (esp_mbedtls_mutex_lock(&mbedtls_threading_gmtime_mutex) != 0) {
        return NULL;
    }
...
#if defined(MBEDTLS_THREADING_C)
    if (esp_mbedtls_mutex_unlock(&mbedtls_threading_gmtime_mutex) != 0) {
        return NULL;
    }
#endif /* MBEDTLS_THREADING_C */
...
void (*esp_mbedtls_test_hook_test_fail)(const char *, int, const char *);
...
#else
#error "No esp_mbedtls_ms_time available"
#endif
