 ...
 * the implementation").
 */
volatile mbedtls_ct_uint_t esp_mbedtls_ct_zero = 0;
#endif
...
static inline uint32_t esp_mbedtls_get_unaligned_volatile_uint32(volatile const unsigned char *p)
{
...
#else
#error "No assembly defined for esp_mbedtls_get_unaligned_volatile_uint32"
#endif
    return r;
}
                      ...
                      size_t n)
{
    ...
    for (; (i + 4) <= n; i += 4) {
        uint32_t x = esp_mbedtls_get_unaligned_volatile_uint32(A + i);
        uint32_t y = esp_mbedtls_get_unaligned_volatile_uint32(B + i);
        diff |= x ^ y;
    }
#endif
...
#error "esp_mbedtls_ct_memcmp() requires minimum 32-bit ints"
#else
                              ...
                              size_t skip_tail)
{
    ...
    for (size_t i = 0; i < n; i++) {
        unsigned char x = A[i], y = B[i];
        unsigned int d = x ^ y;
        mbedtls_ct_condition_t valid = esp_mbedtls_ct_bool_and(esp_mbedtls_ct_uint_ge(i, skip_head),
                                                           esp_mbedtls_ct_uint_lt(i, valid_end));
        diff |= esp_mbedtls_ct_uint_if_else_0(valid, d);
    }
...
void esp_mbedtls_ct_memmove_left(void *start, size_t total, size_t offset)
{
    volatile unsigned char *buf = start;
    for (size_t i = 0; i < total; i++) {
        mbedtls_ct_condition_t no_op = esp_mbedtls_ct_uint_gt(total - offset, i);
        ...
        for (size_t n = 0; n < total - 1; n++) {
            ...
            buf[n] = esp_mbedtls_ct_uint_if(no_op, current, next);
        }
        buf[total-1] = esp_mbedtls_ct_uint_if_else_0(no_op, buf[total-1]);
    }
}
                          ...
                          size_t len)
{
    ...
    const uint64_t not_mask = (uint64_t) ~esp_mbedtls_ct_compiler_opaque(condition);
#else
    const uint32_t mask     = (uint32_t) condition;
    const uint32_t not_mask = (uint32_t) ~esp_mbedtls_ct_compiler_opaque(condition);
#endif
...
#if defined(MBEDTLS_CT_SIZE_64)
    for (; (i + 8) <= len; i += 8) {
        uint64_t a = esp_mbedtls_get_unaligned_uint64(src1 + i) & mask;
        uint64_t b = esp_mbedtls_get_unaligned_uint64(src2 + i) & not_mask;
        esp_mbedtls_put_unaligned_uint64(dest + i, a | b);
    }
#else
    for (; (i + 4) <= len; i += 4) {
        uint32_t a = esp_mbedtls_get_unaligned_uint32(src1 + i) & mask;
        uint32_t b = esp_mbedtls_get_unaligned_uint32(src2 + i) & not_mask;
                              ...
                              size_t len)
{
    ...
    for (offsetval = offset_min; offsetval <= offset_max; offsetval++) {
        esp_mbedtls_ct_memcpy_if(esp_mbedtls_ct_uint_eq(offsetval, offset), dest, src + offsetval, NULL,
                             len);
    }
}
...
#if defined(MBEDTLS_EFFICIENT_UNALIGNED_ACCESS)
    for (; (i + 4) <= len; i += 4) {
        esp_mbedtls_put_unaligned_uint32((void *) (p + i),
                                     esp_mbedtls_get_unaligned_uint32((void *) (p + i)) & mask);
    }
#endif
    for (; i < len; i++) {
        p[i] = p[i] & mask;
