...
/* armcc5 --gnu defines __GNUC__ but doesn't support GNU's extended asm */
    ...
    __ARM_ARCH >= 6
static inline uint32_t esp_aria_p1(uint32_t x)
{
    uint32_t r;
    __asm("rev16 %0, %1" : "=l" (r) : "l" (x));
    return r;
}
#define ARIA_P1 esp_aria_p1
 ...
 */
static inline void esp_aria_a(uint32_t *a, uint32_t *b,
 ...
 */
static inline void esp_aria_sl(uint32_t *a, uint32_t *b,
                           uint32_t *c, uint32_t *d,
 ...
 * S-Boxes
 */
static const uint8_t esp_aria_sb1[256] =
{
...
static const uint8_t esp_aria_sb2[256] =
{
...
static const uint8_t esp_aria_is1[256] =
{
...
static const uint8_t esp_aria_is2[256] =
{
 ...
 */
static void esp_aria_fo_xor(uint32_t r[4], const uint32_t p[4],
                        const uint32_t k[4], const uint32_t x[4])
{
    ...
    esp_aria_sl(&a, &b, &c, &d, esp_aria_sb1, esp_aria_sb2, esp_aria_is1, esp_aria_is2);
    esp_aria_a(&a, &b, &c, &d);
 ...
 */
static void esp_aria_fe_xor(uint32_t r[4], const uint32_t p[4],
                        const uint32_t k[4], const uint32_t x[4])
{
    uint32_t a, b, c, d;
    ...
    esp_aria_sl(&a, &b, &c, &d, esp_aria_is1, esp_aria_is2, esp_aria_sb1, esp_aria_sb2);
    esp_aria_a(&a, &b, &c, &d);
 ...
 * bytes here.
 */
static void esp_aria_rot128(uint32_t r[4], const uint32_t a[4],
 ...
 * Set encryption key
 */
int esp_mbedtls_aria_setkey_enc(mbedtls_aria_context *ctx,
                            const unsigned char *key, unsigned int keybits)
{
    ...
    esp_aria_fo_xor(w[1], w[0], rc[i], w[1]);   // W1 = FO(W0, CK1) ^ KR
    i = i < 2 ? i + 1 : 0;
    esp_aria_fe_xor(w[2], w[1], rc[i], w[0]);   // W2 = FE(W1, CK2) ^ W0
    i = i < 2 ? i + 1 : 0;
    esp_aria_fo_xor(w[3], w[2], rc[i], w[1]);   // W3 = FO(W2, CK3) ^ W1

    for (i = 0; i < 4; i++) {               // create round keys
        w2 = w[(i + 1) & 3];
        esp_aria_rot128(ctx->rk[i], w[i], w2, 128 - 19);
        esp_aria_rot128(ctx->rk[i +  4], w[i], w2, 128 - 31);
        esp_aria_rot128(ctx->rk[i +  8], w[i], w2,       61);
        esp_aria_rot128(ctx->rk[i + 12], w[i], w2,       31);
    }
    esp_aria_rot128(ctx->rk[16], w[0], w[1], 19);

    /* w holds enough info to reconstruct the round keys */
    esp_mbedtls_platform_zeroize(w, sizeof(w));
...
int esp_mbedtls_aria_setkey_dec(mbedtls_aria_context *ctx,
                            const unsigned char *key, unsigned int keybits)
{
    int i, j, k, ret;

    ret = esp_mbedtls_aria_setkey_enc(ctx, key, keybits);
    if (ret != 0) {
        return ret;
    }
    ...
    for (i = 1; i < ctx->nr; i++) {
        esp_aria_a(&ctx->rk[i][0], &ctx->rk[i][1],
 ...
 * Encrypt a block
 */
int esp_mbedtls_aria_crypt_ecb(mbedtls_aria_context *ctx,
...
/* Initialize context */
void esp_mbedtls_aria_init(mbedtls_aria_context *ctx)
{
    memset(ctx, 0, sizeof(mbedtls_aria_context));
}

/* Clear context */
void esp_mbedtls_aria_free(mbedtls_aria_context *ctx)
{
    if (ctx == NULL) {
        return;
    }

    esp_mbedtls_platform_zeroize(ctx, sizeof(mbedtls_aria_context));
}
 ...
 */
int esp_mbedtls_aria_crypt_cbc(mbedtls_aria_context *ctx,
                           int mode,
                           size_t length,
 ...
 */
int esp_mbedtls_aria_crypt_cfb128(mbedtls_aria_context *ctx,
                              int mode,
                              size_t length,
                              size_t *iv_off,
                              unsigned char iv[MBEDTLS_ARIA_BLOCKSIZE],
                              ...
                              unsigned char *output)
{
    ...
    if (mode == MBEDTLS_ARIA_DECRYPT) {
        while (length--) {
            if (n == 0) {
                esp_mbedtls_aria_crypt_ecb(ctx, iv, iv);
            }
 ...
 */
int esp_mbedtls_aria_crypt_ctr(mbedtls_aria_context *ctx,
                           size_t length,
                           size_t *nc_off,
                           ...
                           unsigned char *output)
{
    ...
    while (length--) {
        if (n == 0) {
            esp_mbedtls_aria_crypt_ecb(ctx, nonce_counter,
                                   stream_block);
 ...
 */
static const uint8_t esp_aria_test1_ecb_key[32] =           // test key
{
...
static const uint8_t esp_aria_test1_ecb_pt[MBEDTLS_ARIA_BLOCKSIZE] =            // plaintext
{
    0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,     // same for all
    0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF      // key sizes
};

static const uint8_t esp_aria_test1_ecb_ct[3][MBEDTLS_ARIA_BLOCKSIZE] =         // ciphertext
{
...
static const uint8_t esp_aria_test2_key[32] =
{
...
static const uint8_t esp_aria_test2_pt[48] =
{
...
#if (defined(MBEDTLS_CIPHER_MODE_CBC) || defined(MBEDTLS_CIPHER_MODE_CFB))
static const uint8_t esp_aria_test2_iv[MBEDTLS_ARIA_BLOCKSIZE] =
{
...
#if defined(MBEDTLS_CIPHER_MODE_CBC)
static const uint8_t esp_aria_test2_cbc_ct[3][48] =         // CBC ciphertext
{
...
#if defined(MBEDTLS_CIPHER_MODE_CFB)
static const uint8_t esp_aria_test2_cfb_ct[3][48] =         // CFB ciphertext
{
...
#if defined(MBEDTLS_CIPHER_MODE_CTR)
static const uint8_t esp_aria_test2_ctr_ct[3][48] =         // CTR ciphertext
{
 ...
 * Checkup routine
 */
int esp_mbedtls_aria_self_test(int verbose)
{
    int i;
    ...
    for (i = 0; i < 3; i++) {
        /* test ECB encryption */
        if (verbose) {
            esp_mbedtls_printf("  ARIA-ECB-%d (enc): ", 128 + 64 * i);
        }
        esp_mbedtls_aria_setkey_enc(&ctx, esp_aria_test1_ecb_key, 128 + 64 * i);
        esp_mbedtls_aria_crypt_ecb(&ctx, esp_aria_test1_ecb_pt, blk);
        ARIA_SELF_TEST_ASSERT(
            memcmp(blk, esp_aria_test1_ecb_ct[i], MBEDTLS_ARIA_BLOCKSIZE)
            != 0);

        /* test ECB decryption */
        if (verbose) {
            esp_mbedtls_printf("  ARIA-ECB-%d (dec): ", 128 + 64 * i);
#if defined(MBEDTLS_BLOCK_CIPHER_NO_DECRYPT)
...
#if !defined(MBEDTLS_BLOCK_CIPHER_NO_DECRYPT)
        esp_mbedtls_aria_setkey_dec(&ctx, esp_aria_test1_ecb_key, 128 + 64 * i);
        esp_mbedtls_aria_crypt_ecb(&ctx, esp_aria_test1_ecb_ct[i], blk);
        ARIA_SELF_TEST_ASSERT(
            memcmp(blk, esp_aria_test1_ecb_pt, MBEDTLS_ARIA_BLOCKSIZE)
            != 0);
#endif
    }
    if (verbose) {
    ...
    for (i = 0; i < 3; i++) {
        /* Test CBC encryption */
        if (verbose) {
            esp_mbedtls_printf("  ARIA-CBC-%d (enc): ", 128 + 64 * i);
        }
        esp_mbedtls_aria_setkey_enc(&ctx, esp_aria_test2_key, 128 + 64 * i);
        memcpy(iv, esp_aria_test2_iv, MBEDTLS_ARIA_BLOCKSIZE);
        memset(buf, 0x55, sizeof(buf));
        esp_mbedtls_aria_crypt_cbc(&ctx, MBEDTLS_ARIA_ENCRYPT, 48, iv,
                               esp_aria_test2_pt, buf);
        ARIA_SELF_TEST_ASSERT(memcmp(buf, esp_aria_test2_cbc_ct[i], 48)
                              != 0);

        /* Test CBC decryption */
        if (verbose) {
            esp_mbedtls_printf("  ARIA-CBC-%d (dec): ", 128 + 64 * i);
        }
        esp_mbedtls_aria_setkey_dec(&ctx, esp_aria_test2_key, 128 + 64 * i);
        memcpy(iv, esp_aria_test2_iv, MBEDTLS_ARIA_BLOCKSIZE);
        memset(buf, 0xAA, sizeof(buf));
        esp_mbedtls_aria_crypt_cbc(&ctx, MBEDTLS_ARIA_DECRYPT, 48, iv,
                               esp_aria_test2_cbc_ct[i], buf);
        ARIA_SELF_TEST_ASSERT(memcmp(buf, esp_aria_test2_pt, 48) != 0);
    }
    if (verbose) {
        esp_mbedtls_printf("\n");
    }
    ...
    for (i = 0; i < 3; i++) {
        /* Test CFB encryption */
        if (verbose) {
            esp_mbedtls_printf("  ARIA-CFB-%d (enc): ", 128 + 64 * i);
        }
        esp_mbedtls_aria_setkey_enc(&ctx, esp_aria_test2_key, 128 + 64 * i);
        memcpy(iv, esp_aria_test2_iv, MBEDTLS_ARIA_BLOCKSIZE);
        memset(buf, 0x55, sizeof(buf));
        j = 0;
        esp_mbedtls_aria_crypt_cfb128(&ctx, MBEDTLS_ARIA_ENCRYPT, 48, &j, iv,
                                  esp_aria_test2_pt, buf);
        ARIA_SELF_TEST_ASSERT(memcmp(buf, esp_aria_test2_cfb_ct[i], 48) != 0);

        /* Test CFB decryption */
        if (verbose) {
            esp_mbedtls_printf("  ARIA-CFB-%d (dec): ", 128 + 64 * i);
        }
        esp_mbedtls_aria_setkey_enc(&ctx, esp_aria_test2_key, 128 + 64 * i);
        memcpy(iv, esp_aria_test2_iv, MBEDTLS_ARIA_BLOCKSIZE);
        memset(buf, 0xAA, sizeof(buf));
        j = 0;
        esp_mbedtls_aria_crypt_cfb128(&ctx, MBEDTLS_ARIA_DECRYPT, 48, &j,
                                  iv, esp_aria_test2_cfb_ct[i], buf);
        ARIA_SELF_TEST_ASSERT(memcmp(buf, esp_aria_test2_pt, 48) != 0);
    }
    if (verbose) {
        esp_mbedtls_printf("\n");
    ...
    for (i = 0; i < 3; i++) {
        /* Test CTR encryption */
        if (verbose) {
            esp_mbedtls_printf("  ARIA-CTR-%d (enc): ", 128 + 64 * i);
        }
        esp_mbedtls_aria_setkey_enc(&ctx, esp_aria_test2_key, 128 + 64 * i);
        memset(iv, 0, MBEDTLS_ARIA_BLOCKSIZE);                      // IV = 0
        memset(buf, 0x55, sizeof(buf));
        j = 0;
        esp_mbedtls_aria_crypt_ctr(&ctx, 48, &j, iv, blk,
                               esp_aria_test2_pt, buf);
        ARIA_SELF_TEST_ASSERT(memcmp(buf, esp_aria_test2_ctr_ct[i], 48) != 0);

        /* Test CTR decryption */
        if (verbose) {
            esp_mbedtls_printf("  ARIA-CTR-%d (dec): ", 128 + 64 * i);
        }
        esp_mbedtls_aria_setkey_enc(&ctx, esp_aria_test2_key, 128 + 64 * i);
        memset(iv, 0, MBEDTLS_ARIA_BLOCKSIZE);                      // IV = 0
        memset(buf, 0xAA, sizeof(buf));
