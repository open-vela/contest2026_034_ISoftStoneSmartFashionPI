...
static void esp_psa_its_fill_filename(psa_storage_uid_t uid, char *filename)
{
    /* Break up the UID into two 32-bit pieces so as not to rely on
     * long long support in snprintf. */
    esp_mbedtls_snprintf(filename, PSA_ITS_STORAGE_FILENAME_LENGTH,
...
static psa_status_t esp_psa_its_read_file(psa_storage_uid_t uid,
                                      ...
                                      FILE **p_stream)
{
    ...
    *p_stream = NULL;
    esp_psa_its_fill_filename(uid, filename);
    *p_stream = fopen(filename, "rb");
    ...
    esp_mbedtls_setbuf(*p_stream, NULL);
...
psa_status_t esp_psa_its_get_info(psa_storage_uid_t uid,
                              struct psa_storage_info_t *p_info)
{
    psa_status_t status;
    FILE *stream = NULL;
    status = esp_psa_its_read_file(uid, p_info, &stream);
    if (stream != NULL) {
        fclose(stream);
    }
    return status;
...
psa_status_t esp_psa_its_get(psa_storage_uid_t uid,
                         ...
                         size_t *p_data_length)
{
    ...
    status = esp_psa_its_read_file(uid, &info, &stream);
...
psa_status_t esp_psa_its_set(psa_storage_uid_t uid,
                         uint32_t data_length,
                         const void *p_data,
                         psa_storage_create_flags_t create_flags)
{
    ...
    /* Ensure no stdio buffering of secrets, as such buffers cannot be wiped. */
    esp_mbedtls_setbuf(stream, NULL);
...
psa_status_t esp_psa_its_remove(psa_storage_uid_t uid)
{
    char filename[PSA_ITS_STORAGE_FILENAME_LENGTH];
    FILE *stream;
    esp_psa_its_fill_filename(uid, filename);
