...
int  esp_mbedtls_mps_trace_get_depth(void);
void esp_mbedtls_mps_trace_inc_depth(void);
void esp_mbedtls_mps_trace_dec_depth(void);

void esp_mbedtls_mps_trace_color(int id);
void esp_mbedtls_mps_trace_indent(int level, mbedtls_mps_trace_type ty);

void esp_mbedtls_mps_trace_print_msg(int id, int line, const char *format, ...);

#define MBEDTLS_MPS_TRACE(type, ...)                                              \
    do {                                                                            \
        if (!(MBEDTLS_MPS_TRACE_MASK & (1u << esp_mbedtls_mps_trace_id)))         \
        break;                                                                  \
        esp_mbedtls_mps_trace_indent(esp_mbedtls_mps_trace_get_depth(), type);            \
        esp_mbedtls_mps_trace_color(esp_mbedtls_mps_trace_id);                            \
        esp_mbedtls_mps_trace_print_msg(esp_mbedtls_mps_trace_id, __LINE__, __VA_ARGS__); \
        esp_mbedtls_mps_trace_color(0);                                               \
    } while (0)
...
#define MBEDTLS_MPS_TRACE_INIT(...)                                         \
    do {                                                                      \
        ...
        esp_mbedtls_mps_trace_inc_depth();                                        \
    } while (0)
...
#define MBEDTLS_MPS_TRACE_END(val)                                        \
    do {                                                                    \
        ...
        esp_mbedtls_mps_trace_dec_depth();                                      \
    } while (0)
