 ...
 */
void esp_mbedtls_pkcs7_init(mbedtls_pkcs7 *pkcs7)
{
    memset(pkcs7, 0, sizeof(*pkcs7));
}

static int esp_pkcs7_get_next_content_len(unsigned char **p, unsigned char *end,
                                      size_t *len)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;

    ret = esp_mbedtls_asn1_get_tag(p, end, len, MBEDTLS_ASN1_CONSTRUCTED
 ...
 * Version ::= INTEGER
 **/
static int esp_pkcs7_get_version(unsigned char **p, unsigned char *end, int *ver)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;

    ret = esp_mbedtls_asn1_get_int(p, end, ver);
    if (ret != 0) {
 ...
 **/
static int esp_pkcs7_get_content_info_type(unsigned char **p, unsigned char *end,
                                       ...
                                       mbedtls_pkcs7_buf *pkcs7)
{
    ...
    *seq_end = *p + len;
    ret = esp_mbedtls_asn1_get_tag(p, *seq_end, &len, MBEDTLS_ASN1_OID);
    if (ret != 0) {
        *p = start;
        return MBEDTLS_ERROR_ADD(MBEDTLS_ERR_PKCS7_INVALID_CONTENT_INFO, ret);
    }
 ...
 * This is from x509.h
 **/
static int esp_pkcs7_get_digest_algorithm(unsigned char **p, unsigned char *end,
 ...
 **/
static int esp_pkcs7_get_digest_algorithm_set(unsigned char **p,
                                          unsigned char *end,
                                          mbedtls_x509_buf *alg)
{
    size_t len = 0;
    ...
    ret = esp_mbedtls_asn1_get_alg_null(p, end, alg);
    if (ret != 0) {
        return MBEDTLS_ERROR_ADD(MBEDTLS_ERR_PKCS7_INVALID_ALG, ret);
    }
 ...
 **/
static int esp_pkcs7_get_certificates(unsigned char **p, unsigned char *end,
                                  mbedtls_x509_crt *certs)
{
    ...
    ret = esp_mbedtls_asn1_get_tag(p, end, &len1, MBEDTLS_ASN1_CONSTRUCTED
                               | MBEDTLS_ASN1_CONTEXT_SPECIFIC);
    ...
    ret = esp_mbedtls_asn1_get_tag(p, end_set, &len2, MBEDTLS_ASN1_CONSTRUCTED
                               | MBEDTLS_ASN1_SEQUENCE);
    if (ret != 0) {
    ...
    if ((ret = esp_mbedtls_x509_crt_parse_der(certs, start, len1)) < 0) {
 ...
 **/
static int esp_pkcs7_get_signature(unsigned char **p, unsigned char *end,
                               mbedtls_pkcs7_buf *signature)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    size_t len = 0;

    ret = esp_mbedtls_asn1_get_tag(p, end, &len, MBEDTLS_ASN1_OCTET_STRING);
    if (ret != 0) {
        return ret;
    }
...
static void esp_pkcs7_free_signer_info(mbedtls_pkcs7_signer_info *signer)
{
 ...
 **/
static int esp_pkcs7_get_signer_info(unsigned char **p, unsigned char *end,
                                 ...
                                 mbedtls_x509_buf *alg)
{
    ...
    asn1_ret = esp_mbedtls_asn1_get_tag(p, end, &len, MBEDTLS_ASN1_CONSTRUCTED
                                    | MBEDTLS_ASN1_SEQUENCE);
    if (asn1_ret != 0) {
        goto out;
    }
    ...
    ret = esp_pkcs7_get_version(p, end_signer, &signer->version);
    if (ret != 0) {
        goto out;
    }

    asn1_ret = esp_mbedtls_asn1_get_tag(p, end_signer, &len,
    ...
    asn1_ret = esp_mbedtls_asn1_get_tag(p, end_issuer_and_sn, &len,
                                    MBEDTLS_ASN1_CONSTRUCTED | MBEDTLS_ASN1_SEQUENCE);
    if (asn1_ret != 0) {
        goto out;
    }

    ret  = esp_mbedtls_x509_get_name(p, *p + len, &signer->issuer);
    if (ret != 0) {
        goto out;
    }
    ...
    ret = esp_mbedtls_x509_get_serial(p, end_issuer_and_sn, &signer->serial);
    if (ret != 0) {
        goto out;
    }
    ...
    ret = esp_pkcs7_get_digest_algorithm(p, end_signer, &signer->alg_identifier);
    if (ret != 0) {
        goto out;
    }
    ...
    ret = esp_pkcs7_get_digest_algorithm(p, end_signer, &signer->sig_alg_identifier);
    if (ret != 0) {
        goto out;
    }

    ret = esp_pkcs7_get_signature(p, end_signer, &signer->sig);
    if (ret != 0) {
        goto out;
    }
...
out:
    if (asn1_ret != 0 || ret != 0) {
        esp_pkcs7_free_signer_info(signer);
 ...
 * Return negative error code for failure.
 **/
static int esp_pkcs7_get_signers_info_set(unsigned char **p, unsigned char *end,
                                      ...
                                      mbedtls_x509_buf *digest_alg)
{
    ...
    ret = esp_pkcs7_get_signer_info(p, end_set, signers_set, digest_alg);
    if (ret != 0) {
        return ret;
    }
    count++;
    ...
    while (*p != end_set) {
        mbedtls_pkcs7_signer_info *signer =
            esp_mbedtls_calloc(1, sizeof(mbedtls_pkcs7_signer_info));
        if (!signer) {
            ret = MBEDTLS_ERR_PKCS7_ALLOC_FAILED;
            goto cleanup;
        }

        ret = esp_pkcs7_get_signer_info(p, end_set, signer, digest_alg);
        if (ret != 0) {
            esp_mbedtls_free(signer);
            goto cleanup;
        }
        prev->next = signer;
        prev = signer;
...
cleanup:
    esp_pkcs7_free_signer_info(signers_set);
    mbedtls_pkcs7_signer_info *signer = signers_set->next;
    while (signer != NULL) {
        prev = signer;
        signer = signer->next;
        esp_pkcs7_free_signer_info(prev);
        esp_mbedtls_free(prev);
    }
 ...
 */
static int esp_pkcs7_get_signed_data(unsigned char *buf, size_t buflen,
                                 mbedtls_pkcs7_signed_data *signed_data)
{
    ...
    ret = esp_pkcs7_get_version(&p, end, &signed_data->version);
    if (ret != 0) {
        return ret;
    }

    /* Get digest algorithm */
    ret = esp_pkcs7_get_digest_algorithm_set(&p, end,
    ...
    ret = esp_pkcs7_get_content_info_type(&p, end, &end_content_info, &content_type);
    if (ret != 0) {
        return ret;
    }
    ...
    if (p != end_content_info) {
        ...
        ret = esp_mbedtls_asn1_get_tag(&p,
                                   end_content_info,
                                   &len,
    ...
    esp_mbedtls_x509_crt_init(&signed_data->certs);
    ret = esp_pkcs7_get_certificates(&p, end, &signed_data->certs);
    if (ret < 0) {
        return ret;
    }
    ...
    /* Get signers info */
    ret = esp_pkcs7_get_signers_info_set(&p,
                                     end,
                                     &signed_data->signers,
                                     &signed_data->digest_alg_identifiers);
    if (ret < 0) {
...
int esp_mbedtls_pkcs7_parse_der(mbedtls_pkcs7 *pkcs7, const unsigned char *buf,
                            const size_t buflen)
{
    unsigned char *p;
    unsigned char *end;
    ...
    pkcs7->raw.p = p = esp_mbedtls_calloc(1, buflen);
    if (pkcs7->raw.p == NULL) {
        ret = MBEDTLS_ERR_PKCS7_ALLOC_FAILED;
        goto out;
    }
    ...
    ret = esp_pkcs7_get_next_content_len(&p, end, &len);
    if (ret != 0) {
        goto out;
    }
...
try_data:
    ret = esp_pkcs7_get_signed_data(p, len, &pkcs7->signed_data);
    if (ret != 0) {
        goto out;
    }
...
out:
    if (ret < 0) {
        esp_mbedtls_pkcs7_free(pkcs7);
    }

    return ret;
}

static int esp_mbedtls_pkcs7_data_or_hash_verify(mbedtls_pkcs7 *pkcs7,
                                             ...
                                             const int is_data_hash)
{
    ...
    if (esp_mbedtls_x509_time_is_past(&cert->valid_to) ||
        esp_mbedtls_x509_time_is_future(&cert->valid_from)) {
    ...
    hash = esp_mbedtls_calloc(esp_mbedtls_md_get_size(md_info), 1);
    if (hash == NULL) {
    ...
    if (is_data_hash) {
        if (datalen != esp_mbedtls_md_get_size(md_info)) {
            ret = MBEDTLS_ERR_PKCS7_VERIFY_FAIL;
        } else {
            memcpy(hash, data, datalen);
        }
    } else {
        ret = esp_mbedtls_md(md_info, data, datalen, hash);
    }
    if (ret != 0) {
        esp_mbedtls_free(hash);
        return MBEDTLS_ERR_PKCS7_VERIFY_FAIL;
    }
    ...
    for (signer = &pkcs7->signed_data.signers; signer; signer = signer->next) {
        ret = esp_mbedtls_pk_verify(&pk_cxt, md_alg, hash,
                                esp_mbedtls_md_get_size(md_info),
...
int esp_mbedtls_pkcs7_signed_data_verify(mbedtls_pkcs7 *pkcs7,
                                     const mbedtls_x509_crt *cert,
                                     const unsigned char *data,
                                     size_t datalen)
{
    ...
    return esp_mbedtls_pkcs7_data_or_hash_verify(pkcs7, cert, data, datalen, 0);
}

int esp_mbedtls_pkcs7_signed_hash_verify(mbedtls_pkcs7 *pkcs7,
                                     const mbedtls_x509_crt *cert,
                                     ...
                                     size_t hashlen)
{
    ...
    return esp_mbedtls_pkcs7_data_or_hash_verify(pkcs7, cert, hash, hashlen, 1);
}

/*
 * Unallocate all pkcs7 data
 */
void esp_mbedtls_pkcs7_free(mbedtls_pkcs7 *pkcs7)
{
    ...
    esp_mbedtls_free(pkcs7->raw.p);

    esp_mbedtls_x509_crt_free(&pkcs7->signed_data.certs);
    esp_mbedtls_x509_crl_free(&pkcs7->signed_data.crl);

    signer_cur = pkcs7->signed_data.signers.next;
    esp_pkcs7_free_signer_info(&pkcs7->signed_data.signers);
    while (signer_cur != NULL) {
        signer_prev = signer_cur;
        signer_cur = signer_prev->next;
        esp_pkcs7_free_signer_info(signer_prev);
        esp_mbedtls_free(signer_prev);
    }
