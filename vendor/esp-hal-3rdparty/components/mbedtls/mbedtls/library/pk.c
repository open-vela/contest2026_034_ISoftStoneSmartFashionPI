 ...
 */
void esp_mbedtls_pk_init(mbedtls_pk_context *ctx)
{
    ctx->pk_info = NULL;
    ctx->pk_ctx = NULL;
 ...
 */
void esp_mbedtls_pk_free(mbedtls_pk_context *ctx)
{
    if (ctx == NULL) {
        return;
    }
    ...
    if ((ctx->pk_info != NULL) && (ctx->pk_info->type != MBEDTLS_PK_OPAQUE)) {
        esp_psa_destroy_key(ctx->priv_id);
    }
#endif /* MBEDTLS_PK_USE_PSA_EC_DATA */

    esp_mbedtls_platform_zeroize(ctx, sizeof(mbedtls_pk_context));
}
 ...
 */
void esp_mbedtls_pk_restart_init(mbedtls_pk_restart_ctx *ctx)
{
    ctx->pk_info = NULL;
    ctx->rs_ctx = NULL;
}
 ...
 */
void esp_mbedtls_pk_restart_free(mbedtls_pk_restart_ctx *ctx)
{
 ...
 */
const mbedtls_pk_info_t *esp_mbedtls_pk_info_from_type(mbedtls_pk_type_t pk_type)
{
    switch (pk_type) {
#if defined(MBEDTLS_RSA_C)
        case MBEDTLS_PK_RSA:
            return &esp_mbedtls_rsa_info;
#endif /* MBEDTLS_RSA_C */
#if defined(MBEDTLS_PK_HAVE_ECC_KEYS)
        case MBEDTLS_PK_ECKEY:
            return &esp_mbedtls_eckey_info;
        case MBEDTLS_PK_ECKEY_DH:
            return &esp_mbedtls_eckeydh_info;
#endif /* MBEDTLS_PK_HAVE_ECC_KEYS */
#if defined(MBEDTLS_PK_CAN_ECDSA_SOME)
        case MBEDTLS_PK_ECDSA:
            return &esp_mbedtls_ecdsa_info;
 ...
 * Initialise context
 */
int esp_mbedtls_pk_setup(mbedtls_pk_context *ctx, const mbedtls_pk_info_t *info)
{
 ...
 */
int esp_mbedtls_pk_setup_opaque(mbedtls_pk_context *ctx,
                            const mbedtls_svc_key_id_t key)
{
    ...
    if (PSA_SUCCESS != esp_psa_get_key_attributes(key, &attributes)) {
        return MBEDTLS_ERR_PK_BAD_INPUT_DATA;
    }
    ...
    if (PSA_KEY_TYPE_IS_ECC_KEY_PAIR(type)) {
        info = &esp_mbedtls_ecdsa_opaque_info;
    } else
#endif /* MBEDTLS_PK_HAVE_ECC_KEYS */
    if (type == PSA_KEY_TYPE_RSA_KEY_PAIR) {
        info = &esp_mbedtls_rsa_opaque_info;
    } else {
 ...
 */
int esp_mbedtls_pk_setup_rsa_alt(mbedtls_pk_context *ctx, void *key,
                             mbedtls_pk_rsa_alt_decrypt_func decrypt_func,
                             mbedtls_pk_rsa_alt_sign_func sign_func,
                             mbedtls_pk_rsa_alt_key_len_func key_len_func)
{
    mbedtls_rsa_alt_context *rsa_alt;
    const mbedtls_pk_info_t *info = &esp_mbedtls_rsa_alt_info;
 ...
 */
int esp_mbedtls_pk_can_do_ext(const mbedtls_pk_context *ctx, psa_algorithm_t alg,
                          psa_key_usage_t usage)
{
    ...
    if (esp_mbedtls_pk_get_type(ctx) != MBEDTLS_PK_OPAQUE) {
    ...
    status = esp_psa_get_key_attributes(ctx->priv_id, &attributes);
...
#if defined(MBEDTLS_PSA_CRYPTO_CLIENT)
#if defined(MBEDTLS_RSA_C)
static psa_algorithm_t esp_psa_algorithm_for_rsa(const mbedtls_rsa_context *rsa,
                                             int want_crypt)
{
    if (esp_mbedtls_rsa_get_padding_mode(rsa) == MBEDTLS_RSA_PKCS_V21) {
        if (want_crypt) {
            mbedtls_md_type_t md_type = (mbedtls_md_type_t) esp_mbedtls_rsa_get_md_alg(rsa);
            return PSA_ALG_RSA_OAEP(esp_mbedtls_md_psa_alg_from_type(md_type));
        } else {
...
int esp_mbedtls_pk_get_psa_attributes(const mbedtls_pk_context *pk,
                                  psa_key_usage_t usage,
                                  psa_key_attributes_t *attributes)
{
    mbedtls_pk_type_t pk_type = esp_mbedtls_pk_get_type(pk);
    ...
    switch (pk_type) {
        ...
        case MBEDTLS_PK_RSA:
        {
            ...
            int has_private = (esp_mbedtls_rsa_check_privkey(rsa) == 0);
            ...
            psa_set_key_bits(attributes, esp_mbedtls_pk_get_bitlen(pk));
            psa_set_key_algorithm(attributes,
                                  esp_psa_algorithm_for_rsa(rsa, want_crypt));
            break;
        }
#endif /* MBEDTLS_RSA_C */
...
#else
            const mbedtls_ecp_keypair *ec = esp_mbedtls_pk_ec_ro(*pk);
            int has_private = (ec->d.n != 0);
            size_t bits = 0;
            psa_ecc_family_t family =
                esp_mbedtls_ecc_group_to_psa(ec->grp.id, &bits);
#endif
        ...
        case MBEDTLS_PK_OPAQUE:
        {
            ...
            status = esp_psa_get_key_attributes(pk->priv_id, &old_attributes);
            if (status != PSA_SUCCESS) {
                return MBEDTLS_ERR_PK_BAD_INPUT_DATA;
            }
...
static psa_status_t esp_export_import_into_psa(mbedtls_svc_key_id_t old_key_id,
                                           const psa_key_attributes_t *attributes,
                                           mbedtls_svc_key_id_t *new_key_id)
{
    unsigned char key_buffer[PSA_EXPORT_KEY_PAIR_MAX_SIZE];
    size_t key_length = 0;
    psa_status_t status = esp_psa_export_key(old_key_id,
                                         key_buffer, sizeof(key_buffer),
                                         &key_length);
    if (status != PSA_SUCCESS) {
        return status;
    }
    status = esp_psa_import_key(attributes, key_buffer, key_length, new_key_id);
    esp_mbedtls_platform_zeroize(key_buffer, key_length);
    return status;
}

static int esp_copy_into_psa(mbedtls_svc_key_id_t old_key_id,
                         const psa_key_attributes_t *attributes,
                         mbedtls_svc_key_id_t *new_key_id)
{
    /* Normally, we prefer copying: it's more efficient and works even
     * for non-exportable keys. */
    psa_status_t status = esp_psa_copy_key(old_key_id, attributes, new_key_id);
    if (status == PSA_ERROR_NOT_PERMITTED /*missing COPY usage*/ ||
         ...
         *   more liberal than esp_psa_copy_key() here.
         */
        /* Here we need to check that the types match, otherwise we risk
         * importing nonsensical data. */
        psa_key_attributes_t old_attributes = PSA_KEY_ATTRIBUTES_INIT;
        status = esp_psa_get_key_attributes(old_key_id, &old_attributes);
        if (status != PSA_SUCCESS) {
            return MBEDTLS_ERR_PK_BAD_INPUT_DATA;
        }
        psa_key_type_t old_type = psa_get_key_type(&old_attributes);
        esp_psa_reset_key_attributes(&old_attributes);
        if (old_type != psa_get_key_type(attributes)) {
            return MBEDTLS_ERR_PK_TYPE_MISMATCH;
        }
        status = esp_export_import_into_psa(old_key_id, attributes, new_key_id);
    }
    return PSA_PK_TO_MBEDTLS_ERR(status);
}
#endif /* MBEDTLS_PK_USE_PSA_EC_DATA || MBEDTLS_USE_PSA_CRYPTO */

static int esp_import_pair_into_psa(const mbedtls_pk_context *pk,
                                const psa_key_attributes_t *attributes,
                                mbedtls_svc_key_id_t *key_id)
{
    switch (esp_mbedtls_pk_get_type(pk)) {
#if defined(MBEDTLS_RSA_C)
        case MBEDTLS_PK_RSA:
        {
            ...
            int ret = esp_mbedtls_rsa_write_key(esp_mbedtls_pk_rsa(*pk),
                                            key_buffer, &key_data);
            if (ret < 0) {
                return ret;
            }
            size_t key_length = key_end - key_data;
            ret = PSA_PK_TO_MBEDTLS_ERR(esp_psa_import_key(attributes,
                                                       key_data, key_length,
                                                       key_id));
            esp_mbedtls_platform_zeroize(key_data, key_length);
            return ret;
        }
#endif /* MBEDTLS_RSA_C */
        ...
        case MBEDTLS_PK_ECDSA:
        {
             ...
             * and if it's specified, esp_psa_import_key() will know from the key
            ...
            const mbedtls_ecp_keypair *ec = esp_mbedtls_pk_ec_ro(*pk);
            size_t from_bits = 0;
            psa_ecc_family_t from_family = esp_mbedtls_ecc_group_to_psa(ec->grp.id,
                                                                    &from_bits);
#endif /* MBEDTLS_PK_USE_PSA_EC_DATA */
            ...
            return esp_copy_into_psa(pk->priv_id, attributes, key_id);
#else /* MBEDTLS_PK_USE_PSA_EC_DATA */
            if (ec->d.n == 0) {
            ...
            size_t key_length = 0;
            int ret = esp_mbedtls_ecp_write_key_ext(ec, &key_length,
                                                key_buffer, sizeof(key_buffer));
            if (ret < 0) {
                return ret;
            }
...
static int esp_import_public_into_psa(const mbedtls_pk_context *pk,
                                  const psa_key_attributes_t *attributes,
                                  mbedtls_svc_key_id_t *key_id)
{
    ...
    switch (esp_mbedtls_pk_get_type(pk)) {
        ...
        case MBEDTLS_PK_RSA:
        {
            ...
            unsigned char *const key_end = key_buffer + sizeof(key_buffer);
            key_data = key_end;
            int ret = esp_mbedtls_rsa_write_pubkey(esp_mbedtls_pk_rsa(*pk),
                                               key_buffer, &key_data);
            if (ret < 0) {
                return ret;
            }
        ...
        case MBEDTLS_PK_ECDSA:
        {
            ...
            int ret = esp_mbedtls_ecp_write_public_key(
        ...
        case MBEDTLS_PK_OPAQUE:
        {
            psa_key_attributes_t old_attributes = PSA_KEY_ATTRIBUTES_INIT;
            psa_status_t status =
                esp_psa_get_key_attributes(pk->priv_id, &old_attributes);
            if (status != PSA_SUCCESS) {
                return MBEDTLS_ERR_PK_BAD_INPUT_DATA;
            }
            psa_key_type_t old_type = psa_get_key_type(&old_attributes);
            ...
            status = esp_psa_export_public_key(pk->priv_id,
                                           key_buffer, sizeof(key_buffer),
                                           &key_length);
            if (status != PSA_SUCCESS) {
                return PSA_PK_TO_MBEDTLS_ERR(status);
    ...
    return PSA_PK_TO_MBEDTLS_ERR(esp_psa_import_key(attributes,
                                                key_data, key_length,
                                                key_id));
}

int esp_mbedtls_pk_import_into_psa(const mbedtls_pk_context *pk,
                               const psa_key_attributes_t *attributes,
                               mbedtls_svc_key_id_t *key_id)
{
    /* Set the output immediately so that it won't contain garbage even
     * if we error out before calling esp_psa_import_key(). */
    *key_id = MBEDTLS_SVC_KEY_ID_INIT;

#if defined(MBEDTLS_PK_RSA_ALT_SUPPORT)
    if (esp_mbedtls_pk_get_type(pk) == MBEDTLS_PK_RSA_ALT) {
        return MBEDTLS_ERR_PK_FEATURE_UNAVAILABLE;
    }
#endif /* MBEDTLS_PK_RSA_ALT_SUPPORT */
    ...
    if (want_public) {
        return esp_import_public_into_psa(pk, attributes, key_id);
    } else {
        return esp_import_pair_into_psa(pk, attributes, key_id);
    }
}

static int esp_copy_from_psa(mbedtls_svc_key_id_t key_id,
                         ...
                         int public_only)
{
    ...
    if (public_only) {
        status = esp_psa_export_public_key(key_id, exp_key, sizeof(exp_key), &exp_key_len);
    } else {
        status = esp_psa_export_key(key_id, exp_key, sizeof(exp_key), &exp_key_len);
    }
    if (status != PSA_SUCCESS) {
    ...
    if ((key_type == PSA_KEY_TYPE_RSA_KEY_PAIR) ||
        ...
        ret = esp_mbedtls_pk_setup(pk, esp_mbedtls_pk_info_from_type(MBEDTLS_PK_RSA));
        if (ret != 0) {
            goto exit;
        }

        if (key_type == PSA_KEY_TYPE_RSA_KEY_PAIR) {
            ret = esp_mbedtls_rsa_parse_key(esp_mbedtls_pk_rsa(*pk), exp_key, exp_key_len);
        } else {
            ret = esp_mbedtls_rsa_parse_pubkey(esp_mbedtls_pk_rsa(*pk), exp_key, exp_key_len);
        }
        if (ret != 0) {
            goto exit;
        }
        ...
        if (PSA_ALG_GET_HASH(alg_type) != PSA_ALG_ANY_HASH) {
            md_type = esp_mbedtls_md_type_from_psa_alg(alg_type);
        }

        if (PSA_ALG_IS_RSA_OAEP(alg_type) || PSA_ALG_IS_RSA_PSS(alg_type)) {
            ret = esp_mbedtls_rsa_set_padding(esp_mbedtls_pk_rsa(*pk), MBEDTLS_RSA_PKCS_V21, md_type);
        } else if (PSA_ALG_IS_RSA_PKCS1V15_SIGN(alg_type) ||
                   alg_type == PSA_ALG_RSA_PKCS1V15_CRYPT) {
            ret = esp_mbedtls_rsa_set_padding(esp_mbedtls_pk_rsa(*pk), MBEDTLS_RSA_PKCS_V15, md_type);
        }
        if (ret != 0) {
            goto exit;
        }
    ...
    if (PSA_KEY_TYPE_IS_ECC_KEY_PAIR(key_type) ||
        ...
        ret = esp_mbedtls_pk_setup(pk, esp_mbedtls_pk_info_from_type(MBEDTLS_PK_ECKEY));
        if (ret != 0) {
            goto exit;
        }

        grp_id = esp_mbedtls_ecc_group_from_psa(PSA_KEY_TYPE_ECC_GET_FAMILY(key_type), key_bits);
        ret = esp_mbedtls_pk_ecc_set_group(pk, grp_id);
        if (ret != 0) {
            goto exit;
        }

        if (PSA_KEY_TYPE_IS_ECC_KEY_PAIR(key_type)) {
            ret = esp_mbedtls_pk_ecc_set_key(pk, exp_key, exp_key_len);
            if (ret != 0) {
                goto exit;
            }
            ret = esp_mbedtls_pk_ecc_set_pubkey_from_prv(pk, exp_key, exp_key_len,
                                                     esp_mbedtls_psa_get_random,
                                                     MBEDTLS_PSA_RANDOM_STATE);
        } else {
            ret = esp_mbedtls_pk_ecc_set_pubkey(pk, exp_key, exp_key_len);
        }
        if (ret != 0) {
            goto exit;
        }
...
exit:
    ...
    esp_mbedtls_platform_zeroize(exp_key, sizeof(exp_key));

    return ret;
}

int esp_mbedtls_pk_copy_from_psa(mbedtls_svc_key_id_t key_id,
                             mbedtls_pk_context *pk)
{
    return esp_copy_from_psa(key_id, pk, 0);
}

int esp_mbedtls_pk_copy_public_from_psa(mbedtls_svc_key_id_t key_id,
                                    mbedtls_pk_context *pk)
{
    return esp_copy_from_psa(key_id, pk, 1);
}
#endif /* MBEDTLS_PSA_CRYPTO_CLIENT */

/*
 * Helper for esp_mbedtls_pk_sign and esp_mbedtls_pk_verify
 */
static inline int esp_pk_hashlen_helper(mbedtls_md_type_t md_alg, size_t *hash_len)
{
    if (*hash_len != 0) {
        return 0;
    }

    *hash_len = esp_mbedtls_md_get_size_from_type(md_alg);
 ...
 */
static int esp_pk_restart_setup(mbedtls_pk_restart_ctx *ctx,
 ...
 */
int esp_mbedtls_pk_verify_restartable(mbedtls_pk_context *ctx,
                                  ...
                                  mbedtls_pk_restart_ctx *rs_ctx)
{
    ...
    if (ctx->pk_info == NULL ||
        esp_pk_hashlen_helper(md_alg, &hash_len) != 0) {
        return MBEDTLS_ERR_PK_BAD_INPUT_DATA;
    }
    ...
    if (rs_ctx != NULL &&
        esp_mbedtls_ecp_restart_is_enabled() &&
        ctx->pk_info->verify_rs_func != NULL) {
        int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;

        if ((ret = esp_pk_restart_setup(rs_ctx, ctx->pk_info)) != 0) {
            return ret;
        }
        ...
        if (ret != MBEDTLS_ERR_ECP_IN_PROGRESS) {
            esp_mbedtls_pk_restart_free(rs_ctx);
        }
 ...
 * Verify a signature
 */
int esp_mbedtls_pk_verify(mbedtls_pk_context *ctx, mbedtls_md_type_t md_alg,
                      const unsigned char *hash, size_t hash_len,
                      const unsigned char *sig, size_t sig_len)
{
    return esp_mbedtls_pk_verify_restartable(ctx, md_alg, hash, hash_len,
                                         sig, sig_len, NULL);
}
 ...
 */
int esp_mbedtls_pk_verify_ext(mbedtls_pk_type_t type, const void *options,
                          ...
                          const unsigned char *sig, size_t sig_len)
{
    ...
    if (!esp_mbedtls_pk_can_do(ctx, type)) {
        return MBEDTLS_ERR_PK_TYPE_MISMATCH;
    }
    ...
    if (type != MBEDTLS_PK_RSASSA_PSS) {
        ...
        return esp_mbedtls_pk_verify(ctx, md_alg, hash, hash_len, sig, sig_len);
    }

    /* Ensure the PK context is of the right type otherwise esp_mbedtls_pk_rsa()
     * below would return a NULL pointer. */
    if (esp_mbedtls_pk_get_type(ctx) != MBEDTLS_PK_RSA) {
        return MBEDTLS_ERR_PK_FEATURE_UNAVAILABLE;
    }
    ...
    if (pss_opts->mgf1_hash_id == md_alg) {
        ...
        psa_algorithm_t psa_md_alg = esp_mbedtls_md_psa_alg_from_type(md_alg);
        mbedtls_svc_key_id_t key_id = MBEDTLS_SVC_KEY_ID_INIT;
        psa_key_attributes_t attributes = PSA_KEY_ATTRIBUTES_INIT;
        psa_algorithm_t psa_sig_alg = PSA_ALG_RSA_PSS_ANY_SALT(psa_md_alg);
        p = buf + sizeof(buf);
        esp_key_len = esp_mbedtls_rsa_write_pubkey(esp_mbedtls_pk_rsa(*ctx), buf, &p);
...
#endif /* MBEDTLS_USE_PSA_CRYPTO */
    {
        if (sig_len < esp_mbedtls_pk_get_len(ctx)) {
            return MBEDTLS_ERR_RSA_VERIFY_FAILED;
        }

        ret = esp_mbedtls_rsa_rsassa_pss_verify_ext(esp_mbedtls_pk_rsa(*ctx),
 ...
 */
int esp_mbedtls_pk_sign_restartable(mbedtls_pk_context *ctx,
                                mbedtls_md_type_t md_alg,
                                const unsigned char *hash, size_t hash_len,
                                ...
                                mbedtls_pk_restart_ctx *rs_ctx)
{
    ...
    if (ctx->pk_info == NULL || esp_pk_hashlen_helper(md_alg, &hash_len) != 0) {
        return MBEDTLS_ERR_PK_BAD_INPUT_DATA;
    }
 ...
 * Make a signature
 */
int esp_mbedtls_pk_sign(mbedtls_pk_context *ctx, mbedtls_md_type_t md_alg,
                    const unsigned char *hash, size_t hash_len,
                    unsigned char *sig, size_t sig_size, size_t *sig_len,
                    int (*f_rng)(void *, unsigned char *, size_t), void *p_rng)
{
    return esp_mbedtls_pk_sign_restartable(ctx, md_alg, hash, hash_len,
                                       sig, sig_size, sig_len,
                                       f_rng, p_rng, NULL);
}
 ...
 */
int esp_mbedtls_pk_sign_ext(mbedtls_pk_type_t pk_type,
                        ...
                        void *p_rng)
{
    ...
    if (!esp_mbedtls_pk_can_do(ctx, pk_type)) {
        return MBEDTLS_ERR_PK_TYPE_MISMATCH;
    }

    if (pk_type != MBEDTLS_PK_RSASSA_PSS) {
        return esp_mbedtls_pk_sign(ctx, md_alg, hash, hash_len,
...
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    const psa_algorithm_t psa_md_alg = esp_mbedtls_md_psa_alg_from_type(md_alg);
    if (psa_md_alg == 0) {
        return MBEDTLS_ERR_PK_BAD_INPUT_DATA;
    }
    ...
    if (esp_mbedtls_pk_get_type(ctx) == MBEDTLS_PK_OPAQUE) {
        ...
        status = esp_psa_get_key_attributes(ctx->priv_id, &key_attr);
        if (status != PSA_SUCCESS) {
            return PSA_PK_RSA_TO_MBEDTLS_ERR(status);
        }
        ...
        status = esp_psa_sign_hash(ctx->priv_id, sign_alg,
                               hash, hash_len,
                               sig, sig_size, sig_len);
        return PSA_PK_RSA_TO_MBEDTLS_ERR(status);
    }

    return esp_mbedtls_pk_psa_rsa_sign_ext(PSA_ALG_RSA_PSS(psa_md_alg),
                                       ctx->pk_ctx, hash, hash_len,
                                       sig, sig_size, sig_len);
#else /* MBEDTLS_USE_PSA_CRYPTO */

    if (sig_size < esp_mbedtls_pk_get_len(ctx)) {
        return MBEDTLS_ERR_PK_BUFFER_TOO_SMALL;
    }

    if (esp_pk_hashlen_helper(md_alg, &hash_len) != 0) {
        return MBEDTLS_ERR_PK_BAD_INPUT_DATA;
    }

    mbedtls_rsa_context *const rsa_ctx = esp_mbedtls_pk_rsa(*ctx);

    const int ret = esp_mbedtls_rsa_rsassa_pss_sign_no_mode_check(rsa_ctx, f_rng, p_rng, md_alg,
 ...
 * Decrypt message
 */
int esp_mbedtls_pk_decrypt(mbedtls_pk_context *ctx,
 ...
 * Encrypt message
 */
int esp_mbedtls_pk_encrypt(mbedtls_pk_context *ctx,
                       const unsigned char *input, size_t ilen,
                       unsigned char *output, size_t *olen, size_t osize,
                       int (*f_rng)(void *, unsigned char *, size_t), void *p_rng)
{
 ...
 */
int esp_mbedtls_pk_check_pair(const mbedtls_pk_context *pub,
 ...
 */
size_t esp_mbedtls_pk_get_bitlen(const mbedtls_pk_context *ctx)
{
 ...
 */
int esp_mbedtls_pk_debug(const mbedtls_pk_context *ctx, mbedtls_pk_debug_item *items)
{
    if (ctx->pk_info == NULL) {
        return MBEDTLS_ERR_PK_BAD_INPUT_DATA;
    }
 ...
 */
const char *esp_mbedtls_pk_get_name(const mbedtls_pk_context *ctx)
{
    if (ctx == NULL || ctx->pk_info == NULL) {
        return "invalid PK";
    }
 ...
 * Access the PK type
 */
mbedtls_pk_type_t esp_mbedtls_pk_get_type(const mbedtls_pk_context *ctx)
{
    if (ctx == NULL || ctx->pk_info == NULL) {
