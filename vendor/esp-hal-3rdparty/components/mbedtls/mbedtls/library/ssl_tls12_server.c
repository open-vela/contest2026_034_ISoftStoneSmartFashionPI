                                        ...
                                        size_t ilen)
{
    ...
    if ((ssl->cli_id = esp_mbedtls_calloc(1, ilen)) == NULL) {
...
static int esp_ssl_conf_has_psk_or_cb(mbedtls_ssl_config const *conf)
{
    if (conf->f_psk != NULL) {
        return 1;
    }
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_parse_renegotiation_info(mbedtls_ssl_context *ssl,
                                        ...
                                        size_t len)
{
    ...
    if (ssl->renego_status != MBEDTLS_SSL_INITIAL_HANDSHAKE) {
        ...
        if (len    != 1 + ssl->verify_data_len ||
            ...
            esp_mbedtls_ct_memcmp(buf + 1, ssl->peer_verify_data,
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_parse_supported_groups_ext(mbedtls_ssl_context *ssl,
                                          const unsigned char *buf,
                                          size_t len)
{
    ...
    if ((curves_tls_id = esp_mbedtls_calloc(our_size,
    ...
    while (list_size > 0 && our_size > 1) {
        ...
        if (esp_mbedtls_ssl_get_ecp_group_id_from_tls_id(curr_tls_id) !=
            MBEDTLS_ECP_DP_NONE) {
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_parse_supported_point_formats(mbedtls_ssl_context *ssl,
                                             const unsigned char *buf,
                                             size_t len)
{
    size_t list_size;
...
#if !defined(MBEDTLS_USE_PSA_CRYPTO) &&                             \
            defined(MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
            esp_mbedtls_ecjpake_set_point_format(&ssl->handshake->ecjpake_ctx,
                                             p[0]);
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_parse_ecjpake_kkpp(mbedtls_ssl_context *ssl,
                                  const unsigned char *buf,
                                  size_t len)
{
...
#else
    if (esp_mbedtls_ecjpake_check(&ssl->handshake->ecjpake_ctx) != 0)
#endif /* MBEDTLS_USE_PSA_CRYPTO */
    {
...
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    if ((ret = esp_mbedtls_psa_ecjpake_read_round(
        ...
        MBEDTLS_SSL_DEBUG_RET(1, "esp_psa_pake_input round one", ret);
...
#else
    if ((ret = esp_mbedtls_ecjpake_read_round_one(&ssl->handshake->ecjpake_ctx,
                                              buf, len)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ecjpake_read_round_one", ret);
        esp_mbedtls_ssl_send_alert_message(ssl, MBEDTLS_SSL_ALERT_LEVEL_FATAL,
                                       MBEDTLS_SSL_ALERT_MSG_ILLEGAL_PARAMETER);
        return ret;
    }
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_parse_max_fragment_length_ext(mbedtls_ssl_context *ssl,
                                             const unsigned char *buf,
                                             size_t len)
{
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_parse_cid_ext(mbedtls_ssl_context *ssl,
                             const unsigned char *buf,
                             size_t len)
{
    size_t peer_cid_len;
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_parse_encrypt_then_mac_ext(mbedtls_ssl_context *ssl,
                                          const unsigned char *buf,
                                          size_t len)
{
    if (len != 0) {
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_parse_extended_ms_ext(mbedtls_ssl_context *ssl,
                                     const unsigned char *buf,
                                     size_t len)
{
    if (len != 0) {
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_parse_session_ticket_ext(mbedtls_ssl_context *ssl,
                                        unsigned char *buf,
                                        size_t len)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    esp_mbedtls_ssl_session_init(&session);
    ...
    if ((ret = ssl->conf->f_ticket_parse(ssl->conf->p_ticket, &session,
        ...
        if (ret == MBEDTLS_ERR_SSL_INVALID_MAC) {
            ...
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_ticket_parse", ret);
        }
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_parse_use_srtp_ext(mbedtls_ssl_context *ssl,
                                  const unsigned char *buf,
                                  size_t len)
{
    ...
    for (j = 0; j < profile_length; j += 2) {
        ...
        if (client_protection != MBEDTLS_TLS_SRTP_UNSET) {
            MBEDTLS_SSL_DEBUG_MSG(3, ("found srtp profile: %s",
                                      esp_mbedtls_ssl_get_srtp_profile_as_string(
                                          client_protection)));
        } else {
            continue;
        }
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_check_key_curve(mbedtls_pk_context *pk,
                               uint16_t *curves_tls_id)
{
    ...
    while (*curr_tls_id != 0) {
        curr_grp_id = esp_mbedtls_ssl_get_ecp_group_id_from_tls_id(*curr_tls_id);
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_pick_cert(mbedtls_ssl_context *ssl,
                         const mbedtls_ssl_ciphersuite_t *ciphersuite_info)
{
    mbedtls_ssl_key_cert *cur, *list;
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    psa_algorithm_t pk_alg =
        esp_mbedtls_ssl_get_ciphersuite_sig_pk_psa_alg(ciphersuite_info);
    psa_key_usage_t pk_usage =
        esp_mbedtls_ssl_get_ciphersuite_sig_pk_psa_usage(ciphersuite_info);
#else
    mbedtls_pk_type_t pk_alg =
        esp_mbedtls_ssl_get_ciphersuite_sig_pk_alg(ciphersuite_info);
#endif /* MBEDTLS_USE_PSA_CRYPTO */
    uint32_t flags;
    ...
    for (cur = list; cur != NULL; cur = cur->next) {
        ...
        key_type_matches = ((ssl->conf->f_async_sign_start != NULL ||
                             ...
                             esp_mbedtls_pk_can_do_ext(cur->key, pk_alg, pk_usage)) &&
                            esp_mbedtls_pk_can_do_ext(&cur->cert->pk, pk_alg, pk_usage));
#else
        key_type_matches = (
            esp_mbedtls_pk_can_do_ext(cur->key, pk_alg, pk_usage));
#endif /* MBEDTLS_SSL_ASYNC_PRIVATE */
#else
        key_type_matches = esp_mbedtls_pk_can_do(&cur->cert->pk, pk_alg);
#endif /* MBEDTLS_USE_PSA_CRYPTO */
        if (!key_type_matches) {
         ...
         */
        if (esp_mbedtls_ssl_check_cert_usage(cur->cert, ciphersuite_info,
        ...
        if (pk_alg == MBEDTLS_PK_ECDSA &&
            esp_ssl_check_key_curve(&cur->cert->pk,
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_ciphersuite_match(mbedtls_ssl_context *ssl, int suite_id,
                                 const mbedtls_ssl_ciphersuite_t **ciphersuite_info)
{
    ...
    suite_info = esp_mbedtls_ssl_ciphersuite_from_id(suite_id);
    if (suite_info == NULL) {
...
#if defined(MBEDTLS_KEY_EXCHANGE_SOME_ECDH_OR_ECDHE_1_2_ENABLED) || \
    defined(MBEDTLS_KEY_EXCHANGE_ECDSA_CERT_REQ_ALLOWED_ENABLED)
    if (esp_mbedtls_ssl_ciphersuite_uses_ec(suite_info) &&
    ...
    if (esp_mbedtls_ssl_ciphersuite_uses_psk(suite_info) &&
        esp_ssl_conf_has_psk_or_cb(ssl->conf) == 0) {
     ...
     */
    if (esp_ssl_pick_cert(ssl, suite_info) != 0) {
        MBEDTLS_SSL_DEBUG_MSG(3, ("ciphersuite mismatch: "
    ...
    sig_type = esp_mbedtls_ssl_get_ciphersuite_sig_alg(suite_info);
    if (sig_type != MBEDTLS_PK_NONE &&
        esp_mbedtls_ssl_tls12_get_preferred_hash_for_sig_alg(
            ssl, esp_mbedtls_ssl_sig_from_pk_alg(sig_type)) == MBEDTLS_SSL_HASH_NONE) {
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_parse_client_hello(mbedtls_ssl_context *ssl)
{
...
read_record_header:
#endif
    /*
     * If renegotiating, then the input was read with esp_mbedtls_ssl_read_record(),
    ...
    if (!renegotiating && !ssl->keep_current_message) {
        if ((ret = esp_mbedtls_ssl_fetch_input(ssl, 5)) != 0) {
...
#if defined(MBEDTLS_SSL_DTLS_ANTI_REPLAY)
        if (esp_mbedtls_ssl_dtls_replay_check(ssl) != 0) {
...
#if defined(MBEDTLS_SSL_RENEGOTIATION)
    if (ssl->renego_status != MBEDTLS_SSL_INITIAL_HANDSHAKE) {
        /* Set by esp_mbedtls_ssl_read_record() */
    ...
    ssl->tls_version = (mbedtls_ssl_protocol_version) esp_mbedtls_ssl_read_version(buf,
                                                                               ssl->conf->transport);
    ...
    while (ext_len != 0) {
        ...
        switch (ext_id) {
            ...
            case MBEDTLS_TLS_EXT_SERVERNAME:
                ...
                ret = esp_mbedtls_ssl_parse_server_name_ext(ssl, ext + 4,
                                                        ext + 4 + ext_size);
                if (ret != 0) {
                    return ret;
                }
            ...
            case MBEDTLS_TLS_EXT_RENEGOTIATION_INFO:
                ...
                ret = esp_ssl_parse_renegotiation_info(ssl, ext + 4, ext_size);
                if (ret != 0) {
                    return ret;
                }
                break;
            ...
            case MBEDTLS_TLS_EXT_SIG_ALG:
                ...
                ret = esp_mbedtls_ssl_parse_sig_alg_ext(ssl, ext + 4, ext + 4 + ext_size);
                if (ret != 0) {
                    return ret;
                }
            ...
            case MBEDTLS_TLS_EXT_SUPPORTED_GROUPS:
                ...
                ret = esp_ssl_parse_supported_groups_ext(ssl, ext + 4, ext_size);
                if (ret != 0) {
                    return ret;
                }
                break;
            ...
            case MBEDTLS_TLS_EXT_SUPPORTED_POINT_FORMATS:
                ...
                ret = esp_ssl_parse_supported_point_formats(ssl, ext + 4, ext_size);
                if (ret != 0) {
                    return ret;
                }
                break;
          ...
          MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED */
            ...
            case MBEDTLS_TLS_EXT_ECJPAKE_KKPP:
                ...
                ret = esp_ssl_parse_ecjpake_kkpp(ssl, ext + 4, ext_size);
                if (ret != 0) {
                    return ret;
                }
                break;
            ...
            case MBEDTLS_TLS_EXT_MAX_FRAGMENT_LENGTH:
                ...
                ret = esp_ssl_parse_max_fragment_length_ext(ssl, ext + 4, ext_size);
                if (ret != 0) {
                    return ret;
                }
                break;
            ...
            case MBEDTLS_TLS_EXT_CID:
                ...
                ret = esp_ssl_parse_cid_ext(ssl, ext + 4, ext_size);
                if (ret != 0) {
                    return ret;
                }
                break;
            ...
            case MBEDTLS_TLS_EXT_ENCRYPT_THEN_MAC:
                ...
                ret = esp_ssl_parse_encrypt_then_mac_ext(ssl, ext + 4, ext_size);
                if (ret != 0) {
                    return ret;
                }
                break;
            ...
            case MBEDTLS_TLS_EXT_EXTENDED_MASTER_SECRET:
                ...
                ret = esp_ssl_parse_extended_ms_ext(ssl, ext + 4, ext_size);
                if (ret != 0) {
                    return ret;
                }
                break;
            ...
            case MBEDTLS_TLS_EXT_SESSION_TICKET:
                ...
                ret = esp_ssl_parse_session_ticket_ext(ssl, ext + 4, ext_size);
                if (ret != 0) {
                    return ret;
                }
                break;
            ...
            case MBEDTLS_TLS_EXT_ALPN:
                ...
                ret = esp_mbedtls_ssl_parse_alpn_ext(ssl, ext + 4,
                                                 ext + 4 + ext_size);
                if (ret != 0) {
                    return ret;
                }
            ...
            case MBEDTLS_TLS_EXT_USE_SRTP:
                ...
                ret = esp_ssl_parse_use_srtp_ext(ssl, ext + 4, ext_size);
                if (ret != 0) {
                    return ret;
                }
                break;
    ...
    if (ssl->conf->respect_cli_pref == MBEDTLS_SSL_SRV_CIPHERSUITE_ORDER_CLIENT) {
        for (j = 0, p = buf + ciph_offset + 2; j < ciph_len; j += 2, p += 2) {
            for (i = 0; ciphersuites[i] != 0; i++) {
                ...
                if ((ret = esp_ssl_ciphersuite_match(ssl, ciphersuites[i],
                                                 &ciphersuite_info)) != 0) {
                    return ret;
                }
...
have_ciphersuite:
    ...
    mbedtls_pk_type_t sig_alg = esp_mbedtls_ssl_get_ciphersuite_sig_alg(ciphersuite_info);
    if (sig_alg != MBEDTLS_PK_NONE) {
        unsigned int sig_hash = esp_mbedtls_ssl_tls12_get_preferred_hash_for_sig_alg(
            ssl, esp_mbedtls_ssl_sig_from_pk_alg(sig_alg));
...
#if defined(MBEDTLS_SSL_DTLS_CONNECTION_ID)
static void esp_ssl_write_cid_ext(mbedtls_ssl_context *ssl,
                              unsigned char *buf,
                              size_t *olen)
{
    unsigned char *p = buf;
...
static void esp_ssl_write_encrypt_then_mac_ext(mbedtls_ssl_context *ssl,
                                           unsigned char *buf,
                                           size_t *olen)
{
    unsigned char *p = buf;
     ...
     */
    suite = esp_mbedtls_ssl_ciphersuite_from_id(
        ssl->session_negotiate->ciphersuite);
    if (suite == NULL) {
        ssl->session_negotiate->encrypt_then_mac = MBEDTLS_SSL_ETM_DISABLED;
    } else {
        mbedtls_ssl_mode_t ssl_mode =
            esp_mbedtls_ssl_get_mode_from_ciphersuite(
...
#if defined(MBEDTLS_SSL_EXTENDED_MASTER_SECRET)
static void esp_ssl_write_extended_ms_ext(mbedtls_ssl_context *ssl,
                                      unsigned char *buf,
                                      size_t *olen)
{
    unsigned char *p = buf;
...
#if defined(MBEDTLS_SSL_SESSION_TICKETS)
static void esp_ssl_write_session_ticket_ext(mbedtls_ssl_context *ssl,
                                         unsigned char *buf,
                                         size_t *olen)
{
    unsigned char *p = buf;
...
static void esp_ssl_write_renegotiation_ext(mbedtls_ssl_context *ssl,
                                        unsigned char *buf,
                                        size_t *olen)
{
    unsigned char *p = buf;
...
#if defined(MBEDTLS_SSL_MAX_FRAGMENT_LENGTH)
static void esp_ssl_write_max_fragment_length_ext(mbedtls_ssl_context *ssl,
                                              unsigned char *buf,
                                              size_t *olen)
{
    unsigned char *p = buf;
    ...
    *olen = 5;
    ...
    defined(MBEDTLS_KEY_EXCHANGE_ECDSA_CERT_REQ_ALLOWED_ENABLED) || \
    defined(MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
static void esp_ssl_write_supported_point_formats_ext(mbedtls_ssl_context *ssl,
                                                  unsigned char *buf,
                                                  size_t *olen)
{
    unsigned char *p = buf;
...
#if defined(MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
static void esp_ssl_write_ecjpake_kkpp_ext(mbedtls_ssl_context *ssl,
                                       unsigned char *buf,
                                       size_t *olen)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
...
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    ret = esp_mbedtls_psa_ecjpake_write_round(&ssl->handshake->psa_pake_ctx,
                                          p + 2, (size_t) (end - p - 2), &kkpp_len,
                                          MBEDTLS_ECJPAKE_ROUND_ONE);
    if (ret != 0) {
        esp_psa_destroy_key(ssl->handshake->psa_pake_password);
        esp_psa_pake_abort(&ssl->handshake->psa_pake_ctx);
        MBEDTLS_SSL_DEBUG_RET(1, "esp_psa_pake_output", ret);
        return;
    }
#else
    ret = esp_mbedtls_ecjpake_write_round_one(&ssl->handshake->ecjpake_ctx,
                                          p + 2, (size_t) (end - p - 2), &kkpp_len,
                                          ssl->conf->f_rng, ssl->conf->p_rng);
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ecjpake_write_round_one", ret);
        return;
    }
#endif /* MBEDTLS_USE_PSA_CRYPTO */
...
static void esp_ssl_write_use_srtp_ext(mbedtls_ssl_context *ssl,
                                   unsigned char *buf,
                                   size_t *olen)
{
...
#if defined(MBEDTLS_SSL_DTLS_HELLO_VERIFY)
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_write_hello_verify_request(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    esp_mbedtls_ssl_write_version(p, ssl->conf->transport, ssl->tls_version);
...
static void esp_ssl_handle_id_based_session_resumption(mbedtls_ssl_context *ssl)
{
    int ret;
    mbedtls_ssl_session session_tmp;
    mbedtls_ssl_session * const session = ssl->session_negotiate;

    /* Resume is 0  by default, see esp_ssl_handshake_init().
     * It may be already set to 1 by esp_ssl_parse_session_ticket_ext(). */
    ...
    esp_mbedtls_ssl_session_init(&session_tmp);
...
exit:
    ...
    esp_mbedtls_ssl_session_free(&session_tmp);
}

MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_write_server_hello(mbedtls_ssl_context *ssl)
{
#if defined(MBEDTLS_HAVE_TIME)
    mbedtls_time_t t;
#endif
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM &&
        ...
        return esp_ssl_write_hello_verify_request(ssl);
    }
#endif /* MBEDTLS_SSL_DTLS_HELLO_VERIFY */
    ...
    esp_ssl_handle_id_based_session_resumption(ssl);
    ...
    if (ssl->handshake->resume == 0) {
...
#if defined(MBEDTLS_HAVE_TIME)
        ssl->session_negotiate->start = esp_mbedtls_time(NULL);
#endif
        ...
        if ((ret = esp_mbedtls_ssl_derive_keys(ssl)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_derive_keys", ret);
            return ret;
        }
    }
    ...
    *p++ = MBEDTLS_BYTE_0(MBEDTLS_SSL_COMPRESS_NULL);
    ...
    MBEDTLS_SSL_DEBUG_MSG(3, ("server hello, chosen ciphersuite: %s",
                              esp_mbedtls_ssl_get_ciphersuite_name(ssl->session_negotiate->ciphersuite)));
     ...
     */
    esp_ssl_write_renegotiation_ext(ssl, p + 2 + ext_len, &olen);
    ext_len += olen;

#if defined(MBEDTLS_SSL_MAX_FRAGMENT_LENGTH)
    esp_ssl_write_max_fragment_length_ext(ssl, p + 2 + ext_len, &olen);
    ext_len += olen;
#endif

#if defined(MBEDTLS_SSL_DTLS_CONNECTION_ID)
    esp_ssl_write_cid_ext(ssl, p + 2 + ext_len, &olen);
    ext_len += olen;
#endif

#if defined(MBEDTLS_SSL_SOME_SUITES_USE_CBC_ETM)
    esp_ssl_write_encrypt_then_mac_ext(ssl, p + 2 + ext_len, &olen);
    ext_len += olen;
#endif

#if defined(MBEDTLS_SSL_EXTENDED_MASTER_SECRET)
    esp_ssl_write_extended_ms_ext(ssl, p + 2 + ext_len, &olen);
    ext_len += olen;
#endif

#if defined(MBEDTLS_SSL_SESSION_TICKETS)
    esp_ssl_write_session_ticket_ext(ssl, p + 2 + ext_len, &olen);
    ext_len += olen;
#endif
    ...
    const mbedtls_ssl_ciphersuite_t *suite =
        esp_mbedtls_ssl_ciphersuite_from_id(ssl->session_negotiate->ciphersuite);
    if (suite != NULL && esp_mbedtls_ssl_ciphersuite_uses_ec(suite)) {
        esp_ssl_write_supported_point_formats_ext(ssl, p + 2 + ext_len, &olen);
        ext_len += olen;
    }
#endif

#if defined(MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
    esp_ssl_write_ecjpake_kkpp_ext(ssl, p + 2 + ext_len, &olen);
    ext_len += olen;
#endif

#if defined(MBEDTLS_SSL_ALPN)
    unsigned char *end = buf + MBEDTLS_SSL_OUT_CONTENT_LEN - 4;
    if ((ret = esp_mbedtls_ssl_write_alpn_ext(ssl, p + 2 + ext_len, end, &olen))
        != 0) {
        return ret;
    }
...
#if defined(MBEDTLS_SSL_DTLS_SRTP)
    esp_ssl_write_use_srtp_ext(ssl, p + 2 + ext_len, &olen);
    ext_len += olen;
#endif
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_write_certificate_request(mbedtls_ssl_context *ssl)
{
    const mbedtls_ssl_ciphersuite_t *ciphersuite_info =
    ...
    if (!esp_mbedtls_ssl_ciphersuite_cert_req_allowed(ciphersuite_info)) {
...
static int esp_ssl_write_certificate_request(mbedtls_ssl_context *ssl)
{
    ...
    if (ssl->conf->cert_req_ca_list ==  MBEDTLS_SSL_CERT_REQ_CA_LIST_ENABLED) {
         ...
         *       `esp_mbedtls_ssl_conf_ca_cb()`, then the
...
#if defined(MBEDTLS_USE_PSA_CRYPTO)
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_get_ecdh_params_from_cert(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    pk_type = esp_mbedtls_pk_get_type(pk);
    ...
    switch (pk_type) {
        ...
        case MBEDTLS_PK_ECDSA:
            ...
            status = esp_psa_get_key_attributes(pk->priv_id, &key_attributes);
            ...
            if (pk_type == MBEDTLS_PK_OPAQUE) {
                ...
                status = esp_psa_export_key(pk->priv_id, buf, sizeof(buf), &esp_key_len);
                if (status != PSA_SUCCESS) {
                    ret = PSA_TO_MBEDTLS_ERR(status);
                    goto exit;
                }
                status = esp_psa_import_key(&key_attributes, buf, esp_key_len,
        ...
        case MBEDTLS_PK_ECKEY_DH:
        case MBEDTLS_PK_ECDSA:
            key = esp_mbedtls_pk_ec_rw(*pk);
            grp_id = esp_mbedtls_pk_get_ec_group_id(pk);
            if (grp_id == MBEDTLS_ECP_DP_NONE) {
                return MBEDTLS_ERR_ECP_BAD_INPUT_DATA;
            }
            tls_id = esp_mbedtls_ssl_get_tls_id_from_ecp_group_id(grp_id);
            if (tls_id == 0) {
            ...
            esp_mbedtls_ssl_get_psa_curve_info_from_tls_id(tls_id, &key_type,
            ...
            ret = esp_mbedtls_ecp_write_key_ext(key, &esp_key_len, buf, sizeof(buf));
            if (ret != 0) {
...
exit:
    esp_psa_reset_key_attributes(&key_attributes);
    esp_mbedtls_platform_zeroize(buf, sizeof(buf));
...
static int esp_ssl_get_ecdh_params_from_cert(mbedtls_ssl_context *ssl)
{
    ...
    if (!esp_mbedtls_pk_can_do(private_key, MBEDTLS_PK_ECKEY)) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("server key not ECDH capable"));
        return MBEDTLS_ERR_SSL_PK_TYPE_MISMATCH;
    }

    if ((ret = esp_mbedtls_ecdh_get_params(&ssl->handshake->ecdh_ctx,
                                       esp_mbedtls_pk_ec_ro(*esp_mbedtls_ssl_own_key(ssl)),
                                       MBEDTLS_ECDH_OURS)) != 0) {
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_resume_server_key_exchange(mbedtls_ssl_context *ssl,
                                          size_t *signature_len)
{
    /* Append the signature to ssl->out_msg, leaving 2 bytes for the
     * signature length which will be added in esp_ssl_write_server_key_exchange
    ...
    if (ret != MBEDTLS_ERR_SSL_ASYNC_IN_PROGRESS) {
        ...
        esp_mbedtls_ssl_set_async_operation_data(ssl, NULL);
    }
    MBEDTLS_SSL_DEBUG_RET(2, "esp_ssl_resume_server_key_exchange", ret);
    return ret;
}
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_prepare_server_key_exchange(mbedtls_ssl_context *ssl,
                                           size_t *signature_len)
{
    const mbedtls_ssl_ciphersuite_t *ciphersuite_info =
        ssl->handshake->ciphersuite_info;
    ...
    if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECJPAKE) {
        ...
        *out_p = MBEDTLS_ECP_TLS_NAMED_CURVE;
        ...
        ret = esp_mbedtls_ecjpake_write_round_two(
            &ssl->handshake->ecjpake_ctx,
            ssl->out_msg + ssl->out_msglen,
            MBEDTLS_SSL_OUT_CONTENT_LEN - ssl->out_msglen, &len,
            ssl->conf->f_rng, ssl->conf->p_rng);
        if (ret != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ecjpake_write_round_two", ret);
            return ret;
        }
    ...
    if (esp_mbedtls_ssl_ciphersuite_uses_dhe(ciphersuite_info)) {
        int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
        size_t len = 0;
         ...
         * } ServerDHParams;
         */
        if ((ret = esp_mbedtls_dhm_set_group(&ssl->handshake->dhm_ctx,
                                         &ssl->conf->dhm_P,
                                         &ssl->conf->dhm_G)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_dhm_set_group", ret);
            return ret;
        }

        if ((ret = esp_mbedtls_dhm_make_params(
                 &ssl->handshake->dhm_ctx,
                 (int) esp_mbedtls_dhm_get_len(&ssl->handshake->dhm_ctx),
                 ssl->out_msg + ssl->out_msglen, &len,
                 ssl->conf->f_rng, ssl->conf->p_rng)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_dhm_make_params", ret);
            return ret;
        }
    ...
    if (esp_mbedtls_ssl_ciphersuite_uses_ecdhe(ciphersuite_info)) {
        /*
...
curve_matching_done:
        ...
        MBEDTLS_SSL_DEBUG_MSG(2, ("ECDHE curve: %s",
                                  esp_mbedtls_ssl_get_curve_name_from_tls_id(*curr_tls_id)));
        ...
        if (esp_mbedtls_ssl_get_psa_curve_info_from_tls_id(*curr_tls_id,
                                                       &key_type,
        ...
        *p++ = MBEDTLS_ECP_TLS_NAMED_CURVE;
        ...
        status = esp_psa_generate_key(&key_attributes,
                                  &handshake->xxdh_psa_privkey);
        if (status != PSA_SUCCESS) {
            ret = PSA_TO_MBEDTLS_ERR(status);
            MBEDTLS_SSL_DEBUG_RET(1, "esp_psa_generate_key", ret);
            return ret;
        }
        ...
        status = esp_psa_export_public_key(handshake->xxdh_psa_privkey,
                                       own_pubkey, own_pubkey_max_len,
                                       &len);
        if (status != PSA_SUCCESS) {
            ret = PSA_TO_MBEDTLS_ERR(status);
            MBEDTLS_SSL_DEBUG_RET(1, "esp_psa_export_public_key", ret);
            (void) esp_psa_destroy_key(handshake->xxdh_psa_privkey);
...
#else
        mbedtls_ecp_group_id curr_grp_id =
            esp_mbedtls_ssl_get_ecp_group_id_from_tls_id(*curr_tls_id);

        if ((ret = esp_mbedtls_ecdh_setup(&ssl->handshake->ecdh_ctx,
                                      curr_grp_id)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ecp_group_load", ret);
            return ret;
        }

        if ((ret = esp_mbedtls_ecdh_make_params(
                 &ssl->handshake->ecdh_ctx, &len,
                 ssl->out_msg + ssl->out_msglen,
                 MBEDTLS_SSL_OUT_CONTENT_LEN - ssl->out_msglen,
                 ssl->conf->f_rng, ssl->conf->p_rng)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ecdh_make_params", ret);
            return ret;
        }
     ...
     */
#if defined(MBEDTLS_KEY_EXCHANGE_WITH_SERVER_SIGNATURE_ENABLED)
    if (esp_mbedtls_ssl_ciphersuite_uses_server_signature(ciphersuite_info)) {
        if (dig_signed == NULL) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("should never happen"));
            return MBEDTLS_ERR_SSL_INTERNAL_ERROR;
        }
        ...
        unsigned char sig_hash =
            (unsigned char) esp_mbedtls_ssl_tls12_get_preferred_hash_for_sig_alg(
                ssl, esp_mbedtls_ssl_sig_from_pk_alg(sig_alg));

        mbedtls_md_type_t md_alg = esp_mbedtls_ssl_md_alg_from_hash(sig_hash);
        ...
        if (md_alg != MBEDTLS_MD_NONE) {
            ret = esp_mbedtls_ssl_get_key_exchange_md_tls1_2(ssl, hash, &hashlen,
                                                         dig_signed,
                                                         dig_signed_len,
                                                         md_alg);
            if (ret != 0) {
        ...
        ssl->out_msg[ssl->out_msglen++] = esp_mbedtls_ssl_hash_from_md_alg(md_alg);
        ssl->out_msg[ssl->out_msglen++] = esp_mbedtls_ssl_sig_from_pk_alg(sig_alg);
        ...
        if (ssl->conf->f_async_sign_start != NULL) {
            ...
            switch (ret) {
                ...
                case 0:
                    ...
                    return esp_ssl_resume_server_key_exchange(ssl, signature_len);
        ...
        if ((ret = esp_mbedtls_pk_sign(esp_mbedtls_ssl_own_key(ssl),
                                   ...
                                   ssl->conf->f_rng,
                                   ssl->conf->p_rng)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_pk_sign", ret);
            return ret;
        }
    }
#endif /* MBEDTLS_KEY_EXCHANGE_WITH_SERVER_SIGNATURE_ENABLED */
 ...
 * machine. */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_write_server_key_exchange(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
     ...
     * is not needed. */
    if (esp_mbedtls_ssl_ciphersuite_no_pfs(ciphersuite_info)) {
        /* For suites involving ECDH, extract DH parameters
         * from certificate at this point. */
#if defined(MBEDTLS_KEY_EXCHANGE_SOME_ECDH_ENABLED)
        if (esp_mbedtls_ssl_ciphersuite_uses_ecdh(ciphersuite_info)) {
            ret = esp_ssl_get_ecdh_params_from_cert(ssl);
            if (ret != 0) {
                MBEDTLS_SSL_DEBUG_RET(1, "esp_ssl_get_ecdh_params_from_cert", ret);
                return ret;
            }
        }
    ...
    if (ssl->handshake->async_in_progress != 0) {
        ...
        ret = esp_ssl_resume_server_key_exchange(ssl, &signature_len);
    } else
#endif /* defined(MBEDTLS_KEY_EXCHANGE_WITH_SERVER_SIGNATURE_ENABLED) &&
          defined(MBEDTLS_SSL_ASYNC_PRIVATE) */
    {
        /* ServerKeyExchange is needed. Prepare the message. */
        ret = esp_ssl_prepare_server_key_exchange(ssl, &signature_len);
    }
     ...
     * esp_ssl_prepare_server_key_exchange already wrote the signature
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_write_server_hello_done(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_parse_client_dh_public(mbedtls_ssl_context *ssl, unsigned char **p,
                                      const unsigned char *end)
{
    ...
    *p += 2;
    ...
    if ((ret = esp_mbedtls_dhm_read_public(&ssl->handshake->dhm_ctx, *p, n)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_dhm_read_public", ret);
        return MBEDTLS_ERR_SSL_DECODE_ERROR;
    }
...
#if defined(MBEDTLS_SSL_ASYNC_PRIVATE)
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_resume_decrypt_pms(mbedtls_ssl_context *ssl,
                                  ...
                                  size_t peer_pmssize)
{
    ...
    MBEDTLS_SSL_DEBUG_RET(2, "esp_ssl_decrypt_encrypted_pms", ret);
    return ret;
}
#endif /* MBEDTLS_SSL_ASYNC_PRIVATE */

MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_decrypt_encrypted_pms(mbedtls_ssl_context *ssl,
                                     const unsigned char *p,
                                     const unsigned char *end,
                                     unsigned char *peer_pms,
                                     size_t *peer_pmslen,
                                     size_t peer_pmssize)
{
    ...
    size_t len = esp_mbedtls_pk_get_len(public_key);
    ...
    if (ssl->handshake->async_in_progress != 0) {
        ...
        return esp_ssl_resume_decrypt_pms(ssl,
                                      peer_pms, peer_pmslen, peer_pmssize);
    }
#endif /* MBEDTLS_SSL_ASYNC_PRIVATE */
    ...
    if (!esp_mbedtls_pk_can_do(private_key, MBEDTLS_PK_RSA)) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("got no RSA private key"));
        return MBEDTLS_ERR_SSL_PRIVATE_KEY_REQUIRED;
    }

    ret = esp_mbedtls_pk_decrypt(private_key, p, len,
                             peer_pms, peer_pmslen, peer_pmssize,
                             ssl->conf->f_rng, ssl->conf->p_rng);
    return ret;
}
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_parse_encrypted_pms(mbedtls_ssl_context *ssl,
                                   const unsigned char *p,
                                   const unsigned char *end,
                                   size_t pms_offset)
{
    ...
    ret = esp_ssl_decrypt_encrypted_pms(ssl, p, end,
                                    peer_pms,
                                    &peer_pmslen,
                                    sizeof(peer_pms));
    ...
    esp_mbedtls_ssl_write_version(ver, ssl->conf->transport,
                              ssl->session_negotiate->tls_version);

    /* Avoid data-dependent branches while checking for invalid
     * padding, to protect against timing-based Bleichenbacher-type
     * attacks. */
    diff = esp_mbedtls_ct_bool(ret);
    diff = esp_mbedtls_ct_bool_or(diff, esp_mbedtls_ct_uint_ne(peer_pmslen, 48));
    diff = esp_mbedtls_ct_bool_or(diff, esp_mbedtls_ct_uint_ne(peer_pms[0], ver[0]));
    diff = esp_mbedtls_ct_bool_or(diff, esp_mbedtls_ct_uint_ne(peer_pms[1], ver[1]));
    ...
    esp_mbedtls_ct_memcpy_if(diff, pms, fake_pms, peer_pms, ssl->handshake->pmslen);
...
#if defined(MBEDTLS_KEY_EXCHANGE_SOME_PSK_ENABLED)
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_parse_client_psk_identity(mbedtls_ssl_context *ssl, unsigned char **p,
                                         const unsigned char *end)
{
    int ret = 0;
    uint16_t n;

    if (esp_ssl_conf_has_psk_or_cb(ssl->conf) == 0) {
    ...
    *p += 2;
    ...
    if (ssl->conf->f_psk != NULL) {
        ...
        if (n != ssl->conf->psk_identity_len ||
            esp_mbedtls_ct_memcmp(ssl->conf->psk_identity, *p, n) != 0) {
            ret = MBEDTLS_ERR_SSL_UNKNOWN_IDENTITY;
        }
    }
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_parse_client_key_exchange(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    const mbedtls_ssl_ciphersuite_t *ciphersuite_info;
    unsigned char *p, *end;
    ...
    if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_DHE_RSA) {
        if ((ret = esp_ssl_parse_client_dh_public(ssl, &p, end)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, ("esp_ssl_parse_client_dh_public"), ret);
            return ret;
        }
    ...
    if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECDHE_RSA ||
        ...
        status = esp_psa_raw_key_agreement(
        ...
        if (status != PSA_SUCCESS) {
            ret = PSA_TO_MBEDTLS_ERR(status);
            MBEDTLS_SSL_DEBUG_RET(1, "esp_psa_raw_key_agreement", ret);
        ...
        if (handshake->xxdh_psa_privkey_is_external == 0) {
            status = esp_psa_destroy_key(handshake->xxdh_psa_privkey);

            if (status != PSA_SUCCESS) {
                ret = PSA_TO_MBEDTLS_ERR(status);
                MBEDTLS_SSL_DEBUG_RET(1, "esp_psa_destroy_key", ret);
                return ret;
            }
        }
        handshake->xxdh_psa_privkey = MBEDTLS_SVC_KEY_ID_INIT;
#else
        if ((ret = esp_mbedtls_ecdh_read_public(&ssl->handshake->ecdh_ctx,
                                            p, (size_t) (end - p))) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ecdh_read_public", ret);
            return MBEDTLS_ERR_SSL_DECODE_ERROR;
        }
        ...
        if ((ret = esp_mbedtls_ecdh_calc_secret(&ssl->handshake->ecdh_ctx,
                                            &ssl->handshake->pmslen,
                                            ssl->handshake->premaster,
    ...
    if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_PSK) {
        if ((ret = esp_ssl_parse_client_psk_identity(ssl, &p, end)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, ("esp_ssl_parse_client_psk_identity"), ret);
            return ret;
        }
...
#if !defined(MBEDTLS_USE_PSA_CRYPTO)
        if ((ret = esp_mbedtls_ssl_psk_derive_premaster(ssl,
                                                    (mbedtls_key_exchange_type_t) ciphersuite_info->
                                                    key_exchange)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_psk_derive_premaster", ret);
            return ret;
        }
#endif /* !MBEDTLS_USE_PSA_CRYPTO */
    } else
    ...
    if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_RSA_PSK) {
        ...
        if (ssl->handshake->async_in_progress != 0) {
            ...
            /* Update p to skip the PSK identity. esp_ssl_parse_encrypted_pms
        ...
        if ((ret = esp_ssl_parse_encrypted_pms(ssl, p, end, 2)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, ("esp_ssl_parse_encrypted_pms"), ret);
            return ret;
        }
    ...
    if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECDHE_PSK) {
        ...
        /* Compute ECDH shared secret. */
        status = esp_psa_raw_key_agreement(PSA_ALG_ECDH,
                                       handshake->xxdh_psa_privkey,
        ...
        destruction_status = esp_psa_destroy_key(handshake->xxdh_psa_privkey);
        handshake->xxdh_psa_privkey = MBEDTLS_SVC_KEY_ID_INIT;
    ...
    if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_RSA) {
        if ((ret = esp_ssl_parse_encrypted_pms(ssl, p, end, 0)) != 0) {
    ...
    if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECJPAKE) {
        ...
        if ((ret = esp_mbedtls_psa_ecjpake_read_round(
            ...
            MBEDTLS_SSL_DEBUG_RET(1, "esp_psa_pake_input round two", ret);
            return ret;
        }
#else
        ret = esp_mbedtls_ecjpake_read_round_two(&ssl->handshake->ecjpake_ctx,
                                             p, (size_t) (end - p));
        if (ret != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ecjpake_read_round_two", ret);
            return MBEDTLS_ERR_SSL_INTERNAL_ERROR;
        }

        ret = esp_mbedtls_ecjpake_derive_secret(&ssl->handshake->ecjpake_ctx,
                                            ssl->handshake->premaster, 32, &ssl->handshake->pmslen,
                                            ssl->conf->f_rng, ssl->conf->p_rng);
        if (ret != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ecjpake_derive_secret", ret);
            return ret;
        }
#endif /* MBEDTLS_USE_PSA_CRYPTO */
    } else
...
static int esp_ssl_parse_certificate_verify(mbedtls_ssl_context *ssl)
{
    ...
    ret = esp_mbedtls_ssl_read_record(ssl, 0 /* no checksum update */);
    if (0 != ret) {
        MBEDTLS_SSL_DEBUG_RET(1, ("esp_mbedtls_ssl_read_record"), ret);
        return ret;
    }
     ...
     * Hash
     */
    md_alg = esp_mbedtls_ssl_md_alg_from_hash(ssl->in_msg[i]);

    if (md_alg == MBEDTLS_MD_NONE || esp_mbedtls_ssl_set_calc_verify_md(ssl, ssl->in_msg[i])) {
     ...
     * Signature
     */
    if ((pk_alg = esp_mbedtls_ssl_pk_alg_from_sig(ssl->in_msg[i]))
        == MBEDTLS_PK_NONE) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("peer not adhering to requested sig_alg"
                                  " for verify message"));
        return MBEDTLS_ERR_SSL_ILLEGAL_PARAMETER;
     ...
     */
    if (!esp_mbedtls_pk_can_do(peer_pk, pk_alg)) {
    ...
    if ((ret = esp_mbedtls_pk_verify(peer_pk,
                                 md_alg, hash_start, hashlen,
                                 ssl->in_msg + i, sig_len)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_pk_verify", ret);
        return ret;
    }
...
#if defined(MBEDTLS_SSL_SESSION_TICKETS)
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_write_new_session_ticket(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    size_t tlen;
    uint32_t lifetime;
...
#if defined(MBEDTLS_HAVE_TIME)
    ssl->session_negotiate->ticket_creation_time = esp_mbedtls_ms_time();
#endif
    if ((ret = ssl->conf->f_ticket_write(ssl->conf->p_ticket,
                                         ...
                                         &tlen, &lifetime)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_ticket_write", ret);
        tlen = 0;
    }
...
int esp_mbedtls_ssl_handshake_server_step(mbedtls_ssl_context *ssl)
{
    ...
    switch (ssl->state) {
        ...
        case MBEDTLS_SSL_CLIENT_HELLO:
            ret = esp_ssl_parse_client_hello(ssl);
            break;
        ...
        case MBEDTLS_SSL_SERVER_HELLO:
            ret = esp_ssl_write_server_hello(ssl);
            break;

        case MBEDTLS_SSL_SERVER_CERTIFICATE:
            ret = esp_mbedtls_ssl_write_certificate(ssl);
            break;

        case MBEDTLS_SSL_SERVER_KEY_EXCHANGE:
            ret = esp_ssl_write_server_key_exchange(ssl);
            break;

        case MBEDTLS_SSL_CERTIFICATE_REQUEST:
            ret = esp_ssl_write_certificate_request(ssl);
            break;

        case MBEDTLS_SSL_SERVER_HELLO_DONE:
            ret = esp_ssl_write_server_hello_done(ssl);
            break;
        ...
        case MBEDTLS_SSL_CLIENT_CERTIFICATE:
            ret = esp_mbedtls_ssl_parse_certificate(ssl);
            break;

        case MBEDTLS_SSL_CLIENT_KEY_EXCHANGE:
            ret = esp_ssl_parse_client_key_exchange(ssl);
            break;

        case MBEDTLS_SSL_CERTIFICATE_VERIFY:
            ret = esp_ssl_parse_certificate_verify(ssl);
            break;

        case MBEDTLS_SSL_CLIENT_CHANGE_CIPHER_SPEC:
            ret = esp_mbedtls_ssl_parse_change_cipher_spec(ssl);
            break;

        case MBEDTLS_SSL_CLIENT_FINISHED:
            ret = esp_mbedtls_ssl_parse_finished(ssl);
            break;
...
#if defined(MBEDTLS_SSL_SESSION_TICKETS)
            if (ssl->handshake->new_session_ticket != 0) {
                ret = esp_ssl_write_new_session_ticket(ssl);
            } else
#endif
            ret = esp_mbedtls_ssl_write_change_cipher_spec(ssl);
            break;

        case MBEDTLS_SSL_SERVER_FINISHED:
            ret = esp_mbedtls_ssl_write_finished(ssl);
            break;
        ...
        case MBEDTLS_SSL_HANDSHAKE_WRAPUP:
            esp_mbedtls_ssl_handshake_wrapup(ssl);
            break;
