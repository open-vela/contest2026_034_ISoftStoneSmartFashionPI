...
static int esp_local_err_translation(psa_status_t status)
{
    return esp_psa_status_to_mbedtls(status, esp_psa_to_ssl_errors,
                                 ARRAY_LENGTH(esp_psa_to_ssl_errors),
                                 esp_psa_generic_status_to_mbedtls);
}
#define PSA_TO_MBEDTLS_ERR(status) esp_local_err_translation(status)
#endif

#if defined(MBEDTLS_TEST_HOOKS)
static mbedtls_ssl_chk_buf_ptr_args esp_chk_buf_ptr_fail_args;

void esp_mbedtls_ssl_set_chk_buf_ptr_fail_args(
    const uint8_t *cur, const uint8_t *end, size_t need)
{
    esp_chk_buf_ptr_fail_args.cur = cur;
    esp_chk_buf_ptr_fail_args.end = end;
    esp_chk_buf_ptr_fail_args.need = need;
}

void esp_mbedtls_ssl_reset_chk_buf_ptr_fail_args(void)
{
    memset(&esp_chk_buf_ptr_fail_args, 0, sizeof(esp_chk_buf_ptr_fail_args));
}

int esp_mbedtls_ssl_cmp_chk_buf_ptr_fail_args(mbedtls_ssl_chk_buf_ptr_args *args)
{
    return (esp_chk_buf_ptr_fail_args.cur  != args->cur) ||
           (esp_chk_buf_ptr_fail_args.end  != args->end) ||
           (esp_chk_buf_ptr_fail_args.need != args->need);
}
...
int esp_mbedtls_ssl_conf_cid(mbedtls_ssl_config *conf,
                         size_t len,
...
int esp_mbedtls_ssl_set_cid(mbedtls_ssl_context *ssl,
                        int enable,
...
int esp_mbedtls_ssl_get_own_cid(mbedtls_ssl_context *ssl,
                            int *enabled,
...
int esp_mbedtls_ssl_get_peer_cid(mbedtls_ssl_context *ssl,
                             int *enabled,
                             ...
                             size_t *peer_cid_len)
{
    *enabled = MBEDTLS_SSL_CID_DISABLED;
    ...
    if (ssl->conf->transport != MBEDTLS_SSL_TRANSPORT_DATAGRAM ||
        esp_mbedtls_ssl_is_handshake_over(ssl) == 0) {
        return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
    }
 ...
 */
static unsigned int esp_ssl_mfl_code_to_length(int mfl)
{
    switch (mfl) {
...
int esp_mbedtls_ssl_session_copy(mbedtls_ssl_session *dst,
                             const mbedtls_ssl_session *src)
{
    esp_mbedtls_ssl_session_free(dst);
    ...
    if (src->peer_cert != NULL) {
        ...
        dst->peer_cert = esp_mbedtls_calloc(1, sizeof(mbedtls_x509_crt));
        if (dst->peer_cert == NULL) {
            return MBEDTLS_ERR_SSL_ALLOC_FAILED;
        }

        esp_mbedtls_x509_crt_init(dst->peer_cert);

        if ((ret = esp_mbedtls_x509_crt_parse_der(dst->peer_cert, src->peer_cert->raw.p,
                                              src->peer_cert->raw.len)) != 0) {
            esp_mbedtls_free(dst->peer_cert);
            dst->peer_cert = NULL;
            return ret;
        }
    }
    ...
    if (src->peer_cert_digest != NULL) {
        dst->peer_cert_digest =
            esp_mbedtls_calloc(1, src->peer_cert_digest_len);
    ...
    defined(MBEDTLS_SSL_EARLY_DATA)
    {
        int ret = esp_mbedtls_ssl_session_set_ticket_alpn(dst, src->ticket_alpn);
        if (ret != 0) {
            return ret;
        }
    }
...
#if defined(MBEDTLS_SSL_SESSION_TICKETS) && defined(MBEDTLS_SSL_CLI_C)
    if (src->ticket != NULL) {
        dst->ticket = esp_mbedtls_calloc(1, src->ticket_len);
        if (dst->ticket == NULL) {
            return MBEDTLS_ERR_SSL_ALLOC_FAILED;
        }
    ...
    if (src->endpoint == MBEDTLS_SSL_IS_CLIENT) {
        int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
        ret = esp_mbedtls_ssl_session_set_hostname(dst, src->hostname);
        if (ret != 0) {
            return ret;
        }
    }
...
static int esp_resize_buffer(unsigned char **buffer, size_t len_new, size_t *len_old)
{
    unsigned char *resized_buffer = esp_mbedtls_calloc(1, len_new);
    ...
    esp_mbedtls_zeroize_and_free(*buffer, *len_old);
...
static void esp_handle_buffer_resizing(mbedtls_ssl_context *ssl, int downsizing,
                                   ...
                                   size_t out_buf_new_len)
{
    ...
    if (ssl->in_buf != NULL) {
        ...
        if (downsizing ?
            ...
            if (esp_resize_buffer(&ssl->in_buf, in_buf_new_len, &ssl->in_buf_len) != 0) {
    ...
    if (ssl->out_buf != NULL) {
        ...
        if (downsizing ?
            ...
            if (esp_resize_buffer(&ssl->out_buf, out_buf_new_len, &ssl->out_buf_len) != 0) {
    ...
    if (modified) {
        ...
        esp_mbedtls_ssl_reset_in_out_pointers(ssl);
...
static tls_prf_fn esp_ssl_tls12prf_from_cs(int ciphersuite_id);
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls12_populate_transform(mbedtls_ssl_transform *transform,
                                        int ciphersuite,
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_tls_prf_sha256(const unsigned char *secret, size_t slen,
                          const char *label,
                          const unsigned char *random, size_t rlen,
                          unsigned char *dstbuf, size_t dlen);
static int esp_ssl_calc_verify_tls_sha256(const mbedtls_ssl_context *, unsigned char *, size_t *);
static int esp_ssl_calc_finished_tls_sha256(mbedtls_ssl_context *, unsigned char *, int);

#endif /* MBEDTLS_MD_CAN_SHA256*/

#if defined(MBEDTLS_MD_CAN_SHA384)
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_tls_prf_sha384(const unsigned char *secret, size_t slen,
                          const char *label,
                          const unsigned char *random, size_t rlen,
                          unsigned char *dstbuf, size_t dlen);

static int esp_ssl_calc_verify_tls_sha384(const mbedtls_ssl_context *, unsigned char *, size_t *);
static int esp_ssl_calc_finished_tls_sha384(mbedtls_ssl_context *, unsigned char *, int);
#endif /* MBEDTLS_MD_CAN_SHA384*/

MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls12_session_load(mbedtls_ssl_session *session,
                                  const unsigned char *buf,
                                  size_t len);
#endif /* MBEDTLS_SSL_PROTO_TLS1_2 */

static int esp_ssl_update_checksum_start(mbedtls_ssl_context *, const unsigned char *, size_t);

#if defined(MBEDTLS_MD_CAN_SHA256)
static int esp_ssl_update_checksum_sha256(mbedtls_ssl_context *, const unsigned char *, size_t);
#endif /* MBEDTLS_MD_CAN_SHA256*/

#if defined(MBEDTLS_MD_CAN_SHA384)
static int esp_ssl_update_checksum_sha384(mbedtls_ssl_context *, const unsigned char *, size_t);
#endif /* MBEDTLS_MD_CAN_SHA384*/

int  esp_mbedtls_ssl_tls_prf(const mbedtls_tls_prf_types prf,
                         ...
                         unsigned char *dstbuf, size_t dlen)
{
    ...
    switch (prf) {
        ...
        case MBEDTLS_SSL_TLS_PRF_SHA384:
            tls_prf = esp_tls_prf_sha384;
            break;
#endif /* MBEDTLS_MD_CAN_SHA384*/
#if defined(MBEDTLS_MD_CAN_SHA256)
        case MBEDTLS_SSL_TLS_PRF_SHA256:
            tls_prf = esp_tls_prf_sha256;
            break;
#endif /* MBEDTLS_MD_CAN_SHA256*/
#endif /* MBEDTLS_SSL_PROTO_TLS1_2 */
        default:
...
#if defined(MBEDTLS_X509_CRT_PARSE_C)
static void esp_ssl_clear_peer_cert(mbedtls_ssl_session *session)
{
#if defined(MBEDTLS_SSL_KEEP_PEER_CERTIFICATE)
    if (session->peer_cert != NULL) {
        esp_mbedtls_x509_crt_free(session->peer_cert);
        esp_mbedtls_free(session->peer_cert);
        session->peer_cert = NULL;
    }
#else /* MBEDTLS_SSL_KEEP_PEER_CERTIFICATE */
    if (session->peer_cert_digest != NULL) {
        /* Zeroization is not necessary. */
        esp_mbedtls_free(session->peer_cert_digest);
...
uint32_t esp_mbedtls_ssl_get_extension_id(unsigned int extension_type)
{
...
uint32_t esp_mbedtls_ssl_get_extension_mask(unsigned int extension_type)
{
    return 1 << esp_mbedtls_ssl_get_extension_id(extension_type);
}

#if defined(MBEDTLS_DEBUG_C)
static const char *esp_extension_name_table[] = {
...
static const unsigned int esp_extension_type_table[] = {
...
const char *esp_mbedtls_ssl_get_extension_name(unsigned int extension_type)
{
    return esp_extension_name_table[
        esp_mbedtls_ssl_get_extension_id(extension_type)];
}

static const char *esp_ssl_tls13_get_hs_msg_name(int hs_msg_type)
{
    switch (hs_msg_type) {
...
void esp_mbedtls_ssl_print_extension(const mbedtls_ssl_context *ssl,
                                 ...
                                 const char *extra_msg0, const char *extra_msg1)
{
    ...
    if (extra_msg0 && extra_msg1) {
        esp_mbedtls_debug_print_msg(
            ssl, level, file, line,
            "%s: %s(%u) extension %s %s.",
            esp_ssl_tls13_get_hs_msg_name(hs_msg_type),
            esp_mbedtls_ssl_get_extension_name(extension_type),
            extension_type,
    ...
    if (extra_msg) {
        esp_mbedtls_debug_print_msg(
            ssl, level, file, line,
            "%s: %s(%u) extension %s.", esp_ssl_tls13_get_hs_msg_name(hs_msg_type),
            esp_mbedtls_ssl_get_extension_name(extension_type), extension_type,
            extra_msg);
        return;
    }
    ...
    esp_mbedtls_debug_print_msg(
        ssl, level, file, line,
        "%s: %s(%u) extension.", esp_ssl_tls13_get_hs_msg_name(hs_msg_type),
        esp_mbedtls_ssl_get_extension_name(extension_type), extension_type);
}

void esp_mbedtls_ssl_print_extensions(const mbedtls_ssl_context *ssl,
                                  int level, const char *file, int line,
                                  ...
                                  const char *extra)
{
    ...
    for (unsigned i = 0;
         i < sizeof(esp_extension_name_table) / sizeof(esp_extension_name_table[0]);
         i++) {
        esp_mbedtls_ssl_print_extension(
            ssl, level, file, line, hs_msg_type, esp_extension_type_table[i],
            extensions_mask & (1 << i) ? "exists" : "does not exist", extra);
    }
}

#if defined(MBEDTLS_SSL_PROTO_TLS1_3) && defined(MBEDTLS_SSL_SESSION_TICKETS)
static const char *esp_ticket_flag_name_table[] =
{
...
void esp_mbedtls_ssl_print_ticket_flags(const mbedtls_ssl_context *ssl,
                                    int level, const char *file, int line,
                                    unsigned int flags)
{
    size_t i;

    esp_mbedtls_debug_print_msg(ssl, level, file, line,
                            "print ticket_flags (0x%02x)", flags);

    flags = flags & MBEDTLS_SSL_TLS1_3_TICKET_FLAGS_MASK;

    for (i = 0; i < ARRAY_LENGTH(esp_ticket_flag_name_table); i++) {
        if ((flags & (1 << i))) {
            esp_mbedtls_debug_print_msg(ssl, level, file, line, "- %s is set.",
                                    esp_ticket_flag_name_table[i]);
        }
    }
}
...
void esp_mbedtls_ssl_optimize_checksum(mbedtls_ssl_context *ssl,
                                   const mbedtls_ssl_ciphersuite_t *ciphersuite_info)
{
    ...
    if (ciphersuite_info->mac == MBEDTLS_MD_SHA384) {
        ssl->handshake->update_checksum = esp_ssl_update_checksum_sha384;
    } else
#endif
#if defined(MBEDTLS_MD_CAN_SHA256)
    if (ciphersuite_info->mac != MBEDTLS_MD_SHA384) {
        ssl->handshake->update_checksum = esp_ssl_update_checksum_sha256;
    } else
#endif
    {
...
int esp_mbedtls_ssl_add_hs_hdr_to_checksum(mbedtls_ssl_context *ssl,
                                       unsigned hs_type,
                                       size_t total_hs_len)
{
...
int esp_mbedtls_ssl_add_hs_msg_to_checksum(mbedtls_ssl_context *ssl,
                                       unsigned hs_type,
                                       unsigned char const *msg,
                                       size_t msg_len)
{
    int ret;
    ret = esp_mbedtls_ssl_add_hs_hdr_to_checksum(ssl, hs_type, msg_len);
    if (ret != 0) {
        return ret;
    }
...
int esp_mbedtls_ssl_reset_checksum(mbedtls_ssl_context *ssl)
{
#if defined(MBEDTLS_MD_CAN_SHA256) || \
...
#if defined(MBEDTLS_MD_CAN_SHA256)
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    status = esp_psa_hash_abort(&ssl->handshake->fin_sha256_psa);
    if (status != PSA_SUCCESS) {
        return esp_mbedtls_md_error_from_psa(status);
    }
    status = esp_psa_hash_setup(&ssl->handshake->fin_sha256_psa, PSA_ALG_SHA_256);
    if (status != PSA_SUCCESS) {
        return esp_mbedtls_md_error_from_psa(status);
    }
#else
    esp_mbedtls_md_free(&ssl->handshake->fin_sha256);
    esp_mbedtls_md_init(&ssl->handshake->fin_sha256);
    ret = esp_mbedtls_md_setup(&ssl->handshake->fin_sha256,
                           esp_mbedtls_md_info_from_type(MBEDTLS_MD_SHA256),
                           0);
    if (ret != 0) {
        return ret;
    }
    ret = esp_mbedtls_md_starts(&ssl->handshake->fin_sha256);
    if (ret != 0) {
        return ret;
    }
#endif
...
#if defined(MBEDTLS_MD_CAN_SHA384)
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    status = esp_psa_hash_abort(&ssl->handshake->fin_sha384_psa);
    if (status != PSA_SUCCESS) {
        return esp_mbedtls_md_error_from_psa(status);
    }
    status = esp_psa_hash_setup(&ssl->handshake->fin_sha384_psa, PSA_ALG_SHA_384);
    if (status != PSA_SUCCESS) {
        return esp_mbedtls_md_error_from_psa(status);
    }
#else
    esp_mbedtls_md_free(&ssl->handshake->fin_sha384);
    esp_mbedtls_md_init(&ssl->handshake->fin_sha384);
    ret = esp_mbedtls_md_setup(&ssl->handshake->fin_sha384,
                           esp_mbedtls_md_info_from_type(MBEDTLS_MD_SHA384), 0);
    if (ret != 0) {
        return ret;
    }
    ret = esp_mbedtls_md_starts(&ssl->handshake->fin_sha384);
    if (ret != 0) {
        return ret;
    }
#endif
...
static int esp_ssl_update_checksum_start(mbedtls_ssl_context *ssl,
...
#if defined(MBEDTLS_MD_CAN_SHA256)
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    status = esp_psa_hash_update(&ssl->handshake->fin_sha256_psa, buf, len);
    if (status != PSA_SUCCESS) {
        return esp_mbedtls_md_error_from_psa(status);
    }
#else
    ret = esp_mbedtls_md_update(&ssl->handshake->fin_sha256, buf, len);
    if (ret != 0) {
        return ret;
    }
#endif
...
#if defined(MBEDTLS_MD_CAN_SHA384)
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    status = esp_psa_hash_update(&ssl->handshake->fin_sha384_psa, buf, len);
    if (status != PSA_SUCCESS) {
        return esp_mbedtls_md_error_from_psa(status);
    }
#else
    ret = esp_mbedtls_md_update(&ssl->handshake->fin_sha384, buf, len);
    if (ret != 0) {
        return ret;
    }
#endif
...
#if defined(MBEDTLS_MD_CAN_SHA256)
static int esp_ssl_update_checksum_sha256(mbedtls_ssl_context *ssl,
                                      const unsigned char *buf, size_t len)
{
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    return esp_mbedtls_md_error_from_psa(esp_psa_hash_update(
                                         &ssl->handshake->fin_sha256_psa, buf, len));
#else
    return esp_mbedtls_md_update(&ssl->handshake->fin_sha256, buf, len);
#endif
}
#endif

#if defined(MBEDTLS_MD_CAN_SHA384)
static int esp_ssl_update_checksum_sha384(mbedtls_ssl_context *ssl,
                                      const unsigned char *buf, size_t len)
{
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    return esp_mbedtls_md_error_from_psa(esp_psa_hash_update(
...
#else
    return esp_mbedtls_md_update(&ssl->handshake->fin_sha384, buf, len);
#endif
}
#endif
...
#else
    esp_mbedtls_md_init(&handshake->fin_sha256);
#endif
#endif
#if defined(MBEDTLS_MD_CAN_SHA384)
#if defined(MBEDTLS_USE_PSA_CRYPTO)
...
#else
    esp_mbedtls_md_init(&handshake->fin_sha384);
#endif
#endif
...
void esp_mbedtls_ssl_transform_init(mbedtls_ssl_transform *transform)
{
...
#else
    esp_mbedtls_cipher_init(&transform->cipher_ctx_enc);
    esp_mbedtls_cipher_init(&transform->cipher_ctx_dec);
#endif
...
#else
    esp_mbedtls_md_init(&transform->md_ctx_enc);
    esp_mbedtls_md_init(&transform->md_ctx_dec);
#endif
#endif
}

void esp_mbedtls_ssl_session_init(mbedtls_ssl_session *session)
{
...
static int esp_ssl_handshake_init(mbedtls_ssl_context *ssl)
{
    ...
    ret = esp_mbedtls_ssl_reset_checksum(ssl);
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_reset_checksum", ret);
        return ret;
    }
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
        ...
        esp_mbedtls_ssl_set_timer(ssl, 0);
    }
#endif
 ...
 */
    ...
    if (ssl->conf->curve_list != NULL) {
        ...
        for (size_t i = 0; i < length; i++) {
            uint16_t tls_id = esp_mbedtls_ssl_get_tls_id_from_ecp_group_id(
                curve_list[i]);
            if (tls_id == 0) {
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_cookie_write_dummy(void *ctx,
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_cookie_check_dummy(void *ctx,
 ...
 */
void esp_mbedtls_ssl_init(mbedtls_ssl_context *ssl)
{
    memset(ssl, 0, sizeof(mbedtls_ssl_context));
}

MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_conf_version_check(const mbedtls_ssl_context *ssl)
{
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_conf_check(const mbedtls_ssl_context *ssl)
{
    int ret;
    ret = esp_ssl_conf_version_check(ssl);
    if (ret != 0) {
        return ret;
    }
     ...
     *
     */
    if (esp_mbedtls_ssl_conf_tls13_is_ephemeral_enabled(
...
int esp_mbedtls_ssl_setup(mbedtls_ssl_context *ssl,
                      const mbedtls_ssl_config *conf)
{
    ...
    if ((ret = esp_ssl_conf_check(ssl)) != 0) {
        return ret;
    }
...
#endif
    ssl->in_buf = esp_mbedtls_calloc(1, in_buf_len);
    if (ssl->in_buf == NULL) {
...
#endif
    ssl->out_buf = esp_mbedtls_calloc(1, out_buf_len);
    if (ssl->out_buf == NULL) {
    ...
    if ((ret = esp_ssl_handshake_init(ssl)) != 0) {
        goto error;
    }
...
error:
    esp_mbedtls_free(ssl->in_buf);
    esp_mbedtls_free(ssl->out_buf);
 ...
 */
void esp_mbedtls_ssl_session_reset_msg_layer(mbedtls_ssl_context *ssl,
                                         int partial)
{
#if defined(MBEDTLS_SSL_VARIABLE_BUFFER_LENGTH)
    ...
    esp_mbedtls_ssl_dtls_replay_reset(ssl);
#endif

#if defined(MBEDTLS_SSL_PROTO_TLS1_2)
    if (ssl->transform) {
        esp_mbedtls_ssl_transform_free(ssl->transform);
        esp_mbedtls_free(ssl->transform);
        ssl->transform = NULL;
    }
#endif /* MBEDTLS_SSL_PROTO_TLS1_2 */

#if defined(MBEDTLS_SSL_PROTO_TLS1_3)
    esp_mbedtls_ssl_transform_free(ssl->transform_application);
    esp_mbedtls_free(ssl->transform_application);
    ssl->transform_application = NULL;

    if (ssl->handshake != NULL) {
#if defined(MBEDTLS_SSL_EARLY_DATA)
        esp_mbedtls_ssl_transform_free(ssl->handshake->transform_earlydata);
        esp_mbedtls_free(ssl->handshake->transform_earlydata);
        ssl->handshake->transform_earlydata = NULL;
#endif

        esp_mbedtls_ssl_transform_free(ssl->handshake->transform_handshake);
        esp_mbedtls_free(ssl->handshake->transform_handshake);
...
int esp_mbedtls_ssl_session_reset_int(mbedtls_ssl_context *ssl, int partial)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    esp_mbedtls_ssl_session_reset_msg_layer(ssl, partial);
    ...
    if (ssl->session) {
        esp_mbedtls_ssl_session_free(ssl->session);
        esp_mbedtls_free(ssl->session);
        ssl->session = NULL;
    }
 ...
 * all application-set variables, function pointers and data.
 */
int esp_mbedtls_ssl_session_reset(mbedtls_ssl_context *ssl)
{
    return esp_mbedtls_ssl_session_reset_int(ssl, 0);
}

/*
 * SSL set accessors
 */
void esp_mbedtls_ssl_conf_endpoint(mbedtls_ssl_config *conf, int endpoint)
{
    conf->endpoint   = endpoint;
}

void esp_mbedtls_ssl_conf_transport(mbedtls_ssl_config *conf, int transport)
{
    conf->transport = transport;
}

#if defined(MBEDTLS_SSL_DTLS_ANTI_REPLAY)
void esp_mbedtls_ssl_conf_dtls_anti_replay(mbedtls_ssl_config *conf, char mode)
{
    conf->anti_replay = mode;
}
#endif

void esp_mbedtls_ssl_conf_dtls_badmac_limit(mbedtls_ssl_config *conf, unsigned limit)
{
...
void esp_mbedtls_ssl_set_datagram_packing(mbedtls_ssl_context *ssl,
                                      unsigned allow_packing)
{
    ssl->disable_datagram_packing = !allow_packing;
}

void esp_mbedtls_ssl_conf_handshake_timeout(mbedtls_ssl_config *conf,
...
void esp_mbedtls_ssl_conf_authmode(mbedtls_ssl_config *conf, int authmode)
{
    conf->authmode   = authmode;
}

#if defined(MBEDTLS_X509_CRT_PARSE_C)
void esp_mbedtls_ssl_conf_verify(mbedtls_ssl_config *conf,
...
void esp_mbedtls_ssl_conf_rng(mbedtls_ssl_config *conf,
...
void esp_mbedtls_ssl_conf_dbg(mbedtls_ssl_config *conf,
...
void esp_mbedtls_ssl_set_bio(mbedtls_ssl_context *ssl,
                         void *p_bio,
...
#if defined(MBEDTLS_SSL_PROTO_DTLS)
void esp_mbedtls_ssl_set_mtu(mbedtls_ssl_context *ssl, uint16_t mtu)
{
    ssl->mtu = mtu;
}
#endif

void esp_mbedtls_ssl_conf_read_timeout(mbedtls_ssl_config *conf, uint32_t timeout)
{
    conf->read_timeout   = timeout;
}

void esp_mbedtls_ssl_set_timer_cb(mbedtls_ssl_context *ssl,
                              void *p_timer,
...
#if defined(MBEDTLS_SSL_SRV_C)
void esp_mbedtls_ssl_conf_session_cache(mbedtls_ssl_config *conf,
                                    void *p_cache,
...
#if defined(MBEDTLS_SSL_CLI_C)
int esp_mbedtls_ssl_set_session(mbedtls_ssl_context *ssl, const mbedtls_ssl_session *session)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    if (session->tls_version == MBEDTLS_SSL_VERSION_TLS1_3) {
        const mbedtls_ssl_ciphersuite_t *ciphersuite_info =
            esp_mbedtls_ssl_ciphersuite_from_id(session->ciphersuite);

        if (esp_mbedtls_ssl_validate_ciphersuite(
    ...
    if ((ret = esp_mbedtls_ssl_session_copy(ssl->session_negotiate,
                                        session)) != 0) {
        return ret;
    }
...
void esp_mbedtls_ssl_conf_ciphersuites(mbedtls_ssl_config *conf,
...
#if defined(MBEDTLS_SSL_PROTO_TLS1_3)
void esp_mbedtls_ssl_conf_tls13_key_exchange_modes(mbedtls_ssl_config *conf,
                                               const int kex_modes)
{
...
#if defined(MBEDTLS_SSL_EARLY_DATA)
void esp_mbedtls_ssl_conf_early_data(mbedtls_ssl_config *conf,
...
#if defined(MBEDTLS_SSL_SRV_C)
void esp_mbedtls_ssl_conf_max_early_data_size(
...
#if defined(MBEDTLS_X509_CRT_PARSE_C)
void esp_mbedtls_ssl_conf_cert_profile(mbedtls_ssl_config *conf,
                                   const mbedtls_x509_crt_profile *profile)
{
    conf->cert_profile = profile;
}

static void esp_ssl_key_cert_free(mbedtls_ssl_key_cert *key_cert)
{
    mbedtls_ssl_key_cert *cur = key_cert, *next;

    while (cur != NULL) {
        next = cur->next;
        esp_mbedtls_free(cur);
        cur = next;
    }
}
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_append_key_cert(mbedtls_ssl_key_cert **head,
                               ...
                               mbedtls_pk_context *key)
{
    ...
    if (cert == NULL) {
        ...
        esp_ssl_key_cert_free(*head);
        *head = NULL;
        return 0;
    }

    new_cert = esp_mbedtls_calloc(1, sizeof(mbedtls_ssl_key_cert));
    if (new_cert == NULL) {
        return MBEDTLS_ERR_SSL_ALLOC_FAILED;
    }
...
int esp_mbedtls_ssl_conf_own_cert(mbedtls_ssl_config *conf,
                              mbedtls_x509_crt *own_cert,
                              mbedtls_pk_context *pk_key)
{
    return esp_ssl_append_key_cert(&conf->key_cert, own_cert, pk_key);
}

void esp_mbedtls_ssl_conf_ca_chain(mbedtls_ssl_config *conf,
                               ...
                               mbedtls_x509_crl *ca_crl)
{
    ...
    /* esp_mbedtls_ssl_conf_ca_chain() and esp_mbedtls_ssl_conf_ca_cb()
...
#if defined(MBEDTLS_X509_TRUSTED_CERTIFICATE_CALLBACK)
void esp_mbedtls_ssl_conf_ca_cb(mbedtls_ssl_config *conf,
...
#if defined(MBEDTLS_SSL_SERVER_NAME_INDICATION)
const unsigned char *esp_mbedtls_ssl_get_hs_sni(mbedtls_ssl_context *ssl,
                                            size_t *name_len)
{
...
int esp_mbedtls_ssl_set_hs_own_cert(mbedtls_ssl_context *ssl,
                                mbedtls_x509_crt *own_cert,
                                mbedtls_pk_context *pk_key)
{
    return esp_ssl_append_key_cert(&ssl->handshake->sni_key_cert,
                               own_cert, pk_key);
}

void esp_mbedtls_ssl_set_hs_ca_chain(mbedtls_ssl_context *ssl,
                                 mbedtls_x509_crt *ca_chain,
                                 mbedtls_x509_crl *ca_crl)
{
...
void esp_mbedtls_ssl_set_hs_dn_hints(mbedtls_ssl_context *ssl,
...
void esp_mbedtls_ssl_set_hs_authmode(mbedtls_ssl_context *ssl,
                                 int authmode)
{
...
#if defined(MBEDTLS_X509_CRT_PARSE_C)
void esp_mbedtls_ssl_set_verify(mbedtls_ssl_context *ssl,
                            int (*f_vrfy)(void *, mbedtls_x509_crt *, int, uint32_t *),
                            void *p_vrfy)
{
    ssl->f_vrfy = f_vrfy;
...
#if defined(MBEDTLS_USE_PSA_CRYPTO)
static const uint8_t esp_jpake_server_id[] = { 's', 'e', 'r', 'v', 'e', 'r' };
static const uint8_t esp_jpake_client_id[] = { 'c', 'l', 'i', 'e', 'n', 't' };

static psa_status_t esp_mbedtls_ssl_set_hs_ecjpake_password_common(
    mbedtls_ssl_context *ssl,
    mbedtls_svc_key_id_t pwd)
{
    ...
    status = esp_psa_pake_setup(&ssl->handshake->psa_pake_ctx, &cipher_suite);
    if (status != PSA_SUCCESS) {
        return status;
    }

    if (ssl->conf->endpoint == MBEDTLS_SSL_IS_SERVER) {
        user = esp_jpake_server_id;
        user_len = sizeof(esp_jpake_server_id);
        peer = esp_jpake_client_id;
        peer_len = sizeof(esp_jpake_client_id);
    } else {
        user = esp_jpake_client_id;
        user_len = sizeof(esp_jpake_client_id);
        peer = esp_jpake_server_id;
        peer_len = sizeof(esp_jpake_server_id);
    }

    status = esp_psa_pake_set_user(&ssl->handshake->psa_pake_ctx, user, user_len);
    if (status != PSA_SUCCESS) {
        return status;
    }

    status = esp_psa_pake_set_peer(&ssl->handshake->psa_pake_ctx, peer, peer_len);
    if (status != PSA_SUCCESS) {
        return status;
    }

    status = esp_psa_pake_set_password_key(&ssl->handshake->psa_pake_ctx, pwd);
    if (status != PSA_SUCCESS) {
        return status;
    }
...
int esp_mbedtls_ssl_set_hs_ecjpake_password(mbedtls_ssl_context *ssl,
                                        ...
                                        size_t pw_len)
{
    ...
    status = esp_psa_import_key(&attributes, pw, pw_len,
                            &ssl->handshake->psa_pake_password);
    if (status != PSA_SUCCESS) {
        return MBEDTLS_ERR_SSL_HW_ACCEL_FAILED;
    }

    status = esp_mbedtls_ssl_set_hs_ecjpake_password_common(ssl,
                                                        ssl->handshake->psa_pake_password);
    if (status != PSA_SUCCESS) {
        esp_psa_destroy_key(ssl->handshake->psa_pake_password);
        esp_psa_pake_abort(&ssl->handshake->psa_pake_ctx);
        return MBEDTLS_ERR_SSL_HW_ACCEL_FAILED;
    }
...
int esp_mbedtls_ssl_set_hs_ecjpake_password_opaque(mbedtls_ssl_context *ssl,
                                               mbedtls_svc_key_id_t pwd)
{
    psa_status_t status;
    ...
    status = esp_mbedtls_ssl_set_hs_ecjpake_password_common(ssl, pwd);
    if (status != PSA_SUCCESS) {
        esp_psa_pake_abort(&ssl->handshake->psa_pake_ctx);
        return MBEDTLS_ERR_SSL_HW_ACCEL_FAILED;
    }
                                        ...
                                        size_t pw_len)
{
    ...
    return esp_mbedtls_ecjpake_setup(&ssl->handshake->ecjpake_ctx,
                                 role,
                                 MBEDTLS_MD_SHA256,
...
int esp_mbedtls_ssl_conf_has_static_psk(mbedtls_ssl_config const *conf)
{
...
static void esp_ssl_conf_remove_psk(mbedtls_ssl_config *conf)
{
...
#endif /* MBEDTLS_USE_PSA_CRYPTO */
    if (conf->psk != NULL) {
        esp_mbedtls_zeroize_and_free(conf->psk, conf->psk_len);
        conf->psk = NULL;
        conf->psk_len = 0;
    }
    ...
    if (conf->psk_identity != NULL) {
        esp_mbedtls_free(conf->psk_identity);
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_conf_set_psk_identity(mbedtls_ssl_config *conf,
                                     ...
                                     size_t psk_identity_len)
{
    ...
    conf->psk_identity = esp_mbedtls_calloc(1, psk_identity_len);
    if (conf->psk_identity == NULL) {
        return MBEDTLS_ERR_SSL_ALLOC_FAILED;
    }
...
int esp_mbedtls_ssl_conf_psk(mbedtls_ssl_config *conf,
                         ...
                         const unsigned char *psk_identity, size_t psk_identity_len)
{
    ...
    if (esp_mbedtls_ssl_conf_has_static_psk(conf)) {
        return MBEDTLS_ERR_SSL_FEATURE_UNAVAILABLE;
    }
    ...
    if ((conf->psk = esp_mbedtls_calloc(1, psk_len)) == NULL) {
        return MBEDTLS_ERR_SSL_ALLOC_FAILED;
    }
    ...
    ret = esp_ssl_conf_set_psk_identity(conf, psk_identity, psk_identity_len);
    if (ret != 0) {
        esp_ssl_conf_remove_psk(conf);
    }

    return ret;
}

static void esp_ssl_remove_psk(mbedtls_ssl_context *ssl)
{
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    if (!mbedtls_svc_key_id_is_null(ssl->handshake->psk_opaque)) {
        ...
        if (ssl->handshake->psk_opaque_is_internal) {
            esp_psa_destroy_key(ssl->handshake->psk_opaque);
    ...
    if (ssl->handshake->psk != NULL) {
        esp_mbedtls_zeroize_and_free(ssl->handshake->psk,
...
int esp_mbedtls_ssl_set_hs_psk(mbedtls_ssl_context *ssl,
                           const unsigned char *psk, size_t psk_len)
{
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    ...
    status = esp_psa_import_key(&key_attributes, psk, psk_len, &key);
    if (status != PSA_SUCCESS) {
        return MBEDTLS_ERR_SSL_HW_ACCEL_FAILED;
    }

    /* Allow calling esp_psa_destroy_key() on psk remove */
    ssl->handshake->psk_opaque_is_internal = 1;
    return esp_mbedtls_ssl_set_hs_psk_opaque(ssl, key);
#else
    if ((ssl->handshake->psk = esp_mbedtls_calloc(1, psk_len)) == NULL) {
        return MBEDTLS_ERR_SSL_ALLOC_FAILED;
    }
...
#if defined(MBEDTLS_USE_PSA_CRYPTO)
int esp_mbedtls_ssl_conf_psk_opaque(mbedtls_ssl_config *conf,
                                ...
                                size_t psk_identity_len)
{
    ...
    /* Check and set PSK Identity */
    ret = esp_ssl_conf_set_psk_identity(conf, psk_identity,
                                    psk_identity_len);
    if (ret != 0) {
        esp_ssl_conf_remove_psk(conf);
    }
...
int esp_mbedtls_ssl_set_hs_psk_opaque(mbedtls_ssl_context *ssl,
                                  mbedtls_svc_key_id_t psk)
{
    if ((mbedtls_svc_key_id_is_null(psk)) ||
        (ssl->handshake == NULL)) {
...
#if defined(MBEDTLS_SSL_SRV_C)
void esp_mbedtls_ssl_conf_psk_cb(mbedtls_ssl_config *conf,
...
#if defined(MBEDTLS_USE_PSA_CRYPTO)
static mbedtls_ssl_mode_t esp_mbedtls_ssl_get_base_mode(
    psa_algorithm_t alg)
{
#if defined(MBEDTLS_SSL_SOME_SUITES_USE_MAC)
...
static mbedtls_ssl_mode_t esp_mbedtls_ssl_get_actual_mode(
...
mbedtls_ssl_mode_t esp_mbedtls_ssl_get_mode_from_transform(
    const mbedtls_ssl_transform *transform)
{
    mbedtls_ssl_mode_t base_mode = esp_mbedtls_ssl_get_base_mode(
#if defined(MBEDTLS_USE_PSA_CRYPTO)
        transform->psa_alg
#else
        esp_mbedtls_cipher_get_cipher_mode(&transform->cipher_ctx_enc)
#endif
        );
...
#endif
    return esp_mbedtls_ssl_get_actual_mode(base_mode, encrypt_then_mac);
}

mbedtls_ssl_mode_t esp_mbedtls_ssl_get_mode_from_ciphersuite(
#if defined(MBEDTLS_SSL_SOME_SUITES_USE_CBC_ETM)
    int encrypt_then_mac,
#endif /* MBEDTLS_SSL_SOME_SUITES_USE_CBC_ETM */
    const mbedtls_ssl_ciphersuite_t *suite)
{
    ...
    psa_key_type_t type;
    size_t size;
    status = esp_mbedtls_ssl_cipher_to_psa((mbedtls_cipher_type_t) suite->cipher,
                                       0, &alg, &type, &size);
    if (status == PSA_SUCCESS) {
        base_mode = esp_mbedtls_ssl_get_base_mode(alg);
    }
#else
    const mbedtls_cipher_info_t *cipher =
        esp_mbedtls_cipher_info_from_type((mbedtls_cipher_type_t) suite->cipher);
    if (cipher != NULL) {
        base_mode =
            esp_mbedtls_ssl_get_base_mode(
                esp_mbedtls_cipher_info_get_mode(cipher));
    }
#endif /* MBEDTLS_USE_PSA_CRYPTO */
...
psa_status_t esp_mbedtls_ssl_cipher_to_psa(mbedtls_cipher_type_t mbedtls_cipher_type,
                                       size_t taglen,
                                       psa_algorithm_t *alg,
...
int esp_mbedtls_ssl_conf_dh_param_bin(mbedtls_ssl_config *conf,
                                  const unsigned char *dhm_P, size_t P_len,
                                  const unsigned char *dhm_G, size_t G_len)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;

    esp_mbedtls_mpi_free(&conf->dhm_P);
    esp_mbedtls_mpi_free(&conf->dhm_G);

    if ((ret = esp_mbedtls_mpi_read_binary(&conf->dhm_P, dhm_P, P_len)) != 0 ||
        (ret = esp_mbedtls_mpi_read_binary(&conf->dhm_G, dhm_G, G_len)) != 0) {
        esp_mbedtls_mpi_free(&conf->dhm_P);
        esp_mbedtls_mpi_free(&conf->dhm_G);
        return ret;
    }
...
int esp_mbedtls_ssl_conf_dh_param_ctx(mbedtls_ssl_config *conf, mbedtls_dhm_context *dhm_ctx)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    if ((ret = esp_mbedtls_dhm_get_value(dhm_ctx, MBEDTLS_DHM_PARAM_P,
                                     &conf->dhm_P)) != 0 ||
        (ret = esp_mbedtls_dhm_get_value(dhm_ctx, MBEDTLS_DHM_PARAM_G,
                                     &conf->dhm_G)) != 0) {
        esp_mbedtls_mpi_free(&conf->dhm_P);
        esp_mbedtls_mpi_free(&conf->dhm_G);
        return ret;
 ...
 */
void esp_mbedtls_ssl_conf_dhm_min_bitlen(mbedtls_ssl_config *conf,
                                     unsigned int bitlen)
{
 ...
 */
void esp_mbedtls_ssl_conf_sig_hashes(mbedtls_ssl_config *conf,
                                 const int *hashes)
{
...
void esp_mbedtls_ssl_conf_sig_algs(mbedtls_ssl_config *conf,
 ...
 *
 * esp_mbedtls_ssl_setup() takes the provided list
 * and translates it to a list of IANA TLS group identifiers,
 * stored in ssl->handshake->group_list.
 *
 */
void esp_mbedtls_ssl_conf_curves(mbedtls_ssl_config *conf,
 ...
 */
void esp_mbedtls_ssl_conf_groups(mbedtls_ssl_config *conf,
...
#if defined(MBEDTLS_X509_CRT_PARSE_C)
int esp_mbedtls_ssl_set_hostname(mbedtls_ssl_context *ssl, const char *hostname)
{
    ...
    if (ssl->hostname != NULL) {
        esp_mbedtls_zeroize_and_free(ssl->hostname, strlen(ssl->hostname));
    }
    ...
    if (hostname == NULL) {
        ...
        ssl->hostname = esp_mbedtls_calloc(1, hostname_len + 1);
        if (ssl->hostname == NULL) {
            return MBEDTLS_ERR_SSL_ALLOC_FAILED;
        }
...
#if defined(MBEDTLS_SSL_SERVER_NAME_INDICATION)
void esp_mbedtls_ssl_conf_sni(mbedtls_ssl_config *conf,
...
#if defined(MBEDTLS_SSL_ALPN)
int esp_mbedtls_ssl_conf_alpn_protocols(mbedtls_ssl_config *conf, const char **protos)
{
...
const char *esp_mbedtls_ssl_get_alpn_protocol(const mbedtls_ssl_context *ssl)
{
...
#if defined(MBEDTLS_SSL_DTLS_SRTP)
void esp_mbedtls_ssl_conf_srtp_mki_value_supported(mbedtls_ssl_config *conf,
                                               int support_mki_value)
{
    conf->dtls_srtp_mki_support = support_mki_value;
}

int esp_mbedtls_ssl_dtls_srtp_set_mki_value(mbedtls_ssl_context *ssl,
...
int esp_mbedtls_ssl_conf_dtls_srtp_protection_profiles(mbedtls_ssl_config *conf,
...
void esp_mbedtls_ssl_get_dtls_srtp_negotiation_result(const mbedtls_ssl_context *ssl,
...
#if !defined(MBEDTLS_DEPRECATED_REMOVED)
void esp_mbedtls_ssl_conf_max_version(mbedtls_ssl_config *conf, int major, int minor)
{
    conf->max_tls_version = (mbedtls_ssl_protocol_version) ((major << 8) | minor);
}

void esp_mbedtls_ssl_conf_min_version(mbedtls_ssl_config *conf, int major, int minor)
{
...
#if defined(MBEDTLS_SSL_SRV_C)
void esp_mbedtls_ssl_conf_cert_req_ca_list(mbedtls_ssl_config *conf,
...
void esp_mbedtls_ssl_conf_encrypt_then_mac(mbedtls_ssl_config *conf, char etm)
{
...
void esp_mbedtls_ssl_conf_extended_master_secret(mbedtls_ssl_config *conf, char ems)
{
...
#if defined(MBEDTLS_SSL_MAX_FRAGMENT_LENGTH)
int esp_mbedtls_ssl_conf_max_frag_len(mbedtls_ssl_config *conf, unsigned char mfl_code)
{
    if (mfl_code >= MBEDTLS_SSL_MAX_FRAG_LEN_INVALID ||
        esp_ssl_mfl_code_to_length(mfl_code) > MBEDTLS_TLS_EXT_ADV_CONTENT_LEN) {
        return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
    }
...
void esp_mbedtls_ssl_conf_legacy_renegotiation(mbedtls_ssl_config *conf, int allow_legacy)
{
    conf->allow_legacy_renegotiation = allow_legacy;
}

#if defined(MBEDTLS_SSL_RENEGOTIATION)
void esp_mbedtls_ssl_conf_renegotiation(mbedtls_ssl_config *conf, int renegotiation)
{
    conf->disable_renegotiation = renegotiation;
}

void esp_mbedtls_ssl_conf_renegotiation_enforced(mbedtls_ssl_config *conf, int max_records)
{
    conf->renego_max_records = max_records;
}

void esp_mbedtls_ssl_conf_renegotiation_period(mbedtls_ssl_config *conf,
...
#if defined(MBEDTLS_SSL_CLI_C)
void esp_mbedtls_ssl_conf_session_tickets(mbedtls_ssl_config *conf, int use_tickets)
{
...
#if defined(MBEDTLS_SSL_PROTO_TLS1_3) && defined(MBEDTLS_SSL_SESSION_TICKETS)
void esp_mbedtls_ssl_conf_new_session_tickets(mbedtls_ssl_config *conf,
...
void esp_mbedtls_ssl_conf_session_tickets_cb(mbedtls_ssl_config *conf,
...
void esp_mbedtls_ssl_set_export_keys_cb(mbedtls_ssl_context *ssl,
...
void esp_mbedtls_ssl_conf_async_private_cb(
...
void *esp_mbedtls_ssl_conf_get_async_config_data(const mbedtls_ssl_config *conf)
{
    return conf->p_async_config_data;
}

void *esp_mbedtls_ssl_get_async_operation_data(const mbedtls_ssl_context *ssl)
{
    if (ssl->handshake == NULL) {
        return NULL;
    } else {
...
void esp_mbedtls_ssl_set_async_operation_data(mbedtls_ssl_context *ssl,
                                          void *ctx)
{
    if (ssl->handshake != NULL) {
 ...
 * SSL get accessors
 */
uint32_t esp_mbedtls_ssl_get_verify_result(const mbedtls_ssl_context *ssl)
{
    if (ssl->session != NULL) {
...
int esp_mbedtls_ssl_get_ciphersuite_id_from_ssl(const mbedtls_ssl_context *ssl)
{
...
const char *esp_mbedtls_ssl_get_ciphersuite(const mbedtls_ssl_context *ssl)
{
    if (ssl == NULL || ssl->session == NULL) {
        return NULL;
    }

    return esp_mbedtls_ssl_get_ciphersuite_name(ssl->session->ciphersuite);
}

const char *esp_mbedtls_ssl_get_version(const mbedtls_ssl_context *ssl)
{
#if defined(MBEDTLS_SSL_PROTO_DTLS)
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
...
size_t esp_mbedtls_ssl_get_output_record_size_limit(const mbedtls_ssl_context *ssl)
{
...
#if defined(MBEDTLS_SSL_MAX_FRAGMENT_LENGTH)
size_t esp_mbedtls_ssl_get_input_max_frag_len(const mbedtls_ssl_context *ssl)
{
    ...
    if (ssl->conf->endpoint == MBEDTLS_SSL_IS_CLIENT &&
        ...
        return esp_ssl_mfl_code_to_length(ssl->conf->mfl_code);
    }
#endif

    /* Check if a smaller max length was negotiated */
    if (ssl->session_out != NULL) {
        read_mfl = esp_ssl_mfl_code_to_length(ssl->session_out->mfl_code);
        if (read_mfl < max_len) {
            max_len = read_mfl;
        }
    }
    ...
    /* During a handshake, use the value being negotiated */
    if (ssl->session_negotiate != NULL) {
        read_mfl = esp_ssl_mfl_code_to_length(ssl->session_negotiate->mfl_code);
        if (read_mfl < max_len) {
            max_len = read_mfl;
        }
    }
...
size_t esp_mbedtls_ssl_get_output_max_frag_len(const mbedtls_ssl_context *ssl)
{
    size_t max_len;
     ...
     */
    max_len = esp_ssl_mfl_code_to_length(ssl->conf->mfl_code);

    /* Check if a smaller max length was negotiated */
    if (ssl->session_out != NULL &&
        esp_ssl_mfl_code_to_length(ssl->session_out->mfl_code) < max_len) {
        max_len = esp_ssl_mfl_code_to_length(ssl->session_out->mfl_code);
    }

    /* During a handshake, use the value being negotiated */
    if (ssl->session_negotiate != NULL &&
        esp_ssl_mfl_code_to_length(ssl->session_negotiate->mfl_code) < max_len) {
        max_len = esp_ssl_mfl_code_to_length(ssl->session_negotiate->mfl_code);
    }
...
#if defined(MBEDTLS_SSL_PROTO_DTLS)
size_t esp_mbedtls_ssl_get_current_mtu(const mbedtls_ssl_context *ssl)
{
...
int esp_mbedtls_ssl_get_max_out_record_payload(const mbedtls_ssl_context *ssl)
{
...
#if defined(MBEDTLS_SSL_MAX_FRAGMENT_LENGTH)
    const size_t mfl = esp_mbedtls_ssl_get_output_max_frag_len(ssl);
...
#if defined(MBEDTLS_SSL_RECORD_SIZE_LIMIT)
    const size_t record_size_limit = esp_mbedtls_ssl_get_output_record_size_limit(ssl);
    ...
    if (ssl->transform_out != NULL &&
         ...
         * in esp_mbedtls_ssl_encrypt_buf(), compute the maximum length for
...
#if defined(MBEDTLS_SSL_PROTO_DTLS)
    if (esp_mbedtls_ssl_get_current_mtu(ssl) != 0) {
        const size_t mtu = esp_mbedtls_ssl_get_current_mtu(ssl);
        const int ret = esp_mbedtls_ssl_get_record_expansion(ssl);
...
int esp_mbedtls_ssl_get_max_in_record_payload(const mbedtls_ssl_context *ssl)
{
    size_t max_len = MBEDTLS_SSL_IN_CONTENT_LEN;
...
#if defined(MBEDTLS_SSL_MAX_FRAGMENT_LENGTH)
    const size_t mfl = esp_mbedtls_ssl_get_input_max_frag_len(ssl);
...
#if defined(MBEDTLS_X509_CRT_PARSE_C)
const mbedtls_x509_crt *esp_mbedtls_ssl_get_peer_cert(const mbedtls_ssl_context *ssl)
{
    if (ssl == NULL || ssl->session == NULL) {
        return NULL;
    }
...
#if defined(MBEDTLS_SSL_CLI_C)
int esp_mbedtls_ssl_get_session(const mbedtls_ssl_context *ssl,
                            mbedtls_ssl_session *dst)
{
    ...
    /* Since Mbed TLS 3.0, esp_mbedtls_ssl_get_session() is no longer
     ...
     * which will be achieved by calling esp_mbedtls_ssl_get_session()
    ...
    ret = esp_mbedtls_ssl_session_copy(dst, ssl->session);
    if (ret != 0) {
        return ret;
    }
 ...
 *
 * For more detail, see the description of esp_ssl_session_save().
 */
static size_t esp_ssl_tls12_session_save(const mbedtls_ssl_session *session,
                                     unsigned char *buf,
                                     size_t buf_len)
{
    unsigned char *p = buf;
                                  ...
                                  size_t len)
{
    ...
    if (cert_len != 0) {
        ...
        session->peer_cert = esp_mbedtls_calloc(1, sizeof(mbedtls_x509_crt));

        if (session->peer_cert == NULL) {
            return MBEDTLS_ERR_SSL_ALLOC_FAILED;
        }

        esp_mbedtls_x509_crt_init(session->peer_cert);

        if ((ret = esp_mbedtls_x509_crt_parse_der(session->peer_cert,
                                              p, cert_len)) != 0) {
            esp_mbedtls_x509_crt_free(session->peer_cert);
            esp_mbedtls_free(session->peer_cert);
            session->peer_cert = NULL;
    ...
    if (session->peer_cert_digest_len != 0) {
        const mbedtls_md_info_t *md_info =
            esp_mbedtls_md_info_from_type(session->peer_cert_digest_type);
        if (md_info == NULL) {
            return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
        }
        if (session->peer_cert_digest_len != esp_mbedtls_md_get_size(md_info)) {
            return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
        }
        ...
        session->peer_cert_digest =
            esp_mbedtls_calloc(1, session->peer_cert_digest_len);
        if (session->peer_cert_digest == NULL) {
            return MBEDTLS_ERR_SSL_ALLOC_FAILED;
        }
    ...
    if (session->endpoint == MBEDTLS_SSL_IS_CLIENT) {
        ...
        if (session->ticket_len != 0) {
            ...
            session->ticket = esp_mbedtls_calloc(1, session->ticket_len);
            if (session->ticket == NULL) {
                return MBEDTLS_ERR_SSL_ALLOC_FAILED;
            }
...
#if defined(MBEDTLS_SSL_SESSION_TICKETS)
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_session_save(const mbedtls_ssl_session *session,
                                  unsigned char *buf,
                                  size_t buf_len,
                                  size_t *olen)
{
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_session_load(mbedtls_ssl_session *session,
                                  const unsigned char *buf,
                                  size_t len)
{
    const unsigned char *p = buf;
    ...
    if (session->endpoint == MBEDTLS_SSL_IS_SERVER) {
        ...
        if (alpn_len > 0) {
            int ret = esp_mbedtls_ssl_session_set_ticket_alpn(session, (char *) p);
            if (ret != 0) {
                return ret;
            }
            p += alpn_len;
    ...
    if (session->endpoint == MBEDTLS_SSL_IS_CLIENT) {
        ...
        if (hostname_len > 0) {
            session->hostname = esp_mbedtls_calloc(1, hostname_len);
...
static int esp_ssl_tls13_session_load(const mbedtls_ssl_session *session,
                                  unsigned char *buf,
                                  size_t buf_len)
{
    ((void) session);
...
static const unsigned char esp_ssl_serialized_session_header[] = {
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_session_save(const mbedtls_ssl_session *session,
                            ...
                            size_t *olen)
{
    ...
    if (!omit_header) {
         ...
         */
        used += sizeof(esp_ssl_serialized_session_header);

        if (used <= buf_len) {
            memcpy(p, esp_ssl_serialized_session_header,
                   sizeof(esp_ssl_serialized_session_header));
            p += sizeof(esp_ssl_serialized_session_header);
        }
    }
...
#if defined(MBEDTLS_SSL_PROTO_TLS1_2)
        case MBEDTLS_SSL_VERSION_TLS1_2:
            used += esp_ssl_tls12_session_save(session, p, remaining_len);
            break;
#endif /* MBEDTLS_SSL_PROTO_TLS1_2 */

#if defined(MBEDTLS_SSL_PROTO_TLS1_3)
        case MBEDTLS_SSL_VERSION_TLS1_3:
            ret = esp_ssl_tls13_session_save(session, p, remaining_len, &out_len);
...
/*
 * Public wrapper for esp_ssl_session_save()
 */
int esp_mbedtls_ssl_session_save(const mbedtls_ssl_session *session,
                             unsigned char *buf,
                             size_t buf_len,
                             size_t *olen)
{
    return esp_ssl_session_save(session, 0, buf, buf_len, olen);
}

/*
 * Deserialize session, see esp_mbedtls_ssl_session_save() for format.
 *
 * This internal version is wrapped by a public function that cleans up in
 * case of error, and has an extra option omit_header.
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_session_load(mbedtls_ssl_session *session,
                            unsigned char omit_header,
                            const unsigned char *buf,
                            size_t len)
{
    ...
    if (!omit_header) {
        ...
        if ((size_t) (end - p) < sizeof(esp_ssl_serialized_session_header)) {
            return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
        }

        if (memcmp(p, esp_ssl_serialized_session_header,
                   sizeof(esp_ssl_serialized_session_header)) != 0) {
...
#if defined(MBEDTLS_SSL_PROTO_TLS1_2)
        case MBEDTLS_SSL_VERSION_TLS1_2:
            return esp_ssl_tls12_session_load(session, p, remaining_len);
#endif /* MBEDTLS_SSL_PROTO_TLS1_2 */

#if defined(MBEDTLS_SSL_PROTO_TLS1_3)
        case MBEDTLS_SSL_VERSION_TLS1_3:
            return esp_ssl_tls13_session_load(session, p, remaining_len);
#endif /* MBEDTLS_SSL_PROTO_TLS1_3 */
 ...
 */
int esp_mbedtls_ssl_session_load(mbedtls_ssl_session *session,
                             const unsigned char *buf,
                             size_t len)
{
    int ret = esp_ssl_session_load(session, 0, buf, len);

    if (ret != 0) {
        esp_mbedtls_ssl_session_free(session);
    }
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_prepare_handshake_step(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
     ...
     * the case here by calling `esp_mbedtls_ssl_flush_output()`. The function may
     ...
     * `esp_mbedtls_ssl_handle_pending_alert` in case an error that triggered an
     * alert occurred.
     */
    if ((ret = esp_mbedtls_ssl_flush_output(ssl)) != 0) {
        return ret;
    }
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM &&
        ...
        if ((ret = esp_mbedtls_ssl_flight_transmit(ssl)) != 0) {
            return ret;
        }
    }
#endif /* MBEDTLS_SSL_PROTO_DTLS */
...
int esp_mbedtls_ssl_handshake_step(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    ret = esp_ssl_prepare_handshake_step(ssl);
    if (ret != 0) {
        return ret;
    }

    ret = esp_mbedtls_ssl_handle_pending_alert(ssl);
    if (ret != 0) {
        goto cleanup;
    }
    ...
    if (ssl->conf->endpoint == MBEDTLS_SSL_IS_CLIENT) {
        MBEDTLS_SSL_DEBUG_MSG(2, ("client state: %s",
                                  esp_mbedtls_ssl_states_str((mbedtls_ssl_states) ssl->state)));
        ...
        switch (ssl->state) {
            ...
            case MBEDTLS_SSL_CLIENT_HELLO:
                ret = esp_mbedtls_ssl_write_client_hello(ssl);
                break;

            default:
#if defined(MBEDTLS_SSL_PROTO_TLS1_2) && defined(MBEDTLS_SSL_PROTO_TLS1_3)
                if (ssl->tls_version == MBEDTLS_SSL_VERSION_TLS1_3) {
                    ret = esp_mbedtls_ssl_tls13_handshake_client_step(ssl);
                } else {
                    ret = esp_mbedtls_ssl_handshake_client_step(ssl);
                }
 ...
 */
int esp_mbedtls_ssl_handshake(mbedtls_ssl_context *ssl)
{
    int ret = 0;
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM &&
        ...
        MBEDTLS_SSL_DEBUG_MSG(1, ("You must use "
                                  "esp_mbedtls_ssl_set_timer_cb() for DTLS"));
        return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
    }
#endif /* MBEDTLS_SSL_PROTO_DTLS */
    ...
    while (ssl->state != MBEDTLS_SSL_HANDSHAKE_OVER) {
        ret = esp_mbedtls_ssl_handshake_step(ssl);
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_write_hello_request(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
 ...
 * - any side: calling esp_mbedtls_ssl_renegotiate(),
 * - client: receiving a HelloRequest during esp_mbedtls_ssl_read(),
 * - server: receiving any handshake message on server during esp_mbedtls_ssl_read() after
 *   the initial handshake is completed.
 * If the handshake doesn't complete due to waiting for I/O, it will continue
 * during the next calls to esp_mbedtls_ssl_renegotiate() or esp_mbedtls_ssl_read() respectively.
 */
int esp_mbedtls_ssl_start_renegotiation(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    if ((ret = esp_mbedtls_ssl_handshake(ssl)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_handshake", ret);
        return ret;
    }
 ...
 */
int esp_mbedtls_ssl_renegotiate(mbedtls_ssl_context *ssl)
{
    ...
    if (ssl->conf->endpoint == MBEDTLS_SSL_IS_SERVER) {
        if (esp_mbedtls_ssl_is_handshake_over(ssl) == 0) {
            return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
        }
        ...
        if (ssl->out_left != 0) {
            return esp_mbedtls_ssl_flush_output(ssl);
        }

        return esp_ssl_write_hello_request(ssl);
    }
#endif /* MBEDTLS_SSL_SRV_C */
    ...
    if (ssl->renego_status != MBEDTLS_SSL_RENEGOTIATION_IN_PROGRESS) {
        ...
        if ((ret = esp_mbedtls_ssl_start_renegotiation(ssl)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_start_renegotiation", ret);
            return ret;
        }
    } else {
        if ((ret = esp_mbedtls_ssl_handshake(ssl)) != 0) {
...
void esp_mbedtls_ssl_handshake_free(mbedtls_ssl_context *ssl)
{
    ...
    if (ssl->handshake->group_list_heap_allocated) {
        esp_mbedtls_free((void *) handshake->group_list);
    }
    ...
    if (ssl->handshake->sig_algs_heap_allocated) {
        esp_mbedtls_free((void *) handshake->sig_algs);
    }
    handshake->sig_algs = NULL;
#endif /* MBEDTLS_DEPRECATED_REMOVED */
#if defined(MBEDTLS_SSL_PROTO_TLS1_3)
    if (ssl->handshake->certificate_request_context) {
        esp_mbedtls_free((void *) handshake->certificate_request_context);
    }
#endif /* MBEDTLS_SSL_PROTO_TLS1_3 */
#endif /* MBEDTLS_SSL_HANDSHAKE_WITH_CERT_ENABLED */
...
#if defined(MBEDTLS_MD_CAN_SHA256)
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    esp_psa_hash_abort(&handshake->fin_sha256_psa);
#else
    esp_mbedtls_md_free(&handshake->fin_sha256);
#endif
#endif
#if defined(MBEDTLS_MD_CAN_SHA384)
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    esp_psa_hash_abort(&handshake->fin_sha384_psa);
#else
    esp_mbedtls_md_free(&handshake->fin_sha384);
#endif
#endif

#if defined(MBEDTLS_DHM_C)
    esp_mbedtls_dhm_free(&handshake->dhm_ctx);
#endif
#if !defined(MBEDTLS_USE_PSA_CRYPTO) && \
    defined(MBEDTLS_KEY_EXCHANGE_SOME_ECDH_OR_ECDHE_1_2_ENABLED)
    esp_mbedtls_ecdh_free(&handshake->ecdh_ctx);
#endif

#if defined(MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    esp_psa_pake_abort(&handshake->psa_pake_ctx);
    /*
    ...
    if (!mbedtls_svc_key_id_is_null(handshake->psa_pake_password)) {
        esp_psa_destroy_key(handshake->psa_pake_password);
    }
    handshake->psa_pake_password = MBEDTLS_SVC_KEY_ID_INIT;
#else
    esp_mbedtls_ecjpake_free(&handshake->ecjpake_ctx);
#endif /* MBEDTLS_USE_PSA_CRYPTO */
#if defined(MBEDTLS_SSL_CLI_C)
    esp_mbedtls_free(handshake->ecjpake_cache);
    handshake->ecjpake_cache = NULL;
    handshake->ecjpake_cache_len = 0;
#endif
#endif
    ...
    esp_mbedtls_free((void *) handshake->curves_tls_id);
#endif
    ...
    if (handshake->psk != NULL) {
        esp_mbedtls_zeroize_and_free(handshake->psk, handshake->psk_len);
    }
#endif /* MBEDTLS_USE_PSA_CRYPTO */
#endif /* MBEDTLS_SSL_HANDSHAKE_WITH_PSK_ENABLED */
     ...
     */
    esp_ssl_key_cert_free(handshake->sni_key_cert);
#endif /* MBEDTLS_X509_CRT_PARSE_C && MBEDTLS_SSL_SERVER_NAME_INDICATION */

#if defined(MBEDTLS_SSL_ECP_RESTARTABLE_ENABLED)
    esp_mbedtls_x509_crt_restart_free(&handshake->ecrs_ctx);
    if (handshake->ecrs_peer_cert != NULL) {
        esp_mbedtls_x509_crt_free(handshake->ecrs_peer_cert);
        esp_mbedtls_free(handshake->ecrs_peer_cert);
    }
#endif

#if defined(MBEDTLS_X509_CRT_PARSE_C) &&        \
    !defined(MBEDTLS_SSL_KEEP_PEER_CERTIFICATE)
    esp_mbedtls_pk_free(&handshake->peer_pubkey);
#endif /* MBEDTLS_X509_CRT_PARSE_C && !MBEDTLS_SSL_KEEP_PEER_CERTIFICATE */

#if defined(MBEDTLS_SSL_CLI_C) && \
    (defined(MBEDTLS_SSL_PROTO_DTLS) || defined(MBEDTLS_SSL_PROTO_TLS1_3))
    esp_mbedtls_free(handshake->cookie);
#endif /* MBEDTLS_SSL_CLI_C &&
          ( MBEDTLS_SSL_PROTO_DTLS || MBEDTLS_SSL_PROTO_TLS1_3 ) */

#if defined(MBEDTLS_SSL_PROTO_DTLS)
    esp_mbedtls_ssl_flight_free(handshake->flight);
    esp_mbedtls_ssl_buffering_free(ssl);
#endif /* MBEDTLS_SSL_PROTO_DTLS */

#if defined(MBEDTLS_KEY_EXCHANGE_SOME_XXDH_PSA_ANY_ENABLED)
    if (handshake->xxdh_psa_privkey_is_external == 0) {
        esp_psa_destroy_key(handshake->xxdh_psa_privkey);
    }
#endif /* MBEDTLS_KEY_EXCHANGE_SOME_XXDH_PSA_ANY_ENABLED */

#if defined(MBEDTLS_SSL_PROTO_TLS1_3)
    esp_mbedtls_ssl_transform_free(handshake->transform_handshake);
    esp_mbedtls_free(handshake->transform_handshake);
#if defined(MBEDTLS_SSL_EARLY_DATA)
    esp_mbedtls_ssl_transform_free(handshake->transform_earlydata);
    esp_mbedtls_free(handshake->transform_earlydata);
#endif
#endif /* MBEDTLS_SSL_PROTO_TLS1_3 */
    ...
    esp_mbedtls_platform_zeroize(handshake,
                             sizeof(mbedtls_ssl_handshake_params));
}

void esp_mbedtls_ssl_session_free(mbedtls_ssl_session *session)
{
    if (session == NULL) {
        return;
    }
...
#if defined(MBEDTLS_X509_CRT_PARSE_C)
    esp_ssl_clear_peer_cert(session);
#endif

#if defined(MBEDTLS_SSL_SESSION_TICKETS) && defined(MBEDTLS_SSL_CLI_C)
#if defined(MBEDTLS_SSL_PROTO_TLS1_3) && \
    defined(MBEDTLS_SSL_SERVER_NAME_INDICATION)
    esp_mbedtls_free(session->hostname);
#endif
    esp_mbedtls_free(session->ticket);
#endif

#if defined(MBEDTLS_SSL_EARLY_DATA) && defined(MBEDTLS_SSL_ALPN) && \
    defined(MBEDTLS_SSL_SRV_C)
    esp_mbedtls_free(session->ticket_alpn);
#endif

    esp_mbedtls_platform_zeroize(session, sizeof(mbedtls_ssl_session));
}
...
static const unsigned char esp_ssl_serialized_context_header[] = {
    MBEDTLS_VERSION_MAJOR,
    MBEDTLS_VERSION_MINOR,
    MBEDTLS_VERSION_PATCH,
    MBEDTLS_BYTE_1(SSL_SERIALIZED_SESSION_CONFIG_BITFLAG),
 ...
 *  opaque session<1..2^32-1>;  // see esp_mbedtls_ssl_session_save()
 ...
 */
int esp_mbedtls_ssl_context_save(mbedtls_ssl_context *ssl,
                             unsigned char *buf,
                             size_t buf_len,
                             size_t *olen)
{
    ...
    if (esp_mbedtls_ssl_check_pending(ssl) != 0) {
     ...
     */
    used += sizeof(esp_ssl_serialized_context_header);

    if (used <= buf_len) {
        memcpy(p, esp_ssl_serialized_context_header,
               sizeof(esp_ssl_serialized_context_header));
        p += sizeof(esp_ssl_serialized_context_header);
    }

    /*
     * Session (length + data)
     */
    ret = esp_ssl_session_save(ssl->session, 1, NULL, 0, &session_len);
    ...
    if (used <= buf_len) {
        ...
        ret = esp_ssl_session_save(ssl->session, 1,
...
/*
 * Deserialize context, see esp_mbedtls_ssl_context_save() for format.
 *
 * This internal version is wrapped by a public function that cleans up in
 * case of error.
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_context_load(mbedtls_ssl_context *ssl,
                            const unsigned char *buf,
                            size_t len)
{
    const unsigned char *p = buf;
     ...
     */
    if ((size_t) (end - p) < sizeof(esp_ssl_serialized_context_header)) {
        return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
    }

    if (memcmp(p, esp_ssl_serialized_context_header,
               sizeof(esp_ssl_serialized_context_header)) != 0) {
        return MBEDTLS_ERR_SSL_VERSION_MISMATCH;
    }
    p += sizeof(esp_ssl_serialized_context_header);
    ...
    /* This has been allocated by esp_ssl_handshake_init(), called by
     * by either esp_mbedtls_ssl_session_reset_int() or esp_mbedtls_ssl_setup(). */
    ...
    ret = esp_ssl_session_load(ssl->session, 1, p, session_len);
    if (ret != 0) {
        esp_mbedtls_ssl_session_free(ssl->session);
        return ret;
    }
...
#if defined(MBEDTLS_SSL_PROTO_TLS1_2)
    prf_func = esp_ssl_tls12prf_from_cs(ssl->session->ciphersuite);
    if (prf_func == NULL) {
        return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
    }
    ...
    ret = esp_ssl_tls12_populate_transform(ssl->transform,
     ...
     *
     * Most of them already set to the correct value by esp_mbedtls_ssl_init() and
    ...
    esp_mbedtls_ssl_update_out_pointers(ssl, ssl->transform);
     ...
     * by calling esp_mbedtls_ssl_handshake_wrapup_free_hs_transform()
     * inappropriately. */
    if (ssl->handshake != NULL) {
        esp_mbedtls_ssl_handshake_free(ssl);
        esp_mbedtls_free(ssl->handshake);
 ...
 */
int esp_mbedtls_ssl_context_load(mbedtls_ssl_context *context,
                             const unsigned char *buf,
                             size_t len)
{
    int ret = esp_ssl_context_load(context, buf, len);

    if (ret != 0) {
        esp_mbedtls_ssl_free(context);
    }
 ...
 * Free an SSL context
 */
void esp_mbedtls_ssl_free(mbedtls_ssl_context *ssl)
{
    if (ssl == NULL) {
        return;
    }
    ...
    if (ssl->out_buf != NULL) {
        ...
        esp_mbedtls_zeroize_and_free(ssl->out_buf, out_buf_len);
        ssl->out_buf = NULL;
    }
    ...
    if (ssl->in_buf != NULL) {
        ...
        esp_mbedtls_zeroize_and_free(ssl->in_buf, in_buf_len);
        ssl->in_buf = NULL;
    }
    ...
    esp_mbedtls_platform_zeroize(ssl, sizeof(mbedtls_ssl_context));
}

/*
 * Initialize mbedtls_ssl_config
 */
void esp_mbedtls_ssl_config_init(mbedtls_ssl_config *conf)
{
    memset(conf, 0, sizeof(mbedtls_ssl_config));
}

/* The selection should be the same as esp_mbedtls_x509_crt_profile_default in
 * x509_crt.c, plus Montgomery curves for ECDHE. Here, the order matters:
 * curves with a lower resource usage come first.
 * See the documentation of esp_mbedtls_ssl_conf_curves() for what we promise
 * about this list.
 */
static const uint16_t esp_ssl_preset_default_groups[] = {
...
static const int esp_ssl_preset_suiteb_ciphersuites[] = {
 ...
 */
static const uint16_t esp_ssl_preset_default_sig_algs[] = {
...
/* NOTICE: see above */
#if defined(MBEDTLS_SSL_PROTO_TLS1_2)
static uint16_t esp_ssl_tls12_preset_default_sig_algs[] = {
...
/* NOTICE: see above */
static const uint16_t esp_ssl_preset_suiteb_sig_algs[] = {
...
/* NOTICE: see above */
#if defined(MBEDTLS_SSL_PROTO_TLS1_2)
static uint16_t esp_ssl_tls12_preset_suiteb_sig_algs[] = {
...
static const uint16_t esp_ssl_preset_suiteb_groups[] = {
#if defined(MBEDTLS_ECP_HAVE_SECP256R1)
    MBEDTLS_SSL_IANA_TLS_GROUP_SECP256R1,
#endif
#if defined(MBEDTLS_ECP_HAVE_SECP384R1)
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_check_no_sig_alg_duplication(const uint16_t *sig_algs)
{
    size_t i, j;
    int ret = 0;
    ...
    for (i = 0; sig_algs[i] != MBEDTLS_TLS_SIG_NONE; i++) {
        for (j = 0; j < i; j++) {
            ...
            esp_mbedtls_printf(" entry(%04x,%" MBEDTLS_PRINTF_SIZET
 ...
 */
int esp_mbedtls_ssl_config_defaults(mbedtls_ssl_config *conf,
                                int endpoint, int transport, int preset)
{
...
#if defined(MBEDTLS_DEBUG_C) && defined(MBEDTLS_SSL_HANDSHAKE_WITH_CERT_ENABLED)
    if (esp_ssl_check_no_sig_alg_duplication(esp_ssl_preset_suiteb_sig_algs)) {
        esp_mbedtls_printf("esp_ssl_preset_suiteb_sig_algs has duplicated entries\n");
        return MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    }

    if (esp_ssl_check_no_sig_alg_duplication(esp_ssl_preset_default_sig_algs)) {
        esp_mbedtls_printf("esp_ssl_preset_default_sig_algs has duplicated entries\n");
        return MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    }

#if defined(MBEDTLS_SSL_PROTO_TLS1_2)
    if (esp_ssl_check_no_sig_alg_duplication(esp_ssl_tls12_preset_suiteb_sig_algs)) {
        esp_mbedtls_printf("esp_ssl_tls12_preset_suiteb_sig_algs has duplicated entries\n");
        return MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    }

    if (esp_ssl_check_no_sig_alg_duplication(esp_ssl_tls12_preset_default_sig_algs)) {
        esp_mbedtls_printf("esp_ssl_tls12_preset_default_sig_algs has duplicated entries\n");
        return MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    }
#endif /* MBEDTLS_SSL_PROTO_TLS1_2 */
#endif /* MBEDTLS_DEBUG_C && MBEDTLS_SSL_HANDSHAKE_WITH_CERT_ENABLED */
    ...
    esp_mbedtls_ssl_conf_endpoint(conf, endpoint);
    esp_mbedtls_ssl_conf_transport(conf, transport);
...
#if defined(MBEDTLS_SSL_DTLS_HELLO_VERIFY) && defined(MBEDTLS_SSL_SRV_C)
    conf->f_cookie_write = esp_ssl_cookie_write_dummy;
    conf->f_cookie_check = esp_ssl_cookie_check_dummy;
#endif
    ...
    if (endpoint == MBEDTLS_SSL_IS_SERVER) {
        ...
        if ((ret = esp_mbedtls_ssl_conf_dh_param_bin(conf,
                                                 dhm_p, sizeof(dhm_p),
...
#if defined(MBEDTLS_SSL_EARLY_DATA)
    esp_mbedtls_ssl_conf_early_data(conf, MBEDTLS_SSL_EARLY_DATA_DISABLED);
#if defined(MBEDTLS_SSL_SRV_C)
    esp_mbedtls_ssl_conf_max_early_data_size(conf, MBEDTLS_SSL_MAX_EARLY_DATA_SIZE);
#endif
#endif /* MBEDTLS_SSL_EARLY_DATA */

#if defined(MBEDTLS_SSL_SRV_C) && defined(MBEDTLS_SSL_SESSION_TICKETS)
    esp_mbedtls_ssl_conf_new_session_tickets(
    ...
    switch (preset) {
        ...
        case MBEDTLS_SSL_PRESET_SUITEB:
            ...
            conf->ciphersuite_list = esp_ssl_preset_suiteb_ciphersuites;

#if defined(MBEDTLS_X509_CRT_PARSE_C)
            conf->cert_profile = &esp_mbedtls_x509_crt_profile_suiteb;
#endif
...
#endif /* MBEDTLS_SSL_PROTO_TLS1_2 */
            conf->sig_algs = esp_ssl_preset_suiteb_sig_algs;
#endif /* MBEDTLS_SSL_HANDSHAKE_WITH_CERT_ENABLED */

#if defined(MBEDTLS_ECP_C) && !defined(MBEDTLS_DEPRECATED_REMOVED)
            conf->curve_list = NULL;
#endif
            conf->group_list = esp_ssl_preset_suiteb_groups;
            break;
        ...
        default:
            ...
            conf->ciphersuite_list = esp_mbedtls_ssl_list_ciphersuites();

#if defined(MBEDTLS_X509_CRT_PARSE_C)
            conf->cert_profile = &esp_mbedtls_x509_crt_profile_default;
#endif
...
#endif /* MBEDTLS_SSL_PROTO_TLS1_2 */
            conf->sig_algs = esp_ssl_preset_default_sig_algs;
#endif /* MBEDTLS_SSL_HANDSHAKE_WITH_CERT_ENABLED */

#if defined(MBEDTLS_ECP_C) && !defined(MBEDTLS_DEPRECATED_REMOVED)
            conf->curve_list = NULL;
#endif
            conf->group_list = esp_ssl_preset_default_groups;
 ...
 */
void esp_mbedtls_ssl_config_free(mbedtls_ssl_config *conf)
{
#if defined(MBEDTLS_DHM_C)
    esp_mbedtls_mpi_free(&conf->dhm_P);
    esp_mbedtls_mpi_free(&conf->dhm_G);
    ...
    if (conf->psk_identity != NULL) {
        esp_mbedtls_zeroize_and_free(conf->psk_identity, conf->psk_identity_len);
        conf->psk_identity = NULL;
        conf->psk_identity_len = 0;
    }
#endif /* MBEDTLS_SSL_HANDSHAKE_WITH_PSK_ENABLED */
...
#if defined(MBEDTLS_X509_CRT_PARSE_C)
    esp_ssl_key_cert_free(conf->key_cert);
#endif

    esp_mbedtls_platform_zeroize(conf, sizeof(mbedtls_ssl_config));
}
 ...
 */
unsigned char esp_mbedtls_ssl_sig_from_pk(mbedtls_pk_context *pk)
{
#if defined(MBEDTLS_RSA_C)
...
#endif
#if defined(MBEDTLS_KEY_EXCHANGE_ECDSA_CERT_REQ_ANY_ALLOWED_ENABLED)
    if (esp_mbedtls_pk_can_do(pk, MBEDTLS_PK_ECDSA)) {
...
unsigned char esp_mbedtls_ssl_sig_from_pk_alg(mbedtls_pk_type_t type)
{
    switch (type) {
        case MBEDTLS_PK_RSA:
            return MBEDTLS_SSL_SIG_RSA;
...
mbedtls_pk_type_t esp_mbedtls_ssl_pk_alg_from_sig(unsigned char sig)
{
    switch (sig) {
#if defined(MBEDTLS_RSA_C)
 ...
 */
mbedtls_md_type_t esp_mbedtls_ssl_md_alg_from_hash(unsigned char hash)
{
    switch (hash) {
 ...
 */
unsigned char esp_mbedtls_ssl_hash_from_md_alg(int md)
{
    switch (md) {
#if defined(MBEDTLS_MD_CAN_MD5)
        case MBEDTLS_MD_MD5:
...
#if defined(MBEDTLS_PK_HAVE_ECC_KEYS)
/*
 * Same as esp_mbedtls_ssl_check_curve_tls_id() but with a mbedtls_ecp_group_id.
 */
int esp_mbedtls_ssl_check_curve(const mbedtls_ssl_context *ssl, mbedtls_ecp_group_id grp_id)
{
    uint16_t tls_id = esp_mbedtls_ssl_get_tls_id_from_ecp_group_id(grp_id);

    if (tls_id == 0) {
        return -1;
    }

    return esp_mbedtls_ssl_check_curve_tls_id(ssl, tls_id);
}
#endif /* MBEDTLS_PK_HAVE_ECC_KEYS */
...
int esp_mbedtls_ssl_get_psa_curve_info_from_tls_id(uint16_t tls_id,
                                               psa_key_type_t *type,
                                               size_t *bits)
{
    for (int i = 0; esp_tls_id_match_table[i].tls_id != 0; i++) {
        if (esp_tls_id_match_table[i].tls_id == tls_id) {
            if (type != NULL) {
                *type = PSA_KEY_TYPE_ECC_KEY_PAIR(esp_tls_id_match_table[i].psa_family);
            }
            if (bits != NULL) {
                *bits = esp_tls_id_match_table[i].bits;
            }
            return PSA_SUCCESS;
        }
    }
...
mbedtls_ecp_group_id esp_mbedtls_ssl_get_ecp_group_id_from_tls_id(uint16_t tls_id)
{
    for (int i = 0; esp_tls_id_match_table[i].tls_id != 0; i++) {
        if (esp_tls_id_match_table[i].tls_id == tls_id) {
            return esp_tls_id_match_table[i].ecp_group_id;
        }
    }
...
uint16_t esp_mbedtls_ssl_get_tls_id_from_ecp_group_id(mbedtls_ecp_group_id grp_id)
{
    for (int i = 0; esp_tls_id_match_table[i].ecp_group_id != MBEDTLS_ECP_DP_NONE;
         i++) {
        if (esp_tls_id_match_table[i].ecp_group_id == grp_id) {
            return esp_tls_id_match_table[i].tls_id;
        }
    }
...
const char *esp_mbedtls_ssl_get_curve_name_from_tls_id(uint16_t tls_id)
{
    for (int i = 0; esp_tls_id_curve_name_table[i].tls_id != 0; i++) {
        if (esp_tls_id_curve_name_table[i].tls_id == tls_id) {
            return esp_tls_id_curve_name_table[i].name;
        }
    }
...
#if defined(MBEDTLS_X509_CRT_PARSE_C)
int esp_mbedtls_ssl_check_cert_usage(const mbedtls_x509_crt *cert,
                                 ...
                                 uint32_t *flags)
{
    ...
    if (cert_endpoint == MBEDTLS_SSL_IS_SERVER) {
        ...
        /* Client auth: we only implement rsa_sign and esp_mbedtls_ecdsa_sign for now */
        usage = MBEDTLS_X509_KU_DIGITAL_SIGNATURE;
    }

    if (esp_mbedtls_x509_crt_check_key_usage(cert, usage) != 0) {
    ...
    if (esp_mbedtls_x509_crt_check_extended_key_usage(cert, ext_oid, ext_len) != 0) {
...
#if defined(MBEDTLS_USE_PSA_CRYPTO)
int esp_mbedtls_ssl_get_handshake_transcript(mbedtls_ssl_context *ssl,
                                         ...
                                         size_t *olen)
{
    ...
    *olen = 0;
    ...
    status = esp_psa_hash_clone(hash_operation_to_clone, &hash_operation);
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    status = esp_psa_hash_finish(&hash_operation, dst, dst_len, olen);
    if (status != PSA_SUCCESS) {
        goto exit;
    }
...
#if defined(MBEDTLS_MD_CAN_SHA384)
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_get_handshake_transcript_sha384(mbedtls_ssl_context *ssl,
                                               unsigned char *dst,
                                               size_t dst_len,
                                               size_t *olen)
{
    ...
    esp_mbedtls_md_init(&sha384);
    ret = esp_mbedtls_md_setup(&sha384, esp_mbedtls_md_info_from_type(MBEDTLS_MD_SHA384), 0);
    if (ret != 0) {
        goto exit;
    }
    ret = esp_mbedtls_md_clone(&sha384, &ssl->handshake->fin_sha384);
    if (ret != 0) {
        goto exit;
    }

    if ((ret = esp_mbedtls_md_finish(&sha384, dst)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_md_finish", ret);
        goto exit;
    }
...
#if defined(MBEDTLS_MD_CAN_SHA256)
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_get_handshake_transcript_sha256(mbedtls_ssl_context *ssl,
                                               unsigned char *dst,
                                               size_t dst_len,
                                               size_t *olen)
{
    ...
    esp_mbedtls_md_init(&sha256);
    ret = esp_mbedtls_md_setup(&sha256, esp_mbedtls_md_info_from_type(MBEDTLS_MD_SHA256), 0);
    if (ret != 0) {
        goto exit;
    }
    ret = esp_mbedtls_md_clone(&sha256, &ssl->handshake->fin_sha256);
    if (ret != 0) {
        goto exit;
    }

    if ((ret = esp_mbedtls_md_finish(&sha256, dst)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_md_finish", ret);
        goto exit;
    }
...
#if defined(MBEDTLS_MD_CAN_SHA384)
        case MBEDTLS_MD_SHA384:
            return esp_ssl_get_handshake_transcript_sha384(ssl, dst, dst_len, olen);
#endif /* MBEDTLS_MD_CAN_SHA384*/

#if defined(MBEDTLS_MD_CAN_SHA256)
        case MBEDTLS_MD_SHA256:
            return esp_ssl_get_handshake_transcript_sha256(ssl, dst, dst_len, olen);
#endif /* MBEDTLS_MD_CAN_SHA256*/
...
#if defined(MBEDTLS_SSL_HANDSHAKE_WITH_CERT_ENABLED)
/* esp_mbedtls_ssl_parse_sig_alg_ext()
 *
 ...
 *
 */
int esp_mbedtls_ssl_parse_sig_alg_ext(mbedtls_ssl_context *ssl,
                                  const unsigned char *buf,
...
static psa_status_t esp_setup_psa_key_derivation(psa_key_derivation_operation_t *derivation,
                                             ...
                                             size_t capacity)
{
    ...
    status = esp_psa_key_derivation_setup(derivation, alg);
    if (status != PSA_SUCCESS) {
        return status;
    }

    if (PSA_ALG_IS_TLS12_PRF(alg) || PSA_ALG_IS_TLS12_PSK_TO_MS(alg)) {
        status = esp_psa_key_derivation_input_bytes(derivation,
        ...
        if (mbedtls_svc_key_id_is_null(key)) {
            status = esp_psa_key_derivation_input_bytes(
                derivation, PSA_KEY_DERIVATION_INPUT_SECRET,
                raw_psk, raw_psk_length);
        } else {
            status = esp_psa_key_derivation_input_key(
    ...
    status = esp_psa_key_derivation_set_capacity(derivation, capacity);
    if (status != PSA_SUCCESS) {
        return status;
    }
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_tls_prf_generic(mbedtls_md_type_t md_type,
                           const unsigned char *secret, size_t slen,
                           const char *label,
                           const unsigned char *random, size_t rlen,
                           unsigned char *dstbuf, size_t dlen)
{
     ...
     * keep master_key=0, which esp_setup_psa_key_derivation() understands
    ...
    if (slen != 0) {
        ...
        status = esp_psa_import_key(&key_attributes, secret, slen, &master_key);
        if (status != PSA_SUCCESS) {
            return MBEDTLS_ERR_SSL_HW_ACCEL_FAILED;
        }
    }

    status = esp_setup_psa_key_derivation(&derivation,
                                      master_key, alg,
                                      NULL, 0,
                                      random, rlen,
                                      ...
                                      dlen);
    if (status != PSA_SUCCESS) {
        esp_psa_key_derivation_abort(&derivation);
        esp_psa_destroy_key(master_key);
        return MBEDTLS_ERR_SSL_HW_ACCEL_FAILED;
    }

    status = esp_psa_key_derivation_output_bytes(&derivation, dstbuf, dlen);
    if (status != PSA_SUCCESS) {
        esp_psa_key_derivation_abort(&derivation);
        esp_psa_destroy_key(master_key);
        return MBEDTLS_ERR_SSL_HW_ACCEL_FAILED;
    ...
    status = esp_psa_key_derivation_abort(&derivation);
    if (status != PSA_SUCCESS) {
        esp_psa_destroy_key(master_key);
        return MBEDTLS_ERR_SSL_HW_ACCEL_FAILED;
    }
    ...
    if (!mbedtls_svc_key_id_is_null(master_key)) {
        status = esp_psa_destroy_key(master_key);
    }
    if (status != PSA_SUCCESS) {
        return MBEDTLS_ERR_SSL_HW_ACCEL_FAILED;
    }
                           ...
                           unsigned char *dstbuf, size_t dlen)
{
    ...
    if ((md_info = esp_mbedtls_md_info_from_type(md_type)) == NULL) {
        return MBEDTLS_ERR_SSL_INTERNAL_ERROR;
    }

    md_len = esp_mbedtls_md_get_size(md_info);

    tmp_len = md_len + strlen(label) + rlen;
    tmp = esp_mbedtls_calloc(1, tmp_len);
    if (tmp == NULL) {
        ret = MBEDTLS_ERR_SSL_ALLOC_FAILED;
        goto exit;
    }
     ...
     */
    if ((ret = esp_mbedtls_md_setup(&md_ctx, md_info, 1)) != 0) {
        goto exit;
    }

    ret = esp_mbedtls_md_hmac_starts(&md_ctx, secret, slen);
    if (ret != 0) {
        goto exit;
    }
    ret = esp_mbedtls_md_hmac_update(&md_ctx, tmp + md_len, nb);
    if (ret != 0) {
        goto exit;
    }
    ret = esp_mbedtls_md_hmac_finish(&md_ctx, tmp);
    if (ret != 0) {
        goto exit;
    }

    for (i = 0; i < dlen; i += md_len) {
        ret = esp_mbedtls_md_hmac_reset(&md_ctx);
        if (ret != 0) {
            goto exit;
        }
        ret = esp_mbedtls_md_hmac_update(&md_ctx, tmp, md_len + nb);
        ...
        ret = esp_mbedtls_md_hmac_finish(&md_ctx, h_i);
        if (ret != 0) {
            goto exit;
        }
        ...
        ret = esp_mbedtls_md_hmac_update(&md_ctx, tmp, md_len);
        if (ret != 0) {
            goto exit;
        }
        ret = esp_mbedtls_md_hmac_finish(&md_ctx, tmp);
...
exit:
    ...
    if (tmp != NULL) {
        esp_mbedtls_platform_zeroize(tmp, tmp_len);
    }

    esp_mbedtls_platform_zeroize(h_i, sizeof(h_i));

    esp_mbedtls_free(tmp);
...
static int esp_tls_prf_sha256(const unsigned char *secret, size_t slen,
                          ...
                          unsigned char *dstbuf, size_t dlen)
{
    return esp_tls_prf_generic(MBEDTLS_MD_SHA256, secret, slen,
...
static int esp_tls_prf_sha384(const unsigned char *secret, size_t slen,
                          ...
                          unsigned char *dstbuf, size_t dlen)
{
    return esp_tls_prf_generic(MBEDTLS_MD_SHA384, secret, slen,
                           label, random, rlen, dstbuf, dlen);
}
#endif /* MBEDTLS_MD_CAN_SHA384*/
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_set_handshake_prfs(mbedtls_ssl_handshake_params *handshake,
                                  mbedtls_md_type_t hash)
{
#if defined(MBEDTLS_MD_CAN_SHA384)
    if (hash == MBEDTLS_MD_SHA384) {
        handshake->tls_prf = esp_tls_prf_sha384;
        handshake->calc_verify = esp_ssl_calc_verify_tls_sha384;
        handshake->calc_finished = esp_ssl_calc_finished_tls_sha384;
    } else
#endif
#if defined(MBEDTLS_MD_CAN_SHA256)
    {
        (void) hash;
        handshake->tls_prf = esp_tls_prf_sha256;
        handshake->calc_verify = esp_ssl_calc_verify_tls_sha256;
        handshake->calc_finished = esp_ssl_calc_finished_tls_sha256;
    }
#else
    {
        (void) handshake;
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_compute_master(mbedtls_ssl_handshake_params *handshake,
                              ...
                              const mbedtls_ssl_context *ssl)
{
    ...
    if (esp_mbedtls_ssl_ciphersuite_uses_psk(handshake->ciphersuite_info) == 1) {
        ...
        status = esp_setup_psa_key_derivation(&derivation, psk, alg,
        ...
        status = esp_psa_key_derivation_output_bytes(&derivation,
                                                 master,
                                                 master_secret_len);
        if (status != PSA_SUCCESS) {
            esp_psa_key_derivation_abort(&derivation);
...
#endif
    {
        ...
        if (handshake->ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECJPAKE) {
            ...
            status = esp_psa_key_derivation_set_capacity(&derivation,
            ...
            status = esp_psa_pake_get_implicit_key(&handshake->psa_pake_ctx,
                                               &derivation);
            if (status != PSA_SUCCESS) {
                esp_psa_key_derivation_abort(&derivation);
                return MBEDTLS_ERR_SSL_HW_ACCEL_FAILED;
        ...
        esp_mbedtls_platform_zeroize(handshake->premaster,
...
int esp_mbedtls_ssl_derive_keys(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    ret = esp_ssl_set_handshake_prfs(ssl->handshake,
                                 (mbedtls_md_type_t) ciphersuite_info->mac);
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_ssl_set_handshake_prfs", ret);
        return ret;
    }

    /* Compute master secret if needed */
    ret = esp_ssl_compute_master(ssl->handshake,
                             ssl->session_negotiate->master,
                             ssl);
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_ssl_compute_master", ret);
        return ret;
    }
     ...
     * - key derivation wants server+client (RFC 5246 6.3) */
    {
        ...
        esp_mbedtls_platform_zeroize(tmp, sizeof(tmp));
    }

    /* Populate transform structure */
    ret = esp_ssl_tls12_populate_transform(ssl->transform_negotiate,
                                       ...
                                       ssl);
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_ssl_tls12_populate_transform", ret);
        return ret;
    }

    /* We no longer need Server/ClientHello.random values */
    esp_mbedtls_platform_zeroize(ssl->handshake->randbytes,
...
int esp_mbedtls_ssl_set_calc_verify_md(mbedtls_ssl_context *ssl, int md)
{
    switch (md) {
#if defined(MBEDTLS_MD_CAN_SHA384)
        case MBEDTLS_SSL_HASH_SHA384:
            ssl->handshake->calc_verify = esp_ssl_calc_verify_tls_sha384;
            break;
#endif
#if defined(MBEDTLS_MD_CAN_SHA256)
        case MBEDTLS_SSL_HASH_SHA256:
            ssl->handshake->calc_verify = esp_ssl_calc_verify_tls_sha256;
            break;
#endif
        default:
            return -1;
...
#if defined(MBEDTLS_USE_PSA_CRYPTO)
static int esp_ssl_calc_verify_tls_psa(const mbedtls_ssl_context *ssl,
                                   ...
                                   size_t *hlen)
{
    ...
    status = esp_psa_hash_clone(hs_op, &cloned_op);
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    status = esp_psa_hash_finish(&cloned_op, hash, buffer_size, hlen);
    if (status != PSA_SUCCESS) {
        goto exit;
    }
...
exit:
    esp_psa_hash_abort(&cloned_op);
    return esp_mbedtls_md_error_from_psa(status);
}
#else
static int esp_ssl_calc_verify_tls_legacy(const mbedtls_ssl_context *ssl,
                                      ...
                                      size_t *hlen)
{
    ...
    esp_mbedtls_md_init(&cloned_ctx);
    ...
    ret = esp_mbedtls_md_setup(&cloned_ctx, esp_mbedtls_md_info_from_ctx(hs_ctx), 0);
    if (ret != 0) {
        goto exit;
    }
    ret = esp_mbedtls_md_clone(&cloned_ctx, hs_ctx);
    if (ret != 0) {
        goto exit;
    }

    ret = esp_mbedtls_md_finish(&cloned_ctx, hash);
    if (ret != 0) {
        goto exit;
    }

    *hlen = esp_mbedtls_md_get_size(esp_mbedtls_md_info_from_ctx(hs_ctx));

    MBEDTLS_SSL_DEBUG_BUF(3, "calculated verify result", hash, *hlen);
    MBEDTLS_SSL_DEBUG_MSG(2, ("<= calc verify"));

exit:
    esp_mbedtls_md_free(&cloned_ctx);
    return ret;
}
#endif /* MBEDTLS_USE_PSA_CRYPTO */

#if defined(MBEDTLS_MD_CAN_SHA256)
int esp_ssl_calc_verify_tls_sha256(const mbedtls_ssl_context *ssl,
                               unsigned char *hash,
                               size_t *hlen)
{
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    return esp_ssl_calc_verify_tls_psa(ssl, &ssl->handshake->fin_sha256_psa, 32,
                                   hash, hlen);
#else
    return esp_ssl_calc_verify_tls_legacy(ssl, &ssl->handshake->fin_sha256,
                                      hash, hlen);
#endif /* MBEDTLS_USE_PSA_CRYPTO */
}
#endif /* MBEDTLS_MD_CAN_SHA256 */
...
#if defined(MBEDTLS_MD_CAN_SHA384)
int esp_ssl_calc_verify_tls_sha384(const mbedtls_ssl_context *ssl,
                               unsigned char *hash,
                               size_t *hlen)
{
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    return esp_ssl_calc_verify_tls_psa(ssl, &ssl->handshake->fin_sha384_psa, 48,
                                   hash, hlen);
#else
    return esp_ssl_calc_verify_tls_legacy(ssl, &ssl->handshake->fin_sha384,
                                      hash, hlen);
#endif /* MBEDTLS_USE_PSA_CRYPTO */
}
#endif /* MBEDTLS_MD_CAN_SHA384 */
...
#if !defined(MBEDTLS_USE_PSA_CRYPTO) &&                      \
    defined(MBEDTLS_KEY_EXCHANGE_SOME_PSK_ENABLED)
int esp_mbedtls_ssl_psk_derive_premaster(mbedtls_ssl_context *ssl, mbedtls_key_exchange_type_t key_ex)
{
    ...
    if (key_ex == MBEDTLS_KEY_EXCHANGE_DHE_PSK) {
        ...
        if ((ret = esp_mbedtls_dhm_calc_secret(&ssl->handshake->dhm_ctx,
                                           p + 2, (size_t) (end - (p + 2)), &len,
                                           ssl->conf->f_rng, ssl->conf->p_rng)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_dhm_calc_secret", ret);
            return ret;
        }
        MBEDTLS_PUT_UINT16_BE(len, p, 0);
        p += 2 + len;

        MBEDTLS_SSL_DEBUG_MPI(3, "DHM: esp_K ", &ssl->handshake->dhm_ctx.esp_K);
    } else
    ...
    if (key_ex == MBEDTLS_KEY_EXCHANGE_ECDHE_PSK) {
        ...
        if ((ret = esp_mbedtls_ecdh_calc_secret(&ssl->handshake->ecdh_ctx, &zlen,
                                            p + 2, (size_t) (end - (p + 2)),
                                            ssl->conf->f_rng, ssl->conf->p_rng)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ecdh_calc_secret", ret);
            return ret;
        }
...
#if defined(MBEDTLS_SSL_PROTO_DTLS)
int esp_mbedtls_ssl_resend_hello_request(mbedtls_ssl_context *ssl)
{
...
int esp_mbedtls_ssl_write_certificate(mbedtls_ssl_context *ssl)
{
    const mbedtls_ssl_ciphersuite_t *ciphersuite_info =
        ssl->handshake->ciphersuite_info;
    ...
    if (!esp_mbedtls_ssl_ciphersuite_uses_srv_cert(ciphersuite_info)) {
...
int esp_mbedtls_ssl_parse_certificate(mbedtls_ssl_context *ssl)
{
    const mbedtls_ssl_ciphersuite_t *ciphersuite_info =
        ssl->handshake->ciphersuite_info;
...
#if defined(MBEDTLS_SSL_KEEP_PEER_CERTIFICATE)
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_check_peer_crt_unchanged(mbedtls_ssl_context *ssl,
                                        ...
                                        size_t crt_buf_len)
{
    ...
    mbedtls_md_info_t const * const digest_info =
        esp_mbedtls_md_info_from_type(peer_cert_digest_type);
    ...
    digest_len = esp_mbedtls_md_get_size(digest_info);
    if (digest_len > MBEDTLS_SSL_PEER_CERT_DIGEST_MAX_LEN) {
        return -1;
    }

    ret = esp_mbedtls_md(digest_info, crt_buf, crt_buf_len, tmp_digest);
    if (ret != 0) {
        return -1;
    }
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_parse_certificate_chain(mbedtls_ssl_context *ssl,
                                       mbedtls_x509_crt *chain)
{
    ...
    while (i < ssl->in_hslen) {
        ...
        if (i + 3 > ssl->in_hslen) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("bad certificate message"));
            esp_mbedtls_ssl_send_alert_message(ssl,
        ...
        if (crt_cnt++ == 0 &&
            ...
            if (esp_ssl_check_peer_crt_unchanged(ssl,
                                             &ssl->in_msg[i],
                                             n) != 0) {
            ...
            esp_ssl_clear_peer_cert(ssl->session);
        }
#endif /* MBEDTLS_SSL_RENEGOTIATION && MBEDTLS_SSL_CLI_C */

        /* Parse the next certificate in the chain. */
#if defined(MBEDTLS_SSL_KEEP_PEER_CERTIFICATE)
        ret = esp_mbedtls_x509_crt_parse_der(chain, ssl->in_msg + i, n);
#else
        /* If we don't need to store the CRT chain permanently, parse
         * it in-place from the input buffer instead of making a copy. */
        ret = esp_mbedtls_x509_crt_parse_der_nocopy(chain, ssl->in_msg + i, n);
#endif /* MBEDTLS_SSL_KEEP_PEER_CERTIFICATE */
        switch (ret) {
            case 0: /*ok*/
...
crt_parse_der_failed:
                esp_mbedtls_ssl_send_alert_message(ssl, MBEDTLS_SSL_ALERT_LEVEL_FATAL, alert);
                MBEDTLS_SSL_DEBUG_RET(1, " esp_mbedtls_x509_crt_parse_der", ret);
                return ret;
        }
...
#if defined(MBEDTLS_SSL_SRV_C)
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_srv_check_client_no_crt_notification(mbedtls_ssl_context *ssl)
{
    if (ssl->conf->endpoint == MBEDTLS_SSL_IS_CLIENT) {
        return -1;
    }
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_parse_certificate_coordinate(mbedtls_ssl_context *ssl,
                                            int authmode)
{
    const mbedtls_ssl_ciphersuite_t *ciphersuite_info =
        ssl->handshake->ciphersuite_info;
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_parse_certificate_verify(mbedtls_ssl_context *ssl,
                                        int authmode,
                                        mbedtls_x509_crt *chain,
                                        void *rs_ctx)
{
    ...
    if (ssl->conf->f_ca_cb != NULL) {
        ...
        ret = esp_mbedtls_x509_crt_verify_with_ca_cb(
            chain,
            ssl->conf->f_ca_cb,
            ssl->conf->p_ca_cb,
...
#endif /* MBEDTLS_X509_TRUSTED_CERTIFICATE_CALLBACK */
    {
        ...
        ret = esp_mbedtls_x509_crt_verify_restartable(
            chain,
            ca_chain, ca_crl,
            ssl->conf->cert_profile,
            ssl->hostname,
...
#if defined(MBEDTLS_PK_HAVE_ECC_KEYS)
    {
        ...
        if (esp_mbedtls_pk_can_do(pk, MBEDTLS_PK_ECKEY)) {
            /* and in the unlikely case the above assumption no longer holds
             * we are making sure that pk_ec() here does not return a NULL
             */
            mbedtls_ecp_group_id grp_id = esp_mbedtls_pk_get_ec_group_id(pk);
            if (grp_id == MBEDTLS_ECP_DP_NONE) {
                MBEDTLS_SSL_DEBUG_MSG(1, ("invalid group ID"));
                return MBEDTLS_ERR_SSL_INTERNAL_ERROR;
            }
            if (esp_mbedtls_ssl_check_curve(ssl, grp_id) != 0) {
                ssl->session_negotiate->verify_result |=
    ...
    if (esp_mbedtls_ssl_check_cert_usage(chain,
                                     ciphersuite_info,
                                     !ssl->conf->endpoint,
    ...
    /* esp_mbedtls_x509_crt_verify_with_profile is supposed to report a
...
#if !defined(MBEDTLS_SSL_KEEP_PEER_CERTIFICATE)
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_remember_peer_crt_digest(mbedtls_ssl_context *ssl,
                                        unsigned char *start, size_t len)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    /* Remember digest of the peer's end-CRT. */
    ssl->session_negotiate->peer_cert_digest =
        esp_mbedtls_calloc(1, MBEDTLS_SSL_PEER_CERT_DIGEST_DFL_LEN);
    ...
    ret = esp_mbedtls_md(esp_mbedtls_md_info_from_type(
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_remember_peer_pubkey(mbedtls_ssl_context *ssl,
                                    unsigned char *start, size_t len)
{
    ...
    esp_mbedtls_pk_init(&ssl->handshake->peer_pubkey);
    ret = esp_mbedtls_pk_parse_subpubkey(&start, end,
...
int esp_mbedtls_ssl_parse_certificate(mbedtls_ssl_context *ssl)
{
    ...
    crt_expected = esp_ssl_parse_certificate_coordinate(ssl, authmode);
    ...
    if ((ret = esp_mbedtls_ssl_read_record(ssl, 1)) != 0) {
        /* esp_mbedtls_ssl_read_record may have sent an alert already. We
           let it decide whether to alert. */
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_read_record", ret);
        goto exit;
    }

#if defined(MBEDTLS_SSL_SRV_C)
    if (esp_ssl_srv_check_client_no_crt_notification(ssl) == 0) {
    ...
    esp_ssl_clear_peer_cert(ssl->session_negotiate);

    chain = esp_mbedtls_calloc(1, sizeof(mbedtls_x509_crt));
    if (chain == NULL) {
    ...
    esp_mbedtls_x509_crt_init(chain);

    ret = esp_ssl_parse_certificate_chain(ssl, chain);
    if (ret != 0) {
        goto exit;
    }
...
crt_verify:
    ...
    ret = esp_ssl_parse_certificate_verify(ssl, authmode,
                                       chain, rs_ctx);
    if (ret != 0) {
        goto exit;
    }
...
#if !defined(MBEDTLS_SSL_KEEP_PEER_CERTIFICATE)
    {
        ...
        esp_mbedtls_x509_crt_free(chain);
        esp_mbedtls_free(chain);
        chain = NULL;

        ret = esp_ssl_remember_peer_crt_digest(ssl, crt_start, crt_len);
        if (ret != 0) {
            goto exit;
        }

        ret = esp_ssl_remember_peer_pubkey(ssl, pk_start, pk_len);
        if (ret != 0) {
            goto exit;
        }
    }
...
static int esp_ssl_calc_finished_tls_generic(mbedtls_ssl_context *ssl, void *ctx,
                                         ...
                                         unsigned char *buf, int from)
{
    ...
    status = esp_psa_hash_finish(&cloned_op, padbuf, hlen, &hash_size);
    if (status != PSA_SUCCESS) {
        goto exit;
    }
    ...
    ret = esp_mbedtls_md_finish(&cloned_ctx, padbuf);
    if (ret != 0) {
        goto exit;
    }
#endif /* MBEDTLS_USE_PSA_CRYPTO */
    ...
    esp_mbedtls_platform_zeroize(padbuf, hlen);
...
#if defined(MBEDTLS_MD_CAN_SHA256)
static int esp_ssl_calc_finished_tls_sha256(
    mbedtls_ssl_context *ssl, unsigned char *buf, int from)
{
    unsigned char padbuf[32];
    return esp_ssl_calc_finished_tls_generic(ssl,
#if defined(MBEDTLS_USE_PSA_CRYPTO)
...
#if defined(MBEDTLS_MD_CAN_SHA384)
static int esp_ssl_calc_finished_tls_sha384(
    mbedtls_ssl_context *ssl, unsigned char *buf, int from)
{
...
void esp_mbedtls_ssl_handshake_wrapup_free_hs_transform(mbedtls_ssl_context *ssl)
{
...
#endif
    esp_mbedtls_ssl_handshake_wrapup_free_hs_transform(ssl);
...
int esp_mbedtls_ssl_write_finished(mbedtls_ssl_context *ssl)
{
    int ret;
    ...
    esp_mbedtls_ssl_update_out_pointers(ssl, ssl->transform_negotiate);
...
#if defined(MBEDTLS_SSL_PROTO_DTLS)
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
        esp_mbedtls_ssl_send_flight_completed(ssl);
    }
#endif
...
#if defined(MBEDTLS_SSL_PROTO_DTLS)
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM &&
        (ret = esp_mbedtls_ssl_flight_transmit(ssl)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_flight_transmit", ret);
        return ret;
    }
#endif
...
int esp_mbedtls_ssl_parse_finished(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
...
#if defined(MBEDTLS_SSL_PROTO_DTLS)
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
        esp_mbedtls_ssl_recv_flight_completed(ssl);
    }
#endif
...
exit:
    esp_mbedtls_platform_zeroize(buf, hash_len);
    return ret;
}
 ...
 * (Duplicates bits of logic from esp_ssl_set_handshake_prfs().)
 */
static tls_prf_fn esp_ssl_tls12prf_from_cs(int ciphersuite_id)
{
    const mbedtls_ssl_ciphersuite_t * const ciphersuite_info =
        esp_mbedtls_ssl_ciphersuite_from_id(ciphersuite_id);
#if defined(MBEDTLS_MD_CAN_SHA384)
    if (ciphersuite_info != NULL && ciphersuite_info->mac == MBEDTLS_MD_SHA384) {
        return esp_tls_prf_sha384;
    } else
#endif
#if defined(MBEDTLS_MD_CAN_SHA256)
    {
        if (ciphersuite_info != NULL && ciphersuite_info->mac == MBEDTLS_MD_SHA256) {
            return esp_tls_prf_sha256;
        }
    }
#endif
#if !defined(MBEDTLS_MD_CAN_SHA384) && \
...
static mbedtls_tls_prf_types esp_tls_prf_get_type(mbedtls_ssl_tls_prf_cb *tls_prf)
{
    ((void) tls_prf);
#if defined(MBEDTLS_MD_CAN_SHA384)
    if (tls_prf == esp_tls_prf_sha384) {
        return MBEDTLS_SSL_TLS_PRF_SHA384;
    } else
#endif
#if defined(MBEDTLS_MD_CAN_SHA256)
    if (tls_prf == esp_tls_prf_sha256) {
 ...
 *      [in] must be just initialised with esp_mbedtls_ssl_transform_init()
                                        ...
                                        const mbedtls_ssl_context *ssl)
{
     ...
     */
    ciphersuite_info = esp_mbedtls_ssl_ciphersuite_from_id(ciphersuite);
    ...
    ssl_mode = esp_mbedtls_ssl_get_mode_from_ciphersuite(
#if defined(MBEDTLS_SSL_SOME_SUITES_USE_CBC_ETM)
        encrypt_then_mac,
#endif /* MBEDTLS_SSL_SOME_SUITES_USE_CBC_ETM */
        ciphersuite_info);
...
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    if ((status = esp_mbedtls_ssl_cipher_to_psa((mbedtls_cipher_type_t) ciphersuite_info->cipher,
                                            transform->taglen,
                                            &alg,
                                            &key_type,
                                            &key_bits)) != PSA_SUCCESS) {
        ret = PSA_TO_MBEDTLS_ERR(status);
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_cipher_to_psa", ret);
        goto end;
    }
#else
    cipher_info = esp_mbedtls_cipher_info_from_type((mbedtls_cipher_type_t) ciphersuite_info->cipher);
    if (cipher_info == NULL) {
...
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    mac_alg = esp_mbedtls_md_psa_alg_from_type((mbedtls_md_type_t) ciphersuite_info->mac);
    if (mac_alg == 0) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("esp_mbedtls_md_psa_alg_from_type for %u not found",
                                  (unsigned) ciphersuite_info->mac));
        return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
    }
#else
    md_info = esp_mbedtls_md_info_from_type((mbedtls_md_type_t) ciphersuite_info->mac);
    if (md_info == NULL) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("esp_mbedtls_md info for %u not found",
                                  (unsigned) ciphersuite_info->mac));
        return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
    }
#endif /* MBEDTLS_USE_PSA_CRYPTO */
    ...
    MBEDTLS_SSL_DEBUG_MSG(3, ("ciphersuite = %s",
                              esp_mbedtls_ssl_get_ciphersuite_name(ciphersuite)));
...
#else
    keylen = esp_mbedtls_cipher_info_get_key_bitlen(cipher_info) / 8;
#endif
...
#else
        is_chachapoly = (esp_mbedtls_cipher_info_get_mode(cipher_info)
    ...
    if (ssl_mode == MBEDTLS_SSL_MODE_STREAM ||
...
#else
        size_t block_size = esp_mbedtls_cipher_info_get_block_size(cipher_info);
#endif /* MBEDTLS_USE_PSA_CRYPTO */
        ...
        if ((ret = esp_mbedtls_md_setup(&transform->md_ctx_enc, md_info, 1)) != 0 ||
            (ret = esp_mbedtls_md_setup(&transform->md_ctx_dec, md_info, 1)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_md_setup", ret);
            goto end;
        }

        /* Get MAC length */
        mac_key_len = esp_mbedtls_md_get_size(md_info);
#endif /* MBEDTLS_USE_PSA_CRYPTO */
...
#else
        transform->ivlen = esp_mbedtls_cipher_info_get_iv_size(cipher_info);
#endif /* MBEDTLS_USE_PSA_CRYPTO */
    ...
    if (alg != MBEDTLS_SSL_NULL_CIPHER) {
        ...
        if ((status = esp_psa_import_key(&attributes,
                                     key1,
                                     PSA_BITS_TO_BYTES(key_bits),
                                     &transform->psa_key_enc)) != PSA_SUCCESS) {
            MBEDTLS_SSL_DEBUG_RET(3, "esp_psa_import_key", (int) status);
            ret = PSA_TO_MBEDTLS_ERR(status);
            MBEDTLS_SSL_DEBUG_RET(1, "esp_psa_import_key", ret);
            goto end;
        }
...
#else
    if ((ret = esp_mbedtls_cipher_setup(&transform->cipher_ctx_enc,
                                    cipher_info)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_cipher_setup", ret);
        goto end;
    }

    if ((ret = esp_mbedtls_cipher_setup(&transform->cipher_ctx_dec,
                                    cipher_info)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_cipher_setup", ret);
        goto end;
    }

    if ((ret = esp_mbedtls_cipher_setkey(&transform->cipher_ctx_enc, key1,
                                     (int) esp_mbedtls_cipher_info_get_key_bitlen(cipher_info),
                                     MBEDTLS_ENCRYPT)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_cipher_setkey", ret);
        goto end;
    }

    if ((ret = esp_mbedtls_cipher_setkey(&transform->cipher_ctx_dec, key2,
                                     (int) esp_mbedtls_cipher_info_get_key_bitlen(cipher_info),
                                     MBEDTLS_DECRYPT)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_cipher_setkey", ret);
        goto end;
    ...
    if (esp_mbedtls_cipher_info_get_mode(cipher_info) == MBEDTLS_MODE_CBC) {
        if ((ret = esp_mbedtls_cipher_set_padding_mode(&transform->cipher_ctx_enc,
                                                   MBEDTLS_PADDING_NONE)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_cipher_set_padding_mode", ret);
            goto end;
        }

        if ((ret = esp_mbedtls_cipher_set_padding_mode(&transform->cipher_ctx_dec,
                                                   MBEDTLS_PADDING_NONE)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_cipher_set_padding_mode", ret);
            goto end;
        }
    ...
    if (mac_key_len != 0) {
        ...
        if ((transform->psa_alg == MBEDTLS_SSL_NULL_CIPHER) ||
            ...
            /* esp_mbedtls_ct_hmac() requires the key to be exportable */
...
#else
        ret = esp_mbedtls_md_hmac_starts(&transform->md_ctx_enc, mac_enc, mac_key_len);
        if (ret != 0) {
            goto end;
        }
        ret = esp_mbedtls_md_hmac_starts(&transform->md_ctx_dec, mac_dec, mac_key_len);
        if (ret != 0) {
            goto end;
        }
#endif /* MBEDTLS_USE_PSA_CRYPTO */
    ...
    esp_mbedtls_platform_zeroize(keyblk, sizeof(keyblk));
    return ret;
}

#if defined(MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED) && \
    defined(MBEDTLS_USE_PSA_CRYPTO)
int esp_mbedtls_psa_ecjpake_read_round(
    ...
    size_t len, mbedtls_ecjpake_rounds_t round)
{
    ...
    for (; remaining_steps > 0; remaining_steps--) {
        for (psa_pake_step_t step = PSA_PAKE_STEP_KEY_SHARE;
            ...
            status = esp_psa_pake_input(pake_ctx, step,
...
int esp_mbedtls_psa_ecjpake_write_round(
    psa_pake_operation_t *pake_ctx,
    unsigned char *buf,
    ...
    mbedtls_ecjpake_rounds_t round)
{
    ...
    for (; remaining_steps > 0; remaining_steps--) {
        for (psa_pake_step_t step = PSA_PAKE_STEP_KEY_SHARE;
             ...
             * given by esp_psa_pake_output().
             */
            status = esp_psa_pake_output(pake_ctx, step,
                                     buf + output_offset + 1,
                                     len - output_offset - 1,
                                     &output_len);
            if (status != PSA_SUCCESS) {
...
#if defined(MBEDTLS_USE_PSA_CRYPTO)
int esp_mbedtls_ssl_get_key_exchange_md_tls1_2(mbedtls_ssl_context *ssl,
                                           ...
                                           mbedtls_md_type_t md_alg)
{
    psa_status_t status;
    psa_hash_operation_t hash_operation = PSA_HASH_OPERATION_INIT;
    psa_algorithm_t hash_alg = esp_mbedtls_md_psa_alg_from_type(md_alg);

    MBEDTLS_SSL_DEBUG_MSG(3, ("Perform PSA-based computation of digest of ServerKeyExchange"));

    if ((status = esp_psa_hash_setup(&hash_operation,
                                 hash_alg)) != PSA_SUCCESS) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_psa_hash_setup", status);
        goto exit;
    }

    if ((status = esp_psa_hash_update(&hash_operation, ssl->handshake->randbytes,
                                  64)) != PSA_SUCCESS) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_psa_hash_update", status);
        goto exit;
    }

    if ((status = esp_psa_hash_update(&hash_operation,
                                  data, data_len)) != PSA_SUCCESS) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_psa_hash_update", status);
        goto exit;
    }

    if ((status = esp_psa_hash_finish(&hash_operation, hash, PSA_HASH_MAX_SIZE,
                                  hashlen)) != PSA_SUCCESS) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_psa_hash_finish", status);
        goto exit;
    }
                                           ...
                                           mbedtls_md_type_t md_alg)
{
    ...
    *hashlen = esp_mbedtls_md_get_size(md_info);
     ...
     * };
     */
    if ((ret = esp_mbedtls_md_setup(&ctx, md_info, 0)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_md_setup", ret);
        goto exit;
    }
    if ((ret = esp_mbedtls_md_starts(&ctx)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_md_starts", ret);
        goto exit;
    }
    if ((ret = esp_mbedtls_md_update(&ctx, ssl->handshake->randbytes, 64)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_md_update", ret);
        goto exit;
    }
    if ((ret = esp_mbedtls_md_update(&ctx, data, data_len)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_md_update", ret);
        goto exit;
    }
    if ((ret = esp_mbedtls_md_finish(&ctx, hash)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_md_finish", ret);
        goto exit;
    }
...
unsigned int esp_mbedtls_ssl_tls12_get_preferred_hash_for_sig_alg(
    mbedtls_ssl_context *ssl,
    unsigned int sig_alg)
{
    ...
    for (i = 0; received_sig_algs[i] != MBEDTLS_TLS_SIG_NONE; i++) {
        ...
        mbedtls_md_type_t md_alg =
            esp_mbedtls_ssl_md_alg_from_hash((unsigned char) hash_alg_received);
        ...
        if (sig_alg == sig_alg_received) {
            ...
            if (ssl->handshake->key_cert && ssl->handshake->key_cert->key) {
                psa_algorithm_t psa_hash_alg =
                    esp_mbedtls_md_psa_alg_from_type(md_alg);

                if (sig_alg_received == MBEDTLS_SSL_SIG_ECDSA &&
                    !esp_mbedtls_pk_can_do_ext(ssl->handshake->key_cert->key,
...
int esp_mbedtls_ssl_validate_ciphersuite(
    const mbedtls_ssl_context *ssl,
    ...
    mbedtls_ssl_protocol_version max_tls_version)
{
...
#else
    if (suite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECJPAKE &&
        esp_mbedtls_ecjpake_check(&ssl->handshake->ecjpake_ctx) != 0)
#endif /* MBEDTLS_USE_PSA_CRYPTO */
    {
        return -1;
    }
    ...
    if (esp_mbedtls_ssl_ciphersuite_uses_psk(suite_info) &&
        esp_mbedtls_ssl_conf_has_static_psk(ssl->conf) == 0) {
        return -1;
    }
 ...
 *
 */
int esp_mbedtls_ssl_write_sig_alg_ext(mbedtls_ssl_context *ssl, unsigned char *buf,
...
#if defined(MBEDTLS_SSL_SERVER_NAME_INDICATION)
/*
 * esp_mbedtls_ssl_parse_server_name_ext
 *
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
int esp_mbedtls_ssl_parse_server_name_ext(mbedtls_ssl_context *ssl,
                                      const unsigned char *buf,
                                      const unsigned char *end)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
...
#if defined(MBEDTLS_SSL_ALPN)
MBEDTLS_CHECK_RETURN_CRITICAL
int esp_mbedtls_ssl_parse_alpn_ext(mbedtls_ssl_context *ssl,
                               const unsigned char *buf,
                               const unsigned char *end)
{
    const unsigned char *p = buf;
...
int esp_mbedtls_ssl_write_alpn_ext(mbedtls_ssl_context *ssl,
                               unsigned char *buf,
                               unsigned char *end,
                               size_t *out_len)
{
    ...
    *out_len = 7 + protocol_name_len;
     ...
     * than 255 bytes in `esp_mbedtls_ssl_conf_alpn_protocols`.
     */
...
int esp_mbedtls_ssl_session_set_hostname(mbedtls_ssl_session *session,
                                     const char *hostname)
{
    /* Initialize to suppress unnecessary compiler warning */
    size_t hostname_len = 0;
     ...
     * so we can free it safely */
    if (session->hostname != NULL) {
        esp_mbedtls_zeroize_and_free(session->hostname,
    ...
    if (hostname == NULL) {
        ...
        session->hostname = esp_mbedtls_calloc(1, hostname_len + 1);
        if (session->hostname == NULL) {
            return MBEDTLS_ERR_SSL_ALLOC_FAILED;
        }
...
int esp_mbedtls_ssl_session_set_ticket_alpn(mbedtls_ssl_session *session,
                                        const char *alpn)
{
    size_t alpn_len = 0;
    ...
    if (session->ticket_alpn != NULL) {
        esp_mbedtls_zeroize_and_free(session->ticket_alpn,
                                 strlen(session->ticket_alpn));
        session->ticket_alpn = NULL;
    }

    if (alpn != NULL) {
        session->ticket_alpn = esp_mbedtls_calloc(alpn_len + 1, 1);
        if (session->ticket_alpn == NULL) {
            return MBEDTLS_ERR_SSL_ALLOC_FAILED;
        }
