...
const char *esp_mbedtls_high_level_strerr(int error_code)
{
    ...
    /* Begin Auto-Generated Code. */
        ...
        case -(MBEDTLS_ERR_SSL_RECEIVED_EARLY_DATA):
            return( "SSL - * Early data has been received as part of an on-going handshake. This error code can be returned only on server side if and only if early data has been enabled by means of the esp_mbedtls_ssl_conf_early_data() API. This error code can then be returned by esp_mbedtls_ssl_handshake(), esp_mbedtls_ssl_handshake_step(), esp_mbedtls_ssl_read() or esp_mbedtls_ssl_write() if early data has been received as part of the handshake sequence they triggered. To read the early data, call esp_mbedtls_ssl_read_early_data()" );
...
const char *esp_mbedtls_low_level_strerr(int error_code)
{
...
void esp_mbedtls_strerror(int ret, char *buf, size_t buflen)
{
    size_t len;
    int use_ret;
    ...
    if (ret & 0xFF80) {
        ...
        high_level_error_description = esp_mbedtls_high_level_strerr(ret);

        if (high_level_error_description == NULL) {
            esp_mbedtls_snprintf(buf, buflen, "UNKNOWN ERROR CODE (%04X)", (unsigned int) use_ret);
        } else {
            esp_mbedtls_snprintf(buf, buflen, "%s", high_level_error_description);
        }
    ...
    if (len > 0) {
        ...
        esp_mbedtls_snprintf(buf + len, buflen - len, " : ");
    ...
    low_level_error_description = esp_mbedtls_low_level_strerr(ret);

    if (low_level_error_description == NULL) {
        esp_mbedtls_snprintf(buf, buflen, "UNKNOWN ERROR CODE (%04X)", (unsigned int) use_ret);
    } else {
        esp_mbedtls_snprintf(buf, buflen, "%s", low_level_error_description);
    }
}
...
void (*esp_mbedtls_test_hook_error_add)(int, int, const char *, int);
#endif
