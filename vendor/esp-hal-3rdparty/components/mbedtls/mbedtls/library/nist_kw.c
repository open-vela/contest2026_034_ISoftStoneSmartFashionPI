...
static const unsigned char esp_NIST_KW_ICV1[] = { 0xA6, 0xA6, 0xA6, 0xA6, 0xA6, 0xA6, 0xA6, 0xA6 };
/*! The 32-bit default integrity check value (ICV) for KWP mode. */
static const  unsigned char esp_NIST_KW_ICV2[] = { 0xA6, 0x59, 0x59, 0xA6 };

/*
 * Initialize context
 */
void esp_mbedtls_nist_kw_init(mbedtls_nist_kw_context *ctx)
{
    memset(ctx, 0, sizeof(mbedtls_nist_kw_context));
}

int esp_mbedtls_nist_kw_setkey(mbedtls_nist_kw_context *ctx,
                           ...
                           const int is_wrap)
{
    ...
    cipher_info = esp_mbedtls_cipher_info_from_values(cipher,
                                                  keybits,
                                                  MBEDTLS_MODE_ECB);
    if (cipher_info == NULL) {
 ...
 * Free context
 */
void esp_mbedtls_nist_kw_free(mbedtls_nist_kw_context *ctx)
{
    esp_mbedtls_cipher_free(&ctx->cipher_ctx);
    esp_mbedtls_platform_zeroize(ctx, sizeof(mbedtls_nist_kw_context));
}
 ...
 */
static void esp_calc_a_xor_t(unsigned char A[KW_SEMIBLOCK_LENGTH], uint64_t t)
{
    size_t i = 0;
 ...
 */
int esp_mbedtls_nist_kw_wrap(mbedtls_nist_kw_context *ctx,
                         ...
                         unsigned char *output, size_t *out_len, size_t out_size)
{
    ...
    *out_len = 0;
    ...
    if (mode == MBEDTLS_KW_MODE_KW) {
        ...
        memcpy(output, esp_NIST_KW_ICV1, KW_SEMIBLOCK_LENGTH);
        ...
        memcpy(output, esp_NIST_KW_ICV2, KW_SEMIBLOCK_LENGTH / 2);
    ...
    if (mode == MBEDTLS_KW_MODE_KWP
        ...
        ret = esp_mbedtls_cipher_update(&ctx->cipher_ctx,
...
cleanup:
    ...
    esp_mbedtls_platform_zeroize(inbuff, KW_SEMIBLOCK_LENGTH * 2);
    esp_mbedtls_platform_zeroize(outbuff, KW_SEMIBLOCK_LENGTH * 2);
 ...
 */
static int esp_unwrap(mbedtls_nist_kw_context *ctx,
...
cleanup:
    ...
    esp_mbedtls_platform_zeroize(inbuff, sizeof(inbuff));
    esp_mbedtls_platform_zeroize(outbuff, sizeof(outbuff));
 ...
 */
int esp_mbedtls_nist_kw_unwrap(mbedtls_nist_kw_context *ctx,
                           mbedtls_nist_kw_mode_t mode,
                           const unsigned char *input, size_t in_len,
                           unsigned char *output, size_t *out_len, size_t out_size)
{
    ...
    *out_len = 0;
    ...
    if (mode == MBEDTLS_KW_MODE_KW) {
        ...
        ret = esp_unwrap(ctx, input, in_len / KW_SEMIBLOCK_LENGTH,
                     A, output, out_len);
        if (ret != 0) {
            goto cleanup;
        }
        ...
        diff = esp_mbedtls_ct_memcmp(esp_NIST_KW_ICV1, A, KW_SEMIBLOCK_LENGTH);
        ...
        /* Check ICV in "constant-time" */
        diff = esp_mbedtls_ct_memcmp(esp_NIST_KW_ICV2, A, KW_SEMIBLOCK_LENGTH / 2);
        ...
        ret = esp_mbedtls_ct_error_if(esp_mbedtls_ct_uint_gt(padlen, 7),
                                  MBEDTLS_ERR_CIPHER_AUTH_FAILED, ret);
        padlen &= 7;

        /* Check padding in "constant-time" */
        const uint8_t zero[KW_SEMIBLOCK_LENGTH] = { 0 };
        diff = esp_mbedtls_ct_memcmp_partial(
...
cleanup:
    ...
    esp_mbedtls_platform_zeroize(&diff, sizeof(diff));
    esp_mbedtls_platform_zeroize(A, sizeof(A));
 ...
 */
static const unsigned int esp_key_len[] = {
    16,
...
static const unsigned char esp_kw_key[][32] = {
...
static const unsigned char esp_kw_msg[][40] = {
...
static const size_t esp_kw_msg_len[] = {
    16,
#if !defined(MBEDTLS_AES_ONLY_128_BIT_KEY_LENGTH)
    40,
    24
...
static const size_t esp_kw_out_len[] = {
    24,
#if !defined(MBEDTLS_AES_ONLY_128_BIT_KEY_LENGTH)
    48,
    32
...
static const unsigned char esp_kw_res[][48] = {
...
static const unsigned char esp_kwp_key[][32] = {
...
static const unsigned char esp_kwp_msg[][31] = {
...
static const size_t esp_kwp_msg_len[] = {
    9,
#if !defined(MBEDTLS_AES_ONLY_128_BIT_KEY_LENGTH)
    31,
    1
...
static const unsigned char esp_kwp_res[][48] = {
...
static const size_t esp_kwp_out_len[] = {
    24,
#if !defined(MBEDTLS_AES_ONLY_128_BIT_KEY_LENGTH)
    40,
    16
...
int esp_mbedtls_nist_kw_self_test(int verbose)
{
     ...
     */
    {
        static const int num_tests = sizeof(esp_kw_key) / sizeof(*esp_kw_key);

        for (i = 0; i < num_tests; i++) {
            if (verbose != 0) {
                esp_mbedtls_printf("  KW-AES-%u ", (unsigned int) esp_key_len[i] * 8);
            }

            ret = esp_mbedtls_nist_kw_setkey(&ctx, MBEDTLS_CIPHER_ID_AES,
                                         esp_kw_key[i], esp_key_len[i] * 8, 1);
            if (ret != 0) {
                if (verbose != 0) {
                    esp_mbedtls_printf("  KW: setup failed ");
                }

                goto end;
            }

            ret = esp_mbedtls_nist_kw_wrap(&ctx, MBEDTLS_KW_MODE_KW, esp_kw_msg[i],
                                       esp_kw_msg_len[i], out, &olen, sizeof(out));
            if (ret != 0 || esp_kw_out_len[i] != olen ||
                memcmp(out, esp_kw_res[i], esp_kw_out_len[i]) != 0) {
                if (verbose != 0) {
            ...
            if ((ret = esp_mbedtls_nist_kw_setkey(&ctx, MBEDTLS_CIPHER_ID_AES,
                                              esp_kw_key[i], esp_key_len[i] * 8, 0))
                != 0) {
                if (verbose != 0) {
                    esp_mbedtls_printf("  KW: setup failed ");
                }
            ...
            ret = esp_mbedtls_nist_kw_unwrap(&ctx, MBEDTLS_KW_MODE_KW,
                                         out, olen, out, &olen, sizeof(out));

            if (ret != 0 || olen != esp_kw_msg_len[i] ||
                memcmp(out, esp_kw_msg[i], esp_kw_msg_len[i]) != 0) {
                if (verbose != 0) {
                    esp_mbedtls_printf("failed\n");
                }
     ...
     */
    {
        static const int num_tests = sizeof(esp_kwp_key) / sizeof(*esp_kwp_key);

        for (i = 0; i < num_tests; i++) {
            olen = sizeof(out);
            if (verbose != 0) {
                esp_mbedtls_printf("  KWP-AES-%u ", (unsigned int) esp_key_len[i] * 8);
            }

            ret = esp_mbedtls_nist_kw_setkey(&ctx, MBEDTLS_CIPHER_ID_AES, esp_kwp_key[i],
                                         esp_key_len[i] * 8, 1);
            if (ret  != 0) {
                if (verbose != 0) {
                    esp_mbedtls_printf("  KWP: setup failed ");
                }

                goto end;
            }
            ret = esp_mbedtls_nist_kw_wrap(&ctx, MBEDTLS_KW_MODE_KWP, esp_kwp_msg[i],
                                       esp_kwp_msg_len[i], out, &olen, sizeof(out));

            if (ret != 0 || esp_kwp_out_len[i] != olen ||
                memcmp(out, esp_kwp_res[i], esp_kwp_out_len[i]) != 0) {
                if (verbose != 0) {
                    esp_mbedtls_printf("failed. ");
                }
            ...
            if ((ret = esp_mbedtls_nist_kw_setkey(&ctx, MBEDTLS_CIPHER_ID_AES,
                                              esp_kwp_key[i], esp_key_len[i] * 8, 0))
                != 0) {
                if (verbose != 0) {
                    esp_mbedtls_printf("  KWP: setup failed ");
                }
            ...
            ret = esp_mbedtls_nist_kw_unwrap(&ctx, MBEDTLS_KW_MODE_KWP, out,
                                         olen, out, &olen, sizeof(out));

            if (ret != 0 || olen != esp_kwp_msg_len[i] ||
                memcmp(out, esp_kwp_msg[i], esp_kwp_msg_len[i]) != 0) {
                if (verbose != 0) {
                    esp_mbedtls_printf("failed. ");
                }
