...
static const mbedtls_ssl_ciphersuite_t *esp_ssl_tls13_validate_peer_ciphersuite(
    ...
    unsigned int cipher_suite)
{
    ...
    ciphersuite_info = esp_mbedtls_ssl_ciphersuite_from_id(cipher_suite);
...
static void esp_ssl_tls13_select_ciphersuite(
    mbedtls_ssl_context *ssl,
    ...
    *selected_ciphersuite_info = NULL;
    ...
    for (const unsigned char *p = cipher_suites;
        ...
        const mbedtls_ssl_ciphersuite_t *info =
            esp_ssl_tls13_validate_peer_ciphersuite(ssl, id);
        if (info == NULL) {
            continue;
        }
        ...
        if (psk_ciphersuite_id != 0) {
            ...
            if (esp_mbedtls_md_psa_alg_from_type((mbedtls_md_type_t) info->mac) !=
                psk_hash_alg) {
                continue;
            }
        }
...
static int esp_ssl_tls13_parse_key_exchange_modes_ext(mbedtls_ssl_context *ssl,
 ...
 * esp_ssl_tls13_offered_psks_check_identity_match_ticket() and
 * esp_ssl_tls13_offered_psks_check_identity_match(). They are positive to
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_key_exchange_is_psk_available(mbedtls_ssl_context *ssl);
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_key_exchange_is_psk_ephemeral_available(mbedtls_ssl_context *ssl);

MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_offered_psks_check_identity_match_ticket(
    mbedtls_ssl_context *ssl,
    ...
    mbedtls_ssl_session *session)
{
     ...
     */
    ticket_buffer = esp_mbedtls_calloc(1, identity_len);
    ...
    esp_mbedtls_free(ticket_buffer);
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_offered_psks_check_identity_match(
    mbedtls_ssl_context *ssl,
    const unsigned char *identity,
    size_t identity_len,
    uint32_t obfuscated_ticket_age,
    ...
    mbedtls_ssl_session *session)
{
    ...
    *psk_type = MBEDTLS_SSL_TLS1_3_PSK_EXTERNAL;
...
#if defined(MBEDTLS_SSL_SESSION_TICKETS)
    ret = esp_ssl_tls13_offered_psks_check_identity_match_ticket(
        ssl, identity, identity_len, obfuscated_ticket_age, session);
    if (ret == SSL_TLS1_3_PSK_IDENTITY_MATCH) {
        *psk_type = MBEDTLS_SSL_TLS1_3_PSK_RESUMPTION;
        ret = esp_mbedtls_ssl_set_hs_psk(ssl,
                                     session->resumption_key,
                                     session->resumption_key_len);
        if (ret != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_set_hs_psk", ret);
            return ret;
        }
    ...
    if (ssl->conf->psk_identity != NULL &&
        ...
        esp_mbedtls_ct_memcmp(ssl->conf->psk_identity,
                          identity, identity_len) == 0) {
        ret = esp_mbedtls_ssl_set_hs_psk(ssl, ssl->conf->psk, ssl->conf->psk_len);
        if (ret != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_set_hs_psk", ret);
            return ret;
        }
...
/*
 * Non-error return values of esp_ssl_tls13_offered_psks_check_binder_match().
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_offered_psks_check_binder_match(
    mbedtls_ssl_context *ssl,
    ...
    int psk_type, psa_algorithm_t psk_hash_alg)
{
    ...
    ret = esp_mbedtls_ssl_get_handshake_transcript(
        ssl, esp_mbedtls_md_type_from_psa_alg(psk_hash_alg),
    ...
    ret = esp_mbedtls_ssl_tls13_create_psk_binder(ssl, psk_hash_alg,
                                              psk, psk_len, psk_type,
                                              transcript,
                                              server_computed_binder);
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    esp_mbedtls_free((void *) psk);
#endif
    if (ret != 0) {
    ...
    if (esp_mbedtls_ct_memcmp(server_computed_binder,
                          binder,
                          PSA_HASH_LENGTH(psk_hash_alg)) == 0) {
        return SSL_TLS1_3_BINDER_MATCH;
    }

    esp_mbedtls_platform_zeroize(server_computed_binder,
...
#if defined(MBEDTLS_SSL_SESSION_TICKETS)
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_session_copy_ticket(mbedtls_ssl_session *dst,
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_parse_pre_shared_key_ext(
    mbedtls_ssl_context *ssl,
    ...
    struct psk_attributes *psk)
{
    ...
    while (p_identity_len < identities_end && p_binder_len < binders_end) {
        ...
        ret = esp_ssl_tls13_offered_psks_check_identity_match(
        ...
        esp_ssl_tls13_select_ciphersuite(ssl, ciphersuites, ciphersuites_end,
        ...
        ret = esp_ssl_tls13_offered_psks_check_binder_match(
            ssl, binder, binder_len, psk->type,
            esp_mbedtls_md_psa_alg_from_type((mbedtls_md_type_t) psk->ciphersuite_info->mac));
        if (ret != SSL_TLS1_3_BINDER_MATCH) {
            ...
            MBEDTLS_SSL_DEBUG_RET(
                1, "esp_ssl_tls13_offered_psks_check_binder_match", ret);
            MBEDTLS_SSL_PEND_FATAL_ALERT(
                MBEDTLS_SSL_ALERT_MSG_DECRYPT_ERROR,
                MBEDTLS_ERR_SSL_HANDSHAKE_FAILURE);
            return ret;
        ...
        if (psk->type == MBEDTLS_SSL_TLS1_3_PSK_RESUMPTION) {
            ret = esp_ssl_tls13_session_copy_ticket(ssl->session_negotiate,
                                                &session);
            esp_mbedtls_ssl_session_free(&session);
            if (ret != 0) {
                return ret;
 ...
 * } PreSharedKeyExtension;
 */
static int esp_ssl_tls13_write_server_pre_shared_key_ext(mbedtls_ssl_context *ssl,
                                                     unsigned char *buf,
                                                     unsigned char *end,
                                                     size_t *olen)
{
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_parse_supported_versions_ext(mbedtls_ssl_context *ssl,
                                                  const unsigned char *buf,
                                                  const unsigned char *end)
{
    const unsigned char *p = buf;
    ...
    while (p < versions_end) {
        ...
        tls_version = esp_mbedtls_ssl_read_version(p, ssl->conf->transport);
        p += 2;
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_parse_supported_groups_ext(mbedtls_ssl_context *ssl,
                                                const unsigned char *buf,
                                                const unsigned char *end)
{
    const unsigned char *p = buf;
...
/*
 *  esp_ssl_tls13_parse_key_shares_ext() verifies whether the information in the
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_parse_key_shares_ext(mbedtls_ssl_context *ssl,
                                          const unsigned char *buf,
                                          const unsigned char *end)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    while (p < client_shares_end) {
        ...
        if (esp_mbedtls_ssl_tls13_named_group_is_ecdhe(group) ||
            esp_mbedtls_ssl_tls13_named_group_is_ffdh(group)) {
            MBEDTLS_SSL_DEBUG_MSG(2, ("ECDH/FFDH group: %s (%04x)",
                                      esp_mbedtls_ssl_named_group_to_str(group),
                                      group));
            ret = esp_mbedtls_ssl_tls13_read_public_xxdhe_share(
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_client_hello_has_exts(mbedtls_ssl_context *ssl,
                                           int exts_mask)
{
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_client_hello_has_exts_for_ephemeral_key_exchange(
    mbedtls_ssl_context *ssl)
{
    return esp_ssl_tls13_client_hello_has_exts(
        ssl,
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_client_hello_has_exts_for_psk_key_exchange(
    mbedtls_ssl_context *ssl)
{
    return esp_ssl_tls13_client_hello_has_exts(
        ssl,
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_client_hello_has_exts_for_psk_ephemeral_key_exchange(
    mbedtls_ssl_context *ssl)
{
    return esp_ssl_tls13_client_hello_has_exts(
        ssl,
...
{
#if defined(MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_PSK_ENABLED)
    return esp_mbedtls_ssl_conf_tls13_is_psk_enabled(ssl) &&
           esp_mbedtls_ssl_tls13_is_psk_supported(ssl) &&
...
{
#if defined(MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_PSK_EPHEMERAL_ENABLED)
    return esp_mbedtls_ssl_conf_tls13_is_psk_ephemeral_enabled(ssl) &&
           esp_mbedtls_ssl_tls13_is_psk_ephemeral_supported(ssl) &&
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_key_exchange_is_ephemeral_available(mbedtls_ssl_context *ssl)
{
#if defined(MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_EPHEMERAL_ENABLED)
    return esp_mbedtls_ssl_conf_tls13_is_ephemeral_enabled(ssl) &&
...
#if defined(MBEDTLS_USE_PSA_CRYPTO)
static psa_algorithm_t esp_ssl_tls13_iana_sig_alg_to_psa_alg(uint16_t sig_alg)
{
    switch (sig_alg) {
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_pick_key_cert(mbedtls_ssl_context *ssl)
{
    ...
    for (; *sig_alg != MBEDTLS_TLS1_3_SIG_NONE; sig_alg++) {
        ...
        for (key_cert = key_cert_list; key_cert != NULL;
             ...
             */
            if (esp_mbedtls_x509_crt_check_key_usage(
                    key_cert->cert, MBEDTLS_X509_KU_DIGITAL_SIGNATURE) != 0 ||
                esp_mbedtls_x509_crt_check_extended_key_usage(
            ...
            MBEDTLS_SSL_DEBUG_MSG(3,
                                  ("esp_ssl_tls13_pick_key_cert:"
                                   "check signature algorithm %s [%04x]",
                                   esp_mbedtls_ssl_sig_alg_to_str(*sig_alg),
                                   *sig_alg));
#if defined(MBEDTLS_USE_PSA_CRYPTO)
            psa_alg = esp_ssl_tls13_iana_sig_alg_to_psa_alg(*sig_alg);
#endif /* MBEDTLS_USE_PSA_CRYPTO */

            if (esp_mbedtls_ssl_tls13_check_sig_alg_cert_key_match(
                    *sig_alg, &key_cert->cert->pk)
#if defined(MBEDTLS_USE_PSA_CRYPTO)
                && psa_alg != PSA_ALG_NONE &&
                esp_mbedtls_pk_can_do_ext(&key_cert->cert->pk, psa_alg,
    ...
    MBEDTLS_SSL_DEBUG_MSG(2, ("esp_ssl_tls13_pick_key_cert:"
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_parse_client_hello(mbedtls_ssl_context *ssl,
                                        const unsigned char *buf,
                                        const unsigned char *end)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
     ...
     * uint16 ProtocolVersion;
     */
    if (esp_mbedtls_ssl_read_version(p, ssl->conf->transport) !=
     ...
     */
    ret = esp_mbedtls_ssl_tls13_is_supported_versions_ext_present_in_exts(
        ssl, p + 2, end,
        &supported_versions_data, &supported_versions_data_end);
    if (ret < 0) {
        MBEDTLS_SSL_DEBUG_RET(1,
                              ("esp_mbedtls_ssl_tls13_is_supported_versions_ext_present_in_exts"), ret);
        return ret;
    }
    ...
    if (ret == 1) {
        ret = esp_ssl_tls13_parse_supported_versions_ext(ssl,
                                                     supported_versions_data,
                                                     supported_versions_data_end);
        if (ret < 0) {
            MBEDTLS_SSL_DEBUG_RET(1,
                                  ("esp_ssl_tls13_parse_supported_versions_ext"), ret);
            return ret;
        }

        /*
         * The supported versions extension was parsed successfully as the
         * value returned by esp_ssl_tls13_parse_supported_versions_ext() is
    ...
    esp_ssl_tls13_select_ciphersuite(ssl, cipher_suites, cipher_suites_end,
    ...
    while (p < extensions_end) {
        ...
        ret = esp_mbedtls_ssl_tls13_check_received_extension(
        ...
        switch (extension_type) {
            ...
            case MBEDTLS_TLS_EXT_SERVERNAME:
                ...
                ret = esp_mbedtls_ssl_parse_server_name_ext(ssl, p,
                                                        extension_data_end);
                if (ret != 0) {
                    MBEDTLS_SSL_DEBUG_RET(
            ...
            case MBEDTLS_TLS_EXT_SUPPORTED_GROUPS:
                 ...
                 */
                ret = esp_ssl_tls13_parse_supported_groups_ext(
                    ssl, p, extension_data_end);
                if (ret != 0) {
                    MBEDTLS_SSL_DEBUG_RET(
                        1, "esp_ssl_tls13_parse_supported_groups_ext", ret);
                    return ret;
                }
            ...
            case MBEDTLS_TLS_EXT_KEY_SHARE:
                 ...
                 */
                ret = esp_ssl_tls13_parse_key_shares_ext(
                    ssl, p, extension_data_end);
                ...
                if (ret < 0) {
                    MBEDTLS_SSL_DEBUG_RET(
                        1, "esp_ssl_tls13_parse_key_shares_ext", ret);
                    return ret;
                }
            ...
            case MBEDTLS_TLS_EXT_PSK_KEY_EXCHANGE_MODES:
                ...
                ret = esp_ssl_tls13_parse_key_exchange_modes_ext(
                    ssl, p, extension_data_end);
                if (ret != 0) {
                    MBEDTLS_SSL_DEBUG_RET(
                        1, "esp_ssl_tls13_parse_key_exchange_modes_ext", ret);
                    return ret;
                }
            ...
            case MBEDTLS_TLS_EXT_ALPN:
                ...
                ret = esp_mbedtls_ssl_parse_alpn_ext(ssl, p, extension_data_end);
                if (ret != 0) {
                    MBEDTLS_SSL_DEBUG_RET(
                        1, ("esp_mbedtls_ssl_parse_alpn_ext"), ret);
                    return ret;
                }
                break;
#endif /* MBEDTLS_SSL_ALPN */
            ...
            case MBEDTLS_TLS_EXT_SIG_ALG:
                ...
                ret = esp_mbedtls_ssl_parse_sig_alg_ext(
                    ssl, p, extension_data_end);
                if (ret != 0) {
                    MBEDTLS_SSL_DEBUG_RET(
                        1, "esp_mbedtls_ssl_parse_sig_alg_ext", ret);
                    return ret;
                }
                break;
#endif /* MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_EPHEMERAL_ENABLED */
            ...
            case MBEDTLS_TLS_EXT_RECORD_SIZE_LIMIT:
                ...
                ret = esp_mbedtls_ssl_tls13_parse_record_size_limit_ext(
                    ssl, p, extension_data_end);
                if (ret != 0) {
                    MBEDTLS_SSL_DEBUG_RET(
                        1, ("esp_mbedtls_ssl_tls13_parse_record_size_limit_ext"), ret);
                    return ret;
                }
                break;
    ...
    ret = esp_mbedtls_ssl_add_hs_hdr_to_checksum(ssl,
                                             MBEDTLS_SSL_HS_CLIENT_HELLO,
                                             p - buf);
    if (0 != ret) {
        MBEDTLS_SSL_DEBUG_RET(1, ("esp_mbedtls_ssl_add_hs_hdr_to_checksum"), ret);
        return ret;
    }
    ...
    if (handshake->received_extensions & MBEDTLS_SSL_EXT_MASK(PRE_SHARED_KEY)) {
        ...
        ret = esp_ssl_tls13_parse_pre_shared_key_ext(ssl,
                                                 pre_shared_key_ext,
        ...
        if (ret == 0) {
            ...
            MBEDTLS_SSL_DEBUG_RET(
                1, "esp_ssl_tls13_parse_pre_shared_key_ext", ret);
            return ret;
        }
    } else
#endif /* MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_SOME_PSK_ENABLED */
...
#endif
    if (esp_ssl_tls13_key_exchange_is_ephemeral_available(ssl)) {
        handshake->key_exchange_mode =
    ...
    esp_mbedtls_ssl_optimize_checksum(ssl, handshake->ciphersuite_info);

    return hrr_required ? SSL_CLIENT_HELLO_HRR_REQUIRED : SSL_CLIENT_HELLO_OK;
}

#if defined(MBEDTLS_SSL_EARLY_DATA)
static int esp_ssl_tls13_check_early_data_requirements(mbedtls_ssl_context *ssl)
{
    mbedtls_ssl_handshake_params *handshake = ssl->handshake;
     ...
     *    esp_ssl_tls13_offered_psks_check_identity_match_ticket().
     */
    ...
    if (!esp_mbedtls_ssl_tls13_session_ticket_allow_early_data(ssl->session_negotiate)) {
        MBEDTLS_SSL_DEBUG_MSG(
            1,
...
#if defined(MBEDTLS_SSL_ALPN)
    const char *alpn = esp_mbedtls_ssl_get_alpn_protocol(ssl);
    size_t alpn_len;
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_postprocess_client_hello(mbedtls_ssl_context *ssl,
                                              int hrr_required)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    ret = esp_mbedtls_ssl_tls13_key_schedule_stage_early(ssl);
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1,
    ...
    if (ssl->handshake->received_extensions & MBEDTLS_SSL_EXT_MASK(EARLY_DATA)) {
        ssl->handshake->early_data_accepted =
            (!hrr_required) && (esp_ssl_tls13_check_early_data_requirements(ssl) == 0);

        if (ssl->handshake->early_data_accepted) {
            ret = esp_mbedtls_ssl_tls13_compute_early_transform(ssl);
            if (ret != 0) {
                MBEDTLS_SSL_DEBUG_RET(
                    1, "esp_mbedtls_ssl_tls13_compute_early_transform", ret);
                return ret;
            }
        } else {
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_process_client_hello(mbedtls_ssl_context *ssl)
{
    ...
    MBEDTLS_SSL_PROC_CHK(esp_mbedtls_ssl_tls13_fetch_handshake_msg(
                             ssl, MBEDTLS_SSL_HS_CLIENT_HELLO,
                             &buf, &buflen));

    MBEDTLS_SSL_PROC_CHK_NEG(esp_ssl_tls13_parse_client_hello(ssl, buf,
                                                          buf + buflen));
     ...
     * MBEDTLS_SSL_VERSION_TLS1_2 thus from now on esp_mbedtls_ssl_handshake_step()
    ...
    if (SSL_CLIENT_HELLO_TLS1_2 == parse_client_hello_ret) {
        ...
        if (!esp_mbedtls_ssl_conf_is_tls12_enabled(ssl->conf)) {
            MBEDTLS_SSL_DEBUG_MSG(
    ...
    MBEDTLS_SSL_PROC_CHK(
        esp_ssl_tls13_postprocess_client_hello(ssl, parse_client_hello_ret ==
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_prepare_server_hello(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
...
/*
 * esp_ssl_tls13_write_server_hello_supported_versions_ext ():
 *
 * struct {
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_write_server_hello_supported_versions_ext(
    mbedtls_ssl_context *ssl,
    unsigned char *buf,
    unsigned char *end,
    size_t *out_len)
{
    *out_len = 0;
    ...
    esp_mbedtls_ssl_write_version(buf + 4,
                              ssl->conf->transport,
                              ssl->tls_version);
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_generate_and_write_key_share(mbedtls_ssl_context *ssl,
                                                  uint16_t named_group,
                                                  unsigned char *buf,
                                                  unsigned char *end,
                                                  size_t *out_len)
{
    ...
    *out_len = 0;
    ...
    if (esp_mbedtls_ssl_tls13_named_group_is_ecdhe(named_group) ||
        esp_mbedtls_ssl_tls13_named_group_is_ffdh(named_group)) {
        ret = esp_mbedtls_ssl_tls13_generate_and_write_xxdh_key_exchange(
            ssl, named_group, buf, end, out_len);
        if (ret != 0) {
            MBEDTLS_SSL_DEBUG_RET(
                1, "esp_mbedtls_ssl_tls13_generate_and_write_xxdh_key_exchange",
                ret);
            return ret;
        }
    } else
...
/*
 * esp_ssl_tls13_write_key_share_ext
 *
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_write_key_share_ext(mbedtls_ssl_context *ssl,
                                         unsigned char *buf,
                                         unsigned char *end,
                                         size_t *out_len)
{
    ...
    *out_len = 0;
    ...
    ret = esp_ssl_tls13_generate_and_write_key_share(
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_write_hrr_key_share_ext(mbedtls_ssl_context *ssl,
                                             unsigned char *buf,
                                             unsigned char *end,
                                             size_t *out_len)
{
    ...
    *out_len = 0;
    ...
    MBEDTLS_SSL_DEBUG_MSG(3,
                          ("HRR selected_group: %s (%x)",
                           esp_mbedtls_ssl_named_group_to_str(selected_group),
                           selected_group));
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_write_server_hello_body(mbedtls_ssl_context *ssl,
                                             unsigned char *buf,
                                             unsigned char *end,
                                             size_t *out_len,
                                             int is_hrr)
{
    ...
    *out_len = 0;
    ...
    if (is_hrr) {
        memcpy(p, esp_mbedtls_ssl_tls13_hello_retry_request_magic,
               MBEDTLS_SERVER_HELLO_RANDOM_LEN);
    } else {
    ...
    *p++ = (unsigned char) ssl->session_negotiate->id_len;
    ...
    MBEDTLS_SSL_DEBUG_MSG(3,
                          ("server hello, chosen ciphersuite: %s ( id=%d )",
                           esp_mbedtls_ssl_get_ciphersuite_name(
                               ssl->session_negotiate->ciphersuite),
                           ssl->session_negotiate->ciphersuite));
    ...
    *p++ = MBEDTLS_SSL_COMPRESS_NULL;
    ...
    if ((ret = esp_ssl_tls13_write_server_hello_supported_versions_ext(
             ssl, p, end, &output_len)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(
            1, "esp_ssl_tls13_write_server_hello_supported_versions_ext", ret);
        return ret;
    }
    p += output_len;
    ...
    if (esp_mbedtls_ssl_tls13_key_exchange_mode_with_ephemeral(ssl)) {
        if (is_hrr) {
            ...
            ret = esp_ssl_tls13_write_key_share_ext(ssl, p, end, &output_len);
        }
        if (ret != 0) {
            return ret;
        }
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_finalize_server_hello(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ret = esp_mbedtls_ssl_tls13_compute_handshake_transform(ssl);
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1,
                              "esp_mbedtls_ssl_tls13_compute_handshake_transform",
                              ret);
        return ret;
    }
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_write_server_hello(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    unsigned char *buf;
    ...
    MBEDTLS_SSL_PROC_CHK(esp_ssl_tls13_prepare_server_hello(ssl));

    MBEDTLS_SSL_PROC_CHK(esp_mbedtls_ssl_start_handshake_msg(
                             ssl, MBEDTLS_SSL_HS_SERVER_HELLO, &buf, &buf_len));

    MBEDTLS_SSL_PROC_CHK(esp_ssl_tls13_write_server_hello_body(ssl, buf,
                                                           buf + buf_len,
                                                           &msg_len,
                                                           0));

    MBEDTLS_SSL_PROC_CHK(esp_mbedtls_ssl_add_hs_msg_to_checksum(
                             ssl, MBEDTLS_SSL_HS_SERVER_HELLO, buf, msg_len));

    MBEDTLS_SSL_PROC_CHK(esp_mbedtls_ssl_finish_handshake_msg(
                             ssl, buf_len, msg_len));

    MBEDTLS_SSL_PROC_CHK(esp_ssl_tls13_finalize_server_hello(ssl));
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_prepare_hello_retry_request(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    if (ssl->handshake->hello_retry_request_flag) {
    ...
    ret = esp_mbedtls_ssl_reset_transcript_for_hrr(ssl);
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_reset_transcript_for_hrr", ret);
        return ret;
    }
    esp_mbedtls_ssl_session_reset_msg_layer(ssl, 0);

    return 0;
}

MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_write_hello_retry_request(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    unsigned char *buf;
    size_t buf_len, msg_len;
    ...
    MBEDTLS_SSL_PROC_CHK(esp_ssl_tls13_prepare_hello_retry_request(ssl));
    ...
    MBEDTLS_SSL_PROC_CHK(esp_mbedtls_ssl_finish_handshake_msg(ssl, buf_len,
                                                          msg_len));
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_write_encrypted_extensions_body(mbedtls_ssl_context *ssl,
                                                     unsigned char *buf,
                                                     unsigned char *end,
                                                     size_t *out_len)
{
    ...
    *out_len = 0;
...
#if defined(MBEDTLS_SSL_ALPN)
    ret = esp_mbedtls_ssl_write_alpn_ext(ssl, p, end, &output_len);
    if (ret != 0) {
        return ret;
    }
    p += output_len;
...
#if defined(MBEDTLS_SSL_EARLY_DATA)
    if (ssl->handshake->early_data_accepted) {
        ret = esp_mbedtls_ssl_tls13_write_early_data_ext(
    ...
    if (ssl->handshake->received_extensions & MBEDTLS_SSL_EXT_MASK(RECORD_SIZE_LIMIT)) {
        ret = esp_mbedtls_ssl_tls13_write_record_size_limit_ext(
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_write_encrypted_extensions(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    unsigned char *buf;
    size_t buf_len, msg_len;

    esp_mbedtls_ssl_set_outbound_transform(ssl,
    ...
    MBEDTLS_SSL_PROC_CHK(esp_ssl_tls13_write_encrypted_extensions_body(
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_certificate_request_coordinate(mbedtls_ssl_context *ssl)
{
    int authmode;
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_write_certificate_request_body(mbedtls_ssl_context *ssl,
                                                    unsigned char *buf,
                                                    const unsigned char *end,
                                                    size_t *out_len)
{
    ...
    *p++ = 0x0;
    ...
    p_extensions_len = p;
    p += 2;
    ret = esp_mbedtls_ssl_write_sig_alg_ext(ssl, p, end, &output_len);
    if (ret != 0) {
        return ret;
    }
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_write_certificate_request(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;

    MBEDTLS_SSL_DEBUG_MSG(2, ("=> write certificate request"));

    MBEDTLS_SSL_PROC_CHK_NEG(esp_ssl_tls13_certificate_request_coordinate(ssl));
    ...
    if (ret == SSL_CERTIFICATE_REQUEST_SEND_REQUEST) {
        ...
        MBEDTLS_SSL_PROC_CHK(esp_ssl_tls13_write_certificate_request_body(
                                 ssl, buf, buf + buf_len, &msg_len));
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_write_server_certificate(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    ret = esp_mbedtls_ssl_tls13_write_certificate(ssl);
    if (ret != 0) {
        return ret;
    }
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_write_certificate_verify(mbedtls_ssl_context *ssl)
{
 ...
 * second flight.
 */
static void esp_ssl_tls13_prepare_for_handshake_second_flight(
    mbedtls_ssl_context *ssl)
{
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_write_server_finished(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;

    ret = esp_mbedtls_ssl_tls13_write_finished_message(ssl);
    if (ret != 0) {
        return ret;
    }

    ret = esp_mbedtls_ssl_tls13_compute_application_transform(ssl);
    if (ret != 0) {
        MBEDTLS_SSL_PEND_FATAL_ALERT(
    ...
    if (ssl->handshake->early_data_accepted) {
        ...
        MBEDTLS_SSL_DEBUG_MSG(
            1, ("Switch to early keys for inbound traffic. "
                "( K_recv = early data )"));
        esp_mbedtls_ssl_set_inbound_transform(
    ...
    esp_ssl_tls13_prepare_for_handshake_second_flight(ssl);
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_end_of_early_data_coordinate(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;

    if ((ret = esp_mbedtls_ssl_read_record(ssl, 0)) != 0) {
    ...
    if (ssl->in_msgtype == MBEDTLS_SSL_MSG_APPLICATION_DATA) {
        if (ssl->in_offt == NULL) {
            ...
            ret = esp_mbedtls_ssl_tls13_check_early_data_len(ssl, ssl->in_msglen);
            if (ret != 0) {
                return ret;
            }
        }
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_parse_end_of_early_data(mbedtls_ssl_context *ssl,
                                             const unsigned char *buf,
                                             const unsigned char *end)
{
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_process_end_of_early_data(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;

    MBEDTLS_SSL_DEBUG_MSG(2, ("=> esp_ssl_tls13_process_end_of_early_data"));

    MBEDTLS_SSL_PROC_CHK_NEG(esp_ssl_tls13_end_of_early_data_coordinate(ssl));
    ...
    if (ret == SSL_GOT_END_OF_EARLY_DATA) {
        ...
        MBEDTLS_SSL_PROC_CHK(esp_ssl_tls13_parse_end_of_early_data(
                                 ssl, buf, buf + buf_len));
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_process_client_finished(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;

    ret = esp_mbedtls_ssl_tls13_process_finished_message(ssl);
    if (ret != 0) {
        return ret;
    }

    ret = esp_mbedtls_ssl_tls13_compute_resumption_master_secret(ssl);
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(
            1, "esp_mbedtls_ssl_tls13_compute_resumption_master_secret", ret);
    }
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_handshake_wrapup(mbedtls_ssl_context *ssl)
{
 ...
 */
    ...
    if (esp_mbedtls_ssl_tls13_is_some_psk_supported(ssl)) {
        esp_mbedtls_ssl_handshake_set_state(
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_write_new_session_ticket_coordinate(mbedtls_ssl_context *ssl)
{
...
#if defined(MBEDTLS_SSL_SESSION_TICKETS)
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_prepare_new_session_ticket(mbedtls_ssl_context *ssl,
                                                ...
                                                size_t ticket_nonce_size)
{
    ...
    esp_mbedtls_ssl_tls13_session_clear_ticket_flags(
        session, MBEDTLS_SSL_TLS1_3_TICKET_FLAGS_MASK);
#if defined(MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_SOME_PSK_ENABLED)
    esp_mbedtls_ssl_tls13_session_set_ticket_flags(
    ...
    if (session->ticket_alpn == NULL) {
        ret = esp_mbedtls_ssl_session_set_ticket_alpn(session, ssl->alpn_chosen);
        if (ret != 0) {
            return ret;
        }
    }
    ...
    psa_hash_alg = esp_mbedtls_md_psa_alg_from_type((mbedtls_md_type_t) ciphersuite_info->mac);
     ...
     */
    ret = esp_mbedtls_ssl_tls13_hkdf_expand_label(
        psa_hash_alg,
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_write_new_session_ticket_body(mbedtls_ssl_context *ssl,
                                                   unsigned char *buf,
                                                   unsigned char *end,
                                                   size_t *out_len,
                                                   unsigned char *ticket_nonce,
...
#if defined(MBEDTLS_SSL_EARLY_DATA)
    if (esp_mbedtls_ssl_tls13_session_ticket_allow_early_data(session)) {
        size_t output_len;

        if ((ret = esp_mbedtls_ssl_tls13_write_early_data_ext(
                 ssl, 1, p, end, &output_len)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(
                1, "esp_mbedtls_ssl_tls13_write_early_data_ext", ret);
            return ret;
        }
        p += output_len;
    } else {
 ...
 * Handler for MBEDTLS_SSL_TLS1_3_NEW_SESSION_TICKET
 */
static int esp_ssl_tls13_write_new_session_ticket(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;

    MBEDTLS_SSL_PROC_CHK_NEG(esp_ssl_tls13_write_new_session_ticket_coordinate(ssl));
    ...
    if (ret == SSL_NEW_SESSION_TICKET_WRITE) {
        ...
        MBEDTLS_SSL_PROC_CHK(esp_ssl_tls13_prepare_new_session_ticket(
        ...
        MBEDTLS_SSL_PROC_CHK(esp_ssl_tls13_write_new_session_ticket_body(
                                 ssl, buf, buf + buf_len, &msg_len,
         ...
         *
         * See document of esp_mbedtls_ssl_conf_new_session_tickets.
         */
 ...
 */
int esp_mbedtls_ssl_tls13_handshake_server_step(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    switch (ssl->state) {
        ...
        case MBEDTLS_SSL_CLIENT_HELLO:
            ret = esp_ssl_tls13_process_client_hello(ssl);
            if (ret != 0) {
                MBEDTLS_SSL_DEBUG_RET(1, "esp_ssl_tls13_process_client_hello", ret);
            }
            break;

        case MBEDTLS_SSL_HELLO_RETRY_REQUEST:
            ret = esp_ssl_tls13_write_hello_retry_request(ssl);
            if (ret != 0) {
                MBEDTLS_SSL_DEBUG_RET(1, "esp_ssl_tls13_write_hello_retry_request", ret);
                return ret;
            }
            break;

        case MBEDTLS_SSL_SERVER_HELLO:
            ret = esp_ssl_tls13_write_server_hello(ssl);
            break;

        case MBEDTLS_SSL_ENCRYPTED_EXTENSIONS:
            ret = esp_ssl_tls13_write_encrypted_extensions(ssl);
            if (ret != 0) {
                MBEDTLS_SSL_DEBUG_RET(1, "esp_ssl_tls13_write_encrypted_extensions", ret);
                return ret;
            }
            break;
        ...
        case MBEDTLS_SSL_CERTIFICATE_REQUEST:
            ret = esp_ssl_tls13_write_certificate_request(ssl);
            break;

        case MBEDTLS_SSL_SERVER_CERTIFICATE:
            ret = esp_ssl_tls13_write_server_certificate(ssl);
            break;

        case MBEDTLS_SSL_CERTIFICATE_VERIFY:
            ret = esp_ssl_tls13_write_certificate_verify(ssl);
            break;
#endif /* MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_EPHEMERAL_ENABLED */
        ...
        case MBEDTLS_SSL_SERVER_FINISHED:
            ret = esp_ssl_tls13_write_server_finished(ssl);
            break;

#if defined(MBEDTLS_SSL_EARLY_DATA)
        case MBEDTLS_SSL_END_OF_EARLY_DATA:
            ret = esp_ssl_tls13_process_end_of_early_data(ssl);
            break;
#endif /* MBEDTLS_SSL_EARLY_DATA */

        case MBEDTLS_SSL_CLIENT_FINISHED:
            ret = esp_ssl_tls13_process_client_finished(ssl);
            break;

        case MBEDTLS_SSL_HANDSHAKE_WRAPUP:
            ret = esp_ssl_tls13_handshake_wrapup(ssl);
            break;
        ...
        case MBEDTLS_SSL_TLS1_3_NEW_SESSION_TICKET:
            ret = esp_ssl_tls13_write_new_session_ticket(ssl);
            if (ret != 0) {
                MBEDTLS_SSL_DEBUG_RET(1,
                                      "esp_ssl_tls13_write_new_session_ticket ",
                                      ret);
            }
            break;
        case MBEDTLS_SSL_TLS1_3_NEW_SESSION_TICKET_FLUSH:
             ...
             * as part of esp_ssl_prepare_handshake_step.
             */
            ret = 0;
