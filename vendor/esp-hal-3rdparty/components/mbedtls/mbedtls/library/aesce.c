...
signed char esp_mbedtls_aesce_has_support_result = -1;

#if !defined(MBEDTLS_AES_USE_HARDWARE_ONLY)
/*
 * AES instruction support detection routine
 */
int esp_mbedtls_aesce_has_support_impl(void)
{
     ...
     */
    if (esp_mbedtls_aesce_has_support_result == -1) {
#if defined(MBEDTLS_ARCH_IS_ARM32)
        unsigned long auxval  = getauxval(AT_HWCAP);
        unsigned long auxval2 = getauxval(AT_HWCAP2);
        if (((auxval  & HWCAP_NEON) == HWCAP_NEON) &&
            ((auxval2 & HWCAP2_AES) == HWCAP2_AES)) {
            esp_mbedtls_aesce_has_support_result = 1;
        } else {
            esp_mbedtls_aesce_has_support_result = 0;
        }
#else
        unsigned long auxval = getauxval(AT_HWCAP);
    ...
    return esp_mbedtls_aesce_has_support_result;
}
#endif
...
static uint8x16_t esp_aesce_encrypt_block(uint8x16_t block,
                                      unsigned char *keys,
                                      int rounds)
{
...
static uint8x16_t esp_aesce_decrypt_block(uint8x16_t block,
                                      unsigned char *keys,
                                      int rounds)
{
    /* 10, 12 or 14 rounds. Unroll loop. */
 ...
 */
int esp_mbedtls_aesce_crypt_ecb(mbedtls_aes_context *ctx,
                            int mode,
                            ...
                            unsigned char output[16])
{
    ...
    if (mode == MBEDTLS_AES_DECRYPT) {
        block = esp_aesce_decrypt_block(block, keys, ctx->nr);
    } else
#else
    (void) mode;
#endif
    {
        block = esp_aesce_encrypt_block(block, keys, ctx->nr);
    }
 ...
 */
#if !defined(MBEDTLS_BLOCK_CIPHER_NO_DECRYPT)
void esp_mbedtls_aesce_inverse_key(unsigned char *invkey,
...
static inline uint32_t esp_aes_rot_word(uint32_t word)
{
    return (word << (32 - 8)) | (word >> 8);
}

static inline uint32_t esp_aes_sub_word(uint32_t in)
{
 ...
 */
static void esp_aesce_setkey_enc(unsigned char *rk,
                             ...
                             const size_t key_bit_length)
{
    ...
    for (uint32_t *rki = (uint32_t *) rk;
        ...
        rko[0] = esp_aes_rot_word(esp_aes_sub_word(rki[key_len_in_words - 1]));
        ...
        switch (key_bit_length) {
            ...
            case 192:
                ...
                break;
            case 256:
                rko[4] = esp_aes_sub_word(rko[3]) ^ rki[4];
                rko[5] = rko[4] ^ rki[5];
                rko[6] = rko[5] ^ rki[6];
                rko[7] = rko[6] ^ rki[7];
                break;
 ...
 */
int esp_mbedtls_aesce_setkey_enc(unsigned char *rk,
                             const unsigned char *key,
                             size_t bits)
{
    switch (bits) {
        ...
        case 192:
        case 256:
            esp_aesce_setkey_enc(rk, key, bits);
            break;
        default:
...
static inline poly128_t esp_vmull_p64(poly64_t a, poly64_t b)
{
    poly128_t r;
...
static inline poly128_t esp_vmull_high_p64(poly64x2_t a, poly64x2_t b)
{
    return esp_vmull_p64((poly64_t) (vget_high_u64((uint64x2_t) a)),
                     (poly64_t) (vget_high_u64((uint64x2_t) b)));
}

#endif /* defined(__clang__) */

static inline uint8x16_t esp_vrbitq_u8(uint8x16_t x)
{
    /* There is no esp_vrbitq_u8 instruction in A32/T32, so provide
...
static inline poly64x1_t esp_vget_low_p64(poly64x2_t a)
{
...
/* esp_vmull_p64/esp_vmull_high_p64 wrappers.
 *
...
#define MBEDTLS_VMULL_P64(a, b) esp_vmull_p64((poly64_t) a, (poly64_t) b)
#else
 ...
 * cast for clang also. */
#define MBEDTLS_VMULL_P64(a, b) esp_vmull_p64(a, b)
#endif /* MBEDTLS_COMPILER_IS_GCC */

static inline uint8x16_t esp_pmull_low(uint8x16_t a, uint8x16_t b)
{

    return vreinterpretq_u8_p128(
        MBEDTLS_VMULL_P64(
            (poly64_t) esp_vget_low_p64(vreinterpretq_p64_u8(a)),
            (poly64_t) esp_vget_low_p64(vreinterpretq_p64_u8(b))
            ));
}

static inline uint8x16_t esp_pmull_high(uint8x16_t a, uint8x16_t b)
{
    return vreinterpretq_u8_p128(
        esp_vmull_high_p64(vreinterpretq_p64_u8(a),
 ...
 *
 * `esp_poly_mult_128` executes polynomial multiplication and outputs 256b that
 ...
 */
static inline uint8x16x3_t esp_poly_mult_128(uint8x16_t a, uint8x16_t b)
{
    uint8x16x3_t ret;
    uint8x16_t h, m, l; /* retval high/middle/low */
    uint8x16_t c, d, e;

    h = esp_pmull_high(a, b);                       /* h3:h2:00:00 = a1*b1 */
    l = esp_pmull_low(a, b);                        /*   :  :l1:l0 = a0*b0 */
    c = vextq_u8(b, b, 8);                      /*      :c1:c0 = b0:b1 */
    d = esp_pmull_high(a, c);                       /*   :d2:d1:00 = a1*b0 */
    e = esp_pmull_low(a, c);                        /*   :e2:e1:00 = a0*b1 */
 ...
 */
static inline uint8x16_t esp_poly_mult_reduce(uint8x16x3_t input)
{
    ...
    l = input.val[2];            /*   :  :l1:l0                          */
    c = esp_pmull_high(h, MODULO);   /*   :c2:c1:00 = reduction of h3        */
    d = esp_pmull_low(h, MODULO);    /*   :  :d1:d0 = reduction of h2        */
    e = veorq_u8(c, m);          /*   :e2:e1:00 = m2:m1:00 + c2:c1:00    */
    f = esp_pmull_high(e, MODULO);   /*   :  :f1:f0 = reduction of e2        */
 ...
 */
void esp_mbedtls_aesce_gcm_mult(unsigned char c[16],
                            const unsigned char a[16],
                            const unsigned char b[16])
{
    uint8x16_t va, vb, vc;
    va = esp_vrbitq_u8(vld1q_u8(&a[0]));
    vb = esp_vrbitq_u8(vld1q_u8(&b[0]));
    vc = esp_vrbitq_u8(esp_poly_mult_reduce(esp_poly_mult_128(va, vb)));
    vst1q_u8(&c[0], vc);
}
