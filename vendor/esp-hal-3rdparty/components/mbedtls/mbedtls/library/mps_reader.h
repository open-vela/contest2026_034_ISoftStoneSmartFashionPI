 ...
 *     received through calls to esp_mbedtls_mps_reader_get(),
 ...
 *     esp_mbedtls_mps_reader_reclaim().
 *     The marker is set via esp_mbedtls_mps_reader_commit()
 ...
 *            via esp_mbedtls_mps_reader_feed().
 *
 ...
 * done via successful calls to esp_mbedtls_mps_reader_feed(), while
 * transitioning from Attached to either Unset or Accumulating (depending
 * on what has been processed) is done via esp_mbedtls_mps_reader_reclaim().
 *
...
struct mbedtls_mps_reader {
    unsigned char *frag;  /*!< The fragment of incoming data managed by
                           ...
                           *   through esp_mbedtls_mps_reader_feed(). The reader
     ...
     *   esp_mbedtls_mps_reader_get(), relative to the first
    ...
    unsigned char *acc;   /*!< The accumulator is used to gather incoming
                           *   data if a read-request via esp_mbedtls_mps_reader_get()
 ...
 *                  in case get requests through esp_mbedtls_mps_reader_get()
 *                  exceed the buffer provided by esp_mbedtls_mps_reader_feed().
 ...
 */
int esp_mbedtls_mps_reader_init(mbedtls_mps_reader *reader,
                            unsigned char *acc,
 ...
 * \return          A negative \c MBEDTLS_ERR_READER_XXX error code on failure.
 */
int esp_mbedtls_mps_reader_free(mbedtls_mps_reader *reader);
 ...
 *                  of \p buf until esp_mbedtls_mps_reader_reclaim()
 *                  is called. It is the responsibility of the caller
 *                  to ensure that the \p buf persists and is not changed
 *                  between successful calls to esp_mbedtls_mps_reader_feed()
 *                  and esp_mbedtls_mps_reader_reclaim().
 * \return          \c MBEDTLS_ERR_MPS_READER_NEED_MORE if more input data is
 *                  required to fulfill a previous request to esp_mbedtls_mps_reader_get().
 ...
 */
int esp_mbedtls_mps_reader_feed(mbedtls_mps_reader *reader,
                            unsigned char *buf,
 ...
 *                  esp_mbedtls_mps_reader_get() has exceeded the bounds of the
 *                  input buffer.
 *
 * \return          \c 0 on success.
 * \return          A negative \c MBEDTLS_ERR_READER_XXX error code on failure.
 */
int esp_mbedtls_mps_reader_reclaim(mbedtls_mps_reader *reader,
                               int *paused);
 ...
 *                  until the next call to esp_mbedtls_mps_reader_reclaim().
 ...
 *                  esp_mbedtls_mps_reader_reclaim() and esp_mbedtls_mps_reader_feed().
 ...
 *                  is requested after a esp_mbedtls_mps_reader_reclaim() and
 *                  esp_mbedtls_mps_reader_feed() cycle.
 ...
 *                  afterwards.
 */
int esp_mbedtls_mps_reader_get(mbedtls_mps_reader *reader,
                           mbedtls_mps_size_t desired,
                           unsigned char **buffer,
                           mbedtls_mps_size_t *buflen);

/**
 * \brief         Mark data obtained from esp_mbedtls_mps_reader_get() as processed.
 *
 *                This call indicates that all data received from prior calls to
 *                esp_mbedtls_mps_reader_get() has been or will have been
 *                processed when esp_mbedtls_mps_reader_reclaim() is called,
 *                and thus need not be backed up.
 *
 *                This function has no user observable effect until
 *                esp_mbedtls_mps_reader_reclaim() is called. In particular,
 *                buffers received from esp_mbedtls_mps_reader_get() remain
 *                valid until esp_mbedtls_mps_reader_reclaim() is called.
 *
 ...
 *
 */
int esp_mbedtls_mps_reader_commit(mbedtls_mps_reader *reader);
