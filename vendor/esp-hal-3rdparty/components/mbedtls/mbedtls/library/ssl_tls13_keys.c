...
struct mbedtls_ssl_tls13_labels_struct const esp_mbedtls_ssl_tls13_labels =
{
...
static const char esp_tls13_label_prefix[6] = "tls13 ";
...
#define SSL_TLS1_3_KEY_SCHEDULE_MAX_HKDF_LABEL_LEN                      \
    SSL_TLS1_3_KEY_SCHEDULE_HKDF_LABEL_LEN(                             \
        sizeof(esp_tls13_label_prefix) +                       \
        MBEDTLS_SSL_TLS1_3_KEY_SCHEDULE_MAX_LABEL_LEN,     \
        MBEDTLS_SSL_TLS1_3_KEY_SCHEDULE_MAX_CONTEXT_LEN)

static void esp_ssl_tls13_hkdf_encode_label(
    ...
    unsigned char *dst, size_t *dst_len)
{
    size_t total_label_len =
        sizeof(esp_tls13_label_prefix) + label_len;
...
#error "The implementation of esp_ssl_tls13_hkdf_encode_label() is not fit for the \
    ...
    *p++ = MBEDTLS_BYTE_0(total_label_len);
    memcpy(p, esp_tls13_label_prefix, sizeof(esp_tls13_label_prefix));
    p += sizeof(esp_tls13_label_prefix);
...
int esp_mbedtls_ssl_tls13_hkdf_expand_label(
    ...
    unsigned char *buf, size_t buf_len)
{
    ...
    esp_ssl_tls13_hkdf_encode_label(buf_len,
                                label, label_len,
                                ctx, ctx_len,
                                hkdf_label,
                                &hkdf_label_len);

    status = esp_psa_key_derivation_setup(&operation, PSA_ALG_HKDF_EXPAND(hash_alg));

    if (status != PSA_SUCCESS) {
        goto cleanup;
    }

    status = esp_psa_key_derivation_input_bytes(&operation,
    ...
    status = esp_psa_key_derivation_output_bytes(&operation,
                                             buf,
                                             buf_len);
...
cleanup:
    abort_status = esp_psa_key_derivation_abort(&operation);
    status = (status == PSA_SUCCESS ? abort_status : status);
    esp_mbedtls_platform_zeroize(hkdf_label, hkdf_label_len);
    return PSA_TO_MBEDTLS_ERR(status);
}

MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_make_traffic_key(
    psa_algorithm_t hash_alg,
    const unsigned char *secret, size_t secret_len,
    unsigned char *key, size_t esp_key_len,
 ...
 */
int esp_mbedtls_ssl_tls13_make_traffic_keys(
    psa_algorithm_t hash_alg,
    const unsigned char *client_secret,
    const unsigned char *server_secret, size_t secret_len,
    size_t esp_key_len, size_t iv_len,
    mbedtls_ssl_key_set *keys)
{
    int ret = 0;

    ret = esp_ssl_tls13_make_traffic_key(
        hash_alg, client_secret, secret_len,
        keys->client_write_key, esp_key_len,
    ...
    ret = esp_ssl_tls13_make_traffic_key(
        ...
        keys->server_write_key, esp_key_len,
        keys->server_write_iv, iv_len);
    if (ret != 0) {
        return ret;
    }

    keys->esp_key_len = esp_key_len;
    keys->iv_len = iv_len;

    return 0;
}

int esp_mbedtls_ssl_tls13_derive_secret(
    psa_algorithm_t hash_alg,
    const unsigned char *secret, size_t secret_len,
    const unsigned char *label, size_t label_len,
    const unsigned char *ctx, size_t ctx_len,
    ...
    unsigned char *dstbuf, size_t dstbuf_len)
{
    ...
    if (ctx_hashed == MBEDTLS_SSL_TLS1_3_CONTEXT_UNHASHED) {
        ...
        status = esp_psa_hash_compute(hash_alg, ctx, ctx_len, hashed_context,
    ...
    return esp_mbedtls_ssl_tls13_hkdf_expand_label(hash_alg,
                                               secret, secret_len,
                                               label, label_len,
...
int esp_mbedtls_ssl_tls13_evolve_secret(
    psa_algorithm_t hash_alg,
    ...
    unsigned char *secret_new)
{
     ...
     * on the old secret. */
    if (secret_old != NULL) {
        ret = esp_mbedtls_ssl_tls13_derive_secret(
            hash_alg,
            secret_old, hlen,
    ...
    status = esp_psa_key_derivation_setup(&operation,
...
cleanup:
    ...
    esp_mbedtls_platform_zeroize(tmp_secret, sizeof(tmp_secret));
    return ret;
}

int esp_mbedtls_ssl_tls13_derive_early_secrets(
    psa_algorithm_t hash_alg,
...
int esp_mbedtls_ssl_tls13_derive_handshake_secrets(
    psa_algorithm_t hash_alg,
...
int esp_mbedtls_ssl_tls13_derive_application_secrets(
    psa_algorithm_t hash_alg,
 ...
 *
 * This is not integrated with esp_mbedtls_ssl_tls13_derive_application_secrets()
 * because it uses the transcript hash up to and including ClientFinished. */
int esp_mbedtls_ssl_tls13_derive_resumption_master_secret(
    psa_algorithm_t hash_alg,
    unsigned char const *application_secret,
    unsigned char const *transcript, size_t transcript_len,
    mbedtls_ssl_tls13_application_secrets *derived)
 ...
 *
 *        In the handshake stage, esp_ssl_tls13_generate_application_keys()
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_key_schedule_stage_application(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    mbedtls_ssl_handshake_params *handshake = ssl->handshake;
    psa_algorithm_t const hash_alg = esp_mbedtls_md_psa_alg_from_type(
        (mbedtls_md_type_t) handshake->ciphersuite_info->mac);

    /*
     * Compute MasterSecret
     */
    ret = esp_mbedtls_ssl_tls13_evolve_secret(
        hash_alg,
        handshake->tls13_master_secrets.handshake,
        NULL, 0,
        handshake->tls13_master_secrets.app);
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_tls13_evolve_secret", ret);
        return ret;
    }
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_calc_finished_core(psa_algorithm_t hash_alg,
                                        ...
                                        size_t *dst_len)
{
    ...
    status = esp_psa_import_key(&attributes, finished_key, hash_len, &key);
    if (status != PSA_SUCCESS) {
        ret = PSA_TO_MBEDTLS_ERR(status);
        goto exit;
    }

    status = esp_psa_mac_compute(key, alg, transcript, hash_len,
                             dst, hash_len, dst_len);
    ret = PSA_TO_MBEDTLS_ERR(status);

exit:

    status = esp_psa_destroy_key(key);
    if (ret == 0) {
        ret = PSA_TO_MBEDTLS_ERR(status);
    }

    esp_mbedtls_platform_zeroize(finished_key, sizeof(finished_key));

    return ret;
}

int esp_mbedtls_ssl_tls13_calculate_verify_data(mbedtls_ssl_context *ssl,
                                            unsigned char *dst,
                                            size_t dst_len,
                                            size_t *actual_len,
                                            int from)
{
    ...
    psa_algorithm_t hash_alg = esp_mbedtls_md_psa_alg_from_type(
        (mbedtls_md_type_t) ssl->handshake->ciphersuite_info->mac);
    size_t const hash_len = PSA_HASH_LENGTH(hash_alg);

    MBEDTLS_SSL_DEBUG_MSG(2, ("=> esp_mbedtls_ssl_tls13_calculate_verify_data"));
    ...
    ret = esp_mbedtls_ssl_get_handshake_transcript(ssl, md_type,
                                               transcript, sizeof(transcript),
                                               &transcript_len);
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_get_handshake_transcript", ret);
        goto exit;
    }
    MBEDTLS_SSL_DEBUG_BUF(4, "handshake hash", transcript, transcript_len);

    ret = esp_ssl_tls13_calc_finished_core(hash_alg, base_key,
...
exit:
    ...
    esp_mbedtls_platform_zeroize(base_key, base_key_len);
    esp_mbedtls_platform_zeroize(transcript, sizeof(transcript));
    return ret;
}

int esp_mbedtls_ssl_tls13_create_psk_binder(mbedtls_ssl_context *ssl,
                                        ...
                                        unsigned char *result)
{
    ...
    ret = esp_mbedtls_ssl_tls13_evolve_secret(hash_alg,
                                          NULL,           /* Old secret */
                                          psk, psk_len,   /* Input      */
                                          early_secret);
    if (ret != 0) {
    ...
    MBEDTLS_SSL_DEBUG_BUF(4, "esp_mbedtls_ssl_tls13_create_psk_binder",
                          early_secret, hash_len);
    ...
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_tls13_derive_secret", ret);
        goto exit;
    }
    ...
    ret = esp_ssl_tls13_calc_finished_core(hash_alg, binder_key, transcript,
                                       result, &actual_len);
    if (ret != 0) {
        goto exit;
    }
...
exit:
    ...
    esp_mbedtls_platform_zeroize(early_secret, sizeof(early_secret));
    esp_mbedtls_platform_zeroize(binder_key,   sizeof(binder_key));
    return ret;
}

int esp_mbedtls_ssl_tls13_populate_transform(
    ...
    mbedtls_ssl_context *ssl /* DEBUG ONLY */)
{
...
#if !defined(MBEDTLS_USE_PSA_CRYPTO)
    cipher_info = esp_mbedtls_cipher_info_from_type(ciphersuite_info->cipher);
    if (cipher_info == NULL) {
...
#if !defined(MBEDTLS_USE_PSA_CRYPTO)
    if ((ret = esp_mbedtls_cipher_setkey(&transform->cipher_ctx_enc,
                                     key_enc, (int) esp_mbedtls_cipher_info_get_key_bitlen(cipher_info),
                                     MBEDTLS_ENCRYPT)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_cipher_setkey", ret);
        return ret;
    }

    if ((ret = esp_mbedtls_cipher_setkey(&transform->cipher_ctx_dec,
                                     key_dec, (int) esp_mbedtls_cipher_info_get_key_bitlen(cipher_info),
                                     MBEDTLS_DECRYPT)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_cipher_setkey", ret);
        return ret;
    }
    ...
    if ((status = esp_mbedtls_ssl_cipher_to_psa((mbedtls_cipher_type_t) ciphersuite_info->cipher,
        ...
        MBEDTLS_SSL_DEBUG_RET(
            1, "esp_mbedtls_ssl_cipher_to_psa", PSA_TO_MBEDTLS_ERR(status));
        return PSA_TO_MBEDTLS_ERR(status);
    }
    ...
    if (alg != MBEDTLS_SSL_NULL_CIPHER) {
        ...
        if ((status = esp_psa_import_key(&attributes,
            ...
            MBEDTLS_SSL_DEBUG_RET(
                1, "esp_psa_import_key", PSA_TO_MBEDTLS_ERR(status));
            return PSA_TO_MBEDTLS_ERR(status);
        }
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_get_cipher_key_info(
    const mbedtls_ssl_ciphersuite_t *ciphersuite_info,
    size_t *esp_key_len, size_t *iv_len)
{
    psa_key_type_t key_type;
    ...
    status = esp_mbedtls_ssl_cipher_to_psa((mbedtls_cipher_type_t) ciphersuite_info->cipher, taglen,
                                       &alg, &key_type, &key_bits);
    if (status != PSA_SUCCESS) {
        return PSA_TO_MBEDTLS_ERR(status);
    }

    *esp_key_len = PSA_BITS_TO_BYTES(key_bits);
...
/*
 * esp_ssl_tls13_generate_early_key() generates the key necessary for protecting
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_generate_early_key(mbedtls_ssl_context *ssl,
                                        mbedtls_ssl_key_set *traffic_keys)
{
    ...
    MBEDTLS_SSL_DEBUG_MSG(2, ("=> esp_ssl_tls13_generate_early_key"));

    ret = esp_ssl_tls13_get_cipher_key_info(ciphersuite_info, &esp_key_len, &iv_len);
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_ssl_tls13_get_cipher_key_info", ret);
        goto cleanup;
    }

    md_type = (mbedtls_md_type_t) ciphersuite_info->mac;

    hash_alg = esp_mbedtls_md_psa_alg_from_type((mbedtls_md_type_t) ciphersuite_info->mac);
    ...
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1,
                              "esp_mbedtls_ssl_get_handshake_transcript",
                              ret);
        goto cleanup;
    }

    ret = esp_mbedtls_ssl_tls13_derive_early_secrets(
        hash_alg, handshake->tls13_master_secrets.early,
        transcript, transcript_len, &tls13_early_secrets);
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(
            1, "esp_mbedtls_ssl_tls13_derive_early_secrets", ret);
        goto cleanup;
    }
    ...
    ret = esp_ssl_tls13_make_traffic_key(
        hash_alg,
        tls13_early_secrets.client_early_traffic_secret,
        hash_len, traffic_keys->client_write_key, esp_key_len,
        traffic_keys->client_write_iv, iv_len);
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_ssl_tls13_make_traffic_key", ret);
        goto cleanup;
    }
    traffic_keys->esp_key_len = esp_key_len;
    traffic_keys->iv_len = iv_len;

    MBEDTLS_SSL_DEBUG_BUF(4, "client early write_key",
                          traffic_keys->client_write_key,
                          traffic_keys->esp_key_len);
...
cleanup:
    ...
    esp_mbedtls_platform_zeroize(
        &tls13_early_secrets, sizeof(mbedtls_ssl_tls13_early_secrets));
    esp_mbedtls_platform_zeroize(transcript, sizeof(transcript));
    return ret;
}

int esp_mbedtls_ssl_tls13_compute_early_transform(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    mbedtls_ssl_key_set traffic_keys;
     ...
     * key material. */
    ret = esp_ssl_tls13_generate_early_key(ssl, &traffic_keys);
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_ssl_tls13_generate_early_key",
                              ret);
        goto cleanup;
    }

    transform_earlydata = esp_mbedtls_calloc(1, sizeof(mbedtls_ssl_transform));
    if (transform_earlydata == NULL) {
        ret = MBEDTLS_ERR_SSL_ALLOC_FAILED;
        goto cleanup;
    }

    ret = esp_mbedtls_ssl_tls13_populate_transform(
        transform_earlydata,
        ssl->conf->endpoint,
        ...
        ssl);
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_tls13_populate_transform", ret);
        goto cleanup;
    }
    handshake->transform_earlydata = transform_earlydata;

cleanup:
    esp_mbedtls_platform_zeroize(&traffic_keys, sizeof(traffic_keys));
    if (ret != 0) {
        esp_mbedtls_free(transform_earlydata);
    }
...
int esp_mbedtls_ssl_tls13_key_schedule_stage_early(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    psa_algorithm_t hash_alg;
    mbedtls_ssl_handshake_params *handshake = ssl->handshake;
    ...
    hash_alg = esp_mbedtls_md_psa_alg_from_type((mbedtls_md_type_t) handshake->ciphersuite_info->mac);
    ...
    ret = esp_mbedtls_ssl_tls13_evolve_secret(hash_alg, NULL, psk, psk_len,
    ...
    MBEDTLS_SSL_DEBUG_BUF(4, "esp_mbedtls_ssl_tls13_key_schedule_stage_early",
                          handshake->tls13_master_secrets.early,
                          PSA_HASH_LENGTH(hash_alg));
    return 0;
}
 ...
 *
 *        esp_ssl_tls13_generate_handshake_keys() generates keys necessary for
 *        protecting the handshake messages, as described in Section 7 of
 *        RFC 8446.
 *
 * \param ssl  The SSL context to operate on. This must be in
 *             key schedule stage \c Handshake, see
 *             esp_ssl_tls13_key_schedule_stage_handshake().
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_generate_handshake_keys(mbedtls_ssl_context *ssl,
                                             mbedtls_ssl_key_set *traffic_keys)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    mbedtls_md_type_t md_type;
    ...
    MBEDTLS_SSL_DEBUG_MSG(2, ("=> esp_ssl_tls13_generate_handshake_keys"));
    ...
    ret = esp_mbedtls_ssl_tls13_derive_handshake_secrets(
        hash_alg, handshake->tls13_master_secrets.handshake,
        transcript, transcript_len, tls13_hs_secrets);
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_tls13_derive_handshake_secrets",
                              ret);
        return ret;
    }
    ...
    ret = esp_mbedtls_ssl_tls13_make_traffic_keys(
        hash_alg,
        tls13_hs_secrets->client_handshake_traffic_secret,
        tls13_hs_secrets->server_handshake_traffic_secret,
        hash_len, esp_key_len, iv_len, traffic_keys);
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_tls13_make_traffic_keys", ret);
        goto exit;
    }
 ...
 *
 *        In the handshake stage, esp_ssl_tls13_generate_handshake_keys()
 *        can be used to derive the handshake traffic keys.
 *
 * \param ssl  The SSL context to operate on. This must be in key schedule
 *             stage \c Early.
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_key_schedule_stage_handshake(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    mbedtls_ssl_handshake_params *handshake = ssl->handshake;
    psa_algorithm_t const hash_alg = esp_mbedtls_md_psa_alg_from_type(
    ...
    if (esp_mbedtls_ssl_tls13_key_exchange_mode_with_ephemeral(ssl)) {
        if (esp_mbedtls_ssl_tls13_named_group_is_ecdhe(handshake->offered_group_id) ||
            esp_mbedtls_ssl_tls13_named_group_is_ffdh(handshake->offered_group_id)) {
#if defined(PSA_WANT_ALG_ECDH) || defined(PSA_WANT_ALG_FFDH)
            psa_algorithm_t alg =
                esp_mbedtls_ssl_tls13_named_group_is_ecdhe(handshake->offered_group_id) ?
            ...
            status = esp_psa_get_key_attributes(handshake->xxdh_psa_privkey,
                                            &key_attributes);
            if (status != PSA_SUCCESS) {
                ret = PSA_TO_MBEDTLS_ERR(status);
            }
            ...
            shared_secret = esp_mbedtls_calloc(1, shared_secret_len);
...
cleanup:
    if (shared_secret != NULL) {
        esp_mbedtls_zeroize_and_free(shared_secret, shared_secret_len);
    }
 ...
 *
 *        esp_ssl_tls13_generate_application_keys() generates application traffic
 ...
 *             esp_ssl_tls13_key_schedule_stage_application().
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_generate_application_keys(
    ...
    mbedtls_ssl_key_set *traffic_keys)
{
    ...
    size_t esp_key_len = 0, iv_len = 0;

    MBEDTLS_SSL_DEBUG_MSG(2, ("=> derive application traffic keys"));

    /* Extract basic information about hash and ciphersuite */

    ret = esp_ssl_tls13_get_cipher_key_info(handshake->ciphersuite_info,
                                        &esp_key_len, &iv_len);
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_ssl_tls13_get_cipher_key_info", ret);
        goto cleanup;
    ...
    ret = esp_mbedtls_ssl_tls13_derive_application_secrets(
        hash_alg, handshake->tls13_master_secrets.app,
        transcript, transcript_len, app_secrets);
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(
            1, "esp_mbedtls_ssl_tls13_derive_application_secrets", ret);
        goto cleanup;
    }
    ...
    MBEDTLS_SSL_DEBUG_BUF(4, "client application_write_key:",
                          traffic_keys->client_write_key, esp_key_len);
    MBEDTLS_SSL_DEBUG_BUF(4, "server application write key",
                          traffic_keys->server_write_key, esp_key_len);
...
int esp_mbedtls_ssl_tls13_compute_handshake_transform(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    mbedtls_ssl_key_set traffic_keys;
    ...
    ret = esp_ssl_tls13_key_schedule_stage_handshake(ssl);
    if (ret != 0) {
     ...
     * key material. */
    ret = esp_ssl_tls13_generate_handshake_keys(ssl, &traffic_keys);
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_ssl_tls13_generate_handshake_keys",
                              ret);
        goto cleanup;
    }

    transform_handshake = esp_mbedtls_calloc(1, sizeof(mbedtls_ssl_transform));
...
cleanup:
    esp_mbedtls_platform_zeroize(&traffic_keys, sizeof(traffic_keys));
    if (ret != 0) {
        esp_mbedtls_free(transform_handshake);
    }

    return ret;
}

int esp_mbedtls_ssl_tls13_compute_resumption_master_secret(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    mbedtls_md_type_t md_type;
    mbedtls_ssl_handshake_params *handshake = ssl->handshake;
    ...
    MBEDTLS_SSL_DEBUG_MSG(
        2, ("=> esp_mbedtls_ssl_tls13_compute_resumption_master_secret"));
    ...
    ret = esp_mbedtls_ssl_tls13_derive_resumption_master_secret(
        esp_mbedtls_md_psa_alg_from_type(md_type),
        handshake->tls13_master_secrets.app,
        transcript, transcript_len,
        &ssl->session_negotiate->app_secrets);
    if (ret != 0) {
    ...
    esp_mbedtls_platform_zeroize(&handshake->tls13_master_secrets,
    ...
    MBEDTLS_SSL_DEBUG_BUF(
        ...
        PSA_HASH_LENGTH(esp_mbedtls_md_psa_alg_from_type(md_type)));
...
int esp_mbedtls_ssl_tls13_compute_application_transform(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    mbedtls_ssl_key_set traffic_keys;
    mbedtls_ssl_transform *transform_application = NULL;

    ret = esp_ssl_tls13_key_schedule_stage_application(ssl);
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1,
                              "esp_ssl_tls13_key_schedule_stage_application", ret);
        goto cleanup;
    }

    ret = esp_ssl_tls13_generate_application_keys(ssl, &traffic_keys);
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1,
                              "esp_ssl_tls13_generate_application_keys", ret);
        goto cleanup;
    }

    transform_application =
        esp_mbedtls_calloc(1, sizeof(mbedtls_ssl_transform));
...
cleanup:
    ...
    esp_mbedtls_platform_zeroize(&traffic_keys, sizeof(traffic_keys));
    if (ret != 0) {
        esp_mbedtls_free(transform_application);
    }
    return ret;
}

#if defined(MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_SOME_PSK_ENABLED)
int esp_mbedtls_ssl_tls13_export_handshake_psk(mbedtls_ssl_context *ssl,
                                           unsigned char **psk,
                                           size_t *psk_len)
{
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    ...
    *psk = NULL;
    ...
    status = esp_psa_get_key_attributes(ssl->handshake->psk_opaque, &key_attributes);
    if (status != PSA_SUCCESS) {
        return PSA_TO_MBEDTLS_ERR(status);
    }

    *psk_len = PSA_BITS_TO_BYTES(psa_get_key_bits(&key_attributes));
    *psk = esp_mbedtls_calloc(1, *psk_len);
    if (*psk == NULL) {
        return MBEDTLS_ERR_SSL_ALLOC_FAILED;
    }

    status = esp_psa_export_key(ssl->handshake->psk_opaque,
