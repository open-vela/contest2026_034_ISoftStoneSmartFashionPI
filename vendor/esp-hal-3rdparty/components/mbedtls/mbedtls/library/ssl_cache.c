...
void esp_mbedtls_ssl_cache_init(mbedtls_ssl_cache_context *cache)
{
    ...
    esp_mbedtls_mutex_init(&cache->mutex);
#endif
}

MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_cache_find_entry(mbedtls_ssl_cache_context *cache,
                                ...
                                mbedtls_ssl_cache_entry **dst)
{
    ...
    mbedtls_time_t t = esp_mbedtls_time(NULL);
#endif
...
int esp_mbedtls_ssl_cache_get(void *data,
                          unsigned char const *session_id,
                          size_t session_id_len,
...
#if defined(MBEDTLS_THREADING_C)
    if ((ret = esp_mbedtls_mutex_lock(&cache->mutex)) != 0) {
        return ret;
    }
#endif

    ret = esp_ssl_cache_find_entry(cache, session_id, session_id_len, &entry);
    if (ret != 0) {
        goto exit;
    }

    ret = esp_mbedtls_ssl_session_load(session,
                                   entry->session,
                                   entry->session_len);
    if (ret != 0) {
        goto exit;
...
exit:
#if defined(MBEDTLS_THREADING_C)
    if (esp_mbedtls_mutex_unlock(&cache->mutex) != 0) {
...
static void esp_ssl_cache_entry_zeroize(mbedtls_ssl_cache_entry *entry)
{
    if (entry == NULL) {
        return;
    }
    ...
    if (entry->session != NULL) {
        esp_mbedtls_zeroize_and_free(entry->session, entry->session_len);
    }

    /* zeroize the whole entry structure */
    esp_mbedtls_platform_zeroize(entry, sizeof(mbedtls_ssl_cache_entry));
}

MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_cache_pick_writing_slot(mbedtls_ssl_cache_context *cache,
                                       unsigned char const *session_id,
                                       size_t session_id_len,
                                       mbedtls_ssl_cache_entry **dst)
{
#if defined(MBEDTLS_HAVE_TIME)
    mbedtls_time_t t = esp_mbedtls_time(NULL), oldest = 0;
    ...
    if (count < cache->max_entries) {
        /* Create new entry */
        cur = esp_mbedtls_calloc(1, sizeof(mbedtls_ssl_cache_entry));
        if (cur == NULL) {
...
found:
    ...
    if (cur->session != NULL) {
        /* `esp_ssl_cache_entry_zeroize` would break the chain,
         * so we reuse `old` to record `next` temporarily. */
        old = cur->next;
        esp_ssl_cache_entry_zeroize(cur);
        cur->next = old;
    }
...
int esp_mbedtls_ssl_cache_set(void *data,
                          unsigned char const *session_id,
                          size_t session_id_len,
                          const mbedtls_ssl_session *session)
{
    ...
    ret = esp_ssl_cache_pick_writing_slot(cache,
    ...
    ret = esp_mbedtls_ssl_session_save(session, NULL, 0, &session_serialized_len);
    if (ret != MBEDTLS_ERR_SSL_BUFFER_TOO_SMALL) {
        goto exit;
    }

    session_serialized = esp_mbedtls_calloc(1, session_serialized_len);
    ...
    ret = esp_mbedtls_ssl_session_save(session,
                                   session_serialized,
...
exit:
    ...
    if (session_serialized != NULL) {
        esp_mbedtls_zeroize_and_free(session_serialized, session_serialized_len);
        session_serialized = NULL;
    }
...
int esp_mbedtls_ssl_cache_remove(void *data,
                             unsigned char const *session_id,
                             size_t session_id_len)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
...
free:
    esp_ssl_cache_entry_zeroize(entry);
    esp_mbedtls_free(entry);
    ret = 0;
...
#if defined(MBEDTLS_HAVE_TIME)
void esp_mbedtls_ssl_cache_set_timeout(mbedtls_ssl_cache_context *cache, int timeout)
{
    if (timeout < 0) {
        timeout = 0;
    }
...
void esp_mbedtls_ssl_cache_set_max_entries(mbedtls_ssl_cache_context *cache, int max)
{
    if (max < 0) {
        max = 0;
    }
...
void esp_mbedtls_ssl_cache_free(mbedtls_ssl_cache_context *cache)
{
    ...
    while (cur != NULL) {
        ...
        esp_ssl_cache_entry_zeroize(prv);
        esp_mbedtls_free(prv);
    }

#if defined(MBEDTLS_THREADING_C)
    esp_mbedtls_mutex_free(&cache->mutex);
#endif
    cache->chain = NULL;
}
