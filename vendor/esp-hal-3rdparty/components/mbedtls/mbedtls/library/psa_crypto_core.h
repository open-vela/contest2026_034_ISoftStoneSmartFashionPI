...
typedef struct {
     ...
     * esp_psa_key_slot_state_transition.
     *
     * The state variable is used to help determine whether library functions
     * which operate on the slot succeed. For example, esp_psa_finish_key_creation,
     ...
     * For example, esp_psa_get_and_lock_key_slot_in_memory, which finds a slot
     ...
     *
     * A function must call esp_psa_register_read(slot) before reading the current
     * contents of the slot for an operation.
     * They then must call esp_psa_unregister_read(slot) once they have finished
     ...
     * A function must call esp_psa_key_slot_has_readers(slot) to check if
     ...
     * Format as specified in esp_psa_export_key(). */
    struct key_data {
        uint8_t *data;
        size_t bytes;
    } key;
 ...
 */
static inline int esp_psa_key_slot_has_readers(const psa_key_slot_t *slot)
{
 ...
 */
psa_status_t esp_psa_generate_key_internal(const psa_key_attributes_t *attributes,
                                       const psa_key_production_parameters_t *params,
                                       size_t params_data_length,
                                       uint8_t *key_buffer,
 ...
 *       transparent drivers.
 *
 * \note This function will call the driver for esp_psa_sign_hash
 ...
 *       transparent drivers.
 *
 * \note This function will call the driver for esp_psa_verify_hash
 *       and go through driver dispatch again.
 *
 * \param[in]  attributes       The attributes of the key to use for the
 *                              operation.
 ...
 */
psa_status_t esp_psa_key_agreement_raw_builtin(
    const psa_key_attributes_t *attributes,
 ...
 *                              esp_mbedtls_psa_sign_hash_complete().
 */
 ...
 * \return                      Number of ops that were completed
 *                              in the last call to \c
 *                              esp_mbedtls_psa_verify_hash_complete().
 */
 ...
 *                              esp_psa_crypto_local_input_alloc().
 */
 ...
 *                              esp_psa_crypto_local_output_free().
 *
 ...
 *                              populated by a previous call to
 *                              esp_psa_crypto_local_output_alloc().
