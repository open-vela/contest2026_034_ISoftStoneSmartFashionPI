 ...
 * It is the responsibility of the caller to call esp_psa_unregister_read(slot)
 ...
 */
static inline psa_status_t esp_psa_key_slot_state_transition(
 ...
 */
static inline psa_status_t esp_psa_register_read(psa_key_slot_t *slot)
{
 ...
 * this function will call esp_psa_wipe_key_slot().
 * If multi-threading is enabled, the caller must hold the
 * global key slot mutex.
 *
...
/** Wrap a call to esp_psa_unregister_read in the global key slot mutex.
 *
 * If threading is disabled, this simply calls esp_psa_unregister_read.
 *
 * \note To ease the handling of errors in retrieving a key slot
 *       a NULL input pointer is valid, and the function returns
 *       successfully without doing anything in that case.
