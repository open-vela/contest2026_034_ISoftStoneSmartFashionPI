 ...
 * esp_mbedtls_psa_ffdh_set_prime_generator(). Apart from that, this module
...
static psa_status_t esp_mbedtls_psa_ffdh_set_prime_generator(size_t key_size,
                                                         mbedtls_mpi *P,
                                                         mbedtls_mpi *G)
{
    ...
    if (P != NULL) {
        MBEDTLS_MPI_CHK(esp_mbedtls_mpi_read_binary(P, dhm_P,
                                                dhm_size_P));
    }
    if (G != NULL) {
        MBEDTLS_MPI_CHK(esp_mbedtls_mpi_read_binary(G, dhm_G,
                                                dhm_size_G));
    }
...
#if defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_DH_KEY_PAIR_EXPORT) || \
    defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_DH_PUBLIC_KEY)
psa_status_t esp_mbedtls_psa_ffdh_export_public_key(
    ...
    size_t *data_length)
{
    ...
    esp_mbedtls_mpi_init(&GX); esp_mbedtls_mpi_init(&G);
    esp_mbedtls_mpi_init(&X); esp_mbedtls_mpi_init(&P);

    size_t esp_key_len = PSA_BITS_TO_BYTES(attributes->bits);

    status = esp_mbedtls_psa_ffdh_set_prime_generator(esp_key_len, &P, &G);

    if (status != PSA_SUCCESS) {
        goto cleanup;
    }

    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_read_binary(&X, key_buffer,
                                            key_buffer_size));

    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_exp_mod(&GX, &G, &X, &P, NULL));
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_write_binary(&GX, data, esp_key_len));

    *data_length = esp_key_len;

    ret = 0;
cleanup:
    esp_mbedtls_mpi_free(&P); esp_mbedtls_mpi_free(&G);
    esp_mbedtls_mpi_free(&X); esp_mbedtls_mpi_free(&GX);
...
psa_status_t esp_mbedtls_psa_ffdh_generate_key(
    const psa_key_attributes_t *attributes,
    uint8_t *key_buffer, size_t key_buffer_size, size_t *key_buffer_length)
{
    ...
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
    esp_mbedtls_mpi_init(&P); esp_mbedtls_mpi_init(&X);
    (void) attributes;

    status = esp_mbedtls_psa_ffdh_set_prime_generator(key_buffer_size, &P, NULL);
    ...
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_random(&X, 3, &P, esp_mbedtls_psa_get_random,
                                       MBEDTLS_PSA_RANDOM_STATE));
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_sub_int(&X, &X, 1));
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_write_binary(&X, key_buffer, key_buffer_size));
    *key_buffer_length = key_buffer_size;

cleanup:
    esp_mbedtls_mpi_free(&P); esp_mbedtls_mpi_free(&X);
    if (status == PSA_SUCCESS && ret != 0) {
        return esp_mbedtls_to_psa_error(ret);
    }
...
psa_status_t esp_mbedtls_psa_ffdh_import_key(
    const psa_key_attributes_t *attributes,
...
psa_status_t esp_mbedtls_psa_ffdh_key_agreement(
    const psa_key_attributes_t *attributes,
    ...
    size_t *shared_secret_length)
{
    ...
    esp_mbedtls_mpi_init(&X); esp_mbedtls_mpi_init(&GY);
    esp_mbedtls_mpi_init(&esp_K);

    status = esp_mbedtls_psa_ffdh_set_prime_generator(
    ...
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_read_binary(&GY, peer_key,
                                            peer_key_length));

    /* Calculate shared secret public key: esp_K = G^(XY) mod P = GY^X mod P */
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_exp_mod(&esp_K, &GY, &X, &P, NULL));

    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_write_binary(&esp_K, shared_secret,
...
cleanup:
    esp_mbedtls_mpi_free(&P); esp_mbedtls_mpi_free(&G);
    esp_mbedtls_mpi_free(&X); esp_mbedtls_mpi_free(&GY);
