 ...
 *       drivers.
 *
 * If an error occurs at any step after a call to esp_mbedtls_psa_hash_setup(), the
 * operation will need to be reset by a call to esp_mbedtls_psa_hash_abort(). The
 * core may call esp_mbedtls_psa_hash_abort() at any time after the operation
 * has been initialized.
 *
 * After a successful call to esp_mbedtls_psa_hash_setup(), the core must
 * eventually terminate the operation. The following events terminate an
 * operation:
 * - A successful call to esp_mbedtls_psa_hash_finish() or mbedtls_psa_hash_verify().
 * - A call to esp_mbedtls_psa_hash_abort().
 *
 ...
 * to calling esp_mbedtls_psa_hash_setup() on \p target_operation with the same
 * algorithm that \p source_operation was set up for, then
 * esp_mbedtls_psa_hash_update() on \p target_operation with the same input that
 ...
 *       drivers.
 *
 * The application must call esp_mbedtls_psa_hash_setup() before calling this function.
 *
 * If this function returns an error status, the operation enters an error
 * state and must be aborted by calling esp_mbedtls_psa_hash_abort().
 *
 ...
 * the inputs passed to preceding calls to esp_mbedtls_psa_hash_update().
 *
 ...
 * esp_mbedtls_psa_hash_setup() again.
 *
 * You may call this function any time after the operation object has
 * been initialized by one of the methods described in #psa_hash_operation_t.
 *
 * In particular, calling esp_mbedtls_psa_hash_abort() after the operation has been
 * terminated by a call to esp_mbedtls_psa_hash_abort(), esp_mbedtls_psa_hash_finish() or
