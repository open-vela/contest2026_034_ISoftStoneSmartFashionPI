 ...
 * esp_mbedtls_psa_aead_encrypt_setup(), the operation is reset by the PSA core by a
 * call to esp_mbedtls_psa_aead_abort(). The PSA core may call
 * esp_mbedtls_psa_aead_abort() at any time after the operation has been
 ...
 *
 * If an error occurs at any step after a call to
 * esp_mbedtls_psa_aead_decrypt_setup(), the PSA core resets the operation by a
 * call to esp_mbedtls_psa_aead_abort(). The PSA core may call
 * esp_mbedtls_psa_aead_abort() at any time after the operation has been
 * initialized, and is required to when the operation is no longer needed.
 *
 ...
 *
 * The PSA core calls esp_mbedtls_psa_aead_encrypt_setup() or
 * esp_mbedtls_psa_aead_decrypt_setup() before calling this function.
 *
 * If this function returns an error status, the PSA core will call
 * esp_mbedtls_psa_aead_abort().
 *
 ...
 *       drivers.
 *
 * The PSA core calls this function before calling esp_mbedtls_psa_aead_update_ad()
 * or esp_mbedtls_psa_aead_update() if the algorithm for the operation requires it.
 ...
 * esp_mbedtls_psa_aead_set_nonce().
 *
 ...
 * passing data to encrypt or decrypt with esp_mbedtls_psa_aead_update().
 *
 * Before calling this function, the PSA core will:
 *    1. Call either esp_mbedtls_psa_aead_encrypt_setup() or
 *       esp_mbedtls_psa_aead_decrypt_setup().
 *    2. Set the nonce with esp_mbedtls_psa_aead_set_nonce().
 *
 * If this function returns an error status, the PSA core will call
 * esp_mbedtls_psa_aead_abort().
 *
 ...
 * Before calling this function, the PSA core will:
 *    1. Call either esp_mbedtls_psa_aead_encrypt_setup() or
 *       esp_mbedtls_psa_aead_decrypt_setup(). The choice of setup function
 *       determines whether this function encrypts or decrypts its input.
 *    2. Set the nonce with esp_mbedtls_psa_aead_set_nonce().
 *    3. Call esp_mbedtls_psa_aead_update_ad() to pass all the additional data.
 *
 * If this function returns an error status, the PSA core will call
 * esp_mbedtls_psa_aead_abort().
 *
 ...
 * call to esp_mbedtls_psa_aead_update(), esp_mbedtls_psa_aead_finish() provides
 ...
 * esp_mbedtls_psa_aead_encrypt_setup().
 *
 * This function finishes the authentication of the additional data
 * formed by concatenating the inputs passed to preceding calls to
 * esp_mbedtls_psa_aead_update_ad() with the plaintext formed by concatenating the
 * inputs passed to preceding calls to esp_mbedtls_psa_aead_update().
 *
 * This function has two output buffers:
 * - \p ciphertext contains trailing ciphertext that was buffered from
 *   preceding calls to esp_mbedtls_psa_aead_update().
 * - \p tag contains the authentication tag.
 *
 * Whether or not this function returns successfully, the PSA core subsequently
 * calls esp_mbedtls_psa_aead_abort() to deactivate the operation.
 *
 * \param[in,out] operation     Active AEAD operation.
 ...
 * esp_mbedtls_psa_aead_encrypt_setup() or esp_mbedtls_psa_aead_decrypt_setup() again.
 *
 ...
 *
 * In particular, calling esp_mbedtls_psa_aead_abort() after the operation has been
 * terminated by a call to esp_mbedtls_psa_aead_abort() or
 * esp_mbedtls_psa_aead_finish() is safe and has no effect.
 *
