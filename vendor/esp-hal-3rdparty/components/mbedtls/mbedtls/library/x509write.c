                                      ...
                                      const mbedtls_x509_san_list *san_list)
{
        ...
        switch (cur->node.type) {
            ...
            case MBEDTLS_X509_SAN_DIRECTORY_NAME:
            {
                ...
                while (chunk != NULL) {
                    ...
                    // See esp_x509_write_name for more information.
    ...
    /* Allocate buffer */
    buf = esp_mbedtls_calloc(1, buflen);
    if (buf == NULL) {
        ...
        switch (cur->node.type) {
            ...
            case MBEDTLS_X509_SAN_IP_ADDRESS:
            {
                ...
                MBEDTLS_ASN1_CHK_CLEANUP_ADD(single_san_len,
                                             esp_mbedtls_asn1_write_raw_buffer(
                                                 &p, buf,
                                                 unstructured_name, unstructured_name_len));
                MBEDTLS_ASN1_CHK_CLEANUP_ADD(single_san_len, esp_mbedtls_asn1_write_len(
            ...
            case MBEDTLS_X509_SAN_DIRECTORY_NAME:
                MBEDTLS_ASN1_CHK_CLEANUP_ADD(single_san_len,
                                             esp_mbedtls_x509_write_names(&p, buf,
                                                                      (mbedtls_asn1_named_data *) &
                                                                      cur->node
                                                                      .san.directory_name));
                MBEDTLS_ASN1_CHK_CLEANUP_ADD(single_san_len,
                                             esp_mbedtls_asn1_write_len(&p, buf, single_san_len));
                MBEDTLS_ASN1_CHK_CLEANUP_ADD(single_san_len,
                                             esp_mbedtls_asn1_write_tag(&p, buf,
    ...
    MBEDTLS_ASN1_CHK_CLEANUP_ADD(len, esp_mbedtls_asn1_write_len(&p, buf, len));
    MBEDTLS_ASN1_CHK_CLEANUP_ADD(len,
                                 esp_mbedtls_asn1_write_tag(&p, buf,
                                                        MBEDTLS_ASN1_CONSTRUCTED |
                                                        MBEDTLS_ASN1_SEQUENCE));

    ret = esp_mbedtls_x509_set_extension(extensions,
