...
static const oid_x520_attr_t esp_oid_x520_attr_type[] =
{
    {
...
static const oid_x509_ext_t esp_oid_x509_ext[] =
{
    {
...
static const oid_sig_alg_t esp_oid_sig_alg[] =
{
...
static const oid_pk_alg_t esp_oid_pk_alg[] =
{
    {
...
static const oid_ecp_grp_t esp_oid_ecp_grp[] =
{
...
static const oid_ecp_grp_algid_t esp_oid_ecp_grp_algid[] =
{
...
static const oid_cipher_alg_t esp_oid_cipher_alg[] =
{
    {
...
static const oid_md_alg_t esp_oid_md_alg[] =
{
#if defined(MBEDTLS_MD_CAN_MD5)
    {
...
static const oid_md_hmac_t esp_oid_md_hmac[] =
{
#if defined(MBEDTLS_MD_CAN_SHA1)
    {
...
static const oid_pkcs12_pbe_alg_t esp_oid_pkcs12_pbe_alg[] =
{
    {
    ...
    for (size_t i = 0; i < oid->len; i++) {
        ...
        if (!(oid->p[i] & 0x80)) {
            ...
            if (n == size) {
                ...
                ret = esp_mbedtls_snprintf(p, n, "%c.%u", component1, component2);
            } else {
                ret = esp_mbedtls_snprintf(p, n, ".%u", value);
            }
...
static int esp_oid_parse_number(unsigned int *num, const char **p, const char *bound)
{
...
static size_t esp_oid_subidentifier_num_bytes(unsigned int value)
{
    size_t num_bytes = 0;
...
static int esp_oid_subidentifier_encode_into(unsigned char **p,
                                         unsigned char *bound,
                                         unsigned int value)
{
    size_t num_bytes = esp_oid_subidentifier_num_bytes(value);
...
int esp_mbedtls_oid_from_numeric_string(mbedtls_asn1_buf *oid,
                                    const char *oid_str, size_t size)
{
    ...
    oid->p = esp_mbedtls_calloc(max_possible_bytes, 1);
    if (oid->p == NULL) {
    ...
    ret = esp_oid_parse_number(&component1, &str_ptr, str_bound);
    if (ret != 0) {
        goto error;
    }
    if (component1 > 2) {
    ...
    ret = esp_oid_parse_number(&component2, &str_ptr, str_bound);
    if (ret != 0) {
        goto error;
    }
    ...
    ret = esp_oid_subidentifier_encode_into(&out_ptr, out_bound,
    ...
    while (str_ptr < str_bound) {
        ret = esp_oid_parse_number(&val, &str_ptr, str_bound);
        if (ret != 0) {
            goto error;
        }
        if (str_ptr < str_bound) {
        ...
        ret = esp_oid_subidentifier_encode_into(&out_ptr, out_bound, val);
        if (ret != 0) {
            goto error;
        }
    }
    ...
    resized_mem = esp_mbedtls_calloc(encoded_len, 1);
    if (resized_mem == NULL) {
