...
static int esp_key_type_is_raw_bytes(psa_key_type_t type)
{
...
/* Initialization flags for esp_global_data::initialized */
...
static psa_global_data_t esp_global_data;

static uint8_t esp_psa_get_initialized(void)
{
    uint8_t initialized;

#if defined(MBEDTLS_THREADING_C)
    esp_mbedtls_mutex_lock(&mbedtls_threading_psa_rngdata_mutex);
#endif /* defined(MBEDTLS_THREADING_C) */

    initialized = esp_global_data.rng_state == RNG_SEEDED;

#if defined(MBEDTLS_THREADING_C)
    esp_mbedtls_mutex_unlock(&mbedtls_threading_psa_rngdata_mutex);
#endif /* defined(MBEDTLS_THREADING_C) */

#if defined(MBEDTLS_THREADING_C)
    esp_mbedtls_mutex_lock(&mbedtls_threading_psa_globaldata_mutex);
#endif /* defined(MBEDTLS_THREADING_C) */

    initialized =
        (initialized && (esp_global_data.initialized == PSA_CRYPTO_SUBSYSTEM_ALL_INITIALISED));

#if defined(MBEDTLS_THREADING_C)
    esp_mbedtls_mutex_unlock(&mbedtls_threading_psa_globaldata_mutex);
#endif /* defined(MBEDTLS_THREADING_C) */

    return initialized;
}

static uint8_t esp_psa_get_drivers_initialized(void)
{
    uint8_t initialized;
    ...
    initialized = (esp_global_data.initialized & PSA_CRYPTO_SUBSYSTEM_DRIVER_WRAPPERS_INITIALIZED) != 0;
...
#define LOCAL_INPUT_ALLOC(input, length, input_copy) \
    status = esp_psa_crypto_local_input_alloc(input, length, \
...
#define LOCAL_INPUT_FREE(input, input_copy) \
    input_copy = NULL; \
    esp_psa_crypto_local_input_free(&LOCAL_INPUT_COPY_OF_##input);
...
#define LOCAL_OUTPUT_ALLOC(output, length, output_copy) \
    status = esp_psa_crypto_local_output_alloc(output, length, \
...
#define LOCAL_OUTPUT_FREE(output, output_copy) \
    ...
    do { \
        ...
        local_output_status = esp_psa_crypto_local_output_free(&LOCAL_OUTPUT_COPY_OF_##output); \
...
int esp_psa_can_do_hash(psa_algorithm_t hash_alg)
{
    (void) hash_alg;
    return esp_psa_get_drivers_initialized();
}

int esp_psa_can_do_cipher(psa_key_type_t key_type, psa_algorithm_t cipher_alg)
{
    (void) key_type;
    (void) cipher_alg;
    return esp_psa_get_drivers_initialized();
...
static int esp_psa_is_dh_key_size_valid(size_t bits)
{
    switch (bits) {
...
psa_status_t esp_mbedtls_to_psa_error(int ret)
{
 ...
 */
static void esp_psa_wipe_tag_output_buffer(uint8_t *output_buffer, psa_status_t status,
...
psa_status_t esp_psa_validate_unstructured_key_bit_size(psa_key_type_t type,
                                                    size_t bits)
{
 ...
 */
MBEDTLS_STATIC_TESTABLE psa_status_t esp_psa_mac_key_can_do(
...
psa_status_t esp_psa_allocate_buffer_to_slot(psa_key_slot_t *slot,
                                         size_t buffer_length)
{
    ...
    slot->key.data = esp_mbedtls_calloc(1, buffer_length);
    if (slot->key.data == NULL) {
        return PSA_ERROR_INSUFFICIENT_MEMORY;
    }
...
psa_status_t esp_psa_copy_key_material_into_slot(psa_key_slot_t *slot,
                                             const uint8_t *data,
                                             size_t data_length)
{
    psa_status_t status = esp_psa_allocate_buffer_to_slot(slot,
                                                      data_length);
    if (status != PSA_SUCCESS) {
        return status;
    }
...
psa_status_t esp_psa_import_key_into_slot(
    ...
    size_t *key_buffer_length, size_t *bits)
{
    ...
    if (esp_key_type_is_raw_bytes(type)) {
        *bits = PSA_BYTES_TO_BITS(data_length);

        status = esp_psa_validate_unstructured_key_bit_size(attributes->type,
                                                        *bits);
        if (status != PSA_SUCCESS) {
            return status;
        }
        ...
        *key_buffer_length = data_length;
        ...
        if (PSA_KEY_TYPE_IS_DH(type)) {
            if (esp_psa_is_dh_key_size_valid(PSA_BYTES_TO_BITS(data_length)) == 0) {
                return PSA_ERROR_NOT_SUPPORTED;
            }
            return esp_mbedtls_psa_ffdh_import_key(attributes,
                                               data, data_length,
        ...
        * defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_DH_PUBLIC_KEY) */
        ...
        if (PSA_KEY_TYPE_IS_ECC(type)) {
            return esp_mbedtls_psa_ecp_import_key(attributes,
                                              data, data_length,
                                              key_buffer, key_buffer_size,
                                              key_buffer_length,
                                              bits);
        ...
        * defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_PUBLIC_KEY) */
        ...
        if (PSA_KEY_TYPE_IS_RSA(type)) {
            return esp_mbedtls_psa_rsa_import_key(attributes,
                                              data, data_length,
                                              key_buffer, key_buffer_size,
                                              key_buffer_length,
                                              bits);
 ...
 */
static psa_algorithm_t esp_psa_key_policy_algorithm_intersection(
    psa_key_type_t key_type,
    ...
    psa_algorithm_t alg2)
{
    ...
    if (PSA_ALG_IS_MAC(alg1) && PSA_ALG_IS_MAC(alg2) &&
        ...
        if (PSA_SUCCESS != esp_psa_mac_key_can_do(alg1, key_type)) {
            return 0;
        }
...
static int esp_psa_key_algorithm_permits(psa_key_type_t key_type,
                                     ...
                                     psa_algorithm_t requested_alg)
{
    ...
    if (PSA_ALG_IS_MAC(policy_alg) &&
        ...
        if (PSA_SUCCESS != esp_psa_mac_key_can_do(policy_alg, key_type)) {
            return 0;
        }
 ...
 */
static psa_status_t esp_psa_key_policy_permits(const psa_key_policy_t *policy,
                                           psa_key_type_t key_type,
                                           psa_algorithm_t alg)
{
    ...
    if (esp_psa_key_algorithm_permits(key_type, policy->alg, alg) ||
        esp_psa_key_algorithm_permits(key_type, policy->alg2, alg)) {
        return PSA_SUCCESS;
    } else {
        return PSA_ERROR_NOT_PERMITTED;
    }
 ...
 */
static psa_status_t esp_psa_restrict_key_policy(
    psa_key_type_t key_type,
    psa_key_policy_t *policy,
    const psa_key_policy_t *constraint)
{
    psa_algorithm_t intersection_alg =
        esp_psa_key_policy_algorithm_intersection(key_type, policy->alg,
                                              constraint->alg);
    psa_algorithm_t intersection_alg2 =
        esp_psa_key_policy_algorithm_intersection(key_type, policy->alg2,
                                              constraint->alg2);
 ...
 * The caller unregisters by calling esp_psa_unregister_read() or
 * esp_psa_unregister_read_under_mutex(). esp_psa_unregister_read() must be called
 * if and only if the caller already holds the global key slot mutex
 * (when mutexes are enabled). esp_psa_unregister_read_under_mutex() encapsulates
 * the unregister with mutex lock and unlock operations.
 */
static psa_status_t esp_psa_get_and_lock_key_slot_with_policy(
    ...
    psa_algorithm_t alg)
{
    ...
    status = esp_psa_get_and_lock_key_slot(key, p_slot);
    if (status != PSA_SUCCESS) {
        return status;
    }
    slot = *p_slot;
    ...
    if (alg != 0) {
        status = esp_psa_key_policy_permits(&slot->attr.policy,
                                        slot->attr.type,
                                        alg);
        if (status != PSA_SUCCESS) {
            goto error;
...
error:
    *p_slot = NULL;
    esp_psa_unregister_read_under_mutex(slot);
 ...
 * esp_psa_get_and_lock_key_slot_with_policy() when there is no opaque key support
 ...
 * if and only if the caller already holds the global key slot mutex
 * (when mutexes are enabled). esp_psa_unregister_read_under_mutex() encapsulates
 * esp_psa_unregister_read() with mutex lock and unlock operations.
 */
static psa_status_t esp_psa_get_and_lock_transparent_key_slot_with_policy(
    mbedtls_svc_key_id_t key,
    psa_key_slot_t **p_slot,
    psa_key_usage_t usage,
    psa_algorithm_t alg)
{
    psa_status_t status = esp_psa_get_and_lock_key_slot_with_policy(key, p_slot,
                                                                usage, alg);
    if (status != PSA_SUCCESS) {
        return status;
    }
    ...
    if (esp_psa_key_lifetime_is_external((*p_slot)->attr.lifetime)) {
        esp_psa_unregister_read_under_mutex(*p_slot);
        *p_slot = NULL;
        return PSA_ERROR_NOT_SUPPORTED;
    }
...
psa_status_t esp_psa_remove_key_data_from_memory(psa_key_slot_t *slot)
{
    if (slot->key.data != NULL) {
        esp_mbedtls_zeroize_and_free(slot->key.data, slot->key.bytes);
    }
...
psa_status_t esp_psa_wipe_key_slot(psa_key_slot_t *slot)
{
    psa_status_t status = esp_psa_remove_key_data_from_memory(slot);
    ...
    switch (slot->state) {
        case PSA_SLOT_FULL:
        /* In this state esp_psa_wipe_key_slot() must only be called if the
...
psa_status_t esp_psa_destroy_key(mbedtls_svc_key_id_t key)
{
    psa_key_slot_t *slot;
    ...
    status = esp_psa_get_and_lock_key_slot(key, &slot);
    if (status != PSA_SUCCESS) {
        return status;
    }
     ...
     * fully destroyed. */
    PSA_THREADING_CHK_GOTO_EXIT(esp_mbedtls_mutex_lock(
    ...
    if (slot->state == PSA_SLOT_PENDING_DELETION) {
        ...
        status = esp_psa_unregister_read(slot);

        PSA_THREADING_CHK_RET(esp_mbedtls_mutex_unlock(
                                  &mbedtls_threading_key_slot_mutex));
     ...
     * automatically wipe the slot when they call esp_psa_unregister_read().
    ...
    overall_status = esp_psa_key_slot_state_transition(slot, PSA_SLOT_FULL,
...
#if defined(MBEDTLS_PSA_CRYPTO_SE_C)
    driver = esp_psa_get_se_driver_entry(slot->attr.lifetime);
    if (driver != NULL) {
        ...
        if (status != PSA_SUCCESS) {
            (void) esp_psa_crypto_stop_transaction();
    ...
    if (!PSA_KEY_LIFETIME_IS_VOLATILE(slot->attr.lifetime)) {
         ...
         * unregisters. */
        status = esp_psa_destroy_persistent_key(slot->attr.id);
        if (overall_status == PSA_SUCCESS) {
            overall_status = status;
        }
    }
...
#if defined(MBEDTLS_PSA_CRYPTO_SE_C)
    if (driver != NULL) {
        status = esp_psa_save_se_persistent_data(driver);
        if (overall_status == PSA_SUCCESS) {
            overall_status = status;
        }
        status = esp_psa_crypto_stop_transaction();
        if (overall_status == PSA_SUCCESS) {
            overall_status = status;
        }
    }
 ...
 */
psa_status_t esp_psa_get_key_attributes(mbedtls_svc_key_id_t key,
                                    psa_key_attributes_t *attributes)
{
    ...
    status = esp_psa_get_and_lock_key_slot_with_policy(key, &slot, 0, 0);
    if (status != PSA_SUCCESS) {
        return status;
    }
    ...
    *attributes = slot->attr;
    ...
    return esp_psa_unregister_read_under_mutex(slot);
}

#if defined(MBEDTLS_PSA_CRYPTO_SE_C)
psa_status_t esp_psa_get_key_slot_number(
    const psa_key_attributes_t *attributes,
...
static psa_status_t esp_psa_export_key_buffer_internal(const uint8_t *key_buffer,
...
psa_status_t esp_psa_export_key_internal(
    const psa_key_attributes_t *attributes,
    ...
    uint8_t *data, size_t data_size, size_t *data_length)
{
    ...
    if (esp_key_type_is_raw_bytes(type) ||
        ...
        return esp_psa_export_key_buffer_internal(
            key_buffer, key_buffer_size,
...
psa_status_t esp_psa_export_key(mbedtls_svc_key_id_t key,
                            ...
                            size_t *data_length)
{
    ...
    *data_length = 0;
     ...
     * esp_psa_get_and_lock_key_slot_with_policy() takes care of this.
     */
    status = esp_psa_get_and_lock_key_slot_with_policy(key, &slot,
    ...
    status = esp_psa_driver_wrapper_export_key(&slot->attr,
...
exit:
#endif
    unlock_status = esp_psa_unregister_read_under_mutex(slot);

    LOCAL_OUTPUT_FREE(data_external, data);
    return (status == PSA_SUCCESS) ? unlock_status : status;
}

psa_status_t esp_psa_export_public_key_internal(
    const psa_key_attributes_t *attributes,
    ...
    size_t *data_length)
{
    ...
    if (PSA_KEY_TYPE_IS_PUBLIC_KEY(type) &&
        ...
        defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_RSA_PUBLIC_KEY)
        return esp_mbedtls_psa_rsa_export_public_key(attributes,
                                                 key_buffer,
                                                 key_buffer_size,
                                                 data,
                                                 data_size,
        ...
        * defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_RSA_PUBLIC_KEY) */
        ...
        defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_PUBLIC_KEY)
        return esp_mbedtls_psa_ecp_export_public_key(attributes,
                                                 key_buffer,
                                                 key_buffer_size,
                                                 data,
                                                 data_size,
        ...
        * defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_PUBLIC_KEY) */
        ...
        defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_DH_PUBLIC_KEY)
        return esp_mbedtls_psa_ffdh_export_public_key(attributes,
                                                  key_buffer,
                                                  key_buffer_size,
                                                  data, data_size,
                                                  data_length);
...
psa_status_t esp_psa_export_public_key(mbedtls_svc_key_id_t key,
                                   uint8_t *data_external,
                                   size_t data_size,
                                   size_t *data_length)
{
    ...
    *data_length = 0;
    ...
    status = esp_psa_driver_wrapper_export_public_key(
 ...
 */
static psa_status_t esp_psa_validate_key_policy(const psa_key_policy_t *policy)
{
 ...
 *
 */
static psa_status_t esp_psa_validate_key_attributes(
    const psa_key_attributes_t *attributes,
    psa_se_drv_table_entry_t **p_drv)
{
    ...
    status = esp_psa_validate_key_location(lifetime, p_drv);
    if (status != PSA_SUCCESS) {
        return status;
    }

    status = esp_psa_validate_key_persistence(lifetime);
    if (status != PSA_SUCCESS) {
        return status;
    }
    ...
    if (PSA_KEY_LIFETIME_IS_VOLATILE(lifetime)) {
        ...
        if (!esp_psa_is_valid_key_id(psa_get_key_id(attributes), 0)) {
            return PSA_ERROR_INVALID_ARGUMENT;
        }
    }

    status = esp_psa_validate_key_policy(&attributes->policy);
    if (status != PSA_SUCCESS) {
        return status;
    }
     ...
     * esp_psa_import_key() needs its own checks. */
 ...
 *
 * If this function fails, call esp_psa_fail_key_creation().
 *
 * This function is intended to be used as follows:
 * -# Call esp_psa_start_key_creation() to allocate a key slot, prepare
 ...
 * -# Call esp_psa_finish_key_creation() to finalize the creation of the slot.
 * In case of failure at any step, stop the sequence and call
 * esp_psa_fail_key_creation().
 *
 ...
 *         You must call esp_psa_fail_key_creation() to wipe and free the slot.
 */
static psa_status_t esp_psa_start_key_creation(
    ...
    psa_se_drv_table_entry_t **p_drv)
{
    ...
    *p_drv = NULL;
    ...
    status = esp_psa_validate_key_attributes(attributes, p_drv);
    if (status != PSA_SUCCESS) {
        return status;
    }

#if defined(MBEDTLS_THREADING_C)
    PSA_THREADING_CHK_RET(esp_mbedtls_mutex_lock(
                              &mbedtls_threading_key_slot_mutex));
#endif
    status = esp_psa_reserve_free_key_slot(&volatile_key_id, p_slot);
#if defined(MBEDTLS_THREADING_C)
    PSA_THREADING_CHK_RET(esp_mbedtls_mutex_unlock(
                              &mbedtls_threading_key_slot_mutex));
#endif
    ...
    if (*p_drv != NULL) {
        psa_key_slot_number_t slot_number;
        status = esp_psa_find_se_slot_for_key(attributes, method, *p_drv,
                                          &slot_number);
        if (status != PSA_SUCCESS) {
            return status;
        }
        ...
        status = esp_psa_copy_key_material_into_slot(
 ...
 *
 * If this function fails, call esp_psa_fail_key_creation().
 * See the documentation of esp_psa_start_key_creation() for the intended use
 * of this function.
 *
 ...
 *         You must call esp_psa_fail_key_creation() to wipe and free the slot.
 */
static psa_status_t esp_psa_finish_key_creation(
    psa_key_slot_t *slot,
    psa_se_drv_table_entry_t *driver,
    mbedtls_svc_key_id_t *key)
{
    ...
    if (!PSA_KEY_LIFETIME_IS_VOLATILE(slot->attr.lifetime)) {
        ...
        if (driver != NULL) {
            ...
            status = esp_psa_save_persistent_key(&slot->attr,
                                             (uint8_t *) &data,
                                             sizeof(data));
        } else
#endif /* MBEDTLS_PSA_CRYPTO_SE_C */
    ...
    if (driver != NULL &&
        esp_psa_crypto_transaction.unknown.type == PSA_CRYPTO_TRANSACTION_CREATE_KEY) {
        status = esp_psa_save_se_persistent_data(driver);
        if (status != PSA_SUCCESS) {
            esp_psa_destroy_persistent_key(slot->attr.id);
    ...
    if (status == PSA_SUCCESS) {
        *key = slot->attr.id;
        status = esp_psa_key_slot_state_transition(slot, PSA_SLOT_FILLING,
                                               PSA_SLOT_FULL);
        if (status != PSA_SUCCESS) {
 ...
 *
 * You may call this function after calling esp_psa_start_key_creation(),
 * or after esp_psa_finish_key_creation() fails. In other circumstances, this
 ...
 *                      or NULL for a transparent key.
 */
static void esp_psa_fail_key_creation(psa_key_slot_t *slot,
                                  psa_se_drv_table_entry_t *driver)
{
    (void) driver;
    ...
    esp_mbedtls_mutex_lock(&mbedtls_threading_key_slot_mutex);
#endif
    ...
    esp_psa_wipe_key_slot(slot);

#if defined(MBEDTLS_THREADING_C)
    esp_mbedtls_mutex_unlock(&mbedtls_threading_key_slot_mutex);
#endif
}
 ...
 */
static psa_status_t esp_psa_validate_optional_attributes(
...
psa_status_t esp_psa_import_key(const psa_key_attributes_t *attributes,
                            ...
                            mbedtls_svc_key_id_t *key)
{
    ...
    *key = MBEDTLS_SVC_KEY_ID_INIT;
    ...
    status = esp_psa_start_key_creation(PSA_KEY_CREATION_IMPORT, attributes,
                                    &slot, &driver);
    if (status != PSA_SUCCESS) {
        goto exit;
    }
    ...
    if (slot->key.data == NULL) {
        if (esp_psa_key_lifetime_is_external(attributes->lifetime)) {
        ...
        status = esp_psa_allocate_buffer_to_slot(slot, storage_size);
        if (status != PSA_SUCCESS) {
            goto exit;
        }
    }
    ...
    bits = slot->attr.bits;
    status = esp_psa_driver_wrapper_import_key(attributes,
                                           data, data_length,
                                           slot->key.data,
                                           slot->key.bytes,
                                           &slot->key.bytes, &bits);
    ...
    status = esp_psa_validate_optional_attributes(slot, attributes);
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    status = esp_psa_finish_key_creation(slot, driver, key);
exit:
    LOCAL_INPUT_FREE(data_external, data);
    if (status != PSA_SUCCESS) {
        esp_psa_fail_key_creation(slot, driver);
    }
...
#if defined(MBEDTLS_PSA_CRYPTO_SE_C)
psa_status_t esp_mbedtls_psa_register_se_key(
    const psa_key_attributes_t *attributes)
{
    psa_status_t status;
    psa_key_slot_t *slot = NULL;
    ...
    status = esp_psa_start_key_creation(PSA_KEY_CREATION_REGISTER, attributes,
                                    &slot, &driver);
    if (status != PSA_SUCCESS) {
        goto exit;
    }
...
exit:
    ...
    esp_psa_close_key(key);
    return status;
}
#endif /* MBEDTLS_PSA_CRYPTO_SE_C */

psa_status_t esp_psa_copy_key(mbedtls_svc_key_id_t source_key,
                          ...
                          mbedtls_svc_key_id_t *target_key)
{
    ...
    *target_key = MBEDTLS_SVC_KEY_ID_INIT;
    ...
    status = esp_psa_get_and_lock_key_slot_with_policy(
        source_key, &source_slot, PSA_KEY_USAGE_COPY, 0);
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    status = esp_psa_validate_optional_attributes(source_slot,
     ...
     * esp_psa_validate_optional_attributes() to be either equal to zero or
    ...
    status = esp_psa_restrict_key_policy(source_slot->attr.type,
    ...
    status = esp_psa_start_key_creation(PSA_KEY_CREATION_COPY, &actual_attributes,
                                    &target_slot, &driver);
    if (status != PSA_SUCCESS) {
        goto exit;
    }
     ...
     * */
    if (esp_psa_key_lifetime_is_external(actual_attributes.lifetime)) {
        ...
        status = esp_psa_allocate_buffer_to_slot(target_slot, storage_size);
        if (status != PSA_SUCCESS) {
            goto exit;
        }

        status = esp_psa_driver_wrapper_copy_key(&actual_attributes,
        ...
        status = esp_psa_copy_key_material_into_slot(target_slot,
                                                 source_slot->key.data,
                                                 source_slot->key.bytes);
        if (status != PSA_SUCCESS) {
            goto exit;
    ...
    status = esp_psa_finish_key_creation(target_slot, driver, target_key);
exit:
    if (status != PSA_SUCCESS) {
        esp_psa_fail_key_creation(target_slot, driver);
    }

    unlock_status = esp_psa_unregister_read_under_mutex(source_slot);
...
psa_status_t esp_psa_hash_abort(psa_hash_operation_t *operation)
{
    ...
    psa_status_t status = esp_psa_driver_wrapper_hash_abort(operation);
    operation->id = 0;

    return status;
}

psa_status_t esp_psa_hash_setup(psa_hash_operation_t *operation,
                            psa_algorithm_t alg)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
    ...
    status = esp_psa_driver_wrapper_hash_setup(operation, alg);

exit:
    if (status != PSA_SUCCESS) {
        esp_psa_hash_abort(operation);
    }

    return status;
}

psa_status_t esp_psa_hash_update(psa_hash_operation_t *operation,
                             ...
                             size_t input_length)
{
    ...
    status = esp_psa_driver_wrapper_hash_update(operation, input, input_length);
...
static psa_status_t esp_psa_hash_finish_internal(psa_hash_operation_t *operation,
                                             uint8_t *hash,
                                             size_t hash_size,
                                             size_t *hash_length)
{
    ...
    *hash_length = 0;
    ...
    status = esp_psa_driver_wrapper_hash_finish(
...
psa_status_t esp_psa_hash_finish(psa_hash_operation_t *operation,
                             ...
                             size_t *hash_length)
{
    ...
    status = esp_psa_hash_finish_internal(operation, hash, hash_size, hash_length);
...
psa_status_t esp_psa_hash_verify(psa_hash_operation_t *operation,
                             ...
                             size_t hash_length)
{
    ...
    status = esp_psa_hash_finish_internal(
        operation,
    ...
    if (esp_mbedtls_ct_memcmp(hash, actual_hash, actual_hash_length) != 0) {
        status = PSA_ERROR_INVALID_SIGNATURE;
    }

exit:
    esp_mbedtls_platform_zeroize(actual_hash, sizeof(actual_hash));
    if (status != PSA_SUCCESS) {
        esp_psa_hash_abort(operation);
    }
...
psa_status_t esp_psa_hash_compute(psa_algorithm_t alg,
                              ...
                              size_t *hash_length)
{
    ...
    *hash_length = 0;
    ...
    LOCAL_INPUT_ALLOC(input_external, input_length, input);
    LOCAL_OUTPUT_ALLOC(hash_external, hash_size, hash);
    status = esp_psa_driver_wrapper_hash_compute(alg, input, input_length,
...
psa_status_t esp_psa_hash_compare(psa_algorithm_t alg,
                              const uint8_t *input_external, size_t input_length,
                              const uint8_t *hash_external, size_t hash_length)
{
    ...
    LOCAL_INPUT_ALLOC(input_external, input_length, input);
    status = esp_psa_driver_wrapper_hash_compute(
...
psa_status_t esp_psa_hash_clone(const psa_hash_operation_t *source_operation,
                            psa_hash_operation_t *target_operation)
{
    ...
    psa_status_t status = esp_psa_driver_wrapper_hash_clone(source_operation,
                                                        target_operation);
    if (status != PSA_SUCCESS) {
        esp_psa_hash_abort(target_operation);
    }
...
psa_status_t esp_psa_mac_abort(psa_mac_operation_t *operation)
{
    /* Aborting a non-active operation is allowed */
    if (operation->id == 0) {
        return PSA_SUCCESS;
    ...
    psa_status_t status = esp_psa_driver_wrapper_mac_abort(operation);
...
static psa_status_t esp_psa_mac_finalize_alg_and_key_validation(
    psa_algorithm_t alg,
    const psa_key_attributes_t *attributes,
    uint8_t *mac_size)
{
    ...
    status = esp_psa_mac_key_can_do(alg, key_type);
    if (status != PSA_SUCCESS) {
        return status;
    }
    ...
    *mac_size = PSA_MAC_LENGTH(key_type, key_bits, alg);
    ...
    if (*mac_size > PSA_MAC_MAX_SIZE) {
         ...
         * PSA_ERROR_NOT_SUPPORTED here. Otherwise esp_psa_mac_verify(), or
         * esp_psa_mac_compute(mac_size=PSA_MAC_MAX_SIZE), would return
...
static psa_status_t esp_psa_mac_setup(psa_mac_operation_t *operation,
                                  mbedtls_svc_key_id_t key,
                                  psa_algorithm_t alg,
                                  int is_sign)
{
    ...
    status = esp_psa_mac_finalize_alg_and_key_validation(alg, &slot->attr,
                                                     &operation->mac_size);
    if (status != PSA_SUCCESS) {
        goto exit;
    }
    ...
    if (is_sign) {
        status = esp_psa_driver_wrapper_mac_sign_setup(operation,
                                                   &slot->attr,
                                                   slot->key.data,
                                                   slot->key.bytes,
                                                   alg);
    } else {
        status = esp_psa_driver_wrapper_mac_verify_setup(operation,
                                                     &slot->attr,
                                                     slot->key.data,
                                                     slot->key.bytes,
                                                     alg);
...
psa_status_t esp_psa_mac_sign_setup(psa_mac_operation_t *operation,
                                mbedtls_svc_key_id_t key,
                                psa_algorithm_t alg)
{
    return esp_psa_mac_setup(operation, key, alg, 1);
}

psa_status_t esp_psa_mac_verify_setup(psa_mac_operation_t *operation,
                                  mbedtls_svc_key_id_t key,
                                  psa_algorithm_t alg)
{
    return esp_psa_mac_setup(operation, key, alg, 0);
}

psa_status_t esp_psa_mac_update(psa_mac_operation_t *operation,
                            const uint8_t *input_external,
                            size_t input_length)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
    ...
    LOCAL_INPUT_ALLOC(input_external, input_length, input);
    status = esp_psa_driver_wrapper_mac_update(operation, input, input_length);
...
psa_status_t esp_psa_mac_sign_finish(psa_mac_operation_t *operation,
                                 ...
                                 size_t *mac_length)
{
    ...
    status = esp_psa_driver_wrapper_mac_sign_finish(operation,
...
exit:
    ...
    if (mac != NULL) {
        esp_psa_wipe_tag_output_buffer(mac, status, mac_size, *mac_length);
    }

    abort_status = esp_psa_mac_abort(operation);
    LOCAL_OUTPUT_FREE(mac_external, mac);

    return status == PSA_SUCCESS ? abort_status : status;
}

psa_status_t esp_psa_mac_verify_finish(psa_mac_operation_t *operation,
                                   ...
                                   size_t mac_length)
{
    ...
    status = esp_psa_driver_wrapper_mac_verify_finish(operation,
                                                  mac, mac_length);
...
static psa_status_t esp_psa_mac_compute_internal(mbedtls_svc_key_id_t key,
                                             psa_algorithm_t alg,
                                             const uint8_t *input,
                                             size_t input_length,
                                             uint8_t *mac,
                                             ...
                                             int is_sign)
{
    ...
    status = esp_psa_driver_wrapper_mac_compute(
        &slot->attr,
        slot->key.data, slot->key.bytes,
        alg,
        input, input_length,
...
psa_status_t esp_psa_mac_compute(mbedtls_svc_key_id_t key,
                             psa_algorithm_t alg,
                             const uint8_t *input_external,
                             size_t input_length,
                             uint8_t *mac_external,
                             ...
                             size_t *mac_length)
{
    ...
    LOCAL_INPUT_ALLOC(input_external, input_length, input);
    LOCAL_OUTPUT_ALLOC(mac_external, mac_size, mac);
    status = esp_psa_mac_compute_internal(key, alg,
                                      input, input_length,
...
psa_status_t esp_psa_mac_verify(mbedtls_svc_key_id_t key,
                            psa_algorithm_t alg,
                            const uint8_t *input_external,
                            size_t input_length,
                            const uint8_t *mac_external,
                            size_t mac_length)
{
    ...
    LOCAL_INPUT_ALLOC(mac_external, mac_length, mac);
    if (esp_mbedtls_ct_memcmp(mac, actual_mac, actual_mac_length) != 0) {
        status = PSA_ERROR_INVALID_SIGNATURE;
        goto exit;
    }

exit:
    esp_mbedtls_platform_zeroize(actual_mac, sizeof(actual_mac));
    LOCAL_INPUT_FREE(input_external, input);
    LOCAL_INPUT_FREE(mac_external, mac);
...
static psa_status_t esp_psa_sign_verify_check_alg(int input_is_message,
                                              psa_algorithm_t alg)
{
    if (input_is_message) {
...
static psa_status_t esp_psa_sign_internal(mbedtls_svc_key_id_t key,
                                      ...
                                      size_t *signature_length)
{
    ...
    *signature_length = 0;
    ...
    status = esp_psa_sign_verify_check_alg(input_is_message, alg);
    if (status != PSA_SUCCESS) {
        return status;
    }
    ...
    if (input_is_message) {
        status = esp_psa_driver_wrapper_sign_message(
            &slot->attr, slot->key.data, slot->key.bytes,
            alg, input, input_length,
            signature, signature_size, signature_length);
    } else {

        status = esp_psa_driver_wrapper_sign_hash(
            &slot->attr, slot->key.data, slot->key.bytes,
            alg, input, input_length,
            signature, signature_size, signature_length);
    }
...
exit:
    esp_psa_wipe_tag_output_buffer(signature, status, signature_size,
                               *signature_length);
...
static psa_status_t esp_psa_verify_internal(mbedtls_svc_key_id_t key,
                                        int input_is_message,
                                        psa_algorithm_t alg,
                                        const uint8_t *input,
                                        size_t input_length,
                                        ...
                                        size_t signature_length)
{
    ...
    if (input_is_message) {
        status = esp_psa_driver_wrapper_verify_message(
            &slot->attr, slot->key.data, slot->key.bytes,
            alg, input, input_length,
            signature, signature_length);
    } else {
        status = esp_psa_driver_wrapper_verify_hash(
            &slot->attr, slot->key.data, slot->key.bytes,
            alg, input, input_length,
            signature, signature_length);
    }
...
psa_status_t esp_psa_sign_message_builtin(
    const psa_key_attributes_t *attributes,
    const uint8_t *key_buffer,
    size_t key_buffer_size,
    psa_algorithm_t alg,
    ...
    size_t *signature_length)
{
    ...
    if (PSA_ALG_IS_SIGN_HASH(alg)) {
        ...
        return esp_psa_driver_wrapper_sign_hash(
...
psa_status_t esp_psa_sign_message(mbedtls_svc_key_id_t key,
                              psa_algorithm_t alg,
                              const uint8_t *input_external,
                              size_t input_length,
                              ...
                              size_t *signature_length)
{
    ...
    status = esp_psa_sign_internal(key, 1, alg, input, input_length, signature,
...
psa_status_t esp_psa_verify_message_builtin(
    const psa_key_attributes_t *attributes,
    const uint8_t *key_buffer,
    size_t key_buffer_size,
    psa_algorithm_t alg,
    ...
    size_t signature_length)
{
    ...
    if (PSA_ALG_IS_SIGN_HASH(alg)) {
        ...
        return esp_psa_driver_wrapper_verify_hash(
            attributes, key_buffer, key_buffer_size,
            alg, hash, hash_length,
            signature, signature_length);
    }
...
psa_status_t esp_psa_verify_message(mbedtls_svc_key_id_t key,
                                psa_algorithm_t alg,
                                const uint8_t *input_external,
                                size_t input_length,
                                ...
                                size_t signature_length)
{
    ...
    status = esp_psa_verify_internal(key, 1, alg, input, input_length, signature,
                                 signature_length);
...
psa_status_t esp_psa_sign_hash_builtin(
    const psa_key_attributes_t *attributes,
    const uint8_t *key_buffer, size_t key_buffer_size,
    ...
    uint8_t *signature, size_t signature_size, size_t *signature_length)
{
    if (attributes->type == PSA_KEY_TYPE_RSA_KEY_PAIR) {
        if (PSA_ALG_IS_RSA_PKCS1V15_SIGN(alg) ||
            ...
            return esp_mbedtls_psa_rsa_sign_hash(
                attributes,
                key_buffer, key_buffer_size,
                alg, hash, hash_length,
                signature, signature_size, signature_length);
        ...
        * defined(MBEDTLS_PSA_BUILTIN_ALG_RSA_PSS) */
        ...
        if (PSA_ALG_IS_ECDSA(alg)) {
            ...
            return esp_mbedtls_psa_ecdsa_sign_hash(
                attributes,
                key_buffer, key_buffer_size,
                alg, hash, hash_length,
                signature, signature_size, signature_length);
...
psa_status_t esp_psa_sign_hash(mbedtls_svc_key_id_t key,
                           psa_algorithm_t alg,
                           const uint8_t *hash_external,
                           size_t hash_length,
                           uint8_t *signature_external,
                           ...
                           size_t *signature_length)
{
    ...
    LOCAL_INPUT_ALLOC(hash_external, hash_length, hash);
    LOCAL_OUTPUT_ALLOC(signature_external, signature_size, signature);
    status = esp_psa_sign_internal(key, 0, alg, hash, hash_length, signature,
                               signature_size, signature_length);
...
psa_status_t esp_psa_verify_hash_builtin(
    const psa_key_attributes_t *attributes,
    const uint8_t *key_buffer, size_t key_buffer_size,
    psa_algorithm_t alg, const uint8_t *hash, size_t hash_length,
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_RSA_PKCS1V15_SIGN) || \
            defined(MBEDTLS_PSA_BUILTIN_ALG_RSA_PSS)
            return esp_mbedtls_psa_rsa_verify_hash(
                attributes,
                key_buffer, key_buffer_size,
                alg, hash, hash_length,
                signature, signature_length);
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_ECDSA) || \
            defined(MBEDTLS_PSA_BUILTIN_ALG_DETERMINISTIC_ECDSA)
            return esp_mbedtls_psa_ecdsa_verify_hash(
                attributes,
                key_buffer, key_buffer_size,
                alg, hash, hash_length,
                signature, signature_length);
...
psa_status_t esp_psa_verify_hash(mbedtls_svc_key_id_t key,
                             psa_algorithm_t alg,
                             const uint8_t *hash_external,
                             size_t hash_length,
                             const uint8_t *signature_external,
                             size_t signature_length)
{
    ...
    LOCAL_INPUT_ALLOC(hash_external, hash_length, hash);
    LOCAL_INPUT_ALLOC(signature_external, signature_length, signature);
    status = esp_psa_verify_internal(key, 0, alg, hash, hash_length, signature,
                                 signature_length);
...
psa_status_t esp_psa_asymmetric_encrypt(mbedtls_svc_key_id_t key,
                                    psa_algorithm_t alg,
                                    const uint8_t *input_external,
                                    size_t input_length,
                                    ...
                                    size_t *output_length)
{
    ...
    *output_length = 0;
    ...
    status = esp_psa_driver_wrapper_asymmetric_encrypt(
        &slot->attr, slot->key.data, slot->key.bytes,
...
psa_status_t esp_psa_asymmetric_decrypt(mbedtls_svc_key_id_t key,
                                    psa_algorithm_t alg,
                                    const uint8_t *input_external,
                                    size_t input_length,
                                    const uint8_t *salt_external,
                                    ...
                                    size_t *output_length)
{
    ...
    *output_length = 0;
    ...
    status = esp_psa_driver_wrapper_asymmetric_decrypt(
        &slot->attr, slot->key.data, slot->key.bytes,
        alg, input, input_length, salt, salt_length,
        output, output_size, output_length);
...
static uint32_t esp_psa_interruptible_max_ops = PSA_INTERRUPTIBLE_MAX_OPS_UNLIMITED;

void esp_psa_interruptible_set_max_ops(uint32_t max_ops)
{
    esp_psa_interruptible_max_ops = max_ops;
}

uint32_t esp_psa_interruptible_get_max_ops(void)
{
    return esp_psa_interruptible_max_ops;
}

uint32_t esp_psa_sign_hash_get_num_ops(
    const psa_sign_hash_interruptible_operation_t *operation)
{
    return operation->num_ops;
}

uint32_t esp_psa_verify_hash_get_num_ops(
    const psa_verify_hash_interruptible_operation_t *operation)
{
    return operation->num_ops;
}

static psa_status_t esp_psa_sign_hash_abort_internal(
    psa_sign_hash_interruptible_operation_t *operation)
{
    ...
    status = esp_psa_driver_wrapper_sign_hash_abort(operation);
...
psa_status_t esp_psa_sign_hash_start(
    psa_sign_hash_interruptible_operation_t *operation,
    ...
    const uint8_t *hash_external, size_t hash_length)
{
    ...
    status = esp_psa_sign_verify_check_alg(0, alg);
    if (status != PSA_SUCCESS) {
    ...
    status = esp_psa_driver_wrapper_sign_hash_start(operation, &slot->attr,
                                                slot->key.data,
                                                slot->key.bytes, alg,
                                                hash, hash_length);
exit:
    ...
    if (status != PSA_SUCCESS) {
        operation->error_occurred = 1;
        esp_psa_sign_hash_abort_internal(operation);
    }
...
psa_status_t esp_psa_sign_hash_complete(
    psa_sign_hash_interruptible_operation_t *operation,
    ...
    size_t *signature_length)
{
    ...
    *signature_length = 0;
    ...
    status = esp_psa_driver_wrapper_sign_hash_complete(operation, signature,
                                                   signature_size,
                                                   signature_length);

    /* Update ops count with work done. */
    operation->num_ops = esp_psa_driver_wrapper_sign_hash_get_num_ops(operation);
...
psa_status_t esp_psa_sign_hash_abort(
    psa_sign_hash_interruptible_operation_t *operation)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;

    status = esp_psa_sign_hash_abort_internal(operation);
...
static psa_status_t esp_psa_verify_hash_abort_internal(
    psa_verify_hash_interruptible_operation_t *operation)
{
    ...
    status = esp_psa_driver_wrapper_verify_hash_abort(operation);
...
psa_status_t esp_psa_verify_hash_start(
    psa_verify_hash_interruptible_operation_t *operation,
    mbedtls_svc_key_id_t key, psa_algorithm_t alg,
    const uint8_t *hash_external, size_t hash_length,
    const uint8_t *signature_external, size_t signature_length)
{
    ...
    status = esp_psa_driver_wrapper_verify_hash_start(operation, &slot->attr,
                                                  slot->key.data,
                                                  slot->key.bytes,
                                                  alg, hash, hash_length,
                                                  signature, signature_length);
...
exit:
    ...
    if (status != PSA_SUCCESS) {
        operation->error_occurred = 1;
        esp_psa_verify_hash_abort_internal(operation);
    }
...
psa_status_t esp_psa_verify_hash_complete(
    psa_verify_hash_interruptible_operation_t *operation)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
    ...
    status = esp_psa_driver_wrapper_verify_hash_complete(operation);

    /* Update ops count with work done. */
    operation->num_ops = esp_psa_driver_wrapper_verify_hash_get_num_ops(
        operation);
...
psa_status_t esp_psa_verify_hash_abort(
    psa_verify_hash_interruptible_operation_t *operation)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;

    status = esp_psa_verify_hash_abort_internal(operation);
...
void esp_mbedtls_psa_interruptible_set_max_ops(uint32_t max_ops)
{
    ...
    esp_mbedtls_ecp_set_max_ops(max_ops);
#else
    (void) max_ops;
#endif /* defined(MBEDTLS_PSA_BUILTIN_ALG_ECDSA) ||
        * defined(MBEDTLS_PSA_BUILTIN_ALG_DETERMINISTIC_ECDSA) &&
...
uint32_t esp_mbedtls_psa_sign_hash_get_num_ops(
...
uint32_t esp_mbedtls_psa_verify_hash_get_num_ops(
...
psa_status_t esp_mbedtls_psa_sign_hash_start(
    ...
    const uint8_t *hash, size_t hash_length)
{
    ...
    esp_mbedtls_ecdsa_restart_init(&operation->restart_ctx);

    /* Ensure num_ops is zero'ed in case of context re-use. */
    operation->num_ops = 0;

    status = esp_mbedtls_psa_ecp_load_representation(attributes->type,
                                                 attributes->bits,
                                                 key_buffer,
                                                 key_buffer_size,
                                                 &operation->ctx);
    ...
    operation->md_alg = esp_mbedtls_md_type_from_psa_alg(hash_alg);
    operation->alg = alg;
...
psa_status_t esp_mbedtls_psa_sign_hash_complete(
    mbedtls_psa_sign_hash_interruptible_operation_t *operation,
    ...
    size_t *signature_length)
{
    ...
    esp_mbedtls_mpi_init(&r);
    esp_mbedtls_mpi_init(&s);

    /* Ensure max_ops is set to the current value (or default). */
    esp_mbedtls_psa_interruptible_set_max_ops(esp_psa_interruptible_get_max_ops());
    ...
    if (PSA_ALG_ECDSA_IS_DETERMINISTIC(operation->alg)) {
        ...
        status = esp_mbedtls_to_psa_error(
            esp_mbedtls_ecdsa_sign_det_restartable(&operation->ctx->grp,
                                               &r,
                                               &s,
                                               &operation->ctx->d,
                                               operation->hash,
        ...
        status = esp_mbedtls_to_psa_error(
            esp_mbedtls_ecdsa_sign_restartable(&operation->ctx->grp,
                                           &r,
                                           &s,
                                           &operation->ctx->d,
                                           operation->hash,
    ...
    if (status == PSA_SUCCESS) {
        status =  esp_mbedtls_to_psa_error(
            esp_mbedtls_mpi_write_binary(&r,
                                     signature,
        ...
        status =  esp_mbedtls_to_psa_error(
            esp_mbedtls_mpi_write_binary(&s,
                                     signature +
                                     operation->coordinate_bytes,
                                     operation->coordinate_bytes)
            );
...
exit:
    ...
    esp_mbedtls_mpi_free(&r);
    esp_mbedtls_mpi_free(&s);
    return status;
...
psa_status_t esp_mbedtls_psa_sign_hash_abort(
    mbedtls_psa_sign_hash_interruptible_operation_t *operation)
{
    ...
    if (operation->ctx) {
        esp_mbedtls_ecdsa_free(operation->ctx);
        esp_mbedtls_free(operation->ctx);
        operation->ctx = NULL;
    }

    esp_mbedtls_ecdsa_restart_free(&operation->restart_ctx);
...
psa_status_t esp_mbedtls_psa_verify_hash_start(
    ...
    const uint8_t *signature, size_t signature_length)
{
    ...
    esp_mbedtls_ecdsa_restart_init(&operation->restart_ctx);
    esp_mbedtls_mpi_init(&operation->r);
    esp_mbedtls_mpi_init(&operation->s);
    ...
    status = esp_mbedtls_to_psa_error(
        esp_mbedtls_mpi_read_binary(&operation->r,
                                signature,
                                coordinate_bytes));
    ...
    status = esp_mbedtls_to_psa_error(
        esp_mbedtls_mpi_read_binary(&operation->s,
                                signature +
                                coordinate_bytes,
                                coordinate_bytes));
    ...
    status = esp_mbedtls_psa_ecp_load_public_part(operation->ctx);
...
psa_status_t esp_mbedtls_psa_verify_hash_complete(
    mbedtls_psa_verify_hash_interruptible_operation_t *operation)
{
    ...
    status = esp_mbedtls_to_psa_error(
        esp_mbedtls_ecdsa_verify_restartable(&operation->ctx->grp,
                                         operation->hash,
                                         operation->hash_length,
                                         &operation->ctx->Q,
                                         &operation->r,
...
psa_status_t esp_mbedtls_psa_verify_hash_abort(
    mbedtls_psa_verify_hash_interruptible_operation_t *operation)
{
    ...
    esp_mbedtls_mpi_free(&operation->r);
    esp_mbedtls_mpi_free(&operation->s);
...
static psa_status_t esp_psa_generate_random_internal(uint8_t *output,
                                                 size_t output_size)
{
    ...
    status = mbedtls_psa_external_get_random(&esp_global_data.rng,
                                             output, output_size,
                                             &output_length);
    if (status != PSA_SUCCESS) {
        return status;
    ...
    while (output_size > 0) {
        ...
        ret = esp_mbedtls_ctr_drbg_random(&esp_global_data.rng.drbg, output, request_size);
#elif defined(MBEDTLS_HMAC_DRBG_C)
        ret = esp_mbedtls_hmac_drbg_random(&esp_global_data.rng.drbg, output, request_size);
...
static psa_status_t esp_psa_cipher_setup(psa_cipher_operation_t *operation,
                                     mbedtls_svc_key_id_t key,
                                     psa_algorithm_t alg,
                                     mbedtls_operation_t cipher_operation)
{
    ...
    status = esp_psa_get_and_lock_key_slot_with_policy(key, &slot, usage, alg);
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    /* Initialize the operation struct members, except for id. The id member
     * is used to indicate to esp_psa_cipher_abort that there are resources to free,
    ...
    if (cipher_operation == MBEDTLS_ENCRYPT) {
        status = esp_psa_driver_wrapper_cipher_encrypt_setup(operation,
                                                         &slot->attr,
                                                         slot->key.data,
                                                         slot->key.bytes,
                                                         alg);
    } else {
        status = esp_psa_driver_wrapper_cipher_decrypt_setup(operation,
                                                         &slot->attr,
                                                         slot->key.data,
                                                         slot->key.bytes,
                                                         alg);
...
exit:
    if (status != PSA_SUCCESS) {
        esp_psa_cipher_abort(operation);
    }
...
psa_status_t esp_psa_cipher_encrypt_setup(psa_cipher_operation_t *operation,
                                      mbedtls_svc_key_id_t key,
                                      psa_algorithm_t alg)
{
    return esp_psa_cipher_setup(operation, key, alg, MBEDTLS_ENCRYPT);
}

psa_status_t esp_psa_cipher_decrypt_setup(psa_cipher_operation_t *operation,
                                      mbedtls_svc_key_id_t key,
                                      psa_algorithm_t alg)
{
    return esp_psa_cipher_setup(operation, key, alg, MBEDTLS_DECRYPT);
}

psa_status_t esp_psa_cipher_generate_iv(psa_cipher_operation_t *operation,
                                    uint8_t *iv_external,
                                    size_t iv_size,
                                    size_t *iv_length)
{
    ...
    status = esp_psa_generate_random_internal(iv, default_iv_length);
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    status = esp_psa_driver_wrapper_cipher_set_iv(operation,
...
exit:
    if (status == PSA_SUCCESS) {
        ...
        *iv_length = 0;
        esp_psa_cipher_abort(operation);
        if (iv != NULL) {
            esp_mbedtls_platform_zeroize(iv, default_iv_length);
        }
    }
...
psa_status_t esp_psa_cipher_set_iv(psa_cipher_operation_t *operation,
...
psa_status_t esp_psa_cipher_update(psa_cipher_operation_t *operation,
                               const uint8_t *input_external,
                               size_t input_length,
                               uint8_t *output_external,
                               size_t output_size,
                               size_t *output_length)
{
    ...
    status = esp_psa_driver_wrapper_cipher_update(operation,
                                              input,
                                              input_length,
                                              output,
                                              output_size,
...
psa_status_t esp_psa_cipher_finish(psa_cipher_operation_t *operation,
                               uint8_t *output_external,
                               size_t output_size,
                               size_t *output_length)
{
    ...
    status = esp_psa_driver_wrapper_cipher_finish(operation,
                                              output,
                                              output_size,
                                              output_length);
...
exit:
    if (status == PSA_SUCCESS) {
        status = esp_psa_cipher_abort(operation);
    } else {
        *output_length = 0;
        (void) esp_psa_cipher_abort(operation);
    }
...
psa_status_t esp_psa_cipher_abort(psa_cipher_operation_t *operation)
{
    if (operation->id == 0) {
        /* The object has (apparently) been initialized but it is not (yet)
         * in use. It's ok to call abort on such an object, and there's
    ...
    esp_psa_driver_wrapper_cipher_abort(operation);
...
psa_status_t esp_psa_cipher_encrypt(mbedtls_svc_key_id_t key,
                                psa_algorithm_t alg,
                                const uint8_t *input_external,
                                size_t input_length,
                                uint8_t *output_external,
                                ...
                                size_t *output_length)
{
    ...
    if (default_iv_length > 0) {
        ...
        status = esp_psa_generate_random_internal(local_iv, default_iv_length);
        if (status != PSA_SUCCESS) {
            goto exit;
        }
    }
    ...
    status = esp_psa_driver_wrapper_cipher_encrypt(
        ...
        esp_psa_crypto_buffer_offset(output, default_iv_length),
...
psa_status_t esp_psa_cipher_decrypt(mbedtls_svc_key_id_t key,
                                psa_algorithm_t alg,
                                const uint8_t *input_external,
                                size_t input_length,
                                uint8_t *output_external,
                                ...
                                size_t *output_length)
{
    ...
    status = esp_psa_driver_wrapper_cipher_decrypt(
        &slot->attr, slot->key.data, slot->key.bytes,
        alg, input, input_length,
        output, output_size, output_length);
...
static psa_algorithm_t esp_psa_aead_get_base_algorithm(psa_algorithm_t alg)
{
    return PSA_ALG_AEAD_WITH_DEFAULT_LENGTH_TAG(alg);
}

/* Helper function to perform common nonce length checks. */
static psa_status_t esp_psa_aead_check_nonce_length(psa_algorithm_t alg,
                                                size_t nonce_length)
{
    psa_algorithm_t base_alg = esp_psa_aead_get_base_algorithm(alg);
...
static psa_status_t esp_psa_aead_check_algorithm(psa_algorithm_t alg)
{
...
psa_status_t esp_psa_aead_encrypt(mbedtls_svc_key_id_t key,
                              psa_algorithm_t alg,
                              ...
                              size_t *ciphertext_length)
{
    ...
    *ciphertext_length = 0;
    ...
    status = esp_psa_aead_check_algorithm(alg);
    if (status != PSA_SUCCESS) {
        return status;
    }
    ...
    status = esp_psa_aead_check_nonce_length(alg, nonce_length);
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    status = esp_psa_driver_wrapper_aead_encrypt(
        &slot->attr, slot->key.data, slot->key.bytes,
        alg,
        nonce, nonce_length,
...
psa_status_t esp_psa_aead_decrypt(mbedtls_svc_key_id_t key,
                              psa_algorithm_t alg,
                              const uint8_t *nonce_external,
                              size_t nonce_length,
                              const uint8_t *additional_data_external,
                              ...
                              size_t *plaintext_length)
{
    ...
    *plaintext_length = 0;
    ...
    status = esp_psa_driver_wrapper_aead_decrypt(
        &slot->attr, slot->key.data, slot->key.bytes,
        alg,
        nonce, nonce_length,
        additional_data, additional_data_length,
...
static psa_status_t esp_psa_validate_tag_length(psa_algorithm_t alg)
{
...
static psa_status_t esp_psa_aead_setup(psa_aead_operation_t *operation,
                                   int is_encrypt,
                                   mbedtls_svc_key_id_t key,
                                   psa_algorithm_t alg)
{
    ...
    status = esp_psa_get_and_lock_key_slot_with_policy(key, &slot, key_usage,
                                                   alg);
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    if ((status = esp_psa_validate_tag_length(alg)) != PSA_SUCCESS) {
        goto exit;
    }

    if (is_encrypt) {
        status = esp_psa_driver_wrapper_aead_encrypt_setup(operation,
                                                       &slot->attr,
                                                       slot->key.data,
                                                       slot->key.bytes,
                                                       alg);
    } else {
        status = esp_psa_driver_wrapper_aead_decrypt_setup(operation,
                                                       &slot->attr,
                                                       slot->key.data,
                                                       slot->key.bytes,
                                                       alg);
...
exit:
    ...
    if (status == PSA_SUCCESS) {
        status = unlock_status;
        operation->alg = esp_psa_aead_get_base_algorithm(alg);
        operation->is_encrypt = is_encrypt;
    } else {
        esp_psa_aead_abort(operation);
    }
...
psa_status_t esp_psa_aead_encrypt_setup(psa_aead_operation_t *operation,
                                    mbedtls_svc_key_id_t key,
                                    psa_algorithm_t alg)
{
    return esp_psa_aead_setup(operation, 1, key, alg);
}

/* Set the key for a multipart authenticated decryption operation. */
psa_status_t esp_psa_aead_decrypt_setup(psa_aead_operation_t *operation,
                                    mbedtls_svc_key_id_t key,
                                    psa_algorithm_t alg)
{
    return esp_psa_aead_setup(operation, 0, key, alg);
}

static psa_status_t esp_psa_aead_set_nonce_internal(psa_aead_operation_t *operation,
                                                const uint8_t *nonce,
                                                size_t nonce_length)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
    ...
    status = esp_psa_aead_check_nonce_length(operation->alg, nonce_length);
    if (status != PSA_SUCCESS) {
        status = PSA_ERROR_INVALID_ARGUMENT;
        goto exit;
    }

    status = esp_psa_driver_wrapper_aead_set_nonce(operation, nonce,
                                               nonce_length);
...
psa_status_t esp_psa_aead_generate_nonce(psa_aead_operation_t *operation,
                                     ...
                                     size_t *nonce_length)
{
    ...
    *nonce_length = 0;
    ...
    status = esp_psa_generate_random_internal(local_nonce, required_nonce_size);
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    status = esp_psa_aead_set_nonce_internal(operation, local_nonce,
                                         required_nonce_size);
...
/* Set the nonce for a multipart authenticated encryption or decryption
   operation.*/
psa_status_t esp_psa_aead_set_nonce(psa_aead_operation_t *operation,
                                const uint8_t *nonce_external,
                                size_t nonce_length)
{
    psa_status_t status;
    ...
    status = esp_psa_aead_set_nonce_internal(operation, nonce, nonce_length);
...
psa_status_t esp_psa_aead_set_lengths(psa_aead_operation_t *operation,
                                  size_t ad_length,
                                  size_t plaintext_length)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
    ...
    status = esp_psa_driver_wrapper_aead_set_lengths(operation, ad_length,
                                                 plaintext_length);
...
psa_status_t esp_psa_aead_update_ad(psa_aead_operation_t *operation,
                                const uint8_t *input_external,
                                size_t input_length)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
    ...
    status = esp_psa_driver_wrapper_aead_update_ad(operation, input,
                                               input_length);
...
/* Encrypt or decrypt a message fragment in an active multipart AEAD
   operation.*/
psa_status_t esp_psa_aead_update(psa_aead_operation_t *operation,
                             const uint8_t *input_external,
                             size_t input_length,
                             uint8_t *output_external,
                             size_t output_size,
                             size_t *output_length)
{
    ...
    *output_length = 0;
    ...
    status = esp_psa_driver_wrapper_aead_update(operation, input, input_length,
                                            output, output_size,
                                            output_length);
...
static psa_status_t esp_psa_aead_final_checks(const psa_aead_operation_t *operation)
{
...
psa_status_t esp_psa_aead_finish(psa_aead_operation_t *operation,
                             uint8_t *ciphertext_external,
                             size_t ciphertext_size,
                             size_t *ciphertext_length,
                             ...
                             size_t *tag_length)
{
    ...
    *tag_length = tag_size;
    ...
    status = esp_psa_aead_final_checks(operation);
    if (status != PSA_SUCCESS) {
        goto exit;
    }
    ...
    status = esp_psa_driver_wrapper_aead_finish(operation, ciphertext,
                                            ciphertext_size,
                                            ciphertext_length,
...
exit:
     ...
     * the same buffer.*/
    esp_psa_wipe_tag_output_buffer(tag, status, tag_size, *tag_length);
...
/* Finish authenticating and decrypting a message in a multipart AEAD
   operation.*/
psa_status_t esp_psa_aead_verify(psa_aead_operation_t *operation,
                             uint8_t *plaintext_external,
                             size_t plaintext_size,
                             size_t *plaintext_length,
                             ...
                             size_t tag_length)
{
    ...
    *plaintext_length = 0;
    ...
    status = esp_psa_driver_wrapper_aead_verify(operation, plaintext,
                                            plaintext_size,
                                            plaintext_length,
                                            tag, tag_length);
...
psa_status_t esp_psa_aead_abort(psa_aead_operation_t *operation)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
    ...
    status = esp_psa_driver_wrapper_aead_abort(operation);
...
#define AT_LEAST_ONE_BUILTIN_KDF
    ...
    defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_PRF) || \
    defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_PSK_TO_MS)
static psa_status_t esp_psa_key_derivation_start_hmac(
...
static psa_algorithm_t esp_psa_key_derivation_get_kdf_alg(
...
psa_status_t esp_psa_key_derivation_abort(psa_key_derivation_operation_t *operation)
{
    psa_status_t status = PSA_SUCCESS;
    psa_algorithm_t kdf_alg = esp_psa_key_derivation_get_kdf_alg(operation);
    if (kdf_alg == 0) {
    ...
    if (PSA_ALG_IS_ANY_HKDF(kdf_alg)) {
        esp_mbedtls_free(operation->ctx.hkdf.info);
        status = esp_psa_mac_abort(&operation->ctx.hkdf.hmac);
    } else
    ...
    if (PSA_ALG_IS_TLS12_PRF(kdf_alg) ||
        ...
        if (operation->ctx.tls12_prf.secret != NULL) {
            esp_mbedtls_zeroize_and_free(operation->ctx.tls12_prf.secret,
                                     operation->ctx.tls12_prf.secret_length);
        }

        if (operation->ctx.tls12_prf.seed != NULL) {
            esp_mbedtls_zeroize_and_free(operation->ctx.tls12_prf.seed,
                                     operation->ctx.tls12_prf.seed_length);
        }

        if (operation->ctx.tls12_prf.label != NULL) {
            esp_mbedtls_zeroize_and_free(operation->ctx.tls12_prf.label,
                                     operation->ctx.tls12_prf.label_length);
        }
#if defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_PSK_TO_MS)
        if (operation->ctx.tls12_prf.other_secret != NULL) {
            esp_mbedtls_zeroize_and_free(operation->ctx.tls12_prf.other_secret,
         ...
         * esp_mbedtls_platform_zeroize() in the end of this function. */
    } else
#endif /* defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_PRF) ||
        * defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_PSK_TO_MS) */
#if defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_ECJPAKE_TO_PMS)
    if (kdf_alg == PSA_ALG_TLS12_ECJPAKE_TO_PMS) {
        esp_mbedtls_platform_zeroize(operation->ctx.tls12_ecjpake_to_pms.data,
    ...
    if (PSA_ALG_IS_PBKDF2(kdf_alg)) {
        if (operation->ctx.pbkdf2.salt != NULL) {
            esp_mbedtls_zeroize_and_free(operation->ctx.pbkdf2.salt,
    ...
    esp_mbedtls_platform_zeroize(operation, sizeof(*operation));
    return status;
}

psa_status_t esp_psa_key_derivation_get_capacity(const psa_key_derivation_operation_t *operation,
                                             size_t *capacity)
{
    if (operation->alg == 0) {
...
psa_status_t esp_psa_key_derivation_set_capacity(psa_key_derivation_operation_t *operation,
                                             size_t capacity)
{
    if (operation->alg == 0) {
        return PSA_ERROR_BAD_STATE;
...
static psa_status_t esp_psa_key_derivation_hkdf_read(psa_hkdf_key_derivation_t *hkdf,
                                                 ...
                                                 size_t output_length)
{
    ...
    while (output_length != 0) {
         ...
         * the capacity check in esp_psa_key_derivation_output_bytes() would have
        ...
        status = esp_psa_key_derivation_start_hmac(&hkdf->hmac,
                                               hash_alg,
                                               hkdf->prk,
                                               hash_length);
        if (status != PSA_SUCCESS) {
        ...
        if (hkdf->block_number != 1) {
            status = esp_psa_mac_update(&hkdf->hmac,
                                    hkdf->output_block,
                                    hash_length);
            if (status != PSA_SUCCESS) {
                return status;
        ...
        status = esp_psa_mac_sign_finish(&hkdf->hmac,
                                     hkdf->output_block,
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_PRF) || \
    defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_PSK_TO_MS)
static psa_status_t esp_psa_key_derivation_tls12_prf_generate_next_block(
    ...
    psa_algorithm_t alg)
{
    ...
    status = esp_psa_key_derivation_start_hmac(&hmac,
                                           hash_alg,
                                           tls12_prf->secret,
    ...
    if (tls12_prf->block_number == 1) {
        ...
        status = esp_psa_mac_update(&hmac,
                                tls12_prf->label,
        ...
        /* A(i) = HMAC_hash(secret, A(i-1)) */
        status = esp_psa_mac_update(&hmac, tls12_prf->Ai, hash_length);
        if (status != PSA_SUCCESS) {
            goto cleanup;
        }
    }

    status = esp_psa_mac_sign_finish(&hmac,
    ...
    status = esp_psa_mac_update(&hmac, tls12_prf->label, tls12_prf->label_length);
    if (status != PSA_SUCCESS) {
        goto cleanup;
    }
    status = esp_psa_mac_update(&hmac, tls12_prf->seed, tls12_prf->seed_length);
    if (status != PSA_SUCCESS) {
        goto cleanup;
    }
    status = esp_psa_mac_sign_finish(&hmac,
...
cleanup:
    cleanup_status = esp_psa_mac_abort(&hmac);
...
static psa_status_t esp_psa_key_derivation_tls12_prf_read(
    psa_tls12_prf_key_derivation_t *tls12_prf,
    psa_algorithm_t alg,
    uint8_t *output,
    size_t output_length)
{
    ...
    while (output_length != 0) {
        ...
        if (tls12_prf->left_in_block == 0) {
            status = esp_psa_key_derivation_tls12_prf_generate_next_block(tls12_prf,
                                                                      alg);
            if (status != PSA_SUCCESS) {
                return status;
            }
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_ECJPAKE_TO_PMS)
static psa_status_t esp_psa_key_derivation_tls12_ecjpake_to_pms_read(
    ...
    size_t output_length)
{
    ...
    status = esp_psa_hash_compute(PSA_ALG_SHA_256, ecjpake->data,
...
#if defined(PSA_HAVE_SOFT_PBKDF2)
static psa_status_t esp_psa_key_derivation_pbkdf2_generate_block(
    ...
    psa_key_attributes_t *attributes)
{
    ...
    status = esp_psa_driver_wrapper_mac_sign_setup(&mac_operation,
                                               attributes,
                                               pbkdf2->password,
    ...
    status = esp_psa_mac_update(&mac_operation, pbkdf2->salt, pbkdf2->salt_length);
    if (status != PSA_SUCCESS) {
        goto cleanup;
    }
    status = esp_psa_mac_update(&mac_operation, block_counter, sizeof(block_counter));
    if (status != PSA_SUCCESS) {
        goto cleanup;
    }
    status = esp_psa_mac_sign_finish(&mac_operation, U_i, sizeof(U_i),
                                 &mac_output_length);
    if (status != PSA_SUCCESS) {
        goto cleanup;
    }
    ...
    for (i = 1; i < pbkdf2->input_cost; i++) {
        ...
        status = esp_psa_driver_wrapper_mac_compute(attributes,
                                                pbkdf2->password,
                                                pbkdf2->password_length,
        ...
        esp_mbedtls_xor(U_accumulator, U_accumulator, U_i, prf_output_length);
    }

cleanup:
    /* Zeroise buffers to clear sensitive data from memory. */
    esp_mbedtls_platform_zeroize(U_i, PSA_MAC_MAX_SIZE);
    return status;
}

static psa_status_t esp_psa_key_derivation_pbkdf2_read(
    psa_pbkdf2_key_derivation_t *pbkdf2,
    psa_algorithm_t kdf_alg,
    uint8_t *output,
    size_t output_length)
{
    ...
    while (output_length != 0) {
        ...
        status = esp_psa_key_derivation_pbkdf2_generate_block(pbkdf2, prf_alg,
                                                          prf_output_length,
                                                          &attributes);
        if (status != PSA_SUCCESS) {
            return status;
...
psa_status_t esp_psa_key_derivation_output_bytes(
    ...
    size_t output_length)
{
...
#if defined(BUILTIN_ALG_ANY_HKDF)
    if (PSA_ALG_IS_ANY_HKDF(kdf_alg)) {
        status = esp_psa_key_derivation_hkdf_read(&operation->ctx.hkdf, kdf_alg,
    ...
    if (PSA_ALG_IS_TLS12_PRF(kdf_alg) ||
        PSA_ALG_IS_TLS12_PSK_TO_MS(kdf_alg)) {
        status = esp_psa_key_derivation_tls12_prf_read(&operation->ctx.tls12_prf,
                                                   kdf_alg, output,
                                                   output_length);
    } else
#endif /* MBEDTLS_PSA_BUILTIN_ALG_TLS12_PRF ||
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_ECJPAKE_TO_PMS)
    if (kdf_alg == PSA_ALG_TLS12_ECJPAKE_TO_PMS) {
        status = esp_psa_key_derivation_tls12_ecjpake_to_pms_read(
            &operation->ctx.tls12_ecjpake_to_pms, output, output_length);
    } else
#endif /* MBEDTLS_PSA_BUILTIN_ALG_TLS12_ECJPAKE_TO_PMS */
#if defined(PSA_HAVE_SOFT_PBKDF2)
    if (PSA_ALG_IS_PBKDF2(kdf_alg)) {
        status = esp_psa_key_derivation_pbkdf2_read(&operation->ctx.pbkdf2, kdf_alg,
                                                output, output_length);
    } else
#endif /* PSA_HAVE_SOFT_PBKDF2 */
...
exit:
    if (status != PSA_SUCCESS) {
        ...
        esp_psa_key_derivation_abort(operation);
        operation->alg = alg;
        if (output != NULL) {
...
static void esp_psa_des_set_key_parity(uint8_t *data, size_t data_size)
{
    if (data_size >= 8) {
        esp_mbedtls_des_key_set_parity(data);
    }
    if (data_size >= 16) {
        esp_mbedtls_des_key_set_parity(data + 8);
    }
    if (data_size >= 24) {
        esp_mbedtls_des_key_set_parity(data + 16);
    }
}
...
static psa_status_t esp_psa_generate_derived_ecc_key_weierstrass_helper(
    psa_key_slot_t *slot,
    size_t bits,
    psa_key_derivation_operation_t *operation,
    uint8_t **data
    )
{
    ...
    esp_mbedtls_mpi_init(&k);
    esp_mbedtls_mpi_init(&diff_N_2);

    psa_ecc_family_t curve = PSA_KEY_TYPE_ECC_GET_FAMILY(
        slot->attr.type);
    mbedtls_ecp_group_id grp_id =
        esp_mbedtls_ecc_group_from_psa(curve, bits);
    ...
    esp_mbedtls_ecp_group_init(&ecp_group);

    MBEDTLS_MPI_CHK(esp_mbedtls_ecp_group_load(&ecp_group, grp_id));
    ...
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_sub_int(&diff_N_2, &ecp_group.N, 2));

    /* Note: This function is always called with *data == NULL and it
     * allocates memory for the data buffer. */
    *data = esp_mbedtls_calloc(1, m_bytes);
    if (*data == NULL) {
    ...
    while (key_out_of_range) {
        /* 1. Draw a byte string of length ceiling(m/8) bytes. */
        if ((status = esp_psa_key_derivation_output_bytes(operation, *data, m_bytes)) != 0) {
            goto cleanup;
        }
         ...
         */
        MBEDTLS_MPI_CHK(esp_mbedtls_mpi_read_binary(&k, *data, m_bytes));

        /* 4. If k > N - 2, discard the result and return to step 1.
         *    Result of comparison is returned. When it indicates error
         *    then this function is called again.
         */
        MBEDTLS_MPI_CHK(esp_mbedtls_mpi_lt_mpi_ct(&diff_N_2, &k, &key_out_of_range));
    }

    /* 5. Output k + 1 as the private key. */
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_add_int(&k, &k, 1));
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_write_binary(&k, *data, m_bytes));
cleanup:
    if (ret != 0) {
        status = esp_mbedtls_to_psa_error(ret);
    }
    if (status != PSA_SUCCESS) {
        esp_mbedtls_free(*data);
        *data = NULL;
    }
    esp_mbedtls_mpi_free(&k);
    esp_mbedtls_mpi_free(&diff_N_2);
    return status;
}
...
static psa_status_t esp_psa_generate_derived_ecc_key_montgomery_helper(
    size_t bits,
    psa_key_derivation_operation_t *operation,
    uint8_t **data
    )
{
    ...
    *data = esp_mbedtls_calloc(1, output_length);

    if (*data == NULL) {
        return PSA_ERROR_INSUFFICIENT_MEMORY;
    }

    status = esp_psa_key_derivation_output_bytes(operation, *data, output_length);
...
static psa_status_t esp_psa_generate_derived_key_internal(
    psa_key_slot_t *slot,
    size_t bits,
    psa_key_derivation_operation_t *operation)
{
    ...
    if (PSA_KEY_TYPE_IS_ECC(slot->attr.type)) {
        ...
        if (PSA_ECC_FAMILY_IS_WEIERSTRASS(curve)) {
            ...
            status = esp_psa_generate_derived_ecc_key_weierstrass_helper(slot, bits, operation, &data);
            if (status != PSA_SUCCESS) {
                goto exit;
            }
        } else {
            /* Montgomery elliptic curve */
            status = esp_psa_generate_derived_ecc_key_montgomery_helper(bits, operation, &data);
            if (status != PSA_SUCCESS) {
                goto exit;
            }
        }
          ...
          defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_ECC_KEY_PAIR_DERIVE) */
    if (esp_key_type_is_raw_bytes(slot->attr.type)) {
        if (bits % 8 != 0) {
            return PSA_ERROR_INVALID_ARGUMENT;
        }
        data = esp_mbedtls_calloc(1, bytes);
        if (data == NULL) {
            return PSA_ERROR_INSUFFICIENT_MEMORY;
        }

        status = esp_psa_key_derivation_output_bytes(operation, data, bytes);
        if (status != PSA_SUCCESS) {
            goto exit;
        }
#if defined(MBEDTLS_PSA_BUILTIN_KEY_TYPE_DES)
        if (slot->attr.type == PSA_KEY_TYPE_DES) {
            esp_psa_des_set_key_parity(data, bytes);
        }
    ...
    if (esp_psa_key_lifetime_is_external(slot->attr.lifetime)) {
        status = esp_psa_driver_wrapper_get_key_buffer_size(&slot->attr,
                                                        &storage_size);
        if (status != PSA_SUCCESS) {
            goto exit;
        }
    ...
    status = esp_psa_driver_wrapper_import_key(&slot->attr,
                                           data, bytes,
                                           slot->key.data,
                                           slot->key.bytes,
                                           &slot->key.bytes, &bits);
...
static const psa_key_production_parameters_t esp_default_production_parameters =
    PSA_KEY_PRODUCTION_PARAMETERS_INIT;

int esp_psa_key_production_parameters_are_default(
...
psa_status_t esp_psa_key_derivation_output_key_ext(
    const psa_key_attributes_t *attributes,
    psa_key_derivation_operation_t *operation,
    const psa_key_production_parameters_t *params,
    size_t params_data_length,
    mbedtls_svc_key_id_t *key)
{
    ...
    *key = MBEDTLS_SVC_KEY_ID_INIT;
    ...
    if (!esp_psa_key_production_parameters_are_default(params, params_data_length)) {
        return PSA_ERROR_INVALID_ARGUMENT;
    }
    ...
    status = esp_psa_start_key_creation(PSA_KEY_CREATION_DERIVE, attributes,
                                    &slot, &driver);
#if defined(MBEDTLS_PSA_CRYPTO_SE_C)
    if (driver != NULL) {
...
#endif /* MBEDTLS_PSA_CRYPTO_SE_C */
    if (status == PSA_SUCCESS) {
        status = esp_psa_generate_derived_key_internal(slot,
                                                   attributes->bits,
                                                   operation);
    }
    if (status == PSA_SUCCESS) {
...
psa_status_t esp_psa_key_derivation_output_key(
    const psa_key_attributes_t *attributes,
    psa_key_derivation_operation_t *operation,
    mbedtls_svc_key_id_t *key)
{
    return esp_psa_key_derivation_output_key_ext(attributes, operation,
                                             &esp_default_production_parameters, 0,
                                             key);
}
...
static int esp_is_kdf_alg_supported(psa_algorithm_t kdf_alg)
{
#if defined(MBEDTLS_PSA_BUILTIN_ALG_HKDF)
...
static psa_status_t esp_psa_hash_try_support(psa_algorithm_t alg)
{
    psa_hash_operation_t operation = PSA_HASH_OPERATION_INIT;
    psa_status_t status = esp_psa_hash_setup(&operation, alg);
    esp_psa_hash_abort(&operation);
    return status;
}

static psa_status_t esp_psa_key_derivation_set_maximum_capacity(
    psa_key_derivation_operation_t *operation,
    psa_algorithm_t kdf_alg)
{
     ...
     * risk-prone. */
    psa_status_t status = esp_psa_hash_try_support(hash_alg);
    if (status != PSA_SUCCESS) {
        return status;
    }
...
static psa_status_t esp_psa_key_derivation_setup_kdf(
    psa_key_derivation_operation_t *operation,
    psa_algorithm_t kdf_alg)
{
    ...
    if (!esp_is_kdf_alg_supported(kdf_alg)) {
        return PSA_ERROR_NOT_SUPPORTED;
    }

    psa_status_t status = esp_psa_key_derivation_set_maximum_capacity(operation,
                                                                  kdf_alg);
    return status;
}

static psa_status_t esp_psa_key_agreement_try_support(psa_algorithm_t alg)
{
...
static int esp_psa_key_derivation_allows_free_form_secret_input(
    psa_algorithm_t kdf_alg)
{
#if defined(PSA_WANT_ALG_TLS12_ECJPAKE_TO_PMS)
    if (kdf_alg == PSA_ALG_TLS12_ECJPAKE_TO_PMS) {
...
psa_status_t esp_psa_key_derivation_setup(psa_key_derivation_operation_t *operation,
                                      psa_algorithm_t alg)
{
    psa_status_t status;
    ...
    if (PSA_ALG_IS_RAW_KEY_AGREEMENT(alg)) {
        ...
        status = esp_psa_key_agreement_try_support(ka_alg);
        if (status != PSA_SUCCESS) {
            return status;
        }
        if (!esp_psa_key_derivation_allows_free_form_secret_input(kdf_alg)) {
            return PSA_ERROR_INVALID_ARGUMENT;
        }
        status = esp_psa_key_derivation_setup_kdf(operation, kdf_alg);
#else
        return PSA_ERROR_NOT_SUPPORTED;
#endif /* AT_LEAST_ONE_BUILTIN_KDF */
    } else if (PSA_ALG_IS_KEY_DERIVATION(alg)) {
#if defined(AT_LEAST_ONE_BUILTIN_KDF)
        status = esp_psa_key_derivation_setup_kdf(operation, alg);
#else
        return PSA_ERROR_NOT_SUPPORTED;
#endif /* AT_LEAST_ONE_BUILTIN_KDF */
    } else {
...
#if defined(BUILTIN_ALG_ANY_HKDF)
static psa_status_t esp_psa_hkdf_input(psa_hkdf_key_derivation_t *hkdf,
                                   psa_algorithm_t kdf_alg,
    ...
    switch (step) {
        ...
        case PSA_KEY_DERIVATION_INPUT_SECRET:
...
#endif /* MBEDTLS_PSA_BUILTIN_ALG_HKDF_EXTRACT */
            {
                 ...
                 * generated by esp_psa_key_derivation_hkdf_read(). */
        ...
        case PSA_KEY_DERIVATION_INPUT_INFO:
            ...
            if (data_length != 0) {
                hkdf->info = esp_mbedtls_calloc(1, data_length);
                if (hkdf->info == NULL) {
                    return PSA_ERROR_INSUFFICIENT_MEMORY;
                }
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_PRF) || \
    defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_PSK_TO_MS)
static psa_status_t esp_psa_tls12_prf_set_seed(psa_tls12_prf_key_derivation_t *prf,
                                           const uint8_t *data,
                                           size_t data_length)
{
    ...
    if (data_length != 0) {
        prf->seed = esp_mbedtls_calloc(1, data_length);
        if (prf->seed == NULL) {
            return PSA_ERROR_INSUFFICIENT_MEMORY;
        }
...
static psa_status_t esp_psa_tls12_prf_set_key(psa_tls12_prf_key_derivation_t *prf,
                                          const uint8_t *data,
                                          size_t data_length)
{
    ...
    if (data_length != 0) {
        prf->secret = esp_mbedtls_calloc(1, data_length);
        if (prf->secret == NULL) {
            return PSA_ERROR_INSUFFICIENT_MEMORY;
        }
...
static psa_status_t esp_psa_tls12_prf_set_label(psa_tls12_prf_key_derivation_t *prf,
                                            const uint8_t *data,
                                            size_t data_length)
{
    ...
    if (data_length != 0) {
        prf->label = esp_mbedtls_calloc(1, data_length);
        if (prf->label == NULL) {
            return PSA_ERROR_INSUFFICIENT_MEMORY;
        }
...
static psa_status_t esp_psa_tls12_prf_input(psa_tls12_prf_key_derivation_t *prf,
                                        psa_key_derivation_step_t step,
                                        const uint8_t *data,
                                        size_t data_length)
{
    switch (step) {
        case PSA_KEY_DERIVATION_INPUT_SEED:
            return esp_psa_tls12_prf_set_seed(prf, data, data_length);
        case PSA_KEY_DERIVATION_INPUT_SECRET:
            return esp_psa_tls12_prf_set_key(prf, data, data_length);
        case PSA_KEY_DERIVATION_INPUT_LABEL:
            return esp_psa_tls12_prf_set_label(prf, data, data_length);
        default:
            return PSA_ERROR_INVALID_ARGUMENT;
    }
}
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_PSK_TO_MS)
static psa_status_t esp_psa_tls12_prf_psk_to_ms_set_key(
    ...
    size_t data_length)
{
    ...
    uint8_t *pms = esp_mbedtls_calloc(1, pms_len);
    if (pms == NULL) {
        return PSA_ERROR_INSUFFICIENT_MEMORY;
    }
    uint8_t *cur = pms;
    ...
    if (prf->state == PSA_TLS12_PRF_STATE_OTHER_KEY_SET) {
        ...
        *cur++ = MBEDTLS_BYTE_0(prf->other_secret_length);
        if (prf->other_secret_length != 0) {
            ...
            esp_mbedtls_platform_zeroize(prf->other_secret, prf->other_secret_length);
    ...
    *cur++ = MBEDTLS_BYTE_0(data_length);
    ...
    status = esp_psa_tls12_prf_set_key(prf, pms, (size_t) (cur - pms));

    esp_mbedtls_zeroize_and_free(pms, pms_len);
    return status;
}

static psa_status_t esp_psa_tls12_prf_psk_to_ms_set_other_key(
    psa_tls12_prf_key_derivation_t *prf,
    const uint8_t *data,
    size_t data_length)
{
    ...
    if (data_length != 0) {
        prf->other_secret = esp_mbedtls_calloc(1, data_length);
...
static psa_status_t esp_psa_tls12_prf_psk_to_ms_input(
    psa_tls12_prf_key_derivation_t *prf,
    psa_key_derivation_step_t step,
    const uint8_t *data,
    size_t data_length)
{
    switch (step) {
        case PSA_KEY_DERIVATION_INPUT_SECRET:
            return esp_psa_tls12_prf_psk_to_ms_set_key(prf,
                                                   data, data_length);
            break;
        case PSA_KEY_DERIVATION_INPUT_OTHER_SECRET:
            return esp_psa_tls12_prf_psk_to_ms_set_other_key(prf,
                                                         data,
                                                         data_length);
            break;
        default:
            return esp_psa_tls12_prf_input(prf, step, data, data_length);
            break;
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_ECJPAKE_TO_PMS)
static psa_status_t esp_psa_tls12_ecjpake_to_pms_input(
    psa_tls12_ecjpake_to_pms_t *ecjpake,
    psa_key_derivation_step_t step,
    const uint8_t *data,
    size_t data_length)
{
    ...
    /* Only esp_K.X has to be extracted - bytes 1 to 32 inclusive. */
...
#if defined(PSA_HAVE_SOFT_PBKDF2)
static psa_status_t esp_psa_pbkdf2_set_input_cost(
    psa_pbkdf2_key_derivation_t *pbkdf2,
    psa_key_derivation_step_t step,
    uint64_t data)
{
...
static psa_status_t esp_psa_pbkdf2_set_salt(psa_pbkdf2_key_derivation_t *pbkdf2,
                                        const uint8_t *data,
                                        size_t data_length)
{
    ...
    if (data_length == 0) {
        ...
        next_salt = esp_mbedtls_calloc(1, data_length + pbkdf2->salt_length);
        if (next_salt == NULL) {
            return PSA_ERROR_INSUFFICIENT_MEMORY;
        }
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_PBKDF2_HMAC)
static psa_status_t esp_psa_pbkdf2_hmac_set_password(psa_algorithm_t hash_alg,
                                                 const uint8_t *input,
                                                 size_t input_len,
                                                 uint8_t *output,
                                                 size_t *output_len)
{
    ...
    if (input_len > PSA_HASH_BLOCK_LENGTH(hash_alg)) {
        return esp_psa_hash_compute(hash_alg, input, input_len, output,
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_PBKDF2_AES_CMAC_PRF_128)
static psa_status_t esp_psa_pbkdf2_cmac_set_password(const uint8_t *input,
                                                 size_t input_len,
                                                 uint8_t *output,
                                                 size_t *output_len)
{
...
static psa_status_t esp_psa_pbkdf2_set_password(psa_pbkdf2_key_derivation_t *pbkdf2,
                                            psa_algorithm_t kdf_alg,
                                            const uint8_t *data,
                                            size_t data_length)
{
    ...
    if (PSA_ALG_IS_PBKDF2_HMAC(kdf_alg)) {
        ...
        status = esp_psa_pbkdf2_hmac_set_password(hash_alg, data, data_length,
                                              pbkdf2->password,
                                              &pbkdf2->password_length);
    } else
#endif /* MBEDTLS_PSA_BUILTIN_ALG_PBKDF2_HMAC */
#if defined(MBEDTLS_PSA_BUILTIN_ALG_PBKDF2_AES_CMAC_PRF_128)
    if (kdf_alg == PSA_ALG_PBKDF2_AES_CMAC_PRF_128) {
        status = esp_psa_pbkdf2_cmac_set_password(data, data_length,
                                              pbkdf2->password,
                                              &pbkdf2->password_length);
    } else
#endif /* MBEDTLS_PSA_BUILTIN_ALG_PBKDF2_AES_CMAC_PRF_128 */
...
static psa_status_t esp_psa_pbkdf2_input(psa_pbkdf2_key_derivation_t *pbkdf2,
                                     psa_algorithm_t kdf_alg,
                                     psa_key_derivation_step_t step,
                                     const uint8_t *data,
                                     size_t data_length)
{
    switch (step) {
        case PSA_KEY_DERIVATION_INPUT_SALT:
            return esp_psa_pbkdf2_set_salt(pbkdf2, data, data_length);
        case PSA_KEY_DERIVATION_INPUT_PASSWORD:
            return esp_psa_pbkdf2_set_password(pbkdf2, kdf_alg, data, data_length);
        default:
            return PSA_ERROR_INVALID_ARGUMENT;
    }
}
 ...
 */
static int esp_psa_key_derivation_check_input_type(
    psa_key_derivation_step_t step,
    psa_key_type_t key_type)
{
    switch (step) {
...
static psa_status_t esp_psa_key_derivation_input_internal(
    psa_key_derivation_operation_t *operation,
    psa_key_derivation_step_t step,
    psa_key_type_t key_type,
    const uint8_t *data,
    size_t data_length)
{
    ...
    status = esp_psa_key_derivation_check_input_type(step, key_type);
    if (status != PSA_SUCCESS) {
        goto exit;
    }
...
#if defined(BUILTIN_ALG_ANY_HKDF)
    if (PSA_ALG_IS_ANY_HKDF(kdf_alg)) {
        status = esp_psa_hkdf_input(&operation->ctx.hkdf, kdf_alg,
                                step, data, data_length);
    } else
#endif /* BUILTIN_ALG_ANY_HKDF */
#if defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_PRF)
    if (PSA_ALG_IS_TLS12_PRF(kdf_alg)) {
        status = esp_psa_tls12_prf_input(&operation->ctx.tls12_prf,
                                     step, data, data_length);
    } else
#endif /* MBEDTLS_PSA_BUILTIN_ALG_TLS12_PRF */
#if defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_PSK_TO_MS)
    if (PSA_ALG_IS_TLS12_PSK_TO_MS(kdf_alg)) {
        status = esp_psa_tls12_prf_psk_to_ms_input(&operation->ctx.tls12_prf,
                                               step, data, data_length);
    } else
#endif /* MBEDTLS_PSA_BUILTIN_ALG_TLS12_PSK_TO_MS */
#if defined(MBEDTLS_PSA_BUILTIN_ALG_TLS12_ECJPAKE_TO_PMS)
    if (kdf_alg == PSA_ALG_TLS12_ECJPAKE_TO_PMS) {
        status = esp_psa_tls12_ecjpake_to_pms_input(
            &operation->ctx.tls12_ecjpake_to_pms, step, data, data_length);
    } else
#endif /* MBEDTLS_PSA_BUILTIN_ALG_TLS12_ECJPAKE_TO_PMS */
#if defined(PSA_HAVE_SOFT_PBKDF2)
    if (PSA_ALG_IS_PBKDF2(kdf_alg)) {
        status = esp_psa_pbkdf2_input(&operation->ctx.pbkdf2, kdf_alg,
                                  step, data, data_length);
    } else
#endif /* PSA_HAVE_SOFT_PBKDF2 */
    {
...
static psa_status_t esp_psa_key_derivation_input_integer_internal(
    psa_key_derivation_operation_t *operation,
    psa_key_derivation_step_t step,
    uint64_t value)
{
...
#if defined(PSA_HAVE_SOFT_PBKDF2)
    if (PSA_ALG_IS_PBKDF2(kdf_alg)) {
        status = esp_psa_pbkdf2_set_input_cost(
...
psa_status_t esp_psa_key_derivation_input_bytes(
    psa_key_derivation_operation_t *operation,
    psa_key_derivation_step_t step,
    const uint8_t *data_external,
    size_t data_length)
{
    ...
    status = esp_psa_key_derivation_input_internal(operation, step,
                                               PSA_KEY_TYPE_NONE,
                                               data, data_length);
#if !defined(MBEDTLS_PSA_ASSUME_EXCLUSIVE_BUFFERS)
exit:
...
psa_status_t esp_psa_key_derivation_input_integer(
    psa_key_derivation_operation_t *operation,
    psa_key_derivation_step_t step,
    uint64_t value)
{
    return esp_psa_key_derivation_input_integer_internal(operation, step, value);
}

psa_status_t esp_psa_key_derivation_input_key(
    psa_key_derivation_operation_t *operation,
    psa_key_derivation_step_t step,
    mbedtls_svc_key_id_t key)
{
    ...
    status = esp_psa_get_and_lock_transparent_key_slot_with_policy(
    ...
    status = esp_psa_key_derivation_input_internal(operation,
                                               step, slot->attr.type,
                                               slot->key.data,
                                               slot->key.bytes);
...
psa_status_t esp_psa_key_agreement_raw_builtin(const psa_key_attributes_t *attributes,
                                           const uint8_t *key_buffer,
                                           size_t key_buffer_size,
                                           psa_algorithm_t alg,
                                           ...
                                           size_t *shared_secret_length)
{
    switch (alg) {
        ...
        case PSA_ALG_ECDH:
            return esp_mbedtls_psa_key_agreement_ecdh(attributes, key_buffer,
                                                  key_buffer_size, alg,
        ...
        case PSA_ALG_FFDH:
            return esp_mbedtls_psa_ffdh_key_agreement(attributes,
                                                  peer_key,
                                                  peer_key_length,
                                                  key_buffer,
                                                  key_buffer_size,
 ...
 *  (esp_psa_key_agreement_raw_builtin).
 */
static psa_status_t esp_psa_key_agreement_raw_internal(psa_algorithm_t alg,
                                                   ...
                                                   size_t *shared_secret_length)
{
    ...
    return esp_psa_driver_wrapper_key_agreement(&private_key->attr,
...
/* Note that if this function fails, you must call esp_psa_key_derivation_abort()
 * to potentially free embedded data structures and wipe confidential data.
 */
static psa_status_t esp_psa_key_agreement_internal(psa_key_derivation_operation_t *operation,
                                               psa_key_derivation_step_t step,
                                               psa_key_slot_t *private_key,
                                               const uint8_t *peer_key,
                                               size_t peer_key_length)
{
     ...
     * secret. */
    status = esp_psa_key_agreement_raw_internal(ka_alg,
                                            private_key,
                                            peer_key, peer_key_length,
                                            shared_secret,
    ...
    status = esp_psa_key_derivation_input_internal(operation, step,
                                               ...
                                               shared_secret_length);
exit:
    esp_mbedtls_platform_zeroize(shared_secret, shared_secret_length);
    return status;
}

psa_status_t esp_psa_key_derivation_key_agreement(psa_key_derivation_operation_t *operation,
                                              psa_key_derivation_step_t step,
                                              ...
                                              size_t peer_key_length)
{
    ...
    status = esp_psa_key_agreement_internal(operation, step,
                                        slot,
                                        peer_key, peer_key_length);
...
psa_status_t esp_psa_raw_key_agreement(psa_algorithm_t alg,
                                   mbedtls_svc_key_id_t private_key,
                                   const uint8_t *peer_key_external,
                                   size_t peer_key_length,
                                   uint8_t *output_external,
                                   ...
                                   size_t *output_length)
{
    ...
    LOCAL_INPUT_ALLOC(peer_key_external, peer_key_length, peer_key);
    status = esp_psa_key_agreement_raw_internal(alg, slot,
                                            peer_key, peer_key_length,
                                            output, output_size,
                                            output_length);
...
exit:
    ...
    if (output != NULL && status != PSA_SUCCESS) {
         ...
         */
        esp_psa_generate_random_internal(output, output_size);
 ...
 */
static void esp_mbedtls_psa_random_init(mbedtls_psa_random_context_t *rng)
{
#if defined(MBEDTLS_PSA_CRYPTO_EXTERNAL_RNG)
     ...
     * esp_mbedtls_psa_crypto_configure_entropy_sources() hasn't been called. */
    if (rng->entropy_init == NULL) {
        rng->entropy_init = esp_mbedtls_entropy_init;
    }
    if (rng->entropy_free == NULL) {
        rng->entropy_free = esp_mbedtls_entropy_free;
    }
    ...
    esp_mbedtls_entropy_add_source(&rng->entropy,
                               esp_mbedtls_nv_seed_poll, NULL,
                               MBEDTLS_ENTROPY_BLOCK_SIZE,
                               MBEDTLS_ENTROPY_SOURCE_STRONG);
#endif

    esp_mbedtls_psa_drbg_init(&rng->drbg);
#endif /* MBEDTLS_PSA_CRYPTO_EXTERNAL_RNG */
}
 ...
 *  this function if mutexes are enabled.
 */
static void esp_mbedtls_psa_random_free(mbedtls_psa_random_context_t *rng)
{
#if defined(MBEDTLS_PSA_CRYPTO_EXTERNAL_RNG)
    memset(rng, 0, sizeof(*rng));
#else /* MBEDTLS_PSA_CRYPTO_EXTERNAL_RNG */
    esp_mbedtls_psa_drbg_free(&rng->drbg);
 ...
 */
static psa_status_t esp_mbedtls_psa_random_seed(mbedtls_psa_random_context_t *rng)
{
#if defined(MBEDTLS_PSA_CRYPTO_EXTERNAL_RNG)
    ...
    int ret = esp_mbedtls_psa_drbg_seed(&rng->drbg, &rng->entropy,
                                    drbg_seed, sizeof(drbg_seed) - 1);
    return esp_mbedtls_to_psa_error(ret);
#endif /* MBEDTLS_PSA_CRYPTO_EXTERNAL_RNG */
}

psa_status_t esp_psa_generate_random(uint8_t *output_external,
                                 size_t output_size)
{
    psa_status_t status;
    ...
    status = esp_psa_generate_random_internal(output, output_size);
...
#if defined(MBEDTLS_PSA_INJECT_ENTROPY)
psa_status_t esp_mbedtls_psa_inject_entropy(const uint8_t *seed,
                                        size_t seed_size)
{
    if (esp_psa_get_initialized()) {
        return PSA_ERROR_NOT_PERMITTED;
    }
    ...
    return esp_mbedtls_psa_storage_inject_entropy(seed, seed_size);
}
 ...
 */
static psa_status_t esp_psa_validate_key_type_and_size_for_key_generation(
    psa_key_type_t type, size_t bits)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;

    if (esp_key_type_is_raw_bytes(type)) {
        status = esp_psa_validate_unstructured_key_bit_size(type, bits);
        if (status != PSA_SUCCESS) {
            return status;
        }
    } else
    ...
    if (PSA_KEY_TYPE_IS_DH(type) && PSA_KEY_TYPE_IS_KEY_PAIR(type)) {
        if (esp_psa_is_dh_key_size_valid(bits) == 0) {
            return PSA_ERROR_NOT_SUPPORTED;
        }
    } else
...
psa_status_t esp_psa_generate_key_internal(
    const psa_key_attributes_t *attributes,
    ...
    uint8_t *key_buffer, size_t key_buffer_size, size_t *key_buffer_length)
{
    ...
    if (esp_key_type_is_raw_bytes(type)) {
        status = esp_psa_generate_random_internal(key_buffer, key_buffer_size);
        if (status != PSA_SUCCESS) {
            return status;
        }
        ...
        if (type == PSA_KEY_TYPE_DES) {
            esp_psa_des_set_key_parity(key_buffer, key_buffer_size);
        }
#endif /* MBEDTLS_PSA_BUILTIN_KEY_TYPE_DES */
    } else
    ...
    if (type == PSA_KEY_TYPE_RSA_KEY_PAIR) {
        return esp_mbedtls_psa_rsa_generate_key(attributes,
    ...
    if (PSA_KEY_TYPE_IS_ECC(type) && PSA_KEY_TYPE_IS_KEY_PAIR(type)) {
        return esp_mbedtls_psa_ecp_generate_key(attributes,
                                            key_buffer,
                                            key_buffer_size,
                                            key_buffer_length);
    } else
    ...
    if (PSA_KEY_TYPE_IS_DH(type) && PSA_KEY_TYPE_IS_KEY_PAIR(type)) {
        return esp_mbedtls_psa_ffdh_generate_key(attributes,
                                             key_buffer,
                                             key_buffer_size,
                                             key_buffer_length);
    } else
...
psa_status_t esp_psa_generate_key_ext(const psa_key_attributes_t *attributes,
                                  const psa_key_production_parameters_t *params,
                                  size_t params_data_length,
                                  mbedtls_svc_key_id_t *key)
{
    ...
    *key = MBEDTLS_SVC_KEY_ID_INIT;
    ...
    status = esp_psa_start_key_creation(PSA_KEY_CREATION_GENERATE, attributes,
                                    &slot, &driver);
    if (status != PSA_SUCCESS) {
        goto exit;
    }
    ...
    if (slot->key.data == NULL) {
        if (PSA_KEY_LIFETIME_GET_LOCATION(attributes->lifetime) ==
            ...
            status = esp_psa_validate_key_type_and_size_for_key_generation(
            ...
            status = esp_psa_driver_wrapper_get_key_buffer_size(
        ...
        status = esp_psa_allocate_buffer_to_slot(slot, key_buffer_size);
        if (status != PSA_SUCCESS) {
            goto exit;
        }
    }

    status = esp_psa_driver_wrapper_generate_key(attributes,
                                             params, params_data_length,
                                             slot->key.data, slot->key.bytes,
                                             &slot->key.bytes);
    if (status != PSA_SUCCESS) {
        esp_psa_remove_key_data_from_memory(slot);
    }
...
psa_status_t esp_psa_generate_key(const psa_key_attributes_t *attributes,
                              mbedtls_svc_key_id_t *key)
{
    return esp_psa_generate_key_ext(attributes,
                                &esp_default_production_parameters, 0,
                                key);
}
...
#if !defined(MBEDTLS_PSA_CRYPTO_EXTERNAL_RNG)
psa_status_t esp_mbedtls_psa_crypto_configure_entropy_sources(
    ...
    void (* entropy_free)(mbedtls_entropy_context *ctx))
{
    ...
    if (esp_global_data.rng_state != RNG_NOT_INITIALIZED) {
        status = PSA_ERROR_BAD_STATE;
    } else {
        esp_global_data.rng.entropy_init = entropy_init;
        esp_global_data.rng.entropy_free = entropy_free;
        status = PSA_SUCCESS;
    }
...
void esp_mbedtls_psa_crypto_free(void)
{
    ...
    if (esp_global_data.initialized & PSA_CRYPTO_SUBSYSTEM_TRANSACTION_INITIALIZED) {
        esp_global_data.initialized &= ~PSA_CRYPTO_SUBSYSTEM_TRANSACTION_INITIALIZED;
    }

    if (esp_global_data.initialized & PSA_CRYPTO_SUBSYSTEM_KEY_SLOTS_INITIALIZED) {
        esp_psa_wipe_all_key_slots();
        esp_global_data.initialized &= ~PSA_CRYPTO_SUBSYSTEM_KEY_SLOTS_INITIALIZED;
    }
    ...
    if (esp_global_data.rng_state != RNG_NOT_INITIALIZED) {
        esp_mbedtls_psa_random_free(&esp_global_data.rng);
    }
    esp_global_data.rng_state = RNG_NOT_INITIALIZED;
    esp_mbedtls_platform_zeroize(&esp_global_data.rng, sizeof(esp_global_data.rng));
    ...
    /* Terminate drivers */
    if (esp_global_data.initialized & PSA_CRYPTO_SUBSYSTEM_DRIVER_WRAPPERS_INITIALIZED) {
        esp_psa_driver_wrapper_free();
        esp_global_data.initialized &= ~PSA_CRYPTO_SUBSYSTEM_DRIVER_WRAPPERS_INITIALIZED;
    }
 ...
 *
 * This function is called during initialization, before esp_psa_crypto_init()
 * returns. If this function returns a failure status, the initialization
 * fails.
 */
static psa_status_t esp_psa_crypto_recover_transaction(
...
static psa_status_t esp_mbedtls_psa_crypto_init_subsystem(mbedtls_psa_crypto_subsystem subsystem)
{
    psa_status_t status = PSA_SUCCESS;
    ...
    switch (subsystem) {
...
#if defined(MBEDTLS_THREADING_C)
            PSA_THREADING_CHK_GOTO_EXIT(esp_mbedtls_mutex_lock(&mbedtls_threading_psa_globaldata_mutex));
#endif /* defined(MBEDTLS_THREADING_C) */

            if (!(esp_global_data.initialized & PSA_CRYPTO_SUBSYSTEM_DRIVER_WRAPPERS_INITIALIZED)) {
                /* Init drivers */
                status = esp_psa_driver_wrapper_init();
...
#if defined(MBEDTLS_THREADING_C)
            PSA_THREADING_CHK_GOTO_EXIT(esp_mbedtls_mutex_unlock(
        ...
        case PSA_CRYPTO_SUBSYSTEM_KEY_SLOTS:
            ...
            if (!(esp_global_data.initialized & PSA_CRYPTO_SUBSYSTEM_KEY_SLOTS_INITIALIZED)) {
                status = esp_psa_initialize_key_slots();
        ...
        case PSA_CRYPTO_SUBSYSTEM_RNG:
             ...
             * mutex. */
#if defined(MBEDTLS_THREADING_C)
            PSA_THREADING_CHK_GOTO_EXIT(esp_mbedtls_mutex_lock(&mbedtls_threading_psa_rngdata_mutex));
#endif /* defined(MBEDTLS_THREADING_C) */

            /* Initialize and seed the random generator. */
            if (esp_global_data.rng_state == RNG_NOT_INITIALIZED && driver_wrappers_initialized) {
                esp_mbedtls_psa_random_init(&esp_global_data.rng);
                esp_global_data.rng_state = RNG_INITIALIZED;

                status = esp_mbedtls_psa_random_seed(&esp_global_data.rng);
                if (status == PSA_SUCCESS) {
                    esp_global_data.rng_state = RNG_SEEDED;
                }
            }
        ...
        case PSA_CRYPTO_SUBSYSTEM_TRANSACTION:
            ...
            if (!(esp_global_data.initialized & PSA_CRYPTO_SUBSYSTEM_TRANSACTION_INITIALIZED)) {
#if defined(PSA_CRYPTO_STORAGE_HAS_TRANSACTIONS)
                status = esp_psa_crypto_load_transaction();
                if (status == PSA_SUCCESS) {
                    status = esp_psa_crypto_recover_transaction(&esp_psa_crypto_transaction);
                    if (status == PSA_SUCCESS) {
                        esp_global_data.initialized |= PSA_CRYPTO_SUBSYSTEM_TRANSACTION_INITIALIZED;
                    }
                    status = esp_psa_crypto_stop_transaction();
...
psa_status_t esp_psa_crypto_init(void)
{
    psa_status_t status;
    ...
    status = esp_mbedtls_psa_crypto_init_subsystem(PSA_CRYPTO_SUBSYSTEM_DRIVER_WRAPPERS);
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    status = esp_mbedtls_psa_crypto_init_subsystem(PSA_CRYPTO_SUBSYSTEM_KEY_SLOTS);
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    status = esp_mbedtls_psa_crypto_init_subsystem(PSA_CRYPTO_SUBSYSTEM_RNG);
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    status = esp_mbedtls_psa_crypto_init_subsystem(PSA_CRYPTO_SUBSYSTEM_TRANSACTION);
...
psa_status_t esp_psa_crypto_driver_pake_get_password_len(
...
psa_status_t esp_psa_crypto_driver_pake_get_password(
    const psa_crypto_driver_pake_inputs_t *inputs,
...
psa_status_t esp_psa_crypto_driver_pake_get_user_len(
    const psa_crypto_driver_pake_inputs_t *inputs,
    size_t *user_len)
{
...
psa_status_t esp_psa_crypto_driver_pake_get_user(
    const psa_crypto_driver_pake_inputs_t *inputs,
...
psa_status_t esp_psa_crypto_driver_pake_get_peer_len(
    const psa_crypto_driver_pake_inputs_t *inputs,
    size_t *peer_len)
{
...
psa_status_t esp_psa_crypto_driver_pake_get_peer(
    const psa_crypto_driver_pake_inputs_t *inputs,
...
psa_status_t esp_psa_crypto_driver_pake_get_cipher_suite(
    const psa_crypto_driver_pake_inputs_t *inputs,
...
psa_status_t esp_psa_pake_setup(
    ...
    const psa_pake_cipher_suite_t *cipher_suite)
{
    ...
    return PSA_SUCCESS;
exit:
    esp_psa_pake_abort(operation);
    return status;
}

psa_status_t esp_psa_pake_set_password_key(
    psa_pake_operation_t *operation,
    mbedtls_svc_key_id_t password)
{
    ...
    status = esp_psa_get_and_lock_key_slot_with_policy(password, &slot,
    ...
    operation->data.inputs.password = esp_mbedtls_calloc(1, slot->key.bytes);
...
psa_status_t esp_psa_pake_set_user(
    psa_pake_operation_t *operation,
    ...
    size_t user_id_len)
{
    ...
    operation->data.inputs.user = esp_mbedtls_calloc(1, user_id_len);
...
psa_status_t esp_psa_pake_set_peer(
    psa_pake_operation_t *operation,
    ...
    size_t peer_id_len)
{
    ...
    operation->data.inputs.peer = esp_mbedtls_calloc(1, peer_id_len);
...
psa_status_t esp_psa_pake_set_role(
    psa_pake_operation_t *operation,
...
#if defined(PSA_WANT_ALG_JPAKE)
static psa_crypto_driver_pake_step_t esp_convert_jpake_computation_stage_to_driver_step(
...
static psa_status_t esp_psa_pake_complete_inputs(
    psa_pake_operation_t *operation)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
    ...
    esp_mbedtls_platform_zeroize(&operation->data, sizeof(operation->data));

    status = esp_psa_driver_wrapper_pake_setup(operation, &inputs);

    /* Driver is responsible for creating its own copy of the password. */
    esp_mbedtls_zeroize_and_free(inputs.password, inputs.password_len);

    /* User and peer are translated to role. */
    esp_mbedtls_free(inputs.user);
    esp_mbedtls_free(inputs.peer);
...
#if defined(PSA_WANT_ALG_JPAKE)
static psa_status_t esp_psa_jpake_prologue(
    psa_pake_operation_t *operation,
...
static psa_status_t esp_psa_jpake_epilogue(
    psa_pake_operation_t *operation,
    psa_jpake_io_mode_t io_mode)
{
    psa_jpake_computation_stage_t *stage =
...
psa_status_t esp_psa_pake_output(
    psa_pake_operation_t *operation,
    psa_pake_step_t step,
    uint8_t *output_external,
    size_t output_size,
    size_t *output_length)
{
    ...
    *output_length = 0;
    ...
    if (operation->stage == PSA_PAKE_OPERATION_STAGE_COLLECT_INPUTS) {
        status = esp_psa_pake_complete_inputs(operation);
        if (status != PSA_SUCCESS) {
            goto exit;
        }
    }
...
#if defined(PSA_WANT_ALG_JPAKE)
        case PSA_ALG_JPAKE:
            status = esp_psa_jpake_prologue(operation, step, PSA_JPAKE_OUTPUT);
            if (status != PSA_SUCCESS) {
                goto exit;
            }
            driver_step = esp_convert_jpake_computation_stage_to_driver_step(
                &operation->computation_stage.jpake);
            break;
#endif /* PSA_WANT_ALG_JPAKE */
        default:
    ...
    status = esp_psa_driver_wrapper_pake_output(operation, driver_step,
                                            output, output_size, output_length);
...
#if defined(PSA_WANT_ALG_JPAKE)
        case PSA_ALG_JPAKE:
            status = esp_psa_jpake_epilogue(operation, PSA_JPAKE_OUTPUT);
            if (status != PSA_SUCCESS) {
                goto exit;
            }
            break;
...
psa_status_t esp_psa_pake_input(
    psa_pake_operation_t *operation,
    psa_pake_step_t step,
    const uint8_t *input_external,
    size_t input_length)
{
...
#if defined(PSA_WANT_ALG_JPAKE)
        case PSA_ALG_JPAKE:
            status = esp_psa_jpake_prologue(operation, step, PSA_JPAKE_INPUT);
            if (status != PSA_SUCCESS) {
                goto exit;
            }
            driver_step = esp_convert_jpake_computation_stage_to_driver_step(
    ...
    LOCAL_INPUT_ALLOC(input_external, input_length, input);
    status = esp_psa_driver_wrapper_pake_input(operation, driver_step,
                                           input, input_length);
...
#if defined(PSA_WANT_ALG_JPAKE)
        case PSA_ALG_JPAKE:
            status = esp_psa_jpake_epilogue(operation, PSA_JPAKE_INPUT);
            if (status != PSA_SUCCESS) {
                goto exit;
            }
            break;
...
psa_status_t esp_psa_pake_get_implicit_key(
    psa_pake_operation_t *operation,
    psa_key_derivation_operation_t *output)
{
    ...
    status = esp_psa_driver_wrapper_pake_get_implicit_key(operation,
                                                      shared_key,
                                                      sizeof(shared_key),
                                                      &shared_key_len);
    ...
    status = esp_psa_key_derivation_input_bytes(output,
                                            PSA_KEY_DERIVATION_INPUT_SECRET,
                                            shared_key,
                                            shared_key_len);

    esp_mbedtls_platform_zeroize(shared_key, sizeof(shared_key));
exit:
    abort_status = esp_psa_pake_abort(operation);
    return status == PSA_SUCCESS ? abort_status : status;
}

psa_status_t esp_psa_pake_abort(
    psa_pake_operation_t *operation)
{
    psa_status_t status = PSA_SUCCESS;

    if (operation->stage == PSA_PAKE_OPERATION_STAGE_COMPUTATION) {
        status = esp_psa_driver_wrapper_pake_abort(operation);
    }

    if (operation->stage == PSA_PAKE_OPERATION_STAGE_COLLECT_INPUTS) {
        if (operation->data.inputs.password != NULL) {
            esp_mbedtls_zeroize_and_free(operation->data.inputs.password,
                                     operation->data.inputs.password_len);
        }
        if (operation->data.inputs.user != NULL) {
            esp_mbedtls_free(operation->data.inputs.user);
        }
        if (operation->data.inputs.peer != NULL) {
            esp_mbedtls_free(operation->data.inputs.peer);
        }
    }
...
void (*esp_psa_input_pre_copy_hook)(const uint8_t *input, size_t input_len) = NULL;
void (*esp_psa_input_post_copy_hook)(const uint8_t *input, size_t input_len) = NULL;
void (*esp_psa_output_pre_copy_hook)(const uint8_t *output, size_t output_len) = NULL;
void (*esp_psa_output_post_copy_hook)(const uint8_t *output, size_t output_len) = NULL;
#endif
...
psa_status_t esp_psa_crypto_copy_input(const uint8_t *input, size_t input_len,
...
#if defined(MBEDTLS_TEST_HOOKS)
    if (esp_psa_input_pre_copy_hook != NULL) {
        esp_psa_input_pre_copy_hook(input, input_len);
    }
#endif
...
#if defined(MBEDTLS_TEST_HOOKS)
    if (esp_psa_input_post_copy_hook != NULL) {
        esp_psa_input_post_copy_hook(input, input_len);
    }
#endif
 ...
 */
MBEDTLS_STATIC_TESTABLE
psa_status_t esp_psa_crypto_copy_output(const uint8_t *output_copy, size_t output_copy_len,
...
#if defined(MBEDTLS_TEST_HOOKS)
    if (esp_psa_output_pre_copy_hook != NULL) {
        esp_psa_output_pre_copy_hook(output, output_len);
    }
#endif
...
#if defined(MBEDTLS_TEST_HOOKS)
    if (esp_psa_output_post_copy_hook != NULL) {
        esp_psa_output_post_copy_hook(output, output_len);
    }
#endif
...
psa_status_t esp_psa_crypto_local_input_alloc(const uint8_t *input, size_t input_len,
                                          psa_crypto_local_input_t *local_input)
{
    ...
    *local_input = PSA_CRYPTO_LOCAL_INPUT_INIT;
    ...
    local_input->buffer = esp_mbedtls_calloc(input_len, 1);
    ...
    status = esp_psa_crypto_copy_input(input, input_len,
...
error:
    esp_mbedtls_free(local_input->buffer);
    local_input->buffer = NULL;
    local_input->length = 0;
    return status;
}

void esp_psa_crypto_local_input_free(psa_crypto_local_input_t *local_input)
{
    esp_mbedtls_free(local_input->buffer);
    local_input->buffer = NULL;
    local_input->length = 0;
...
psa_status_t esp_psa_crypto_local_output_alloc(uint8_t *output, size_t output_len,
                                           psa_crypto_local_output_t *local_output)
{
    *local_output = PSA_CRYPTO_LOCAL_OUTPUT_INIT;
    ...
    local_output->buffer = esp_mbedtls_calloc(output_len, 1);
...
psa_status_t esp_psa_crypto_local_output_free(psa_crypto_local_output_t *local_output)
{
    psa_status_t status;
    ...
    status = esp_psa_crypto_copy_output(local_output->buffer, local_output->length,
                                    local_output->original, local_output->length);
    if (status != PSA_SUCCESS) {
        return status;
    }

    esp_mbedtls_free(local_output->buffer);
