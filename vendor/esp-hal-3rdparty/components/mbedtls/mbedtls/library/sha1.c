...
void esp_mbedtls_sha1_free(mbedtls_sha1_context *ctx)
{
    ...
    esp_mbedtls_platform_zeroize(ctx, sizeof(mbedtls_sha1_context));
}
...
int esp_mbedtls_internal_sha1_process(mbedtls_sha1_context *ctx,
 ...
 */
int esp_mbedtls_sha1_update(mbedtls_sha1_context *ctx,
 ...
 * SHA-1 final digest
 */
int esp_mbedtls_sha1_finish(mbedtls_sha1_context *ctx,
 ...
 */
int esp_mbedtls_sha1(const unsigned char *input,
                 size_t ilen,
                 unsigned char output[20])
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    esp_mbedtls_sha1_init(&ctx);

    if ((ret = esp_mbedtls_sha1_starts(&ctx)) != 0) {
        goto exit;
    }

    if ((ret = esp_mbedtls_sha1_update(&ctx, input, ilen)) != 0) {
        goto exit;
    }

    if ((ret = esp_mbedtls_sha1_finish(&ctx, output)) != 0) {
        goto exit;
    }
 ...
 */
static const unsigned char esp_sha1_test_buf[3][57] =
{
    { "abc" },
...
static const size_t esp_sha1_test_buflen[3] =
{
    3, 56, 1000
};

static const unsigned char esp_sha1_test_sum[3][20] =
{
 ...
 * Checkup routine
 */
int esp_mbedtls_sha1_self_test(int verbose)
{
    ...
    for (i = 0; i < 3; i++) {
        if (verbose != 0) {
            esp_mbedtls_printf("  SHA-1 test #%d: ", i + 1);
        }
        ...
        if (i == 2) {
            ...
            for (j = 0; j < 1000; j++) {
                ret = esp_mbedtls_sha1_update(&ctx, buf, buflen);
                if (ret != 0) {
                    goto fail;
                }
            }
        } else {
            ret = esp_mbedtls_sha1_update(&ctx, esp_sha1_test_buf[i],
                                      esp_sha1_test_buflen[i]);
            if (ret != 0) {
                goto fail;
            }
        }

        if ((ret = esp_mbedtls_sha1_finish(&ctx, sha1sum)) != 0) {
            goto fail;
        }

        if (memcmp(sha1sum, esp_sha1_test_sum[i], 20) != 0) {
            ret = 1;
            goto fail;
        }
