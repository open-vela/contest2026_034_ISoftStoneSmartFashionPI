 ...
 */
static const int esp_ciphersuite_preference[] =
{
...
static const mbedtls_ssl_ciphersuite_t esp_ciphersuite_definitions[] =
{
#if defined(MBEDTLS_SSL_PROTO_TLS1_3)
...
{
    return esp_ciphersuite_preference;
}
#else
#define MAX_CIPHERSUITES    sizeof(esp_ciphersuite_definitions) /         \
    sizeof(esp_ciphersuite_definitions[0])
static int esp_supported_ciphersuites[MAX_CIPHERSUITES];
static int esp_supported_init = 0;

MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ciphersuite_is_removed(const mbedtls_ssl_ciphersuite_t *cs_info)
{
    (void) cs_info;
...
const int *esp_mbedtls_ssl_list_ciphersuites(void)
{
     ...
     * build based on presence in the esp_ciphersuite_definitions.
     */
    if (esp_supported_init == 0) {
        const int *p;
        int *q;

        for (p = esp_ciphersuite_preference, q = esp_supported_ciphersuites;
             *p != 0 && q < esp_supported_ciphersuites + MAX_CIPHERSUITES - 1;
             p++) {
            const mbedtls_ssl_ciphersuite_t *cs_info;
            if ((cs_info = esp_mbedtls_ssl_ciphersuite_from_id(*p)) != NULL &&
                !esp_ciphersuite_is_removed(cs_info)) {
                *(q++) = *p;
            }
        }
        *q = 0;
    ...
    return esp_supported_ciphersuites;
}
#endif /* MBEDTLS_SSL_CIPHERSUITES */

const mbedtls_ssl_ciphersuite_t *esp_mbedtls_ssl_ciphersuite_from_string(
    const char *ciphersuite_name)
{
    const mbedtls_ssl_ciphersuite_t *cur = esp_ciphersuite_definitions;
...
const mbedtls_ssl_ciphersuite_t *esp_mbedtls_ssl_ciphersuite_from_id(int ciphersuite)
{
    const mbedtls_ssl_ciphersuite_t *cur = esp_ciphersuite_definitions;
...
const char *esp_mbedtls_ssl_get_ciphersuite_name(const int ciphersuite_id)
{
    ...
    cur = esp_mbedtls_ssl_ciphersuite_from_id(ciphersuite_id);
...
int esp_mbedtls_ssl_get_ciphersuite_id(const char *ciphersuite_name)
{
    ...
    cur = esp_mbedtls_ssl_ciphersuite_from_string(ciphersuite_name);
...
size_t esp_mbedtls_ssl_ciphersuite_get_cipher_key_bitlen(const mbedtls_ssl_ciphersuite_t *info)
{
    ...
    status = esp_mbedtls_ssl_cipher_to_psa((mbedtls_cipher_type_t) info->cipher,
    ...
    const mbedtls_cipher_info_t * const cipher_info =
        esp_mbedtls_cipher_info_from_type((mbedtls_cipher_type_t) info->cipher);

    return esp_mbedtls_cipher_info_get_key_bitlen(cipher_info);
    ...
    switch (info->key_exchange) {
        ...
        case MBEDTLS_KEY_EXCHANGE_ECDHE_RSA:
            return PSA_ALG_RSA_PKCS1V15_SIGN(
                esp_mbedtls_md_psa_alg_from_type((mbedtls_md_type_t) info->mac));

        case MBEDTLS_KEY_EXCHANGE_ECDHE_ECDSA:
            return PSA_ALG_ECDSA(esp_mbedtls_md_psa_alg_from_type((mbedtls_md_type_t) info->mac));
