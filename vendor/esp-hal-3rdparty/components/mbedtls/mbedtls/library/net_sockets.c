...
static int esp_wsa_init_done = 0;
 ...
 */
static int esp_check_fd(int fd, int for_select)
{
    if (fd < 0) {
 ...
 */
int esp_mbedtls_net_connect(mbedtls_net_context *ctx, const char *host,
...
int esp_mbedtls_net_poll(mbedtls_net_context *ctx, uint32_t rw, uint32_t timeout)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    struct timeval tv;
 ...
 */
int esp_mbedtls_net_recv_timeout(void *ctx, unsigned char *buf,
 ...
 */
void esp_mbedtls_net_close(mbedtls_net_context *ctx)
{
    if (ctx->fd == -1) {
        return;
    }
