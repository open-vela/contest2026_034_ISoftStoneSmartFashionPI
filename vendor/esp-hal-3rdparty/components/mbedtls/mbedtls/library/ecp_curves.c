...
static const mbedtls_mpi_uint esp_mpi_one[] = { 1 };
#endif
...
static inline void esp_ecp_mpi_set1(mbedtls_mpi *X)
{
    X->s = 1;
    X->n = 1;
    X->p = (mbedtls_mpi_uint *) esp_mpi_one; /* X->p will not be modified so the cast is safe */
}
...
static const unsigned char esp_curve25519_part_of_n[] = {
...
static int esp_ecp_use_curve25519(mbedtls_ecp_group *grp)
{
    ...
    /* P = 2^255 - 19 */
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_lset(&grp->P, 1));
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_shift_l(&grp->P, 255));
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_sub_int(&grp->P, &grp->P, 19));
    grp->pbits = esp_mbedtls_mpi_bitlen(&grp->P);

    /* N = 2^252 + 27742317777372353535851937790883648493 */
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_read_binary(&grp->N,
                                            esp_curve25519_part_of_n, sizeof(esp_curve25519_part_of_n)));
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_set_bit(&grp->N, 252, 1));
...
static const unsigned char esp_curve448_part_of_n[] = {
...
static int esp_ecp_use_curve448(mbedtls_ecp_group *grp)
{
    ...
    /* P = 2^448 - 2^224 - 1 */
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_lset(&grp->P, 1));
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_shift_l(&grp->P, 224));
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_sub_int(&grp->P, &grp->P, 1));
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_shift_l(&grp->P, 224));
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_sub_int(&grp->P, &grp->P, 1));
    grp->pbits = esp_mbedtls_mpi_bitlen(&grp->P);
    ...
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_set_bit(&grp->N, 446, 1));
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_read_binary(&Ns,
                                            esp_curve448_part_of_n, sizeof(esp_curve448_part_of_n)));
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_sub_mpi(&grp->N, &grp->N, &Ns));

    /* Actually, the required msb for private keys */
    grp->nbits = 447;

cleanup:
    esp_mbedtls_mpi_free(&Ns);
    if (ret != 0) {
        esp_mbedtls_ecp_group_free(grp);
    }
...
static int esp_ecp_mod_p192(mbedtls_mpi *N)
{
    ...
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_grow(N, 6 * WIDTH));
 ...
 */
static inline void esp_add32(uint32_t *dst, uint32_t src, signed char *carry)
{
    *dst += src;
    *carry += (*dst < src);
}

static inline void esp_sub32(uint32_t *dst, uint32_t src, signed char *carry)
{
    *carry -= (*dst < src);
    *dst -= src;
}

#define ADD(j)    esp_add32(&cur, A(j), &c);
#define SUB(j)    esp_sub32(&cur, A(j), &c);
...
#define INIT(b)                                                       \
    ...
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_grow(N, (b) * 2 / biL + 1));      \
    LOAD32;
...
#define LAST                                    \
    ...
    if (c < 0) esp_mbedtls_ecp_fix_negative(N, c, bits);

/*
 * If the result is negative, we get it in the form
 * c * 2^bits + N, with c negative and N positive shorter than 'bits'
 */
static void esp_mbedtls_ecp_fix_negative(mbedtls_mpi *N, signed char c, size_t bits)
{
    size_t i;
...
static int esp_ecp_mod_p521(mbedtls_mpi *N)
{
    ...
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_shift_r(&M, 521 % (8 * sizeof(mbedtls_mpi_uint))));
    ...
    /* N = A0 + A1 */
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_add_abs(N, N, &M));
...
static int esp_ecp_mod_p255(mbedtls_mpi *N)
{
    ...
    /* N = A0 + 38 * A1 */
    esp_mbedtls_mpi_core_mla(N->p, P255_WIDTH + 1,
                         Mp, NT_n,
                         38);
...
static int esp_ecp_mod_p448(mbedtls_mpi *N)
{
    ...
    /* N += A1 */
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_add_mpi(N, N, &M));

    /* Q = B1, N += B1 */
    Q = M;
    Q.p = Qp;
    memcpy(Qp, Mp, sizeof(Qp));
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_shift_r(&Q, 224));
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_add_mpi(N, N, &Q));
    ...
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_add_mpi(&M, &M, &Q));
    M.n = P448_WIDTH + 1; /* Make room for shifted carry bit from the addition */
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_shift_l(&M, 224));
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_add_mpi(N, N, &M));
...
static inline int esp_ecp_mod_koblitz(mbedtls_mpi *N, const mbedtls_mpi_uint *Rp, size_t p_limbs,
                                  size_t adjust, size_t shift, mbedtls_mpi_uint mask)
{
    ...
    if (shift != 0) {
        MBEDTLS_MPI_CHK(esp_mbedtls_mpi_shift_r(&M, shift));
    }
    ...
    /* N = A0 + R * A1 */
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_mul_mpi(&M, &M, &R));
    MBEDTLS_MPI_CHK(esp_mbedtls_mpi_add_abs(N, N, &M));
...
static int esp_ecp_mod_p192k1(mbedtls_mpi *N)
{
    ...
    return esp_ecp_mod_koblitz(N, Rp, 192 / 8 / sizeof(mbedtls_mpi_uint), 0, 0,
                           0);
}
#endif /* MBEDTLS_ECP_DP_SECP192K1_ENABLED */
...
#if defined(MBEDTLS_HAVE_INT64)
    return esp_ecp_mod_koblitz(N, Rp, 4, 1, 32, 0xFFFFFFFF);
#else
    return esp_ecp_mod_koblitz(N, Rp, 224 / 8 / sizeof(mbedtls_mpi_uint), 0, 0,
                           0);
#endif
}
...
static int esp_ecp_mod_p256k1(mbedtls_mpi *N)
{
    ...
    return esp_ecp_mod_koblitz(N, Rp, 256 / 8 / sizeof(mbedtls_mpi_uint), 0, 0,
                           0);
}
#endif /* MBEDTLS_ECP_DP_SECP256K1_ENABLED */
