 ...
 *   |
 *   |-- In any order:
 *   |   | esp_psa_pake_set_password_key()
 *   |   | esp_psa_pake_set_user()
 *   |   | esp_psa_pake_set_peer()
 *   |   | esp_psa_pake_set_role()
 *   |
 *   |--- In any order: (First round input before or after first round output)
 *   |   |
 *   |   |------ In Order
 *   |   |       | esp_psa_pake_output(PSA_PAKE_STEP_KEY_SHARE)
 *   |   |       | esp_psa_pake_output(PSA_PAKE_STEP_ZK_PUBLIC)
 *   |   |       | esp_psa_pake_output(PSA_PAKE_STEP_ZK_PROOF)
 *   |   |       | esp_psa_pake_output(PSA_PAKE_STEP_KEY_SHARE)
 *   |   |       | esp_psa_pake_output(PSA_PAKE_STEP_ZK_PUBLIC)
 *   |   |       | esp_psa_pake_output(PSA_PAKE_STEP_ZK_PROOF)
 *   |   |
 *   |   |------ In Order:
 *   |           | esp_psa_pake_input(PSA_PAKE_STEP_KEY_SHARE)
 *   |           | esp_psa_pake_input(PSA_PAKE_STEP_ZK_PUBLIC)
 *   |           | esp_psa_pake_input(PSA_PAKE_STEP_ZK_PROOF)
 *   |           | esp_psa_pake_input(PSA_PAKE_STEP_KEY_SHARE)
 *   |           | esp_psa_pake_input(PSA_PAKE_STEP_ZK_PUBLIC)
 *   |           | esp_psa_pake_input(PSA_PAKE_STEP_ZK_PROOF)
 *   |
 ...
 * |   |
 * |   |------ In Order
 * |   |       | esp_mbedtls_psa_pake_output(PSA_JPAKE_X1_STEP_KEY_SHARE)
 * |   |       | esp_mbedtls_psa_pake_output(PSA_JPAKE_X1_STEP_ZK_PUBLIC)
 * |   |       | esp_mbedtls_psa_pake_output(PSA_JPAKE_X1_STEP_ZK_PROOF)
 * |   |       | esp_mbedtls_psa_pake_output(PSA_JPAKE_X2_STEP_KEY_SHARE)
 * |   |       | esp_mbedtls_psa_pake_output(PSA_JPAKE_X2_STEP_ZK_PUBLIC)
 * |   |       | esp_mbedtls_psa_pake_output(PSA_JPAKE_X2_STEP_ZK_PROOF)
 * |   |
 * |   |------ In Order:
 * |           | esp_mbedtls_psa_pake_input(PSA_JPAKE_X1_STEP_KEY_SHARE)
 * |           | esp_mbedtls_psa_pake_input(PSA_JPAKE_X1_STEP_ZK_PUBLIC)
 * |           | esp_mbedtls_psa_pake_input(PSA_JPAKE_X1_STEP_ZK_PROOF)
 * |           | esp_mbedtls_psa_pake_input(PSA_JPAKE_X2_STEP_KEY_SHARE)
 * |           | esp_mbedtls_psa_pake_input(PSA_JPAKE_X2_STEP_ZK_PUBLIC)
 * |           | esp_mbedtls_psa_pake_input(PSA_JPAKE_X2_STEP_ZK_PROOF)
 * |
 * |--- In any order:
 * |   |
 * |   |------ In Order
 * |   |       | esp_mbedtls_psa_pake_output(PSA_JPAKE_X2S_STEP_KEY_SHARE)
 * |   |       | esp_mbedtls_psa_pake_output(PSA_JPAKE_X2S_STEP_ZK_PUBLIC)
 * |   |       | esp_mbedtls_psa_pake_output(PSA_JPAKE_X2S_STEP_ZK_PROOF)
 * |   |
 * |   |------ In Order:
 * |           | esp_mbedtls_psa_pake_input(PSA_JPAKE_X4S_STEP_KEY_SHARE)
 * |           | esp_mbedtls_psa_pake_input(PSA_JPAKE_X4S_STEP_ZK_PUBLIC)
 * |           | esp_mbedtls_psa_pake_input(PSA_JPAKE_X4S_STEP_ZK_PROOF)
 */

#if defined(MBEDTLS_PSA_BUILTIN_ALG_JPAKE)
static psa_status_t esp_mbedtls_ecjpake_to_psa_error(int ret)
{
    switch (ret) {
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_JPAKE)
static psa_status_t esp_psa_pake_ecjpake_setup(mbedtls_psa_pake_operation_t *operation)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;

    esp_mbedtls_ecjpake_init(&operation->ctx.jpake);

    ret = esp_mbedtls_ecjpake_setup(&operation->ctx.jpake,
                                operation->role,
                                MBEDTLS_MD_SHA256,
    ...
    esp_mbedtls_platform_zeroize(operation->password, operation->password_len);

    if (ret != 0) {
        return esp_mbedtls_ecjpake_to_psa_error(ret);
    }
...
psa_status_t esp_mbedtls_psa_pake_setup(mbedtls_psa_pake_operation_t *operation,
                                    const psa_crypto_driver_pake_inputs_t *inputs)
{
    ...
    status = esp_psa_crypto_driver_pake_get_password_len(inputs, &password_len);
    if (status != PSA_SUCCESS) {
        return status;
    }

    status = esp_psa_crypto_driver_pake_get_user_len(inputs, &user_len);
    if (status != PSA_SUCCESS) {
        return status;
    }

    status = esp_psa_crypto_driver_pake_get_peer_len(inputs, &peer_len);
    if (status != PSA_SUCCESS) {
        return status;
    }

    status = esp_psa_crypto_driver_pake_get_cipher_suite(inputs, &cipher_suite);
    if (status != PSA_SUCCESS) {
        return status;
    }

    operation->password = esp_mbedtls_calloc(1, password_len);
    if (operation->password == NULL) {
        status = PSA_ERROR_INSUFFICIENT_MEMORY;
        goto error;
    }

    user = esp_mbedtls_calloc(1, user_len);
    if (user == NULL) {
        status = PSA_ERROR_INSUFFICIENT_MEMORY;
        goto error;
    }

    peer = esp_mbedtls_calloc(1, peer_len);
    if (peer == NULL) {
        status = PSA_ERROR_INSUFFICIENT_MEMORY;
        goto error;
    }

    status = esp_psa_crypto_driver_pake_get_password(inputs, operation->password,
                                                 password_len, &actual_password_len);
    if (status != PSA_SUCCESS) {
        goto error;
    }

    status = esp_psa_crypto_driver_pake_get_user(inputs, user,
                                             user_len, &actual_user_len);
    if (status != PSA_SUCCESS) {
        goto error;
    }

    status = esp_psa_crypto_driver_pake_get_peer(inputs, peer,
    ...
    if (cipher_suite.algorithm == PSA_ALG_JPAKE) {
        ...
        const size_t user_peer_len = sizeof(esp_jpake_client_id); // client and server have the same length
        ...
        if (memcmp(user, esp_jpake_client_id, actual_user_len) == 0 &&
            memcmp(peer, esp_jpake_server_id, actual_peer_len) == 0) {
            operation->role = MBEDTLS_ECJPAKE_CLIENT;
        } else
        if (memcmp(user, esp_jpake_server_id, actual_user_len) == 0 &&
            memcmp(peer, esp_jpake_client_id, actual_peer_len) == 0) {
        ...
        status = esp_psa_pake_ecjpake_setup(operation);
        if (status != PSA_SUCCESS) {
            goto error;
        }

        /* Role has been set, release user/peer buffers. */
        esp_mbedtls_free(user); esp_mbedtls_free(peer);
...
error:
     ...
     * call esp_mbedtls_psa_pake_abort(). Therefore call it here to do the needed clean
     * up like freeing the memory that may have been allocated to store the password.
     */
    esp_mbedtls_psa_pake_abort(operation);
    return status;
}

static psa_status_t esp_mbedtls_psa_pake_output_internal(
    ...
    size_t *output_length)
{
    ...
    if (operation->alg == PSA_ALG_JPAKE) {
        ...
        if (step == PSA_JPAKE_X1_STEP_KEY_SHARE) {
            ret = esp_mbedtls_ecjpake_write_round_one(&operation->ctx.jpake,
                                                  operation->buffer,
            ...
            ret = esp_mbedtls_ecjpake_write_round_two(&operation->ctx.jpake,
                                                  operation->buffer,
                                                  sizeof(operation->buffer),
                                                  &operation->buffer_length,
                                                  esp_mbedtls_psa_get_random,
        ...
        *output_length = length;
        ...
        if ((step == PSA_JPAKE_X2_STEP_ZK_PROOF) ||
            ...
            esp_mbedtls_platform_zeroize(operation->buffer, sizeof(operation->buffer));
            operation->buffer_length = 0;
            operation->buffer_offset = 0;
        }
...
psa_status_t esp_mbedtls_psa_pake_output(mbedtls_psa_pake_operation_t *operation,
                                     psa_crypto_driver_pake_step_t step,
                                     uint8_t *output,
                                     size_t output_size,
                                     size_t *output_length)
{
    psa_status_t status = esp_mbedtls_psa_pake_output_internal(
        operation, step, output, output_size, output_length);

    return status;
}

static psa_status_t esp_mbedtls_psa_pake_input_internal(
    mbedtls_psa_pake_operation_t *operation,
    psa_crypto_driver_pake_step_t step,
    const uint8_t *input,
    size_t input_length)
{
    ...
    if (operation->alg == PSA_ALG_JPAKE) {
        ...
        if (step == PSA_JPAKE_X2_STEP_ZK_PROOF) {
            ret = esp_mbedtls_ecjpake_read_round_one(&operation->ctx.jpake,
                                                 operation->buffer,
                                                 operation->buffer_length);
            ...
            ret = esp_mbedtls_ecjpake_read_round_two(&operation->ctx.jpake,
                                                 operation->buffer,
                                                 operation->buffer_length);
...
psa_status_t esp_mbedtls_psa_pake_input(mbedtls_psa_pake_operation_t *operation,
                                    psa_crypto_driver_pake_step_t step,
                                    const uint8_t *input,
                                    size_t input_length)
{
    psa_status_t status = esp_mbedtls_psa_pake_input_internal(
        operation, step, input, input_length);

    return status;
}

psa_status_t esp_mbedtls_psa_pake_get_implicit_key(
    mbedtls_psa_pake_operation_t *operation,
...
#if defined(MBEDTLS_PSA_BUILTIN_ALG_JPAKE)
    if (operation->alg == PSA_ALG_JPAKE) {
        ret = esp_mbedtls_ecjpake_write_shared_key(&operation->ctx.jpake,
                                               output,
                                               output_size,
                                               output_length,
                                               esp_mbedtls_psa_get_random,
...
psa_status_t esp_mbedtls_psa_pake_abort(mbedtls_psa_pake_operation_t *operation)
{
    esp_mbedtls_zeroize_and_free(operation->password, operation->password_len);
    ...
    if (operation->alg == PSA_ALG_JPAKE) {
        ...
        operation->buffer_length = 0;
        operation->buffer_offset = 0;
        esp_mbedtls_ecjpake_free(&operation->ctx.jpake);
    }
#endif
