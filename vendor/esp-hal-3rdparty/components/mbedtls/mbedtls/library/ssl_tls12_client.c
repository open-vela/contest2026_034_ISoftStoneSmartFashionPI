...
static int esp_ssl_write_renegotiation_ext(mbedtls_ssl_context *ssl,
                                       unsigned char *buf,
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_write_supported_point_formats_ext(mbedtls_ssl_context *ssl,
                                                 unsigned char *buf,
                                                 const unsigned char *end,
                                                 size_t *olen)
{
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_write_ecjpake_kkpp_ext(mbedtls_ssl_context *ssl,
                                      unsigned char *buf,
                                      const unsigned char *end,
                                      size_t *olen)
{
    ...
    if (ssl->handshake->ecjpake_cache == NULL ||
        ...
        if (ret != 0) {
            MBEDTLS_SSL_DEBUG_RET(1,
                                  "esp_mbedtls_ecjpake_write_round_one", ret);
            return ret;
        }
#endif /* MBEDTLS_USE_PSA_CRYPTO */

        ssl->handshake->ecjpake_cache = esp_mbedtls_calloc(1, kkpp_len);
        if (ssl->handshake->ecjpake_cache == NULL) {
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_write_cid_ext(mbedtls_ssl_context *ssl,
                             unsigned char *buf,
                             const unsigned char *end,
                             size_t *olen)
{
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_write_max_fragment_length_ext(mbedtls_ssl_context *ssl,
                                             unsigned char *buf,
                                             const unsigned char *end,
                                             size_t *olen)
{
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_write_encrypt_then_mac_ext(mbedtls_ssl_context *ssl,
                                          unsigned char *buf,
                                          const unsigned char *end,
                                          size_t *olen)
{
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_write_extended_ms_ext(mbedtls_ssl_context *ssl,
                                     unsigned char *buf,
                                     const unsigned char *end,
                                     size_t *olen)
{
...
#if defined(MBEDTLS_SSL_SESSION_TICKETS)
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_write_session_ticket_ext(mbedtls_ssl_context *ssl,
                                        unsigned char *buf,
                                        const unsigned char *end,
                                        size_t *olen)
{
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_write_use_srtp_ext(mbedtls_ssl_context *ssl,
                                  unsigned char *buf,
                                  const unsigned char *end,
                                  size_t *olen)
{
    ...
    *p++ = MBEDTLS_BYTE_0(2 * ssl->conf->dtls_srtp_profile_list_len);
    ...
    for (protection_profiles_index = 0;
        ...
        if (profile_value != MBEDTLS_TLS_SRTP_UNSET) {
             ...
             * is checked by esp_mbedtls_ssl_conf_dtls_srtp_protection_profiles function
             */
            MBEDTLS_SSL_DEBUG_MSG(3,
                                  ("client hello, "
                                              ...
                                              size_t *out_len)
{
    ...
    *out_len = 0;
...
#if defined(MBEDTLS_SSL_RENEGOTIATION)
    if ((ret = esp_ssl_write_renegotiation_ext(ssl, p, end, &ext_len)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_ssl_write_renegotiation_ext", ret);
        return ret;
    }
    p += ext_len;
#endif
    ...
    defined(MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
    if (uses_ec) {
        if ((ret = esp_ssl_write_supported_point_formats_ext(ssl, p, end,
                                                         &ext_len)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_ssl_write_supported_point_formats_ext", ret);
            return ret;
        }
        p += ext_len;
    }
...
#if defined(MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
    if ((ret = esp_ssl_write_ecjpake_kkpp_ext(ssl, p, end, &ext_len)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_ssl_write_ecjpake_kkpp_ext", ret);
        return ret;
    }
    p += ext_len;
#endif
...
#if defined(MBEDTLS_SSL_DTLS_CONNECTION_ID)
    if ((ret = esp_ssl_write_cid_ext(ssl, p, end, &ext_len)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_ssl_write_cid_ext", ret);
        return ret;
    }
    p += ext_len;
#endif /* MBEDTLS_SSL_DTLS_CONNECTION_ID */
...
#if defined(MBEDTLS_SSL_MAX_FRAGMENT_LENGTH)
    if ((ret = esp_ssl_write_max_fragment_length_ext(ssl, p, end,
                                                 &ext_len)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_ssl_write_max_fragment_length_ext", ret);
        return ret;
    }
    p += ext_len;
#endif
...
#if defined(MBEDTLS_SSL_ENCRYPT_THEN_MAC)
    if ((ret = esp_ssl_write_encrypt_then_mac_ext(ssl, p, end, &ext_len)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_ssl_write_encrypt_then_mac_ext", ret);
        return ret;
    }
    p += ext_len;
#endif
...
#if defined(MBEDTLS_SSL_EXTENDED_MASTER_SECRET)
    if ((ret = esp_ssl_write_extended_ms_ext(ssl, p, end, &ext_len)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_ssl_write_extended_ms_ext", ret);
        return ret;
    }
    p += ext_len;
#endif
...
#if defined(MBEDTLS_SSL_DTLS_SRTP)
    if ((ret = esp_ssl_write_use_srtp_ext(ssl, p, end, &ext_len)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_ssl_write_use_srtp_ext", ret);
        return ret;
    }
    p += ext_len;
#endif
...
#if defined(MBEDTLS_SSL_SESSION_TICKETS)
    if ((ret = esp_ssl_write_session_ticket_ext(ssl, p, end, &ext_len)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_ssl_write_session_ticket_ext", ret);
        return ret;
    }
    p += ext_len;
#endif
                                        ...
                                        size_t len)
{
    ...
    if (ssl->renego_status != MBEDTLS_SSL_INITIAL_HANDSHAKE) {
        ...
        if (len    != 1 + ssl->verify_data_len * 2 ||
            ...
            esp_mbedtls_ct_memcmp(buf + 1,
                              ssl->own_verify_data, ssl->verify_data_len) != 0 ||
            esp_mbedtls_ct_memcmp(buf + 1 + ssl->verify_data_len,
                                        ...
                                        size_t len)
{
    ...
    defined(MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_parse_supported_point_formats_ext(mbedtls_ssl_context *ssl,
                                                 const unsigned char *buf,
                                                 size_t len)
{
    size_t list_size;
                                  ...
                                  size_t len)
{
    ...
    esp_mbedtls_free(ssl->handshake->ecjpake_cache);
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_parse_alpn_ext(mbedtls_ssl_context *ssl,
...
static int esp_ssl_parse_hello_verify_request(mbedtls_ssl_context *ssl)
{
    ...
    esp_mbedtls_free(ssl->handshake->cookie);

    ssl->handshake->cookie = esp_mbedtls_calloc(1, cookie_len);
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_parse_server_hello(mbedtls_ssl_context *ssl)
{
    int ret, i;
    size_t n;
    size_t ext_len;
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
        if (buf[0] == MBEDTLS_SSL_HS_HELLO_VERIFY_REQUEST) {
            ...
            MBEDTLS_SSL_DEBUG_MSG(2, ("<= parse server hello"));
            return esp_ssl_parse_hello_verify_request(ssl);
        } else {
     ...
     */
    ssl->handshake->ciphersuite_info = esp_mbedtls_ssl_ciphersuite_from_id(i);
    ...
    esp_mbedtls_ssl_optimize_checksum(ssl, ssl->handshake->ciphersuite_info);
    ...
    suite_info = esp_mbedtls_ssl_ciphersuite_from_id(
        ssl->session_negotiate->ciphersuite);
    if (esp_mbedtls_ssl_validate_ciphersuite(ssl, suite_info, ssl->tls_version,
                                         ssl->tls_version) != 0) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("bad server hello message"));
        esp_mbedtls_ssl_send_alert_message(
            ssl,
    ...
    while (ext_len) {
        ...
        switch (ext_id) {
            case MBEDTLS_TLS_EXT_RENEGOTIATION_INFO:
                ...
                if ((ret = esp_ssl_parse_renegotiation_info(ssl, ext + 4,
                                                        ext_size)) != 0) {
                    return ret;
                }
            ...
            case MBEDTLS_TLS_EXT_MAX_FRAGMENT_LENGTH:
                ...
                if ((ret = esp_ssl_parse_max_fragment_length_ext(ssl,
                                                             ext + 4, ext_size)) != 0) {
                    return ret;
                }
            ...
            case MBEDTLS_TLS_EXT_CID:
                ...
                if ((ret = esp_ssl_parse_cid_ext(ssl,
                                             ext + 4,
                                             ext_size)) != 0) {
                    return ret;
                }
            ...
            case MBEDTLS_TLS_EXT_ENCRYPT_THEN_MAC:
                ...
                if ((ret = esp_ssl_parse_encrypt_then_mac_ext(ssl,
                                                          ext + 4, ext_size)) != 0) {
                    return ret;
                }
            ...
            case MBEDTLS_TLS_EXT_EXTENDED_MASTER_SECRET:
                ...
                if ((ret = esp_ssl_parse_extended_ms_ext(ssl,
                                                     ext + 4, ext_size)) != 0) {
                    return ret;
                }
            ...
            case MBEDTLS_TLS_EXT_SESSION_TICKET:
                ...
                if ((ret = esp_ssl_parse_session_ticket_ext(ssl,
                                                        ext + 4, ext_size)) != 0) {
                    return ret;
                }
            ...
            case MBEDTLS_TLS_EXT_SUPPORTED_POINT_FORMATS:
                ...
                if ((ret = esp_ssl_parse_supported_point_formats_ext(ssl,
                                                                 ext + 4, ext_size)) != 0) {
                    return ret;
                }
          ...
          MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED */
            ...
            case MBEDTLS_TLS_EXT_ECJPAKE_KKPP:
                ...
                if ((ret = esp_ssl_parse_ecjpake_kkpp(ssl,
                                                  ext + 4, ext_size)) != 0) {
                    return ret;
                }
            ...
            case MBEDTLS_TLS_EXT_ALPN:
                ...
                if ((ret = esp_ssl_parse_alpn_ext(ssl, ext + 4, ext_size)) != 0) {
                    return ret;
                }
            ...
            case MBEDTLS_TLS_EXT_USE_SRTP:
                ...
                if ((ret = esp_ssl_parse_use_srtp_ext(ssl, ext + 4, ext_size)) != 0) {
                    return ret;
                }
    ...
    /*
     * esp_mbedtls_ssl_derive_keys() has to be called after the parsing of the
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_parse_server_dh_params(mbedtls_ssl_context *ssl,
                                      unsigned char **p,
                                      unsigned char *end)
{
    int ret = MBEDTLS_ERR_SSL_FEATURE_UNAVAILABLE;
     ...
     * } ServerDHParams;
     */
    if ((ret = esp_mbedtls_dhm_read_params(&ssl->handshake->dhm_ctx,
                                       p, end)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(2, ("esp_mbedtls_dhm_read_params"), ret);
        return ret;
    }

    dhm_actual_bitlen = esp_mbedtls_dhm_get_bitlen(&ssl->handshake->dhm_ctx);
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_parse_server_ecdh_params(mbedtls_ssl_context *ssl,
                                        unsigned char **p,
                                        unsigned char *end)
{
    uint16_t tls_id;
    ...
    *p += 2;
    ...
    if (esp_mbedtls_ssl_check_curve_tls_id(ssl, tls_id) != 0) {
        MBEDTLS_SSL_DEBUG_MSG(2,
    ...
    if (esp_mbedtls_ssl_get_psa_curve_info_from_tls_id(tls_id, &key_type,
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_check_server_ecdh_params(const mbedtls_ssl_context *ssl)
{
    uint16_t tls_id;
    ...
    MBEDTLS_SSL_DEBUG_MSG(2, ("ECDH curve: %s",
                              esp_mbedtls_ssl_get_curve_name_from_tls_id(tls_id)));
                                        ...
                                        unsigned char *end)
{
     ...
     * } ServerECDHParams;
     */
    if ((ret = esp_mbedtls_ecdh_read_params(&ssl->handshake->ecdh_ctx,
                                        (const unsigned char **) p, end)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, ("esp_mbedtls_ecdh_read_params"), ret);
#if defined(MBEDTLS_SSL_ECP_RESTARTABLE_ENABLED)
    ...
    if (esp_ssl_check_server_ecdh_params(ssl) != 0) {
        MBEDTLS_SSL_DEBUG_MSG(1,
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_parse_server_psk_hint(mbedtls_ssl_context *ssl,
                                     unsigned char **p,
                                     unsigned char *end)
{
    int ret = MBEDTLS_ERR_SSL_FEATURE_UNAVAILABLE;
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_write_encrypted_pms(mbedtls_ssl_context *ssl,
                                   ...
                                   size_t pms_offset)
{
     ...
     *  } PreMasterSecret;
     */
    esp_mbedtls_ssl_write_version(p, ssl->conf->transport,
     ...
     */
    if (!esp_mbedtls_pk_can_do(peer_pk, MBEDTLS_PK_RSA)) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("certificate key type mismatch"));
        return MBEDTLS_ERR_SSL_PK_TYPE_MISMATCH;
    }

    if ((ret = esp_mbedtls_pk_encrypt(peer_pk,
                                  p, ssl->handshake->pmslen,
                                  ssl->out_msg + offset + len_bytes, olen,
                                  MBEDTLS_SSL_OUT_CONTENT_LEN - offset - len_bytes,
                                  ssl->conf->f_rng, ssl->conf->p_rng)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_rsa_pkcs1_encrypt", ret);
        return ret;
    }
...
static int esp_ssl_get_ecdh_params_from_cert(mbedtls_ssl_context *ssl)
{
    ...
    if (!esp_mbedtls_pk_can_do(peer_pk, MBEDTLS_PK_ECKEY)) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("server key not ECDH capable"));
        return MBEDTLS_ERR_SSL_PK_TYPE_MISMATCH;
    }

#if !defined(MBEDTLS_PK_USE_PSA_EC_DATA)
    const mbedtls_ecp_keypair *peer_key = esp_mbedtls_pk_ec_ro(*peer_pk);
#endif /* !defined(MBEDTLS_PK_USE_PSA_EC_DATA) */

#if defined(MBEDTLS_USE_PSA_CRYPTO)
    uint16_t tls_id = 0;
    psa_key_type_t key_type = PSA_KEY_TYPE_NONE;
    mbedtls_ecp_group_id grp_id = esp_mbedtls_pk_get_ec_group_id(peer_pk);
    ...
    size_t olen = 0;
    ret = esp_mbedtls_ecp_point_write_binary(&peer_key->grp, &peer_key->Q,
                                         MBEDTLS_ECP_PF_UNCOMPRESSED, &olen,
                                         ssl->handshake->xxdh_psa_peerkey,
                                         sizeof(ssl->handshake->xxdh_psa_peerkey));

    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, ("esp_mbedtls_ecp_point_write_binary"), ret);
        return ret;
    }
    ssl->handshake->xxdh_psa_peerkey_len = olen;
#endif /* MBEDTLS_PK_USE_PSA_EC_DATA */
#else /* MBEDTLS_USE_PSA_CRYPTO */
    if ((ret = esp_mbedtls_ecdh_get_params(&ssl->handshake->ecdh_ctx, peer_key,
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_parse_server_key_exchange(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECDH_RSA ||
        ...
        if ((ret = esp_ssl_get_ecdh_params_from_cert(ssl)) != 0) {
...
start_processing:
    ...
    if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_PSK ||
        ...
        if (esp_ssl_parse_server_psk_hint(ssl, &p, end) != 0) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("bad server key exchange message"));
            esp_mbedtls_ssl_send_alert_message(
                ssl,
                MBEDTLS_SSL_ALERT_LEVEL_FATAL,
    ...
    if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_DHE_RSA ||
        ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_DHE_PSK) {
        if (esp_ssl_parse_server_dh_params(ssl, &p, end) != 0) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("bad server key exchange message"));
            esp_mbedtls_ssl_send_alert_message(
                ssl,
                MBEDTLS_SSL_ALERT_LEVEL_FATAL,
    ...
    if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECDHE_RSA ||
        ...
        if (esp_ssl_parse_server_ecdh_params(ssl, &p, end) != 0) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("bad server key exchange message"));
            esp_mbedtls_ssl_send_alert_message(
                ssl,
                MBEDTLS_SSL_ALERT_LEVEL_FATAL,
    ...
    if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECJPAKE) {
        ...
        uint16_t exp_tls_id = esp_mbedtls_ssl_get_tls_id_from_ecp_group_id(
    ...
    if (esp_mbedtls_ssl_ciphersuite_uses_server_signature(ciphersuite_info)) {
        ...
        if (pk_alg == MBEDTLS_PK_RSASSA_PSS) {
            ...
            rsassa_pss_options.expected_salt_len =
                esp_mbedtls_md_get_size_from_type(md_alg);
            if (rsassa_pss_options.expected_salt_len == 0) {
                return MBEDTLS_ERR_SSL_INTERNAL_ERROR;
            }

            ret = esp_mbedtls_pk_verify_ext(pk_alg, &rsassa_pss_options,
                                        peer_pk,
                                        md_alg, hash, hashlen,
                                        p, sig_len);
        } else
#endif /* MBEDTLS_X509_RSASSA_PSS_SUPPORT */
        ret = esp_mbedtls_pk_verify_restartable(peer_pk,
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_parse_certificate_request(mbedtls_ssl_context *ssl)
{
    const mbedtls_ssl_ciphersuite_t *ciphersuite_info =
        ssl->handshake->ciphersuite_info;
...
static int esp_ssl_parse_certificate_request(mbedtls_ssl_context *ssl)
{
    ...
    sig_alg_len = MBEDTLS_GET_UINT16_BE(buf, esp_mbedtls_ssl_hs_hdr_len(ssl) + 1 + n);
    ...
    dn_len = MBEDTLS_GET_UINT16_BE(buf, esp_mbedtls_ssl_hs_hdr_len(ssl) + 1 + n);
    ...
    for (size_t i = 0, dni_len = 0; i < dn_len; i += 2 + dni_len) {
        ...
        if (dni_len > dn_len - i - 2 ||
            esp_mbedtls_asn1_get_tag(&p, p + dni_len, &asn1_len,
                                 MBEDTLS_ASN1_CONSTRUCTED | MBEDTLS_ASN1_SEQUENCE) != 0 ||
            esp_mbedtls_x509_get_name(&p, p + asn1_len, &name) != 0) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("bad certificate request message"));
            esp_mbedtls_ssl_send_alert_message(
                ssl,
                MBEDTLS_SSL_ALERT_LEVEL_FATAL,
        ...
        MBEDTLS_SSL_DEBUG_MSG(3,
                              ("DN hint: %.*s",
                               esp_mbedtls_x509_dn_gets(s, sizeof(s), &name), s));
        esp_mbedtls_asn1_free_named_data_list_shallow(name.next);
    }
#endif
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_parse_server_hello_done(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_write_client_key_exchange(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_DHE_RSA) {
         ...
         */
        content_len = esp_mbedtls_dhm_get_len(&ssl->handshake->dhm_ctx);

        MBEDTLS_PUT_UINT16_BE(content_len, ssl->out_msg, 4);
        header_len = 6;

        ret = esp_mbedtls_dhm_make_public(&ssl->handshake->dhm_ctx,
                                      (int) esp_mbedtls_dhm_get_len(&ssl->handshake->dhm_ctx),
                                      &ssl->out_msg[header_len], content_len,
                                      ssl->conf->f_rng, ssl->conf->p_rng);
        if (ret != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_dhm_make_public", ret);
            return ret;
        }
    ...
    if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECDHE_RSA ||
        ...
        if (ssl->handshake->ecrs_enabled) {
            ...
            esp_mbedtls_ecdh_enable_restart(&ssl->handshake->ecdh_ctx);
        }
#endif

        ret = esp_mbedtls_ecdh_make_public(&ssl->handshake->ecdh_ctx,
                                       &content_len,
                                       &ssl->out_msg[header_len], 1000,
                                       ssl->conf->f_rng, ssl->conf->p_rng);
        if (ret != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ecdh_make_public", ret);
#if defined(MBEDTLS_SSL_ECP_RESTARTABLE_ENABLED)
            if (ret == MBEDTLS_ERR_ECP_IN_PROGRESS) {
                ret = MBEDTLS_ERR_SSL_CRYPTO_IN_PROGRESS;
            }
...
ecdh_calc_secret:
    ...
    if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECDHE_PSK) {
         ...
         */
        if (esp_mbedtls_ssl_conf_has_static_psk(ssl->conf) == 0) {
        ...
        *p = (unsigned char) own_pubkey_len;
        ...
        pms += zlen_size + zlen;
          ...
          MBEDTLS_KEY_EXCHANGE_ECDHE_PSK_ENABLED */
#if defined(MBEDTLS_KEY_EXCHANGE_SOME_PSK_ENABLED)
    if (esp_mbedtls_ssl_ciphersuite_uses_psk(ciphersuite_info)) {
        /*
         * opaque psk_identity<0..2^16-1>;
         */
        if (esp_mbedtls_ssl_conf_has_static_psk(ssl->conf) == 0) {
        ...
        if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_RSA_PSK) {
            if ((ret = esp_ssl_write_encrypted_pms(ssl, header_len,
                                               &content_len, 2)) != 0) {
                return ret;
            }
        } else
        ...
        if ((ret = esp_mbedtls_ssl_psk_derive_premaster(ssl,
                                                    ...
                                                    key_exchange)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1,
                                  "esp_mbedtls_ssl_psk_derive_premaster", ret);
            return ret;
        }
#endif /* !MBEDTLS_USE_PSA_CRYPTO */
    } else
...
#else
        ret = esp_mbedtls_ecjpake_write_round_two(&ssl->handshake->ecjpake_ctx,
                                              ssl->out_msg + header_len,
                                              MBEDTLS_SSL_OUT_CONTENT_LEN - header_len,
                                              &content_len,
                                              ssl->conf->f_rng, ssl->conf->p_rng);
...
#if !defined(MBEDTLS_KEY_EXCHANGE_CERT_REQ_ALLOWED_ENABLED)
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_write_certificate_verify(mbedtls_ssl_context *ssl)
{
    const mbedtls_ssl_ciphersuite_t *ciphersuite_info =
        ssl->handshake->ciphersuite_info;
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
...
#if defined(MBEDTLS_SSL_SESSION_TICKETS)
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_parse_new_session_ticket(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    uint32_t lifetime;
    size_t ticket_len;
    ...
    if (ssl->session != NULL && ssl->session->ticket != NULL) {
        esp_mbedtls_zeroize_and_free(ssl->session->ticket,
                                 ssl->session->ticket_len);
        ssl->session->ticket = NULL;
        ssl->session->ticket_len = 0;
    }

    esp_mbedtls_zeroize_and_free(ssl->session_negotiate->ticket,
...
int esp_mbedtls_ssl_handshake_client_step(mbedtls_ssl_context *ssl)
{
    ...
    switch (ssl->state) {
        ...
        case MBEDTLS_SSL_SERVER_HELLO:
            ret = esp_ssl_parse_server_hello(ssl);
            break;
        ...
        case MBEDTLS_SSL_SERVER_KEY_EXCHANGE:
            ret = esp_ssl_parse_server_key_exchange(ssl);
            break;

        case MBEDTLS_SSL_CERTIFICATE_REQUEST:
            ret = esp_ssl_parse_certificate_request(ssl);
            break;

        case MBEDTLS_SSL_SERVER_HELLO_DONE:
            ret = esp_ssl_parse_server_hello_done(ssl);
            break;
        ...
        case MBEDTLS_SSL_CLIENT_KEY_EXCHANGE:
            ret = esp_ssl_write_client_key_exchange(ssl);
            break;

        case MBEDTLS_SSL_CERTIFICATE_VERIFY:
            ret = esp_ssl_write_certificate_verify(ssl);
            break;
        ...
        case MBEDTLS_SSL_NEW_SESSION_TICKET:
            ret = esp_ssl_parse_new_session_ticket(ssl);
            break;
#endif
