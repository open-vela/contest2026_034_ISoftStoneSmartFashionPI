...
#if defined(MBEDTLS_MD_LIGHT)
const mbedtls_error_pair_t esp_psa_to_md_errors[] =
{
    { PSA_SUCCESS,                     0 },
...
#if defined(MBEDTLS_BLOCK_CIPHER_SOME_PSA)
const mbedtls_error_pair_t esp_psa_to_cipher_errors[] =
{
    { PSA_SUCCESS,                     0 },
...
#if defined(MBEDTLS_LMS_C)
const mbedtls_error_pair_t esp_psa_to_lms_errors[] =
{
    { PSA_SUCCESS,                     0 },
...
#if defined(MBEDTLS_SSL_TLS_C) && \
    (defined(MBEDTLS_USE_PSA_CRYPTO) || defined(MBEDTLS_SSL_PROTO_TLS1_3))
const mbedtls_error_pair_t esp_psa_to_ssl_errors[] =
{
    { PSA_SUCCESS,                     0 },
...
#if defined(PSA_WANT_KEY_TYPE_RSA_PUBLIC_KEY) ||    \
    defined(PSA_WANT_KEY_TYPE_RSA_KEY_PAIR_BASIC)
const mbedtls_error_pair_t esp_psa_to_pk_rsa_errors[] =
{
    { PSA_SUCCESS,                     0 },
...
#if defined(MBEDTLS_USE_PSA_CRYPTO) && \
    defined(PSA_WANT_KEY_TYPE_ECC_PUBLIC_KEY)
const mbedtls_error_pair_t esp_psa_to_pk_ecdsa_errors[] =
{
    { PSA_SUCCESS,                     0 },
...
int esp_psa_generic_status_to_mbedtls(psa_status_t status)
{
    switch (status) {
        case PSA_SUCCESS:
            return 0;
...
int esp_psa_status_to_mbedtls(psa_status_t status,
...
#if defined(MBEDTLS_PK_C)
int esp_psa_pk_status_to_mbedtls(psa_status_t status)
{
    switch (status) {
        ...
        default:
            return esp_psa_generic_status_to_mbedtls(status);
    }
}
#endif /* MBEDTLS_PK_C */
...
psa_ecc_family_t esp_mbedtls_ecc_group_to_psa(mbedtls_ecp_group_id grpid,
                                          size_t *bits)
{
    switch (grpid) {
...
mbedtls_ecp_group_id esp_mbedtls_ecc_group_from_psa(psa_ecc_family_t family,
                                                size_t bits)
{
    switch (family) {
 ...
 *
 * `esp_mbedtls_psa_get_random(MBEDTLS_PSA_RANDOM_STATE, ...)` calls
 * `esp_psa_generate_random(...)`. The state parameter is ignored since the
 * PSA API doesn't support passing an explicit state.
 */
int esp_mbedtls_psa_get_random(void *p_rng,
                           ...
                           size_t output_size)
{
     ...
     * NULL. */
    (void) p_rng;
    psa_status_t status = esp_psa_generate_random(output, output_size);
 ...
 */
static int esp_convert_raw_to_der_single_int(const unsigned char *raw_buf, size_t raw_len,
                                         ...
                                         unsigned char *der_buf_end)
{
    ...
    MBEDTLS_ASN1_CHK_ADD(len, esp_mbedtls_asn1_write_len(&p, der_buf_start, len));
    MBEDTLS_ASN1_CHK_ADD(len, esp_mbedtls_asn1_write_tag(&p, der_buf_start, MBEDTLS_ASN1_INTEGER));

    return len;
}

int esp_mbedtls_ecdsa_raw_to_der(size_t bits, const unsigned char *raw, size_t raw_len,
                             unsigned char *der, size_t der_size, size_t *der_len)
{
     ...
     * first and then r. */
    ret = esp_convert_raw_to_der_single_int(s, coordinate_len, der, p);
    if (ret < 0) {
        return ret;
    }
    p -= ret;
    ...
    ret = esp_convert_raw_to_der_single_int(r, coordinate_len, der, p);
    if (ret < 0) {
        return ret;
    }
    p -= ret;
    ...
    MBEDTLS_ASN1_CHK_ADD(len, esp_mbedtls_asn1_write_len(&p, der, len));
    MBEDTLS_ASN1_CHK_ADD(len, esp_mbedtls_asn1_write_tag(&p, der,
 ...
 */
static int esp_convert_der_to_raw_single_int(unsigned char *der, size_t der_len,
                                         unsigned char *raw, size_t coordinate_size)
{
    ...
    ret = esp_mbedtls_asn1_get_tag(&p, p + der_len, &unpadded_len,
...
int esp_mbedtls_ecdsa_der_to_raw(size_t bits, const unsigned char *der, size_t der_len,
                             unsigned char *raw, size_t raw_size, size_t *raw_len)
{
    ...
    ret = esp_mbedtls_asn1_get_tag(&p, der + der_len, &data_len,
    ...
    /* Extract r */
    ret = esp_convert_der_to_raw_single_int(p, data_len, raw_tmp, coordinate_size);
    if (ret < 0) {
        return ret;
    }
    p += ret;
