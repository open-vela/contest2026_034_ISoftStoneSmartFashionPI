...
static const uint8_t esp_iota_r_packed[24] = {
...
static void esp_keccak_f1600(mbedtls_sha3_context *ctx)
{
    uint64_t lane[5];
    ...
    for (int round = 0; round < 24; round++) {
        ...
        s[0] ^= ((esp_iota_r_packed[round] & 0x40ull) << 57 |
                 (esp_iota_r_packed[round] & 0x20ull) << 26 |
                 (esp_iota_r_packed[round] & 0x10ull) << 11 |
                 (esp_iota_r_packed[round] & 0x8f));
    }
}

void esp_mbedtls_sha3_init(mbedtls_sha3_context *ctx)
{
    memset(ctx, 0, sizeof(mbedtls_sha3_context));
}

void esp_mbedtls_sha3_free(mbedtls_sha3_context *ctx)
{
    if (ctx == NULL) {
        return;
    }

    esp_mbedtls_platform_zeroize(ctx, sizeof(mbedtls_sha3_context));
}

void esp_mbedtls_sha3_clone(mbedtls_sha3_context *dst,
 ...
 * SHA-3 context setup
 */
int esp_mbedtls_sha3_starts(mbedtls_sha3_context *ctx, mbedtls_sha3_id id)
{
    switch (id) {
 ...
 */
int esp_mbedtls_sha3_update(mbedtls_sha3_context *ctx,
                        const uint8_t *input,
                        size_t ilen)
{
    if (ilen >= 8) {
...
int esp_mbedtls_sha3_finish(mbedtls_sha3_context *ctx,
 ...
 */
int esp_mbedtls_sha3(mbedtls_sha3_id id, const uint8_t *input,
                 size_t ilen, uint8_t *output, size_t olen)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    mbedtls_sha3_context ctx;

    esp_mbedtls_sha3_init(&ctx);

    /* Sanity checks are performed in every mbedtls_sha3_xxx() */
    if ((ret = esp_mbedtls_sha3_starts(&ctx, id)) != 0) {
        goto exit;
    }

    if ((ret = esp_mbedtls_sha3_update(&ctx, input, ilen)) != 0) {
        goto exit;
    }

    if ((ret = esp_mbedtls_sha3_finish(&ctx, output, olen)) != 0) {
        goto exit;
    }
...
static const unsigned char esp_test_data[2][4] =
{
    "",
    "abc",
};

static const size_t esp_test_data_len[2] =
{
    0, /* "" */
    3  /* "abc" */
};

static const unsigned char esp_test_hash_sha3_224[2][28] =
{
    { /* "" */
...
static const unsigned char esp_test_hash_sha3_256[2][32] =
{
    { /* "" */
...
static const unsigned char esp_test_hash_sha3_384[2][48] =
{
    { /* "" */
...
static const unsigned char esp_test_hash_sha3_512[2][64] =
{
    { /* "" */
...
static const unsigned char esp_long_kat_hash_sha3_224[28] =
{
...
static const unsigned char esp_long_kat_hash_sha3_256[32] =
{
...
static const unsigned char esp_long_kat_hash_sha3_384[48] =
{
...
static const unsigned char esp_long_kat_hash_sha3_512[64] =
{
...
static int esp_mbedtls_sha3_kat_test(int verbose,
                                 ...
                                 int test_num)
{
    ...
    result = esp_mbedtls_sha3(id,
                          esp_test_data[test_num], esp_test_data_len[test_num],
                          hash, sizeof(hash));
    if (result != 0) {
        if (verbose != 0) {
            esp_mbedtls_printf("  %s test %d error code: %d\n",
    ...
    switch (id) {
        case MBEDTLS_SHA3_224:
            result = memcmp(hash, esp_test_hash_sha3_224[test_num], 28);
            break;
        case MBEDTLS_SHA3_256:
            result = memcmp(hash, esp_test_hash_sha3_256[test_num], 32);
            break;
        case MBEDTLS_SHA3_384:
            result = memcmp(hash, esp_test_hash_sha3_384[test_num], 48);
            break;
        case MBEDTLS_SHA3_512:
            result = memcmp(hash, esp_test_hash_sha3_512[test_num], 64);
            break;
        default:
            break;
    }
    ...
    if (0 != result) {
        if (verbose != 0) {
            esp_mbedtls_printf("  %s test %d failed\n", type_name, test_num);
        }
    ...
    if (verbose != 0) {
        esp_mbedtls_printf("  %s test %d passed\n", type_name, test_num);
    }

    return 0;
}

static int esp_mbedtls_sha3_long_kat_test(int verbose,
                                      const char *type_name,
                                      mbedtls_sha3_id id)
{
    mbedtls_sha3_context ctx;
    ...
    if (verbose != 0) {
        esp_mbedtls_printf("  %s long KAT test ", type_name);
    }

    esp_mbedtls_sha3_init(&ctx);

    result = esp_mbedtls_sha3_starts(&ctx, id);
    if (result != 0) {
        if (verbose != 0) {
            esp_mbedtls_printf("setup failed\n ");
        }
    }

    /* Process 1,000,000 (one million) 'a' characters */
    for (int i = 0; i < 1000; i++) {
        result = esp_mbedtls_sha3_update(&ctx, buffer, 1000);
        if (result != 0) {
            if (verbose != 0) {
                esp_mbedtls_printf("update error code: %i\n", result);
            }
    ...
    result = esp_mbedtls_sha3_finish(&ctx, hash, sizeof(hash));
    if (result != 0) {
        if (verbose != 0) {
            esp_mbedtls_printf("finish error code: %d\n", result);
        }
    ...
    switch (id) {
        case MBEDTLS_SHA3_224:
            result = memcmp(hash, esp_long_kat_hash_sha3_224, 28);
            break;
        case MBEDTLS_SHA3_256:
            result = memcmp(hash, esp_long_kat_hash_sha3_256, 32);
            break;
        case MBEDTLS_SHA3_384:
            result = memcmp(hash, esp_long_kat_hash_sha3_384, 48);
            break;
        case MBEDTLS_SHA3_512:
            result = memcmp(hash, esp_long_kat_hash_sha3_512, 64);
            break;
        default:
            break;
    }
...
int esp_mbedtls_sha3_self_test(int verbose)
{
    int i;

    /* SHA-3 Known Answer Tests (KAT) */
    for (i = 0; i < 2; i++) {
        if (0 != esp_mbedtls_sha3_kat_test(verbose,
    ...
    if (0 != esp_mbedtls_sha3_long_kat_test(verbose,
