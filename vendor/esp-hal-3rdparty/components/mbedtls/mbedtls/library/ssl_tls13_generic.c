...
const uint8_t esp_mbedtls_ssl_tls13_hello_retry_request_magic[
...
/*
 * The esp_ssl_tls13_create_verify_structure() creates the verify structure.
 ...
 */
static void esp_ssl_tls13_create_verify_structure(const unsigned char *transcript_hash,
...
static int esp_ssl_tls13_parse_certificate_verify(mbedtls_ssl_context *ssl,
                                              ...
                                              size_t verify_buffer_len)
{
    ...
    hash_alg = esp_mbedtls_md_psa_alg_from_type(md_alg);
    if (hash_alg == 0) {
        goto error;
    }
     ...
     */
    if (!esp_mbedtls_pk_can_do(&ssl->session_negotiate->peer_cert->pk, sig_alg)) {
    ...
    if ((ret = esp_mbedtls_pk_verify_ext(sig_alg, options,
                                     &ssl->session_negotiate->peer_cert->pk,
                                     md_alg, verify_hash, verify_hash_len,
                                     p, signature_len)) == 0) {
        return 0;
    }
    MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_pk_verify_ext", ret);
...
int esp_mbedtls_ssl_tls13_process_certificate_verify(mbedtls_ssl_context *ssl)
{
    ...
    MBEDTLS_SSL_PROC_CHK(
        esp_mbedtls_ssl_tls13_fetch_handshake_msg(
    ...
    esp_ssl_tls13_create_verify_structure(transcript,
                                      transcript_len,
                                      verify_buffer,
                                      &verify_buffer_len,
    ...
    MBEDTLS_SSL_PROC_CHK(esp_ssl_tls13_parse_certificate_verify(
                             ssl, buf, buf + buf_len,
...
cleanup:
    ...
    MBEDTLS_SSL_DEBUG_MSG(2, ("<= parse certificate verify"));
    MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_tls13_process_certificate_verify", ret);
    return ret;
#else
    ((void) ssl);
...
int esp_mbedtls_ssl_tls13_parse_certificate(mbedtls_ssl_context *ssl,
                                        const unsigned char *buf,
                                        const unsigned char *end)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    if (ssl->session_negotiate->peer_cert != NULL) {
        esp_mbedtls_x509_crt_free(ssl->session_negotiate->peer_cert);
        esp_mbedtls_free(ssl->session_negotiate->peer_cert);
    }
    ...
    if ((ssl->session_negotiate->peer_cert =
             esp_mbedtls_calloc(1, sizeof(mbedtls_x509_crt))) == NULL) {
    ...
    esp_mbedtls_x509_crt_init(ssl->session_negotiate->peer_cert);
    ...
    while (p < certificate_list_end) {
        ...
        ret = esp_mbedtls_x509_crt_parse_der(ssl->session_negotiate->peer_cert,
                                         p, cert_data_len);
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_validate_certificate(mbedtls_ssl_context *ssl)
{
    int ret = 0;
     ...
     */
    ret = esp_mbedtls_x509_crt_verify_with_profile(
        ssl->session_negotiate->peer_cert,
        ca_chain, ca_crl,
     ...
     * esp_mbedtls_ssl_tls13_parse_certificate even if verification was optional.
     */
    if (authmode == MBEDTLS_SSL_VERIFY_OPTIONAL &&
...
int esp_mbedtls_ssl_tls13_process_certificate(mbedtls_ssl_context *ssl)
{
    ...
    MBEDTLS_SSL_PROC_CHK(esp_mbedtls_ssl_tls13_parse_certificate(ssl, buf,
                                                             buf + buf_len));
    /* Validate the certificate chain and set the verification results. */
    MBEDTLS_SSL_PROC_CHK(esp_ssl_tls13_validate_certificate(ssl));
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_write_certificate_body(mbedtls_ssl_context *ssl,
                                            unsigned char *buf,
                                            unsigned char *end,
                                            size_t *out_len)
{
...
int esp_mbedtls_ssl_tls13_write_certificate(mbedtls_ssl_context *ssl)
{
    ...
    MBEDTLS_SSL_PROC_CHK(esp_ssl_tls13_write_certificate_body(ssl,
                                                          buf,
                                                          buf + buf_len,
                                                          &msg_len));
...
{
    mbedtls_pk_type_t pk_type = (mbedtls_pk_type_t) esp_mbedtls_ssl_sig_from_pk(key);
    size_t key_size = esp_mbedtls_pk_get_bitlen(key);
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_write_certificate_verify_body(mbedtls_ssl_context *ssl,
                                                   unsigned char *buf,
                                                   unsigned char *end,
                                                   size_t *out_len)
{
    ...
    *out_len = 0;
    ...
    esp_ssl_tls13_create_verify_structure(handshake_hash, handshake_hash_len,
                                      verify_buffer, &verify_buffer_len,
                                      ssl->conf->endpoint);
    ...
    for (; *sig_alg != MBEDTLS_TLS1_3_SIG_NONE; sig_alg++) {
        ...
        if (!esp_mbedtls_ssl_tls13_check_sig_alg_cert_key_match(*sig_alg, own_key)) {
            continue;
        }
        ...
        psa_algorithm = esp_mbedtls_md_psa_alg_from_type(md_alg);
        status = esp_psa_hash_compute(psa_algorithm,
                                  verify_buffer,
                                  verify_buffer_len,
        ...
        if ((ret = esp_mbedtls_pk_sign_ext(pk_type, own_key,
                                       md_alg, verify_hash, verify_hash_len,
            ...
            MBEDTLS_SSL_DEBUG_RET(2, "esp_mbedtls_pk_sign_ext", ret);
...
int esp_mbedtls_ssl_tls13_write_certificate_verify(mbedtls_ssl_context *ssl)
{
    ...
    MBEDTLS_SSL_PROC_CHK(esp_ssl_tls13_write_certificate_verify_body(
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_preprocess_finished_message(mbedtls_ssl_context *ssl)
{
    int ret;

    ret = esp_mbedtls_ssl_tls13_calculate_verify_data(
        ssl,
    ...
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_tls13_calculate_verify_data", ret);
        return ret;
    }
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_parse_finished_message(mbedtls_ssl_context *ssl,
                                            const unsigned char *buf,
                                            const unsigned char *end)
{
    /*
...
int esp_mbedtls_ssl_tls13_process_finished_message(mbedtls_ssl_context *ssl)
{
    ...
    MBEDTLS_SSL_PROC_CHK(esp_ssl_tls13_preprocess_finished_message(ssl));

    MBEDTLS_SSL_PROC_CHK(esp_ssl_tls13_parse_finished_message(
                             ssl, buf, buf + buf_len));
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_prepare_finished_message(mbedtls_ssl_context *ssl)
{
    int ret;

    /* Compute transcript of handshake up to now. */
    ret = esp_mbedtls_ssl_tls13_calculate_verify_data(ssl,
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_write_finished_message_body(mbedtls_ssl_context *ssl,
                                                 unsigned char *buf,
                                                 unsigned char *end,
                                                 size_t *out_len)
{
...
int esp_mbedtls_ssl_tls13_write_finished_message(mbedtls_ssl_context *ssl)
{
    ...
    MBEDTLS_SSL_PROC_CHK(esp_ssl_tls13_prepare_finished_message(ssl));

    MBEDTLS_SSL_PROC_CHK(esp_mbedtls_ssl_start_handshake_msg(ssl,
                                                         MBEDTLS_SSL_HS_FINISHED, &buf, &buf_len));

    MBEDTLS_SSL_PROC_CHK(esp_ssl_tls13_write_finished_message_body(
                             ssl, buf, buf + buf_len, &msg_len));

    MBEDTLS_SSL_PROC_CHK(esp_mbedtls_ssl_add_hs_msg_to_checksum(ssl,
...
void esp_mbedtls_ssl_tls13_handshake_wrapup(mbedtls_ssl_context *ssl)
{
    ...
    esp_mbedtls_ssl_set_inbound_transform(ssl, ssl->transform_application);

    MBEDTLS_SSL_DEBUG_MSG(1, ("Switch to application keys for outbound traffic"));
    esp_mbedtls_ssl_set_outbound_transform(ssl, ssl->transform_application);
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_write_change_cipher_spec_body(mbedtls_ssl_context *ssl,
                                                   unsigned char *buf,
                                                   unsigned char *end,
                                                   size_t *olen)
{
...
int esp_mbedtls_ssl_tls13_write_change_cipher_spec(mbedtls_ssl_context *ssl)
{
    ...
    /* Write CCS message */
    MBEDTLS_SSL_PROC_CHK(esp_ssl_tls13_write_change_cipher_spec_body(
                             ssl, ssl->out_msg,
    ...
    /* Dispatch message */
    MBEDTLS_SSL_PROC_CHK(esp_mbedtls_ssl_write_record(ssl, 0));
...
int esp_mbedtls_ssl_reset_transcript_for_hrr(mbedtls_ssl_context *ssl)
{
    ...
    ret = esp_mbedtls_ssl_get_handshake_transcript(ssl, (mbedtls_md_type_t) ciphersuite_info->mac,
                                               hash_transcript + 4,
                                               PSA_HASH_MAX_SIZE,
                                               &hash_len);
    if (ret != 0) {
...
static psa_status_t  esp_mbedtls_ssl_get_psa_ffdh_info_from_tls_id(
...
#endif
#if defined(PSA_WANT_ALG_FFDH)
    if (esp_mbedtls_ssl_get_psa_ffdh_info_from_tls_id(named_group, &bits,
...
{
    uint32_t extension_mask = esp_mbedtls_ssl_get_extension_mask(
                                                  ...
                                                  size_t *out_len)
{
    ...
    *out_len = 6;
    ...
    esp_mbedtls_ssl_tls13_set_hs_sent_ext_mask(ssl, MBEDTLS_TLS_EXT_RECORD_SIZE_LIMIT);
