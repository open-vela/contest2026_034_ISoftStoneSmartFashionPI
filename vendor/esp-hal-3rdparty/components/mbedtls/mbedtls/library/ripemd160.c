 ...
 *  http://homes.esat.kuleuven.be/~bosselae/esp_mbedtls_ripemd160.html
...
void esp_mbedtls_ripemd160_init(mbedtls_ripemd160_context *ctx)
{
    memset(ctx, 0, sizeof(mbedtls_ripemd160_context));
}

void esp_mbedtls_ripemd160_free(mbedtls_ripemd160_context *ctx)
{
    if (ctx == NULL) {
        return;
    }

    esp_mbedtls_platform_zeroize(ctx, sizeof(mbedtls_ripemd160_context));
}

void esp_mbedtls_ripemd160_clone(mbedtls_ripemd160_context *dst,
 ...
 */
int esp_mbedtls_ripemd160_starts(mbedtls_ripemd160_context *ctx)
{
    ctx->total[0] = 0;
    ctx->total[1] = 0;
 ...
 * Process one block
 */
int esp_mbedtls_internal_ripemd160_process(mbedtls_ripemd160_context *ctx,
...
#define P2(a, b, c, d, e, r, s, rp, sp)                               \
    do                                                                  \
    {                                                                   \
        P((a), (b), (c), (d), (e), (r), (s), F, esp_K);                   \
        P(a ## p, b ## p, c ## p, d ## p, e ## p,                      \
          (rp), (sp), Fp, Kp);                                        \
    } while (0)

#define F   F1
#define esp_K   0x00000000
#define Fp  F5
 ...
 */
int esp_mbedtls_ripemd160_update(mbedtls_ripemd160_context *ctx,
                             ...
                             size_t ilen)
{
    ...
    if (left && ilen >= fill) {
        ...
        if ((ret = esp_mbedtls_internal_ripemd160_process(ctx, ctx->buffer)) != 0) {
            return ret;
        }
    ...
    while (ilen >= 64) {
        if ((ret = esp_mbedtls_internal_ripemd160_process(ctx, input)) != 0) {
            return ret;
        }
...
static const unsigned char esp_ripemd160_padding[64] =
{
 ...
 */
int esp_mbedtls_ripemd160_finish(mbedtls_ripemd160_context *ctx,
                             unsigned char output[20])
{
    ...
    ret = esp_mbedtls_ripemd160_update(ctx, esp_ripemd160_padding, padn);
    if (ret != 0) {
        goto exit;
    }

    ret = esp_mbedtls_ripemd160_update(ctx, msglen, 8);
    if (ret != 0) {
        goto exit;
    }
...
exit:
    esp_mbedtls_ripemd160_free(ctx);
    return ret;
}
 ...
 */
int esp_mbedtls_ripemd160(const unsigned char *input,
                      size_t ilen,
                      unsigned char output[20])
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    esp_mbedtls_ripemd160_init(&ctx);

    if ((ret = esp_mbedtls_ripemd160_starts(&ctx)) != 0) {
        goto exit;
    }

    if ((ret = esp_mbedtls_ripemd160_update(&ctx, input, ilen)) != 0) {
        goto exit;
    }

    if ((ret = esp_mbedtls_ripemd160_finish(&ctx, output)) != 0) {
        goto exit;
    }
 ...
 * http://homes.esat.kuleuven.be/~bosselae/esp_mbedtls_ripemd160.html#HMAC
 */
#define TESTS   8
static const unsigned char esp_ripemd160_test_str[TESTS][81] =
{
    { "" },
    { "a" },
    { "abc" },
...
static const size_t esp_ripemd160_test_strlen[TESTS] =
{
    0, 1, 3, 14, 26, 56, 62, 80
};

static const unsigned char esp_ripemd160_test_md[TESTS][20] =
{
 ...
 * Checkup routine
 */
int esp_mbedtls_ripemd160_self_test(int verbose)
{
    int i, ret = 0;
    unsigned char output[20];
    ...
    for (i = 0; i < TESTS; i++) {
        if (verbose != 0) {
            esp_mbedtls_printf("  RIPEMD-160 test #%d: ", i + 1);
        }

        ret = esp_mbedtls_ripemd160(esp_ripemd160_test_str[i],
                                esp_ripemd160_test_strlen[i], output);
        if (ret != 0) {
            goto fail;
        }

        if (memcmp(output, esp_ripemd160_test_md[i], 20) != 0) {
            ret = 1;
            goto fail;
        }
