...
static const unsigned char esp_D_LEAF_CONSTANT_BYTES[D_CONST_LEN] = { 0x82, 0x82 };
static const unsigned char esp_D_INTR_CONSTANT_BYTES[D_CONST_LEN] = { 0x83, 0x83 };
 ...
 */
static int esp_create_merkle_leaf_value(const mbedtls_lms_parameters_t *params,
                                    ...
                                    unsigned char *out)
{
    ...
    status = esp_psa_hash_update(&op, r_node_idx_bytes, 4);
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    status = esp_psa_hash_update(&op, esp_D_LEAF_CONSTANT_BYTES, D_CONST_LEN);
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    status = esp_psa_hash_update(&op, pub_key,
                             MBEDTLS_LMOTS_N_HASH_LEN(params->otstype));
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    status = esp_psa_hash_finish(&op, out, MBEDTLS_LMS_M_NODE_BYTES(params->type),
                             &output_hash_len);
    if (status != PSA_SUCCESS) {
        goto exit;
    }
 ...
 *  out                 The output node value, which is a hash output.
 */
static int esp_create_merkle_internal_value(const mbedtls_lms_parameters_t *params,
                                        ...
                                        unsigned char *out)
{
    ...
    status = esp_psa_hash_update(&op, esp_D_INTR_CONSTANT_BYTES, D_CONST_LEN);
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    status = esp_psa_hash_update(&op, left_node,
                             MBEDTLS_LMS_M_NODE_BYTES(params->type));
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    status = esp_psa_hash_update(&op, right_node,
                             MBEDTLS_LMS_M_NODE_BYTES(params->type));
    if (status != PSA_SUCCESS) {
        goto exit;
    }
...
void esp_mbedtls_lms_public_init(mbedtls_lms_public_t *ctx)
{
    memset(ctx, 0, sizeof(*ctx));
}

void esp_mbedtls_lms_public_free(mbedtls_lms_public_t *ctx)
{
    esp_mbedtls_platform_zeroize(ctx, sizeof(*ctx));
}

int esp_mbedtls_lms_import_public_key(mbedtls_lms_public_t *ctx,
...
int esp_mbedtls_lms_export_public_key(const mbedtls_lms_public_t *ctx,
                                  unsigned char *key,
                                  size_t key_size, size_t *esp_key_len)
{
    if (key_size < MBEDTLS_LMS_PUBLIC_KEY_LEN(ctx->params.type)) {
    ...
    if (esp_key_len != NULL) {
        *esp_key_len = MBEDTLS_LMS_PUBLIC_KEY_LEN(ctx->params.type);
    }

    return 0;
}

int esp_mbedtls_lms_verify(const mbedtls_lms_public_t *ctx,
                       ...
                       const unsigned char *sig, size_t sig_size)
{
    ...
    ret = esp_mbedtls_lmots_calculate_public_key_candidate(&ots_params,
                                                       msg,
                                                       msg_size,
    ...
    for (height = 0; height < MBEDTLS_LMS_H_TREE_HEIGHT(ctx->params.type);
        ...
        esp_create_merkle_internal_value(&ctx->params, left_node, right_node,
 ...
 */
static int esp_calculate_merkle_tree(const mbedtls_lms_private_t *ctx,
                                 unsigned char *tree)
{
    ...
    for (priv_key_idx = 0;
        ...
        ret = esp_create_merkle_leaf_value(&ctx->params,
    ...
    for (r_node_idx = MERKLE_TREE_INTERNAL_NODE_AM(ctx->params.type) - 1;
         r_node_idx > 0;
         r_node_idx--) {
        ret = esp_create_merkle_internal_value(&ctx->params,
                                           &tree[(r_node_idx * 2) *
                                                 MBEDTLS_LMS_M_NODE_BYTES(ctx->params.type)],
                                           &tree[(r_node_idx * 2 + 1) *
                                                 MBEDTLS_LMS_M_NODE_BYTES(ctx->params.type)],
 ...
 */
static int esp_get_merkle_path(mbedtls_lms_private_t *ctx,
                           ...
                           unsigned char *path)
{
    ...
    tree = esp_mbedtls_calloc((size_t) MERKLE_TREE_NODE_AM(ctx->params.type),
                          node_bytes);
    if (tree == NULL) {
        return MBEDTLS_ERR_LMS_ALLOC_FAILED;
    }

    ret = esp_calculate_merkle_tree(ctx, tree);
    if (ret != 0) {
        goto exit;
    }
...
exit:
    esp_mbedtls_zeroize_and_free(tree, node_bytes *
                             (size_t) MERKLE_TREE_NODE_AM(ctx->params.type));

    return ret;
}

void esp_mbedtls_lms_private_init(mbedtls_lms_private_t *ctx)
{
    memset(ctx, 0, sizeof(*ctx));
}

void esp_mbedtls_lms_private_free(mbedtls_lms_private_t *ctx)
{
    unsigned int idx;
    ...
    if (ctx->have_private_key) {
        if (ctx->ots_private_keys != NULL) {
            for (idx = 0; idx < MERKLE_TREE_LEAF_NODE_AM(ctx->params.type); idx++) {
                esp_mbedtls_lmots_private_free(&ctx->ots_private_keys[idx]);
            }
        }

        if (ctx->ots_public_keys != NULL) {
            for (idx = 0; idx < MERKLE_TREE_LEAF_NODE_AM(ctx->params.type); idx++) {
                esp_mbedtls_lmots_public_free(&ctx->ots_public_keys[idx]);
            }
        }

        esp_mbedtls_free(ctx->ots_private_keys);
        esp_mbedtls_free(ctx->ots_public_keys);
    }
...
int esp_mbedtls_lms_generate_private_key(mbedtls_lms_private_t *ctx,
                                     mbedtls_lms_algorithm_type_t type,
                                     mbedtls_lmots_algorithm_type_t otstype,
                                     ...
                                     size_t seed_size)
{
    ...
    ctx->ots_private_keys = esp_mbedtls_calloc((size_t) MERKLE_TREE_LEAF_NODE_AM(ctx->params.type),
    ...
    /* Requires a cast to size_t to avoid an implicit cast warning on certain
     * platforms (particularly Windows) */
    ctx->ots_public_keys = esp_mbedtls_calloc((size_t) MERKLE_TREE_LEAF_NODE_AM(ctx->params.type),
    ...
    for (idx = 0; idx < MERKLE_TREE_LEAF_NODE_AM(ctx->params.type); idx++) {
        esp_mbedtls_lmots_private_init(&ctx->ots_private_keys[idx]);
        esp_mbedtls_lmots_public_init(&ctx->ots_public_keys[idx]);
    }


    for (idx = 0; idx < MERKLE_TREE_LEAF_NODE_AM(ctx->params.type); idx++) {
        ret = esp_mbedtls_lmots_generate_private_key(&ctx->ots_private_keys[idx],
                                                 otstype,
                                                 ctx->params.I_key_identifier,
                                                 idx, seed, seed_size);
        if (ret != 0) {
        ...
        ret = esp_mbedtls_lmots_calculate_public_key(&ctx->ots_public_keys[idx],
...
exit:
    if (ret != 0) {
        esp_mbedtls_lms_private_free(ctx);
    }

    return ret;
}

int esp_mbedtls_lms_calculate_public_key(mbedtls_lms_public_t *ctx,
                                     const mbedtls_lms_private_t *priv_ctx)
{
    ...
    tree = esp_mbedtls_calloc((size_t) MERKLE_TREE_NODE_AM(priv_ctx->params.type),
                          node_bytes);
    if (tree == NULL) {
        return MBEDTLS_ERR_LMS_ALLOC_FAILED;
    }
    ...
    ret = esp_calculate_merkle_tree(priv_ctx, tree);
    if (ret != 0) {
        goto exit;
    }
...
int esp_mbedtls_lms_sign(mbedtls_lms_private_t *ctx,
                     int (*f_rng)(void *, unsigned char *, size_t),
                     ...
                     size_t *sig_len)
{
    ...
    ret = esp_mbedtls_lmots_sign(&ctx->ots_private_keys[q_leaf_identifier],
                             f_rng,
                             p_rng,
                             msg,
                             msg_size,
