...
{
    return esp_psa_status_to_mbedtls(status, esp_psa_to_lms_errors,
                                 ARRAY_LENGTH(esp_psa_to_lms_errors),
...
#define D_CONST_LEN            (2)
static const unsigned char esp_D_PUBLIC_CONSTANT_BYTES[D_CONST_LEN] = { 0x80, 0x80 };
static const unsigned char esp_D_MESSAGE_CONSTANT_BYTES[D_CONST_LEN] = { 0x81, 0x81 };

#if defined(MBEDTLS_TEST_HOOKS)
int (*esp_mbedtls_lmots_sign_private_key_invalidated_hook)(unsigned char *) = NULL;
 ...
 */
static unsigned short esp_lmots_checksum_calculate(const mbedtls_lmots_parameters_t *params,
 ...
 *                      size P bytes).
 */
static int esp_create_digit_array_with_checksum(const mbedtls_lmots_parameters_t *params,
                                            ...
                                            unsigned char *out)
{
    ...
    status = esp_psa_hash_setup(&op, PSA_ALG_SHA_256);
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    status = esp_psa_hash_update(&op, params->I_key_identifier,
                             MBEDTLS_LMOTS_I_KEY_ID_LEN);
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    status = esp_psa_hash_update(&op, params->q_leaf_identifier,
                             MBEDTLS_LMOTS_Q_LEAF_ID_LEN);
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    status = esp_psa_hash_update(&op, esp_D_MESSAGE_CONSTANT_BYTES, D_CONST_LEN);
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    status = esp_psa_hash_update(&op, C_random_value,
                             MBEDTLS_LMOTS_C_RANDOM_VALUE_LEN(params->type));
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    status = esp_psa_hash_update(&op, msg, msg_len);
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    status = esp_psa_hash_finish(&op, out,
 ...
 *                      34.
 */
static int esp_hash_digit_array(const mbedtls_lmots_parameters_t *params,
                            ...
                            unsigned char *output)
{
    ...
    for (i_digit_idx = 0;
        ...
        for (j_hash_idx = j_hash_idx_min;
            ...
            status = esp_psa_hash_update(&op, i_digit_idx_bytes, I_DIGIT_IDX_LEN);
            if (status != PSA_SUCCESS) {
                goto exit;
            }

            j_hash_idx_bytes[0] = (uint8_t) j_hash_idx;
            status = esp_psa_hash_update(&op, j_hash_idx_bytes, J_HASH_IDX_LEN);
            if (status != PSA_SUCCESS) {
                goto exit;
            }

            status = esp_psa_hash_update(&op, tmp_hash,
                                     MBEDTLS_LMOTS_N_HASH_LEN(params->type));
            if (status != PSA_SUCCESS) {
                goto exit;
            }

            status = esp_psa_hash_finish(&op, tmp_hash, sizeof(tmp_hash),
                                     &output_hash_len);
            if (status != PSA_SUCCESS) {
                goto exit;
            }
...
exit:
    esp_psa_hash_abort(&op);
    esp_mbedtls_platform_zeroize(tmp_hash, sizeof(tmp_hash));
 ...
 */
static int esp_public_key_from_hashed_digit_array(const mbedtls_lmots_parameters_t *params,
                                              ...
                                              unsigned char *pub_key)
{
    ...
    status = esp_psa_hash_update(&op, esp_D_PUBLIC_CONSTANT_BYTES, D_CONST_LEN);
    if (status != PSA_SUCCESS) {
        goto exit;
    }

    status = esp_psa_hash_update(&op, y_hashed_digits,
    ...
    status = esp_psa_hash_finish(&op, pub_key,
                             MBEDTLS_LMOTS_N_HASH_LEN(params->type),
                             &output_hash_len);
    if (status != PSA_SUCCESS) {
...
int esp_mbedtls_lms_error_from_psa(psa_status_t status)
{
    switch (status) {
        case PSA_SUCCESS:
            return 0;
...
void esp_mbedtls_lmots_public_init(mbedtls_lmots_public_t *ctx)
{
    memset(ctx, 0, sizeof(*ctx));
}

void esp_mbedtls_lmots_public_free(mbedtls_lmots_public_t *ctx)
{
    esp_mbedtls_platform_zeroize(ctx, sizeof(*ctx));
}

int esp_mbedtls_lmots_import_public_key(mbedtls_lmots_public_t *ctx,
                                    const unsigned char *key, size_t esp_key_len)
{
    if (esp_key_len < MBEDTLS_LMOTS_SIG_TYPE_OFFSET + MBEDTLS_LMOTS_TYPE_LEN) {
        return MBEDTLS_ERR_LMS_BAD_INPUT_DATA;
    }
    ...
    if (esp_key_len != MBEDTLS_LMOTS_PUBLIC_KEY_LEN(ctx->params.type)) {
        return MBEDTLS_ERR_LMS_BAD_INPUT_DATA;
    }
...
int esp_mbedtls_lmots_export_public_key(const mbedtls_lmots_public_t *ctx,
                                    ...
                                    size_t *esp_key_len)
{
    ...
    if (esp_key_len != NULL) {
        *esp_key_len = MBEDTLS_LMOTS_PUBLIC_KEY_LEN(ctx->params.type);
    }

    return 0;
}

int esp_mbedtls_lmots_calculate_public_key_candidate(const mbedtls_lmots_parameters_t *params,
                                                 const unsigned char  *msg,
                                                 size_t msg_size,
                                                 ...
                                                 size_t *out_len)
{
    ...
    ret = esp_create_digit_array_with_checksum(params, msg, msg_size,
    ...
    ret = esp_hash_digit_array(params,
    ...
    ret = esp_public_key_from_hashed_digit_array(params,
...
int esp_mbedtls_lmots_verify(const mbedtls_lmots_public_t *ctx,
                         ...
                         const unsigned char *sig, size_t sig_size)
{
    ...
    ret = esp_mbedtls_lmots_calculate_public_key_candidate(&ctx->params,
...
void esp_mbedtls_lmots_private_init(mbedtls_lmots_private_t *ctx)
{
    memset(ctx, 0, sizeof(*ctx));
}

void esp_mbedtls_lmots_private_free(mbedtls_lmots_private_t *ctx)
{
    esp_mbedtls_platform_zeroize(ctx,
                             sizeof(*ctx));
}

int esp_mbedtls_lmots_generate_private_key(mbedtls_lmots_private_t *ctx,
                                       ...
                                       size_t seed_size)
{
    ...
    for (i_digit_idx = 0;
        ...
        status = esp_psa_hash_update(&op, const_bytes, sizeof(const_bytes));
        if (status != PSA_SUCCESS) {
            goto exit;
        }

        status = esp_psa_hash_update(&op, seed, seed_size);
        if (status != PSA_SUCCESS) {
            goto exit;
        }

        status = esp_psa_hash_finish(&op,
...
int esp_mbedtls_lmots_calculate_public_key(mbedtls_lmots_public_t *ctx,
                                       const mbedtls_lmots_private_t *priv_ctx)
{
    ...
    ret = esp_hash_digit_array(&priv_ctx->params,
    ...
    ret = esp_public_key_from_hashed_digit_array(&priv_ctx->params,
                                             (unsigned char *) y_hashed_digits,
                                             ctx->public_key);
    if (ret) {
        goto exit;
...
exit:
    esp_mbedtls_platform_zeroize(y_hashed_digits, sizeof(y_hashed_digits));

    return ret;
}

int esp_mbedtls_lmots_sign(mbedtls_lmots_private_t *ctx,
                       ...
                       unsigned char *sig, size_t sig_size, size_t *sig_len)
{
    ...
    ret = esp_create_digit_array_with_checksum(&ctx->params,
                                           msg, msg_size,
                                           tmp_c_random,
                                           tmp_digit_array);
    if (ret) {
    ...
    ret = esp_hash_digit_array(&ctx->params, (unsigned char *) ctx->private_key,
     ...
     */
#if defined(MBEDTLS_TEST_HOOKS)
    if (esp_mbedtls_lmots_sign_private_key_invalidated_hook != NULL) {
        ret = (*esp_mbedtls_lmots_sign_private_key_invalidated_hook)(sig);
        if (ret != 0) {
            return ret;
        }
    }
    ...
    esp_mbedtls_platform_zeroize(ctx->private_key,
...
exit:
    esp_mbedtls_platform_zeroize(tmp_digit_array, sizeof(tmp_digit_array));
    esp_mbedtls_platform_zeroize(tmp_sig, sizeof(tmp_sig));
