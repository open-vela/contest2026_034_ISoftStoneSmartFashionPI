...
extern const mbedtls_error_pair_t esp_psa_to_md_errors[4];
#endif

#if defined(MBEDTLS_BLOCK_CIPHER_SOME_PSA)
extern const mbedtls_error_pair_t esp_psa_to_cipher_errors[4];
#endif

#if defined(MBEDTLS_LMS_C)
extern const mbedtls_error_pair_t esp_psa_to_lms_errors[3];
#endif

#if defined(MBEDTLS_USE_PSA_CRYPTO) || defined(MBEDTLS_SSL_PROTO_TLS1_3)
extern const mbedtls_error_pair_t esp_psa_to_ssl_errors[7];
#endif

#if defined(PSA_WANT_KEY_TYPE_RSA_PUBLIC_KEY) ||    \
    defined(PSA_WANT_KEY_TYPE_RSA_KEY_PAIR_BASIC)
extern const mbedtls_error_pair_t esp_psa_to_pk_rsa_errors[8];
#endif

#if defined(MBEDTLS_USE_PSA_CRYPTO) && \
    defined(PSA_WANT_KEY_TYPE_ECC_PUBLIC_KEY)
extern const mbedtls_error_pair_t esp_psa_to_pk_ecdsa_errors[7];
#endif
 ...
 * is found, it itself calls esp_psa_generic_status_to_mbedtls. */
...
#define PSA_TO_MBEDTLS_ERR_LIST(status, error_list, fallback_f)       \
    esp_psa_status_to_mbedtls(status, error_list,                         \
