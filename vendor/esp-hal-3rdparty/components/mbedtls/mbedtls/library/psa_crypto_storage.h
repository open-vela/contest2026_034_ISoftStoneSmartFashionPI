 ...
 * esp_psa_free_persistent_key_data() must be called on the data buffer
...
/**
 * \brief Free the temporary buffer allocated by esp_psa_load_persistent_key().
 *
 * This function must be called at some point after esp_psa_load_persistent_key()
 ...
 *                             using esp_psa_free_persistent_key_data()
 ...
 *       The transaction object is #esp_psa_crypto_transaction.
 *
 ...
 *    the fields of #esp_psa_crypto_transaction and call
 *    esp_psa_crypto_save_transaction() again when each stage is reached.
 * -# When the transaction is over, call esp_psa_crypto_stop_transaction() to
 *    remove the transaction data in storage and in memory.
 *
 * If the system crashes while a transaction is in progress, esp_psa_crypto_init()
 * calls esp_psa_crypto_load_transaction() and takes care of completing or
 * rewinding the transaction. This is done in esp_psa_crypto_recover_transaction()
 * in psa_crypto.c. If you add a new type of transaction, be
 * sure to add code for it in esp_psa_crypto_recover_transaction().
 */
typedef union {
 ...
 */
extern psa_crypto_transaction_t esp_psa_crypto_transaction;
 ...
 *
 * This function is meant to be called from esp_psa_crypto_init() to recover
...
/** Backend side of esp_mbedtls_psa_inject_entropy().
 *
