...
int esp_mbedtls_ct_hmac(mbedtls_svc_key_id_t key,
...
{
    /*
     * This function breaks the HMAC abstraction and uses esp_psa_hash_clone()
...
#define PSA_CHK(func_call)        \
     ...
     */
    PSA_CHK(esp_psa_export_key(key, key_buf, block_size, &mac_key_length));
    ...
    PSA_CHK(esp_psa_hash_setup(&operation, hash_alg));

    /* Now compute inner_hash = HASH(ikey + msg) */
    PSA_CHK(esp_psa_hash_update(&operation, key_buf, block_size));
    PSA_CHK(esp_psa_hash_update(&operation, add_data, add_data_len));
    PSA_CHK(esp_psa_hash_update(&operation, data, min_data_len));
    ...
    for (offset = min_data_len; offset <= max_data_len; offset++) {
        PSA_CHK(esp_psa_hash_clone(&operation, &aux_operation));
        PSA_CHK(esp_psa_hash_finish(&aux_operation, aux_out,
                                PSA_HASH_MAX_SIZE, &hash_length));
        /* Keep only the correct inner_hash in the output buffer */
        esp_mbedtls_ct_memcpy_if(esp_mbedtls_ct_uint_eq(offset, data_len_secret),
                             output, aux_out, NULL, hash_size);

        if (offset < max_data_len) {
            PSA_CHK(esp_psa_hash_update(&operation, data + offset, 1));
        }
    }

    /* Abort current operation to prepare for final operation */
    PSA_CHK(esp_psa_hash_abort(&operation));
    ...
    PSA_CHK(esp_psa_hash_setup(&operation, hash_alg));
    PSA_CHK(esp_psa_hash_update(&operation, key_buf, block_size));
    PSA_CHK(esp_psa_hash_update(&operation, output, hash_size));
    PSA_CHK(esp_psa_hash_finish(&operation, output, hash_size, &hash_length));

#undef PSA_CHK

cleanup:
    esp_mbedtls_platform_zeroize(key_buf, MAX_HASH_BLOCK_LENGTH);
    esp_mbedtls_platform_zeroize(aux_out, PSA_HASH_MAX_SIZE);

    esp_psa_hash_abort(&operation);
    esp_psa_hash_abort(&aux_operation);
...
#else
MBEDTLS_STATIC_TESTABLE
int esp_mbedtls_ct_hmac(mbedtls_md_context_t *ctx,
                    const unsigned char *add_data,
                    size_t add_data_len,
                    const unsigned char *data,
                    size_t data_len_secret,
                    ...
                    unsigned char *output)
{
     ...
     * Then we only need to compute HASH(okey + inner_hash) and we're done.
     */
    const mbedtls_md_type_t md_alg = esp_mbedtls_md_get_type(ctx->md_info);
    /* TLS 1.2 only supports SHA-384, SHA-256, SHA-1, MD-5,
     * all of which have the same block size except SHA-384. */
    const size_t block_size = md_alg == MBEDTLS_MD_SHA384 ? 128 : 64;
    const unsigned char * const ikey = ctx->hmac_ctx;
    const unsigned char * const okey = ikey + block_size;
    const size_t hash_size = esp_mbedtls_md_get_size(ctx->md_info);
...
#define MD_CHK(func_call) \
    ...
    MD_CHK(esp_mbedtls_md_setup(&aux, ctx->md_info, 0));

    /* After hmac_start() of hmac_reset(), ikey has already been hashed,
     * so we can start directly with the message */
    MD_CHK(esp_mbedtls_md_update(ctx, add_data, add_data_len));
    MD_CHK(esp_mbedtls_md_update(ctx, data, min_data_len));
    ...
    /* For each possible length, compute the hash up to that point */
    for (offset = min_data_len; offset <= max_data_len; offset++) {
        MD_CHK(esp_mbedtls_md_clone(&aux, ctx));
        MD_CHK(esp_mbedtls_md_finish(&aux, aux_out));
        /* Keep only the correct inner_hash in the output buffer */
        esp_mbedtls_ct_memcpy_if(esp_mbedtls_ct_uint_eq(offset, data_len_secret),
                             output, aux_out, NULL, hash_size);

        if (offset < max_data_len) {
            MD_CHK(esp_mbedtls_md_update(ctx, data + offset, 1));
        }
    }

    /* The context needs to finish() before it starts() again */
    MD_CHK(esp_mbedtls_md_finish(ctx, aux_out));

    /* Now compute HASH(okey + inner_hash) */
    MD_CHK(esp_mbedtls_md_starts(ctx));
    MD_CHK(esp_mbedtls_md_update(ctx, okey, block_size));
    MD_CHK(esp_mbedtls_md_update(ctx, output, hash_size));
    MD_CHK(esp_mbedtls_md_finish(ctx, output));

    /* Done, get ready for next time */
    MD_CHK(esp_mbedtls_md_hmac_reset(ctx));

#undef MD_CHK

cleanup:
    esp_mbedtls_md_free(&aux);
    return ret;
}
...
static uint32_t esp_ssl_get_hs_total_len(mbedtls_ssl_context const *ssl);

/*
 * Start a timer.
 * Passing millisecs = 0 cancels a running timer.
 */
void esp_mbedtls_ssl_set_timer(mbedtls_ssl_context *ssl, uint32_t millisecs)
{
 ...
 */
int esp_mbedtls_ssl_check_timer(mbedtls_ssl_context *ssl)
{
...
static int esp_ssl_parse_record_header(mbedtls_ssl_context const *ssl,
                                   unsigned char *buf,
                                   size_t len,
                                   mbedtls_record *rec);
...
{
    int ret = 0;
    MBEDTLS_SSL_DEBUG_MSG(1, ("=> esp_mbedtls_ssl_check_record"));
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_STREAM) {
        ...
        ret = esp_ssl_parse_record_header(ssl, buf, buflen, &rec);
        if (ret != 0) {
            MBEDTLS_SSL_DEBUG_RET(3, "esp_ssl_parse_record_header", ret);
            goto exit;
        }

        if (ssl->transform_in != NULL) {
            ret = esp_mbedtls_ssl_decrypt_buf(ssl, ssl->transform_in, &rec);
            if (ret != 0) {
                MBEDTLS_SSL_DEBUG_RET(3, "esp_mbedtls_ssl_decrypt_buf", ret);
                goto exit;
            }
        }
    }
...
exit:
    ...
    esp_mbedtls_platform_zeroize(buf, buflen);
...
static void esp_ssl_buffering_free_slot(mbedtls_ssl_context *ssl,
                                    uint8_t slot);
static void esp_ssl_free_buffered_record(mbedtls_ssl_context *ssl);
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_load_buffered_message(mbedtls_ssl_context *ssl);
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_load_buffered_record(mbedtls_ssl_context *ssl);
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_buffer_message(mbedtls_ssl_context *ssl);
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_buffer_future_record(mbedtls_ssl_context *ssl,
                                    mbedtls_record const *rec);
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_next_record_is_in_datagram(mbedtls_ssl_context *ssl);

static size_t esp_ssl_get_maximum_datagram_size(mbedtls_ssl_context const *ssl)
{
    size_t mtu = esp_mbedtls_ssl_get_current_mtu(ssl);
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_get_remaining_space_in_datagram(mbedtls_ssl_context const *ssl)
{
    size_t const bytes_written = ssl->out_left;
    size_t const mtu           = esp_ssl_get_maximum_datagram_size(ssl);
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_get_remaining_payload_in_datagram(mbedtls_ssl_context const *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    ret = esp_ssl_get_remaining_space_in_datagram(ssl);
    if (ret < 0) {
        return ret;
    }
    remaining = (size_t) ret;

    ret = esp_mbedtls_ssl_get_record_expansion(ssl);
    if (ret < 0) {
        return ret;
    }
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_double_retransmit_timeout(mbedtls_ssl_context *ssl)
{
...
static void esp_ssl_reset_retransmit_timeout(mbedtls_ssl_context *ssl)
{
...
static size_t esp_ssl_compute_padding_length(size_t len,
                                         size_t granularity)
{
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_build_inner_plaintext(unsigned char *content,
                                     size_t *content_size,
                                     size_t remaining,
                                     uint8_t rec_type,
                                     size_t pad)
 ...
 * See esp_ssl_build_inner_plaintext() for details. */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_parse_inner_plaintext(unsigned char const *content,
                                     size_t *content_size,
                                     uint8_t *rec_type)
{
 ...
 *
 */
static void esp_ssl_extract_add_data_from_record(unsigned char *add_data,
                                             size_t *add_data_len,
                                             mbedtls_record *rec,
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_transform_aead_dynamic_iv_is_explicit(
 ...
 */
static void esp_ssl_build_record_nonce(unsigned char *dst_iv,
                                   size_t dst_iv_len,
...
int esp_mbedtls_ssl_encrypt_buf(mbedtls_ssl_context *ssl,
                            ...
                            void *p_rng)
{
     ...
     * the description of esp_ssl_extract_add_data_from_record().
     */
#if defined(MBEDTLS_SSL_DTLS_CONNECTION_ID)
    ...
    ssl_mode = esp_mbedtls_ssl_get_mode_from_transform(transform);
     ...
     *
     * See esp_ssl_build_inner_plaintext() for more information.
     *
    ...
    if (transform->tls_version == MBEDTLS_SSL_VERSION_TLS1_3) {
        size_t padding =
            esp_ssl_compute_padding_length(rec->data_len,
                                       MBEDTLS_SSL_CID_TLS1_3_PADDING_GRANULARITY);
        if (esp_ssl_build_inner_plaintext(data,
                                      &rec->data_len,
                                      post_avail,
                                      rec->type,
                                      padding) != 0) {
    ...
    if (ssl_mode == MBEDTLS_SSL_MODE_STREAM ||
        ...
        esp_ssl_extract_add_data_from_record(add_data, &add_data_len, rec,
                                         transform->tls_version,
                                         transform->taglen);

#if defined(MBEDTLS_USE_PSA_CRYPTO)
        status = esp_psa_mac_sign_setup(&operation, transform->psa_mac_enc,
                                    transform->psa_mac_alg);
        if (status != PSA_SUCCESS) {
            goto hmac_failed_etm_disabled;
        }

        status = esp_psa_mac_update(&operation, add_data, add_data_len);
        if (status != PSA_SUCCESS) {
            goto hmac_failed_etm_disabled;
        }

        status = esp_psa_mac_update(&operation, data, rec->data_len);
        if (status != PSA_SUCCESS) {
            goto hmac_failed_etm_disabled;
        }

        status = esp_psa_mac_sign_finish(&operation, mac, MBEDTLS_SSL_MAC_ADD,
                                     &sign_mac_length);
        if (status != PSA_SUCCESS) {
            goto hmac_failed_etm_disabled;
        }
#else
        ret = esp_mbedtls_md_hmac_update(&transform->md_ctx_enc, add_data,
                                     add_data_len);
        if (ret != 0) {
            goto hmac_failed_etm_disabled;
        }
        ret = esp_mbedtls_md_hmac_update(&transform->md_ctx_enc, data, rec->data_len);
        if (ret != 0) {
            goto hmac_failed_etm_disabled;
        }
        ret = esp_mbedtls_md_hmac_finish(&transform->md_ctx_enc, mac);
        if (ret != 0) {
            goto hmac_failed_etm_disabled;
        }
        ret = esp_mbedtls_md_hmac_reset(&transform->md_ctx_enc);
        if (ret != 0) {
            goto hmac_failed_etm_disabled;
        }
#endif /* MBEDTLS_USE_PSA_CRYPTO */
...
hmac_failed_etm_disabled:
        esp_mbedtls_platform_zeroize(mac, transform->maclen);
#if defined(MBEDTLS_USE_PSA_CRYPTO)
        ret = PSA_TO_MBEDTLS_ERR(status);
        status = esp_psa_mac_abort(&operation);
    ...
    if (ssl_mode == MBEDTLS_SSL_MODE_AEAD) {
        ...
        int dynamic_iv_is_explicit =
            esp_ssl_transform_aead_dynamic_iv_is_explicit(transform);
#if defined(MBEDTLS_USE_PSA_CRYPTO)
        psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
#endif /* MBEDTLS_USE_PSA_CRYPTO */
        int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
        ...
        esp_ssl_build_record_nonce(iv, sizeof(iv),
                               transform->iv_enc,
         ...
         */
#if defined(MBEDTLS_USE_PSA_CRYPTO)
        status = esp_psa_aead_encrypt(transform->psa_key_enc,
                                  transform->psa_alg,
                                  iv, transform->ivlen,
                                  add_data, add_data_len,
                                  data, rec->data_len,
        ...
        if (status != PSA_SUCCESS) {
            ret = PSA_TO_MBEDTLS_ERR(status);
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_encrypt_buf", ret);
            return ret;
        }
#else
        if ((ret = esp_mbedtls_cipher_auth_encrypt_ext(&transform->cipher_ctx_enc,
                                                   iv, transform->ivlen,
                                                   add_data, add_data_len,
                                                   ...
                                                   &rec->data_len,
                                                   transform->taglen)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_cipher_auth_encrypt_ext", ret);
            return ret;
        }
#endif /* MBEDTLS_USE_PSA_CRYPTO */
    ...
    if (ssl_mode == MBEDTLS_SSL_MODE_CBC ||
...
#if defined(MBEDTLS_USE_PSA_CRYPTO)
        status = esp_psa_cipher_encrypt_setup(&cipher_op,
                                          transform->psa_key_enc, transform->psa_alg);

        if (status != PSA_SUCCESS) {
            ret = PSA_TO_MBEDTLS_ERR(status);
            MBEDTLS_SSL_DEBUG_RET(1, "esp_psa_cipher_encrypt_setup", ret);
            return ret;
        }

        status = esp_psa_cipher_set_iv(&cipher_op, transform->iv_enc, transform->ivlen);

        if (status != PSA_SUCCESS) {
            ret = PSA_TO_MBEDTLS_ERR(status);
            MBEDTLS_SSL_DEBUG_RET(1, "esp_psa_cipher_set_iv", ret);
            return ret;

        }

        status = esp_psa_cipher_update(&cipher_op,
                                   data, rec->data_len,
                                   data, rec->data_len, &olen);

        if (status != PSA_SUCCESS) {
            ret = PSA_TO_MBEDTLS_ERR(status);
            MBEDTLS_SSL_DEBUG_RET(1, "esp_psa_cipher_update", ret);
            return ret;

        }

        status = esp_psa_cipher_finish(&cipher_op,
                                   data + olen, rec->data_len - olen,
                                   &part_len);

        if (status != PSA_SUCCESS) {
            ret = PSA_TO_MBEDTLS_ERR(status);
            MBEDTLS_SSL_DEBUG_RET(1, "esp_psa_cipher_finish", ret);
            return ret;
        ...
        olen += part_len;
#else
        if ((ret = esp_mbedtls_cipher_crypt(&transform->cipher_ctx_enc,
                                        transform->iv_enc,
                                        transform->ivlen,
                                        data, rec->data_len,
                                        data, &olen)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_cipher_crypt", ret);
            return ret;
        }
#endif /* MBEDTLS_USE_PSA_CRYPTO */
        ...
        if (auth_done == 0) {
            ...
            esp_ssl_extract_add_data_from_record(add_data, &add_data_len,
            ...
            ret = esp_mbedtls_md_hmac_update(&transform->md_ctx_enc,
                                         data, rec->data_len);
            if (ret != 0) {
                goto hmac_failed_etm_enabled;
            }
...
int esp_mbedtls_ssl_decrypt_buf(mbedtls_ssl_context const *ssl,
                            mbedtls_ssl_transform *transform,
                            mbedtls_record *rec)
{
    ...
    if (ssl_mode == MBEDTLS_SSL_MODE_AEAD) {
         ...
         */
        dynamic_iv_len = sizeof(rec->ctr);
        if (esp_ssl_transform_aead_dynamic_iv_is_explicit(transform) == 1) {
         ...
         * esp_mbedtls_cipher_auth_decrypt_ext() below. */
         ...
         */
#if defined(MBEDTLS_USE_PSA_CRYPTO)
        status = esp_psa_aead_decrypt(transform->psa_key_dec,
                                  transform->psa_alg,
                                  iv, transform->ivlen,
                                  add_data, add_data_len,
                                  data, rec->data_len + transform->taglen,
        ...
        if (status != PSA_SUCCESS) {
            ret = PSA_TO_MBEDTLS_ERR(status);
            MBEDTLS_SSL_DEBUG_RET(1, "esp_psa_aead_decrypt", ret);
            return ret;
        }
#else
        if ((ret = esp_mbedtls_cipher_auth_decrypt_ext
                       ...
                       transform->taglen)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_cipher_auth_decrypt_ext", ret);
    ...
    if (ssl_mode == MBEDTLS_SSL_MODE_CBC ||
        ...
        if (ssl_mode == MBEDTLS_SSL_MODE_CBC_ETM) {
            ...
            MBEDTLS_SSL_DEBUG_BUF(4, "MAC'd meta-data", add_data,
                                  add_data_len);
#if defined(MBEDTLS_USE_PSA_CRYPTO)
            status = esp_psa_mac_verify_setup(&operation, transform->psa_mac_dec,
                                          transform->psa_mac_alg);
            if (status != PSA_SUCCESS) {
                goto hmac_failed_etm_enabled;
            }
            ...
            status = esp_psa_mac_verify_finish(&operation, data + rec->data_len,
                                           transform->maclen);
            if (status != PSA_SUCCESS) {
                goto hmac_failed_etm_enabled;
            }
#else
            ret = esp_mbedtls_md_hmac_update(&transform->md_ctx_dec, add_data,
                                         add_data_len);
            if (ret != 0) {
                goto hmac_failed_etm_enabled;
            }
            ret = esp_mbedtls_md_hmac_update(&transform->md_ctx_dec,
                                         data, rec->data_len);
            if (ret != 0) {
                goto hmac_failed_etm_enabled;
            }
            ret = esp_mbedtls_md_hmac_finish(&transform->md_ctx_dec, mac_expect);
            if (ret != 0) {
                goto hmac_failed_etm_enabled;
            }
            ret = esp_mbedtls_md_hmac_reset(&transform->md_ctx_dec);
            if (ret != 0) {
                goto hmac_failed_etm_enabled;
            }
            ...
            /* Compare expected MAC with MAC at the end of the record. */
            if (esp_mbedtls_ct_memcmp(data + rec->data_len, mac_expect,
                                  transform->maclen) != 0) {
...
hmac_failed_etm_enabled:
...
#else
            esp_mbedtls_platform_zeroize(mac_expect, transform->maclen);
#endif /* MBEDTLS_USE_PSA_CRYPTO */
            if (ret != 0) {
...
#if defined(MBEDTLS_USE_PSA_CRYPTO)
        status = esp_psa_cipher_decrypt_setup(&cipher_op,
                                          transform->psa_key_dec, transform->psa_alg);

        if (status != PSA_SUCCESS) {
            ret = PSA_TO_MBEDTLS_ERR(status);
            MBEDTLS_SSL_DEBUG_RET(1, "esp_psa_cipher_decrypt_setup", ret);
            return ret;
        }

        status = esp_psa_cipher_set_iv(&cipher_op, transform->iv_dec, transform->ivlen);
        ...
        if ((ret = esp_mbedtls_cipher_crypt(&transform->cipher_ctx_dec,
        ...
        if (auth_done == 1) {
            const mbedtls_ct_condition_t ge = esp_mbedtls_ct_uint_ge(
                rec->data_len,
                padlen + 1);
            correct = esp_mbedtls_ct_bool_and(ge, correct);
            padlen  = esp_mbedtls_ct_size_if_else_0(ge, padlen);
        } else {
        ...
        for (idx = start_idx; idx < rec->data_len; idx++) {
             ...
             *              (check[idx] == padlen - 1);
             */
            const mbedtls_ct_condition_t a = esp_mbedtls_ct_uint_ge(idx, padding_idx);
            size_t increment = esp_mbedtls_ct_size_if_else_0(a, 1);
            const mbedtls_ct_condition_t b = esp_mbedtls_ct_uint_eq(check[idx], padlen - 1);
            increment = esp_mbedtls_ct_size_if_else_0(b, increment);
            pad_count += increment;
        }
        correct = esp_mbedtls_ct_bool_and(esp_mbedtls_ct_uint_eq(pad_count, padlen), correct);
...
#endif
        padlen = esp_mbedtls_ct_size_if_else_0(correct, padlen);
    ...
    if (auth_done == 0) {
...
#if defined(MBEDTLS_USE_PSA_CRYPTO)
        ret = esp_mbedtls_ct_hmac(transform->psa_mac_dec,
                              transform->psa_mac_alg,
                              add_data, add_data_len,
                              data, rec->data_len, min_len, max_len,
                              mac_expect);
#else
        ret = esp_mbedtls_ct_hmac(&transform->md_ctx_dec,
                              add_data, add_data_len,
                              data, rec->data_len, min_len, max_len,
                              mac_expect);
#endif /* MBEDTLS_USE_PSA_CRYPTO */
        if (ret != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ct_hmac", ret);
            goto hmac_failed_etm_disabled;
        }

        esp_mbedtls_ct_memcpy_offset(mac_peer, data,
                                 rec->data_len,
                                 min_len, max_len,
                                 transform->maclen);
#endif /* MBEDTLS_SSL_PROTO_TLS1_2 */
        ...
        if (esp_mbedtls_ct_memcmp(mac_peer, mac_expect,
                              transform->maclen) != 0) {
#if defined(MBEDTLS_SSL_DEBUG_ALL)
            MBEDTLS_SSL_DEBUG_MSG(1, ("message mac does not match"));
#endif
...
hmac_failed_etm_disabled:
        esp_mbedtls_platform_zeroize(mac_peer, transform->maclen);
        esp_mbedtls_platform_zeroize(mac_expect, transform->maclen);
        if (ret != 0) {
            return ret;
        }
    ...
    if (transform->tls_version == MBEDTLS_SSL_VERSION_TLS1_3) {
        ...
        ret = esp_ssl_parse_inner_plaintext(data, &rec->data_len,
                                        &rec->type);
 ...
 */
int esp_mbedtls_ssl_fetch_input(mbedtls_ssl_context *ssl, size_t nb_want)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    size_t len;
#if defined(MBEDTLS_SSL_VARIABLE_BUFFER_LENGTH)
    ...
    if (ssl->f_recv == NULL && ssl->f_recv_timeout == NULL) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("Bad usage of esp_mbedtls_ssl_set_bio() "));
        return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
    }
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
         ...
         */
        if (esp_mbedtls_ssl_check_timer(ssl) != 0) {
        ...
        if (ret == MBEDTLS_ERR_SSL_TIMEOUT) {
            ...
            if (ssl->state != MBEDTLS_SSL_HANDSHAKE_OVER) {
                if (esp_ssl_double_retransmit_timeout(ssl) != 0) {
                    MBEDTLS_SSL_DEBUG_MSG(1, ("handshake timeout"));
                    return MBEDTLS_ERR_SSL_TIMEOUT;
                }

                if ((ret = esp_mbedtls_ssl_resend(ssl)) != 0) {
                    MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_resend", ret);
                    return ret;
                }
                ...
                if ((ret = esp_mbedtls_ssl_resend_hello_request(ssl)) != 0) {
                    MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_resend_hello_request",
                                          ret);
                    return ret;
                }
 ...
 */
int esp_mbedtls_ssl_flush_output(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    unsigned char *buf;
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_flight_append(mbedtls_ssl_context *ssl)
{
    mbedtls_ssl_flight_item *msg;
    MBEDTLS_SSL_DEBUG_MSG(2, ("=> esp_ssl_flight_append"));
    MBEDTLS_SSL_DEBUG_BUF(4, "message appended to flight",
                          ssl->out_msg, ssl->out_msglen);

    /* Allocate space for current message */
    if ((msg = esp_mbedtls_calloc(1, sizeof(mbedtls_ssl_flight_item))) == NULL) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("alloc %" MBEDTLS_PRINTF_SIZET " bytes failed",
                                  sizeof(mbedtls_ssl_flight_item)));
        return MBEDTLS_ERR_SSL_ALLOC_FAILED;
    }

    if ((msg->p = esp_mbedtls_calloc(1, ssl->out_msglen)) == NULL) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("alloc %" MBEDTLS_PRINTF_SIZET " bytes failed",
                                  ssl->out_msglen));
        esp_mbedtls_free(msg);
        return MBEDTLS_ERR_SSL_ALLOC_FAILED;
    }
 ...
 */
void esp_mbedtls_ssl_flight_free(mbedtls_ssl_flight_item *flight)
{
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_swap_epochs(mbedtls_ssl_context *ssl)
{
 ...
 */
int esp_mbedtls_ssl_resend(mbedtls_ssl_context *ssl)
{
    int ret = 0;

    MBEDTLS_SSL_DEBUG_MSG(2, ("=> esp_mbedtls_ssl_resend"));

    ret = esp_mbedtls_ssl_flight_transmit(ssl);
 ...
 */
int esp_mbedtls_ssl_flight_transmit(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    MBEDTLS_SSL_DEBUG_MSG(2, ("=> esp_mbedtls_ssl_flight_transmit"));
    ...
    while (ssl->handshake->cur_msg != NULL) {
        ...
        ret = esp_ssl_get_remaining_payload_in_datagram(ssl);
        if (ret < 0) {
            return ret;
        }
        ...
        if ((ret = esp_mbedtls_ssl_write_record(ssl, force_flush)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_write_record", ret);
            return ret;
        }
    }
    ...
    if (esp_mbedtls_ssl_is_handshake_over(ssl) == 1) {
        ssl->handshake->retransmit_state = MBEDTLS_SSL_RETRANS_FINISHED;
    } else {
        ssl->handshake->retransmit_state = MBEDTLS_SSL_RETRANS_WAITING;
        esp_mbedtls_ssl_set_timer(ssl, ssl->handshake->retransmit_timeout);
    }
 ...
 */
void esp_mbedtls_ssl_recv_flight_completed(mbedtls_ssl_context *ssl)
{
    /* We won't need to resend that one any more */
    esp_mbedtls_ssl_flight_free(ssl->handshake->flight);
 ...
 */
void esp_mbedtls_ssl_send_flight_completed(mbedtls_ssl_context *ssl)
{
    esp_ssl_reset_retransmit_timeout(ssl);
    esp_mbedtls_ssl_set_timer(ssl, ssl->handshake->retransmit_timeout);
 ...
 */
int esp_mbedtls_ssl_start_handshake_msg(mbedtls_ssl_context *ssl, unsigned char hs_type,
                                        ...
                                        int force_flush)
{
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM &&
        !(ssl->out_msgtype == MBEDTLS_SSL_MSG_HANDSHAKE &&
          hs_type          == MBEDTLS_SSL_HS_HELLO_REQUEST)) {
        if ((ret = esp_ssl_flight_append(ssl)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_ssl_flight_append", ret);
            return ret;
        }
    } else
#endif
...
int esp_mbedtls_ssl_finish_handshake_msg(mbedtls_ssl_context *ssl,
                                     size_t buf_len, size_t msg_len)
{
    ...
    MBEDTLS_SSL_PROC_CHK(esp_mbedtls_ssl_write_handshake_msg_ext(ssl, 0, 0));
 ...
 */
int esp_mbedtls_ssl_write_record(mbedtls_ssl_context *ssl, int force_flush)
{
    int ret, done = 0;
    ...
    if (!done) {
...
#endif /* MBEDTLS_SSL_PROTO_TLS1_3 */
        esp_mbedtls_ssl_write_version(ssl->out_hdr + 1, ssl->conf->transport,
                                  tls_ver);
        ...
        if (ssl->transform_out != NULL) {
            ...
            esp_mbedtls_ssl_write_version(rec.ver, ssl->conf->transport, tls_ver);
            rec.type = ssl->out_msgtype;

#if defined(MBEDTLS_SSL_DTLS_CONNECTION_ID)
            /* The CID is set by esp_mbedtls_ssl_encrypt_buf(). */
            rec.cid_len = 0;
#endif /* MBEDTLS_SSL_DTLS_CONNECTION_ID */

            if ((ret = esp_mbedtls_ssl_encrypt_buf(ssl, ssl->transform_out, &rec,
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM &&
        ...
        ret = esp_ssl_get_remaining_payload_in_datagram(ssl);
        if (ret < 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_ssl_get_remaining_payload_in_datagram",
                                  ret);
            return ret;
        }
    ...
    if ((flush == SSL_FORCE_FLUSH) &&
        (ret = esp_mbedtls_ssl_flush_output(ssl)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_flush_output", ret);
        return ret;
    }
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_hs_is_proper_fragment(mbedtls_ssl_context *ssl)
{
...
static uint32_t esp_ssl_get_hs_frag_len(mbedtls_ssl_context const *ssl)
{
    return MBEDTLS_GET_UINT24_BE(ssl->in_msg, 9);
}

static uint32_t esp_ssl_get_hs_frag_off(mbedtls_ssl_context const *ssl)
{
    return MBEDTLS_GET_UINT24_BE(ssl->in_msg, 6);
}

MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_check_hs_header(mbedtls_ssl_context const *ssl)
{
    uint32_t msg_len, frag_off, frag_len;

    msg_len  = esp_ssl_get_hs_total_len(ssl);
    frag_off = esp_ssl_get_hs_frag_off(ssl);
    frag_len = esp_ssl_get_hs_frag_len(ssl);
 ...
 */
static void esp_ssl_bitmask_set(unsigned char *mask, size_t offset, size_t len)
{
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_bitmask_check(unsigned char *mask, size_t len)
{
    size_t i;
...
static size_t esp_ssl_get_reassembly_buffer_size(size_t msg_len,
                                             unsigned add_bitmap)
{
    size_t alloc_len;
...
int esp_mbedtls_ssl_prepare_handshake_record(mbedtls_ssl_context *ssl)
{
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
        ...
        if (esp_ssl_check_hs_header(ssl) != 0) {
        ...
        if (ssl->handshake != NULL &&
            ((esp_mbedtls_ssl_is_handshake_over(ssl) == 0 &&
             ...
             (esp_mbedtls_ssl_is_handshake_over(ssl) == 1 &&
        ...
        if (esp_ssl_hs_is_proper_fragment(ssl) == 1) {
...
int esp_mbedtls_ssl_update_handshake_status(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    mbedtls_ssl_handshake_params * const hs = ssl->handshake;

    if (esp_mbedtls_ssl_is_handshake_over(ssl) == 0 && hs != NULL) {
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM &&
        ...
        /* Free first entry */
        esp_ssl_buffering_free_slot(ssl, 0);
...
void esp_mbedtls_ssl_dtls_replay_reset(mbedtls_ssl_context *ssl)
{
    ssl->in_window_top = 0;
    ssl->in_window = 0;
}

static inline uint64_t esp_ssl_load_six_bytes(unsigned char *buf)
{
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_mbedtls_ssl_dtls_record_replay_check(mbedtls_ssl_context *ssl, uint8_t *record_in_ctr)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    ret = esp_mbedtls_ssl_dtls_replay_check((mbedtls_ssl_context const *) ssl);
 ...
 */
int esp_mbedtls_ssl_dtls_replay_check(mbedtls_ssl_context const *ssl)
{
    uint64_t rec_seqnum = esp_ssl_load_six_bytes(ssl->in_ctr + 2);
    uint64_t bit;
 ...
 */
void esp_mbedtls_ssl_dtls_replay_update(mbedtls_ssl_context *ssl)
{
    uint64_t rec_seqnum = esp_ssl_load_six_bytes(ssl->in_ctr + 2);
...
MBEDTLS_CHECK_RETURN_CRITICAL
MBEDTLS_STATIC_TESTABLE
int esp_mbedtls_ssl_check_dtls_clihlo_cookie(
 ...
 *
 * Called by esp_ssl_parse_record_header() in case we receive an epoch 0 record
 ...
 *
 * This function is called (through esp_ssl_check_client_reconnect()) when an
 * unexpected record is found in esp_ssl_get_next_record(), which will discard the
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_handle_possible_reconnect(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    size_t len = 0;
    ...
    ret = esp_mbedtls_ssl_check_dtls_clihlo_cookie(
        ssl,
        ssl->cli_id, ssl->cli_id_len,
        ssl->in_buf, ssl->in_left,
        ssl->out_buf, MBEDTLS_SSL_OUT_CONTENT_LEN, &len);

    MBEDTLS_SSL_DEBUG_RET(2, "esp_mbedtls_ssl_check_dtls_clihlo_cookie", ret);
    ...
    if (ret == 0) {
        ...
        if ((ret = esp_mbedtls_ssl_session_reset_int(ssl, 1)) != 0) {
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_check_record_type(uint8_t record_type)
{
 ...
 *
 * With DTLS, esp_mbedtls_ssl_read_record() will:
                                   ...
                                   mbedtls_record *rec)
{
...
#endif /* MBEDTLS_SSL_DTLS_CONNECTION_ID */
    {
        if (esp_ssl_check_record_type(rec->type)) {
    ...
    tls_version = (mbedtls_ssl_protocol_version) esp_mbedtls_ssl_read_version(
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
        ...
        else if (esp_mbedtls_ssl_dtls_record_replay_check((mbedtls_ssl_context *) ssl,
                                                      &rec->ctr[0]) != 0) {
...
#if defined(MBEDTLS_SSL_DTLS_CLIENT_PORT_REUSE) && defined(MBEDTLS_SSL_SRV_C)
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_check_client_reconnect(mbedtls_ssl_context *ssl)
{
    ...
    if (rec_epoch == 0 &&
        ...
        MBEDTLS_SSL_DEBUG_MSG(1, ("possible client reconnect "
                                  "from the same port"));
        return esp_ssl_handle_possible_reconnect(ssl);
    }
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_prepare_record_content(mbedtls_ssl_context *ssl,
                                      mbedtls_record *rec)
{
    int ret, done = 0;
    ...
    if (!done && ssl->transform_in != NULL) {
        ...
        if ((ret = esp_mbedtls_ssl_decrypt_buf(ssl, ssl->transform_in,
                                           rec)) != 0) {
            ...
            if ((old_msg_type == MBEDTLS_SSL_MSG_APPLICATION_DATA) &&
                ...
                ret = esp_mbedtls_ssl_tls13_check_early_data_len(ssl, rec->data_len);
                if (ret != 0) {
                    return ret;
                }
         ...
         * in esp_ssl_parse_record_header(), failing or silently
...
#if defined(MBEDTLS_SSL_DTLS_ANTI_REPLAY)
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
        esp_mbedtls_ssl_dtls_replay_update(ssl);
    }
#endif
...
/* Helper functions for esp_mbedtls_ssl_read_record(). */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_consume_current_message(mbedtls_ssl_context *ssl);
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_get_next_record(mbedtls_ssl_context *ssl);
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_record_is_in_progress(mbedtls_ssl_context *ssl);

int esp_mbedtls_ssl_read_record(mbedtls_ssl_context *ssl,
                            unsigned update_hs_digest)
{
    ...
    if (ssl->keep_current_message == 0) {
        do {
            ...
            ret = esp_ssl_consume_current_message(ssl);
            if (ret != 0) {
                return ret;
            }

            if (esp_ssl_record_is_in_progress(ssl) == 0) {
                ...
                if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM &&
                    esp_ssl_next_record_is_in_datagram(ssl) == 0) {
                    if (esp_ssl_load_buffered_message(ssl) == 0) {
                ...
                if (dtls_have_buffered == 0) {
                    ret = esp_ssl_get_next_record(ssl);
                    if (ret == MBEDTLS_ERR_SSL_CONTINUE_PROCESSING) {
                        continue;
                    }

                    if (ret != 0) {
                        MBEDTLS_SSL_DEBUG_RET(1, ("esp_ssl_get_next_record"), ret);
                        return ret;
                    }
                }
            }

            ret = esp_mbedtls_ssl_handle_message_type(ssl);

#if defined(MBEDTLS_SSL_PROTO_DTLS)
            if (ret == MBEDTLS_ERR_SSL_EARLY_MESSAGE) {
                /* Buffer future message */
                ret = esp_ssl_buffer_message(ssl);
                if (ret != 0) {
                    return ret;
                }
        ...
        if (0 != ret) {
            MBEDTLS_SSL_DEBUG_RET(1, ("esp_mbedtls_ssl_handle_message_type"), ret);
            return ret;
        }

        if (ssl->in_msgtype == MBEDTLS_SSL_MSG_HANDSHAKE &&
            update_hs_digest == 1) {
            ret = esp_mbedtls_ssl_update_handshake_status(ssl);
            if (0 != ret) {
                MBEDTLS_SSL_DEBUG_RET(1, ("esp_mbedtls_ssl_update_handshake_status"), ret);
                return ret;
            }
        }
    } else {
...
static int esp_ssl_load_buffered_message(mbedtls_ssl_context *ssl)
{
    ...
    MBEDTLS_SSL_DEBUG_MSG(2, ("=> esp_ssl_load_buffered_message"));
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_buffer_make_space(mbedtls_ssl_context *ssl,
                                 size_t desired)
{
    int offset;
    mbedtls_ssl_handshake_params * const hs = ssl->handshake;
    ...
    esp_ssl_free_buffered_record(ssl);
    ...
    for (offset = MBEDTLS_SSL_MAX_BUFFERED_HS - 1;
        ...
        esp_ssl_buffering_free_slot(ssl, (uint8_t) offset);
...
static int esp_ssl_buffer_message(mbedtls_ssl_context *ssl)
{
    ...
    MBEDTLS_SSL_DEBUG_MSG(2, ("=> esp_ssl_buffer_message"));
        ...
        case MBEDTLS_SSL_MSG_HANDSHAKE:
        {
            ...
            if (!hs_buf->is_valid) {
                ...
                hs_buf->is_fragmented =
                    (esp_ssl_hs_is_proper_fragment(ssl) == 1);

                /* We copy the message back into the input buffer
                 * after reassembly, so check that it's not too large.
                 * This is an implementation-specific limitation
                 * and not one from the standard, hence it is not
                 * checked in esp_ssl_check_hs_header(). */
                if (msg_len + 12 > MBEDTLS_SSL_IN_CONTENT_LEN) {
                    /* Ignore message */
                    goto exit;
                }
                ...
                reassembly_buf_sz = esp_ssl_get_reassembly_buffer_size(msg_len,
                                                                   hs_buf->is_fragmented);
                ...
                if (reassembly_buf_sz > (MBEDTLS_SSL_DTLS_MAX_BUFFERING -
                    ...
                    if (esp_ssl_buffer_make_space(ssl, reassembly_buf_sz) != 0) {
                        MBEDTLS_SSL_DEBUG_MSG(2,
                ...
                hs_buf->data = esp_mbedtls_calloc(1, reassembly_buf_sz);
                if (hs_buf->data == NULL) {
            ...
            if (!hs_buf->is_complete) {
                 ...
                 * esp_mbedtls_ssl_prepare_handshake_record(). */
                frag_off = esp_ssl_get_hs_frag_off(ssl);
                frag_len = esp_ssl_get_hs_frag_len(ssl);
                ...
                if (hs_buf->is_fragmented) {
                    ...
                    esp_ssl_bitmask_set(bitmask, frag_off, frag_len);
                    hs_buf->is_complete = (esp_ssl_bitmask_check(bitmask,
                                                             msg_len) == 0);
                } else {
...
static int esp_ssl_consume_current_message(mbedtls_ssl_context *ssl)
{
     ...
     *     and consumes through esp_mbedtls_ssl_read only.
     *
     */
    ...
    if (ssl->in_hslen != 0) {
         ...
         *     sequence number, see esp_mbedtls_ssl_prepare_handshake_record.
         */
...
static void esp_ssl_free_buffered_record(mbedtls_ssl_context *ssl)
{
    ...
    if (hs->buffering.future_record.data != NULL) {
        ...
        esp_mbedtls_free(hs->buffering.future_record.data);
...
static int esp_ssl_load_buffered_record(mbedtls_ssl_context *ssl)
{
    ...
    if (esp_ssl_next_record_is_in_datagram(ssl) == 1) {
        return 0;
    }

    MBEDTLS_SSL_DEBUG_MSG(2, ("=> esp_ssl_load_buffered_record"));
                                    ...
                                    mbedtls_record const *rec)
{
    ...
    /* esp_ssl_parse_record_header() only considers records
     * of the next epoch as candidates for buffering. */
    hs->buffering.future_record.epoch = ssl->in_epoch + 1;
    hs->buffering.future_record.len   = rec->buf_len;

    hs->buffering.future_record.data =
        esp_mbedtls_calloc(1, hs->buffering.future_record.len);
    if (hs->buffering.future_record.data == NULL) {
...
static int esp_ssl_get_next_record(mbedtls_ssl_context *ssl)
{
    ...
    ret = esp_ssl_load_buffered_record(ssl);
    if (ret != 0) {
        return ret;
    }
#endif /* MBEDTLS_SSL_PROTO_DTLS */
    ...
    ret = esp_ssl_parse_record_header(ssl, ssl->in_hdr, ssl->in_left, &rec);
    if (ret != 0) {
#if defined(MBEDTLS_SSL_PROTO_DTLS)
        if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
            if (ret == MBEDTLS_ERR_SSL_EARLY_MESSAGE) {
                ret = esp_ssl_buffer_future_record(ssl, &rec);
                if (ret != 0) {
                    return ret;
                }
            ...
            if (ret == MBEDTLS_ERR_SSL_UNEXPECTED_RECORD) {
                 ...
                 * record plaintext. */
                esp_mbedtls_ssl_update_in_pointers(ssl);
                ...
                ret = esp_ssl_check_client_reconnect(ssl);
                MBEDTLS_SSL_DEBUG_RET(2, "esp_ssl_check_client_reconnect", ret);
                if (ret != 0) {
                    return ret;
                }
#endif
...
#endif
    {
         ...
         */
        ret = esp_mbedtls_ssl_fetch_input(ssl, rec.buf_len);
        if (ret != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_fetch_input", ret);
            return ret;
        }
    ...
    if ((ret = esp_ssl_prepare_record_content(ssl, &rec)) != 0) {
#if defined(MBEDTLS_SSL_PROTO_DTLS)
        if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
     ...
     * the server-side esp_ssl_parse_client_hello() reparses the
...
int esp_mbedtls_ssl_handle_message_type(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
     ...
     */
    if (ssl->in_msgtype == MBEDTLS_SSL_MSG_HANDSHAKE) {
        if ((ret = esp_mbedtls_ssl_prepare_handshake_record(ssl)) != 0) {
            return ret;
        }
    }
...
int esp_mbedtls_ssl_send_fatal_handshake_failure(mbedtls_ssl_context *ssl)
{
    return esp_mbedtls_ssl_send_alert_message(ssl,
                                          MBEDTLS_SSL_ALERT_LEVEL_FATAL,
                                   ...
                                   unsigned char message)
{
    ...
    if ((ret = esp_mbedtls_ssl_write_record(ssl, SSL_FORCE_FLUSH)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_write_record", ret);
        return ret;
    }
    MBEDTLS_SSL_DEBUG_MSG(2, ("<= send alert message"));
...
int esp_mbedtls_ssl_write_change_cipher_spec(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
...
int esp_mbedtls_ssl_parse_change_cipher_spec(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
...
static size_t esp_ssl_transform_get_explicit_iv_len(
    mbedtls_ssl_transform const *transform)
{
    return transform->ivlen - transform->fixed_ivlen;
}

void esp_mbedtls_ssl_update_out_pointers(mbedtls_ssl_context *ssl,
                                     mbedtls_ssl_transform *transform)
{
#if defined(MBEDTLS_SSL_PROTO_DTLS)
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
    ...
    if (transform != NULL) {
        ssl->out_msg += esp_ssl_transform_get_explicit_iv_len(transform);
    }
}
...
void esp_mbedtls_ssl_update_in_pointers(mbedtls_ssl_context *ssl)
{
...
void esp_mbedtls_ssl_reset_in_out_pointers(mbedtls_ssl_context *ssl)
{
    ...
    esp_mbedtls_ssl_update_out_pointers(ssl, NULL /* no transform enabled */);
    esp_mbedtls_ssl_update_in_pointers(ssl);
}
...
int esp_mbedtls_ssl_check_pending(const mbedtls_ssl_context *ssl)
{
     ...
     * As in esp_ssl_get_next_record, this needs to be adapted if
...
int esp_mbedtls_ssl_get_record_expansion(const mbedtls_ssl_context *ssl)
{
    ...
    if (transform->psa_alg == PSA_ALG_GCM ||
        ...
        (void) esp_psa_get_key_attributes(transform->psa_key_enc, &attr);
        ...
        MBEDTLS_SSL_DEBUG_MSG(1,
                              ("Unsupported psa_alg spotted in esp_mbedtls_ssl_get_record_expansion()"));
        return MBEDTLS_ERR_SSL_INTERNAL_ERROR;
    }
#else
    switch (esp_mbedtls_cipher_get_cipher_mode(&transform->cipher_ctx_enc)) {
        ...
        case MBEDTLS_MODE_CBC:
            ...
            block_size = esp_mbedtls_cipher_get_block_size(
...
static int esp_ssl_check_ctr_renegotiate(mbedtls_ssl_context *ssl)
{
    ...
    return esp_mbedtls_ssl_renegotiate(ssl);
}
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_handle_hs_message_post_handshake(mbedtls_ssl_context *ssl)
{
    ...
    if (ssl->conf->endpoint == MBEDTLS_SSL_IS_CLIENT) {
        int ret = esp_ssl_tls13_check_new_session_ticket(ssl);
        if (ret != 0) {
            return ret;
        }
    }
...
#if defined(MBEDTLS_SSL_PROTO_TLS1_2)
/* This function is called from esp_mbedtls_ssl_read() when a handshake message is
 ...
 * TLS 1.3 in the future without bloating the logic of esp_mbedtls_ssl_read().
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls12_handle_hs_message_post_handshake(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
...
#endif
        ret = esp_mbedtls_ssl_start_renegotiation(ssl);
        if (ret != MBEDTLS_ERR_SSL_WAITING_SERVER_HELLO_RENEGO &&
            ret != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_start_renegotiation",
                                  ret);
            return ret;
        }
    } else
#endif /* MBEDTLS_SSL_RENEGOTIATION */
    {
        ...
        if ((ret = esp_mbedtls_ssl_send_alert_message(ssl,
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_handle_hs_message_post_handshake(mbedtls_ssl_context *ssl)
{
    /* Check protocol version and dispatch accordingly. */
#if defined(MBEDTLS_SSL_PROTO_TLS1_3)
    if (ssl->tls_version == MBEDTLS_SSL_VERSION_TLS1_3) {
        return esp_ssl_tls13_handle_hs_message_post_handshake(ssl);
    }
#endif /* MBEDTLS_SSL_PROTO_TLS1_3 */

#if defined(MBEDTLS_SSL_PROTO_TLS1_2)
    if (ssl->tls_version <= MBEDTLS_SSL_VERSION_TLS1_2) {
        return esp_ssl_tls12_handle_hs_message_post_handshake(ssl);
    }
#endif /* MBEDTLS_SSL_PROTO_TLS1_2 */
 ...
 */
static int esp_ssl_read_application_data(
    mbedtls_ssl_context *ssl, unsigned char *buf, size_t len)
{
    ...
    /* Zeroising the plaintext buffer to erase unused application data
       from the memory. */
    esp_mbedtls_platform_zeroize(ssl->in_offt, n);
...
int esp_mbedtls_ssl_read(mbedtls_ssl_context *ssl, unsigned char *buf, size_t len)
{
...
#if defined(MBEDTLS_SSL_RENEGOTIATION)
    ret = esp_ssl_check_ctr_renegotiate(ssl);
    if (ret != MBEDTLS_ERR_SSL_WAITING_SERVER_HELLO_RENEGO &&
        ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_ssl_check_ctr_renegotiate", ret);
        return ret;
    }
#endif
    ...
    while (ssl->in_offt == NULL) {
        ...
        if (ssl->f_get_timer != NULL &&
            ...
            esp_mbedtls_ssl_set_timer(ssl, ssl->conf->read_timeout);
        }
        ...
        if (ssl->in_msgtype == MBEDTLS_SSL_MSG_HANDSHAKE) {
            ret = esp_ssl_handle_hs_message_post_handshake(ssl);
            if (ret != 0) {
                MBEDTLS_SSL_DEBUG_RET(1, "esp_ssl_handle_hs_message_post_handshake",
                                      ret);
                return ret;
            }
        ...
        /* Fatal and closure alerts handled by esp_mbedtls_ssl_read_record() */
        if (ssl->in_msgtype == MBEDTLS_SSL_MSG_ALERT) {
         ...
         * again if esp_ssl_write_hello_request() returns WANT_WRITE */
#if defined(MBEDTLS_SSL_SRV_C) && defined(MBEDTLS_SSL_RENEGOTIATION)
        if (ssl->conf->endpoint == MBEDTLS_SSL_IS_SERVER &&
            ssl->renego_status == MBEDTLS_SSL_RENEGOTIATION_PENDING) {
            if ((ret = esp_mbedtls_ssl_resend_hello_request(ssl)) != 0) {
    ...
    ret = esp_ssl_read_application_data(ssl, buf, len);
                                ...
                                unsigned char *buf, size_t len)
{
    ...
    return esp_ssl_read_application_data(ssl, buf, len);
}
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_write_real(mbedtls_ssl_context *ssl,
                          const unsigned char *buf, size_t len)
{
    int ret = esp_mbedtls_ssl_get_max_out_record_payload(ssl);
    const size_t max_len = (size_t) ret;

    if (ret < 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_get_max_out_record_payload", ret);
        return ret;
    }
    ...
    if (ssl->out_left != 0) {
         ...
         * (e.g. esp_mbedtls_ssl_write()) to be called with the same parameters
         */
        if ((ret = esp_mbedtls_ssl_flush_output(ssl)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_flush_output", ret);
            return ret;
...
int esp_mbedtls_ssl_write(mbedtls_ssl_context *ssl, const unsigned char *buf, size_t len)
{
...
#if defined(MBEDTLS_SSL_RENEGOTIATION)
    if ((ret = esp_ssl_check_ctr_renegotiate(ssl)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_ssl_check_ctr_renegotiate", ret);
        return ret;
    }
#endif
    ...
    ret = esp_ssl_write_real(ssl, buf, len);
                                 ...
                                 const unsigned char *buf, size_t len)
{
    ...
    if ((!esp_mbedtls_ssl_conf_is_tls13_enabled(conf)) ||
    ...
    if ((ssl->early_data_state == MBEDTLS_SSL_EARLY_DATA_STATE_IDLE) ||
        ...
        while ((ssl->early_data_state == MBEDTLS_SSL_EARLY_DATA_STATE_IDLE) ||
            ...
            if (ret != 0) {
                MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_handshake_step", ret);
                return ret;
            }

            ret = esp_mbedtls_ssl_flush_output(ssl);
            if (ret != 0) {
                MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_flush_output", ret);
                return ret;
            }
...
int esp_mbedtls_ssl_close_notify(mbedtls_ssl_context *ssl)
{
    ...
    if (esp_mbedtls_ssl_is_handshake_over(ssl) == 1) {
        if ((ret = esp_mbedtls_ssl_send_alert_message(ssl,
            ...
            MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_send_alert_message", ret);
            return ret;
        }
    }
...
void esp_mbedtls_ssl_transform_free(mbedtls_ssl_transform *transform)
{
    if (transform == NULL) {
        return;
    }
...
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    esp_psa_destroy_key(transform->psa_key_enc);
    esp_psa_destroy_key(transform->psa_key_dec);
#else
    esp_mbedtls_cipher_free(&transform->cipher_ctx_enc);
    esp_mbedtls_cipher_free(&transform->cipher_ctx_dec);
#endif /* MBEDTLS_USE_PSA_CRYPTO */

#if defined(MBEDTLS_SSL_SOME_SUITES_USE_MAC)
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    esp_psa_destroy_key(transform->psa_mac_enc);
    esp_psa_destroy_key(transform->psa_mac_dec);
#else
    esp_mbedtls_md_free(&transform->md_ctx_enc);
    esp_mbedtls_md_free(&transform->md_ctx_dec);
#endif /* MBEDTLS_USE_PSA_CRYPTO */
#endif

    esp_mbedtls_platform_zeroize(transform, sizeof(mbedtls_ssl_transform));
}

void esp_mbedtls_ssl_set_inbound_transform(mbedtls_ssl_context *ssl,
                                       mbedtls_ssl_transform *transform)
{
...
void esp_mbedtls_ssl_set_outbound_transform(mbedtls_ssl_context *ssl,
                                        mbedtls_ssl_transform *transform)
{
...
void esp_mbedtls_ssl_buffering_free(mbedtls_ssl_context *ssl)
{
    unsigned offset;
    mbedtls_ssl_handshake_params * const hs = ssl->handshake;
    ...
    for (offset = 0; offset < MBEDTLS_SSL_MAX_BUFFERED_HS; offset++) {
        esp_ssl_buffering_free_slot(ssl, offset);
    }
}
                                    ...
                                    uint8_t slot)
{
    ...
    if (hs_buf->is_valid == 1) {
        ...
        esp_mbedtls_zeroize_and_free(hs_buf->data, hs_buf->data_len);
        memset(hs_buf, 0, sizeof(mbedtls_ssl_hs_buffer));
    }
}
 ...
 */
void esp_mbedtls_ssl_write_version(unsigned char version[2], int transport,
                               mbedtls_ssl_protocol_version tls_version)
{
 ...
 * 0,   No alert message.
 * !0,  if esp_mbedtls_ssl_send_alert_message() returned in error, the error code it
 *      returned, ssl->alert_reason otherwise.
 */
int esp_mbedtls_ssl_handle_pending_alert(mbedtls_ssl_context *ssl)
{
    int ret;
    ...
    ret = esp_mbedtls_ssl_send_alert_message(ssl,
                                         MBEDTLS_SSL_ALERT_LEVEL_FATAL,
                                         ssl->alert_type);

    /* If esp_mbedtls_ssl_send_alert_message() returned with MBEDTLS_ERR_SSL_WANT_WRITE,
 ...
 */
void esp_mbedtls_ssl_pend_fatal_alert(mbedtls_ssl_context *ssl,
