...
static int esp_ssl_write_hostname_ext(mbedtls_ssl_context *ssl,
                                  unsigned char *buf,
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_write_alpn_ext(mbedtls_ssl_context *ssl,
                              unsigned char *buf,
                              const unsigned char *end,
                              size_t *out_len)
{
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_write_supported_groups_ext(mbedtls_ssl_context *ssl,
                                          unsigned char *buf,
                                          const unsigned char *end,
                                          int flags,
                                          size_t *out_len)
    ...
    for (; *group_list != 0; group_list++) {
        ...
        if (flags & SSL_WRITE_SUPPORTED_GROUPS_EXT_TLS1_3_FLAG) {
            ...
            if (esp_mbedtls_ssl_tls13_named_group_is_ecdhe(*group_list) &&
                (esp_mbedtls_ssl_get_ecp_group_id_from_tls_id(*group_list) !=
                 MBEDTLS_ECP_DP_NONE)) {
                propose_group = 1;
            }
#endif
        ...
        if (propose_group) {
            ...
            MBEDTLS_SSL_DEBUG_MSG(3, ("NamedGroup: %s ( %x )",
                                      esp_mbedtls_ssl_named_group_to_str(*group_list),
                                      *group_list));
        }
    }
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_write_client_hello_cipher_suites(
    ...
    size_t *out_len)
{
    ...
    *out_len = 0;
    ...
    for (size_t i = 0; ciphersuite_list[i] != 0; i++) {
        ...
        *tls12_uses_ec |= esp_mbedtls_ssl_ciphersuite_uses_ec(ciphersuite_info);
#endif
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_write_client_hello_body(mbedtls_ssl_context *ssl,
                                       unsigned char *buf,
                                       unsigned char *end,
                                       size_t *out_len,
                                       size_t *binders_len)
{
    ...
    *binders_len = 0;
     ...
     * ...
     *
     * The random bytes have been prepared by esp_ssl_prepare_client_hello() into
     ...
     * esp_ssl_prepare_client_hello() into the ssl->session_negotiate->id buffer
    ...
    *p++ = (unsigned char) ssl->session_negotiate->id_len;
    ...
    /* Write cipher_suites */
    ret = esp_ssl_write_client_hello_cipher_suites(ssl, p, end,
                                               &tls12_uses_ec,
                                               &output_len);
    if (ret != 0) {
        return ret;
    ...
    *p++ = MBEDTLS_SSL_COMPRESS_NULL;
    ...
    ret = esp_ssl_write_hostname_ext(ssl, p, end, &output_len);
    if (ret != 0) {
        return ret;
    }
    p += output_len;
...
#if defined(MBEDTLS_SSL_ALPN)
    ret = esp_ssl_write_alpn_ext(ssl, p, end, &output_len);
    if (ret != 0) {
        return ret;
    }
    p += output_len;
...
#if defined(MBEDTLS_SSL_PROTO_TLS1_3)
    if (propose_tls13) {
        ret = esp_mbedtls_ssl_tls13_write_client_hello_exts(ssl, p, end,
                                                        &output_len);
        if (ret != 0) {
            return ret;
        }
    ...
    defined(MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_SOME_EPHEMERAL_ENABLED)
    {
...
#if defined(MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_SOME_EPHEMERAL_ENABLED)
        if (propose_tls13 && esp_mbedtls_ssl_conf_tls13_is_some_ephemeral_enabled(ssl)) {
        ...
        if (ssl_write_supported_groups_ext_flags != 0) {
            ret = esp_ssl_write_supported_groups_ext(ssl, p, end,
                                                 ssl_write_supported_groups_ext_flags,
                                                 &output_len);
            if (ret != 0) {
                return ret;
    ...
    write_sig_alg_ext = write_sig_alg_ext ||
                        (propose_tls13 && esp_mbedtls_ssl_conf_tls13_is_ephemeral_enabled(ssl));
#endif
#if defined(MBEDTLS_SSL_PROTO_TLS1_2)
...
#if defined(MBEDTLS_SSL_PROTO_TLS1_2)
    if (propose_tls12) {
        ret = esp_mbedtls_ssl_tls12_write_client_hello_exts(ssl, p, end,
                                                        tls12_uses_ec,
                                                        &output_len);
        if (ret != 0) {
            return ret;
     ...
     */
    if (propose_tls13 && esp_mbedtls_ssl_conf_tls13_is_some_psk_enabled(ssl)) {
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_generate_random(mbedtls_ssl_context *ssl)
{
    ...
    if (ssl->tls_version == MBEDTLS_SSL_VERSION_TLS1_2) {
        ...
        mbedtls_time_t gmt_unix_time = esp_mbedtls_time(NULL);
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_prepare_client_hello(mbedtls_ssl_context *ssl)
{
    int ret;
...
#endif
        {
            ret = esp_ssl_generate_random(ssl);
            if (ret != 0) {
    ...
    if (ssl->tls_version == MBEDTLS_SSL_VERSION_TLS1_3  &&
        ...
        return esp_mbedtls_ssl_session_set_hostname(session_negotiate,
                                                ssl->hostname);
    }
#endif /* MBEDTLS_SSL_PROTO_TLS1_3 &&
          MBEDTLS_SSL_SESSION_TICKETS &&
 ...
 */
int esp_mbedtls_ssl_write_client_hello(mbedtls_ssl_context *ssl)
{
    int ret = 0;
    unsigned char *buf;
    ...
    MBEDTLS_SSL_PROC_CHK(esp_ssl_prepare_client_hello(ssl));

    MBEDTLS_SSL_PROC_CHK(esp_mbedtls_ssl_start_handshake_msg(
                             ssl, MBEDTLS_SSL_HS_CLIENT_HELLO,
                             &buf, &buf_len));

    MBEDTLS_SSL_PROC_CHK(esp_ssl_write_client_hello_body(ssl, buf,
                                                     buf + buf_len,
                                                     &msg_len,
                                                     &binders_len));
...
#endif /* MBEDTLS_SSL_PROTO_TLS1_2 && MBEDTLS_SSL_PROTO_DTLS */
    {
        ...
        if (binders_len > 0) {
            MBEDTLS_SSL_PROC_CHK(
                esp_mbedtls_ssl_tls13_write_binders_of_pre_shared_key_ext(
        ...
        MBEDTLS_SSL_PROC_CHK(esp_mbedtls_ssl_finish_handshake_msg(ssl,
                                                              buf_len,
                                                              msg_len));
        ...
        if (ssl->handshake->min_tls_version <=  MBEDTLS_SSL_VERSION_TLS1_3 &&
            MBEDTLS_SSL_VERSION_TLS1_3 <= ssl->tls_version) {
            ret = esp_mbedtls_ssl_tls13_finalize_client_hello(ssl);
        }
#endif
    }
