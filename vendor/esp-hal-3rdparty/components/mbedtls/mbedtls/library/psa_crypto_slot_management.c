...
static uint8_t esp_psa_get_key_slots_initialized(void)
{
    uint8_t initialized;
    ...
    initialized = esp_global_data.key_slots_initialized;
...
int esp_psa_is_valid_key_id(mbedtls_svc_key_id_t key, int vendor_ok)
{
 ...
 */
static psa_status_t esp_psa_get_and_lock_key_slot_in_memory(
    mbedtls_svc_key_id_t key, psa_key_slot_t **p_slot)
{
    ...
    if (esp_psa_key_id_is_volatile(key_id)) {
        ...
        if (!esp_psa_is_valid_key_id(key, 1)) {
            return PSA_ERROR_INVALID_HANDLE;
        }

        for (slot_idx = 0; slot_idx < MBEDTLS_PSA_KEY_SLOT_COUNT; slot_idx++) {
            slot = &esp_global_data.key_slots[slot_idx];
    ...
    if (status == PSA_SUCCESS) {
        status = esp_psa_register_read(slot);
        if (status == PSA_SUCCESS) {
            *p_slot = slot;
        }
    }
...
psa_status_t esp_psa_initialize_key_slots(void)
{
    /* Nothing to do: program startup and esp_psa_wipe_all_key_slots() both
    ...
    esp_global_data.key_slots_initialized = 1;
    return PSA_SUCCESS;
}

void esp_psa_wipe_all_key_slots(void)
{
    size_t slot_idx;
    ...
    for (slot_idx = 0; slot_idx < MBEDTLS_PSA_KEY_SLOT_COUNT; slot_idx++) {
        ...
        (void) esp_psa_wipe_key_slot(slot);
    }
    /* The global data mutex is already held when calling this function. */
    esp_global_data.key_slots_initialized = 0;
}

psa_status_t esp_psa_reserve_free_key_slot(psa_key_id_t *volatile_key_id,
                                       psa_key_slot_t **p_slot)
{
    ...
    if (!esp_psa_get_key_slots_initialized()) {
    ...
    for (slot_idx = 0; slot_idx < MBEDTLS_PSA_KEY_SLOT_COUNT; slot_idx++) {
        ...
        if ((unused_persistent_key_slot == NULL) &&
            ...
            (!esp_psa_key_slot_has_readers(slot)) &&
    ...
    if ((selected_slot == NULL) &&
        ...
        esp_psa_register_read(selected_slot);
        status = esp_psa_wipe_key_slot(selected_slot);
        if (status != PSA_SUCCESS) {
            goto error;
        }
    }
    ...
    if (selected_slot != NULL) {
        status = esp_psa_key_slot_state_transition(selected_slot, PSA_SLOT_EMPTY,
                                               PSA_SLOT_FILLING);
        if (status != PSA_SUCCESS) {
            goto error;
        }
        ...
        *volatile_key_id = PSA_KEY_ID_VOLATILE_MIN +
                           ((psa_key_id_t) (selected_slot - esp_global_data.key_slots));
...
static psa_status_t esp_psa_load_persistent_key_into_slot(psa_key_slot_t *slot)
{
    psa_status_t status = PSA_SUCCESS;
    uint8_t *key_data = NULL;
    size_t key_data_length = 0;

    status = esp_psa_load_persistent_key(&slot->attr,
    ...
    if (esp_psa_get_se_driver(slot->attr.lifetime, &drv, &drv_context)) {
    ...
    status = esp_psa_copy_key_material_into_slot(slot, key_data, key_data_length);
    if (status != PSA_SUCCESS) {
        goto exit;
    }

exit:
    esp_psa_free_persistent_key_data(key_data, key_data_length);
    return status;
}
...
static psa_status_t esp_psa_load_builtin_key_into_slot(psa_key_slot_t *slot)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
    ...
    status = esp_psa_driver_wrapper_get_builtin_key(
...
psa_status_t esp_psa_get_and_lock_key_slot(mbedtls_svc_key_id_t key,
                                       psa_key_slot_t **p_slot)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
    ...
    *p_slot = NULL;
     ...
     */
    status = esp_psa_get_and_lock_key_slot_in_memory(key, p_slot);
    ...
    status = esp_psa_load_builtin_key_into_slot(*p_slot);
#endif /* MBEDTLS_PSA_CRYPTO_BUILTIN_KEYS */

#if defined(MBEDTLS_PSA_CRYPTO_STORAGE_C)
    if (status == PSA_ERROR_DOES_NOT_EXIST) {
        status = esp_psa_load_persistent_key_into_slot(*p_slot);
    }
#endif /* defined(MBEDTLS_PSA_CRYPTO_STORAGE_C) */

    if (status != PSA_SUCCESS) {
        esp_psa_wipe_key_slot(*p_slot);
        ...
        esp_psa_key_slot_state_transition((*p_slot), PSA_SLOT_FILLING,
                                      PSA_SLOT_FULL);
        status = esp_psa_register_read(*p_slot);
    }
...
psa_status_t esp_psa_unregister_read(psa_key_slot_t *slot)
{
    if (slot == NULL) {
        return PSA_SUCCESS;
    }
    ...
    if ((slot->state == PSA_SLOT_PENDING_DELETION) &&
        (slot->registered_readers == 1)) {
        return esp_psa_wipe_key_slot(slot);
    }

    if (esp_psa_key_slot_has_readers(slot)) {
     ...
     */
    MBEDTLS_TEST_HOOK_TEST_ASSERT(esp_psa_key_slot_has_readers(slot));
    return PSA_ERROR_CORRUPTION_DETECTED;
}

psa_status_t esp_psa_unregister_read_under_mutex(psa_key_slot_t *slot)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
#if defined(MBEDTLS_THREADING_C)
    /* We need to set status as success, otherwise CORRUPTION_DETECTED
...
psa_status_t esp_psa_validate_key_persistence(psa_key_lifetime_t lifetime)
{
...
psa_status_t esp_psa_open_key(mbedtls_svc_key_id_t key, psa_key_handle_t *handle)
{
#if defined(MBEDTLS_PSA_CRYPTO_STORAGE_C) || \
    defined(MBEDTLS_PSA_CRYPTO_BUILTIN_KEYS)
    psa_status_t status;
...
psa_status_t esp_psa_close_key(psa_key_handle_t handle)
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
    psa_key_slot_t *slot;
    ...
    PSA_THREADING_CHK_RET(esp_mbedtls_mutex_lock(
                              &mbedtls_threading_key_slot_mutex));
#endif
    status = esp_psa_get_and_lock_key_slot_in_memory(handle, &slot);
    if (status != PSA_SUCCESS) {
        if (status == PSA_ERROR_DOES_NOT_EXIST) {
            status = PSA_ERROR_INVALID_HANDLE;
        }
    ...
    if (slot->registered_readers == 1) {
        status = esp_psa_wipe_key_slot(slot);
    } else {
        status = esp_psa_unregister_read(slot);
    }
#if defined(MBEDTLS_THREADING_C)
...
psa_status_t esp_psa_purge_key(mbedtls_svc_key_id_t key)
{
    ...
    PSA_THREADING_CHK_RET(esp_mbedtls_mutex_lock(
                              &mbedtls_threading_key_slot_mutex));
#endif
    status = esp_psa_get_and_lock_key_slot_in_memory(key, &slot);
    if (status != PSA_SUCCESS) {
#if defined(MBEDTLS_THREADING_C)
        PSA_THREADING_CHK_RET(esp_mbedtls_mutex_unlock(
                                  &mbedtls_threading_key_slot_mutex));
