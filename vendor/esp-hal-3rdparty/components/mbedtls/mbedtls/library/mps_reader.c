...
static int esp_mbedtls_mps_trace_id = MBEDTLS_MPS_TRACE_BIT_READER;
...
static inline int esp_mps_reader_is_accumulating(
...
static inline int esp_mps_reader_is_producing(
    mbedtls_mps_reader const *rd)
{
...
static inline int esp_mps_reader_is_consuming(
    mbedtls_mps_reader const *rd)
{
    return !esp_mps_reader_is_producing(rd);
}

static inline mbedtls_mps_size_t esp_mps_reader_get_fragment_offset(
    mbedtls_mps_reader const *rd)
{
...
static inline mbedtls_mps_size_t esp_mps_reader_serving_from_accumulator(
    mbedtls_mps_reader const *rd)
{
    mbedtls_mps_size_t frag_offset, end;

    frag_offset = esp_mps_reader_get_fragment_offset(rd);
    end = rd->end;

    return end < frag_offset;
}

static inline void esp_mps_reader_zero(mbedtls_mps_reader *rd)
{
...
int esp_mbedtls_mps_reader_init(mbedtls_mps_reader *rd,
                            unsigned char *acc,
                            mbedtls_mps_size_t acc_len)
{
    MBEDTLS_MPS_TRACE_INIT("esp_mbedtls_mps_reader_init");
    MBEDTLS_MPS_TRACE(MBEDTLS_MPS_TRACE_TYPE_COMMENT,
                      "* Accumulator size: %u bytes", (unsigned) acc_len);
    esp_mps_reader_zero(rd);
    rd->acc = acc;
    rd->acc_len = acc_len;
    MBEDTLS_MPS_TRACE_RETURN(0);
}

int esp_mbedtls_mps_reader_free(mbedtls_mps_reader *rd)
{
    MBEDTLS_MPS_TRACE_INIT("esp_mbedtls_mps_reader_free");
    esp_mps_reader_zero(rd);
    MBEDTLS_MPS_TRACE_RETURN(0);
}

int esp_mbedtls_mps_reader_feed(mbedtls_mps_reader *rd,
                            unsigned char *new_frag,
                            mbedtls_mps_size_t new_frag_len)
{
    mbedtls_mps_size_t copy_to_acc;
    MBEDTLS_MPS_TRACE_INIT("esp_mbedtls_mps_reader_feed");
    MBEDTLS_MPS_TRACE(MBEDTLS_MPS_TRACE_TYPE_COMMENT,
    ...
    MBEDTLS_MPS_STATE_VALIDATE_RAW(esp_mps_reader_is_producing(
                                       rd),
                                   "esp_mbedtls_mps_reader_feed() requires reader to be in producing mode");

    if (esp_mps_reader_is_accumulating(rd)) {
        unsigned char *acc    = rd->acc;
...
int esp_mbedtls_mps_reader_get(mbedtls_mps_reader *rd,
                           ...
                           mbedtls_mps_size_t *buflen)
{
    ...
    MBEDTLS_MPS_TRACE_INIT("esp_mbedtls_mps_reader_get");
    MBEDTLS_MPS_TRACE(MBEDTLS_MPS_TRACE_TYPE_COMMENT,
                      "* Bytes requested: %u", (unsigned) desired);

    MBEDTLS_MPS_STATE_VALIDATE_RAW(esp_mps_reader_is_consuming(
                                       rd),
                                   "esp_mbedtls_mps_reader_get() requires reader to be in consuming mode");

    end = rd->end;
    frag_offset = esp_mps_reader_get_fragment_offset(rd);

    /* Check if we're still serving from the accumulator. */
    if (esp_mps_reader_serving_from_accumulator(rd)) {
         ...
         * `frag` starting from the next call to esp_mbedtls_mps_reader_get().
         */
...
int esp_mbedtls_mps_reader_commit(mbedtls_mps_reader *rd)
{
    mbedtls_mps_size_t end;
    MBEDTLS_MPS_TRACE_INIT("esp_mbedtls_mps_reader_commit");
    MBEDTLS_MPS_STATE_VALIDATE_RAW(esp_mps_reader_is_consuming(
                                       rd),
                                   "esp_mbedtls_mps_reader_commit() requires reader to be in consuming mode");
...
int esp_mbedtls_mps_reader_reclaim(mbedtls_mps_reader *rd,
                               int *paused)
{
    unsigned char *frag, *acc;
    mbedtls_mps_size_t pending, commit;
    mbedtls_mps_size_t acc_len, frag_offset, frag_len;
    MBEDTLS_MPS_TRACE_INIT("esp_mbedtls_mps_reader_reclaim");
    ...
    MBEDTLS_MPS_STATE_VALIDATE_RAW(esp_mps_reader_is_consuming(
                                       rd),
                                   "esp_mbedtls_mps_reader_reclaim() requires reader to be in consuming mode");
    ...
    if (pending == 0) {
        ...
        if (overflow || acc_len < acc_len_needed) {
             ...
             * there hadn't been a call to esp_mbedtls_mps_reader_get()
