 ...
 *
 * This function may only be called before a call to esp_psa_crypto_init(),
 * or after a call to esp_mbedtls_psa_crypto_free() and before any
 * subsequent call to esp_psa_crypto_init().
 *
 ...
 *                      It is called by esp_psa_crypto_init().
 *                      By default this is esp_mbedtls_entropy_init().
 *                      This function cannot report failures directly.
 *                      To indicate a failure, set the entropy context
 *                      to a state where esp_mbedtls_entropy_func() will
 *                      return an error.
 * \param entropy_free  Function to free the entropy context
 *                      and associated resources.
 *                      It is called by esp_mbedtls_psa_crypto_free().
 *                      By default this is esp_mbedtls_entropy_free().
 *
 * \retval #PSA_SUCCESS
 *         Success.
...
psa_status_t esp_psa_mac_key_can_do(
 ...
 */
extern void (*esp_psa_input_pre_copy_hook)(const uint8_t *input, size_t input_len);
extern void (*esp_psa_input_post_copy_hook)(const uint8_t *input, size_t input_len);
extern void (*esp_psa_output_pre_copy_hook)(const uint8_t *output, size_t output_len);
extern void (*esp_psa_output_post_copy_hook)(const uint8_t *output, size_t output_len);
