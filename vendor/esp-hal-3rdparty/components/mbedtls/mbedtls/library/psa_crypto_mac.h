 ...
 *                              esp_psa_export_key() (i.e. the raw key bytes).
 ...
 *       drivers.
 *
 * The PSA core calls esp_mbedtls_psa_mac_sign_setup() or
 * esp_mbedtls_psa_mac_verify_setup() before calling this function.
 *
 * If this function returns an error status, the PSA core aborts the
 * operation by calling esp_mbedtls_psa_mac_abort().
 *
 ...
 *       drivers.
 *
 * The PSA core calls esp_mbedtls_psa_mac_sign_setup() before calling this function.
 * This function calculates the MAC of the message formed by concatenating
 * the inputs passed to preceding calls to esp_mbedtls_psa_mac_update().
 *
 * Whether this function returns successfully or not, the PSA core subsequently
 * aborts the operation by calling esp_mbedtls_psa_mac_abort().
 *
 * \param[in,out] operation Active MAC operation.
 * \param[out] mac          Buffer where the MAC value is to be written.
 ...
 *                          esp_mbedtls_psa_mac_sign_setup(). It also guarantees the
 ...
 *
 * The PSA core calls esp_mbedtls_psa_mac_verify_setup() before calling this
 * function. This function calculates the MAC of the message formed by
 * concatenating the inputs passed to preceding calls to
 * esp_mbedtls_psa_mac_update(). It then compares the calculated MAC with the
 ...
 *                          to esp_mbedtls_psa_mac_verify_setup().
 *
 * \retval #PSA_SUCCESS
 ...
 * esp_mbedtls_psa_mac_sign_setup() or esp_mbedtls_psa_mac_verify_setup() again.
 *
 ...
 *
 * In particular, calling esp_mbedtls_psa_mac_abort() after the operation has been
 * terminated by a call to esp_mbedtls_psa_mac_abort(),
 * esp_mbedtls_psa_mac_sign_finish() or esp_mbedtls_psa_mac_verify_finish() is safe and
 * has no effect.
 *
 ...
 * \retval #PSA_ERROR_CORRUPTION_DETECTED \emptydescription
 */
psa_status_t esp_mbedtls_psa_mac_abort(
    mbedtls_psa_mac_operation_t *operation);
