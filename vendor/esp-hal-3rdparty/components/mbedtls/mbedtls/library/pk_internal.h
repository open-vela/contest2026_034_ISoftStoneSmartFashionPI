...
#define PSA_PK_TO_MBEDTLS_ERR(status) esp_psa_pk_status_to_mbedtls(status)
#define PSA_PK_RSA_TO_MBEDTLS_ERR(status) PSA_TO_MBEDTLS_ERR_LIST(status,     \
                                                                  esp_psa_to_pk_rsa_errors,            \
                                                                  esp_psa_pk_status_to_mbedtls)
#define PSA_PK_ECDSA_TO_MBEDTLS_ERR(status) PSA_TO_MBEDTLS_ERR_LIST(status,   \
                                                                    esp_psa_to_pk_ecdsa_errors,        \
                                                                    esp_psa_pk_status_to_mbedtls)
...
/**
 * Public function esp_mbedtls_pk_ec() can be used to get direct access to the
 * wrapped ecp_keypair structure pointed to the pk_ctx. However this is not
 * ideal because it bypasses the PK module on the control of its internal
 * structure (pk_context) fields.
 * For backward compatibility we keep esp_mbedtls_pk_ec() when ECP_C is defined, but
 ...
 */
static inline const mbedtls_ecp_keypair *esp_mbedtls_pk_ec_ro(const mbedtls_pk_context pk)
{
...
static inline mbedtls_ecp_keypair *esp_mbedtls_pk_ec_rw(const mbedtls_pk_context pk)
{
    switch (esp_mbedtls_pk_get_type(&pk)) {
        case MBEDTLS_PK_ECKEY:
        case MBEDTLS_PK_ECKEY_DH:
...
#if defined(MBEDTLS_PK_HAVE_ECC_KEYS)
static inline mbedtls_ecp_group_id esp_mbedtls_pk_get_ec_group_id(const mbedtls_pk_context *pk)
{
    ...
    if (esp_mbedtls_pk_get_type(pk) == MBEDTLS_PK_OPAQUE) {
        ...
        id = esp_mbedtls_ecc_group_from_psa(curve, psa_get_key_bits(&opaque_attrs));
        esp_psa_reset_key_attributes(&opaque_attrs);
    } else
#endif /* MBEDTLS_USE_PSA_CRYPTO */
    {
#if defined(MBEDTLS_PK_USE_PSA_EC_DATA)
        id = esp_mbedtls_ecc_group_from_psa(pk->ec_family, pk->ec_bits);
#else /* MBEDTLS_PK_USE_PSA_EC_DATA */
        id = esp_mbedtls_pk_ec_ro(*pk)->grp.id;
...
static inline int esp_mbedtls_pk_is_rfc8410(const mbedtls_pk_context *pk)
{
    mbedtls_ecp_group_id id = esp_mbedtls_pk_get_ec_group_id(pk);
 ...
 *
 * [in/out] pk: in: must have the group set already, see esp_mbedtls_pk_ecc_set_group().
 *              out: will have the private key set.
 * [in] key, esp_key_len: the raw private key (no ASN.1 wrapping).
 */
 ...
 * Set the public key.
 *
 * [in/out] pk: in: must have its group set, see esp_mbedtls_pk_ecc_set_group().
 ...
 *
 * [in/out] pk: in: must have the private key set, see esp_mbedtls_pk_ecc_set_key().
 *              out: will have the public key set.
