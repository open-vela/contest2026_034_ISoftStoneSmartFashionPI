...
/* esp_mbedtls_cipher_values_from_psa() below only checks if the proper build symbols
 ...
 * esp_mbedtls_cipher_info_from_psa() might be used for the same purpose, but it
 * requires CIPHER_C to be enabled.
 */
static psa_status_t esp_mbedtls_cipher_validate_values(
    psa_algorithm_t alg,
...
psa_status_t esp_mbedtls_cipher_values_from_psa(
    psa_algorithm_t alg,
    psa_key_type_t key_type,
    size_t *key_bits,
    ...
    mbedtls_cipher_id_t *cipher_id)
{
    ...
    return esp_mbedtls_cipher_validate_values(alg, key_type);
}

#if defined(MBEDTLS_CIPHER_C)
const mbedtls_cipher_info_t *esp_mbedtls_cipher_info_from_psa(
    psa_algorithm_t alg,
    psa_key_type_t key_type,
    size_t key_bits,
    mbedtls_cipher_id_t *cipher_id)
{
    ...
    status = esp_mbedtls_cipher_values_from_psa(alg, key_type, &key_bits, &mode, &cipher_id_tmp);
    ...
    return esp_mbedtls_cipher_info_from_values(cipher_id_tmp, (int) key_bits, mode);
}
#endif /* MBEDTLS_CIPHER_C */

#if defined(MBEDTLS_PSA_BUILTIN_CIPHER)

static psa_status_t esp_psa_cipher_setup(
    ...
    mbedtls_operation_t cipher_operation)
{
    ...
    esp_mbedtls_cipher_init(&operation->ctx.cipher);

    operation->alg = alg;
    key_bits = attributes->bits;
    cipher_info = esp_mbedtls_cipher_info_from_psa(alg, key_type,
                                               key_bits, NULL);
    if (cipher_info == NULL) {
        return PSA_ERROR_NOT_SUPPORTED;
    }

    ret = esp_mbedtls_cipher_setup(&operation->ctx.cipher, cipher_info);
    if (ret != 0) {
        goto exit;
    }
    ...
    if (key_type == PSA_KEY_TYPE_DES && key_bits == 128) {
        ...
        ret = esp_mbedtls_cipher_setkey(&operation->ctx.cipher,
                                    keys,
                                    192, cipher_operation);
    } else
#endif
    {
        ret = esp_mbedtls_cipher_setkey(&operation->ctx.cipher, key_buffer,
    ...
    switch (alg) {
        case PSA_ALG_CBC_NO_PADDING:
            ret = esp_mbedtls_cipher_set_padding_mode(&operation->ctx.cipher,
...
psa_status_t esp_mbedtls_psa_cipher_encrypt_setup(
    mbedtls_psa_cipher_operation_t *operation,
    const psa_key_attributes_t *attributes,
    const uint8_t *key_buffer, size_t key_buffer_size,
    psa_algorithm_t alg)
{
    return esp_psa_cipher_setup(operation, attributes,
                            key_buffer, key_buffer_size,
                            alg, MBEDTLS_ENCRYPT);
}

psa_status_t esp_mbedtls_psa_cipher_decrypt_setup(
    mbedtls_psa_cipher_operation_t *operation,
    const psa_key_attributes_t *attributes,
    const uint8_t *key_buffer, size_t key_buffer_size,
    psa_algorithm_t alg)
...
psa_status_t esp_mbedtls_psa_cipher_set_iv(
    mbedtls_psa_cipher_operation_t *operation,
    const uint8_t *iv, size_t iv_length)
{
    ...
    return esp_mbedtls_to_psa_error(
        esp_mbedtls_cipher_set_iv(&operation->ctx.cipher,
                              iv, iv_length));
}
 ...
 * able to process arbitrary-length calls to esp_psa_cipher_update() with ECB mode,
 * but the underlying esp_mbedtls_cipher_update only takes full blocks.
 *
 ...
 */
static psa_status_t esp_psa_cipher_update_ecb(
    ...
    size_t *output_length)
{
    ...
    size_t block_size = esp_mbedtls_cipher_info_get_block_size(ctx->cipher_info);
    ...
    *output_length = 0;
    ...
    while (input_length >= block_size) {
        ...
        status = esp_mbedtls_to_psa_error(
            esp_mbedtls_cipher_update(ctx, input,
                                  block_size,
                                  output, &internal_output_length));
...
psa_status_t esp_mbedtls_psa_cipher_update(
    mbedtls_psa_cipher_operation_t *operation,
    ...
    uint8_t *output, size_t output_size, size_t *output_length)
{
    ...
    if (operation->alg == PSA_ALG_ECB_NO_PADDING) {
        /* esp_mbedtls_cipher_update has an API inconsistency: it will only
         * process a single block at a time in ECB mode. Abstract away that
         * inconsistency here to match the PSA API behaviour. */
        status = esp_psa_cipher_update_ecb(&operation->ctx.cipher,
                                       input,
                                       input_length,
                                       output,
                                       output_length);
    ...
    if (input_length == 0) {
        ...
        *output_length = 0;
        ...
        status = esp_mbedtls_to_psa_error(
            esp_mbedtls_cipher_update(&operation->ctx.cipher, input,
...
psa_status_t esp_mbedtls_psa_cipher_finish(
    mbedtls_psa_cipher_operation_t *operation,
    uint8_t *output, size_t output_size, size_t *output_length)
{
    ...
    status = esp_mbedtls_to_psa_error(
        esp_mbedtls_cipher_finish(&operation->ctx.cipher,
                              temp_output_buffer,
                              output_length));
    if (status != PSA_SUCCESS) {
        goto exit;
...
exit:
    esp_mbedtls_platform_zeroize(temp_output_buffer,
                             sizeof(temp_output_buffer));

    return status;
}

psa_status_t esp_mbedtls_psa_cipher_abort(
    mbedtls_psa_cipher_operation_t *operation)
{
    ...
    esp_mbedtls_cipher_free(&operation->ctx.cipher);

    return PSA_SUCCESS;
}

psa_status_t esp_mbedtls_psa_cipher_encrypt(
    const psa_key_attributes_t *attributes,
    ...
    size_t *output_length)
{
    ...
    status = esp_mbedtls_psa_cipher_encrypt_setup(&operation, attributes,
                                              key_buffer, key_buffer_size,
                                              alg);
    if (status != PSA_SUCCESS) {
        goto exit;
    ...
    if (iv_length > 0) {
        status = esp_mbedtls_psa_cipher_set_iv(&operation, iv, iv_length);
        if (status != PSA_SUCCESS) {
            goto exit;
        }
    }

    status = esp_mbedtls_psa_cipher_update(&operation, input, input_length,
                                       output, output_size,
...
exit:
    if (status == PSA_SUCCESS) {
        status = esp_mbedtls_psa_cipher_abort(&operation);
    } else {
        esp_mbedtls_psa_cipher_abort(&operation);
    }

    return status;
}

psa_status_t esp_mbedtls_psa_cipher_decrypt(
    const psa_key_attributes_t *attributes,
    const uint8_t *key_buffer,
    size_t key_buffer_size,
    psa_algorithm_t alg,
    ...
    size_t *output_length)
{
    ...
    status = esp_mbedtls_psa_cipher_decrypt_setup(&operation, attributes,
                                              key_buffer, key_buffer_size,
                                              alg);
    if (status != PSA_SUCCESS) {
        goto exit;
    ...
    if (operation.iv_length > 0) {
        status = esp_mbedtls_psa_cipher_set_iv(&operation,
