...
/*
 * esp_ssl_tls13_write_supported_versions_ext():
 *
 * struct {
...
static int esp_ssl_tls13_write_supported_versions_ext(mbedtls_ssl_context *ssl,
                                                  unsigned char *buf,
                                                  unsigned char *end,
                                                  size_t *out_len)
{
    ...
    *p++ = versions_len;
     ...
     */
    esp_mbedtls_ssl_write_version(p, MBEDTLS_SSL_TRANSPORT_STREAM,
                              MBEDTLS_SSL_VERSION_TLS1_3);
    MBEDTLS_SSL_DEBUG_MSG(3, ("supported version: [3:4]"));


    if (ssl->handshake->min_tls_version <= MBEDTLS_SSL_VERSION_TLS1_2) {
        esp_mbedtls_ssl_write_version(p + 2, MBEDTLS_SSL_TRANSPORT_STREAM,
                                                  ...
                                                  const unsigned char *end)
{
    ...
    if (esp_mbedtls_ssl_read_version(buf, ssl->conf->transport) !=
        MBEDTLS_SSL_VERSION_TLS1_3) {
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_parse_alpn_ext(mbedtls_ssl_context *ssl,
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_reset_key_share(mbedtls_ssl_context *ssl)
{
    ...
    if (esp_mbedtls_ssl_tls13_named_group_is_ecdhe(group_id) ||
        esp_mbedtls_ssl_tls13_named_group_is_ffdh(group_id)) {
        int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
        psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;

        /* Destroy generated private key. */
        status = esp_psa_destroy_key(ssl->handshake->xxdh_psa_privkey);
...
#if defined(MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_SOME_EPHEMERAL_ENABLED)
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_get_default_group_id(mbedtls_ssl_context *ssl,
                                          uint16_t *group_id)
{
    ...
    for (; *group_list != 0; group_list++) {
        ...
        if ((esp_mbedtls_ssl_get_psa_curve_info_from_tls_id(
        ...
        if (esp_mbedtls_ssl_tls13_named_group_is_ffdh(*group_list)) {
            *group_id = *group_list;
            return 0;
        }
#endif
...
/*
 * esp_ssl_tls13_parse_hrr_key_share_ext()
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_parse_hrr_key_share_ext(mbedtls_ssl_context *ssl,
                                             const unsigned char *buf,
                                             const unsigned char *end)
{
#if defined(PSA_WANT_ALG_ECDH) || defined(PSA_WANT_ALG_FFDH)
    ...
    for (; *group_list != 0; group_list++) {
#if defined(PSA_WANT_ALG_ECDH)
        if (esp_mbedtls_ssl_tls13_named_group_is_ecdhe(*group_list)) {
            if ((esp_mbedtls_ssl_get_psa_curve_info_from_tls_id(
...
/*
 * esp_ssl_tls13_parse_key_share_ext()
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_parse_key_share_ext(mbedtls_ssl_context *ssl,
                                         const unsigned char *buf,
                                         const unsigned char *end)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    if (esp_mbedtls_ssl_tls13_named_group_is_ecdhe(group) ||
        ...
        MBEDTLS_SSL_DEBUG_MSG(2,
                              ("DHE group name: %s", esp_mbedtls_ssl_named_group_to_str(group)));
        ret = esp_mbedtls_ssl_tls13_read_public_xxdhe_share(ssl, p, end - p);
        if (ret != 0) {
            return ret;
        }
    } else
...
/*
 * esp_ssl_tls13_parse_cookie_ext()
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_parse_cookie_ext(mbedtls_ssl_context *ssl,
                                      const unsigned char *buf,
                                      const unsigned char *end)
{
    uint16_t cookie_len;
    ...
    handshake->cookie = esp_mbedtls_calloc(1, cookie_len);
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_write_cookie_ext(mbedtls_ssl_context *ssl,
                                      unsigned char *buf,
                                      unsigned char *end,
                                      size_t *out_len)
{
...
/*
 * esp_ssl_tls13_write_psk_key_exchange_modes_ext() structure:
 *
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_write_psk_key_exchange_modes_ext(mbedtls_ssl_context *ssl,
                                                      unsigned char *buf,
                                                      unsigned char *end,
                                                      size_t *out_len)
{
    ...
    *out_len = 0;
     ...
     */
    if (!esp_mbedtls_ssl_conf_tls13_is_some_psk_enabled(ssl)) {
    ...
    if (esp_mbedtls_ssl_conf_tls13_is_psk_ephemeral_enabled(ssl)) {
    ...
    if (esp_mbedtls_ssl_conf_tls13_is_psk_enabled(ssl)) {
...
static psa_algorithm_t esp_ssl_tls13_get_ciphersuite_hash_alg(int ciphersuite)
{
    const mbedtls_ssl_ciphersuite_t *ciphersuite_info = NULL;
    ciphersuite_info = esp_mbedtls_ssl_ciphersuite_from_id(ciphersuite);

    if (ciphersuite_info != NULL) {
        return esp_mbedtls_md_psa_alg_from_type((mbedtls_md_type_t) ciphersuite_info->mac);
    }
...
static int esp_ssl_tls13_has_configured_ticket(mbedtls_ssl_context *ssl)
{
    mbedtls_ssl_session *session = ssl->session_negotiate;
    return ssl->handshake->resume &&
           session != NULL && session->ticket != NULL &&
           esp_mbedtls_ssl_conf_tls13_is_kex_mode_enabled(
        ssl, esp_mbedtls_ssl_tls13_session_get_ticket_flags(
            session, MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_PSK_ALL));
}

#if defined(MBEDTLS_SSL_EARLY_DATA)
static int esp_ssl_tls13_early_data_has_valid_ticket(mbedtls_ssl_context *ssl)
{
    mbedtls_ssl_session *session = ssl->session_negotiate;
    return ssl->handshake->resume &&
           session->tls_version == MBEDTLS_SSL_VERSION_TLS1_3 &&
           esp_mbedtls_ssl_tls13_session_ticket_allow_early_data(session) &&
           esp_mbedtls_ssl_tls13_cipher_suite_is_offered(ssl, session->ciphersuite);
}
#endif

MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_ticket_get_identity(mbedtls_ssl_context *ssl,
                                         ...
                                         size_t *identity_len)
{
    ...
    if (!esp_ssl_tls13_has_configured_ticket(ssl)) {
        return -1;
    }

    *hash_alg = esp_ssl_tls13_get_ciphersuite_hash_alg(session->ciphersuite);
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_ticket_get_psk(mbedtls_ssl_context *ssl,
                                    psa_algorithm_t *hash_alg,
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_psk_get_identity(mbedtls_ssl_context *ssl,
                                      psa_algorithm_t *hash_alg,
                                      const unsigned char **identity,
                                      size_t *identity_len)
{

    if (!esp_mbedtls_ssl_conf_has_static_psk(ssl->conf)) {
        return -1;
    }
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_psk_get_psk(mbedtls_ssl_context *ssl,
                                 psa_algorithm_t *hash_alg,
                                 const unsigned char **psk,
                                 size_t *psk_len)
{
...
static int esp_ssl_tls13_get_configured_psk_count(mbedtls_ssl_context *ssl)
{
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_write_identity(mbedtls_ssl_context *ssl,
                                    unsigned char *buf,
                                    unsigned char *end,
                                    const unsigned char *identity,
                                    size_t identity_len,
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_write_binder(mbedtls_ssl_context *ssl,
                                  unsigned char *buf,
                                  unsigned char *end,
                                  int psk_type,
                                  psa_algorithm_t hash_alg,
                                  ...
                                  size_t *out_len)
{
    ...
    *out_len = 0;
    ...
    ret = esp_mbedtls_ssl_get_handshake_transcript(
        ssl, esp_mbedtls_md_type_from_psa_alg(hash_alg),
        transcript, sizeof(transcript), &transcript_len);
    if (ret != 0) {
        return ret;
    }

    ret = esp_mbedtls_ssl_tls13_create_psk_binder(ssl, hash_alg,
                                              psk, psk_len, psk_type,
                                              transcript, buf + 1);
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "esp_mbedtls_ssl_tls13_create_psk_binder", ret);
        return ret;
    }
...
/*
 * esp_mbedtls_ssl_tls13_write_identities_of_pre_shared_key_ext() structure:
 *
 * struct {
 ...
 *
 */
int esp_mbedtls_ssl_tls13_write_identities_of_pre_shared_key_ext(
    ...
    size_t *out_len, size_t *binders_len)
{
    ...
    *binders_len = 0;
    ...
    configured_psk_count = esp_ssl_tls13_get_configured_psk_count(ssl);
...
#if defined(MBEDTLS_SSL_SESSION_TICKETS)
    if (esp_ssl_tls13_ticket_get_identity(
            ssl, &hash_alg, &identity, &identity_len) == 0) {
#if defined(MBEDTLS_HAVE_TIME)
        mbedtls_ms_time_t now = esp_mbedtls_ms_time();
        mbedtls_ssl_session *session = ssl->session_negotiate;
        /* The ticket age has been checked to be smaller than the
         * `ticket_lifetime` in esp_ssl_prepare_client_hello() which is smaller than
         * 7 days (enforced in esp_ssl_tls13_parse_new_session_ticket()) . Thus the
         * cast to `uint32_t` of the ticket age is safe. */
        uint32_t obfuscated_ticket_age =
            (uint32_t) (now - session->ticket_reception_time);
        obfuscated_ticket_age += session->ticket_age_add;

        ret = esp_ssl_tls13_write_identity(ssl, p, end,
                                       identity, identity_len,
                                       obfuscated_ticket_age,
                                       &output_len);
#else
        ret = esp_ssl_tls13_write_identity(ssl, p, end, identity, identity_len,
                                       0, &output_len);
    ...
    if (esp_ssl_tls13_psk_get_identity(
            ssl, &hash_alg, &identity, &identity_len) == 0) {

        ret = esp_ssl_tls13_write_identity(ssl, p, end, identity, identity_len, 0,
                                       &output_len);
        if (ret != 0) {
            return ret;
        }
...
int esp_mbedtls_ssl_tls13_write_binders_of_pre_shared_key_ext(
    mbedtls_ssl_context *ssl, unsigned char *buf, unsigned char *end)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    unsigned char *p = buf;
...
#if defined(MBEDTLS_SSL_SESSION_TICKETS)
    if (esp_ssl_tls13_ticket_get_psk(ssl, &hash_alg, &psk, &psk_len) == 0) {

        ret = esp_ssl_tls13_write_binder(ssl, p, end,
    ...
    if (esp_ssl_tls13_psk_get_psk(ssl, &hash_alg, &psk, &psk_len) == 0) {
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_parse_server_pre_shared_key_ext(mbedtls_ssl_context *ssl,
                                                     const unsigned char *buf,
                                                     const unsigned char *end)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    if (selected_identity >= esp_ssl_tls13_get_configured_psk_count(ssl)) {
...
#if defined(MBEDTLS_SSL_SESSION_TICKETS)
    if (selected_identity == 0 && esp_ssl_tls13_has_configured_ticket(ssl)) {
        ret = esp_ssl_tls13_ticket_get_psk(ssl, &hash_alg, &psk, &psk_len);
    } else
#endif
    if (esp_mbedtls_ssl_conf_has_static_psk(ssl->conf)) {
        ret = esp_ssl_tls13_psk_get_psk(ssl, &hash_alg, &psk, &psk_len);
    } else {
    ...
    if (esp_mbedtls_md_psa_alg_from_type((mbedtls_md_type_t) ssl->handshake->ciphersuite_info->mac)
        != hash_alg) {
        MBEDTLS_SSL_DEBUG_MSG(
    ...
    ret = esp_mbedtls_ssl_set_hs_psk(ssl, psk, psk_len);
    if (ret != 0) {
...
int esp_mbedtls_ssl_tls13_write_client_hello_exts(mbedtls_ssl_context *ssl,
                                              unsigned char *buf,
                                              unsigned char *end,
                                              size_t *out_len)
{
    ...
    *out_len = 0;
     ...
     */
    ret = esp_ssl_tls13_write_supported_versions_ext(ssl, p, end, &ext_len);
    if (ret != 0) {
        return ret;
    }
    p += ext_len;
     ...
     */
    ret = esp_ssl_tls13_write_cookie_ext(ssl, p, end, &ext_len);
    if (ret != 0) {
        return ret;
    }
    p += ext_len;
...
#if defined(MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_SOME_EPHEMERAL_ENABLED)
    if (esp_mbedtls_ssl_conf_tls13_is_some_ephemeral_enabled(ssl)) {
     ...
     */
    ret = esp_ssl_tls13_write_psk_key_exchange_modes_ext(ssl, p, end, &ext_len);
    if (ret != 0) {
        return ret;
    }
    p += ext_len;
...
int esp_mbedtls_ssl_tls13_finalize_client_hello(mbedtls_ssl_context *ssl)
{
    ((void) ssl);
    ...
    if (ssl->early_data_state == MBEDTLS_SSL_EARLY_DATA_STATE_IND_SENT) {
        ...
        if (ret != 0) {
            MBEDTLS_SSL_DEBUG_RET(
                1, "esp_ssl_tls13_ticket_get_psk", ret);
            return ret;
        }
         ...
         */
        ciphersuite_info = esp_mbedtls_ssl_ciphersuite_from_id(
        ...
        if (ret != 0) {
            MBEDTLS_SSL_DEBUG_RET(
                1, "esp_mbedtls_ssl_tls13_key_schedule_stage_early", ret);
            return ret;
        }
        ...
        esp_mbedtls_ssl_set_outbound_transform(
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_is_supported_versions_ext_present(
    ...
    const unsigned char *end)
{
    ...
    return esp_mbedtls_ssl_tls13_is_supported_versions_ext_present_in_exts(
        ssl, p, end,
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_is_downgrade_negotiation(mbedtls_ssl_context *ssl,
                                              const unsigned char *buf,
                                              const unsigned char *end)
{
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_server_hello_is_hrr(mbedtls_ssl_context *ssl,
                                   const unsigned char *buf,
                                   const unsigned char *end)
{
    ...
    MBEDTLS_SSL_CHK_BUF_READ_PTR(
        buf, end, 2 + sizeof(esp_mbedtls_ssl_tls13_hello_retry_request_magic));

    if (memcmp(buf + 2, esp_mbedtls_ssl_tls13_hello_retry_request_magic,
               sizeof(esp_mbedtls_ssl_tls13_hello_retry_request_magic)) == 0) {
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_preprocess_server_hello(mbedtls_ssl_context *ssl,
                                             const unsigned char *buf,
                                             const unsigned char *end)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    MBEDTLS_SSL_PROC_CHK_NEG(esp_ssl_tls13_is_supported_versions_ext_present(
                                 ssl, buf, end));

    if (ret == 0) {
        MBEDTLS_SSL_PROC_CHK_NEG(
            esp_ssl_tls13_is_downgrade_negotiation(ssl, buf, end));
    ...
    ret = esp_ssl_server_hello_is_hrr(ssl, buf, end);
    switch (ret) {
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_check_server_hello_session_id_echo(mbedtls_ssl_context *ssl,
                                                        const unsigned char **buf,
                                                        const unsigned char *end)
{
    const unsigned char *p = *buf;
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_parse_server_hello(mbedtls_ssl_context *ssl,
                                        const unsigned char *buf,
                                        const unsigned char *end,
                                        int is_hrr)
{
     ...
     * ...
     */
    if (esp_ssl_tls13_check_server_hello_session_id_echo(ssl, &p, end) != 0) {
    ...
    esp_mbedtls_ssl_optimize_checksum(ssl, ciphersuite_info);
    ...
    while (p < extensions_end) {
        ...
        switch (extension_type) {
            case MBEDTLS_TLS_EXT_COOKIE:
                ...
                ret = esp_ssl_tls13_parse_cookie_ext(ssl,
            ...
            case MBEDTLS_TLS_EXT_PRE_SHARED_KEY:
                ...
                if ((ret = esp_ssl_tls13_parse_server_pre_shared_key_ext(
                         ssl, p, extension_data_end)) != 0) {
                    MBEDTLS_SSL_DEBUG_RET(
                        1, ("esp_ssl_tls13_parse_server_pre_shared_key_ext"), ret);
                    return ret;
                }
                break;
#endif
            ...
            case MBEDTLS_TLS_EXT_KEY_SHARE:
                ...
                if (is_hrr) {
                    ret = esp_ssl_tls13_parse_hrr_key_share_ext(ssl,
                                                            p, extension_data_end);
                } else {
                    ret = esp_ssl_tls13_parse_key_share_ext(ssl,
                                                        p, extension_data_end);
                }
                if (ret != 0) {
                    MBEDTLS_SSL_DEBUG_RET(1,
...
static const char *esp_ssl_tls13_get_kex_mode_str(int mode)
{
    switch (mode) {
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_postprocess_server_hello(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    mbedtls_ssl_handshake_params *handshake = ssl->handshake;
    ...
    if (!esp_mbedtls_ssl_conf_tls13_is_kex_mode_enabled(
            ssl, handshake->key_exchange_mode)) {
        ret = MBEDTLS_ERR_SSL_HANDSHAKE_FAILURE;
        MBEDTLS_SSL_DEBUG_MSG(
            2, ("Key exchange mode(%s) is not supported.",
                esp_ssl_tls13_get_kex_mode_str(handshake->key_exchange_mode)));
        goto cleanup;
    }
    ...
    esp_mbedtls_ssl_set_inbound_transform(ssl, handshake->transform_handshake);
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_postprocess_hrr(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_process_server_hello(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    ret = esp_ssl_tls13_preprocess_server_hello(ssl, buf, buf + buf_len);
    if (ret < 0) {
        goto cleanup;
    } else {
    ...
    MBEDTLS_SSL_PROC_CHK(esp_ssl_tls13_parse_server_hello(ssl, buf,
                                                      buf + buf_len,
                                                      is_hrr));
    if (is_hrr) {
        MBEDTLS_SSL_PROC_CHK(esp_mbedtls_ssl_reset_transcript_for_hrr(ssl));
    }
    ...
    if (is_hrr) {
        MBEDTLS_SSL_PROC_CHK(esp_ssl_tls13_postprocess_hrr(ssl));
#if defined(MBEDTLS_SSL_TLS1_3_COMPATIBILITY_MODE)
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_parse_encrypted_extensions(mbedtls_ssl_context *ssl,
                                                const unsigned char *buf,
                                                const unsigned char *end)
{
    int ret = 0;
    ...
    while (p < extensions_end) {
        ...
        switch (extension_type) {
            ...
            case MBEDTLS_TLS_EXT_ALPN:
                ...
                if ((ret = esp_ssl_tls13_parse_alpn_ext(
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_process_encrypted_extensions(mbedtls_ssl_context *ssl)
{
    int ret;
    unsigned char *buf;
    size_t buf_len;
    ...
    MBEDTLS_SSL_PROC_CHK(
        esp_ssl_tls13_parse_encrypted_extensions(ssl, buf, buf + buf_len));
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_write_end_of_early_data(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    unsigned char *buf = NULL;
    size_t buf_len;
    ...
    MBEDTLS_SSL_PROC_CHK(esp_mbedtls_ssl_add_hs_hdr_to_checksum(
...
int esp_mbedtls_ssl_get_early_data_status(mbedtls_ssl_context *ssl)
{
    if ((ssl->conf->endpoint != MBEDTLS_SSL_IS_CLIENT) ||
        (!esp_mbedtls_ssl_is_handshake_over(ssl))) {
        return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
    }
...
/*
 * esp_ssl_tls13_parse_certificate_request()
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_parse_certificate_request(mbedtls_ssl_context *ssl,
                                               const unsigned char *buf,
                                               const unsigned char *end)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    if (certificate_request_context_len > 0) {
        ...
        handshake->certificate_request_context =
            esp_mbedtls_calloc(1, certificate_request_context_len);
    ...
    while (p < extensions_end) {
        ...
        switch (extension_type) {
            case MBEDTLS_TLS_EXT_SIG_ALG:
                ...
                ret = esp_mbedtls_ssl_parse_sig_alg_ext(ssl, p,
                                                    p + extension_data_len);
                if (ret != 0) {
                    return ret;
                }
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_process_certificate_request(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    if (ret == SSL_CERTIFICATE_REQUEST_EXPECT_REQUEST) {
        ...
        MBEDTLS_SSL_PROC_CHK(esp_ssl_tls13_parse_certificate_request(
                                 ssl, buf, buf + buf_len));
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_process_server_certificate(mbedtls_ssl_context *ssl)
{
    int ret;
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_process_certificate_verify(mbedtls_ssl_context *ssl)
{
    int ret;
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_process_server_finished(mbedtls_ssl_context *ssl)
{
    int ret;
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_write_client_certificate(mbedtls_ssl_context *ssl)
{
    int non_empty_certificate_msg = 0;

    MBEDTLS_SSL_DEBUG_MSG(1,
                          ("Switch to handshake traffic keys for outbound traffic"));
    esp_mbedtls_ssl_set_outbound_transform(ssl, ssl->handshake->transform_handshake);

#if defined(MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_EPHEMERAL_ENABLED)
    if (ssl->handshake->client_auth) {
        int ret = esp_mbedtls_ssl_tls13_write_certificate(ssl);
        if (ret != 0) {
            return ret;
        }
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_write_client_certificate_verify(mbedtls_ssl_context *ssl)
{
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_write_client_finished(mbedtls_ssl_context *ssl)
{
    int ret;
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_parse_new_session_ticket_early_data_ext(
    mbedtls_ssl_context *ssl,
    const unsigned char *buf,
    const unsigned char *end)
{
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_parse_new_session_ticket_exts(mbedtls_ssl_context *ssl,
                                                   const unsigned char *buf,
                                                   const unsigned char *end)
{
    mbedtls_ssl_handshake_params *handshake = ssl->handshake;
...
#if defined(MBEDTLS_SSL_EARLY_DATA)
            case MBEDTLS_TLS_EXT_EARLY_DATA:
                ret = esp_ssl_tls13_parse_new_session_ticket_early_data_ext(
                    ssl, p, p + extension_data_len);
                if (ret != 0) {
                    MBEDTLS_SSL_DEBUG_RET(
                        1, "esp_ssl_tls13_parse_new_session_ticket_early_data_ext",
                        ret);
                }
                break;
#endif /* MBEDTLS_SSL_EARLY_DATA */
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_parse_new_session_ticket(mbedtls_ssl_context *ssl,
                                              unsigned char *buf,
                                              unsigned char *end,
                                              ...
                                              size_t *ticket_nonce_len)
{
    ...
    *ticket_nonce = p;
    ...
    if ((ticket = esp_mbedtls_calloc(1, ticket_len)) == NULL) {
    ...
    ret = esp_ssl_tls13_parse_new_session_ticket_exts(ssl, p, p + extensions_len);
    if (ret != 0) {
        MBEDTLS_SSL_DEBUG_RET(1,
                              "esp_ssl_tls13_parse_new_session_ticket_exts",
                              ret);
        return ret;
    }
...
/* Non negative return values for esp_ssl_tls13_postprocess_new_session_ticket().
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_postprocess_new_session_ticket(mbedtls_ssl_context *ssl,
                                                    unsigned char *ticket_nonce,
                                                    size_t ticket_nonce_len)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    session->ticket_reception_time = esp_mbedtls_ms_time();
#endif

    ciphersuite_info = esp_mbedtls_ssl_ciphersuite_from_id(session->ciphersuite);
    if (ciphersuite_info == NULL) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("should never happen"));
        return MBEDTLS_ERR_SSL_INTERNAL_ERROR;
    }
 ...
 */
MBEDTLS_CHECK_RETURN_CRITICAL
static int esp_ssl_tls13_process_new_session_ticket(mbedtls_ssl_context *ssl)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    unsigned char *buf;
    size_t buf_len;
    ...
    MBEDTLS_SSL_PROC_CHK(esp_ssl_tls13_parse_new_session_ticket(
                             ssl, buf, buf + buf_len,
                             &ticket_nonce, &ticket_nonce_len));

    MBEDTLS_SSL_PROC_CHK_NEG(esp_ssl_tls13_postprocess_new_session_ticket(
...
int esp_mbedtls_ssl_tls13_handshake_client_step(mbedtls_ssl_context *ssl)
{
    int ret = 0;
    ...
    switch (ssl->state) {
        ...
        case MBEDTLS_SSL_SERVER_HELLO:
            ret = esp_ssl_tls13_process_server_hello(ssl);
            break;

        case MBEDTLS_SSL_ENCRYPTED_EXTENSIONS:
            ret = esp_ssl_tls13_process_encrypted_extensions(ssl);
            break;

#if defined(MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_EPHEMERAL_ENABLED)
        case MBEDTLS_SSL_CERTIFICATE_REQUEST:
            ret = esp_ssl_tls13_process_certificate_request(ssl);
            break;

        case MBEDTLS_SSL_SERVER_CERTIFICATE:
            ret = esp_ssl_tls13_process_server_certificate(ssl);
            break;

        case MBEDTLS_SSL_CERTIFICATE_VERIFY:
            ret = esp_ssl_tls13_process_certificate_verify(ssl);
            break;
#endif /* MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_EPHEMERAL_ENABLED */

        case MBEDTLS_SSL_SERVER_FINISHED:
            ret = esp_ssl_tls13_process_server_finished(ssl);
            break;

#if defined(MBEDTLS_SSL_EARLY_DATA)
        case MBEDTLS_SSL_END_OF_EARLY_DATA:
            ret = esp_ssl_tls13_write_end_of_early_data(ssl);
            break;
#endif

        case MBEDTLS_SSL_CLIENT_CERTIFICATE:
            ret = esp_ssl_tls13_write_client_certificate(ssl);
            break;

#if defined(MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_EPHEMERAL_ENABLED)
        case MBEDTLS_SSL_CLIENT_CERTIFICATE_VERIFY:
            ret = esp_ssl_tls13_write_client_certificate_verify(ssl);
            break;
#endif /* MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_EPHEMERAL_ENABLED */

        case MBEDTLS_SSL_CLIENT_FINISHED:
            ret = esp_ssl_tls13_write_client_finished(ssl);
            break;

        case MBEDTLS_SSL_FLUSH_BUFFERS:
            ret = esp_ssl_tls13_flush_buffers(ssl);
            break;
        ...
        case MBEDTLS_SSL_TLS1_3_NEW_SESSION_TICKET:
            ret = esp_ssl_tls13_process_new_session_ticket(ssl);
            break;
#endif /* MBEDTLS_SSL_SESSION_TICKETS */
