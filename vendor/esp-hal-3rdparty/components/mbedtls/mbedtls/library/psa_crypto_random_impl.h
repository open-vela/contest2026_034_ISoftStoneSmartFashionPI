...
/* The maximum number of bytes that esp_mbedtls_psa_get_random() is expected to return. */
#if defined(MBEDTLS_CTR_DRBG_C)
...
#elif defined(MBEDTLS_HMAC_DRBG_C)
    esp_mbedtls_hmac_drbg_init(p_rng);
#endif
}
...
#elif defined(MBEDTLS_HMAC_DRBG_C)
    esp_mbedtls_hmac_drbg_free(p_rng);
#endif
}
 ...
 */
static inline int esp_mbedtls_psa_drbg_seed(mbedtls_psa_drbg_context_t *drbg_ctx,
                                        mbedtls_entropy_context *entropy,
                                        const unsigned char *custom, size_t len)
{
#if defined(MBEDTLS_CTR_DRBG_C)
    return esp_mbedtls_ctr_drbg_seed(drbg_ctx, esp_mbedtls_entropy_func, entropy, custom, len);
#elif defined(MBEDTLS_HMAC_DRBG_C)
    const mbedtls_md_info_t *md_info = esp_mbedtls_md_info_from_type(MBEDTLS_PSA_HMAC_DRBG_MD_TYPE);
    return esp_mbedtls_hmac_drbg_seed(drbg_ctx, md_info, esp_mbedtls_entropy_func, entropy, custom, len);
#endif
}
