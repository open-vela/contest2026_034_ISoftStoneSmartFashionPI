 ...
 *                      the constant time interface (e.g. `esp_mbedtls_ct_bool()`)
 ...
 *               large enough to hold the value of \p A.
 */
int esp_mbedtls_mpi_core_write_be(const mbedtls_mpi_uint *A,
                              size_t A_limbs,
                              unsigned char *output,
                              size_t output_length);
 ...
 */
mbedtls_mpi_uint esp_mbedtls_mpi_core_mla(mbedtls_mpi_uint *X, size_t X_limbs,
 ...
 *                          This can be calculated by `esp_mbedtls_mpi_core_montmul_init()`.
 ...
 */
int esp_mbedtls_mpi_core_fill_random(mbedtls_mpi_uint *X, size_t X_limbs,
                                 size_t bytes,
 ...
 *                   by `esp_mbedtls_mpi_core_exp_mod_working_limbs()`.
...
/**
 * \brief          Returns the number of limbs of working memory required for
 *                 a call to `esp_mbedtls_mpi_core_montmul()`.
 *
 * \param AN_limbs The number of limbs in the input `A` and the modulus `N`
 *                 (they must be the same size) that will be given to
 *                 `esp_mbedtls_mpi_core_montmul()` or one of the other functions
 ...
 *
 * This function is a thin wrapper around `esp_mbedtls_mpi_core_montmul()` that is
 * an alternative to calling `esp_mbedtls_mpi_mod_raw_to_mont_rep()` when we
 ...
 *
 * This function is a thin wrapper around `esp_mbedtls_mpi_core_montmul()` that is
 * an alternative to calling `esp_mbedtls_mpi_mod_raw_from_mont_rep()` when we
 * don't want to allocate memory.
 *
 * \param[out]    X         The result of the conversion.
 *                          Must have the same number of limbs as \p A.
