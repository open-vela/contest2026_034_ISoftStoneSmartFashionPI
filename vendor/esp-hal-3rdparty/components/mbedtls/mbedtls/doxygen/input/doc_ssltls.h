 ...
 * - initialise an SSL/TLS context (see \c esp_mbedtls_ssl_init()).
 * - perform an SSL/TLS handshake (see \c esp_mbedtls_ssl_handshake()).
 * - read/write (see \c esp_mbedtls_ssl_read() and \c esp_mbedtls_ssl_write()).
 * - notify a peer that connection is being closed (see \c esp_mbedtls_ssl_close_notify()).
 *
