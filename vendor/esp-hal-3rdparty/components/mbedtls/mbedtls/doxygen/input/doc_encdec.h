 ...
 * (see \c esp_mbedtls_cipher_setup()).
 *
 * The asymmetric encryption algorithms are accessible via the generic public
 * key layer (see \c esp_mbedtls_pk_init()).
 *
 * The following algorithms are provided:
 * - Symmetric:
 *   - AES (see \c esp_mbedtls_aes_crypt_ecb(), \c esp_mbedtls_aes_crypt_cbc(), \c esp_mbedtls_aes_crypt_cfb128() and
 *     \c esp_mbedtls_aes_crypt_ctr()).
 *   - Camellia (see \c esp_mbedtls_camellia_crypt_ecb(), \c esp_mbedtls_camellia_crypt_cbc(),
 *     \c esp_mbedtls_camellia_crypt_cfb128() and \c esp_mbedtls_camellia_crypt_ctr()).
 *   - DES/3DES (see \c esp_mbedtls_des_crypt_ecb(), \c esp_mbedtls_des_crypt_cbc(), \c esp_mbedtls_des3_crypt_ecb()
 *     and \c esp_mbedtls_des3_crypt_cbc()).
 *   - GCM (AES-GCM and CAMELLIA-GCM) (see \c esp_mbedtls_gcm_init())
 * - Asymmetric:
 *   - Diffie-Hellman-Merkle (see \c esp_mbedtls_dhm_read_public(), \c esp_mbedtls_dhm_make_public()
 *     and \c esp_mbedtls_dhm_calc_secret()).
 *   - RSA (see \c esp_mbedtls_rsa_public() and \c esp_mbedtls_rsa_private()).
 *   - Elliptic Curves over GF(p) (see \c esp_mbedtls_ecp_point_init()).
 *   - Elliptic Curve Digital Signature Algorithm (ECDSA) (see \c esp_mbedtls_ecdsa_init()).
 *   - Elliptic Curve Diffie Hellman (ECDH) (see \c esp_mbedtls_ecdh_init()).
 *
