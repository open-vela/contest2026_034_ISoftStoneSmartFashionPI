 ...
 *
 * If the application calls other functions before calling esp_psa_crypto_init(),
 ...
 * esp_psa_import_key(), esp_psa_generate_key(), esp_psa_generate_key_ext(),
 * esp_psa_key_derivation_output_key(), esp_psa_key_derivation_output_key_ext()
 * or esp_psa_copy_key().
 *
 ...
 * esp_psa_reset_key_attributes(). It then copies the attributes of
 * the given key into the given attribute structure.
 *
 * \note This function may allocate memory or other resources.
 *       Once you have called this function on an attribute structure,
 *       you must call esp_psa_reset_key_attributes() to free these resources.
 *
 ...
 *         The library has not been previously initialized by esp_psa_crypto_init().
 ...
 */
void esp_psa_reset_key_attributes(psa_key_attributes_t *attributes);
 ...
 *         results in this error code.
 */
psa_status_t esp_psa_purge_key(mbedtls_svc_key_id_t key);
 ...
 *
 * This function supports any output from esp_psa_export_key(). Refer to the
 * documentation of esp_psa_export_public_key() for the format of public keys
 * and to the documentation of esp_psa_export_key() for the format for
 * other key types.
 *
 ...
 *                    of esp_psa_export_key() or esp_psa_export_public_key() for
 ...
 *
 * The output of this function can be passed to esp_psa_import_key() to
 * create an equivalent object.
 *
 * If the implementation of esp_psa_import_key() supports other formats
 * beyond the format specified here, the output from esp_psa_export_key()
 ...
 *   true), the format is the same as for esp_psa_export_public_key().
 *
 ...
 *       expected value, use esp_psa_hash_compare() instead.
 *
 ...
 * -# Call esp_psa_hash_setup() to specify the algorithm.
 * -# Call esp_psa_hash_update() zero, one or more times, passing a fragment
 *    of the message each time. The hash that is calculated is the hash
 *    of the concatenation of these messages in order.
 * -# To calculate the hash, call esp_psa_hash_finish().
 *    To compare the hash with an expected value, call esp_psa_hash_verify().
 *
 * If an error occurs at any step after a call to esp_psa_hash_setup(), the
 * operation will need to be reset by a call to esp_psa_hash_abort(). The
 * application may call esp_psa_hash_abort() at any time after the operation
 * has been initialized.
 *
 * After a successful call to esp_psa_hash_setup(), the application must
 * eventually terminate the operation. The following events terminate an
 * operation:
 * - A successful call to esp_psa_hash_finish() or esp_psa_hash_verify().
 * - A call to esp_psa_hash_abort().
 *
 ...
 *
 * The application must call esp_psa_hash_setup() before calling this function.
 *
 * If this function returns an error status, the operation enters an error
 * state and must be aborted by calling esp_psa_hash_abort().
 *
 ...
 * the inputs passed to preceding calls to esp_psa_hash_update().
 *
 ...
 *          a specific value for the hash. Call esp_psa_hash_verify() instead.
 ...
 * The application must call esp_psa_hash_setup() before calling this function.
 * This function calculates the hash of the message formed by concatenating
 * the inputs passed to preceding calls to esp_psa_hash_update(). It then
 ...
 * esp_psa_hash_setup() again.
 *
 * You may call this function any time after the operation object has
 * been initialized by one of the methods described in #psa_hash_operation_t.
 *
 * In particular, calling esp_psa_hash_abort() after the operation has been
 * terminated by a call to esp_psa_hash_abort(), esp_psa_hash_finish() or
 * esp_psa_hash_verify() is safe and has no effect.
 *
 ...
 * to calling esp_psa_hash_setup() on \p target_operation with the same
 * algorithm that \p source_operation was set up for, then
 * esp_psa_hash_update() on \p target_operation with the same input that
 ...
 *       expected value, use esp_psa_mac_verify() instead.
 *       Beware that comparing integrity or authenticity data such as
 ...
 * expected value, use esp_psa_mac_verify_setup() instead.
 *
 ...
 * -# Call esp_psa_mac_sign_setup() to specify the algorithm and key.
 * -# Call esp_psa_mac_update() zero, one or more times, passing a fragment
 *    of the message each time. The MAC that is calculated is the MAC
 *    of the concatenation of these messages in order.
 * -# At the end of the message, call esp_psa_mac_sign_finish() to finish
 *    calculating the MAC value and retrieve it.
 *
 * If an error occurs at any step after a call to esp_psa_mac_sign_setup(), the
 * operation will need to be reset by a call to esp_psa_mac_abort(). The
 * application may call esp_psa_mac_abort() at any time after the operation
 * has been initialized.
 *
 * After a successful call to esp_psa_mac_sign_setup(), the application must
 * eventually terminate the operation through one of the following methods:
 * - A successful call to esp_psa_mac_sign_finish().
 * - A call to esp_psa_mac_abort().
 *
 * \param[in,out] operation The operation object to set up. It must have
 *                          been initialized as per the documentation for
 ...
 * -# Initialize the operation object with one of the methods described in the
 *    documentation for #psa_mac_operation_t, e.g. #PSA_MAC_OPERATION_INIT.
 * -# Call esp_psa_mac_verify_setup() to specify the algorithm and key.
 * -# Call esp_psa_mac_update() zero, one or more times, passing a fragment
 *    of the message each time. The MAC that is calculated is the MAC
 *    of the concatenation of these messages in order.
 * -# At the end of the message, call esp_psa_mac_verify_finish() to finish
 *    calculating the actual MAC of the message and verify it against
 *    the expected value.
 *
 * If an error occurs at any step after a call to esp_psa_mac_verify_setup(), the
 * operation will need to be reset by a call to esp_psa_mac_abort(). The
 * application may call esp_psa_mac_abort() at any time after the operation
 * has been initialized.
 *
 * After a successful call to esp_psa_mac_verify_setup(), the application must
 * eventually terminate the operation through one of the following methods:
 * - A successful call to esp_psa_mac_verify_finish().
 * - A call to esp_psa_mac_abort().
 *
 * \param[in,out] operation The operation object to set up. It must have
 *                          been initialized as per the documentation for
 ...
 *
 * The application must call esp_psa_mac_sign_setup() or esp_psa_mac_verify_setup()
 * before calling this function.
 *
 * If this function returns an error status, the operation enters an error
 * state and must be aborted by calling esp_psa_mac_abort().
 *
 ...
 *
 * The application must call esp_psa_mac_sign_setup() before calling this function.
 * This function calculates the MAC of the message formed by concatenating
 * the inputs passed to preceding calls to esp_psa_mac_update().
 *
 * When this function returns successfully, the operation becomes inactive.
 * If this function returns an error status, the operation enters an error
 * state and must be aborted by calling esp_psa_mac_abort().
 *
 * \warning Applications should not call this function if they expect
 *          a specific value for the MAC. Call esp_psa_mac_verify_finish() instead.
 *          Beware that comparing integrity or authenticity data such as
 *          MAC values with a function such as \c memcmp is risky
 *          because the time taken by the comparison may leak information
 *          about the MAC value which could allow an attacker to guess
 ...
 * an expected value.
 *
 * The application must call esp_psa_mac_verify_setup() before calling this function.
 * This function calculates the MAC of the message formed by concatenating
 * the inputs passed to preceding calls to esp_psa_mac_update(). It then
 ...
 * \p operation structure itself. Once aborted, the operation object
 * can be reused for another operation by calling
 * esp_psa_mac_sign_setup() or esp_psa_mac_verify_setup() again.
 *
 * You may call this function any time after the operation object has
 * been initialized by one of the methods described in #psa_mac_operation_t.
 *
 * In particular, calling esp_psa_mac_abort() after the operation has been
 * terminated by a call to esp_psa_mac_abort(), esp_psa_mac_sign_finish() or
 * esp_psa_mac_verify_finish() is safe and has no effect.
 *
 ...
 * -# Call esp_psa_cipher_encrypt_setup() to specify the algorithm and key.
 * -# Call either esp_psa_cipher_generate_iv() or esp_psa_cipher_set_iv() to
 *    generate or set the IV (initialization vector). You should use
 *    esp_psa_cipher_generate_iv() unless the protocol you are implementing
 *    requires a specific IV value.
 * -# Call esp_psa_cipher_update() zero, one or more times, passing a fragment
 *    of the message each time.
 * -# Call esp_psa_cipher_finish().
 *
 * If an error occurs at any step after a call to esp_psa_cipher_encrypt_setup(),
 * the operation will need to be reset by a call to esp_psa_cipher_abort(). The
 * application may call esp_psa_cipher_abort() at any time after the operation
 * has been initialized.
 *
 * After a successful call to esp_psa_cipher_encrypt_setup(), the application must
 * eventually terminate the operation. The following events terminate an
 * operation:
 * - A successful call to esp_psa_cipher_finish().
 * - A call to esp_psa_cipher_abort().
 *
 * \param[in,out] operation     The operation object to set up. It must have
 *                              been initialized as per the documentation for
 ...
 *    documentation for #psa_cipher_operation_t, e.g.
 *    #PSA_CIPHER_OPERATION_INIT.
 * -# Call esp_psa_cipher_decrypt_setup() to specify the algorithm and key.
 * -# Call esp_psa_cipher_set_iv() with the IV (initialization vector) for the
 *    decryption. If the IV is prepended to the ciphertext, you can call
 *    esp_psa_cipher_update() on a buffer containing the IV followed by the
 *    beginning of the message.
 * -# Call esp_psa_cipher_update() zero, one or more times, passing a fragment
 *    of the message each time.
 * -# Call esp_psa_cipher_finish().
 *
 * If an error occurs at any step after a call to esp_psa_cipher_decrypt_setup(),
 * the operation will need to be reset by a call to esp_psa_cipher_abort(). The
 * application may call esp_psa_cipher_abort() at any time after the operation
 * has been initialized.
 *
 * After a successful call to esp_psa_cipher_decrypt_setup(), the application must
 * eventually terminate the operation. The following events terminate an
 * operation:
 * - A successful call to esp_psa_cipher_finish().
 * - A call to esp_psa_cipher_abort().
 ...
 *
 * The application must call esp_psa_cipher_encrypt_setup() before
 * calling this function.
 *
 * If this function returns an error status, the operation enters an error
 * state and must be aborted by calling esp_psa_cipher_abort().
 *
 ...
 * state and must be aborted by calling esp_psa_cipher_abort().
 *
 * \note When encrypting, applications should use esp_psa_cipher_generate_iv()
 ...
 *         previously initialized by esp_psa_crypto_init().
 *         It is implementation-dependent whether a failure to initialize
 *         results in this error code.
 */
 ...
 * 1. Call either esp_psa_cipher_encrypt_setup() or esp_psa_cipher_decrypt_setup().
 *    The choice of setup function determines whether this function
 *    encrypts or decrypts its input.
 * 2. If the algorithm requires an IV, call esp_psa_cipher_generate_iv()
 *    (recommended when encrypting) or esp_psa_cipher_set_iv().
 *
 * If this function returns an error status, the operation enters an error
 * state and must be aborted by calling esp_psa_cipher_abort().
 *
 ...
 *
 * The application must call esp_psa_cipher_encrypt_setup() or
 * esp_psa_cipher_decrypt_setup() before calling this function. The choice
 * of setup function determines whether this function encrypts or
 * decrypts its input.
 *
 * This function finishes the encryption or decryption of the message
 * formed by concatenating the inputs passed to preceding calls to
 * esp_psa_cipher_update().
 *
 * When this function returns successfully, the operation becomes inactive.
 * If this function returns an error status, the operation enters an error
 * state and must be aborted by calling esp_psa_cipher_abort().
 ...
 * \p operation structure itself. Once aborted, the operation object
 * can be reused for another operation by calling
 * esp_psa_cipher_encrypt_setup() or esp_psa_cipher_decrypt_setup() again.
 *
 * You may call this function any time after the operation object has
 * been initialized as described in #psa_cipher_operation_t.
 *
 * In particular, calling esp_psa_cipher_abort() after the operation has been
 * terminated by a call to esp_psa_cipher_abort() or esp_psa_cipher_finish()
 ...
 * -# Call esp_psa_aead_encrypt_setup() to specify the algorithm and key.
 * -# If needed, call esp_psa_aead_set_lengths() to specify the length of the
 *    inputs to the subsequent calls to esp_psa_aead_update_ad() and
 *    esp_psa_aead_update(). See the documentation of esp_psa_aead_set_lengths()
 *    for details.
 * -# Call either esp_psa_aead_generate_nonce() or esp_psa_aead_set_nonce() to
 *    generate or set the nonce. You should use
 *    esp_psa_aead_generate_nonce() unless the protocol you are implementing
 *    requires a specific nonce value.
 * -# Call esp_psa_aead_update_ad() zero, one or more times, passing a fragment
 *    of the non-encrypted additional authenticated data each time.
 * -# Call esp_psa_aead_update() zero, one or more times, passing a fragment
 *    of the message to encrypt each time.
 * -# Call esp_psa_aead_finish().
 *
 * If an error occurs at any step after a call to esp_psa_aead_encrypt_setup(),
 * the operation will need to be reset by a call to esp_psa_aead_abort(). The
 * application may call esp_psa_aead_abort() at any time after the operation
 * has been initialized.
 *
 * After a successful call to esp_psa_aead_encrypt_setup(), the application must
 * eventually terminate the operation. The following events terminate an
 * operation:
 * - A successful call to esp_psa_aead_finish().
 * - A call to esp_psa_aead_abort().
 *
 * \param[in,out] operation     The operation object to set up. It must have
 *                              been initialized as per the documentation for
 ...
 *    documentation for #psa_aead_operation_t, e.g.
 *    #PSA_AEAD_OPERATION_INIT.
 * -# Call esp_psa_aead_decrypt_setup() to specify the algorithm and key.
 * -# If needed, call esp_psa_aead_set_lengths() to specify the length of the
 *    inputs to the subsequent calls to esp_psa_aead_update_ad() and
 *    esp_psa_aead_update(). See the documentation of esp_psa_aead_set_lengths()
 *    for details.
 * -# Call esp_psa_aead_set_nonce() with the nonce for the decryption.
 * -# Call esp_psa_aead_update_ad() zero, one or more times, passing a fragment
 *    of the non-encrypted additional authenticated data each time.
 * -# Call esp_psa_aead_update() zero, one or more times, passing a fragment
 *    of the ciphertext to decrypt each time.
 * -# Call esp_psa_aead_verify().
 *
 * If an error occurs at any step after a call to esp_psa_aead_decrypt_setup(),
 * the operation will need to be reset by a call to esp_psa_aead_abort(). The
 * application may call esp_psa_aead_abort() at any time after the operation
 * has been initialized.
 *
 * After a successful call to esp_psa_aead_decrypt_setup(), the application must
 * eventually terminate the operation. The following events terminate an
 * operation:
 * - A successful call to esp_psa_aead_verify().
 * - A call to esp_psa_aead_abort().
 *
 * \param[in,out] operation     The operation object to set up. It must have
 *                              been initialized as per the documentation for
 ...
 *         library has not been previously initialized by esp_psa_crypto_init().
 *         It is implementation-dependent whether a failure to initialize
 *         results in this error code.
 */
 ...
 * and key size.
 *
 * The application must call esp_psa_aead_encrypt_setup() before
 * calling this function.
 *
 * If this function returns an error status, the operation enters an error
 * state and must be aborted by calling esp_psa_aead_abort().
 *
 ...
 *
 * The application must call esp_psa_aead_encrypt_setup() or
 * esp_psa_aead_decrypt_setup() before calling this function.
 *
 * If this function returns an error status, the operation enters an error
 * state and must be aborted by calling esp_psa_aead_abort().
 *
 * \note When encrypting, applications should use esp_psa_aead_generate_nonce()
 * instead of this function, unless implementing a protocol that requires
 * a non-random IV.
 *
 * \param[in,out] operation     Active AEAD operation.
 ...
 * esp_psa_aead_update_ad() or esp_psa_aead_update() if the algorithm for
 * the operation requires it. If the algorithm does not require it,
 * calling this function is optional, but if this function is called
 * then the implementation must enforce the lengths.
 *
 * You may call this function before or after setting the nonce with
 * esp_psa_aead_set_nonce() or esp_psa_aead_generate_nonce().
 *
 ...
 *         esp_psa_aead_update_ad() and esp_psa_aead_update() must not have been
 ...
 * data to encrypt or decrypt with esp_psa_aead_update().
 *
 * Before calling this function, you must:
 * 1. Call either esp_psa_aead_encrypt_setup() or esp_psa_aead_decrypt_setup().
 * 2. Set the nonce with esp_psa_aead_generate_nonce() or esp_psa_aead_set_nonce().
 *
 * If this function returns an error status, the operation enters an error
 * state and must be aborted by calling esp_psa_aead_abort().
 *
 * \warning When decrypting, until esp_psa_aead_verify() has returned #PSA_SUCCESS,
 *          there is no guarantee that the input is valid. Therefore, until
 *          you have called esp_psa_aead_verify() and it has returned #PSA_SUCCESS,
 *          treat the input as untrusted and prepare to undo any action that
 *          depends on the input if esp_psa_aead_verify() returns an error status.
 *
 * \param[in,out] operation     Active AEAD operation.
 ...
 *         was previously specified with esp_psa_aead_set_lengths().
 * \retval #PSA_ERROR_INSUFFICIENT_MEMORY \emptydescription
 * \retval #PSA_ERROR_COMMUNICATION_FAILURE \emptydescription
 * \retval #PSA_ERROR_HARDWARE_FAILURE \emptydescription
 * \retval #PSA_ERROR_CORRUPTION_DETECTED \emptydescription
 ...
 *         esp_psa_aead_update() must not have been called yet), or the library
 *         has not been previously initialized by esp_psa_crypto_init().
 *         It is implementation-dependent whether a failure to initialize
 *         results in this error code.
 */
 ...
 *    encrypts or decrypts its input.
 * 2. Set the nonce with esp_psa_aead_generate_nonce() or esp_psa_aead_set_nonce().
 * 3. Call esp_psa_aead_update_ad() to pass all the additional data.
 *
 * If this function returns an error status, the operation enters an error
 * state and must be aborted by calling esp_psa_aead_abort().
 *
 ...
 * call to esp_psa_aead_update(), esp_psa_aead_finish() or esp_psa_aead_verify()
 ...
 *         determine the required buffer size.
 * \retval #PSA_ERROR_INVALID_ARGUMENT
 *         The total length of input to esp_psa_aead_update_ad() so far is
 *         less than the additional data length that was previously
 *         specified with esp_psa_aead_set_lengths(), or
 ...
 *
 * The operation must have been set up with esp_psa_aead_encrypt_setup().
 *
 * This function finishes the authentication of the additional data
 * formed by concatenating the inputs passed to preceding calls to
 * esp_psa_aead_update_ad() with the plaintext formed by concatenating the
 * inputs passed to preceding calls to esp_psa_aead_update().
 *
 * This function has two output buffers:
 * - \p ciphertext contains trailing ciphertext that was buffered from
 *   preceding calls to esp_psa_aead_update().
 ...
 *         less than the additional data length that was previously
 *         specified with esp_psa_aead_set_lengths(), or
 *         the total length of input to esp_psa_aead_update() so far is
 *         less than the plaintext length that was previously
 *         specified with esp_psa_aead_set_lengths().
 * \retval #PSA_ERROR_INSUFFICIENT_MEMORY \emptydescription
 * \retval #PSA_ERROR_COMMUNICATION_FAILURE \emptydescription
 * \retval #PSA_ERROR_HARDWARE_FAILURE \emptydescription
 * \retval #PSA_ERROR_CORRUPTION_DETECTED \emptydescription
 ...
 *         initialized by esp_psa_crypto_init().
 *         It is implementation-dependent whether a failure to initialize
 *         results in this error code.
 */
 ...
 *
 * The operation must have been set up with esp_psa_aead_decrypt_setup().
 *
 * This function finishes the authenticated decryption of the message
 * components:
 *
 * -  The additional data consisting of the concatenation of the inputs
 *    passed to preceding calls to esp_psa_aead_update_ad().
 ...
 *                              from previous calls to esp_psa_aead_update()
 ...
 * \p operation structure itself. Once aborted, the operation object
 * can be reused for another operation by calling
 * esp_psa_aead_encrypt_setup() or esp_psa_aead_decrypt_setup() again.
 *
 * You may call this function any time after the operation object has
 * been initialized as described in #psa_aead_operation_t.
 *
 * In particular, calling esp_psa_aead_abort() after the operation has been
 * terminated by a call to esp_psa_aead_abort(), esp_psa_aead_finish() or
 * esp_psa_aead_verify() is safe and has no effect.
 *
 ...
 *       esp_psa_sign_hash(). PSA_ALG_GET_HASH(\p alg) can be used to determine the
 ...
 *       and then pass the resulting hash to esp_psa_verify_hash().
 ...
 * first calculate the hash by calling esp_psa_hash_setup(), esp_psa_hash_update()
 * and esp_psa_hash_finish(), or alternatively by calling esp_psa_hash_compute().
 ...
 * -# Call esp_psa_key_derivation_setup() to select the algorithm.
 * -# Provide the inputs for the key derivation by calling
 *    esp_psa_key_derivation_input_bytes() or esp_psa_key_derivation_input_key()
 *    as appropriate. Which inputs are needed, in what order, and whether
 *    they may be keys and if so of what type depends on the algorithm.
 * -# Optionally set the operation's maximum capacity with
 *    esp_psa_key_derivation_set_capacity(). You may do this before, in the middle
 *    of or after providing inputs. For some algorithms, this step is mandatory
 *    because the output depends on the maximum capacity.
 * -# To derive a key, call esp_psa_key_derivation_output_key() or
 *    esp_psa_key_derivation_output_key_ext().
 *    To derive a byte string for a different purpose, call
 *    esp_psa_key_derivation_output_bytes().
 *    Successive calls to these functions use successive output bytes
 *    calculated by the key derivation algorithm.
 * -# Clean up the key derivation operation object with
 *    esp_psa_key_derivation_abort().
 *
 * If this function returns an error, the key derivation operation object is
 * not changed.
 *
 * If an error occurs at any step after a call to esp_psa_key_derivation_setup(),
 * the operation will need to be reset by a call to esp_psa_key_derivation_abort().
 *
 ...
 *         results in this error code.
 */
psa_status_t esp_psa_key_derivation_setup(
    psa_key_derivation_operation_t *operation,
    psa_algorithm_t alg);
 ...
 *         results in this error code.
 */
psa_status_t esp_psa_key_derivation_get_capacity(
 ...
 *         results in this error code.
 */
psa_status_t esp_psa_key_derivation_set_capacity(
    psa_key_derivation_operation_t *operation,
    size_t capacity);
 ...
 * object, call esp_psa_key_derivation_input_key() instead of this function.
 ...
 *
 * If this function returns an error status, the operation enters an error
 * state and must be aborted by calling esp_psa_key_derivation_abort().
 *
 * \param[in,out] operation       The key derivation operation object to use.
 *                                It must have been set up with
 *                                esp_psa_key_derivation_setup() and must not
 ...
 * call esp_psa_key_derivation_input_bytes() instead of this function.
 * Refer to the documentation of individual step types
 * (`PSA_KEY_DERIVATION_INPUT_xxx` values of type ::psa_key_derivation_step_t)
 * for more information.
 *
 ...
 * - esp_psa_key_derivation_output_bytes() if each input was either a direct input
 *   or  a key with #PSA_KEY_USAGE_DERIVE set;
 * - esp_psa_key_derivation_output_key() or esp_psa_key_derivation_output_key_ext()
 ...
 * \param[in,out] operation       The key derivation operation object to use.
 *                                It must have been set up with
 *                                esp_psa_key_derivation_setup() with a
 ...
 *                          same format that esp_psa_import_key() accepts for the
 ...
 *         or the library has not been previously initialized by esp_psa_crypto_init().
 *         It is implementation-dependent whether a failure to initialize
 *         results in this error code.
 */
psa_status_t esp_psa_key_derivation_key_agreement(
    psa_key_derivation_operation_t *operation,
    psa_key_derivation_step_t step,
 ...
 *   calling #esp_psa_key_derivation_output_bytes
 *   and passing the resulting output to #esp_psa_import_key.
 ...
 *   for the output produced by esp_psa_export_key().
 *   The following key types defined in this specification follow this scheme:
 *
 *     - #PSA_KEY_TYPE_DES.
 ...
 * the input to that step must be provided with esp_psa_key_derivation_input_key().
 ...
 *                          esp_psa_key_derivation_output_key().
 ...
 * \code
 * esp_psa_key_derivation_output_bytes(operation, tmp, output_length);
 ...
 *                          esp_psa_key_derivation_output_key() or
 *                          esp_psa_key_derivation_output_key_ext().
 *
 * \retval #PSA_SUCCESS \emptydescription
 * \retval #PSA_ERROR_INVALID_SIGNATURE
 ...
 * for another operation by calling esp_psa_key_derivation_setup() again.
 *
 * This function may be called at any time after the operation
 * object has been initialized as described in #psa_key_derivation_operation_t.
 *
 * In particular, it is valid to call esp_psa_key_derivation_abort() twice, or to
 * call esp_psa_key_derivation_abort() on an operation that has not been set up.
 *
 ...
 *         results in this error code.
 */
psa_status_t esp_psa_key_derivation_abort(
    psa_key_derivation_operation_t *operation);
 ...
 * a key derivation, use esp_psa_key_derivation_key_agreement() and other
 ...
 *                                in the same format that esp_psa_import_key()
 *                                accepts. The standard formats for public
 *                                keys are documented in the documentation
 *                                of esp_psa_export_public_key().
 * \param peer_key_length         Size of \p peer_key in bytes.
 * \param[out] output             Buffer where the decrypted message is to
 *                                be written.
 * \param output_size             Size of the \c output buffer in bytes.
 ...
 *
 * \note    To generate a key, use esp_psa_generate_key() instead.
 *
 ...
 *         results in this error code.
 */
psa_status_t esp_psa_generate_random(uint8_t *output,
                                 size_t output_size);
 ...
 *   attributes.
 *
 * \note This function is equivalent to calling esp_psa_generate_key_ext()
 *       with the production parameters #PSA_KEY_PRODUCTION_PARAMETERS_INIT
 *       and `params_data_length == 0` (i.e. `params->data` is empty).
 *
 * \param[in] attributes    The attributes for the new key.
 ...
 *
 * See the description of esp_psa_generate_key() for the operation of this
 ...
 *                              documentation for \c esp_mbedtls_ecp_set_max_ops(),
 ...
 *                              value set by
 *                              \c esp_psa_interruptible_set_max_ops() or
 ...
 *                              value passed to \c
 *                              esp_psa_interruptible_set_max_ops().
 *
 ...
 *
 * \see                         \c esp_psa_sign_hash_complete()
 *
 * \warning                     This is a beta API, and thus subject to change
 *                              at any point. It is not bound by the usual
 *                              interface stability promises.
 ...
 *                              esp_psa_sign_hash_complete() is equivalent to
 *                              \c esp_psa_sign_hash() but
 *                              \c esp_psa_sign_hash_complete() can return early and
 *                              resume according to the limit set with \c
 *                              esp_psa_interruptible_set_max_ops() to reduce the
 *                              maximum time spent in a function call.
 *
 * \note                        Users should call \c esp_psa_sign_hash_complete()
 *                              repeatedly on the same context after a
 *                              successful call to this function until \c
 *                              esp_psa_sign_hash_complete() either returns 0 or an
 *                              error. \c esp_psa_sign_hash_complete() will return
 *                              #PSA_OPERATION_INCOMPLETE if there is more work
 *                              to do. Alternatively users can call
 *                              \c esp_psa_sign_hash_abort() at any point if they no
 *                              longer want the result.
 *
 * \note                        If this function returns an error status, the
 *                              operation enters an error state and must be
 *                              aborted by calling \c esp_psa_sign_hash_abort().
 *
 ...
 *
 * \retval #PSA_SUCCESS
 *         The operation started successfully - call \c esp_psa_sign_hash_complete()
 ...
 *
 * \note                        This function combined with \c
 *                              esp_psa_sign_hash_start() is equivalent to
 *                              \c esp_psa_sign_hash() but this function can return
 *                              early and resume according to the limit set with
 *                              \c esp_psa_interruptible_set_max_ops() to reduce the
 *                              maximum time spent in a function call.
 *
 ...
 *                              \c esp_psa_sign_hash_abort().
 *
 * \param[in, out] operation    The \c psa_sign_hash_interruptible_operation_t
 *                              to use. This must be initialized first, and have
 *                              had \c esp_psa_sign_hash_start() called with it
 *                              first.
 *
 * \param[out] signature        Buffer where the signature is to be written.
 ...
 *         esp_psa_interruptible_set_max_ops(). There is still work to be done.
 ...
 *         esp_psa_crypto_init() or you did not previously call
 *         esp_psa_sign_hash_start() with this operation object. It is
 ...
 *                              \c esp_psa_sign_hash_get_num_ops() if required
 *                              before calling.
 *
 ...
 *                              esp_psa_sign_hash_start() again.
 *
 * \note                        You may call this function any time after the
 *                              operation object has been initialized. In
 *                              particular, calling \c esp_psa_sign_hash_abort()
 *                              after the operation has already been terminated
 *                              by a call to \c esp_psa_sign_hash_abort() or
 *                              esp_psa_sign_hash_complete() is safe.
 *
 ...
 *
 * \see                         \c esp_psa_verify_hash_complete()
 *
 * \warning                     This is a beta API, and thus subject to change
 *                              at any point. It is not bound by the usual
 *                              interface stability promises.
 *
 * \note                        This function combined with \c
 *                              esp_psa_verify_hash_complete() is equivalent to
 *                              \c esp_psa_verify_hash() but \c
 *                              esp_psa_verify_hash_complete() can return early and
 *                              resume according to the limit set with \c
 *                              esp_psa_interruptible_set_max_ops() to reduce the
 *                              maximum time spent in a function.
 *
 * \note                        Users should call \c esp_psa_verify_hash_complete()
 *                              repeatedly on the same operation object after a
 *                              successful call to this function until \c
 *                              esp_psa_verify_hash_complete() either returns 0 or
 *                              an error. \c esp_psa_verify_hash_complete() will
 *                              return #PSA_OPERATION_INCOMPLETE if there is
 *                              more work to do. Alternatively users can call
 *                              \c esp_psa_verify_hash_abort() at any point if they
 *                              no longer want the result.
 *
 * \note                        If this function returns an error status, the
 *                              operation enters an error state and must be
 *                              aborted by calling \c esp_psa_verify_hash_abort().
 *
 ...
 *         esp_psa_verify_hash_complete() with the same context to complete the
 *         operation.
 *
 * \retval #PSA_ERROR_BAD_STATE
 ...
 *                              manner.
 *
 * \see                         \c esp_psa_verify_hash_start()
 *
 * \warning                     This is a beta API, and thus subject to change
 *                              at any point. It is not bound by the usual
 *                              interface stability promises.
 *
 * \note                        This function combined with \c
 *                              esp_psa_verify_hash_start() is equivalent to
 *                              \c esp_psa_verify_hash() but this function can
 *                              return early and resume according to the limit
 *                              set with \c esp_psa_interruptible_set_max_ops() to
 ...
 *                              returns an error status, the operation enters an
 *                              error state and must be aborted by calling
 *                              \c esp_psa_verify_hash_abort().
 *
 * \param[in, out] operation    The \c psa_verify_hash_interruptible_operation_t
 *                              to use. This must be initialized first, and have
 *                              had \c esp_psa_verify_hash_start() called with it
 *                              first.
 *
 * \retval #PSA_SUCCESS
 ...
 *         The library has either not been previously initialized by
 *         esp_psa_crypto_init() or you did not previously call
 *         esp_psa_verify_hash_start() on this object. It is
 *         implementation-dependent whether a failure to initialize results in
 *         this error code.
 */
 ...
 *                            \c esp_psa_verify_hash_get_num_ops() if required
 *                            before calling.
 *
 * \note                      Aborting an operation frees all associated
 ...
 *                            esp_psa_verify_hash_start() again.
 *
 * \note                      You may call this function any time after the
 *                            operation object has been initialized.
 *                            In particular, calling \c esp_psa_verify_hash_abort()
 *                            after the operation has already been terminated by
 *                            a call to \c esp_psa_verify_hash_abort() or
 *                            esp_psa_verify_hash_complete() is safe.
 *
