 ...
 * to certain attacks.
 *
 * Implementations should avoid returning this error after esp_psa_crypto_init()
 ...
 *       used to determine the size of the data that #esp_psa_cipher_update()
 ...
 * - Call esp_psa_sign_hash() or esp_psa_verify_hash(), passing
 ...
 *   ```
 *   esp_psa_sign_hash(key, PSA_xxx_SIGNATURE(PSA_ALG_SHA_256), ...);
 *   esp_psa_sign_hash(key, PSA_xxx_SIGNATURE(PSA_ALG_SHA_512), ...);
 *   esp_psa_sign_hash(key, PSA_xxx_SIGNATURE(PSA_ALG_SHA3_256), ...);
 *   ```
 *
 ...
 * multi-part cipher operation with this algorithm, esp_psa_cipher_generate_iv()
 * and esp_psa_cipher_set_iv() must not be called.
 */
 ...
 *   SHAKE256 output.
 *
 * This algorithm can be used with esp_psa_sign_message() and
 * esp_psa_verify_message(). Since there is no prehashing, it cannot be used
 * with esp_psa_sign_hash() or esp_psa_verify_hash().
 *
 ...
 * you can either:
 * - call esp_psa_sign_message() on the message;
 * - or calculate the SHA-512 hash of the message
 *   with esp_psa_hash_compute()
 *   or with a multi-part hash operation started with esp_psa_hash_setup(),
 *   using the hash algorithm #PSA_ALG_SHA_512,
 *   then sign the calculated hash with esp_psa_sign_hash().
 * Verifying a signature is similar, using esp_psa_verify_message() or
 * esp_psa_verify_hash() instead of the signature function.
 */
 ...
 * with esp_psa_sign_hash() and esp_psa_verify_hash().
 *
...
/** Whether the specified algorithm is a signature algorithm that can be used
 * with esp_psa_sign_message() and esp_psa_verify_message().
 *
 * \param alg An algorithm identifier (value of type #psa_algorithm_t).
 *
 * \return 1 if alg is a signature algorithm that can be used to sign a
 ...
 * `esp_psa_sign_message(key, alg, input, ...)` is equivalent to
 * ```
 * esp_psa_hash_compute(PSA_ALG_SIGN_GET_HASH(alg), input, ..., hash, ...);
 * esp_psa_sign_hash(key, alg, hash, ..., signature, ...);
 * ```
 * Most usefully, separating the hash from the signature allows the hash
 * to be calculated in multiple steps with esp_psa_hash_setup(), esp_psa_hash_update()
 * and esp_psa_hash_finish(). Likewise esp_psa_verify_message() is equivalent to
 * calculating the hash and then calling esp_psa_verify_hash().
 *
 * \param alg An algorithm identifier (value of type #psa_algorithm_t).
 *
 ...
 *
 * \warning     Calling esp_psa_asymmetric_decrypt() with this algorithm as a
 ...
 *   and to call esp_psa_key_derivation_key_agreement(). Alternatively,
 *   this input may be an output of `esp_psa_raw_key_agreement()` passed with
 *   esp_psa_key_derivation_input_bytes(), or an equivalent input passed with
 ...
 *   typically an output of esp_psa_asymmetric_decrypt() using
 *   PSA_ALG_RSA_PKCS1V15_CRYPT, passed to the key derivation operation
 *   with `esp_psa_key_derivation_input_bytes()`.
 *
...
/* The TLS 1.2 ECJPAKE-to-PMS KDF. It takes the shared secret esp_K (an EC point
 * in case of EC J-PAKE) and calculates SHA256(esp_K.X) that the rest of TLS 1.2
 ...
 * #PSA_KEY_DERIVATION_INPUT_SECRET is the shared secret esp_K from EC J-PAKE.
 ...
 * whether through esp_psa_export_key() or through a proprietary interface.
 ...
 *
 * This flag allows the use of esp_psa_copy_key() to make a copy of the key
 ...
 * esp_psa_key_derivation_input_key() for a key derivation operation, then it
 * permits calling esp_psa_key_derivation_output_bytes() or
 * esp_psa_key_derivation_output_key() at the end of the operation.
 */
 ...
 * (passed to esp_psa_key_derivation_input_key())
 * or the shared secret resulting from a key agreement
 * (obtained via esp_psa_key_derivation_key_agreement()).
 *
 ...
 * esp_psa_key_derivation_input_key()) or a direct input (passed to
 * esp_psa_key_derivation_input_bytes()) that is a password or passphrase. It can
 ...
 * via `esp_psa_key_derivation_key_agreement()`. It may alternatively be a key of
 * type `PSA_KEY_TYPE_DERIVE` passed to `esp_psa_key_derivation_input_key()`, or
 * a direct input passed to `esp_psa_key_derivation_input_bytes()`.
 */
 ...
 *
 * This must be a direct input, passed to esp_psa_key_derivation_input_integer().
 */
...
/** Maximum value for use with \c esp_psa_interruptible_set_max_ops() to determine
