 ...
 * The application can call esp_psa_open_key() to open a persistent key that
 ...
 * functions such as esp_psa_open_key().
 */
typedef struct {
 ...
 * must call esp_psa_reset_key_attributes(). As an exception, calling
 * esp_psa_reset_key_attributes() on an attribute structure is optional if
 * the structure has only been modified by the following functions
 * since it was initialized or last reset with esp_psa_reset_key_attributes():
 * - psa_set_key_id()
 ...
 *    Skip this step if copying an existing key with esp_psa_copy_key().
 * -# When generating a random key with esp_psa_generate_key() or deriving a key
 *    with esp_psa_key_derivation_output_key(), set the desired key size with
 *    psa_set_key_bits().
 * -# Call a key creation function: esp_psa_import_key(), esp_psa_generate_key(),
 *    esp_psa_key_derivation_output_key() or esp_psa_copy_key(). This function reads
 *    the attribute structure, creates a key with these attributes, and
 *    outputs a key identifier to the newly created key.
 * -# The attribute structure is now no longer necessary.
 *    You may call esp_psa_reset_key_attributes(), although this is optional
 *    with the workflow presented here because the attributes currently
 *    defined in this specification do not require any additional resources
 *    beyond the structure itself.
 *
 * A typical sequence to query a key's attributes is as follows:
 * -# Call esp_psa_get_key_attributes().
 * -# Call `psa_get_key_xxx` functions to retrieve the attribute(s) that
 *    you are interested in.
 * -# Call esp_psa_reset_key_attributes() to free any resources that may be
