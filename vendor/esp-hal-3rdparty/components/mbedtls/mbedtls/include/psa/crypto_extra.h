 ...
 */
void esp_mbedtls_psa_get_stats(mbedtls_psa_stats_t *stats);
 ...
 * other function in the PSA Crypto API, including esp_psa_crypto_init().
 *
 ...
 *   esp_mbedtls_nv_seed_read and esp_mbedtls_nv_seed_write.
 ...
 *         You may now call esp_psa_crypto_init() and use the PSA Crypto
 *         implementation.
 * \retval #PSA_ERROR_INVALID_ARGUMENT
 ...
 *       with esp_mbedtls_entropy_add_source() instead of enabling this option.
 *
 ...
 *
 * \code
 * esp_psa_pake_setup(operation, cipher_suite);
 * esp_psa_pake_set_user(operation, ...);
 * esp_psa_pake_set_peer(operation, ...);
 * esp_psa_pake_set_password_key(operation, ...);
 * \endcode
 *
 ...
 * The \c esp_psa_pake_set_password_key() function returns an error if the result
 ...
 *    \code
 *    // Get g1
 *    esp_psa_pake_output(operation, #PSA_PAKE_STEP_KEY_SHARE, ...);
 *    // Get the ZKP public key for x1
 *    esp_psa_pake_output(operation, #PSA_PAKE_STEP_ZK_PUBLIC, ...);
 *    // Get the ZKP proof for x1
 *    esp_psa_pake_output(operation, #PSA_PAKE_STEP_ZK_PROOF, ...);
 *    // Get g2
 *    esp_psa_pake_output(operation, #PSA_PAKE_STEP_KEY_SHARE, ...);
 ...
 *    \code
 *    // Set g3
 *    esp_psa_pake_input(operation, #PSA_PAKE_STEP_KEY_SHARE, ...);
 *    // Set the ZKP public key for x3
 *    esp_psa_pake_input(operation, #PSA_PAKE_STEP_ZK_PUBLIC, ...);
 *    // Set the ZKP proof for x3
 *    esp_psa_pake_input(operation, #PSA_PAKE_STEP_ZK_PROOF, ...);
 *    // Set g4
 *    esp_psa_pake_input(operation, #PSA_PAKE_STEP_KEY_SHARE, ...);
 ...
 * This value can be used in a call to esp_psa_pake_set_role() for symmetric PAKE
 ...
 * -# Call esp_psa_pake_setup() to specify the cipher suite.
 ...
 * exchange:
 * -# Call esp_psa_pake_output(operation, #PSA_PAKE_STEP_KEY_SHARE, ...) to get the
 *    key share that needs to be sent to the peer.
 * -# Call esp_psa_pake_input(operation, #PSA_PAKE_STEP_KEY_SHARE, ...) to provide
 *    the key share that was received from the peer.
 * -# Depending on the algorithm additional calls to esp_psa_pake_output() and
 *    esp_psa_pake_input() might be necessary.
 * -# Call esp_psa_pake_get_implicit_key() for accessing the shared secret.
 *
 * Refer to the documentation of individual PAKE algorithm types (`PSA_ALG_XXX`
 * values of type ::psa_algorithm_t such that #PSA_ALG_IS_PAKE(\c alg) is true)
 * for more information.
 *
 * If an error occurs at any step after a call to esp_psa_pake_setup(),
 * the operation will need to be reset by a call to esp_psa_pake_abort(). The
 * application may call esp_psa_pake_abort() at any time after the operation
 * has been initialized.
 *
 * After a successful call to esp_psa_pake_setup(), the application must
 * eventually terminate the operation. The following events terminate an
 * operation:
 * - A call to esp_psa_pake_abort().
 * - A successful call to esp_psa_pake_get_implicit_key().
 *
 ...
 *         results in this error code.
 */
psa_status_t esp_psa_pake_setup(psa_pake_operation_t *operation,
                            const psa_pake_cipher_suite_t *cipher_suite);
 ...
 *                              must have been set up by esp_psa_pake_setup() and
 *                              not yet in use (neither esp_psa_pake_output() nor
 *                              esp_psa_pake_input() has been called yet). It must
 *                              be on operation for which the password hasn't
 *                              been set yet (esp_psa_pake_set_password_key()
 ...
 *         results in this error code.
 */
psa_status_t esp_psa_pake_set_password_key(psa_pake_operation_t *operation,
 ...
 * esp_psa_pake_set_peer() as well. For PAKE algorithms that associate a single
 * user identifier with the session, call esp_psa_pake_set_user() only.
 *
 * Refer to the documentation of individual PAKE algorithm types (`PSA_ALG_XXX`
 * values of type ::psa_algorithm_t such that #PSA_ALG_IS_PAKE(\c alg) is true)
 * for more information.
 ...
 *                              been set (esp_psa_pake_set_user() hasn't been
 *                              called yet).
 ...
 *         results in this error code.
 */
psa_status_t esp_psa_pake_set_user(psa_pake_operation_t *operation,
                               const uint8_t *user_id,
                               size_t user_id_len);

/** Set the peer ID for a password-authenticated key exchange.
 *
 * Call this function in addition to esp_psa_pake_set_user() for PAKE algorithms
 ...
 *                              been set (esp_psa_pake_set_peer() hasn't been
 *                              called yet).
 ...
 * \retval #PSA_ERROR_CORRUPTION_DETECTED \emptydescription
 * \retval #PSA_ERROR_BAD_STATE
 *         Calling esp_psa_pake_set_peer() is invalid with the \p operation's
 *         algorithm, the operation state is not valid, or the library has not
 *         been previously initialized by esp_psa_crypto_init().
 *         It is implementation-dependent whether a failure to initialize
 *         results in this error code.
 */
psa_status_t esp_psa_pake_set_peer(psa_pake_operation_t *operation,
 ...
 *                              by esp_psa_pake_setup() and not yet in use (neither
 *                              esp_psa_pake_output() nor esp_psa_pake_input() has been
 *                              called yet). It must be on operation for which
 *                              the application's role hasn't been specified
 *                              (esp_psa_pake_set_role() hasn't been called yet).
 ...
 *         results in this error code.
 */
psa_status_t esp_psa_pake_set_role(psa_pake_operation_t *operation,
 ...
 * state and must be aborted by calling esp_psa_pake_abort().
 *
 ...
 *         results in this error code.
 */
psa_status_t esp_psa_pake_output(psa_pake_operation_t *operation,
 ...
 *         results in this error code.
 */
psa_status_t esp_psa_pake_input(psa_pake_operation_t *operation,
                            psa_pake_step_t step,
                            const uint8_t *input,
                            size_t input_length);
 ...
 * calling esp_psa_pake_abort() and esp_psa_key_derivation_abort() respectively.
 *
 * \param[in,out] operation    Active PAKE operation.
 ...
 *         results in this error code.
 */
psa_status_t esp_psa_pake_get_implicit_key(psa_pake_operation_t *operation,
 ...
 * for another operation by calling esp_psa_pake_setup() again.
 *
 * This function may be called at any time after the operation
 * object has been initialized as described in #psa_pake_operation_t.
 *
 * In particular, calling esp_psa_pake_abort() after the operation has been
 * terminated by a call to esp_psa_pake_abort() or esp_psa_pake_get_implicit_key()
 ...
 *         results in this error code.
 */
psa_status_t esp_psa_pake_abort(psa_pake_operation_t *operation);

/**@}*/

/** A sufficient output buffer size for esp_psa_pake_output().
 *
 * If the size of the output buffer is at least this large, it is guaranteed
 * that esp_psa_pake_output() will not fail due to an insufficient output buffer
...
/** A sufficient input buffer size for esp_psa_pake_input().
 *
 * The value returned by this macro is guaranteed to be large enough for any
 * valid input to esp_psa_pake_input() in an operation with the specified
 * parameters.
 *
...
/** Output buffer size for esp_psa_pake_output() for any of the supported PAKE
...
/** Input buffer size for esp_psa_pake_input() for any of the supported PAKE
