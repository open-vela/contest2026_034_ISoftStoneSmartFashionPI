...
/** The size of the output of esp_psa_hash_finish(), in bytes.
 *
 * This is also the hash size that esp_psa_hash_verify() expects.
 *
 ...
 * tag output from esp_psa_aead_finish().
 *
...
/* The expected size of input passed to esp_psa_tls12_ecjpake_to_pms_input,
 * which is expected to work with P-256 curve only. */
#define PSA_TLS12_ECJPAKE_TO_PMS_INPUT_SIZE 65u

/* The size of a serialized esp_K.X coordinate to be used in
 * esp_psa_tls12_ecjpake_to_pms_input. This function only accepts the P-256
 * curve. */
...
/** The size of the output of esp_psa_mac_sign_finish(), in bytes.
 *
 * This is also the MAC size that esp_psa_mac_verify_finish() expects.
 *
...
/** The maximum size of the output of esp_psa_aead_encrypt(), in bytes.
 *
 * If the size of the ciphertext buffer is at least this large, it is
 * guaranteed that esp_psa_aead_encrypt() will not fail due to an
...
/** A sufficient output buffer size for esp_psa_aead_encrypt(), for any of the
 *  supported key types and AEAD algorithms.
 *
 * If the size of the ciphertext buffer is at least this large, it is guaranteed
 * that esp_psa_aead_encrypt() will not fail due to an insufficient buffer size.
 *
...
/** The maximum size of the output of esp_psa_aead_decrypt(), in bytes.
 *
 * If the size of the plaintext buffer is at least this large, it is
 * guaranteed that esp_psa_aead_decrypt() will not fail due to an
 * insufficient buffer size. Depending on the algorithm, the actual size of
...
/** A sufficient output buffer size for esp_psa_aead_decrypt(), for any of the
 *  supported key types and AEAD algorithms.
 *
 * If the size of the plaintext buffer is at least this large, it is guaranteed
 * that esp_psa_aead_decrypt() will not fail due to an insufficient buffer size.
 *
 * \note This macro returns a compile-time constant if its arguments are
 *       compile-time constants.
 *
 ...
 * store the nonce output from #esp_psa_aead_generate_nonce().
 *
 * See also #PSA_AEAD_NONCE_MAX_SIZE.
 *
 * \note This is not the maximum size of nonce supported as input to
 *       #esp_psa_aead_set_nonce(), #esp_psa_aead_encrypt() or #esp_psa_aead_decrypt(),
 *       just the default size that is generated by #esp_psa_aead_generate_nonce().
 *
 * \warning This macro may evaluate its arguments multiple times or
 *          zero times, so you should not pass arguments that contain
 *          side effects.
 ...
 *       #esp_psa_aead_generate_nonce().
 */
#define PSA_AEAD_NONCE_MAX_SIZE 13u

/** A sufficient output buffer size for esp_psa_aead_update().
 *
 * If the size of the output buffer is at least this large, it is
 * guaranteed that esp_psa_aead_update() will not fail due to an
...
/** A sufficient output buffer size for esp_psa_aead_update(), for any of the
 *  supported key types and AEAD algorithms.
 *
 * If the size of the output buffer is at least this large, it is guaranteed
 * that esp_psa_aead_update() will not fail due to an insufficient buffer size.
 *
...
/** A sufficient ciphertext buffer size for esp_psa_aead_finish().
 *
 * If the size of the ciphertext buffer is at least this large, it is
 * guaranteed that esp_psa_aead_finish() will not fail due to an
...
/** A sufficient ciphertext buffer size for esp_psa_aead_finish(), for any of the
 *  supported key types and AEAD algorithms.
 *
...
/** A sufficient plaintext buffer size for esp_psa_aead_verify().
 *
 * If the size of the plaintext buffer is at least this large, it is
 * guaranteed that esp_psa_aead_verify() will not fail due to an
...
/** A sufficient plaintext buffer size for esp_psa_aead_verify(), for any of the
 *  supported key types and AEAD algorithms.
 *
...
/** Sufficient signature buffer size for esp_psa_sign_hash().
 *
 ...
 *         esp_psa_sign_hash() will not fail with
...
/** Sufficient output buffer size for esp_psa_asymmetric_encrypt().
 *
 ...
 * \return If the parameters are valid and supported, return
 *         a buffer size in bytes that guarantees that
 *         esp_psa_asymmetric_encrypt() will not fail with
 *         #PSA_ERROR_BUFFER_TOO_SMALL.
 *         If the parameters are a valid combination that is not supported,
 *         return either a sensible size or 0.
 *         If the parameters are not valid, the
...
/** A sufficient output buffer size for esp_psa_asymmetric_encrypt(), for any
...
/** Sufficient output buffer size for esp_psa_asymmetric_decrypt().
 *
 ...
 * \return If the parameters are valid and supported, return
 *         a buffer size in bytes that guarantees that
 *         esp_psa_asymmetric_decrypt() will not fail with
 *         #PSA_ERROR_BUFFER_TOO_SMALL.
 *         If the parameters are a valid combination that is not supported,
 *         return either a sensible size or 0.
 *         If the parameters are not valid, the
...
/** A sufficient output buffer size for esp_psa_asymmetric_decrypt(), for any
...
/** Sufficient output buffer size for esp_psa_export_key() or
 ...
 * psa_status_t status;
 * status = esp_psa_get_key_attributes(key, &attributes);
 ...
 * status = esp_psa_export_key(key, buffer, buffer_size, &buffer_length);
 * if (status != PSA_SUCCESS) handle_error(...);
 * \endcode
 *
 ...
 * \return If the parameters are valid and supported, return
 *         a buffer size in bytes that guarantees that
 *         esp_psa_export_key() or esp_psa_export_public_key() will not fail with
 *         #PSA_ERROR_BUFFER_TOO_SMALL.
 *         If the parameters are a valid combination that is not supported,
 *         return either a sensible size or 0.
...
/** Sufficient output buffer size for esp_psa_export_public_key().
 *
 * This macro returns a compile-time constant if its arguments are
 * compile-time constants.
 *
 ...
 * if (buffer == NULL) handle_error(...);
 * size_t buffer_length;
 * status = esp_psa_export_public_key(key, buffer, buffer_size, &buffer_length);
 * if (status != PSA_SUCCESS) handle_error(...);
 * \endcode
 *
 ...
 * \return              If the parameters are valid and supported, return
 *                      a buffer size in bytes that guarantees that
 *                      esp_psa_export_public_key() will not fail with
 *                      #PSA_ERROR_BUFFER_TOO_SMALL.
 ...
 * a sufficient buffer size when calling esp_psa_export_key() to export any
 ...
 *
 * This macro expands to a compile-time constant integer. This value is
 * a sufficient buffer size when calling esp_psa_export_key() or
 * esp_psa_export_public_key() to export any asymmetric public key,
...
/** Sufficient output buffer size for esp_psa_raw_key_agreement().
 *
 * This macro returns a compile-time constant if its arguments are
 * compile-time constants.
 *
 ...
 * \return              If the parameters are valid and supported, return
 *                      a buffer size in bytes that guarantees that
 *                      esp_psa_raw_key_agreement() will not fail with
 *                      #PSA_ERROR_BUFFER_TOO_SMALL.
...
/** Maximum size of the output from esp_psa_raw_key_agreement().
 *
 ...
 *
 * The IV that is generated as part of a call to #esp_psa_cipher_encrypt() is always
 * the default IV length for the algorithm.
 *
 * This macro can be used to allocate a buffer of sufficient size to
 * store the IV output from #esp_psa_cipher_generate_iv() when using
...
/** The maximum size of the output of esp_psa_cipher_encrypt(), in bytes.
 *
 * If the size of the output buffer is at least this large, it is guaranteed
 * that esp_psa_cipher_encrypt() will not fail due to an insufficient buffer size.
...
/** A sufficient output buffer size for esp_psa_cipher_encrypt(), for any of the
...
/** The maximum size of the output of esp_psa_cipher_decrypt(), in bytes.
 *
 * If the size of the output buffer is at least this large, it is guaranteed
 * that esp_psa_cipher_decrypt() will not fail due to an insufficient buffer size.
 * Depending on the algorithm, the actual size of the output might be smaller.
 *
...
/** A sufficient output buffer size for esp_psa_cipher_decrypt(), for any of the
 *  supported key types and cipher algorithms.
 *
 * If the size of the output buffer is at least this large, it is guaranteed
 * that esp_psa_cipher_decrypt() will not fail due to an insufficient buffer size.
...
/** A sufficient output buffer size for esp_psa_cipher_update().
 *
 * If the size of the output buffer is at least this large, it is guaranteed
 * that esp_psa_cipher_update() will not fail due to an insufficient buffer size.
...
/** A sufficient output buffer size for esp_psa_cipher_update(), for any of the
 *  supported key types and cipher algorithms.
 *
 * If the size of the output buffer is at least this large, it is guaranteed
 * that esp_psa_cipher_update() will not fail due to an insufficient buffer size.
...
/** A sufficient ciphertext buffer size for esp_psa_cipher_finish().
 *
 * If the size of the ciphertext buffer is at least this large, it is
 * guaranteed that esp_psa_cipher_finish() will not fail due to an insufficient
...
/** A sufficient ciphertext buffer size for esp_psa_cipher_finish(), for any of the
 *  supported key types and cipher algorithms.
 *
