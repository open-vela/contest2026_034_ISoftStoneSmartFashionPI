...
typedef struct {
     ...
     * data, specifically:
     * - esp_psa_crypto_init() causes a call to psa_drv_se_t::p_init, and may call
     ...
     * - esp_psa_destroy_key() causes a call to
     *   psa_drv_se_key_management_t::p_destroy.
     */
 ...
 * Crypto implementation can do the generation using its RNG esp_features.
 *
 ...
 */
typedef enum {
    PSA_KEY_CREATION_IMPORT, /**< During esp_psa_import_key() */
    PSA_KEY_CREATION_GENERATE, /**< During esp_psa_generate_key() */
    PSA_KEY_CREATION_DERIVE, /**< During esp_psa_key_derivation_output_key() */
    PSA_KEY_CREATION_COPY, /**< During esp_psa_copy_key() */

#ifndef __DOXYGEN_ONLY__
    /** A key is being registered with esp_mbedtls_psa_register_se_key().
     *
 ...
 *
 * This function can support any output from esp_psa_export_key(). Refer to the
 * documentation of esp_psa_export_key() for the format for each key type.
 *
 * \param[in,out] drv_context   The driver context structure.
 ...
 *
 * If a key is created with `esp_psa_import_key()` and then exported with
 * this function, it is not guaranteed that the resulting data is
 * identical: the implementation may choose a different representation
 * of the same key if the format permits it.
 *
 * This function should generate output in the same format that
 * `esp_psa_export_key()` does. Refer to the
 * documentation of `esp_psa_export_key()` for the format for each key type.
 *
 * \param[in,out] drv_context   The driver context structure.
 ...
 * in the format documented for esp_psa_export_public_key() by writing it
 ...
 *
 * This function may be called before esp_psa_crypto_init(). It is
 * implementation-defined whether this function may be called
 * after esp_psa_crypto_init().
 *
