 ...
 *              esp_mbedtls_cipher_info_from_string(),
 *              esp_mbedtls_cipher_info_from_type(),
 *              esp_mbedtls_cipher_info_from_values(),
 *              esp_mbedtls_cipher_info_from_psa().
 *
...
typedef struct mbedtls_cipher_context_t {
     ...
     *  esp_mbedtls_cipher_setup(), and set if it was established through
     *  esp_mbedtls_cipher_setup_psa().
     */
 ...
 *        via esp_mbedtls_cipher_info_from_type(), which can then be used
 *        to prepare a cipher context via esp_mbedtls_cipher_setup().
 *
 *
 ...
 */
const mbedtls_cipher_info_t *esp_mbedtls_cipher_info_from_string(const char *cipher_name);
 ...
 * \return              \c NULL if the associated cipher information is not found.
 */
const mbedtls_cipher_info_t *esp_mbedtls_cipher_info_from_type(const mbedtls_cipher_type_t cipher_type);
 ...
 * \return              \c NULL if the associated cipher information is not found.
 */
const mbedtls_cipher_info_t *esp_mbedtls_cipher_info_from_values(const mbedtls_cipher_id_t cipher_id,
                                                             int key_bitlen,
 ...
 */
static inline mbedtls_cipher_type_t esp_mbedtls_cipher_info_get_type(
 ...
 */
static inline mbedtls_cipher_mode_t esp_mbedtls_cipher_info_get_mode(
    const mbedtls_cipher_info_t *info)
{
    if (info == NULL) {
 ...
 */
static inline size_t esp_mbedtls_cipher_info_get_key_bitlen(
    const mbedtls_cipher_info_t *info)
{
    if (info == NULL) {
        return 0;
 ...
 */
static inline const char *esp_mbedtls_cipher_info_get_name(
    const mbedtls_cipher_info_t *info)
{
    if (info == NULL) {
        return NULL;
 ...
 * \return      \c 0 if \p info is \c NULL.
 */
static inline size_t esp_mbedtls_cipher_info_get_iv_size(
    const mbedtls_cipher_info_t *info)
{
    if (info == NULL) {
        return 0;
 ...
 * \return       \c 0 if \p info is \c NULL.
 */
static inline size_t esp_mbedtls_cipher_info_get_block_size(
    const mbedtls_cipher_info_t *info)
{
    if (info == NULL) {
        return 0;
 ...
 */
static inline int esp_mbedtls_cipher_info_has_variable_key_bitlen(
    const mbedtls_cipher_info_t *info)
{
    if (info == NULL) {
        return 0;
 ...
 * \return       \c 0 if the given pointer is \c NULL.
 */
static inline int esp_mbedtls_cipher_info_has_variable_iv_size(
    const mbedtls_cipher_info_t *info)
{
    if (info == NULL) {
        return 0;
 ...
 *                      esp_mbedtls_cipher_setkey() and, if the mode uses padding,
 *                      esp_mbedtls_cipher_set_padding_mode(), then for each
 *                      message to encrypt or decrypt with this key, either:
 *                      - esp_mbedtls_cipher_crypt() for one-shot processing with
 *                      non-AEAD modes;
 *                      - esp_mbedtls_cipher_auth_encrypt_ext() or
 *                      esp_mbedtls_cipher_auth_decrypt_ext() for one-shot
 *                      processing with AEAD modes or NIST_KW;
 *                      - for multi-part processing, see the documentation of
 *                      esp_mbedtls_cipher_reset().
 *
 * \param ctx           The context to prepare. This must be initialized by
 *                      a call to esp_mbedtls_cipher_init() first.
 ...
 *                      esp_mbedtls_cipher_auth_encrypt_ext() or
 *                      esp_mbedtls_cipher_auth_decrypt_ext() must provide
 *                      the same tag length.
 ...
 *                      cipher-specific context fails.
 */
int MBEDTLS_DEPRECATED esp_mbedtls_cipher_setup_psa(mbedtls_cipher_context_t *ctx,
                                                const mbedtls_cipher_info_t *cipher_info,
                                                size_t taglen);
 ...
 */
static inline unsigned int esp_mbedtls_cipher_get_block_size(
 ...
 */
static inline mbedtls_cipher_mode_t esp_mbedtls_cipher_get_cipher_mode(
    const mbedtls_cipher_context_t *ctx)
{
    if (ctx->MBEDTLS_PRIVATE(cipher_info) == NULL) {
        return MBEDTLS_MODE_NONE;
 ...
 */
static inline int esp_mbedtls_cipher_get_iv_size(
    const mbedtls_cipher_context_t *ctx)
{
    if (ctx->MBEDTLS_PRIVATE(cipher_info) == NULL) {
        return 0;
 ...
 */
static inline mbedtls_cipher_type_t esp_mbedtls_cipher_get_type(
    const mbedtls_cipher_context_t *ctx)
{
    if (ctx->MBEDTLS_PRIVATE(cipher_info) == NULL) {
        return MBEDTLS_CIPHER_NONE;
 ...
 */
static inline const char *esp_mbedtls_cipher_get_name(
    const mbedtls_cipher_context_t *ctx)
{
    if (ctx->MBEDTLS_PRIVATE(cipher_info) == NULL) {
        return 0;
 ...
 *                      initialized.
 */
static inline int esp_mbedtls_cipher_get_key_bitlen(
    const mbedtls_cipher_context_t *ctx)
{
    if (ctx->MBEDTLS_PRIVATE(cipher_info) == NULL) {
 ...
 */
static inline mbedtls_operation_t esp_mbedtls_cipher_get_operation(
    const mbedtls_cipher_context_t *ctx)
{
    if (ctx->MBEDTLS_PRIVATE(cipher_info) == NULL) {
 ...
 *                is as follows:
 *                1. esp_mbedtls_cipher_set_iv() if the mode uses an IV/nonce.
 *                2. esp_mbedtls_cipher_reset()
 *                3. esp_mbedtls_cipher_update() one or more times
 *                4. esp_mbedtls_cipher_finish()
 *                .
 ...
 *                1. esp_mbedtls_cipher_set_iv() if the mode uses an IV/nonce.
 *                2. esp_mbedtls_cipher_reset()
 *                3. esp_mbedtls_cipher_update_ad()
 *                4. esp_mbedtls_cipher_update() one or more times
 *                5. esp_mbedtls_cipher_finish()
 *                6. esp_mbedtls_cipher_check_tag() (for decryption) or
 *                esp_mbedtls_cipher_write_tag() (for encryption).
 *                .
 *                This sequence can be repeated to encrypt or decrypt multiple
 *                messages with the same key.
 *
 ...
 *                      esp_mbedtls_cipher_finish() is called.
 ...
 * \return              A cipher-specific error code on failure.
 */
int esp_mbedtls_cipher_update(mbedtls_cipher_context_t *ctx,
 ...
 *                      Currently supported with GCM and ChaCha20+Poly1305.
 *                      This must be called after esp_mbedtls_cipher_finish().
 *
 * \param ctx           The generic cipher context. This must be initialized,
 *                      bound to a key, and have just completed a cipher
 *                      operation through esp_mbedtls_cipher_finish() the tag for
