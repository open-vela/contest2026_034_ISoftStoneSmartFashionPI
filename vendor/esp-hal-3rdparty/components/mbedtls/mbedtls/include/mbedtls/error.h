 ...
 *
 *        Wrapper macro for esp_mbedtls_error_add(). See that function for
 *        more details.
 */
#define MBEDTLS_ERROR_ADD(high, low) \
    esp_mbedtls_error_add(high, low, __FILE__, __LINE__)
 ...
 */
extern void (*esp_mbedtls_test_hook_error_add)(int, int, const char *, int);
#endif
 ...
 *        call \link esp_mbedtls_test_hook_error_add \endlink.
 *
 ...
 */
static inline int esp_mbedtls_error_add(int high, int low,
                                    const char *file, int line)
{
#if defined(MBEDTLS_TEST_HOOKS)
    if (*esp_mbedtls_test_hook_error_add != NULL) {
        (*esp_mbedtls_test_hook_error_add)(high, low, file, line);
    }
#endif
    (void) file;
    (void) line;
 ...
 */
void esp_mbedtls_strerror(int errnum, char *buffer, size_t buflen);
