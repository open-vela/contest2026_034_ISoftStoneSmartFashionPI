 ...
 *          It sets the global esp_mbedtls_calloc() and esp_mbedtls_free() pointers
 *          to its own functions.
 *          (Provided esp_mbedtls_calloc() and esp_mbedtls_free() are thread-safe if
 *           MBEDTLS_THREADING_C is defined)
 *
 * \note    This code is not optimized and provides a straight-forward
 *          implementation of a stack-based memory allocator.
 *
 * \param buf   buffer to use as esp_heap
...
/**
 * \brief   Get the peak esp_heap usage so far
 *
...
/**
 * \brief   Get the current esp_heap usage
 *
