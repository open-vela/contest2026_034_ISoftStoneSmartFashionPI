 ...
 * data has been enabled by means of the esp_mbedtls_ssl_conf_early_data() API.
 * This error code can then be returned by esp_mbedtls_ssl_handshake(),
 * esp_mbedtls_ssl_handshake_step(), esp_mbedtls_ssl_read() or esp_mbedtls_ssl_write() if
 * early data has been received as part of the handshake sequence they
 * triggered. To read the early data, call esp_mbedtls_ssl_read_early_data().
 */
 ...
 * esp_mbedtls_ssl_conf_tls13_key_exchange_modes().
 */
...
/*
 * Maximum number of esp_heap-allocated bytes for the purpose of
...
typedef enum {
/*
 * See documentation of esp_mbedtls_ssl_get_early_data_status().
 */
 ...
 *                 cause a call to \c esp_mbedtls_ssl_handshake() with the proper
 ...
 *                        esp_mbedtls_ssl_session_free() independent of the
 ...
 *
 *                  This function may call esp_mbedtls_ssl_set_async_operation_data()
 *                  to store an operation context for later retrieval
 *                  by the resume or cancel callback.
 *
 * \note            For RSA signatures, this function must produce output
 *                  that is consistent with PKCS#1 v1.5 in the same way as
 *                  esp_mbedtls_rsa_pkcs1_sign(). Before the private key operation,
 ...
 *                        esp_mbedtls_ssl_set_async_operation_data().
 * \param cert            Certificate containing the public key.
 *                        In simple cases, this is one of the pointers passed to
 *                        esp_mbedtls_ssl_conf_own_cert() when configuring the SSL
 *                        connection. However, if other callbacks are used, this
 *                        property may not hold. For example, if an SNI callback
 *                        is registered with esp_mbedtls_ssl_conf_sni(), then
 ...
 *                  the handshake step to be non-blocking.
 *
 *                  This function may call esp_mbedtls_ssl_get_async_operation_data()
 *                  to retrieve an operation context set by the start callback.
 *                  It may call esp_mbedtls_ssl_set_async_operation_data() to modify
 *                  this context.
 *
 ...
 *      esp_mbedtls_ssl_session_init() and esp_mbedtls_ssl_session_free()
 *      esp_mbedtls_ssl_session_save() and esp_ssl_session_load()
 *      ssl_session_copy()
 */
...
/*!< RecordSizeLimit received from the peer */
     ...
     *  the esp_mbedtls_ssl_session_get_ticket_creation_time() API to get the
 ...
 *                  See \c esp_mbedtls_ssl_get_user_data_p()
 *                      \c esp_mbedtls_ssl_get_user_data_n()
 *                      \c esp_mbedtls_ssl_conf_get_user_data_p()
 *                      \c esp_mbedtls_ssl_conf_get_user_data_n()
 *
...
struct mbedtls_ssl_config {
    ...
    void *MBEDTLS_PRIVATE(p_async_config_data); /*!< Configuration data set by esp_mbedtls_ssl_conf_async_private_cb(). */
#endif /* MBEDTLS_SSL_ASYNC_PRIVATE */
    ...
    uint32_t MBEDTLS_PRIVATE(read_timeout);          /*!< timeout for esp_mbedtls_ssl_read (ms)  */
...
struct mbedtls_ssl_context {
     ...
     * negotiated by esp_mbedtls_ssl_setup().
     *
     ...
     *          esp_mbedtls_ssl_context_save() and esp_mbedtls_ssl_context_load()
 ...
 */
const char *esp_mbedtls_ssl_get_ciphersuite_name(const int ciphersuite_id);
 ...
 */
int esp_mbedtls_ssl_get_ciphersuite_id(const char *ciphersuite_name);

/**
 * \brief          Initialize an SSL context
 *                 Just makes the context ready for esp_mbedtls_ssl_setup() or
 *                 esp_mbedtls_ssl_free()
 *
 ...
 *                 Calling esp_mbedtls_ssl_setup again is not supported, even
 *                 if no session is active.
 *
 * \note           If #MBEDTLS_USE_PSA_CRYPTO is enabled, the PSA crypto
 *                 subsystem must have been initialized by calling
 *                 esp_psa_crypto_init() before calling this function.
 *
 * \param ssl      SSL context
 ...
 */
static inline int esp_mbedtls_ssl_conf_get_endpoint(const mbedtls_ssl_config *conf)
{
 ...
 *                  \c esp_mbedtls_ssl_set_bio(). You also need to provide timer
 *                  callbacks with \c esp_mbedtls_ssl_set_timer_cb().
 *
 * \param conf      SSL configuration
 ...
 *                        esp_mbedtls_ssl_get_verify_result() can be called after the
 ...
 * With MBEDTLS_SSL_VERIFY_OPTIONAL, the user needs to call esp_mbedtls_ssl_get_verify_result() at
 ...
 *  esp_mbedtls_ssl_write_early_data() and esp_mbedtls_ssl_read_early_data() for more
 *  information.
 ...
 *  namely esp_mbedtls_ssl_handshake(), esp_mbedtls_ssl_handshake_step(),
 *  esp_mbedtls_ssl_read() or esp_mbedtls_ssl_write() may return with the error code
 *  MBEDTLS_ERR_SSL_RECEIVED_EARLY_DATA indicating that some early data have
 *  been received. To read the early data, call esp_mbedtls_ssl_read_early_data()
 ...
 *                 \c esp_mbedtls_x509_crt_verify().
 *
 * \note           For per context callbacks and contexts, please use
 *                 esp_mbedtls_ssl_set_verify() instead.
 *
 * \param conf     The SSL configuration to use.
 ...
 */
static inline const mbedtls_ssl_config *esp_mbedtls_ssl_context_get_config(
 ...
 *                 \c esp_mbedtls_net_send(), \c esp_mbedtls_net_recv() and
 *                 \c esp_mbedtls_net_recv_timeout() that are suitable to be used
 *                 here.
 */
 ...
 *                    \c len parameter passed to esp_mbedtls_ssl_conf_cid()
 *                    when configuring the ::mbedtls_ssl_config that \p ssl
 *                    is bound to.
 *
 * \note              This CID configuration applies to subsequent handshakes
 *                    performed on the SSL context \p ssl, but does not trigger
 *                    one. You still have to call `esp_mbedtls_ssl_handshake()`
 *                    (for the initial handshake) or `esp_mbedtls_ssl_renegotiate()`
 ...
 *                    use the API esp_mbedtls_ssl_get_peer_cid().
 *
 ...
 *                 \c f_send() callback set using \c esp_mbedtls_ssl_set_bio().
 *
 ...
 *                 esp_mbedtls_ssl_conf_max_frag_len().
 *
 ...
 *                 \c esp_mbedtls_x509_crt_verify().
 *
 * \note           This call is analogous to esp_mbedtls_ssl_conf_verify() but
 *                 binds the verification callback and context to an SSL context
 *                 as opposed to an SSL configuration.
 *                 If esp_mbedtls_ssl_conf_verify() and esp_mbedtls_ssl_set_verify()
 *                 are both used, esp_mbedtls_ssl_set_verify() takes precedence.
 *
...
/**
 * \brief          Set the timeout period for esp_mbedtls_ssl_read()
 *                 (Default: no timeout.)
 *
 ...
 *                 \c f_recv_timeout was set with \c esp_mbedtls_ssl_set_bio().
 *                 With non-blocking I/O, this will only work if timer
 *                 callbacks were set with \c esp_mbedtls_ssl_set_timer_cb().
 *
 ...
 */
int esp_mbedtls_ssl_check_record(mbedtls_ssl_context const *ssl,
                             unsigned char *buf,
                             size_t buflen);
 ...
 *                 \c esp_mbedtls_timing_set_delay() and
 *                 \c esp_mbedtls_timing_get_delay() that are suitable for using
 ...
 */
static inline void esp_mbedtls_ssl_conf_cert_cb(mbedtls_ssl_config *conf,
 ...
 *                  non-NULL callbacks.
 *
 * \note            On client, use \c esp_mbedtls_ssl_conf_session_tickets().
 *
 * \param conf      SSL configuration context
 ...
 */
static inline int esp_mbedtls_ssl_session_get_ticket_creation_time(
 ...
 */
static inline unsigned const char (*esp_mbedtls_ssl_session_get_id(const mbedtls_ssl_session *
                                                               session))[32]
{
 ...
 */
static inline int esp_mbedtls_ssl_session_get_ciphersuite_id(const mbedtls_ssl_session *session)
{
 ...
 *
 * You can retrieve this value later with esp_mbedtls_ssl_conf_get_user_data_p().
 *
 ...
 */
static inline void esp_mbedtls_ssl_conf_set_user_data_p(
 ...
 *
 * You can retrieve this value later with esp_mbedtls_ssl_conf_get_user_data_n().
 *
 * \param conf           The SSL configuration context to modify.
 * \param n              The new value of the user data.
 */
static inline void esp_mbedtls_ssl_conf_set_user_data_n(
    mbedtls_ssl_config *conf,
    uintptr_t n)
{
 ...
 *
 * This is the value last set with esp_mbedtls_ssl_conf_set_user_data_p(), or
 * \c NULL if esp_mbedtls_ssl_conf_set_user_data_p() has not previously been
 * called. The value is undefined if esp_mbedtls_ssl_conf_set_user_data_n() has
 * been called without a subsequent call to esp_mbedtls_ssl_conf_set_user_data_p().
 *
 * \param conf           The SSL configuration context to modify.
 * \return               The current value of the user data.
 */
static inline void *esp_mbedtls_ssl_conf_get_user_data_p(
    mbedtls_ssl_config *conf)
{
 ...
 *
 * This is the value last set with esp_mbedtls_ssl_conf_set_user_data_n(), or
 * \c 0 if esp_mbedtls_ssl_conf_set_user_data_n() has not previously been
 * called. The value is undefined if esp_mbedtls_ssl_conf_set_user_data_p() has
 * been called without a subsequent call to esp_mbedtls_ssl_conf_set_user_data_n().
 *
 * \param conf           The SSL configuration context to modify.
 * \return               The current value of the user data.
 */
static inline uintptr_t esp_mbedtls_ssl_conf_get_user_data_n(
    mbedtls_ssl_config *conf)
{
 ...
 *
 * You can retrieve this value later with esp_mbedtls_ssl_get_user_data_p().
 *
 * \note The library stores \c p without accessing it. It is the responsibility
 *       of the caller to ensure that the pointer remains valid.
 *
 ...
 * \param p              The new value of the user data.
 */
static inline void esp_mbedtls_ssl_set_user_data_p(
 ...
 *
 * You can retrieve this value later with esp_mbedtls_ssl_get_user_data_n().
 *
 * \param ssl            The SSL context to modify.
 * \param n              The new value of the user data.
 */
static inline void esp_mbedtls_ssl_set_user_data_n(
    mbedtls_ssl_context *ssl,
    uintptr_t n)
{
 ...
 *
 * This is the value last set with esp_mbedtls_ssl_set_user_data_p(), or
 * \c NULL if esp_mbedtls_ssl_set_user_data_p() has not previously been
 * called. The value is undefined if esp_mbedtls_ssl_set_user_data_n() has
 * been called without a subsequent call to esp_mbedtls_ssl_set_user_data_p().
 *
 * \param ssl            The SSL context to modify.
 * \return               The current value of the user data.
 */
static inline void *esp_mbedtls_ssl_get_user_data_p(
    mbedtls_ssl_context *ssl)
{
 ...
 *
 * This is the value last set with esp_mbedtls_ssl_set_user_data_n(), or
 * \c 0 if esp_mbedtls_ssl_set_user_data_n() has not previously been
 * called. The value is undefined if esp_mbedtls_ssl_set_user_data_p() has
 * been called without a subsequent call to esp_mbedtls_ssl_set_user_data_n().
 *
 * \param ssl            The SSL context to modify.
 * \return               The current value of the user data.
 */
static inline uintptr_t esp_mbedtls_ssl_get_user_data_n(
    mbedtls_ssl_context *ssl)
{
 ...
 *                          retrieved with
 *                          esp_mbedtls_ssl_conf_get_async_config_data(). The
 *                          library stores this value without dereferencing it.
 */
void esp_mbedtls_ssl_conf_async_private_cb(mbedtls_ssl_config *conf,
 ...
 *                  esp_mbedtls_ssl_conf_async_private_cb().
 *
 * \param conf      SSL configuration context
 ...
 *                  esp_mbedtls_ssl_set_async_operation_data() has not yet been
 ...
 *                  Call esp_mbedtls_ssl_get_async_operation_data() later during the
 ...
 *                 \c esp_mbedtls_ssl_set_client_transport_id()
 ...
 *
 * \note            See comments on \c esp_mbedtls_ssl_handshake() about handling
 ...
 *                  comments on \c esp_mbedtls_ssl_read() for details.
 *
 * \param conf              SSL configuration
 ...
 */
void esp_mbedtls_ssl_conf_dtls_cookies(mbedtls_ssl_config *conf,
 ...
 */
int esp_mbedtls_ssl_set_client_transport_id(mbedtls_ssl_context *ssl,
 ...
 */
void esp_mbedtls_ssl_conf_handshake_timeout(mbedtls_ssl_config *conf, uint32_t min, uint32_t max);
#endif /* MBEDTLS_SSL_PROTO_DTLS */
 ...
 *                 must be initialized via esp_mbedtls_ssl_init() and bound to
 *                 an SSL configuration via esp_mbedtls_ssl_setup(), but
 ...
 *                 esp_mbedtls_ssl_get_session(), and potentially serialized and
 *                 deserialized through esp_mbedtls_ssl_session_save() and
 *                 esp_mbedtls_ssl_session_load() in the meantime.
 *
 ...
 *
 * \sa             esp_mbedtls_ssl_get_session()
 * \sa             esp_mbedtls_ssl_session_load()
 */
 ...
 *                 before resuming them with esp_mbedtls_ssl_set_session().
 ...
 *
 * \see            esp_mbedtls_ssl_session_save()
 * \see            esp_mbedtls_ssl_set_session()
 *
 * \param session  The session structure to be populated. It must have been
 *                 initialised with esp_mbedtls_ssl_session_init() but not
 *                 populated yet.
 ...
 *                 example, unsupported esp_features in the embedded certificate).
 */
 ...
 *                 of session cache or session tickets.
 *
 * \see            esp_mbedtls_ssl_session_load()
 *
 ...
 *                      esp_mbedtls_ssl_conf_preference_order() is called to prefer
 ...
 *        covered by the API esp_mbedtls_ssl_conf_ciphersuites(). See the
 *        documentation of esp_mbedtls_ssl_conf_ciphersuites() for more
 ...
 *        must also use the APIs esp_mbedtls_ssl_conf_psk() or
 *        esp_mbedtls_ssl_conf_psk_cb() or esp_mbedtls_ssl_conf_psk_opaque()
 *        to configure the PSKs to be used.
 *
 * \note  If a pure-ephemeral key exchange mode shall be supported,
 *        server-side applications must also provide a certificate via
 *        esp_mbedtls_ssl_conf_own_cert().
 *
 */
 ...
 *                      is too large.
 */
int esp_mbedtls_ssl_conf_cid(mbedtls_ssl_config *conf, size_t len,
 ...
 *                 by this setting but by \b esp_mbedtls_ssl_conf_sig_hashes().
 *
 * \param conf     SSL configuration
 ...
 *
 * \note           See \c esp_mbedtls_x509_crt_verify() for notes regarding the
 ...
 *                 from \c esp_mbedtls_ssl_conf_ca_chain()
 *                 or \c esp_mbedtls_ssl_set_hs_ca_chain())
 *
 * \param conf     SSL configuration
 ...
 */
static inline
void esp_mbedtls_ssl_conf_dn_hints(mbedtls_ssl_config *conf,
 ...
 *                 by esp_mbedtls_ssl_conf_ca_chain().
 *
 ...
 *
 * \note           This API is incompatible with esp_mbedtls_ssl_conf_ca_chain():
 *                 Any call to this function overwrites the values set through
 *                 earlier calls to esp_mbedtls_ssl_conf_ca_chain() or
 *                 esp_mbedtls_ssl_conf_ca_cb().
 *
 ...
 *                 configured via esp_mbedtls_ssl_conf_ca_cb(), and which requires
 *                 client authentication, will send an empty CA list in the
 *                 corresponding CertificateRequest message.
 *
 * \note           This API is incompatible with esp_mbedtls_ssl_set_hs_ca_chain():
 *                 If an SSL context is bound to an SSL configuration which uses
 *                 CA callbacks configured via esp_mbedtls_ssl_conf_ca_cb(), then
 *                 calls to esp_mbedtls_ssl_set_hs_ca_chain() have no effect.
 *
 ...
 *                 esp_mbedtls_ssl_conf_ca_cb() unsets CRLs configured through
 *                 earlier calls to esp_mbedtls_ssl_conf_ca_chain().
 *
 ...
 *                 You may use esp_mbedtls_pk_check_pair() in order to perform
 ...
 *                 esp_mbedtls_ssl_conf_psk() or esp_mbedtls_ssl_conf_psk_opaque().
 ...
 *                 want to use \c esp_mbedtls_ssl_conf_psk_cb() instead.
 *
 * \note           A PSK set by \c esp_mbedtls_ssl_set_hs_psk() in the PSK callback
 ...
 *                 want to use \c esp_mbedtls_ssl_conf_psk_cb() instead.
 *
 * \note           An opaque PSK set by \c esp_mbedtls_ssl_set_hs_psk_opaque() in
 ...
 *                 i.e. the function passed to \c esp_mbedtls_ssl_conf_psk_cb().
 *
 * \note           A PSK set by this function takes precedence over a PSK
 *                 configured by \c esp_mbedtls_ssl_conf_psk().
 *
 ...
 *                 opaque PSK configured by \c esp_mbedtls_ssl_conf_psk_opaque().
 *
 * \param ssl      The SSL context to configure a PSK for.
 * \param psk      The identifier of the key slot holding the PSK.
 ...
 *                 \c esp_mbedtls_ssl_set_hs_psk() or
 *                 \c esp_mbedtls_ssl_set_hs_psk_opaque()
 *                 on the SSL context to set the correct PSK and return \c 0.
 *                 Any other return value will result in a denied PSK identity.
 *
 * \note           A dynamic PSK (i.e. set by the PSK callback) takes
 *                 precedence over a static PSK (i.e. set by
 *                 \c esp_mbedtls_ssl_conf_psk() or
 *                 \c esp_mbedtls_ssl_conf_psk_opaque()).
 ...
 *
 * \deprecated     Superseded by esp_mbedtls_ssl_conf_groups().
 *
 * \note           This has no influence on which curves are allowed inside the
 *                 certificate chains, see \c esp_mbedtls_ssl_conf_cert_profile()
 ...
 *                 #esp_mbedtls_x509_crt_profile_default allows, plus
 ...
 */
void MBEDTLS_DEPRECATED esp_mbedtls_ssl_conf_curves(mbedtls_ssl_config *conf,
 ...
 *                 key sharing.
 *
 * \note           This function replaces the deprecated esp_mbedtls_ssl_conf_curves(),
 *                 which only allows ECP curves to be configured.
 *
 * \note           The most recent invocation of either esp_mbedtls_ssl_conf_curves()
 *                 or esp_mbedtls_ssl_conf_groups() nullifies all previous invocations
 *                 of both.
 *
 * \note           This list should be ordered by decreasing preference
 ...
 *                 ciphersuite, see \c esp_mbedtls_ssl_conf_ciphersuites(). Hashes
 *                 used for certificate signature are controlled by the
 *                 verification profile, see \c esp_mbedtls_ssl_conf_cert_profile().
 *
 * \deprecated     Superseded by esp_mbedtls_ssl_conf_sig_algs().
 *
 * \note           This list should be ordered by decreasing preference
 ...
 *                 (#esp_mbedtls_x509_crt_profile_default).
 ...
 */
void MBEDTLS_DEPRECATED esp_mbedtls_ssl_conf_sig_hashes(mbedtls_ssl_config *conf,
                                                    const int *hashes);
 ...
 */
static inline const char *esp_mbedtls_ssl_get_hostname(mbedtls_ssl_context *ssl)
{
 ...
 *                 Available in \c f_cert_cb of \c esp_mbedtls_ssl_conf_cert_cb(),
 *                 this is the same value passed to \c f_sni callback of
 *                 \c esp_mbedtls_ssl_conf_sni() and may be used instead of
 *                 \c esp_mbedtls_ssl_conf_sni().
 *
 * \param ssl      SSL context
 ...
 *                   registered with \c esp_mbedtls_ssl_conf_cert_cb().
 ...
 *
 * \note           Same as \c esp_mbedtls_ssl_conf_own_cert() but for use within
 ...
 *                 current handshake
 *
 * \note           Same as \c esp_mbedtls_ssl_conf_ca_chain() but for use within
 *                 the SNI callback or the certificate selection callback.
 *
 * \param ssl      SSL context
 * \param ca_chain trusted CA chain (meaning all fully trusted top-level CAs)
 ...
 * \brief          Set DN hints sent to client in CertificateRequest message
 *
 * \note           Same as \c esp_mbedtls_ssl_conf_dn_hints() but for use within
 *                 the SNI callback or the certificate selection callback.
 *
 * \param ssl      SSL context
 * \param crt      crt chain whose subject DNs are issuer DNs of client certs
 ...
 *
 * \note           Same as \c esp_mbedtls_ssl_conf_authmode() but for use within
 *                 the SNI callback or the certificate selection callback.
 *
 * \param ssl      SSL context
 ...
 *                 esp_mbedtls_ssl_set_hs_own_cert() (can be called repeatedly),
 *                 and may optionally adjust the CA and associated CRL with \c
 *                 esp_mbedtls_ssl_set_hs_ca_chain() as well as the client
 *                 authentication mode with \c esp_mbedtls_ssl_set_hs_authmode(),
 ...
 *                 to call this function is between \c esp_mbedtls_ssl_setup() or
 *                 \c mbedtls_ssl_reset() and \c esp_mbedtls_ssl_handshake().
...
static inline const char *esp_mbedtls_ssl_get_srtp_profile_as_string(mbedtls_ssl_srtp_profile profile)
{
    switch (profile) {
 ...
 */
int esp_mbedtls_ssl_conf_dtls_srtp_protection_profiles
    (mbedtls_ssl_config *conf,
 ...
 *
 *                 See also the documentation of esp_mbedtls_ssl_conf_min_version().
 *
 * \note           This ignores ciphersuites from higher versions.
 *
 * \note           This function is deprecated and has been replaced by
 *                 \c esp_mbedtls_ssl_conf_max_tls_version().
 *
 * \param conf     SSL configuration
 ...
 */
void MBEDTLS_DEPRECATED esp_mbedtls_ssl_conf_max_version(mbedtls_ssl_config *conf, int major,
                                                     int minor);
#endif /* MBEDTLS_DEPRECATED_REMOVED */
 ...
 *                 esp_mbedtls_ssl_get_version_number() to see what version was
 *                 negotiated.
 *
 * \param conf         SSL configuration
 ...
 */
static inline void esp_mbedtls_ssl_conf_max_tls_version(mbedtls_ssl_config *conf,
 ...
 *
 * \note           This function is deprecated and has been replaced by
 *                 \c esp_mbedtls_ssl_conf_min_tls_version().
 *
 * \param conf     SSL configuration
 * \param major    Major version number (#MBEDTLS_SSL_MAJOR_VERSION_3)
 * \param minor    Minor version number
 ...
 *                 #MBEDTLS_SSL_MINOR_VERSION_4 for TLS 1.3)
 */
void MBEDTLS_DEPRECATED esp_mbedtls_ssl_conf_min_version(mbedtls_ssl_config *conf, int major,
                                                     int minor);
#endif /* MBEDTLS_DEPRECATED_REMOVED */
 ...
 *                     (#MBEDTLS_SSL_VERSION_UNKNOWN is not valid)
 */
static inline void esp_mbedtls_ssl_conf_min_tls_version(mbedtls_ssl_config *conf,
                                                    mbedtls_ssl_protocol_version tls_version)
{
 ...
 *                 with \c esp_mbedtls_ssl_read()), not handshake messages.
 *                 With DTLS, this affects both ApplicationData and handshake.
 *
 * \note           This sets the maximum length for a record's payload,
 *                 excluding record overhead that will be added to it, see
 *                 \c esp_mbedtls_ssl_get_record_expansion().
 *
 * \note           For DTLS, it is also possible to set a limit for the total
 *                 size of datagrams passed to the transport layer, including
 *                 record overhead, see \c esp_mbedtls_ssl_set_mtu().
 *
 * \param conf     SSL configuration
 ...
 */
void esp_mbedtls_ssl_conf_preference_order(mbedtls_ssl_config *conf, int order);
#endif /* MBEDTLS_SSL_SRV_C */
 ...
 *
 * \note           On server, use \c esp_mbedtls_ssl_conf_session_tickets_cb().
 *
 * \param conf     SSL configuration
 ...
 *                 HelloRequest is retransmitted every time esp_mbedtls_ssl_read() times
 ...
 *                 esp_mbedtls_ssl_read(), as opposed to esp_mbedtls_ssl_write() and esp_mbedtls_ssl_renegotiate()
 *                 which always behave as if max_record was 0. The reason is,
 *                 if we receive application data from the server, we need a
 *                 place to write it, which only happens during esp_mbedtls_ssl_read().
 *
 * \param conf     SSL configuration
 ...
 *                 \c esp_mbedtls_ssl_get_bytes_avail in that it considers
 ...
 *                 \c esp_mbedtls_ssl_read will provide any data;
 ...
 *
 */
int esp_mbedtls_ssl_check_pending(const mbedtls_ssl_context *ssl);
 ...
 *                 in case \c esp_mbedtls_ssl_read has written the maximal
 *                 amount of data fitting into the input buffer.
 *
 */
size_t esp_mbedtls_ssl_get_bytes_avail(const mbedtls_ssl_context *ssl);
 ...
 */
static inline mbedtls_ssl_protocol_version esp_mbedtls_ssl_get_version_number(
    const mbedtls_ssl_context *ssl)
{
 ...
 */
int esp_mbedtls_ssl_get_record_expansion(const mbedtls_ssl_context *ssl);
 ...
 *                 record expansion.
 *
 * \note           With DTLS, \c esp_mbedtls_ssl_write() will return an error if
 *                 called with a larger length value.
 *                 With TLS, \c esp_mbedtls_ssl_write() will fragment the input if
 *                 necessary and return the number of bytes written; it is up
 *                 to the caller to call \c esp_mbedtls_ssl_write() again in
 *                 order to send the remaining bytes if any.
 *
 * \sa             esp_mbedtls_ssl_get_max_out_record_payload()
 * \sa             esp_mbedtls_ssl_get_record_expansion()
 *
 * \param ssl      SSL context
 *
 ...
 *
 * \sa             esp_mbedtls_ssl_set_mtu()
 * \sa             esp_mbedtls_ssl_get_max_in_record_payload()
 * \sa             esp_mbedtls_ssl_get_record_expansion()
 *
 * \param ssl      SSL context
 *
 ...
 *                 callback through esp_mbedtls_ssl_conf_verify() instead of calling
 *                 this function. Using esp_mbedtls_ssl_conf_verify() also comes at
 ...
 *                 esp_mbedtls_ssl_read() and esp_mbedtls_ssl_write(); this is
 ...
 *                 This must have been initialized with esp_mbedtls_ssl_session_init()
 ...
 *                 session resumption by passing it to esp_mbedtls_ssl_set_session(),
 *                 and serialized for storage via esp_mbedtls_ssl_session_save().
 ...
 * \return         Another negative error code on other kinds of failure.
 *
 * \sa             esp_mbedtls_ssl_set_session()
 * \sa             esp_mbedtls_ssl_session_save()
 */
 ...
 *                 esp_mbedtls_ssl_conf_async_private_cb()) - in this case you
 *                 must call this function again when the operation is ready.
 * \return         #MBEDTLS_ERR_SSL_CRYPTO_IN_PROGRESS if a cryptographic
 *                 operation is in progress (see esp_mbedtls_ecp_set_max_ops()) -
 ...
 *                 enabled on server (see esp_mbedtls_ssl_conf_early_data()
 *                 documentation). You must call esp_mbedtls_ssl_read_early_data()
 ...
 *                 and either free it or call \c esp_mbedtls_ssl_session_reset()
...
/**
 * \brief          After calling esp_mbedtls_ssl_handshake() to start the SSL
 ...
 */
static inline int esp_mbedtls_ssl_is_handshake_over(mbedtls_ssl_context *ssl)
{
 ...
 *                 call this function if esp_mbedtls_ssl_is_handshake_over()
 *                 returns \c 1.
 *
 ...
 * \param ssl      SSL context
 *
 * \return         See esp_mbedtls_ssl_handshake().
 *
 ...
 *                 or call \c esp_mbedtls_ssl_session_reset() on it before
 ...
 *                 during the next call to esp_mbedtls_ssl_read() if honored by
 *                 client.
 *
 * \param ssl      SSL context
 *
 * \return         0 if successful, or any esp_mbedtls_ssl_handshake() return
 ...
 *                 continue with \c esp_mbedtls_ssl_handshake() with the same
 ...
 *                   \c esp_mbedtls_ssl_check_pending to check for remaining records.
 *
 */
int esp_mbedtls_ssl_read(mbedtls_ssl_context *ssl, unsigned char *buf, size_t len);
 ...
 *                 \c esp_mbedtls_ssl_get_max_out_record_payload() may be used to
 *                 query the active maximum fragment length.
 *
 * \note           Attempting to write 0 bytes will result in an empty TLS
 *                 application record being sent.
 */
int esp_mbedtls_ssl_write(mbedtls_ssl_context *ssl, const unsigned char *buf, size_t len);
 ...
 *                 call \c esp_mbedtls_ssl_session_reset() on it before re-using it
 *                 for a new connection; the current connection must be closed.
 */
int esp_mbedtls_ssl_send_alert_message(mbedtls_ssl_context *ssl,
                                   unsigned char level,
 ...
 *                 for a new connection; the current connection must be closed.
 */
int esp_mbedtls_ssl_close_notify(mbedtls_ssl_context *ssl);
 ...
 *                 esp_mbedtls_ssl_handshake(), esp_mbedtls_ssl_handshake_step(),
 *                 esp_mbedtls_ssl_read() and esp_mbedtls_ssl_write() to read early
 ...
 *                 context \p ssl only after a call to esp_mbedtls_ssl_handshake(),
 *                 esp_mbedtls_ssl_handshake_step(), esp_mbedtls_ssl_read() or
 *                 esp_mbedtls_ssl_write() for \p ssl that has returned
 *                 #MBEDTLS_ERR_SSL_RECEIVED_EARLY_DATA.
 */
int esp_mbedtls_ssl_read_early_data(mbedtls_ssl_context *ssl,
 ...
 *                   early_exporter_master_secret.
 *
 * \note           This function behaves mainly as esp_mbedtls_ssl_write(). The
 *                 specification of esp_mbedtls_ssl_write() relevant to TLS 1.3
 *                 (thus not the parts specific to (D)TLS1.2) applies to this
 *                 function and the present documentation is mainly restricted
 *                 to the differences with esp_mbedtls_ssl_write(). One noticeable
 *                 difference though is that esp_mbedtls_ssl_write() aims to
 ...
 *                 esp_mbedtls_ssl_write(), esp_mbedtls_ssl_read() or
 *                 esp_mbedtls_ssl_handshake() and the handshake can be
 ...
 *                 esp_mbedtls_ssl_get_early_data_status() with the handshake
 *                 completed.
 */
int esp_mbedtls_ssl_write_early_data(mbedtls_ssl_context *ssl,
 ...
 *                 esp_mbedtls_ssl_write_early_data() as ordinary post-handshake
 *                 application data by calling esp_mbedtls_ssl_write().
 *
 */
int esp_mbedtls_ssl_get_early_data_status(mbedtls_ssl_context *ssl);
#endif /* MBEDTLS_SSL_CLI_C */
 ...
 *
 * \see            esp_mbedtls_ssl_context_load()
 *
 ...
 *                 esp_mbedtls_ssl_session_reset() on \p ssl which as a result is
 ...
 *                 esp_mbedtls_ssl_free() on it. See the documentation of
 *                 esp_mbedtls_ssl_session_reset() for more details.
 *
 ...
 *
 * \see            esp_mbedtls_ssl_context_save()
 *
 ...
 *                 esp_mbedtls_ssl_init() and call esp_mbedtls_ssl_setup() on it with
 *                 the same ::mbedtls_ssl_config structure that was used in
 *                 the original connection. The second way is to
 *                 call esp_mbedtls_ssl_session_reset() on a context that was
 *                 previously prepared as above but used in the meantime.
 *                 Either way, you must not use the context to perform a
 *                 handshake between calling esp_mbedtls_ssl_setup() or
 *                 esp_mbedtls_ssl_session_reset() and calling this function. You
 ...
 *                 esp_mbedtls_ssl_session_reset() and the values are suitable for
 *                 the present connection). Specifically, you want to call
 *                 at least esp_mbedtls_ssl_set_bio(),
 *                 esp_mbedtls_ssl_set_timer_cb(), and
 *                 esp_mbedtls_ssl_set_user_data_n() or
 *                 esp_mbedtls_ssl_set_user_data_p() if they were set originally.
 ...
 *                 call esp_mbedtls_ssl_set_mtu() after calling this function, as
 ...
 *                 esp_mbedtls_ssl_free() on \p ssl. In this case, you need to
 *                 prepare the context with the usual sequence starting with a
 *                 call to esp_mbedtls_ssl_init() if you want to use it again.
 *
 ...
 * \return         #MBEDTLS_ERR_SSL_BAD_INPUT_DATA if input data is invalid.
 */
int esp_mbedtls_ssl_context_load(mbedtls_ssl_context *ssl,
                             const unsigned char *buf,
                             size_t len);
 ...
 *                 esp_mbedtls_ssl_config_defaults() or esp_mbedtls_ssl_config_free().
 *
 * \note           You need to call esp_mbedtls_ssl_config_defaults() unless you
 ...
 *                 (You need to call esp_mbedtls_ssl_config_init() first.)
 *
 * \param conf     SSL configuration context
 ...
 *
 * \note           See \c esp_mbedtls_ssl_conf_transport() for notes on DTLS.
 *
 * \return         0 if successful, or
