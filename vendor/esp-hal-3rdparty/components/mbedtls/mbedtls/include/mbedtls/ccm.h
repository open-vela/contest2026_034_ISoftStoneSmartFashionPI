 ...
 *                  for esp_mbedtls_ccm_setkey() or esp_mbedtls_ccm_free().
 *
 ...
 *                  operation.
 *
 *                  This function and esp_mbedtls_ccm_set_lengths() must be called
 *                  before calling esp_mbedtls_ccm_update_ad() or
 *                  esp_mbedtls_ccm_update(). This function can be called before
 *                  or after esp_mbedtls_ccm_set_lengths().
 *
 ...
 *                  operation.
 *
 *                  This function and esp_mbedtls_ccm_starts() must be called
 *                  before calling esp_mbedtls_ccm_update_ad() or
 *                  esp_mbedtls_ccm_update(). This function can be called before
 *                  or after esp_mbedtls_ccm_starts().
 *
 * \note            This function is not implemented in Mbed TLS yet.
 *
 * \param ctx       The CCM context. This must be initialized.
 ...
 *                  \c total_ad_len passed to esp_mbedtls_ccm_set_lengths(). You
 *                  may not call this function after calling
 *                  esp_mbedtls_ccm_update().
 *
 * \note            This function is not implemented in Mbed TLS yet.
 *
 * \param ctx       The CCM context. This must have been started with
 *                  esp_mbedtls_ccm_starts(), the lengths of the message and
 *                  additional data must have been declared with
 *                  esp_mbedtls_ccm_set_lengths() and this must not have yet
 *                  received any input with esp_mbedtls_ccm_update().
 ...
 *                    \c plaintext_len passed to esp_mbedtls_ccm_set_lengths()
 ...
 *
 * \param ctx           The CCM context. This must have been started with
 *                      esp_mbedtls_ccm_starts() and the lengths of the message and
 *                      additional data must have been declared with
 *                      esp_mbedtls_ccm_set_lengths().
 ...
 *                  esp_mbedtls_ccm_set_lengths() function.
 *
 * \return          \c 0 on success.
 * \return          #MBEDTLS_ERR_CCM_BAD_INPUT on failure:
 *                  \p ctx is in an invalid state,
 ...
 *                  esp_mbedtls_ccm_update_ad() was lower than the total length of
 *                  additional data \c total_ad_len passed to
 *                  esp_mbedtls_ccm_set_lengths(),
 *                  the total amount of input data passed to
 *                  esp_mbedtls_ccm_update() was lower than the plaintext length
 *                  \c plaintext_len passed to esp_mbedtls_ccm_set_lengths().
 */
