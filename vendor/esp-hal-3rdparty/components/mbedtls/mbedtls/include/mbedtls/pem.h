 ...
 *                  appropriate length
 *
 * \note            \c esp_mbedtls_pem_free must be called on PEM context before
 *                  the PEM context can be reused in another call to
 *                  \c esp_mbedtls_pem_read_buffer
 *
 ...
 * \note            The returned pointer remains valid only until \p ctx is
                    modified or freed.
 */
static inline const unsigned char *esp_mbedtls_pem_get_buffer(mbedtls_pem_context *ctx, size_t *buflen)
{
