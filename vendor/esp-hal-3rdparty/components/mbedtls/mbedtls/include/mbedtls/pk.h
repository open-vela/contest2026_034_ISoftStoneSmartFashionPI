 ...
 *                  See \c esp_mbedtls_rsa_rsassa_pss_verify_ext()
 */
typedef struct mbedtls_pk_rsassa_pss_options {
     ...
     *       to esp_mbedtls_pk_verify_ext(). In a future version of the library,
...
/**
 * \brief           Maximum size of a signature made by esp_mbedtls_pk_sign().
 */
 ...
 * we use that (esp_rsa_alt_sign_wrap in library/pk_wrap.h will check). */
 ...
 * as ECP_C is defined esp_mbedtls_pk_ec() gives the user a read/write access to the
 ...
 *              esp_mbedtls_cipher_info_from_type().
 */
 ...
 *                  esp_mbedtls_pk_setup_opaque(), this does not free the underlying
 *                  PSA key and you still need to call esp_psa_destroy_key()
 ...
 *                  \c esp_mbedtls_pk_setup_rsa_alt() instead.
 */
 ...
 *
 * \note            This function replaces esp_mbedtls_pk_setup() for contexts
 ...
 *                  esp_mbedtls_pk_free() is called on this context. The wrapped
 ...
 *
 * \note            This function replaces \c esp_mbedtls_pk_setup() for RSA-alt.
 */
 ...
 */
static inline size_t esp_mbedtls_pk_get_len(const mbedtls_pk_context *ctx)
{
    return (esp_mbedtls_pk_get_bitlen(ctx) + 7) / 8;
}
 ...
 *                  cleared with esp_mbedtls_pk_free().
 */
 ...
 *                  or that has been cleared with esp_mbedtls_pk_free().
 */
 ...
 *
 * This function is equivalent to calling esp_psa_import_key()
 * with the key material from \p pk.
 *
 * The typical way to use this function is:
 * -# Call esp_mbedtls_pk_get_psa_attributes() to obtain
 ...
 * -# Call esp_mbedtls_pk_import_into_psa().
 *
 * \note            This function does not support RSA-alt contexts
 *                  (set up with esp_mbedtls_pk_setup_rsa_alt()).
 *
 ...
 *                  from.
 *                  Calling esp_mbedtls_pk_sign(), esp_mbedtls_pk_verify(),
 *                  esp_mbedtls_pk_encrypt(), esp_mbedtls_pk_decrypt() on the resulting
 ...
 */
int esp_mbedtls_pk_copy_from_psa(mbedtls_svc_key_id_t key_id, mbedtls_pk_context *pk);
 ...
 *                  independent from the original PSA key that it was generated
 *                  from.
 *                  Calling esp_mbedtls_pk_verify() or
 *                  esp_mbedtls_pk_encrypt() on the resulting
 *                  PK context will perform the corresponding algorithm for that
 *                  PK context type.
 *
 *                  For an RSA key, the output PK context will allow both
 ...
 *                  parameters are not correct.
 */
int esp_mbedtls_pk_copy_public_from_psa(mbedtls_svc_key_id_t key_id, mbedtls_pk_context *pk);
 ...
 *                  by default. Use esp_mbedtls_pk_verify_ext() to explicitly select
...
/**
 * \brief           Restartable version of \c esp_mbedtls_pk_verify()
 *
 * \note            Performs the same job as \c esp_mbedtls_pk_verify(), but can
 *                  return early and restart according to the limit set with
 *                  \c esp_mbedtls_ecp_set_max_ops() to reduce blocking for ECC
 *                  operations. For RSA, same as \c esp_mbedtls_pk_verify().
 *
 * \param ctx       The PK context to use. It must have been set up.
 ...
 *
 * \return          See \c esp_mbedtls_pk_verify(), or
 ...
 *                  esp_mbedtls_pk_verify_ext() to explicitly select a different
 *                  algorithm.
 *
...
/**
 * \brief           Restartable version of \c esp_mbedtls_pk_sign()
 *
 * \note            Performs the same job as \c esp_mbedtls_pk_sign(), but can
 *                  return early and restart according to the limit set with
 *                  \c esp_mbedtls_ecp_set_max_ops() to reduce blocking for ECC
 *                  operations. For RSA, same as \c esp_mbedtls_pk_sign().
 *
 * \param ctx       The PK context to use. It must have been set up
 *                  with a private key.
 * \param md_alg    Hash algorithm used (see notes for esp_mbedtls_pk_sign())
 * \param hash      Hash of the message to sign
 * \param hash_len  Hash length
 * \param sig       Place to write the signature.
 *                  It must have enough room for the signature.
 ...
 * \param rs_ctx    Restart context (NULL to disable restart)
 *
 * \return          See \c esp_mbedtls_pk_sign().
 * \return          #MBEDTLS_ERR_ECP_IN_PROGRESS if maximum number of
 *                  operations was reached: see \c esp_mbedtls_ecp_set_max_ops().
 */
 ...
 * returned by esp_mbedtls_pk_get_type(), is #MBEDTLS_PK_RSA.
 ...
 */
static inline mbedtls_rsa_context *esp_mbedtls_pk_rsa(const mbedtls_pk_context pk)
{
 ...
 *
 * \warning This function can only be used when the type of the context, as
 * returned by esp_mbedtls_pk_get_type(), is #MBEDTLS_PK_ECKEY,
 ...
 */
static inline mbedtls_ecp_keypair *esp_mbedtls_pk_ec(const mbedtls_pk_context pk)
{
    switch (esp_mbedtls_pk_get_type(&pk)) {
 ...
 *                  with esp_mbedtls_pk_init() or reset with esp_mbedtls_pk_free(). If you need a
 *                  specific key type, check the result with esp_mbedtls_pk_can_do().
 *
 * \note            The key is also checked for correctness.
 *
 * \return          0 if successful, or a specific PK or PEM error code
 */
int esp_mbedtls_pk_parse_key(mbedtls_pk_context *ctx,
 ...
 *
 * \note            On entry, ctx must be empty, either freshly initialised
 *                  with esp_mbedtls_pk_init() or reset with esp_mbedtls_pk_free(). If
 *                  you need a specific key type, check the result with
 *                  esp_mbedtls_pk_can_do().
 *
 * \note            The key is also checked for correctness.
 *
 * \return          0 if successful, or a specific PK or PEM error code
 ...
 *                  error code
 */
int esp_mbedtls_pk_write_key_der(const mbedtls_pk_context *ctx, unsigned char *buf, size_t size);
 ...
 *                  error code
 */
int esp_mbedtls_pk_write_pubkey_der(const mbedtls_pk_context *ctx, unsigned char *buf, size_t size);
 ...
 */
int esp_mbedtls_pk_write_pubkey_pem(const mbedtls_pk_context *ctx, unsigned char *buf, size_t size);
 ...
 * \return          0 if successful, or a specific error code
 */
int esp_mbedtls_pk_write_key_pem(const mbedtls_pk_context *ctx, unsigned char *buf, size_t size);
 ...
 */
int esp_mbedtls_pk_write_pubkey(unsigned char **p, unsigned char *start,
