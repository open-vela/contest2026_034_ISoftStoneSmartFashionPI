 ...
 * esp_mbedtls_platform_gmtime_r() (see below).
 ...
 *
 * \note esp_mbedtls_platform_gmtime_r() is an abstraction in platform_util.h that
 * behaves similarly to the gmtime_r() function from the C standard. Refer to
 * the documentation for esp_mbedtls_platform_gmtime_r() for more information.
 *
 * \note It is possible to configure an implementation for
 * esp_mbedtls_platform_gmtime_r() at compile-time by using the macro
 ...
 *
 * An overview of how the value of esp_mbedtls_calloc is determined:
 *
 * - if !MBEDTLS_PLATFORM_MEMORY
 *     - esp_mbedtls_calloc = calloc
 * - if MBEDTLS_PLATFORM_MEMORY
 *     - if (MBEDTLS_PLATFORM_CALLOC_MACRO && MBEDTLS_PLATFORM_FREE_MACRO):
 *         - esp_mbedtls_calloc = MBEDTLS_PLATFORM_CALLOC_MACRO
 *     - if !(MBEDTLS_PLATFORM_CALLOC_MACRO && MBEDTLS_PLATFORM_FREE_MACRO):
 *         - Dynamic setup via esp_mbedtls_platform_set_calloc_free is now possible with a default value MBEDTLS_PLATFORM_STD_CALLOC.
 ...
 *
 *         - esp_mbedtls_calloc = MBEDTLS_PLATFORM_STD_CALLOC.
 *
 ...
 * provide a function "esp_mbedtls_platform_set_printf()" that allows you to set an
 ...
 * esp_mbedtls_platform_gmtime_r(). This replaces the default implementation in
 * platform_util.c.
 *
 * gmtime() is not a thread-safe function as defined in the C standard. The
 * library will try to use safer implementations of this function, such as
 * gmtime_r() when available. However, if Mbed TLS cannot identify the target
 * system, the implementation of esp_mbedtls_platform_gmtime_r() will default to
 * using the standard gmtime(). In this case, calls from the library to
 * gmtime() will be guarded by the global mutex mbedtls_threading_gmtime_mutex
 * if MBEDTLS_THREADING_C is enabled. We recommend that calls from outside the
 * library are also guarded with this mutex to avoid race conditions. However,
 * if the macro MBEDTLS_PLATFORM_GMTIME_R_ALT is defined, Mbed TLS will
 * unconditionally use the implementation for esp_mbedtls_platform_gmtime_r()
...
/**
 * Uncomment the macro to let Mbed TLS use your alternate implementation of
 * esp_mbedtls_platform_zeroize(), to wipe sensitive data in memory. This replaces
 ...
 * `explicit_bzero` to use that function as esp_mbedtls_platform_zeroize().
 */
 ...
 *
 * Mark deprecated functions and esp_features so that they generate a warning if
 ...
 *
 * Uncomment to get warnings on using deprecated functions and esp_features.
 */
 ...
 *
 * Remove deprecated functions and esp_features so that they generate an error if
 * used. Functionality deprecated in one version will usually be removed in the
 * next version. You can enable this to help you prepare the transition to a
 * new major version by making sure your code is not using this functionality.
 *
 * Uncomment to get errors on using deprecated functions and esp_features.
 */
 ...
 * esp_mbedtls_timing_get_timer(), mbedtls_set_alarm(), mbedtls_set/get_delay()
 *
 ...
 *            the static HMAC-DRBG as RNG to esp_mbedtls_ecdsa_sign(). Therefore
 *            alternative implementations should use the RNG only for generating
 *            the ephemeral key and nothing else. If this is not possible, then
 *            MBEDTLS_ECDSA_DETERMINISTIC should be disabled and an alternative
 *            implementation should be provided for esp_mbedtls_ecdsa_sign_det_ext().
 *
 */
 ...
 * MBEDTLS_ECP_DOUBLE_JAC_ALT, Mbed TLS will still provide the esp_ecp_double_jac()
 * function, but will use your mbedtls_internal_ecp_double_jac() if the group
 * for the operation is supported by your implementation (i.e. your
 * mbedtls_internal_ecp_grp_capable() function returns 1 for this group). If the
 * group is not supported by your implementation, then the original Mbed TLS
 * implementation of esp_ecp_double_jac() is used instead, unless this fallback
 * behaviour is disabled by setting MBEDTLS_ECP_NO_FALLBACK (in which case
 * esp_ecp_double_jac() will return MBEDTLS_ERR_ECP_FEATURE_UNAVAILABLE).
 *
 ...
 * necessary esp_features. It will not build unless at least one of MBEDTLS_AESCE_C
 ...
 * - Adds esp_mbedtls_ecdh_enable_restart() in the ECDH module.
 ...
 *        esp_mbedtls_ssl_handshake(), can now return
 ...
 * esp_psa_crypto_init() before the first hanshake (even if
 ...
 *
 * Do not add default entropy sources in esp_mbedtls_entropy_init().
 *
 ...
 * call esp_psa_crypto_init() before doing any PKCS#1 v2.1 operation.
 *
 ...
 * esp_mbedtls_ssl_get_own_cid()`, `esp_mbedtls_ssl_get_peer_cid()` and
 * `esp_mbedtls_ssl_conf_cid()`. See the corresponding documentation for
 * more information.
 *
 ...
 * are only provided by PSA drivers, you must call esp_psa_crypto_init() before
 ...
 *       call esp_psa_crypto_init() before the first TLS 1.3 handshake.
 *
 ...
 *
 * Enable esp_features for invasive testing such as introspection functions and
 ...
 *
 * \warning If you enable this option, you need to call `esp_psa_crypto_init()`
 ...
 *
 * Uncomment to omit mbedtls_x509_*_info(), as well as esp_mbedtls_debug_print_crt()
 ...
 * is used.
 *
 * \warning When using PSA, the user should call `esp_psa_crypto_init()` before
 *          using any CTR_DRBG operation (except `esp_mbedtls_ctr_drbg_init()`).
 *
 ...
 *
 * \warning If using a hash that is only provided by PSA drivers, you must
 * call esp_psa_crypto_init() before doing any EC J-PAKE operations.
 */
 ...
 * own networking callbacks to be passed to \c esp_mbedtls_ssl_set_bio().
 *
 ...
 * a PSA driver, you must call esp_psa_crypto_init() before the first file.
 *
 ...
 *
 * \warning If using a hash that is only provided by PSA drivers, you must
 * call esp_psa_crypto_init() before doing any PKCS5 operations.
 *
 ...
 *
 * \warning If using a hash that is only provided by PSA drivers, you must
 * call esp_psa_crypto_init() before doing any PKCS12 operations.
 *
 ...
 * esp_psa_crypto_init() before doing any X.509 operation.
 *
 ...
 *
 * \warning If building with MBEDTLS_USE_PSA_CRYPTO, you must call
 * esp_psa_crypto_init() before doing any X.509 create operation.
 *
...
//#define MBEDTLS_PLATFORM_STD_NV_SEED_READ   esp_mbedtls_platform_std_nv_seed_read /**< Default nv_seed_read function to use, can be undefined */
//#define MBEDTLS_PLATFORM_STD_NV_SEED_WRITE  esp_mbedtls_platform_std_nv_seed_write /**< Default nv_seed_write function to use, can be undefined */
...
//#define MBEDTLS_PLATFORM_NV_SEED_READ_MACRO   esp_mbedtls_platform_std_nv_seed_read /**< Default nv_seed_read function to use, can be undefined */
//#define MBEDTLS_PLATFORM_NV_SEED_WRITE_MACRO  esp_mbedtls_platform_std_nv_seed_write /**< Default nv_seed_write function to use, can be undefined */
 ...
 * \c esp_mbedtls_ssl_conf_max_early_data_size() for more information.
 *
