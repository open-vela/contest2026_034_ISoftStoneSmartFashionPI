 ...
 *                 #MBEDTLS_MD_NONE. See esp_mbedtls_rsa_set_padding() for more
 ...
 *                 by a call to esp_mbedtls_rsa_complete(), which checks and
 *                 completes the provided information to a ready-for-use
 *                 public or private RSA key.
 *
 * \note           See esp_mbedtls_rsa_complete() for more information on which
 ...
 *                 of the key material, see esp_mbedtls_rsa_check_privkey().
 *
 ...
 *
 * \note           esp_mbedtls_rsa_init() must be called before this function,
 ...
 *                 operation using esp_mbedtls_rsa_public().
 *
 ...
 *             ensure that esp_mbedtls_rsa_private() can be called successfully
 *             on the given context, but that the various parameters are
 *             mutually consistent with high probability, in the sense that
 *             esp_mbedtls_rsa_public() and esp_mbedtls_rsa_private() are inverses.
 *
 ...
 *             and therefore cannot check them. See esp_mbedtls_rsa_complete().
 *             If you want to check the consistency of the entire
 *             content of a PKCS1-encoded RSA private key, for example, you
 *             should use esp_mbedtls_rsa_validate_params() before setting
 *             up the RSA context.
 ...
 *                 esp_mbedtls_rsa_rsaes_pkcs1_v15_decrypt() is called, which is an
 ...
 *                 mandatory; see esp_mbedtls_rsa_private() for more.
 ...
 *                 esp_mbedtls_rsa_rsassa_pss_sign() for details on
 ...
 *                 esp_mbedtls_rsa_set_padding() selects the hash used for the
 ...
 *
 * \note           For PKCS#1 v2.1 encoding, see comments on
 *                 esp_mbedtls_rsa_rsassa_pss_verify() about \c md_alg and
 *                 \c hash_id.
 *
 ...
 *                 esp_mbedtls_rsa_set_padding() is #MBEDTLS_MD_NONE, the \p md_alg
 *                 parameter is used.
 *
 * \param ctx      The initialized RSA public key context to use.
 * \param md_alg   The message-digest algorithm used to hash the original data.
 ...
 *                 of \p ctx->N. For example, 128 Bytes if RSA-1024 is used.
 *
 * \note           The \c hash_id set in \p ctx by esp_mbedtls_rsa_set_padding() is
 *                 ignored.
 *
 * \param ctx      The initialized RSA public key context to use.
 * \param md_alg   The message-digest algorithm used to hash the original data.
