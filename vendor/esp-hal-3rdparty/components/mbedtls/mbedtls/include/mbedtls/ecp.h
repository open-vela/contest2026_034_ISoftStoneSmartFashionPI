 ...
 * sources should be used. See esp_mbedtls_ecp_group_load().
 */
 ...
 * - Add the curve to the esp_ecp_supported_curves array in ecp.c.
 ...
 * odd prime as esp_mbedtls_ecp_mul() requires an odd number, and
 * esp_mbedtls_ecdsa_sign() requires that it is prime for blinding purposes.
 *
 ...
 * for example:
 * \code
 * esp_mbedtls_mpi_init(&A);
 * esp_mbedtls_ecp_group_init(&grp);
 * CHECK_RETURN(esp_mbedtls_ecp_group_load(&grp, grp_id));
 * if (esp_mbedtls_ecp_group_a_is_minus_3(&grp)) {
 *     CHECK_RETURN(esp_mbedtls_mpi_sub_int(&A, &grp.P, 3));
 * } else {
 *     CHECK_RETURN(esp_mbedtls_mpi_copy(&A, &grp.A));
 * }
 *
 * do_something_with_a(&A);
 *
 * cleanup:
 * esp_mbedtls_mpi_free(&A);
...
typedef struct mbedtls_ecp_group {
    ...
    mbedtls_ecp_point *MBEDTLS_PRIVATE(T);       /*!< Pre-computed points for esp_ecp_mul_comb(). */
...
typedef struct {
    ...
    mbedtls_ecp_restart_mul_ctx *MBEDTLS_PRIVATE(rsm);   /*!<  esp_ecp_mul_comb() sub-context    */
...
#define MBEDTLS_ECP_OPS_DBL   8 /*!< basic ops count for esp_ecp_double_jac()    */
#define MBEDTLS_ECP_OPS_ADD  11 /*!< basic ops count for see esp_ecp_add_mixed() */
...
#define MBEDTLS_ECP_BUDGET(ops)   \
    MBEDTLS_MPI_CHK(esp_mbedtls_ecp_check_budget(grp, rs_ctx, \
                                             (unsigned) (ops)));
 ...
 *                  esp_mbedtls_ecdh_enable_restart() was previously called on
 ...
 *                  multiplication (full run of \c esp_mbedtls_ecp_mul()) is:
 ...
 *                  supported for all algorithms. Call esp_mbedtls_ecdh_can_do()
 *                  or esp_mbedtls_ecdsa_can_do() to check if a curve is
 ...
 *                  esp_mbedtls_ecp_group_load() or esp_mbedtls_ecp_tls_read_group()
 *                  functions.
 */
 ...
 */
int esp_mbedtls_ecp_group_copy(mbedtls_ecp_group *dst,
 ...
 *                  set, for example through esp_mbedtls_ecp_group_load().
 ...
 * \return          Another negative error code on other kinds of failure.
 */
int esp_mbedtls_ecp_tls_write_point(const mbedtls_ecp_group *grp,
 ...
 * \return          Another negative error code on other kinds of failure.
 */
int esp_mbedtls_ecp_group_load(mbedtls_ecp_group *grp, mbedtls_ecp_group_id id);
 ...
 * \return          Another negative error code on other kinds of failure.
 */
int esp_mbedtls_ecp_tls_write_group(const mbedtls_ecp_group *grp,
                                size_t *olen,
                                unsigned char *buf, size_t blen);
 ...
 *
 * \see             esp_mbedtls_ecp_mul()
 *
 * \note            This function does the same as \c esp_mbedtls_ecp_mul(), but
 *                  it can return early and restart according to the limit set
 *                  with \c esp_mbedtls_ecp_set_max_ops() to reduce blocking.
 *
 * \param grp       The ECP group to use.
 *                  This must be initialized and have group parameters
 *                  set, for example through esp_mbedtls_ecp_group_load().
 ...
 *                  operations was reached: see \c esp_mbedtls_ecp_set_max_ops().
 * \return          Another negative error code on other kinds of failure.
 */
 ...
 */
static inline int esp_mbedtls_ecp_group_a_is_minus_3(const mbedtls_ecp_group *grp)
{
 ...
 *                  It is not thread-safe to use same group in multiple threads.
 *
 * \note            In contrast to esp_mbedtls_ecp_mul(), this function does not
 ...
 *                  restartable way.
 *
 * \see             \c esp_mbedtls_ecp_muladd()
 *
 * \note            This function works the same as \c esp_mbedtls_ecp_muladd(),
 *                  but it can return early and restart according to the limit
 *                  set with \c esp_mbedtls_ecp_set_max_ops() to reduce blocking.
 *
 * \note            This function is only defined for short Weierstrass curves.
 *                  It may not be included in builds without any short
 *                  Weierstrass curve.
 ...
 *                  point \c grp->G used by esp_mbedtls_ecp_gen_keypair().
 ...
 *                  on failure.
 */
int esp_mbedtls_ecp_gen_keypair(mbedtls_ecp_group *grp, mbedtls_mpi *d,
                            mbedtls_ecp_point *Q,
                            int (*f_rng)(void *, unsigned char *, size_t),
                            void *p_rng);
 ...
 *                  call esp_mbedtls_ecp_check_pub_priv() after setting both
 ...
 *                  Call esp_mbedtls_ecp_keypair_calc_public() to set the public
 *                  key from the private key. Alternatively, you can call
 *                  esp_mbedtls_ecp_set_public_key() to set the public key part,
 *                  and then optionally esp_mbedtls_ecp_check_pub_priv() to check
 ...
 *                  with the private key. Call esp_mbedtls_ecp_check_pub_priv()
 ...
 *                  To avoid this difficulty, use esp_mbedtls_ecp_write_key_ext()
 *                  instead.
 *                  esp_mbedtls_ecp_write_key() is deprecated and will be
 ...
 * \return          Another negative error code on different kinds of failure.
 */
int MBEDTLS_DEPRECATED esp_mbedtls_ecp_write_key(mbedtls_ecp_keypair *key,
                                             unsigned char *buf, size_t buflen);
 ...
 *                  error code on calculation failure.
 */
int esp_mbedtls_ecp_keypair_calc_public(
    mbedtls_ecp_keypair *key,
    int (*f_rng)(void *, unsigned char *, size_t), void *p_rng);
