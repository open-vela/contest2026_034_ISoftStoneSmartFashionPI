...
typedef struct mbedtls_x509_crt {
    ...
    mbedtls_x509_sequence subject_alt_names; /**< Optional list of raw entries of Subject Alternative Names extension. These can be later parsed by esp_mbedtls_x509_parse_subject_alt_name. */
    ...
    void *MBEDTLS_PRIVATE(sig_opts);             /**< Signature options to be passed to esp_mbedtls_pk_verify_ext(), e.g. for RSASSA-PSS */
 ...
 * an existing profile, such as esp_mbedtls_x509_crt_profile_default or
 * mbedtls_x509_ctr_profile_none and then tune it to your needs.
 *
 * For example to allow SHA-224 in addition to the default:
 *
 *  mbedtls_x509_crt_profile my_profile = esp_mbedtls_x509_crt_profile_default;
 *  my_profile.allowed_mds |= MBEDTLS_X509_ID_FLAG( MBEDTLS_MD_SHA224 );
 *
 * Or to allow only RSA-3072+ with SHA-256:
 *
 *  mbedtls_x509_crt_profile my_profile = esp_mbedtls_x509_crt_profile_none;
 ...
 *                  are supported.
 */
int esp_mbedtls_x509write_crt_set_subject_alternative_name(mbedtls_x509write_cert *ctx,
 ...
 */
extern const mbedtls_x509_crt_profile esp_mbedtls_x509_crt_profile_default;
 ...
 */
extern const mbedtls_x509_crt_profile esp_mbedtls_x509_crt_profile_next;

/**
 * NSA Suite B profile.
 */
extern const mbedtls_x509_crt_profile esp_mbedtls_x509_crt_profile_suiteb;

/**
 * Empty profile that allows nothing. Useful as a basis for constructing
 * custom profiles.
 */
extern const mbedtls_x509_crt_profile esp_mbedtls_x509_crt_profile_none;
 ...
 *                 use esp_mbedtls_x509_crt_parse_der_nocopy() instead.
 *
 ...
 *                 esp_mbedtls_x509_crt_parse_der_with_ext_cb() routine when
 ...
 *                 esp_mbedtls_x509_crt_parse_der_with_ext_cb() also fails.
 *                 When the callback fails to parse a non critical extension
 *                 esp_mbedtls_x509_crt_parse_der_with_ext_cb() simply skips
 ...
 *                   is destroyed (like esp_mbedtls_x509_crt_parse_der_nocopy())
 * \param cb         A callback invoked for every unsupported certificate
 *                   extension.
 * \param p_ctx      An opaque context passed to the callback.
 *
 * \note             This call is functionally equivalent to
 *                   esp_mbedtls_x509_crt_parse_der(), and/or
 *                   esp_mbedtls_x509_crt_parse_der_nocopy()
 ...
 *                 variant of esp_mbedtls_x509_crt_parse_der() which takes
 ...
 *                 through a call to esp_mbedtls_x509_crt_free().
 * \param buflen   The size in Bytes of \p buf.
 *
 * \note           This call is functionally equivalent to
 *                 esp_mbedtls_x509_crt_parse_der(), but it avoids creating a
 ...
 *
 */
int esp_mbedtls_x509_crt_parse(mbedtls_x509_crt *chain, const unsigned char *buf, size_t buflen);
 ...
 * \param size     Maximum size of buffer
 * \param prefix   A line prefix
 * \param flags    Verification flags created by esp_mbedtls_x509_crt_verify()
 *
 * \return         The length of the string written (not including the
 *                 terminated nul byte), or a negative error code.
 */
 ...
 *                 using \c esp_mbedtls_x509_crt_verify_info()
 *
 * \note           Same as \c esp_mbedtls_x509_crt_verify_with_profile() with the
 ...
 *                 of esp_mbedtls_x509_crt_verify() for more information.
 ...
 *
 * \note           Same as \c esp_mbedtls_x509_crt_verify(), but with explicit
 *                 security profile.
 *
 ...
 *                      verification routine esp_mbedtls_x509_crt_verify_with_ca_cb()
 ...
 *                      to a esp_heap-allocated linked list of instances of
...
#if defined(MBEDTLS_X509_TRUSTED_CERTIFICATE_CALLBACK)
/**
 * \brief          Version of \c esp_mbedtls_x509_crt_verify_with_profile() which
 ...
 *                 checked by \c esp_mbedtls_x509_crt_verify().
 */
 ...
 *                      non-zero otherwise
 */
static inline int esp_mbedtls_x509_crt_has_ext_type(const mbedtls_x509_crt *ctx,
                                                int ext_type)
{
 ...
 */
void esp_mbedtls_x509write_crt_init(mbedtls_x509write_cert *ctx);
 ...
 */
void esp_mbedtls_x509write_crt_set_version(mbedtls_x509write_cert *ctx, int version);
 ...
 *                  esp_mbedtls_x509write_crt_set_serial_raw() instead.
 *
 ...
 */
int MBEDTLS_DEPRECATED esp_mbedtls_x509write_crt_set_serial(
 ...
 */
int esp_mbedtls_x509write_crt_set_serial_raw(mbedtls_x509write_cert *ctx,
 ...
 */
int esp_mbedtls_x509write_crt_set_validity(mbedtls_x509write_cert *ctx, const char *not_before,
 ...
 *                  a specific error code
 */
int esp_mbedtls_x509write_crt_set_issuer_name(mbedtls_x509write_cert *ctx,
 ...
 *                  a specific error code
 */
int esp_mbedtls_x509write_crt_set_subject_name(mbedtls_x509write_cert *ctx,
 ...
 */
void esp_mbedtls_x509write_crt_set_subject_key(mbedtls_x509write_cert *ctx, mbedtls_pk_context *key);
 ...
 */
void esp_mbedtls_x509write_crt_set_issuer_key(mbedtls_x509write_cert *ctx, mbedtls_pk_context *key);
 ...
 */
void esp_mbedtls_x509write_crt_set_md_alg(mbedtls_x509write_cert *ctx, mbedtls_md_type_t md_alg);
 ...
 */
int esp_mbedtls_x509write_crt_set_extension(mbedtls_x509write_cert *ctx,
 ...
 * \return          0 if successful, or a MBEDTLS_ERR_X509_ALLOC_FAILED
 */
int esp_mbedtls_x509write_crt_set_basic_constraints(mbedtls_x509write_cert *ctx,
                                                int is_ca, int max_pathlen);

#if defined(MBEDTLS_MD_CAN_SHA1)
/**
 * \brief           Set the subjectKeyIdentifier extension for a CRT
 *                  Requires that esp_mbedtls_x509write_crt_set_subject_key() has been
 *                  called before
 *
 * \param ctx       CRT context to use
 *
 * \return          0 if successful, or a MBEDTLS_ERR_X509_ALLOC_FAILED
 */
int esp_mbedtls_x509write_crt_set_subject_key_identifier(mbedtls_x509write_cert *ctx);

/**
 * \brief           Set the authorityKeyIdentifier extension for a CRT
 *                  Requires that esp_mbedtls_x509write_crt_set_issuer_key() has been
 *                  called before
 *
 * \param ctx       CRT context to use
 *
 * \return          0 if successful, or a MBEDTLS_ERR_X509_ALLOC_FAILED
 */
int esp_mbedtls_x509write_crt_set_authority_key_identifier(mbedtls_x509write_cert *ctx);
 ...
 * \return          0 if successful, or MBEDTLS_ERR_X509_ALLOC_FAILED
 */
int esp_mbedtls_x509write_crt_set_key_usage(mbedtls_x509write_cert *ctx,
 ...
 * \return          0 if successful, or MBEDTLS_ERR_X509_ALLOC_FAILED
 */
int esp_mbedtls_x509write_crt_set_ext_key_usage(mbedtls_x509write_cert *ctx,
 ...
 * \return          0 if successful, or MBEDTLS_ERR_X509_ALLOC_FAILED
 */
int esp_mbedtls_x509write_crt_set_ns_cert_type(mbedtls_x509write_cert *ctx,
 ...
 */
void esp_mbedtls_x509write_crt_free(mbedtls_x509write_cert *ctx);
 ...
 */
int esp_mbedtls_x509write_crt_der(mbedtls_x509write_cert *ctx, unsigned char *buf, size_t size,
 ...
 * \note            \p f_rng is used for the signature operation.
 */
int esp_mbedtls_x509write_crt_pem(mbedtls_x509write_cert *ctx, unsigned char *buf, size_t size,
                              int (*f_rng)(void *, unsigned char *, size_t),
                              void *p_rng);
