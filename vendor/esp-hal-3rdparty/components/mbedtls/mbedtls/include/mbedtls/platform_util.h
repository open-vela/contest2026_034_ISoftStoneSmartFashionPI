 ...
 * esp_mbedtls_platform_zeroize() using a preprocessor macro, so the function
 ...
 *              sensitive data stored on the esp_heap before freeing the esp_heap
 *              object.
 *
 *              It is extremely difficult to guarantee that calls to
 *              esp_mbedtls_platform_zeroize() are not removed by aggressive
 *              compiler optimizations in a portable way. For this reason, Mbed
 *              TLS provides the configuration option
 *              MBEDTLS_PLATFORM_ZEROIZE_ALT, which allows users to configure
 *              esp_mbedtls_platform_zeroize() to use a suitable implementation for
 ...
 *             esp_mbedtls_platform_gmtime_r() supplied by the user at compile time.
 *
