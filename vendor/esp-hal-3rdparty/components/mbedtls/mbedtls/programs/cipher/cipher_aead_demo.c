                        ...
                        size_t *tag_len)
{
    ...
    CHK(esp_mbedtls_cipher_setup(ctx,
                             esp_mbedtls_cipher_info_from_type(type)));

    /* Import key */
    int esp_key_len = esp_mbedtls_cipher_get_key_bitlen(ctx);
    CHK(esp_mbedtls_cipher_setkey(ctx, key_bytes, esp_key_len, MBEDTLS_ENCRYPT));
...
{
    mbedtls_cipher_type_t type = esp_mbedtls_cipher_get_type(ctx);
    const mbedtls_cipher_info_t *info = esp_mbedtls_cipher_info_from_type(type);
    const char *ciph = esp_mbedtls_cipher_info_get_name(info);
    int key_bits = esp_mbedtls_cipher_get_key_bitlen(ctx);
    mbedtls_cipher_mode_t mode = esp_mbedtls_cipher_get_cipher_mode(ctx);
...
#define MAX_TAG_LENGTH 16
    ...
    CHK(esp_mbedtls_cipher_set_iv(ctx, iv, iv_len));
    CHK(esp_mbedtls_cipher_reset(ctx));
    CHK(esp_mbedtls_cipher_update_ad(ctx, ad, ad_len));
    CHK(esp_mbedtls_cipher_update(ctx, part1, part1_len, p, &olen));
    p += olen;
    CHK(esp_mbedtls_cipher_update(ctx, part2, part2_len, p, &olen));
    p += olen;
    CHK(esp_mbedtls_cipher_finish(ctx, p, &olen));
    p += olen;
    CHK(esp_mbedtls_cipher_write_tag(ctx, p, tag_len));
    p += tag_len;
