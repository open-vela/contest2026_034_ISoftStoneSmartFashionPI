 ...
 * be set before mbedtls_config.h, which pulls in glibc's esp_features.h indirectly.
...
static int do_handshake(mbedtls_ssl_context *ssl)
{
    ...
    while ((ret = esp_mbedtls_ssl_handshake(ssl)) != 0) {
        if (ret != MBEDTLS_ERR_SSL_WANT_READ && ret != MBEDTLS_ERR_SSL_WANT_WRITE) {
            ...
            esp_mbedtls_strerror(ret, (char *) buf, 1024);
#endif
            esp_mbedtls_printf(" failed\n  ! esp_mbedtls_ssl_handshake returned %d: %s\n\n", ret, buf);
            return -1;
        }
    }
...
#if !defined(MBEDTLS_X509_REMOVE_INFO)
        esp_mbedtls_x509_crt_verify_info(vrfy_buf, sizeof(vrfy_buf), "  ! ", flags);
    ...
    esp_mbedtls_x509_crt_info((char *) buf, sizeof(buf) - 1, "      ",
...
static int write_ssl_data(mbedtls_ssl_context *ssl, unsigned char *buf, size_t len)
{
    ...
    esp_mbedtls_printf("\n%s", buf);
    while (len && (ret = esp_mbedtls_ssl_write(ssl, buf, len)) <= 0) {
        if (ret != MBEDTLS_ERR_SSL_WANT_READ && ret != MBEDTLS_ERR_SSL_WANT_WRITE) {
...
static int write_ssl_and_get_response(mbedtls_ssl_context *ssl, unsigned char *buf, size_t len)
{
    ...
    do {
        ...
        ret = esp_mbedtls_ssl_read(ssl, data, len);
        ...
        if (ret <= 0) {
            esp_mbedtls_printf("failed\n  ! esp_mbedtls_ssl_read returned %d\n\n", ret);
            return -1;
        }
...
static int write_and_get_response(mbedtls_net_context *sock_fd, unsigned char *buf, size_t len)
{
    ...
    esp_mbedtls_printf("\n%s", buf);
    if (len && (ret = esp_mbedtls_net_send(sock_fd, buf, len)) <= 0) {
    ...
    do {
        len = sizeof(data) - 1;
        memset(data, 0, sizeof(data));
        ret = esp_mbedtls_net_recv(sock_fd, data, len);
...
usage:
        ...
        list = esp_mbedtls_ssl_list_ciphersuites();
        while (*list) {
    ...
    ret = esp_mbedtls_x509_crt_parse(&cacert, (const unsigned char *) mbedtls_test_cas_pem,
                                 mbedtls_test_cas_pem_len);
#else
    {
        esp_mbedtls_printf("MBEDTLS_PEM_PARSE_C not defined.");
        goto exit;
    }
#endif
    if (ret < 0) {
...
#endif
    ret = esp_mbedtls_x509_crt_parse(&clicert, (const unsigned char *) mbedtls_test_cli_crt,
    ...
    if (strlen(opt.key_file)) {
        ret = esp_mbedtls_pk_parse_keyfile(&pkey, opt.key_file, "",
     ...
     */
    esp_mbedtls_printf("  . Connecting to tcp/%s/%s...", opt.server_name,
                   opt.server_port);
    fflush(stdout);

    if ((ret = esp_mbedtls_net_connect(&server_fd, opt.server_name,
    ...
    esp_mbedtls_ssl_conf_authmode(&conf, MBEDTLS_SSL_VERIFY_OPTIONAL);
    ...
    esp_mbedtls_ssl_set_bio(&ssl, &server_fd, esp_mbedtls_net_send, esp_mbedtls_net_recv, NULL);
    ...
    if (opt.mode == MODE_SSL_TLS) {
        ...
        esp_mbedtls_printf("  > Get header from server:");
        fflush(stdout);
        ...
        esp_mbedtls_printf("  > Write EHLO to server:");
        fflush(stdout);
        ...
        esp_mbedtls_printf("  > Write STARTTLS to server:");
        fflush(stdout);
    ...
    if (opt.authentication) {
        esp_mbedtls_printf("  > Write AUTH LOGIN to server:");
        fflush(stdout);
        ...
        esp_mbedtls_printf("  > Write username to server: %s", opt.user_name);
        fflush(stdout);

        ret = esp_mbedtls_base64_encode(base, sizeof(base), &n, (const unsigned char *) opt.user_name,
                                    strlen(opt.user_name));

        if (ret != 0) {
            esp_mbedtls_printf(" failed\n  ! esp_mbedtls_base64_encode returned %d\n\n", ret);
            goto exit;
        }
        ...
        esp_mbedtls_printf("  > Write password to server: %s", opt.user_pwd);
        fflush(stdout);

        ret = esp_mbedtls_base64_encode(base, sizeof(base), &n, (const unsigned char *) opt.user_pwd,
                                    strlen(opt.user_pwd));
    ...
    esp_mbedtls_printf("  > Write MAIL FROM to server:");
    fflush(stdout);
    ...
    esp_mbedtls_printf("  > Write RCPT TO to server:");
    fflush(stdout);
    ...
    esp_mbedtls_printf("  > Write DATA to server:");
    fflush(stdout);
    ...
    esp_mbedtls_printf("  > Write content to server:");
    fflush(stdout);
    ...
    esp_mbedtls_ssl_close_notify(&ssl);
