...
int main(void)
{
    ...
    if ((ret = esp_mbedtls_net_bind(&listen_fd, BIND_IP, "4433", MBEDTLS_NET_PROTO_UDP)) != 0) {
...
reset:
    ...
    esp_mbedtls_ssl_set_bio(&ssl, &client_fd,
    ...
    if (ret <= 0) {
        switch (ret) {
            ...
            default:
                printf(" esp_mbedtls_ssl_read returned -0x%x\n\n", (unsigned int) -ret);
                goto reset;
        }
    }
