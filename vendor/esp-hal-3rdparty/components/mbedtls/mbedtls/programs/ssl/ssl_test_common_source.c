                       ...
                       mbedtls_tls_prf_types tls_prf_type)
{
    ...
    esp_mbedtls_printf("\n");
    esp_mbedtls_printf("---------------- NSS KEYLOG -----------------\n");
    esp_mbedtls_printf("%s", nss_keylog_line);
    esp_mbedtls_printf("---------------------------------------------\n");
...
exit:
    esp_mbedtls_platform_zeroize(nss_keylog_line,
                     ...
                     unsigned char const *buf, size_t len)
{
     ...
     * so make a copy. */
    tmp_buf = esp_mbedtls_calloc(1, len);
    if (tmp_buf == NULL) {
        return MBEDTLS_ERR_SSL_ALLOC_FAILED;
    }
    memcpy(tmp_buf, buf, len);

    ret_cr1 = esp_mbedtls_ssl_check_record(ssl, tmp_buf, len);
    if (ret_cr1 != MBEDTLS_ERR_SSL_FEATURE_UNAVAILABLE) {
        /* Test-only: Make sure that esp_mbedtls_ssl_check_record()
         *            doesn't alter state. */
        memcpy(tmp_buf, buf, len);   /* Restore buffer */
        ret_cr2 = esp_mbedtls_ssl_check_record(ssl, tmp_buf, len);
        if (ret_cr2 != ret_cr1) {
            esp_mbedtls_printf("esp_mbedtls_ssl_check_record() returned inconsistent results.\n");
            my_ret = -1;
            goto cleanup;
        }
        ...
        switch (ret_cr1) {
            ...
            case MBEDTLS_ERR_SSL_INVALID_RECORD:
                if (opt.debug_level > 1) {
                    esp_mbedtls_printf("esp_mbedtls_ssl_check_record() detected invalid record.\n");
                }
                break;

            case MBEDTLS_ERR_SSL_INVALID_MAC:
                if (opt.debug_level > 1) {
                    esp_mbedtls_printf("esp_mbedtls_ssl_check_record() detected unauthentic record.\n");
                }
                break;

            case MBEDTLS_ERR_SSL_UNEXPECTED_RECORD:
                if (opt.debug_level > 1) {
                    esp_mbedtls_printf("esp_mbedtls_ssl_check_record() detected unexpected record.\n");
                }
                break;

            default:
                esp_mbedtls_printf("esp_mbedtls_ssl_check_record() failed fatally with -%#04x.\n",
...
int recv_cb(void *ctx, unsigned char *buf, size_t len)
{
    ...
    if (opt.nbio == 2) {
        ...
        ret = esp_mbedtls_net_recv(io_ctx->net, buf, len);
    }
    if (ret < 0) {
        return ret;
    }
                    ...
                    uint32_t timeout)
{
    ...
    ret = esp_mbedtls_net_recv_timeout(io_ctx->net, buf, len, timeout);
    if (ret < 0) {
        return ret;
    }
    recv_len = (size_t) ret;
...
int send_cb(void *ctx, unsigned char const *buf, size_t len)
{
    ...
    return esp_mbedtls_net_send(io_ctx->net, buf, len);
}
...
#if defined(MBEDTLS_X509_CRT_PARSE_C)
/** Functionally equivalent to esp_mbedtls_x509_crt_verify_info, see that function
 *  for more info.
 */
                         ...
                         uint32_t flags)
{
    ...
    return esp_mbedtls_x509_crt_verify_info(buf, size, prefix, flags);
...
{
    esp_mbedtls_printf("supported signature algorithms:\n");
    esp_mbedtls_printf("\trsa_pkcs1_sha256 ");
    esp_mbedtls_printf("rsa_pkcs1_sha384 ");
    esp_mbedtls_printf("rsa_pkcs1_sha512\n");
    esp_mbedtls_printf("\tecdsa_secp256r1_sha256 ");
    esp_mbedtls_printf("ecdsa_secp384r1_sha384 ");
    esp_mbedtls_printf("ecdsa_secp521r1_sha512\n");
    esp_mbedtls_printf("\trsa_pss_rsae_sha256 ");
    esp_mbedtls_printf("rsa_pss_rsae_sha384 ");
    esp_mbedtls_printf("rsa_pss_rsae_sha512\n");
    esp_mbedtls_printf("\trsa_pss_pss_sha256 ");
    esp_mbedtls_printf("rsa_pss_pss_sha384 ");
    esp_mbedtls_printf("rsa_pss_pss_sha512\n");
    esp_mbedtls_printf("\ted25519 ");
    esp_mbedtls_printf("ed448 ");
    esp_mbedtls_printf("rsa_pkcs1_sha1 ");
    esp_mbedtls_printf("ecdsa_sha1\n");
    esp_mbedtls_printf("\n");
}
