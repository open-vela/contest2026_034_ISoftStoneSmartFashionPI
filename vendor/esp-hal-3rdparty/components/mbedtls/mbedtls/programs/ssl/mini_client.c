...
int main(void)
{
    esp_mbedtls_printf("MBEDTLS_CTR_DRBG_C and/or MBEDTLS_ENTROPY_C and/or "
...
int main(void)
{
    ...
    if (esp_mbedtls_ctr_drbg_seed(&ctr_drbg, esp_mbedtls_entropy_func, &entropy,
...
#if defined(MBEDTLS_X509_CRT_PARSE_C)
    if (esp_mbedtls_x509_crt_parse_der(&ca, ca_cert, sizeof(ca_cert)) != 0) {
        ret = x509_crt_parse_failed;
        goto exit;
    }

    esp_mbedtls_ssl_conf_ca_chain(&conf, &ca, NULL);
...
#if defined(MBEDTLS_X509_CRT_PARSE_C)
    if (esp_mbedtls_ssl_set_hostname(&ssl, HOSTNAME) != 0) {
        ret = hostname_failed;
        goto exit;
    }
#endif
    ...
    if (esp_mbedtls_ssl_handshake(&ssl) != 0) {
     ...
     */
    if (esp_mbedtls_ssl_write(&ssl, (const unsigned char *) GET_REQUEST,
