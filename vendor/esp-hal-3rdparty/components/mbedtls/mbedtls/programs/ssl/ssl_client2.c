...
int main(void)
{
    esp_mbedtls_printf("MBEDTLS_SSL_CLI_C not defined.\n");
    esp_mbedtls_exit(0);
}
                     ...
                     int depth, uint32_t *flags)
{
    ...
    esp_mbedtls_x509_crt_info(buf, sizeof(buf) - 1, "", crt);
    if (depth == 0) {
    ...
    if ((*flags) == 0) {
        esp_mbedtls_printf("  This certificate has no flags\n");
    } else {
                     ...
                     const char *additional_description)
{
    ...
    if (cid_negotiated == MBEDTLS_SSL_CID_DISABLED) {
        if (opt.cid_enabled == MBEDTLS_SSL_CID_ENABLED) {
            esp_mbedtls_printf("(%s) Use of Connection ID was rejected by the server.\n",
                                      ...
                                      size_t *session_data_len)
{
    ...
    if (*session_data != NULL) {
        esp_mbedtls_platform_zeroize(*session_data, *session_data_len);
        esp_mbedtls_free(*session_data);
        *session_data = NULL;
        *session_data_len = 0;
    }

    esp_mbedtls_ssl_session_init(&exported_session);
    ret = esp_mbedtls_ssl_get_session(ssl, &exported_session);
    if (ret != 0) {
        esp_mbedtls_printf(
            "failed\n  ! esp_mbedtls_ssl_get_session() returned -%#02x\n",
            (unsigned) -ret);
        goto exit;
    }

    /* get size of the buffer needed */
    (void) esp_mbedtls_ssl_session_save(&exported_session, NULL, 0, session_data_len);
    *session_data = esp_mbedtls_calloc(1, *session_data_len);
    if (*session_data == NULL) {
    ...
    if ((ret = esp_mbedtls_ssl_session_save(&exported_session,
...
exit:
    esp_mbedtls_ssl_session_free(&exported_session);
    return ret;
}
...
static int build_http_request(unsigned char *buf, size_t buf_size, size_t *request_len)
{
    ...
    ret = esp_mbedtls_snprintf((char *) buf, buf_size, GET_REQUEST, opt.request_page);
    if (ret < 0) {
        return ret;
    }
...
int main(int argc, char *argv[])
{
    ...
    esp_mbedtls_ssl_session_init(&saved_session);
    rng_init(&rng);
#if defined(MBEDTLS_SSL_HANDSHAKE_WITH_CERT_ENABLED)
    esp_mbedtls_x509_crt_init(&cacert);
    esp_mbedtls_x509_crt_init(&clicert);
...
usage:
    ...
    if (strlen(opt.psk)) {
        if (mbedtls_test_unhexify(psk, sizeof(psk),
                                  opt.psk, &psk_len) != 0) {
            esp_mbedtls_printf("pre-shared key not valid\n");
            goto exit;
        }
    }
#endif /* MBEDTLS_SSL_HANDSHAKE_WITH_PSK_ENABLED */
    ...
    if (mbedtls_test_unhexify(cid, sizeof(cid),
        ...
        esp_mbedtls_printf("CID not valid\n");
        goto exit;
    }
     ...
     */
    esp_mbedtls_printf("  . Loading the client cert. and key...");
    fflush(stdout);
    ...
    if (strlen(opt.crt_file)) {
        ret = esp_mbedtls_x509_crt_parse_file(&clicert, opt.crt_file);
    } else
#endif
    { ret = esp_mbedtls_x509_crt_parse(&clicert,
    ...
    if (strlen(opt.key_file)) {
        ret = esp_mbedtls_pk_parse_keyfile(&pkey, opt.key_file, opt.key_pwd, rng_get, &rng);
    } else
#endif
    { ret = esp_mbedtls_pk_parse_key(&pkey,
    ...
    esp_mbedtls_printf(" ok (key type: %s)\n",
                   strlen(opt.key_file) || strlen(opt.key_opaque_alg1) ?
                   esp_mbedtls_pk_get_name(&pkey) : "none");
#endif /* MBEDTLS_SSL_HANDSHAKE_WITH_CERT_ENABLED */
    ...
    if (opt.context_crt_cb == 0) {
        esp_mbedtls_ssl_conf_verify(&conf, my_verify, NULL);
    }
    ...
    if (opt.hs_to_min != DFL_HS_TO_MIN || opt.hs_to_max != DFL_HS_TO_MAX) {
        esp_mbedtls_ssl_conf_handshake_timeout(&conf, opt.hs_to_min,
                                           opt.hs_to_max);
    }
    ...
    if (opt.dhmlen != DFL_DHMLEN) {
        esp_mbedtls_ssl_conf_dhm_min_bitlen(&conf, opt.dhmlen);
    }
#endif
...
#if defined(MBEDTLS_SSL_SESSION_TICKETS)
    esp_mbedtls_ssl_conf_session_tickets(&conf, opt.tickets);
#endif
    ...
    if (strcmp(opt.crt_file, "none") != 0 &&
        ...
        if ((ret = esp_mbedtls_ssl_conf_own_cert(&conf, &clicert, &pkey)) != 0) {
    ...
    if (opt.psk_opaque != 0) {
        ...
        if ((ret = esp_mbedtls_ssl_conf_psk_opaque(&conf, slot,
    ...
    if (psk_len > 0) {
        ...
        if (ret != 0) {
            esp_mbedtls_printf(" failed\n  ! esp_mbedtls_ssl_conf_psk returned %d\n\n", ret);
            goto exit;
        }
    }
#endif /* MBEDTLS_SSL_HANDSHAKE_WITH_PSK_ENABLED */
...
#if defined(MBEDTLS_SSL_HANDSHAKE_WITH_CERT_ENABLED)
    if ((ret = esp_mbedtls_ssl_set_hostname(&ssl, opt.server_name)) != 0) {
        esp_mbedtls_printf(" failed\n  ! esp_mbedtls_ssl_set_hostname returned %d\n\n",
                       ret);
        goto exit;
    }
#endif
    ...
    if (opt.context_crt_cb == 1) {
        esp_mbedtls_ssl_set_verify(&ssl, my_verify, NULL);
    }
#endif /* MBEDTLS_SSL_HANDSHAKE_WITH_CERT_ENABLED */
    ...
    if (opt.ec_max_ops != DFL_EC_MAX_OPS) {
        esp_mbedtls_ecp_set_max_ops(opt.ec_max_ops);
    }
#endif
    ...
    if (opt.use_srtp != 0 && strlen(opt.mki) != 0) {
        if (mbedtls_test_unhexify(mki, sizeof(mki),
                                  opt.mki, &mki_len) != 0) {
            esp_mbedtls_printf("mki value not valid hex\n");
            goto exit;
        }

        esp_mbedtls_ssl_conf_srtp_mki_value_supported(&conf, MBEDTLS_SSL_DTLS_SRTP_MKI_SUPPORTED);
        if ((ret = esp_mbedtls_ssl_dtls_srtp_set_mki_value(&ssl, mki,
                                                       (uint16_t) strlen(opt.mki) / 2)) != 0) {
            esp_mbedtls_printf(" failed\n  ! esp_mbedtls_ssl_dtls_srtp_set_mki_value returned %d\n\n", ret);
            goto exit;
        }
    }
#endif
    ...
    esp_mbedtls_printf("  . Connecting to %s/%s/%s...",
                   opt.transport == MBEDTLS_SSL_TRANSPORT_STREAM ? "tcp" : "udp",
                   opt.server_addr, opt.server_port);
    fflush(stdout);

    if ((ret = esp_mbedtls_net_connect(&server_fd,
                                   opt.server_addr, opt.server_port,
                                   opt.transport == MBEDTLS_SSL_TRANSPORT_STREAM ?
                                   MBEDTLS_NET_PROTO_TCP : MBEDTLS_NET_PROTO_UDP)) != 0) {
        esp_mbedtls_printf(" failed\n  ! esp_mbedtls_net_connect returned -0x%x\n\n",
                       (unsigned int) -ret);
        goto exit;
    }

    if (opt.nbio > 0) {
        ret = esp_mbedtls_net_set_nonblock(&server_fd);
    } else {
        ret = esp_mbedtls_net_set_block(&server_fd);
    }
    if (ret != 0) {
    ...
    if (opt.reconnect != 0 && ssl.tls_version != MBEDTLS_SSL_VERSION_TLS1_3) {
        esp_mbedtls_printf("  . Saving session for reuse...");
        fflush(stdout);
        ...
        if (opt.reco_mode == 1) {
            ...
            if ((ret = esp_mbedtls_ssl_get_session(&ssl, &saved_session)) != 0) {
                esp_mbedtls_printf(" failed\n  ! esp_mbedtls_ssl_get_session returned -0x%x\n\n",
                               (unsigned int) -ret);
                goto exit;
            }
        }
        ...
        if (opt.reco_mode == 1) {
            esp_mbedtls_printf("    [ Saved %u bytes of session data]\n",
                           (unsigned) session_data_len);
        }
    }
    ...
    if (opt.renegotiate) {
         ...
         */
        esp_mbedtls_printf("  . Performing renegotiation...");
        fflush(stdout);
...
send_request:
    esp_mbedtls_printf("  > Write to server:");
    fflush(stdout);
    ...
    if (opt.transport == MBEDTLS_SSL_TRANSPORT_STREAM) {
        ...
        do {
            while ((ret = esp_mbedtls_ssl_write(&ssl, buf + written,
                                            len - written)) < 0) {
                if (ret != MBEDTLS_ERR_SSL_WANT_READ &&
                    ret != MBEDTLS_ERR_SSL_WANT_WRITE &&
                    ret != MBEDTLS_ERR_SSL_CRYPTO_IN_PROGRESS) {
                    esp_mbedtls_printf(" failed\n  ! esp_mbedtls_ssl_write returned -0x%x\n\n",
                                   (unsigned int) -ret);
                    goto exit;
                }
    ...
    if (opt.transport == MBEDTLS_SSL_TRANSPORT_STREAM) {
        ...
        do {
            ...
            if (ret <= 0) {
                switch (ret) {
                    ...
                    case MBEDTLS_ERR_SSL_RECEIVED_NEW_SESSION_TICKET:
                        ...
                        esp_mbedtls_printf(" got new session ticket ( %d ).\n",
                                       ticket_id++);
                        if (opt.reconnect != 0) {
                            esp_mbedtls_printf("  . Saving session for reuse...");
                            fflush(stdout);
            ...
            len = ret;
            buf[len] = '\0';
            esp_mbedtls_printf("  < Read from server: %" MBEDTLS_PRINTF_SIZET " bytes read\n\n%s",
                           len,
                           (char *) buf);
            fflush(stdout);
    ...
    if (opt.reconnect_hard != 0) {
        ...
        esp_mbedtls_printf("  . Restarting connection from same port...");
        fflush(stdout);
        ...
        if ((ret = esp_mbedtls_ssl_session_reset(&ssl)) != 0) {
            esp_mbedtls_printf(" failed\n  ! esp_mbedtls_ssl_session_reset returned -0x%x\n\n",
                           (unsigned int) -ret);
            goto exit;
        }
    ...
    if (opt.serialize != 0) {
        ...
        if (opt.serialize == 2) {
            ...
            if (opt.nbio == 2) {
                esp_mbedtls_ssl_set_bio(&ssl, &server_fd, delayed_send,
                                    delayed_recv, NULL);
            } else {
                esp_mbedtls_ssl_set_bio(&ssl, &server_fd, esp_mbedtls_net_send,
...
reconnect:
    if (opt.reconnect != 0) {
...
#if defined(MBEDTLS_TIMING_C)
        if (opt.reco_delay > 0) {
            esp_mbedtls_net_usleep(1000 * opt.reco_delay);
        }
#endif

        esp_mbedtls_printf("  . Reconnecting with saved session...");
        ...
        if (opt.reco_mode == 1) {
            if ((ret = esp_mbedtls_ssl_session_load(&saved_session,
                                                session_data,
                                                session_data_len)) != 0) {
                esp_mbedtls_printf(" failed\n  ! esp_mbedtls_ssl_session_load returned -0x%x\n\n",
                               (unsigned int) -ret);
                goto exit;
            }
        }

        if ((ret = esp_mbedtls_ssl_set_session(&ssl, &saved_session)) != 0) {
            esp_mbedtls_printf(" failed\n  ! esp_mbedtls_ssl_set_session returned -0x%x\n\n",
                           (unsigned int) -ret);
            goto exit;
        }
        ...
        if (opt.reco_server_name != NULL &&
            (ret = esp_mbedtls_ssl_set_hostname(&ssl,
        ...
        if (ssl.conf->early_data_enabled == MBEDTLS_SSL_EARLY_DATA_ENABLED) {
            ...
            written = 0;
            do {
                while ((ret = esp_mbedtls_ssl_write_early_data(&ssl, buf + written,
                                                           len - written)) < 0) {
...
exit:
    ...
    esp_mbedtls_ssl_session_free(&saved_session);
...
#if defined(MBEDTLS_SSL_HANDSHAKE_WITH_CERT_ENABLED)
    esp_mbedtls_x509_crt_free(&clicert);
