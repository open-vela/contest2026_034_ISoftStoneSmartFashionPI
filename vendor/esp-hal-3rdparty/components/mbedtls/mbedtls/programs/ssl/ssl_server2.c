...
int main(void)
{
    esp_mbedtls_printf(MBEDTLS_SSL_TEST_IMPOSSIBLE);
    esp_mbedtls_exit(0);
}
#elif !defined(MBEDTLS_SSL_SRV_C)
int main(void)
{
    esp_mbedtls_printf("MBEDTLS_SSL_SRV_C not defined.\n");
    esp_mbedtls_exit(0);
}
...
/* Size of memory to be allocated for the esp_heap, when using the library's memory
...
#define USAGE_GROUPS \
    ...
    "                        - see esp_mbedtls_ecp_curve_list()\n"                \
...
struct options {
    ...
    uint32_t read_timeout;      /* timeout on esp_mbedtls_ssl_read() in milliseconds    */
...
void sni_free(sni_entry *head)
{
    ...
    while (cur != NULL) {
        esp_mbedtls_x509_crt_free(cur->cert);
        esp_mbedtls_free(cur->cert);

        esp_mbedtls_pk_free(cur->key);
        esp_mbedtls_free(cur->key);

        esp_mbedtls_x509_crt_free(cur->ca);
        esp_mbedtls_free(cur->ca);
#if defined(MBEDTLS_X509_CRL_PARSE_C)
        esp_mbedtls_x509_crl_free(cur->crl);
        esp_mbedtls_free(cur->crl);
#endif
        next = cur->next;
...
sni_entry *sni_parse(char *sni_string)
{
    ...
    *end = ',';
    ...
    while (p <= end) {
        if ((new = esp_mbedtls_calloc(1, sizeof(sni_entry))) == NULL) {
            sni_free(cur);
            return NULL;
        }
        ...
        if ((new->cert = esp_mbedtls_calloc(1, sizeof(mbedtls_x509_crt))) == NULL ||
            (new->key = esp_mbedtls_calloc(1, sizeof(mbedtls_pk_context))) == NULL) {
            goto error;
        }

        esp_mbedtls_x509_crt_init(new->cert);
        esp_mbedtls_pk_init(new->key);

        if (esp_mbedtls_x509_crt_parse_file(new->cert, crt_file) != 0 ||
            esp_mbedtls_pk_parse_keyfile(new->key, key_file, "", rng_get, &rng) != 0) {
            goto error;
        }

        if (strcmp(ca_file, "-") != 0) {
            if ((new->ca = esp_mbedtls_calloc(1, sizeof(mbedtls_x509_crt))) == NULL) {
                goto error;
            }

            esp_mbedtls_x509_crt_init(new->ca);

            if (esp_mbedtls_x509_crt_parse_file(new->ca, ca_file) != 0) {
                goto error;
            }
        }
        ...
        if (strcmp(crl_file, "-") != 0) {
            if ((new->crl = esp_mbedtls_calloc(1, sizeof(mbedtls_x509_crl))) == NULL) {
                goto error;
            }

            esp_mbedtls_x509_crl_init(new->crl);

            if (esp_mbedtls_x509_crl_parse_file(new->crl, crl_file) != 0) {
                goto error;
            }
        }
#endif
                 ...
                 const unsigned char *name, size_t name_len)
{
    ...
    while (cur != NULL) {
        if (name_len == strlen(cur->name) &&
            ...
            void *p;
            *(const void **)&p = cur;
            esp_mbedtls_ssl_set_user_data_p(ssl, p);
            return 0;
        }
...
{
    const sni_entry *cur = (sni_entry *) esp_mbedtls_ssl_get_user_data_p(ssl);
    if (cur != NULL) {
        /*(exercise esp_mbedtls_ssl_get_hs_sni(); not otherwise used here)*/
        size_t name_len;
        const unsigned char *name = esp_mbedtls_ssl_get_hs_sni(ssl, &name_len);
        ...
        if (cur->ca != NULL) {
            esp_mbedtls_ssl_set_hs_ca_chain(ssl, cur->ca, cur->crl);
        }

        if (cur->authmode != DFL_AUTH_MODE) {
            esp_mbedtls_ssl_set_hs_authmode(ssl, cur->authmode);
        }

        return esp_mbedtls_ssl_set_hs_own_cert(ssl, cur->cert, cur->key);
    }
...
int psk_free(psk_entry *head)
{
    ...
    while (head != NULL) {
        ...
        if (MBEDTLS_SVC_KEY_ID_GET_KEY_ID(slot) != 0) {
            status = esp_psa_destroy_key(slot);
...
psk_entry *psk_parse(char *psk_string)
{
    ...
    *end = ',';
    ...
    while (p <= end) {
        if ((new = esp_mbedtls_calloc(1, sizeof(psk_entry))) == NULL) {
            goto error;
        }
    ...
    while (cur != NULL) {
        if (name_len == strlen(cur->name) &&
            ...
            if (MBEDTLS_SVC_KEY_ID_GET_KEY_ID(cur->slot) != 0) {
                return esp_mbedtls_ssl_set_hs_psk_opaque(ssl, cur->slot);
            } else
#endif
            return esp_mbedtls_ssl_set_hs_psk(ssl, cur->key, cur->esp_key_len);
        }
...
void term_handler(int sig)
{
    ((void) sig);
    received_sigterm = 1;
    esp_mbedtls_net_free(&listen_fd);   /* causes esp_mbedtls_net_accept() to abort */
    esp_mbedtls_net_free(&client_fd);   /* causes net_read() to abort */
}
#endif
                           ...
                           size_t input_len)
{
    ssl_async_key_context_t *config_data =
        esp_mbedtls_ssl_conf_get_async_config_data(ssl->conf);
    unsigned slot;
    ...
    {
        char dn[100];
        if (esp_mbedtls_x509_dn_gets(dn, sizeof(dn), &cert->subject) > 0) {
            esp_mbedtls_printf("Async %s callback: looking for DN=%s\n",
                           op_name, dn);
        }
    }
     ...
     * we call esp_mbedtls_pk_check_pair to match a private key with the
     * public key. */
    for (slot = 0; slot < config_data->slots_used; slot++) {
        if (esp_mbedtls_pk_check_pair(&cert->pk,
    ...
    if (slot == config_data->slots_used) {
        esp_mbedtls_printf("Async %s callback: no key matches this certificate.\n",
                       op_name);
        return MBEDTLS_ERR_SSL_HW_ACCEL_FALLTHROUGH;
    }
    esp_mbedtls_printf("Async %s callback: using key slot %u, delay=%u.\n",
                   op_name, slot, config_data->slots[slot].delay);

    if (config_data->inject_error == SSL_ASYNC_INJECT_ERROR_START) {
        esp_mbedtls_printf("Async %s callback: injected error\n", op_name);
    ...
    ctx = esp_mbedtls_calloc(1, sizeof(*ctx));
    if (ctx == NULL) {
    ...
    esp_mbedtls_ssl_set_async_operation_data(ssl, ctx);
...
static int ssl_async_resume(mbedtls_ssl_context *ssl,
                            ...
                            size_t output_size)
{
    ssl_async_operation_context_t *ctx = esp_mbedtls_ssl_get_async_operation_data(ssl);
    ssl_async_key_context_t *config_data =
        esp_mbedtls_ssl_conf_get_async_config_data(ssl->conf);
    ...
    if (ctx->remaining_delay > 0) {
        --ctx->remaining_delay;
        esp_mbedtls_printf("Async resume (slot %u): call %u more times.\n",
    ...
    switch (ctx->operation_type) {
        case ASYNC_OP_DECRYPT:
            ret = esp_mbedtls_pk_decrypt(key_slot->pk,
                                     ctx->input, ctx->input_len,
                                     output, output_len, output_size,
                                     config_data->f_rng, config_data->p_rng);
            break;
        case ASYNC_OP_SIGN:
            ret = esp_mbedtls_pk_sign(key_slot->pk,
                                  ctx->md_alg,
                                  ctx->input, ctx->input_len,
    ...
    if (config_data->inject_error == SSL_ASYNC_INJECT_ERROR_RESUME) {
        esp_mbedtls_printf("Async resume callback: %s done but injected error\n",
                       op_name);
        esp_mbedtls_free(ctx);
        return MBEDTLS_ERR_PK_FEATURE_UNAVAILABLE;
    }

    esp_mbedtls_printf("Async resume (slot %u): %s done, status=%d.\n",
...
{
    ssl_async_operation_context_t *ctx = esp_mbedtls_ssl_get_async_operation_data(ssl);
    esp_mbedtls_printf("Async cancel callback.\n");
    esp_mbedtls_free(ctx);
}
#endif /* MBEDTLS_SSL_HANDSHAKE_WITH_CERT_ENABLED */
#endif /* MBEDTLS_SSL_ASYNC_PRIVATE */
                                           ...
                                           size_t psk_len)
{
    ...
    status = esp_psa_import_key(&key_attributes, psk, psk_len, slot);
    if (status != PSA_SUCCESS) {
                     ...
                     const char *additional_description)
{
    ...
    ret = esp_mbedtls_ssl_get_peer_cid(ssl, &cid_negotiated,
                                   peer_cid, &peer_cid_len);
    if (ret != 0) {
        esp_mbedtls_printf(" failed\n  ! esp_mbedtls_ssl_get_peer_cid returned -0x%x\n\n",
                       (unsigned int) -ret);
        return ret;
    }
    ...
    if (cid_negotiated == MBEDTLS_SSL_CID_DISABLED) {
        if (opt.cid_enabled == MBEDTLS_SSL_CID_ENABLED) {
            esp_mbedtls_printf("(%s) Use of Connection ID was not offered by client.\n",
                           additional_description);
        }
    } else {
        size_t idx = 0;
        esp_mbedtls_printf("(%s) Use of Connection ID has been negotiated.\n",
                       additional_description);
        esp_mbedtls_printf("(%s) Peer CID (length %u Bytes): ",
                       additional_description,
                       (unsigned) peer_cid_len);
        while (idx < peer_cid_len) {
            esp_mbedtls_printf("%02x ", peer_cid[idx]);
            idx++;
        }
        esp_mbedtls_printf("\n");
    }
                       ...
                       size_t *tlen, uint32_t *ticket_lifetime)
{
    ...
    *ticket_lifetime = 7 * 24 * 3600;
    ...
    /* Dump session state */
    if ((ret = esp_mbedtls_ssl_session_save(session, p, end - p,
                                        &clear_len)) != 0) {
        return ret;
    }
                       ...
                       unsigned char *buf, size_t len)
{
    ...
    if ((ret = esp_mbedtls_ssl_session_load(session, buf + 4, len - 4)) != 0) {
        return ret;
    }
    ...
    switch (opt.dummy_ticket % 11) {
        ...
        case 3:
            ...
            session->ticket_creation_time = esp_mbedtls_ms_time() + 1000;
            break;
        case 4:
            /* Ticket has reached the end of lifetime. */
            session->ticket_creation_time = esp_mbedtls_ms_time() -
                                            (7 * 24 * 3600 * 1000 + 1000);
            break;
#if defined(MBEDTLS_SSL_PROTO_TLS1_3)
        case 5:
...
int main(int argc, char *argv[])
{
    ...
    mbedtls_x509_crt_profile crt_profile_for_test = esp_mbedtls_x509_crt_profile_default;
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    ...
    esp_mbedtls_x509_crt_init(&srvcert2);
    esp_mbedtls_pk_init(&pkey2);
#endif
#if defined(MBEDTLS_SSL_ASYNC_PRIVATE)
...
#endif
#if defined(MBEDTLS_SSL_SESSION_TICKETS) && defined(MBEDTLS_SSL_TICKET_C)
    esp_mbedtls_ssl_ticket_init(&ticket_ctx);
#endif
#if defined(MBEDTLS_SSL_ALPN)
    memset((void *) alpn_list, 0, sizeof(alpn_list));
#endif
#if defined(MBEDTLS_SSL_COOKIE_C)
    esp_mbedtls_ssl_cookie_init(&cookie_ctx);
#endif

#if defined(MBEDTLS_USE_PSA_CRYPTO) || defined(MBEDTLS_SSL_PROTO_TLS1_3)
    status = esp_psa_crypto_init();
    if (status != PSA_SUCCESS) {
...
usage:
        ...
        esp_mbedtls_printf("usage: ssl_client2 [param=value] [...]\n");
        esp_mbedtls_printf("       ssl_client2 help[_theme]\n");
        esp_mbedtls_printf("'help' lists acceptable 'param' and 'value'\n");
        esp_mbedtls_printf("'help_ciphersuites' lists available ciphersuites\n");
        esp_mbedtls_printf("\n");
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (strcmp(p, "help") == 0) {
            esp_mbedtls_printf(USAGE1);
            esp_mbedtls_printf(USAGE2);
            esp_mbedtls_printf(USAGE3);
            esp_mbedtls_printf(USAGE4);

            ret = 0;
            goto exit;
        }
        if (strcmp(p, "help_ciphersuites") == 0) {
            esp_mbedtls_printf(" acceptable ciphersuite names:\n");
            for (list = esp_mbedtls_ssl_list_ciphersuites();
                 *list != 0;
                 list++) {
                esp_mbedtls_printf(" %s\n", esp_mbedtls_ssl_get_ciphersuite_name(*list));
            }
        ...
        if ((q = strchr(p, '=')) == NULL) {
            esp_mbedtls_printf("param requires a value: '%s'\n", p);
        ...
        *q++ = '\0';
        ...
        if (strcmp(p, "server_port") == 0) {
            ...
            if (strcmp(q, "1") == 0) {
                esp_mbedtls_printf("build version: %s (build %d)\n",
            ...
            opt.force_ciphersuite[0] = esp_mbedtls_ssl_get_ciphersuite_id(q);
    ...
    if (opt.nss_keylog != 0 && opt.eap_tls != 0) {
        esp_mbedtls_printf("Error: eap_tls and nss_keylog options cannot be used together.\n");
        goto usage;
    }
    ...
    if (opt.event == 1 && opt.nbio != 1) {
        esp_mbedtls_printf("Warning: event-driven IO mandates nbio=1 - overwrite\n");
        opt.nbio = 1;
    }

#if defined(MBEDTLS_DEBUG_C)
    esp_mbedtls_debug_set_threshold(opt.debug_level);
#endif
    ...
    buf = esp_mbedtls_calloc(1, buf_content_size + 1);
    if (buf == NULL) {
        esp_mbedtls_printf("Could not allocate %lu bytes\n",
    ...
    if (opt.psk_opaque != 0) {
        if (strlen(opt.psk) == 0) {
            esp_mbedtls_printf("psk_opaque set but no psk to be imported specified.\n");
            ret = 2;
            goto usage;
        }
    ...
    if (opt.psk_list_opaque != 0) {
        if (opt.psk_list == NULL) {
            esp_mbedtls_printf("psk_slot set but no psk to be imported specified.\n");
            ret = 2;
            goto usage;
        }
    ...
    if (opt.force_ciphersuite[0] > 0) {
        ...
        ciphersuite_info =
            esp_mbedtls_ssl_ciphersuite_from_id(opt.force_ciphersuite[0]);

        if (opt.max_version != -1 &&
            ciphersuite_info->min_tls_version > opt.max_version) {
            esp_mbedtls_printf("forced ciphersuite not allowed with this protocol version\n");
            ret = 2;
            goto usage;
        }
    ...
    if (mbedtls_test_unhexify(cid, sizeof(cid),
        ...
        esp_mbedtls_printf("CID not valid hex\n");
        goto exit;
    }
    ...
    if (mbedtls_test_unhexify(psk, sizeof(psk),
                              opt.psk, &psk_len) != 0) {
        esp_mbedtls_printf("pre-shared key not valid hex\n");
        goto exit;
    }

    if (opt.psk_list != NULL) {
        if ((psk_info = psk_parse(opt.psk_list)) == NULL) {
            esp_mbedtls_printf("psk_list invalid");
            goto exit;
        }
    }
#endif /* MBEDTLS_SSL_HANDSHAKE_WITH_PSK_ENABLED */
    ...
    if (opt.sig_algs != NULL) {
        ...
        while (i < SIG_ALG_LIST_SIZE - 1 && *p != '\0') {
            ...
            if (strcmp(q, "rsa_pkcs1_sha256") == 0) {
                ...
                ret = -1;
                esp_mbedtls_printf("unknown signature algorithm \"%s\"\n", q);
        ...
        if (i == (SIG_ALG_LIST_SIZE - 1) && *p != '\0') {
            esp_mbedtls_printf("signature algorithm list too long, maximum %d",
                           SIG_ALG_LIST_SIZE - 1);
            goto exit;
        }
     ...
     */
    esp_mbedtls_printf("  . Loading the CA root certificate ...");
    fflush(stdout);
    ...
    if (strlen(opt.ca_path)) {
        ret = esp_mbedtls_x509_crt_parse_path(&cacert, opt.ca_path);
    } else if (strlen(opt.ca_file)) {
        ret = esp_mbedtls_x509_crt_parse_file(&cacert, opt.ca_file);
    } else
#endif
    {
#if defined(MBEDTLS_PEM_PARSE_C)
        for (i = 0; mbedtls_test_cas[i] != NULL; i++) {
            ret = esp_mbedtls_x509_crt_parse(&cacert,
        ...
        if (ret == 0) {
            for (i = 0; mbedtls_test_cas_der[i] != NULL; i++) {
                ret = esp_mbedtls_x509_crt_parse_der(&cacert,
    ...
    if (ret < 0) {
        esp_mbedtls_printf(" failed\n  !  esp_mbedtls_x509_crt_parse returned -0x%x\n\n",
                       (unsigned int) -ret);
        goto exit;
    }

    esp_mbedtls_printf(" ok (%d skipped)\n", ret);

    /*
     * 1.2. Load own certificate and private key
     */
    esp_mbedtls_printf("  . Loading the server cert. and key...");
    fflush(stdout);

#if defined(MBEDTLS_FS_IO)
    if (strlen(opt.crt_file) && strcmp(opt.crt_file, "none") != 0) {
        key_cert_init++;
        if ((ret = esp_mbedtls_x509_crt_parse_file(&srvcert, opt.crt_file)) != 0) {
            esp_mbedtls_printf(" failed\n  !  esp_mbedtls_x509_crt_parse_file returned -0x%x\n\n",
                           (unsigned int) -ret);
            goto exit;
        }
    }
    if (strlen(opt.key_file) && strcmp(opt.key_file, "none") != 0) {
        key_cert_init++;
        if ((ret = esp_mbedtls_pk_parse_keyfile(&pkey, opt.key_file,
                                            opt.key_pwd, rng_get, &rng)) != 0) {
            esp_mbedtls_printf(" failed\n  !  esp_mbedtls_pk_parse_keyfile returned -0x%x\n\n",
                           (unsigned int) -ret);
            goto exit;
        }
    }
    ...
    if (strlen(opt.crt_file2) && strcmp(opt.crt_file2, "none") != 0) {
        key_cert_init2++;
        if ((ret = esp_mbedtls_x509_crt_parse_file(&srvcert2, opt.crt_file2)) != 0) {
            esp_mbedtls_printf(" failed\n  !  esp_mbedtls_x509_crt_parse_file(2) returned -0x%x\n\n",
                           (unsigned int) -ret);
            goto exit;
        }
    }
    if (strlen(opt.key_file2) && strcmp(opt.key_file2, "none") != 0) {
        key_cert_init2++;
        if ((ret = esp_mbedtls_pk_parse_keyfile(&pkey2, opt.key_file2,
                                            opt.key_pwd2, rng_get, &rng)) != 0) {
            esp_mbedtls_printf(" failed\n  !  esp_mbedtls_pk_parse_keyfile(2) returned -0x%x\n\n",
                           (unsigned int) -ret);
            goto exit;
        }
    }
    ...
    if (key_cert_init == 0 &&
        ...
        if ((ret = esp_mbedtls_x509_crt_parse(&srvcert,
        ...
        if ((ret = esp_mbedtls_pk_parse_key(&pkey,
                                        (const unsigned char *) mbedtls_test_srv_key_rsa,
                                        mbedtls_test_srv_key_rsa_len, NULL, 0,
                                        rng_get, &rng)) != 0) {
            esp_mbedtls_printf(" failed\n  !  esp_mbedtls_pk_parse_key returned -0x%x\n\n",
                           (unsigned int) -ret);
            goto exit;
        }
        key_cert_init = 2;
        ...
        if ((ret = esp_mbedtls_x509_crt_parse(&srvcert2,
        ...
        if ((ret = esp_mbedtls_pk_parse_key(&pkey2,
    ...
    if (opt.key_opaque != 0) {
        ...
        if (key_opaque_set_alg_usage(opt.key1_opaque_alg1,
                                     ...
                                     &psa_alg, &psa_alg2,
                                     &psa_usage,
                                     esp_mbedtls_pk_get_type(&pkey)) == 0) {
        ...
        if (key_opaque_set_alg_usage(opt.key2_opaque_alg1,
                                     ...
                                     &psa_alg, &psa_alg2,
                                     &psa_usage,
                                     esp_mbedtls_pk_get_type(&pkey2)) == 0) {
            ret = pk_wrap_as_opaque(&pkey2, psa_alg, psa_alg2, psa_usage, &key_slot2);
            if (ret != 0) {
                esp_mbedtls_printf(" failed\n  !  "
                               "esp_mbedtls_pk_get_psa_attributes returned -0x%x\n\n",
                               (unsigned int)  -ret);
                goto exit;
            }
        }
    ...
    esp_mbedtls_printf(" ok (key types: %s, %s)\n",
                   key_cert_init ? esp_mbedtls_pk_get_name(&pkey) : "none",
                   key_cert_init2 ? esp_mbedtls_pk_get_name(&pkey2) : "none");
#endif /* MBEDTLS_SSL_HANDSHAKE_WITH_CERT_ENABLED */

#if defined(MBEDTLS_DHM_C) && defined(MBEDTLS_FS_IO)
    if (opt.dhm_file != NULL) {
        esp_mbedtls_printf("  . Loading DHM parameters...");
        fflush(stdout);

        if ((ret = esp_mbedtls_dhm_parse_dhmfile(&dhm, opt.dhm_file)) != 0) {
            esp_mbedtls_printf(" failed\n  ! esp_mbedtls_dhm_parse_dhmfile returned -0x%04X\n\n",
                           (unsigned int) -ret);
            goto exit;
        }
...
#if defined(SNI_OPTION)
    if (opt.sni != NULL) {
        esp_mbedtls_printf("  . Setting up SNI information...");
        fflush(stdout);
     ...
     * 2. Setup stuff
     */
    esp_mbedtls_printf("  . Setting up the SSL/TLS structure...");
    fflush(stdout);
    ...
    if ((ret = esp_mbedtls_ssl_config_defaults(&conf,
        ...
        esp_mbedtls_printf(" failed\n  ! esp_mbedtls_ssl_config_defaults returned -0x%x\n\n",
                       (unsigned int) -ret);
        goto exit;
    }
    ...
    if (opt.allow_sha1 > 0) {
        ...
        esp_mbedtls_ssl_conf_cert_profile(&conf, &crt_profile_for_test);
        esp_mbedtls_ssl_conf_sig_algs(&conf, ssl_sig_algs_for_test);
    }
#endif /* MBEDTLS_SSL_HANDSHAKE_WITH_CERT_ENABLED */

    if (opt.auth_mode != DFL_AUTH_MODE) {
        esp_mbedtls_ssl_conf_authmode(&conf, opt.auth_mode);
    }

    if (opt.cert_req_ca_list != DFL_CERT_REQ_CA_LIST) {
        esp_mbedtls_ssl_conf_cert_req_ca_list(&conf, opt.cert_req_ca_list);
    }

#if defined(MBEDTLS_SSL_EARLY_DATA)
    if (opt.early_data != DFL_EARLY_DATA) {
        esp_mbedtls_ssl_conf_early_data(&conf, opt.early_data);
    }
    if (opt.max_early_data_size != DFL_MAX_EARLY_DATA_SIZE) {
        esp_mbedtls_ssl_conf_max_early_data_size(
    ...
    if (opt.cert_req_dn_hint == 2 && key_cert_init2) {
        esp_mbedtls_ssl_conf_dn_hints(&conf, &srvcert2);
    }
#endif

#if defined(MBEDTLS_SSL_PROTO_DTLS)
    if (opt.hs_to_min != DFL_HS_TO_MIN || opt.hs_to_max != DFL_HS_TO_MAX) {
        esp_mbedtls_ssl_conf_handshake_timeout(&conf, opt.hs_to_min, opt.hs_to_max);
    }

    if (opt.dgram_packing != DFL_DGRAM_PACKING) {
        esp_mbedtls_ssl_set_datagram_packing(&ssl, opt.dgram_packing);
    }
#endif /* MBEDTLS_SSL_PROTO_DTLS */

#if defined(MBEDTLS_SSL_MAX_FRAGMENT_LENGTH)
    if ((ret = esp_mbedtls_ssl_conf_max_frag_len(&conf, opt.mfl_code)) != 0) {
        esp_mbedtls_printf(" failed\n  ! esp_mbedtls_ssl_conf_max_frag_len returned %d\n\n", ret);
        goto exit;
    }
#endif
    ...
    if (opt.cid_enabled == 1 || opt.cid_enabled_renego == 1) {
        if (opt.cid_enabled == 1        &&
            ...
            esp_mbedtls_printf("CID length must not change during renegotiation\n");
            goto usage;
        }

        if (opt.cid_enabled == 1) {
            ret = esp_mbedtls_ssl_conf_cid(&conf, cid_len,
                                       MBEDTLS_SSL_UNEXPECTED_CID_IGNORE);
        } else {
            ret = esp_mbedtls_ssl_conf_cid(&conf, cid_renego_len,
                                       MBEDTLS_SSL_UNEXPECTED_CID_IGNORE);
        }
    ...
    if (opt.use_srtp == 1) {
        if (opt.force_srtp_profile != 0) {
            ret = esp_mbedtls_ssl_conf_dtls_srtp_protection_profiles(&conf, forced_profile);
        } else {
            ret = esp_mbedtls_ssl_conf_dtls_srtp_protection_profiles(&conf, default_profiles);
        }

        if (ret != 0) {
            esp_mbedtls_printf(
                " failed\n  ! esp_mbedtls_ssl_conf_dtls_srtp_protection_profiles returned %d\n\n",
                ret);
            goto exit;
        }

        esp_mbedtls_ssl_conf_srtp_mki_value_supported(&conf,
                                                  opt.support_mki ?
    ...
    if (opt.extended_ms != DFL_EXTENDED_MS) {
        esp_mbedtls_ssl_conf_extended_master_secret(&conf, opt.extended_ms);
    }
#endif

#if defined(MBEDTLS_SSL_ENCRYPT_THEN_MAC)
    if (opt.etm != DFL_ETM) {
        esp_mbedtls_ssl_conf_encrypt_then_mac(&conf, opt.etm);
    }
#endif

#if defined(MBEDTLS_SSL_ALPN)
    if (opt.alpn_string != NULL) {
        if ((ret = esp_mbedtls_ssl_conf_alpn_protocols(&conf, alpn_list)) != 0) {
            esp_mbedtls_printf(" failed\n  ! esp_mbedtls_ssl_conf_alpn_protocols returned %d\n\n", ret);
            goto exit;
        }
    }
#endif
    ...
    if (opt.reproducible) {
        ...
        esp_mbedtls_platform_set_time(dummy_constant_time);
#else
        fprintf(stderr, "Warning: reproducible option used without constant time\n");
#endif
#endif  /* MBEDTLS_HAVE_TIME */
    }
    esp_mbedtls_ssl_conf_rng(&conf, rng_get, &rng);
    esp_mbedtls_ssl_conf_dbg(&conf, my_debug, stdout);

#if defined(MBEDTLS_SSL_CACHE_C)
    if (opt.cache_max != -1) {
        esp_mbedtls_ssl_cache_set_max_entries(&cache, opt.cache_max);
    }

#if defined(MBEDTLS_HAVE_TIME)
    if (opt.cache_timeout != -1) {
        esp_mbedtls_ssl_cache_set_timeout(&cache, opt.cache_timeout);
    }
#endif
    ...
    if (opt.tickets != MBEDTLS_SSL_SESSION_TICKETS_DISABLED) {
#if defined(MBEDTLS_HAVE_TIME)
        if (opt.dummy_ticket) {
            esp_mbedtls_ssl_conf_session_tickets_cb(&conf,
                                                dummy_ticket_write,
                                                dummy_ticket_parse,
                                                NULL);
        } else
#endif /* MBEDTLS_HAVE_TIME */
        {
            if ((ret = esp_mbedtls_ssl_ticket_setup(&ticket_ctx,
                                                rng_get, &rng,
                                                opt.ticket_aead,
                                                opt.ticket_timeout)) != 0) {
                esp_mbedtls_printf(
                    " failed\n  ! esp_mbedtls_ssl_ticket_setup returned %d\n\n",
                    ret);
                goto exit;
            }

            esp_mbedtls_ssl_conf_session_tickets_cb(&conf,
                                                esp_mbedtls_ssl_ticket_write,
                                                esp_mbedtls_ssl_ticket_parse,
                                                &ticket_ctx);
        }

#if defined(MBEDTLS_SSL_PROTO_TLS1_3)
        esp_mbedtls_ssl_conf_new_session_tickets(&conf, opt.tickets);
#endif
        ...
        if (opt.ticket_rotate) {
            ...
            if ((ret = rng_get(&rng, name, sizeof(name))) != 0 ||
                ...
                (ret = esp_mbedtls_ssl_ticket_rotate(&ticket_ctx,
                                                 name, sizeof(name), kbuf, sizeof(kbuf),
                                                 opt.ticket_timeout)) != 0) {
                esp_mbedtls_printf(" failed\n  ! esp_mbedtls_ssl_ticket_rotate returned %d\n\n", ret);
                goto exit;
            }
        }
    }
    ...
    if (opt.transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
#if defined(MBEDTLS_SSL_COOKIE_C)
        if (opt.cookies > 0) {
            if ((ret = esp_mbedtls_ssl_cookie_setup(&cookie_ctx,
                                                rng_get, &rng)) != 0) {
                esp_mbedtls_printf(" failed\n  ! esp_mbedtls_ssl_cookie_setup returned %d\n\n", ret);
                goto exit;
            }

            esp_mbedtls_ssl_conf_dtls_cookies(&conf, esp_mbedtls_ssl_cookie_write, esp_mbedtls_ssl_cookie_check,
                                          &cookie_ctx);
        } else
#endif /* MBEDTLS_SSL_COOKIE_C */
#if defined(MBEDTLS_SSL_DTLS_HELLO_VERIFY)
        if (opt.cookies == 0) {
            esp_mbedtls_ssl_conf_dtls_cookies(&conf, NULL, NULL, NULL);
        } else
        ...
        if (opt.anti_replay != DFL_ANTI_REPLAY) {
            esp_mbedtls_ssl_conf_dtls_anti_replay(&conf, opt.anti_replay);
        }
#endif

        if (opt.badmac_limit != DFL_BADMAC_LIMIT) {
            esp_mbedtls_ssl_conf_dtls_badmac_limit(&conf, opt.badmac_limit);
        }
    }
#endif /* MBEDTLS_SSL_PROTO_DTLS */

    if (opt.force_ciphersuite[0] != DFL_FORCE_CIPHER) {
        esp_mbedtls_ssl_conf_ciphersuites(&conf, opt.force_ciphersuite);
    }

#if defined(MBEDTLS_SSL_PROTO_TLS1_3)
    esp_mbedtls_ssl_conf_tls13_key_exchange_modes(&conf, opt.tls13_kex_modes);
#endif /* MBEDTLS_SSL_PROTO_TLS1_3 */

    if (opt.allow_legacy != DFL_ALLOW_LEGACY) {
        esp_mbedtls_ssl_conf_legacy_renegotiation(&conf, opt.allow_legacy);
    }
#if defined(MBEDTLS_SSL_RENEGOTIATION)
    esp_mbedtls_ssl_conf_renegotiation(&conf, opt.renegotiation);

    if (opt.renego_delay != DFL_RENEGO_DELAY) {
        esp_mbedtls_ssl_conf_renegotiation_enforced(&conf, opt.renego_delay);
    }

    if (opt.renego_period != DFL_RENEGO_PERIOD) {
        PUT_UINT64_BE(renego_period, opt.renego_period, 0);
        esp_mbedtls_ssl_conf_renegotiation_period(&conf, renego_period);
    }
#endif
    ...
    if (strcmp(opt.ca_path, "none") != 0 &&
        ...
        if (opt.ca_callback != 0) {
            esp_mbedtls_ssl_conf_ca_cb(&conf, ca_callback, &cacert);
        } else
#endif
    ...
    if (key_cert_init) {
        ...
        if (opt.async_private_delay1 >= 0) {
            ...
            if (ret < 0) {
                esp_mbedtls_printf("  Test error: ssl_async_set_key failed (%d)\n",
                               ret);
                goto exit;
            }
            pk = NULL;
...
#endif /* MBEDTLS_SSL_ASYNC_PRIVATE */
        if ((ret = esp_mbedtls_ssl_conf_own_cert(&conf, &srvcert, pk)) != 0) {
...
#endif /* MBEDTLS_SSL_ASYNC_PRIVATE */
        if ((ret = esp_mbedtls_ssl_conf_own_cert(&conf, &srvcert2, pk)) != 0) {
            esp_mbedtls_printf(" failed\n  ! esp_mbedtls_ssl_conf_own_cert returned %d\n\n", ret);
            goto exit;
        }
    }
    ...
    if (opt.async_operations[0] != '-') {
        ...
        esp_mbedtls_ssl_conf_async_private_cb(&conf,
                                          sign,
                                          decrypt,
                                          ssl_async_resume,
                                          ssl_async_cancel,
...
#if defined(SNI_OPTION)
    if (opt.sni != NULL) {
        esp_mbedtls_ssl_conf_sni(&conf, sni_callback, sni_info);
        esp_mbedtls_ssl_conf_cert_cb(&conf, cert_callback);
#if defined(MBEDTLS_SSL_ASYNC_PRIVATE)
        if (opt.async_private_delay2 >= 0) {
            sni_entry *cur;
    ...
    if (opt.groups != NULL &&
        ...
        esp_mbedtls_ssl_conf_groups(&conf, group_list);
    }
#endif

#if defined(MBEDTLS_SSL_HANDSHAKE_WITH_CERT_ENABLED)
    if (opt.sig_algs != NULL) {
        esp_mbedtls_ssl_conf_sig_algs(&conf, sig_alg_list);
    }
#endif
    ...
    if (strlen(opt.psk) != 0 && strlen(opt.psk_identity) != 0) {
        ...
        if (opt.psk_opaque != 0) {
            ...
            if ((ret = esp_mbedtls_ssl_conf_psk_opaque(&conf, psk_slot,
                                                   (const unsigned char *) opt.psk_identity,
                                                   strlen(opt.psk_identity))) != 0) {
                esp_mbedtls_printf(" failed\n  ! esp_mbedtls_ssl_conf_psk_opaque returned %d\n\n",
                               ret);
                goto exit;
            }
        } else
#endif /* MBEDTLS_USE_PSA_CRYPTO */
        if (psk_len > 0) {
            ret = esp_mbedtls_ssl_conf_psk(&conf, psk, psk_len,
                                       (const unsigned char *) opt.psk_identity,
                                       strlen(opt.psk_identity));
            if (ret != 0) {
                esp_mbedtls_printf("  failed\n  esp_mbedtls_ssl_conf_psk returned -0x%04X\n\n",
                               (unsigned int) -ret);
                goto exit;
            }
        }
    ...
    if (opt.psk_list != NULL) {
        ...
        esp_mbedtls_ssl_conf_psk_cb(&conf, psk_callback, psk_info);
    }
#endif
...
#if defined(MBEDTLS_FS_IO)
    if (opt.dhm_file != NULL) {
        ret = esp_mbedtls_ssl_conf_dh_param_ctx(&conf, &dhm);
    }
#endif
    if (ret != 0) {
    ...
    if (opt.min_version != DFL_MIN_VERSION) {
        esp_mbedtls_ssl_conf_min_tls_version(&conf, opt.min_version);
    }

    if (opt.max_version != DFL_MIN_VERSION) {
        esp_mbedtls_ssl_conf_max_tls_version(&conf, opt.max_version);
    }

    if ((ret = esp_mbedtls_ssl_setup(&ssl, &conf)) != 0) {
        esp_mbedtls_printf(" failed\n  ! esp_mbedtls_ssl_setup returned -0x%x\n\n", (unsigned int) -ret);
        goto exit;
    }

    if (opt.eap_tls != 0) {
        esp_mbedtls_ssl_set_export_keys_cb(&ssl, eap_tls_key_derivation,
                                       &eap_tls_keying);
    } else if (opt.nss_keylog != 0) {
        esp_mbedtls_ssl_set_export_keys_cb(&ssl,
                                       nss_keylog_export,
                                       NULL);
    }
#if defined(MBEDTLS_SSL_DTLS_SRTP)
    else if (opt.use_srtp != 0) {
        esp_mbedtls_ssl_set_export_keys_cb(&ssl, dtls_srtp_key_derivation,
                                       &dtls_srtp_keying);
    }
#endif /* MBEDTLS_SSL_DTLS_SRTP */
    ...
    esp_mbedtls_ssl_set_bio(&ssl, &io_ctx, send_cb, recv_cb,
                        opt.nbio == 0 ? recv_timeout_cb : NULL);

#if defined(MBEDTLS_SSL_DTLS_CONNECTION_ID)
    if (opt.transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
        if ((ret = esp_mbedtls_ssl_set_cid(&ssl, opt.cid_enabled,
                                       cid, cid_len)) != 0) {
            esp_mbedtls_printf(" failed\n  ! esp_mbedtls_ssl_set_cid returned %d\n\n",
                           ret);
            goto exit;
        }
    }
    ...
    if (opt.dtls_mtu != DFL_DTLS_MTU) {
        esp_mbedtls_ssl_set_mtu(&ssl, opt.dtls_mtu);
    }
#endif
     ...
     */
    esp_mbedtls_printf("  . Bind on %s://%s:%s/ ...",
                   opt.transport == MBEDTLS_SSL_TRANSPORT_STREAM ? "tcp" : "udp",
                   opt.server_addr ? opt.server_addr : "*",
                   opt.server_port);
    fflush(stdout);

    if ((ret = esp_mbedtls_net_bind(&listen_fd, opt.server_addr, opt.server_port,
                                opt.transport == MBEDTLS_SSL_TRANSPORT_STREAM ?
                                MBEDTLS_NET_PROTO_TCP : MBEDTLS_NET_PROTO_UDP)) != 0) {
        esp_mbedtls_printf(" failed\n  ! esp_mbedtls_net_bind returned -0x%x\n\n", (unsigned int) -ret);
        goto exit;
    }
    esp_mbedtls_printf(" ok\n");
...
reset:
#if !defined(_WIN32)
    if (received_sigterm) {
        esp_mbedtls_printf(" interrupted by SIGTERM (not in net_accept())\n");
    ...
    if (ret == MBEDTLS_ERR_SSL_CLIENT_RECONNECT) {
        esp_mbedtls_printf("  ! Client initiated reconnection from same port\n");
        goto handshake;
    }
    ...
    if ((ret = esp_mbedtls_net_accept(&listen_fd, &client_fd,
...
#if !defined(_WIN32)
        if (received_sigterm) {
            esp_mbedtls_printf(" interrupted by SIGTERM (in net_accept())\n");
        ...
        esp_mbedtls_printf(" failed\n  ! esp_mbedtls_net_accept returned -0x%x\n\n", (unsigned int) -ret);
        goto exit;
    }

    if (opt.nbio > 0) {
        ret = esp_mbedtls_net_set_nonblock(&client_fd);
    } else {
        ret = esp_mbedtls_net_set_block(&client_fd);
    }
    if (ret != 0) {
    ...
    esp_mbedtls_ssl_conf_read_timeout(&conf, opt.read_timeout);

#if defined(MBEDTLS_SSL_DTLS_HELLO_VERIFY)
    if (opt.transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
        if ((ret = esp_mbedtls_ssl_set_client_transport_id(&ssl,
                                                       client_ip, cliip_len)) != 0) {
            esp_mbedtls_printf(" failed\n  ! esp_mbedtls_ssl_set_client_transport_id() returned -0x%x\n\n",
                           (unsigned int) -ret);
            goto exit;
        }
    }
    ...
    if (opt.ecjpake_pw != DFL_ECJPAKE_PW) {
        ...
        if (opt.ecjpake_pw_opaque != DFL_ECJPAKE_PW_OPAQUE) {
            ...
            status = esp_psa_import_key(&attributes,
                                    ...
                                    &ecjpake_pw_slot);
            if (status != PSA_SUCCESS) {
                esp_mbedtls_printf(" failed\n  ! esp_psa_import_key returned %d\n\n",
                               status);
                goto exit;
            }
            if ((ret = esp_mbedtls_ssl_set_hs_ecjpake_password_opaque(&ssl,
                                                                  ecjpake_pw_slot)) != 0) {
                esp_mbedtls_printf(
                    " failed\n  ! esp_mbedtls_ssl_set_hs_ecjpake_password_opaque returned %d\n\n",
                    ret);
                goto exit;
            }
            esp_mbedtls_printf("using opaque password\n");
        } else
#endif  /* MBEDTLS_USE_PSA_CRYPTO */
        {
            if ((ret = esp_mbedtls_ssl_set_hs_ecjpake_password(&ssl,
                                                           (const unsigned char *) opt.ecjpake_pw,
                                                           strlen(opt.ecjpake_pw))) != 0) {
                esp_mbedtls_printf(" failed\n  ! esp_mbedtls_ssl_set_hs_ecjpake_password returned %d\n\n",
                               ret);
                goto exit;
            }
        }
     ...
     *  if being set per-handshake using esp_mbedtls_ssl_set_hs_dn_hints()) */
    if (opt.cert_req_dn_hint == 3 && key_cert_init2) {
        esp_mbedtls_ssl_set_hs_dn_hints(&ssl, &srvcert2);
    }
#endif
#endif
...
handshake:
    ...
    while ((ret = esp_mbedtls_ssl_handshake(&ssl)) != 0) {
        ...
        if (ret == MBEDTLS_ERR_SSL_RECEIVED_EARLY_DATA) {
            ...
            ret = esp_mbedtls_ssl_read_early_data(&ssl, buf, opt.buffer_size);
            if (ret > 0) {
                buf[ret] = '\0';
                esp_mbedtls_printf(" %d early data bytes read\n\n%s\n",
                               ret, (char *) buf);
            }
            continue;
        }
        ...
        if (ret == MBEDTLS_ERR_SSL_ASYNC_IN_PROGRESS &&
            ...
            esp_mbedtls_printf(" cancelling on injected error\n");
            break;
        }
#endif /* MBEDTLS_SSL_ASYNC_PRIVATE */
    ...
    if (ret == MBEDTLS_ERR_SSL_HELLO_VERIFY_REQUIRED) {
        esp_mbedtls_printf(" hello verification requested\n");
        ret = 0;
        goto reset;
    } else if (ret != 0) {
        esp_mbedtls_printf(" failed\n  ! esp_mbedtls_ssl_handshake returned -0x%x\n\n",
                       (unsigned int) -ret);

#if defined(MBEDTLS_SSL_HANDSHAKE_WITH_CERT_ENABLED)
        if (ret == MBEDTLS_ERR_X509_CERT_VERIFY_FAILED) {
            char vrfy_buf[512];
            flags = esp_mbedtls_ssl_get_verify_result(&ssl);
        ...
        int suite_id = esp_mbedtls_ssl_get_ciphersuite_id_from_ssl(&ssl);
        const mbedtls_ssl_ciphersuite_t *ciphersuite_info;
        ciphersuite_info = esp_mbedtls_ssl_ciphersuite_from_id(suite_id);

        esp_mbedtls_printf(" ok\n    [ Protocol is %s ]\n"
                       "    [ Ciphersuite is %s ]\n"
                       "    [ Key size is %u ]\n",
                       esp_mbedtls_ssl_get_version(&ssl),
                       esp_mbedtls_ssl_ciphersuite_get_name(ciphersuite_info),
                       (unsigned int)
                       esp_mbedtls_ssl_ciphersuite_get_cipher_key_bitlen(ciphersuite_info));
    }

    if ((ret = esp_mbedtls_ssl_get_record_expansion(&ssl)) >= 0) {
        esp_mbedtls_printf("    [ Record expansion is %d ]\n", ret);
    } else {
        esp_mbedtls_printf("    [ Record expansion is unknown ]\n");
    }

#if defined(MBEDTLS_SSL_MAX_FRAGMENT_LENGTH) || defined(MBEDTLS_SSL_RECORD_SIZE_LIMIT)
    esp_mbedtls_printf("    [ Maximum incoming record payload length is %u ]\n",
                   (unsigned int) esp_mbedtls_ssl_get_max_in_record_payload(&ssl));
    esp_mbedtls_printf("    [ Maximum outgoing record payload length is %u ]\n",
                   (unsigned int) esp_mbedtls_ssl_get_max_out_record_payload(&ssl));
#endif

#if defined(MBEDTLS_SSL_ALPN)
    if (opt.alpn_string != NULL) {
        const char *alp = esp_mbedtls_ssl_get_alpn_protocol(&ssl);
        esp_mbedtls_printf("    [ Application Layer Protocol is %s ]\n",
                       alp ? alp : "(none)");
    }
#endif
     ...
     */
    esp_mbedtls_printf("  . Verifying peer X.509 certificate...");

    if ((flags = esp_mbedtls_ssl_get_verify_result(&ssl)) != 0) {
        char vrfy_buf[512];
    ...
    if (esp_mbedtls_ssl_get_peer_cert(&ssl) != NULL) {
        char crt_buf[512];

        esp_mbedtls_printf("  . Peer certificate information    ...\n");
        esp_mbedtls_x509_crt_info(crt_buf, sizeof(crt_buf), "      ",
                              esp_mbedtls_ssl_get_peer_cert(&ssl));
        esp_mbedtls_printf("%s\n", crt_buf);
    }
    ...
    if (opt.eap_tls != 0) {
        ...
        if ((ret = esp_mbedtls_ssl_tls_prf(eap_tls_keying.tls_prf_type,
            ...
            != 0) {
            esp_mbedtls_printf(" failed\n  ! esp_mbedtls_ssl_tls_prf returned -0x%x\n\n",
                           (unsigned int) -ret);
            goto reset;
        }

        esp_mbedtls_printf("    EAP-TLS key material is:");
        for (j = 0; j < sizeof(eap_tls_keymaterial); j++) {
            if (j % 8 == 0) {
                esp_mbedtls_printf("\n    ");
            }
            esp_mbedtls_printf("%02x ", eap_tls_keymaterial[j]);
        }
        esp_mbedtls_printf("\n");

        if ((ret = esp_mbedtls_ssl_tls_prf(eap_tls_keying.tls_prf_type, NULL, 0,
                                       eap_tls_label,
                                       eap_tls_keying.randbytes,
                                       sizeof(eap_tls_keying.randbytes),
                                       eap_tls_iv,
        ...
        esp_mbedtls_printf("    EAP-TLS IV is:");
        for (j = 0; j < sizeof(eap_tls_iv); j++) {
            if (j % 8 == 0) {
                esp_mbedtls_printf("\n    ");
            }
            esp_mbedtls_printf("%02x ", eap_tls_iv[j]);
        }
        esp_mbedtls_printf("\n");
    }
        ...
        esp_mbedtls_ssl_get_dtls_srtp_negotiation_result(&ssl, &dtls_srtp_negotiation_result);

        if (dtls_srtp_negotiation_result.chosen_dtls_srtp_profile
            == MBEDTLS_TLS_SRTP_UNSET) {
            esp_mbedtls_printf("    Unable to negotiate "
                           "the use of DTLS-SRTP\n");
        } else {
            if ((ret = esp_mbedtls_ssl_tls_prf(dtls_srtp_keying.tls_prf_type,
            ...
            esp_mbedtls_printf("    DTLS-SRTP key material is:");
            for (j = 0; j < sizeof(dtls_srtp_key_material); j++) {
                if (j % 8 == 0) {
                    esp_mbedtls_printf("\n    ");
                }
                esp_mbedtls_printf("%02x ", dtls_srtp_key_material[j]);
            }
            esp_mbedtls_printf("\n");
             ...
             */
            esp_mbedtls_printf("    Keying material: ");
            for (j = 0; j < sizeof(dtls_srtp_key_material); j++) {
                esp_mbedtls_printf("%02X", dtls_srtp_key_material[j]);
            }
            esp_mbedtls_printf("\n");
            ...
            if (dtls_srtp_negotiation_result.mki_len > 0) {
                esp_mbedtls_printf("    DTLS-SRTP mki value: ");
                for (j = 0; j < dtls_srtp_negotiation_result.mki_len; j++) {
                    esp_mbedtls_printf("%02X", dtls_srtp_negotiation_result.mki_value[j]);
                }
            } else {
                esp_mbedtls_printf("    DTLS-SRTP no mki value negotiated");
            }
            esp_mbedtls_printf("\n");
    ...
    if (opt.transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
        if ((ret = esp_mbedtls_ssl_set_cid(&ssl, opt.cid_enabled_renego,
...
#if defined(MBEDTLS_MEMORY_DEBUG)
    esp_mbedtls_memory_buffer_alloc_cur_get(&current_heap_memory, &heap_blocks);
    esp_mbedtls_memory_buffer_alloc_max_get(&peak_heap_memory, &heap_blocks);
    esp_mbedtls_printf("Heap memory usage after handshake: %lu bytes. Peak memory usage was %lu\n",
...
data_exchange:
    ...
    if (opt.transport == MBEDTLS_SSL_TRANSPORT_STREAM) {
        do {
            ...
            if (esp_mbedtls_ssl_get_bytes_avail(&ssl) == 0) {
                len = ret;
                buf[len] = '\0';
                ...
                ori_len = ret;
                extra_len = (int) esp_mbedtls_ssl_get_bytes_avail(&ssl);

                larger_buf = esp_mbedtls_calloc(1, ori_len + extra_len + 1);
                if (larger_buf == NULL) {
                    esp_mbedtls_printf("  ! memory allocation failed\n");
                    ret = 1;
                    goto reset;
                }
                ...
                ret = esp_mbedtls_ssl_read(&ssl, larger_buf + ori_len, extra_len);
                if (ret != extra_len ||
                    esp_mbedtls_ssl_get_bytes_avail(&ssl) != 0) {
                    esp_mbedtls_printf("  ! esp_mbedtls_ssl_read failed on cached data\n");
                    ret = 1;
                    goto reset;
                }

                larger_buf[ori_len + extra_len] = '\0';
                esp_mbedtls_printf(" %d bytes read (%d + %d)\n\n%s\n",
        ...
        do {
            /* Without the call to `esp_mbedtls_ssl_check_pending`, it might
            ...
            /* Note that even if `esp_mbedtls_ssl_check_pending` returns true,
             * it can happen that the subsequent call to `esp_mbedtls_ssl_read`
    ...
    if (opt.renegotiate && exchanges_left == opt.exchanges) {
        esp_mbedtls_printf("  . Requestion renegotiation...");
        fflush(stdout);

        while ((ret = esp_mbedtls_ssl_renegotiate(&ssl)) != 0) {
            if (!mbedtls_status_is_ssl_in_progress(ret)) {
                esp_mbedtls_printf(" failed\n  ! esp_mbedtls_ssl_renegotiate returned %d\n\n", ret);
                goto reset;
            }
    ...
    if (opt.transport == MBEDTLS_SSL_TRANSPORT_STREAM) {
        for (written = 0, frags = 0; written < len; written += ret, frags++) {
            while ((ret = esp_mbedtls_ssl_write(&ssl, buf + written, len - written))
                   <= 0) {
        ...
        while (1) {
            ret = esp_mbedtls_ssl_write(&ssl, buf, len);
    ...
    buf[written] = '\0';
    esp_mbedtls_printf(" %d bytes written in %d fragments\n\n%s\n", written, frags, (char *) buf);
    ret = 0;
    ...
    if (opt.serialize != 0) {
        ...
        esp_mbedtls_printf("  . Serializing live connection...");

        ret = esp_mbedtls_ssl_context_save(&ssl, NULL, 0, &buf_len);
        if (ret != MBEDTLS_ERR_SSL_BUFFER_TOO_SMALL) {
            esp_mbedtls_printf(" failed\n  ! esp_mbedtls_ssl_context_save returned "
                           "-0x%x\n\n", (unsigned int) -ret);

            goto exit;
        }

        if ((context_buf = esp_mbedtls_calloc(1, buf_len)) == NULL) {
        ...
        if ((ret = esp_mbedtls_ssl_context_save(&ssl, context_buf,
                                            buf_len, &buf_len)) != 0) {
            esp_mbedtls_printf(" failed\n  ! esp_mbedtls_ssl_context_save returned "
                           "-0x%x\n\n", (unsigned int) -ret);
        ...
        if (0 < strlen(opt.context_file)) {
            ...
            esp_mbedtls_printf("  . Save serialized context to a file... ");

            esp_mbedtls_base64_encode(NULL, 0, &b64_len, context_buf, buf_len);

            if ((b64_buf = esp_mbedtls_calloc(1, b64_len)) == NULL) {
                esp_mbedtls_printf("failed\n  ! Couldn't allocate buffer for "
                               "the base64 code\n");
                goto exit;
            }

            if ((ret = esp_mbedtls_base64_encode(b64_buf, b64_len, &b64_len,
                                             context_buf, buf_len)) != 0) {
                esp_mbedtls_printf("failed\n  ! esp_mbedtls_base64_encode returned "
                               "-0x%x\n", (unsigned int) -ret);
                esp_mbedtls_free(b64_buf);
                goto exit;
            }
        ...
        if (opt.serialize == 1) {
            ...
            esp_mbedtls_printf("  . Context has been reset... ok\n");
        }
         ...
         */
        if (opt.serialize == 2) {
            esp_mbedtls_printf("  . Freeing and reinitializing context...");
            ...
            if ((ret = esp_mbedtls_ssl_setup(&ssl, &conf)) != 0) {
                esp_mbedtls_printf(" failed\n  ! esp_mbedtls_ssl_setup returned "
                               "-0x%x\n\n", (unsigned int) -ret);
                goto exit;
            }
             ...
             */
            if (opt.nbio == 2) {
                esp_mbedtls_ssl_set_bio(&ssl, &client_fd, delayed_send,
                                    delayed_recv, NULL);
            } else {
                esp_mbedtls_ssl_set_bio(&ssl, &client_fd, esp_mbedtls_net_send,
                                    esp_mbedtls_net_recv,
                                    opt.nbio == 0 ? esp_mbedtls_net_recv_timeout : NULL);
            }

#if defined(MBEDTLS_TIMING_C)
            esp_mbedtls_ssl_set_timer_cb(&ssl, &timer,
                                     esp_mbedtls_timing_set_delay,
                                     esp_mbedtls_timing_get_delay);
        ...
        esp_mbedtls_printf("  . Deserializing connection...");

        if ((ret = esp_mbedtls_ssl_context_load(&ssl, context_buf,
                                            buf_len)) != 0) {
            esp_mbedtls_printf("failed\n  ! esp_mbedtls_ssl_context_load returned "
                           "-0x%x\n\n", (unsigned int) -ret);

            goto exit;
        }

        esp_mbedtls_free(context_buf);
        context_buf = NULL;
        context_buf_len = 0;
...
close_notify:
    ...
    do {
        ret = esp_mbedtls_ssl_close_notify(&ssl);
    } while (ret == MBEDTLS_ERR_SSL_WANT_WRITE);
    ret = 0;

    esp_mbedtls_printf(" done\n");

#if defined(MBEDTLS_SSL_CACHE_C)
    if (opt.cache_remove > 0) {
        esp_mbedtls_ssl_cache_remove(&cache, ssl.session->id, ssl.session->id_len);
    }
#endif
...
exit:
    ...
    if (ret != 0) {
        char error_buf[100];
        esp_mbedtls_strerror(ret, error_buf, 100);
        esp_mbedtls_printf("Last error was: -0x%X - %s\n\n", (unsigned int) -ret, error_buf);
    }
#endif

    if (opt.query_config_mode == DFL_QUERY_CONFIG_MODE) {
        esp_mbedtls_printf("  . Cleaning up...");
        fflush(stdout);
    }
...
#endif
#if defined(MBEDTLS_SSL_SESSION_TICKETS) && defined(MBEDTLS_SSL_TICKET_C)
    esp_mbedtls_ssl_ticket_free(&ticket_ctx);
#endif
#if defined(MBEDTLS_SSL_COOKIE_C)
    esp_mbedtls_ssl_cookie_free(&cookie_ctx);
#endif

#if defined(MBEDTLS_SSL_CONTEXT_SERIALIZATION)
    if (context_buf != NULL) {
        esp_mbedtls_platform_zeroize(context_buf, context_buf_len);
    }
    esp_mbedtls_free(context_buf);
#endif
    ...
    if ((ret != 0) && (opt.query_config_mode == DFL_QUERY_CONFIG_MODE)) {
        esp_mbedtls_printf("Failed to list of opaque PSKs - error was %d\n", ret);
    }
#endif

#if defined(MBEDTLS_SSL_HANDSHAKE_WITH_CERT_ENABLED)
    esp_mbedtls_x509_crt_free(&cacert);
    esp_mbedtls_x509_crt_free(&srvcert);
    esp_mbedtls_pk_free(&pkey);
    esp_mbedtls_x509_crt_free(&srvcert2);
    esp_mbedtls_pk_free(&pkey2);
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    esp_psa_destroy_key(key_slot);
    esp_psa_destroy_key(key_slot2);
#endif
#endif
    ...
    for (i = 0; (size_t) i < ssl_async_keys.slots_used; i++) {
        if (ssl_async_keys.slots[i].pk_owned) {
            esp_mbedtls_pk_free(ssl_async_keys.slots[i].pk);
            esp_mbedtls_free(ssl_async_keys.slots[i].pk);
    ...
    if (opt.psk_opaque != 0) {
         ...
         * for example). */
        status = esp_psa_destroy_key(psk_slot);
        if ((status != PSA_SUCCESS) &&
            (opt.query_config_mode == DFL_QUERY_CONFIG_MODE)) {
            esp_mbedtls_printf("Failed to destroy key slot %u - error was %d",
    ...
    if ((opt.ecjpake_pw_opaque != DFL_ECJPAKE_PW_OPAQUE)) {
        ...
        if (esp_psa_get_key_attributes(ecjpake_pw_slot, &check_attributes) !=
            PSA_SUCCESS) {
            if (ret == 0) {
                ret = 1;
            }
            esp_mbedtls_printf("The EC J-PAKE password key has unexpectedly been already destroyed\n");
        } else {
            esp_psa_destroy_key(ecjpake_pw_slot);
        }
    }
    ...
    if (message) {
        ...
        esp_mbedtls_printf("PSA memory leak detected: %s\n",  message);
    }
#endif
    ...
    if (opt.query_config_mode == DFL_QUERY_CONFIG_MODE) {
        if (test_hooks_failure_detected()) {
            ...
            esp_mbedtls_printf("Test hooks detected errors.\n");
        }
    }
    test_hooks_free();
#endif /* MBEDTLS_TEST_HOOKS */
...
#if defined(MBEDTLS_MEMORY_BUFFER_ALLOC_C)
#if defined(MBEDTLS_MEMORY_DEBUG)
    esp_mbedtls_memory_buffer_alloc_status();
#endif
    ...
    if (opt.query_config_mode == DFL_QUERY_CONFIG_MODE) {
        ...
        esp_mbedtls_exit(query_config_ret);
    }
}
