              ...
              const char *str)
{
    ...
    esp_mbedtls_fprintf((FILE *) ctx, "%s:%04d: |%d| %s",
...
static int dummy_entropy(void *data, unsigned char *output, size_t len)
{
    ...
    ret = esp_mbedtls_entropy_func(data, output, len);
    for (i = 0; i < len; i++) {
...
void rng_init(rng_context_t *rng)
{
    ...
    esp_mbedtls_ctr_drbg_init(&rng->drbg);
#elif defined(MBEDTLS_HMAC_DRBG_C)
    esp_mbedtls_hmac_drbg_init(&rng->drbg);
#else
#error "No DRBG available"
#endif

    esp_mbedtls_entropy_init(&rng->entropy);
...
int rng_seed(rng_context_t *rng, int reproducible, const char *pers)
{
    ...
    int (*f_entropy)(void *, unsigned char *, size_t) =
        (reproducible ? dummy_entropy : esp_mbedtls_entropy_func);
...
#if defined(MBEDTLS_CTR_DRBG_C)
    int ret = esp_mbedtls_ctr_drbg_seed(&rng->drbg,
...
#endif
    int ret = esp_mbedtls_hmac_drbg_seed(&rng->drbg,
                                     esp_mbedtls_md_info_from_type(md_type),
                                     f_entropy, &rng->entropy,
                                     (const unsigned char *) pers,
                                     strlen(pers));
...
#error "No DRBG available"
    ...
    if (ret != 0) {
        esp_mbedtls_printf(" failed\n  ! esp_mbedtls_ctr_drbg_seed returned -0x%x\n",
                       (unsigned int) -ret);
        return ret;
    }
#endif /* !MBEDTLS_TEST_USE_PSA_CRYPTO_RNG */
...
#if defined(MBEDTLS_CTR_DRBG_C)
    esp_mbedtls_ctr_drbg_free(&rng->drbg);
#elif defined(MBEDTLS_HMAC_DRBG_C)
    esp_mbedtls_hmac_drbg_free(&rng->drbg);
#else
#error "No DRBG available"
#endif

    esp_mbedtls_entropy_free(&rng->entropy);
#endif /* !MBEDTLS_TEST_USE_PSA_CRYPTO_RNG */
}
...
#if defined(MBEDTLS_TEST_USE_PSA_CRYPTO_RNG)
    (void) p_rng;
    return esp_mbedtls_psa_get_random(MBEDTLS_PSA_RANDOM_STATE,
                                  output, output_len);
#else /* !MBEDTLS_TEST_USE_PSA_CRYPTO_RNG */
    rng_context_t *rng = p_rng;

#if defined(MBEDTLS_CTR_DRBG_C)
    return esp_mbedtls_ctr_drbg_random(&rng->drbg, output, output_len);
#elif defined(MBEDTLS_HMAC_DRBG_C)
    return esp_mbedtls_hmac_drbg_random(&rng->drbg, output, output_len);
#else
#error "No DRBG available"
#endif
                      ...
                      psa_key_usage_t psa_usage, mbedtls_svc_key_id_t *key_id)
{
    ...
    ret = esp_mbedtls_pk_get_psa_attributes(pk, PSA_KEY_USAGE_SIGN_HASH, &key_attr);
    if (ret != 0) {
        return ret;
    }
    ...
    ret = esp_mbedtls_pk_import_into_psa(pk, &key_attr, key_id);
    if (ret != 0) {
        return ret;
    }
    esp_mbedtls_pk_free(pk);
    esp_mbedtls_pk_init(pk);
    ret = esp_mbedtls_pk_setup_opaque(pk, *key_id);
    if (ret != 0) {
        return ret;
    }
                ...
                mbedtls_x509_crt **candidates)
{
    ...
    first = esp_mbedtls_calloc(1, sizeof(mbedtls_x509_crt));
    if (first == NULL) {
        ret = -1;
        goto exit;
    }
    esp_mbedtls_x509_crt_init(first);

    if (esp_mbedtls_x509_crt_parse_der(first, ca->raw.p, ca->raw.len) != 0) {
        ret = -1;
        goto exit;
    }
...
exit:
    ...
    if (ret != 0) {
        esp_mbedtls_x509_crt_free(first);
        esp_mbedtls_free(first);
        first = NULL;
    }
...
int delayed_recv(void *ctx, unsigned char *buf, size_t len)
{
    ...
    ret = esp_mbedtls_net_recv(ctx, buf, len);
...
int delayed_send(void *ctx, const unsigned char *buf, size_t len)
{
    ...
    ret = esp_mbedtls_net_send(ctx, buf, len);
...
#endif
{
    ...
    while (1) {
...
#if defined(MBEDTLS_TIMING_C)
        if (timer != NULL &&
            esp_mbedtls_timing_get_delay(timer) == 2) {
            break;
        }
        ...
        if (poll_type != 0) {
            ret = esp_mbedtls_net_poll(fd, poll_type, 0);
            if (ret < 0) {
                return ret;
            }
            if (ret == poll_type) {
...
static void mbedtls_ssl_print_supported_groups_list(void)
{
    for (int i = 0; tls_id_group_name_table[i].tls_id != 0; i++) {
        if (tls_id_group_name_table[i].is_supported == 1) {
            esp_mbedtls_printf("%s ", tls_id_group_name_table[i].name);
        }
    }
}
    ...
    if (strcmp(p, "none") == 0) {
        ...
        while (i < group_list_len - 1 && *p != '\0') {
            ...
            if ((curve_tls_id = mbedtls_ssl_get_curve_tls_id_from_name(q)) != 0) {
                ...
                esp_mbedtls_printf("unknown group %s\n", q);
                esp_mbedtls_printf("supported groups: ");
        ...
        esp_mbedtls_printf("Number of groups: %u\n", (unsigned int) i);

        if (i == group_list_len - 1 && *p != '\0') {
            esp_mbedtls_printf("groups list too long, maximum %u",
