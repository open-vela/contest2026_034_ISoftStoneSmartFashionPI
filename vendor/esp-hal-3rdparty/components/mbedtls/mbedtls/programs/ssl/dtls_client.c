...
int main(void)
{
    esp_mbedtls_printf("MBEDTLS_SSL_CLI_C and/or MBEDTLS_SSL_PROTO_DTLS and/or "
...
int main(int argc, char *argv[])
{
     ...
     */
    esp_mbedtls_printf("  . Connecting to udp/%s/%s...", SERVER_NAME, SERVER_PORT);
    fflush(stdout);

    if ((ret = esp_mbedtls_net_connect(&server_fd, SERVER_ADDR,
     ...
     * 2. Setup stuff
     */
    esp_mbedtls_printf("  . Setting up the DTLS structure...");
    fflush(stdout);
    ...
    esp_mbedtls_ssl_conf_read_timeout(&conf, READ_TIMEOUT_MS);
    ...
    if ((ret = esp_mbedtls_ssl_set_hostname(&ssl, SERVER_NAME)) != 0) {
        esp_mbedtls_printf(" failed\n  ! esp_mbedtls_ssl_set_hostname returned %d\n\n", ret);
        goto exit;
    }

    esp_mbedtls_ssl_set_bio(&ssl, &server_fd,
                        esp_mbedtls_net_send, esp_mbedtls_net_recv, esp_mbedtls_net_recv_timeout);
     ...
     * 4. Handshake
     */
    esp_mbedtls_printf("  . Performing the DTLS handshake...");
    fflush(stdout);
...
send_request:
    ...
    do {
        ret = esp_mbedtls_ssl_write(&ssl, (unsigned char *) MESSAGE, len);
    } while (ret == MBEDTLS_ERR_SSL_WANT_READ ||
             ret == MBEDTLS_ERR_SSL_WANT_WRITE);
     ...
     */
    esp_mbedtls_printf("  < Read from server:");
    fflush(stdout);
