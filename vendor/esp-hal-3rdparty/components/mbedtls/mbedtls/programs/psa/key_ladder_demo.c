 ...
 * - "esp_unwrap": derive a wrapping key from the last intermediate key,
...
#include "mbedtls/platform.h" // for esp_mbedtls_setbuf
#include "mbedtls/platform_util.h" // for esp_mbedtls_platform_zeroize
...
static psa_status_t generate(const char *key_file_name)
{
    ...
    PSA_CHECK(esp_psa_generate_key(&attributes, &key));

    PSA_CHECK(save_key(key, key_file_name));

exit:
    (void) esp_psa_destroy_key(key);
    return status;
}
                                         ...
                                         psa_key_id_t *master_key)
{
    ...
    psa_set_key_type(&attributes, PSA_KEY_TYPE_DERIVE);
    PSA_CHECK(esp_psa_import_key(&attributes, key_data, key_size, master_key));
exit:
    if (key_file != NULL) {
        fclose(key_file);
    }
    esp_mbedtls_platform_zeroize(key_data, sizeof(key_data));
    if (status != PSA_SUCCESS) {
        /* If the key creation hasn't happened yet or has failed,
         * *master_key is null. esp_psa_destroy_key( 0 ) is
         * guaranteed to do nothing and return PSA_SUCCESS. */
        (void) esp_psa_destroy_key(*master_key);
        *master_key = 0;
    }
    return status;
}
                                      ...
                                      psa_key_id_t *key)
{
    ...
    for (i = 0; i < ladder_depth; i++) {
        ...
        PSA_CHECK(esp_psa_key_derivation_setup(&operation, KDF_ALG));
        PSA_CHECK(esp_psa_key_derivation_input_bytes(
                      &operation, PSA_KEY_DERIVATION_INPUT_SALT,
                      DERIVE_KEY_SALT, DERIVE_KEY_SALT_LENGTH));
        PSA_CHECK(esp_psa_key_derivation_input_key(
        ...
        PSA_CHECK(esp_psa_destroy_key(*key));
        *key = 0;
        /* Derive the next intermediate key from the parent key. */
        PSA_CHECK(esp_psa_key_derivation_output_key(&attributes, &operation,
                                                key));
        PSA_CHECK(esp_psa_key_derivation_abort(&operation));
    }

exit:
    esp_psa_key_derivation_abort(&operation);
    if (status != PSA_SUCCESS) {
        esp_psa_destroy_key(*key);
        *key = 0;
    }
    return status;
}
                              ...
                              psa_key_id_t wrapping_key)
{
    ...
    /* Ensure no stdio buffering of secrets, as such buffers cannot be wiped. */
    esp_mbedtls_setbuf(input_file, NULL);
    ...
    PSA_CHECK(esp_psa_get_key_attributes(wrapping_key, &attributes));
    ...
    /* Wrap the data. */
    PSA_CHECK(esp_psa_generate_random(header.iv, WRAPPING_IV_SIZE));
    PSA_CHECK(esp_psa_aead_encrypt(wrapping_key, WRAPPING_ALG,
    ...
    /* Ensure no stdio buffering of secrets, as such buffers cannot be wiped. */
    esp_mbedtls_setbuf(output_file, NULL);
...
exit:
    ...
    if (buffer != NULL) {
        esp_mbedtls_platform_zeroize(buffer, buffer_size);
    }
    free(buffer);
    return status;
}
                                ...
                                psa_key_id_t wrapping_key)
{
    ...
    /* Unwrap the data. */
    PSA_CHECK(esp_psa_aead_decrypt(wrapping_key, WRAPPING_ALG,
                               header.iv, WRAPPING_IV_SIZE,
                               (uint8_t *) &header, sizeof(header),
...
exit:
    ...
    if (buffer != NULL) {
        esp_mbedtls_platform_zeroize(buffer, ciphertext_size);
    }
    free(buffer);
    return status;
}
...
exit:
    ...
    (void) esp_psa_destroy_key(derivation_key);
    (void) esp_psa_destroy_key(wrapping_key);
...
static void usage(void)
{
    ...
    printf("  esp_unwrap    Unwrap (decrypt) input with the derived key\n");
    printf("  wrap      Wrap (encrypt) input with the derived key\n");
    printf("\n");
    printf("Options:\n");
    printf("  input=FILENAME    Input file (required for wrap/esp_unwrap)\n");
    printf("  master=FILENAME   File containing the master key (default: master.key)\n");
    printf("  output=FILENAME   Output file (required for save/wrap/esp_unwrap)\n");
...
int main(int argc, char *argv[])
{
    ...
    if (strcmp(argv[1], "generate") == 0) {
        ...
        mode = MODE_SAVE;
    } else if (strcmp(argv[1], "esp_unwrap") == 0) {
        mode = MODE_UNWRAP;
