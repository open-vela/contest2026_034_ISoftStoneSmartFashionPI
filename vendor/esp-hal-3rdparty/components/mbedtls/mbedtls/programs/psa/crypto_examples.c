                                     ...
                                     size_t *output_len)
{
    ...
    *output_len = 0;
    while (bytes_written != input_size) {
        ...
        status = esp_psa_cipher_update(operation, input + bytes_written,
    ...
    status = esp_psa_cipher_finish(operation, output + *output_len,
                               output_size - *output_len, &len);
    ASSERT_STATUS(status, PSA_SUCCESS);
    *output_len += len;
                                   ...
                                   size_t *output_len)
{
    ...
    status = esp_psa_cipher_generate_iv(&operation, iv, iv_size, &iv_len);
    ASSERT_STATUS(status, PSA_SUCCESS);
                                   ...
                                   size_t *output_len)
{
    ...
    status = esp_psa_cipher_set_iv(&operation, iv, iv_size);
    ASSERT_STATUS(status, PSA_SUCCESS);
...
cipher_example_encrypt_decrypt_aes_cbc_nopad_1_block(void)
{
    ...
    status = esp_psa_generate_random(input, sizeof(input));
    ASSERT_STATUS(status, PSA_SUCCESS);
    ...
    status = esp_psa_generate_key(&attributes, &key);
    ASSERT_STATUS(status, PSA_SUCCESS);
...
int main(void)
{
    ASSERT(esp_psa_crypto_init() == PSA_SUCCESS);
    cipher_examples();
exit:
