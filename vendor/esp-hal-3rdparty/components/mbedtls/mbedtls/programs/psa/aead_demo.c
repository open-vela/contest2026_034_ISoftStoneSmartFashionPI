                                 ...
                                 psa_algorithm_t *alg)
{
    ...
    /* Import key */
    PSA_CHECK(esp_psa_import_key(&attributes, key_bytes, key_bits / 8, key));
...
static void aead_info(psa_key_id_t key, psa_algorithm_t alg)
{
    ...
    (void) esp_psa_get_key_attributes(key, &attr);
                        ...
                        const unsigned char *part2, size_t part2_len)
{
    ...
    PSA_CHECK(esp_psa_aead_encrypt_setup(&op, key, alg));

    PSA_CHECK(esp_psa_aead_set_nonce(&op, iv, iv_len));
    PSA_CHECK(esp_psa_aead_update_ad(&op, ad, ad_len));
    PSA_CHECK(esp_psa_aead_update(&op, part1, part1_len, p, end - p, &olen));
    p += olen;
    PSA_CHECK(esp_psa_aead_update(&op, part2, part2_len, p, end - p, &olen));
    p += olen;
    PSA_CHECK(esp_psa_aead_finish(&op, p, end - p, &olen,
...
exit:
    esp_psa_aead_abort(&op);   // required on errors, harmless on success
    return status;
}
