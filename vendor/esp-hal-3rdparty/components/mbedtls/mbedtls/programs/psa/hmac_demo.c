...
psa_status_t hmac_demo(void)
{
    ...
    PSA_CHECK(esp_psa_mac_sign_setup(&op, key, alg));
    PSA_CHECK(esp_psa_mac_update(&op, msg1_part1, sizeof(msg1_part1)));
    PSA_CHECK(esp_psa_mac_update(&op, msg1_part2, sizeof(msg1_part2)));
    PSA_CHECK(esp_psa_mac_sign_finish(&op, out, sizeof(out), &out_len));
    print_buf("msg1", out, out_len);

    /* compute HMAC(key, msg2_part1 | msg2_part2) */
    PSA_CHECK(esp_psa_mac_sign_setup(&op, key, alg));
    PSA_CHECK(esp_psa_mac_update(&op, msg2_part1, sizeof(msg2_part1)));
    PSA_CHECK(esp_psa_mac_update(&op, msg2_part2, sizeof(msg2_part2)));
    PSA_CHECK(esp_psa_mac_sign_finish(&op, out, sizeof(out), &out_len));
    print_buf("msg2", out, out_len);

exit:
    esp_psa_mac_abort(&op);   // needed on error, harmless on success
