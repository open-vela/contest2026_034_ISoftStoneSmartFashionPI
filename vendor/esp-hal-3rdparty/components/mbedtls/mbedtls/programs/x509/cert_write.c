...
int main(void)
{
    esp_mbedtls_printf("MBEDTLS_X509_CRT_WRITE_C and/or MBEDTLS_X509_CRT_PARSE_C and/or "
                      ...
                      void *p_rng)
{
    ...
    if (opt.format == FORMAT_DER) {
        ret = esp_mbedtls_x509write_crt_der(crt, output_buf, 4096,
                                        f_rng, p_rng);
        if (ret < 0) {
            return ret;
        }
        ...
        ret = esp_mbedtls_x509write_crt_pem(crt, output_buf, 4096,
                                        f_rng, p_rng);
        if (ret < 0) {
            return ret;
        }
...
int main(int argc, char *argv[])
{
     ...
     * Set to sane values
     */
    esp_mbedtls_x509write_crt_init(&crt);
    esp_mbedtls_pk_init(&loaded_issuer_key);
    esp_mbedtls_pk_init(&loaded_subject_key);
    esp_mbedtls_ctr_drbg_init(&ctr_drbg);
    esp_mbedtls_entropy_init(&entropy);
#if defined(MBEDTLS_X509_CSR_PARSE_C)
    esp_mbedtls_x509_csr_init(&csr);
#endif
    esp_mbedtls_x509_crt_init(&issuer_crt);
...
usage:
        ...
        *q++ = '\0';
        ...
        if (strcmp(p, "request_file") == 0) {
            ...
            if (serial_frmt != SERIAL_FRMT_UNSPEC) {
                esp_mbedtls_printf("Invalid attempt to set the serial more than once\n");
                goto usage;
            }
            ...
            if (opt.authority_identifier != 0 &&
                ...
                esp_mbedtls_printf("Invalid argument for option %s\n", p);
                goto usage;
            }
            ...
            const mbedtls_md_info_t *md_info =
                esp_mbedtls_md_info_from_string(q);
            if (md_info == NULL) {
                esp_mbedtls_printf("Invalid argument for option %s\n", p);
                goto usage;
            }
            opt.md = esp_mbedtls_md_get_type(md_info);
            ...
            while (q != NULL) {
                ...
                ext_key_usage = esp_mbedtls_calloc(1, sizeof(mbedtls_asn1_sequence));
            ...
            while (q != NULL) {
                ...
                cur = esp_mbedtls_calloc(1, sizeof(mbedtls_x509_san_list));
                if (cur == NULL) {
                    esp_mbedtls_printf("Not enough memory for subjectAltName list\n");
                    goto usage;
                }
                ...
                if (strcmp(q, "RFC822") == 0) {
                    ...
                    ip_addr_len = esp_mbedtls_x509_crt_parse_cn_inet_pton(subtype_value, ip);
                    if (ip_addr_len == 0) {
                        esp_mbedtls_printf("esp_mbedtls_x509_crt_parse_cn_inet_pton failed to parse %s\n",
                                       subtype_value);
                        goto exit;
                    }
                    ...
                    if ((ret = esp_mbedtls_x509_string_to_names(&ext_san_dirname,
                                                            subtype_value)) != 0) {
                        esp_mbedtls_strerror(ret, buf, sizeof(buf));
                        esp_mbedtls_printf(
                            " failed\n  !  esp_mbedtls_x509_string_to_names "
                            "returned -0x%04x - %s\n\n",
    ...
    if ((ret = esp_mbedtls_ctr_drbg_seed(&ctr_drbg, esp_mbedtls_entropy_func, &entropy,
                                     ...
                                     strlen(pers))) != 0) {
        esp_mbedtls_strerror(ret, buf, sizeof(buf));
        esp_mbedtls_printf(" failed\n  !  esp_mbedtls_ctr_drbg_seed returned %d - %s\n",
                       ret, buf);
        goto exit;
    }
    ...
    // Parse serial to MPI
    //
    esp_mbedtls_printf("  . Reading serial number...");
    fflush(stdout);
    ...
    if (!opt.selfsign && strlen(opt.issuer_crt)) {
         ...
         */
        esp_mbedtls_printf("  . Loading the issuer certificate ...");
        fflush(stdout);

        if ((ret = esp_mbedtls_x509_crt_parse_file(&issuer_crt, opt.issuer_crt)) != 0) {
            esp_mbedtls_strerror(ret, buf, sizeof(buf));
            esp_mbedtls_printf(" failed\n  !  esp_mbedtls_x509_crt_parse_file "
                           "returned -0x%04x - %s\n\n", (unsigned int) -ret, buf);
            goto exit;
        }

        ret = esp_mbedtls_x509_dn_gets(issuer_name, sizeof(issuer_name),
                                   &issuer_crt.subject);
        if (ret < 0) {
            esp_mbedtls_strerror(ret, buf, sizeof(buf));
            esp_mbedtls_printf(" failed\n  !  esp_mbedtls_x509_dn_gets "
                           "returned -0x%04x - %s\n\n", (unsigned int) -ret, buf);
            goto exit;
        }
    ...
    if (!opt.selfsign && strlen(opt.request_file)) {
         ...
         * 1.0.b. Load the CSR
         */
        esp_mbedtls_printf("  . Loading the certificate request ...");
        fflush(stdout);

        if ((ret = esp_mbedtls_x509_csr_parse_file(&csr, opt.request_file)) != 0) {
            esp_mbedtls_strerror(ret, buf, sizeof(buf));
            esp_mbedtls_printf(" failed\n  !  esp_mbedtls_x509_csr_parse_file "
                           "returned -0x%04x - %s\n\n", (unsigned int) -ret, buf);
            goto exit;
        }

        ret = esp_mbedtls_x509_dn_gets(subject_name, sizeof(subject_name),
                                   &csr.subject);
        if (ret < 0) {
            esp_mbedtls_strerror(ret, buf, sizeof(buf));
            esp_mbedtls_printf(" failed\n  !  esp_mbedtls_x509_dn_gets "
     ...
     */
    if (!opt.selfsign && !strlen(opt.request_file)) {
        esp_mbedtls_printf("  . Loading the subject key ...");
        fflush(stdout);

        ret = esp_mbedtls_pk_parse_keyfile(&loaded_subject_key, opt.subject_key,
                                       opt.subject_pwd, esp_mbedtls_ctr_drbg_random, &ctr_drbg);
        if (ret != 0) {
            esp_mbedtls_strerror(ret, buf, sizeof(buf));
            esp_mbedtls_printf(" failed\n  !  esp_mbedtls_pk_parse_keyfile "
                           "returned -0x%04x - %s\n\n", (unsigned int) -ret, buf);
            goto exit;
        }
    ...
    esp_mbedtls_printf("  . Loading the issuer key ...");
    fflush(stdout);

    ret = esp_mbedtls_pk_parse_keyfile(&loaded_issuer_key, opt.issuer_key,
                                   opt.issuer_pwd, esp_mbedtls_ctr_drbg_random, &ctr_drbg);
    if (ret != 0) {
        esp_mbedtls_strerror(ret, buf, sizeof(buf));
        esp_mbedtls_printf(" failed\n  !  esp_mbedtls_pk_parse_keyfile "
                       "returned -x%02x - %s\n\n", (unsigned int) -ret, buf);
    ...
    if (strlen(opt.issuer_crt)) {
        if (esp_mbedtls_pk_check_pair(&issuer_crt.pk, issuer_key,
    ...
    esp_mbedtls_x509write_crt_set_subject_key(&crt, subject_key);
    esp_mbedtls_x509write_crt_set_issuer_key(&crt, issuer_key);

    /*
     * 1.0. Check the names for validity
     */
    if ((ret = esp_mbedtls_x509write_crt_set_subject_name(&crt, opt.subject_name)) != 0) {
        esp_mbedtls_strerror(ret, buf, sizeof(buf));
        esp_mbedtls_printf(" failed\n  !  esp_mbedtls_x509write_crt_set_subject_name "
                       "returned -0x%04x - %s\n\n", (unsigned int) -ret, buf);
        goto exit;
    }

    if ((ret = esp_mbedtls_x509write_crt_set_issuer_name(&crt, opt.issuer_name)) != 0) {
        esp_mbedtls_strerror(ret, buf, sizeof(buf));
        esp_mbedtls_printf(" failed\n  !  esp_mbedtls_x509write_crt_set_issuer_name "
                       "returned -0x%04x - %s\n\n", (unsigned int) -ret, buf);
        goto exit;
    }

    esp_mbedtls_printf("  . Setting certificate values ...");
    fflush(stdout);

    esp_mbedtls_x509write_crt_set_version(&crt, opt.version);
    esp_mbedtls_x509write_crt_set_md_alg(&crt, opt.md);

    ret = esp_mbedtls_x509write_crt_set_serial_raw(&crt, serial, serial_len);
    if (ret != 0) {
        esp_mbedtls_strerror(ret, buf, sizeof(buf));
        esp_mbedtls_printf(" failed\n  !  esp_mbedtls_x509write_crt_set_serial_raw "
                       "returned -0x%04x - %s\n\n", (unsigned int) -ret, buf);
        goto exit;
    }

    ret = esp_mbedtls_x509write_crt_set_validity(&crt, opt.not_before, opt.not_after);
    if (ret != 0) {
        esp_mbedtls_strerror(ret, buf, sizeof(buf));
        esp_mbedtls_printf(" failed\n  !  esp_mbedtls_x509write_crt_set_validity "
                       "returned -0x%04x - %s\n\n", (unsigned int) -ret, buf);
        goto exit;
    }
    ...
    if (opt.version == MBEDTLS_X509_CRT_VERSION_3 &&
        ...
        esp_mbedtls_printf("  . Adding the Basic Constraints extension ...");
        fflush(stdout);

        ret = esp_mbedtls_x509write_crt_set_basic_constraints(&crt, opt.is_ca,
                                                          opt.max_pathlen);
        if (ret != 0) {
            esp_mbedtls_strerror(ret, buf, sizeof(buf));
    ...
    if (opt.version == MBEDTLS_X509_CRT_VERSION_3 &&
        ...
        esp_mbedtls_printf("  . Adding the Subject Key Identifier ...");
        fflush(stdout);

        ret = esp_mbedtls_x509write_crt_set_subject_key_identifier(&crt);
        if (ret != 0) {
            esp_mbedtls_strerror(ret, buf, sizeof(buf));
    ...
    if (opt.version == MBEDTLS_X509_CRT_VERSION_3 &&
        ...
        esp_mbedtls_printf("  . Adding the Authority Key Identifier ...");
        fflush(stdout);

        ret = esp_mbedtls_x509write_crt_set_authority_key_identifier(&crt);
        if (ret != 0) {
            esp_mbedtls_strerror(ret, buf, sizeof(buf));
    ...
    if (opt.version == MBEDTLS_X509_CRT_VERSION_3 &&
        opt.key_usage != 0) {
        esp_mbedtls_printf("  . Adding the Key Usage extension ...");
        fflush(stdout);

        ret = esp_mbedtls_x509write_crt_set_key_usage(&crt, opt.key_usage);
        if (ret != 0) {
            esp_mbedtls_strerror(ret, buf, sizeof(buf));
            esp_mbedtls_printf(" failed\n  !  esp_mbedtls_x509write_crt_set_key_usage "
                           "returned -0x%04x - %s\n\n", (unsigned int) -ret, buf);
            goto exit;
        }
    ...
    if (opt.san_list != NULL) {
        ret = esp_mbedtls_x509write_crt_set_subject_alternative_name(&crt, opt.san_list);

        if (ret != 0) {
            esp_mbedtls_printf(
                " failed\n  !  esp_mbedtls_x509write_crt_set_subject_alternative_name returned %d",
                ret);
            goto exit;
        }
    }
    ...
    if (opt.ext_key_usage) {
        esp_mbedtls_printf("  . Adding the Extended Key Usage extension ...");
        fflush(stdout);

        ret = esp_mbedtls_x509write_crt_set_ext_key_usage(&crt, opt.ext_key_usage);
        if (ret != 0) {
            esp_mbedtls_strerror(ret, buf, sizeof(buf));
            esp_mbedtls_printf(
                " failed\n  !  esp_mbedtls_x509write_crt_set_ext_key_usage returned -0x%02x - %s\n\n",
                (unsigned int) -ret,
                buf);
            goto exit;
        }
    ...
    if (opt.version == MBEDTLS_X509_CRT_VERSION_3 &&
        opt.ns_cert_type != 0) {
        esp_mbedtls_printf("  . Adding the NS Cert Type extension ...");
        fflush(stdout);

        ret = esp_mbedtls_x509write_crt_set_ns_cert_type(&crt, opt.ns_cert_type);
        if (ret != 0) {
            esp_mbedtls_strerror(ret, buf, sizeof(buf));
            esp_mbedtls_printf(" failed\n  !  esp_mbedtls_x509write_crt_set_ns_cert_type "
                           "returned -0x%04x - %s\n\n", (unsigned int) -ret, buf);
            goto exit;
        }
     ...
     */
    esp_mbedtls_printf("  . Writing the certificate...");
    fflush(stdout);
...
#endif /* MBEDTLS_X509_CSR_PARSE_C */
    esp_mbedtls_asn1_free_named_data_list(&ext_san_dirname);
    esp_mbedtls_x509_crt_free(&issuer_crt);
    esp_mbedtls_x509write_crt_free(&crt);
    esp_mbedtls_pk_free(&loaded_subject_key);
    esp_mbedtls_pk_free(&loaded_issuer_key);
