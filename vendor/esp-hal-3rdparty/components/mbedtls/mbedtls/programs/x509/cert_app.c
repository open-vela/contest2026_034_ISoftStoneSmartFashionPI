...
static int my_verify(void *data, mbedtls_x509_crt *crt, int depth, uint32_t *flags)
{
    ...
    if ((*flags) == 0) {
        ...
        esp_mbedtls_x509_crt_verify_info(buf, sizeof(buf), "  ! ", *flags);
...
int main(int argc, char *argv[])
{
    ...
    esp_mbedtls_x509_crl_init(&cacrl);
#else
...
usage:
    ...
    if (strlen(opt.ca_path)) {
        if ((ret = esp_mbedtls_x509_crt_parse_path(&cacert, opt.ca_path)) < 0) {
            esp_mbedtls_printf(" failed\n  !  esp_mbedtls_x509_crt_parse_path returned -0x%x\n\n",
                           (unsigned int) -ret);
            goto exit;
        }
        ...
        if ((ret = esp_mbedtls_x509_crt_parse_file(&cacert, opt.ca_file)) < 0) {
    ...
    if (strlen(opt.crl_file)) {
        if ((ret = esp_mbedtls_x509_crl_parse_file(&cacrl, opt.crl_file)) != 0) {
            esp_mbedtls_printf(" failed\n  !  esp_mbedtls_x509_crl_parse returned -0x%x\n\n",
                           (unsigned int) -ret);
            goto exit;
        }
    ...
    if (opt.mode == MODE_FILE) {
        ...
        ret = esp_mbedtls_x509_crt_parse_file(&crt, opt.filename);

        if (ret < 0) {
            esp_mbedtls_printf(" failed\n  !  esp_mbedtls_x509_crt_parse_file returned %d\n\n", ret);
        ...
        if (opt.permissive == 0 && ret > 0) {
            esp_mbedtls_printf(
                " failed\n  !  esp_mbedtls_x509_crt_parse failed to parse %d certificates\n\n",
                ret);
            esp_mbedtls_x509_crt_free(&crt);
            goto exit;
        }
        ...
        while (cur != NULL) {
            ...
            ret = esp_mbedtls_x509_crt_info((char *) buf, sizeof(buf) - 1, "      ",
                                        cur);
            if (ret == -1) {
                esp_mbedtls_printf(" failed\n  !  esp_mbedtls_x509_crt_info returned %d\n\n", ret);
                esp_mbedtls_x509_crt_free(&crt);
                goto exit;
            }
         ...
         */
        if (verify) {
            esp_mbedtls_printf("  . Verifying X.509 certificate...");

            if ((ret = esp_mbedtls_x509_crt_verify(&crt, &cacert, &cacrl, NULL, &flags,
                                               my_verify, NULL)) != 0) {
                char vrfy_buf[512];
         ...
         */
        esp_mbedtls_printf("  . SSL connection to tcp/%s/%s...", opt.server_name,
                       opt.server_port);
        fflush(stdout);
        ...
        if (verify) {
            esp_mbedtls_ssl_conf_authmode(&conf, MBEDTLS_SSL_VERIFY_REQUIRED);
        ...
        esp_mbedtls_printf("  . Peer certificate information    ... skipped\n");
#else
        esp_mbedtls_printf("  . Peer certificate information    ...\n");
        ret = esp_mbedtls_x509_crt_info((char *) buf, sizeof(buf) - 1, "      ",
...
#if defined(MBEDTLS_X509_CRL_PARSE_C)
    esp_mbedtls_x509_crl_free(&cacrl);
#endif
