...
int main(void)
{
    esp_mbedtls_printf("MBEDTLS_X509_CSR_WRITE_C and/or MBEDTLS_FS_IO and/or "
                              ...
                              void *p_rng)
{
    ...
    if ((ret = esp_mbedtls_x509write_csr_pem(req, output_buf, 4096, f_rng, p_rng)) < 0) {
        return ret;
    }
...
int main(int argc, char *argv[])
{
     ...
     * Set to sane values
     */
    esp_mbedtls_x509write_csr_init(&req);
...
usage:
    ...
    for (i = 1; i < argc; i++) {
        ...
        *q++ = '\0';
        if (strcmp(p, "filename") == 0) {
            ...
            opt.md_alg = esp_mbedtls_md_get_type(md_info);
    ...
    esp_mbedtls_x509write_csr_set_md_alg(&req, opt.md_alg);

    /* Set the Key Usage Extension flags in the CSR */
    if (opt.key_usage || opt.force_key_usage == 1) {
        ret = esp_mbedtls_x509write_csr_set_key_usage(&req, opt.key_usage);

        if (ret != 0) {
            esp_mbedtls_printf(" failed\n  !  esp_mbedtls_x509write_csr_set_key_usage returned %d", ret);
            goto exit;
        }
    }
    ...
    if (opt.ns_cert_type || opt.force_ns_cert_type == 1) {
        ret = esp_mbedtls_x509write_csr_set_ns_cert_type(&req, opt.ns_cert_type);

        if (ret != 0) {
            esp_mbedtls_printf(" failed\n  !  esp_mbedtls_x509write_csr_set_ns_cert_type returned %d", ret);
            goto exit;
        }
    }
    ...
    if (opt.san_list != NULL) {
        ret = esp_mbedtls_x509write_csr_set_subject_alternative_name(&req, opt.san_list);

        if (ret != 0) {
            esp_mbedtls_printf(
                " failed\n  !  esp_mbedtls_x509write_csr_set_subject_alternative_name returned %d",
                ret);
            goto exit;
        }
    }
    ...
    if ((ret = esp_mbedtls_ctr_drbg_seed(&ctr_drbg, esp_mbedtls_entropy_func, &entropy,
                                     ...
                                     strlen(pers))) != 0) {
        esp_mbedtls_printf(" failed\n  !  esp_mbedtls_ctr_drbg_seed returned %d", ret);
        goto exit;
    }
     ...
     */
    esp_mbedtls_printf("  . Checking subject name...");
    fflush(stdout);

    if ((ret = esp_mbedtls_x509write_csr_set_subject_name(&req, opt.subject_name)) != 0) {
        esp_mbedtls_printf(" failed\n  !  esp_mbedtls_x509write_csr_set_subject_name returned %d", ret);
        goto exit;
    }
     ...
     * 1.1. Load the key
     */
    esp_mbedtls_printf("  . Loading the private key ...");
    fflush(stdout);

    ret = esp_mbedtls_pk_parse_keyfile(&key, opt.filename, opt.password,
                                   esp_mbedtls_ctr_drbg_random, &ctr_drbg);

    if (ret != 0) {
        esp_mbedtls_printf(" failed\n  !  esp_mbedtls_pk_parse_keyfile returned %d", ret);
        goto exit;
    }

    esp_mbedtls_x509write_csr_set_key(&req, &key);
     ...
     */
    esp_mbedtls_printf("  . Writing the certificate request ...");
    fflush(stdout);
...
exit:
    ...
    esp_mbedtls_x509write_csr_free(&req);
