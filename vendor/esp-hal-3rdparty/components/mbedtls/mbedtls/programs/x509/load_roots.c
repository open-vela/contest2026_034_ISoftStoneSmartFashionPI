...
int main(void)
{
    esp_mbedtls_printf("MBEDTLS_X509_CRT_PARSE_C and/or MBEDTLS_FS_IO and/or "
...
int read_certificates(const char *const *filenames)
{
    ...
    esp_mbedtls_x509_crt_init(&cas);

    for (cur = filenames; *cur != NULL; cur++) {
        ret = esp_mbedtls_x509_crt_parse_file(&cas, *cur);
        if (ret != 0) {
#if defined(MBEDTLS_ERROR_C) || defined(MBEDTLS_ERROR_STRERROR_DUMMY)
            char error_message[200];
            esp_mbedtls_strerror(ret, error_message, sizeof(error_message));
            printf("\n%s: -0x%04x (%s)\n",
...
int main(int argc, char *argv[])
{
    ...
    for (i = 1; i < (unsigned) argc; i++) {
        ...
        *q++ = '\0';
        ...
        if (strcmp(p, "iterations") == 0) {
            ...
            esp_mbedtls_printf("Unknown option: %s\n", p);
            esp_mbedtls_printf(USAGE);
            goto exit;
        }
    }
    ...
    if (*opt.filenames == 0) {
        esp_mbedtls_printf("Missing list of certificate files to parse\n");
        goto exit;
    }

    esp_mbedtls_printf("Parsing %u certificates", argc - i);
    if (opt.prime_cache) {
    ...
    (void) esp_mbedtls_timing_get_timer(&timer, 1);
    ...
    ms = esp_mbedtls_timing_get_timer(&timer, 0);
    esp_mbedtls_printf("\n%u iterations -> %lu ms\n", opt.iterations, ms);
