...
usage:
    ...
    ret = esp_mbedtls_x509_crl_parse_file(&crl, opt.filename);

    if (ret != 0) {
        esp_mbedtls_printf(" failed\n  !  esp_mbedtls_x509_crl_parse_file returned %d\n\n", ret);
     ...
     * 1.2 Print the CRL
     */
    esp_mbedtls_printf("  . CRL information    ...\n");
    ret = esp_mbedtls_x509_crl_info((char *) buf, sizeof(buf) - 1, "      ", &crl);
    if (ret == -1) {
        esp_mbedtls_printf(" failed\n  !  esp_mbedtls_x509_crl_info returned %d\n\n", ret);
        esp_mbedtls_x509_crl_free(&crl);
        goto exit;
    }
