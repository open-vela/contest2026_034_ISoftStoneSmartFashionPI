...
int main(void)
{
    esp_mbedtls_printf("MBEDTLS_BASE64_C and/or MBEDTLS_FS_IO not defined.\n");
    esp_mbedtls_exit(0);
}
#else
                       ...
                       unsigned char *output, size_t *olen)
{
    ...
    ret = esp_mbedtls_base64_decode(NULL, 0, &len, (const unsigned char *) s1, s2 - s1);
    ...
    if ((ret = esp_mbedtls_base64_decode(output, len, &len, (const unsigned char *) s1,
                                     s2 - s1)) != 0) {
        return ret;
    }
...
usage:
    ...
    ret = esp_load_file(opt.filename, &pem_buffer, &pem_size);

    if (ret != 0) {
#ifdef MBEDTLS_ERROR_C
        esp_mbedtls_strerror(ret, buf, 1024);
#endif
        esp_mbedtls_printf(" failed\n  !  esp_load_file returned %d - %s\n\n", ret, buf);
        goto exit;
    }
     ...
     */
    esp_mbedtls_printf("  . Converting from PEM to DER ...");
    fflush(stdout);
     ...
     */
    esp_mbedtls_printf("  . Writing the DER file ...");
    fflush(stdout);
