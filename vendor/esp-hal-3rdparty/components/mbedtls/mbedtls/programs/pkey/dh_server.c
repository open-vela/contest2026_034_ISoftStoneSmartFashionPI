...
int main(void)
{
    ...
    esp_mbedtls_mpi_init(&D); esp_mbedtls_mpi_init(&E); esp_mbedtls_mpi_init(&dhm_P);
    esp_mbedtls_mpi_init(&dhm_G);
    /*
     * 1. Setup the RNG
     */
    ...
    if ((ret = esp_mbedtls_mpi_read_file(&dhm_P, 16, f)) != 0 ||
        (ret = esp_mbedtls_mpi_read_file(&dhm_G, 16, f)) != 0 ||
        (ret = esp_mbedtls_dhm_set_group(&dhm, &dhm_P, &dhm_G) != 0)) {
    ...
    if ((ret = esp_mbedtls_net_bind(&listen_fd, NULL, SERVER_PORT, MBEDTLS_NET_PROTO_TCP)) != 0) {
    ...
    if ((ret =
             esp_mbedtls_dhm_make_params(&dhm, (int) esp_mbedtls_dhm_get_len(&dhm), buf, &n,
                                     esp_mbedtls_ctr_drbg_random, &ctr_drbg)) != 0) {
        esp_mbedtls_printf(" failed\n  ! esp_mbedtls_dhm_make_params returned %d\n\n", ret);
        goto exit;
    }
    ...
    if ((ret = esp_mbedtls_sha256(buf, n, hash, 0)) != 0) {
        esp_mbedtls_printf(" failed\n  ! esp_mbedtls_sha256 returned %d\n\n", ret);
        goto exit;
    }

    const size_t rsa_key_len = esp_mbedtls_rsa_get_len(&rsa);
    buf[n] = (unsigned char) (rsa_key_len >> 8);
    buf[n + 1] = (unsigned char) (rsa_key_len);

    if ((ret = esp_mbedtls_rsa_pkcs1_sign(&rsa, esp_mbedtls_ctr_drbg_random, &ctr_drbg,
                                      MBEDTLS_MD_SHA256, mdlen,
                                      hash, buf + n + 2)) != 0) {
        esp_mbedtls_printf(" failed\n  ! esp_mbedtls_rsa_pkcs1_sign returned %d\n\n", ret);
        goto exit;
    }
    ...
    if ((ret = esp_mbedtls_net_send(&client_fd, buf2, 2)) != 2 ||
        (ret = esp_mbedtls_net_send(&client_fd, buf, buflen)) != (int) buflen) {
    ...
    if ((ret = esp_mbedtls_net_recv(&client_fd, buf, n)) != (int) n) {
        esp_mbedtls_printf(" failed\n  ! esp_mbedtls_net_recv returned %d\n\n", ret);
        goto exit;
    }

    if ((ret = esp_mbedtls_dhm_read_public(&dhm, buf, n)) != 0) {
        esp_mbedtls_printf(" failed\n  ! esp_mbedtls_dhm_read_public returned %d\n\n", ret);
        goto exit;
    }
    ...
    ret = esp_mbedtls_aes_setkey_enc(&aes, buf, 256);
    if (ret != 0) {
        goto exit;
    }
    memcpy(buf, PLAINTEXT, 16);
    ret = esp_mbedtls_aes_crypt_ecb(&aes, MBEDTLS_AES_ENCRYPT, buf, buf);
    if (ret != 0) {
        goto exit;
    }

    if ((ret = esp_mbedtls_net_send(&client_fd, buf, 16)) != 16) {
        esp_mbedtls_printf(" failed\n  ! esp_mbedtls_net_send returned %d\n\n", ret);
        goto exit;
    }
...
exit:
    ...
    esp_mbedtls_mpi_free(&D); esp_mbedtls_mpi_free(&E); esp_mbedtls_mpi_free(&dhm_P);
    esp_mbedtls_mpi_free(&dhm_G);
