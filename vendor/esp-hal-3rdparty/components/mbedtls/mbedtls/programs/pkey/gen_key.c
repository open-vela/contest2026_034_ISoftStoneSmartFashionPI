...
int main(void)
{
    esp_mbedtls_printf("MBEDTLS_PK_WRITE_C and/or MBEDTLS_FS_IO and/or "
...
#define DFL_EC_CURVE            esp_mbedtls_ecp_curve_list()->grp_id
#else
...
usage:
...
#if defined(MBEDTLS_ECP_C)
        esp_mbedtls_printf(" available ec_curve values:\n");
        curve_info = esp_mbedtls_ecp_curve_list();
        esp_mbedtls_printf("    %s (default)\n", curve_info->name);
    ...
    for (i = 1; i < argc; i++) {
        ...
        *q++ = '\0';
        ...
        if (strcmp(p, "type") == 0) {
            ...
            if ((curve_info = esp_mbedtls_ecp_curve_info_from_name(q)) == NULL) {
                goto usage;
            }
    ...
    if (opt.use_dev_random) {
        if ((ret = esp_mbedtls_entropy_add_source(&entropy, dev_random_entropy_poll,
                                              NULL, DEV_RANDOM_THRESHOLD,
                                              MBEDTLS_ENTROPY_SOURCE_STRONG)) != 0) {
            esp_mbedtls_printf(" failed\n  ! esp_mbedtls_entropy_add_source returned -0x%04x\n",
                           (unsigned int) -ret);
            goto exit;
        }
    ...
    if ((ret = esp_mbedtls_pk_setup(&key,
                                esp_mbedtls_pk_info_from_type((mbedtls_pk_type_t) opt.type))) != 0) {
        esp_mbedtls_printf(" failed\n  !  esp_mbedtls_pk_setup returned -0x%04x", (unsigned int) -ret);
        goto exit;
    }

#if defined(MBEDTLS_RSA_C) && defined(MBEDTLS_GENPRIME)
    if (opt.type == MBEDTLS_PK_RSA) {
        ret = esp_mbedtls_rsa_gen_key(esp_mbedtls_pk_rsa(key), esp_mbedtls_ctr_drbg_random, &ctr_drbg,
                                  opt.rsa_keysize, 65537);
        if (ret != 0) {
            esp_mbedtls_printf(" failed\n  !  esp_mbedtls_rsa_gen_key returned -0x%04x",
                           (unsigned int) -ret);
            goto exit;
        }
    } else
    ...
    if (opt.type == MBEDTLS_PK_ECKEY) {
        ret = esp_mbedtls_ecp_gen_key((mbedtls_ecp_group_id) opt.ec_curve,
                                  esp_mbedtls_pk_ec(key),
                                  esp_mbedtls_ctr_drbg_random, &ctr_drbg);
        if (ret != 0) {
            esp_mbedtls_printf(" failed\n  !  esp_mbedtls_ecp_gen_key returned -0x%04x",
                           (unsigned int) -ret);
            goto exit;
        }
    } else
...
#endif
    esp_mbedtls_printf("  ! key type not supported\n");

    /*
     * 1.3 Export key
     */
    esp_mbedtls_printf("  . Writing key to file...");
