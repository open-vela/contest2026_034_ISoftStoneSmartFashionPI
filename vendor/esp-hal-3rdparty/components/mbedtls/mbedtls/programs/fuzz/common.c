...
int dummy_random(void *p_rng, unsigned char *output, size_t output_len)
{
    ...
    //esp_mbedtls_ctr_drbg_random requires a valid mbedtls_ctr_drbg_context in p_rng
    if (p_rng != NULL) {
        //use esp_mbedtls_ctr_drbg_random to find bugs in it
        ret = esp_mbedtls_ctr_drbg_random(p_rng, output, output_len);
    } else {
...
int dummy_entropy(void *data, unsigned char *output, size_t len)
{
    ...
    //use esp_mbedtls_entropy_func to find bugs in it
