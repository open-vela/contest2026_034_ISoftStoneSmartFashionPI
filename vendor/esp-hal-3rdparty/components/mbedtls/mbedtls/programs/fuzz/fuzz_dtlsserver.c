...
int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size)
{
    ...
    if (esp_mbedtls_ssl_cookie_setup(&cookie_ctx, dummy_random, &ctr_drbg) != 0) {
        goto exit;
    }

    esp_mbedtls_ssl_conf_dtls_cookies(&conf,
                                  esp_mbedtls_ssl_cookie_write,
                                  esp_mbedtls_ssl_cookie_check,
                                  &cookie_ctx);
    ...
    if (esp_mbedtls_ssl_set_client_transport_id(&ssl, client_ip, sizeof(client_ip)) != 0) {
        goto exit;
    }
