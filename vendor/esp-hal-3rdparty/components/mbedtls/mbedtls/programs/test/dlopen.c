...
int main(void)
{
    ...
    const int *(*ssl_list_ciphersuites)(void) =
        dlsym(tls_so, "esp_mbedtls_ssl_list_ciphersuites");
    CHECK_DLERROR("dlsym", "esp_mbedtls_ssl_list_ciphersuites");
    const int *ciphersuites = ssl_list_ciphersuites();
    for (n = 0; ciphersuites[n] != 0; n++) {/* nothing to do, we're just counting */
        ;
    }
    esp_mbedtls_printf("dlopen(%s): %u ciphersuites\n",
                   TLS_SO_FILENAME, n);
    dlclose(tls_so);
    ...
    const mbedtls_x509_crt_profile *profile =
        dlsym(x509_so, "esp_mbedtls_x509_crt_profile_default");
    CHECK_DLERROR("dlsym", "esp_mbedtls_x509_crt_profile_default");
    esp_mbedtls_printf("dlopen(%s): Allowed md mask: %08x\n",
    ...
    const int *(*md_list)(void) =
        dlsym(crypto_so, "esp_mbedtls_md_list");
    CHECK_DLERROR("dlsym", "esp_mbedtls_md_list");
    const int *mds = md_list();
    for (n = 0; mds[n] != 0; n++) {/* nothing to do, we're just counting */
        ;
    }
    esp_mbedtls_printf("dlopen(%s): %u hashes\n",
