...
int main(void)
{
    esp_mbedtls_printf("MBEDTLS_NET_C not defined.\n");
    esp_mbedtls_exit(0);
}
#else
...
{
    if (value == NULL) {
        esp_mbedtls_printf(" unknown option or missing value: %s\n", name);
    } else {
        esp_mbedtls_printf(" option %s: illegal value: %s\n", name, value);
    }
        ...
        *q++ = '\0';
        ...
        if (strcmp(p, "server_addr") == 0) {
            ...
            if (*delay_cnt == MAX_DELAYED_HS) {
                esp_mbedtls_printf(" too many uses of %s: only %d allowed\n",
                               p, MAX_DELAYED_HS);
                exit_usage(p, NULL);
            }

            len = strlen(q);
            buf = esp_mbedtls_calloc(1, len + 1);
            if (buf == NULL) {
                esp_mbedtls_printf(" Allocation failure\n");
                exit(1);
            }
            memcpy(buf, q, len + 1);
...
#else
            esp_mbedtls_printf(" option pack only defined if MBEDTLS_TIMING_C is enabled\n");
            exit(1);
#endif
...
static unsigned elapsed_time(void)
{
    ...
    if (initialized == 0) {
        (void) esp_mbedtls_timing_get_timer(&hires, 1);
        initialized = 1;
        return 0;
    }

    return esp_mbedtls_timing_get_timer(&hires, 0);
}
...
static int ctx_buffer_flush(ctx_buffer *buf)
{
    ...
    esp_mbedtls_printf("  %05u flush    %s: %u bytes, %u datagrams, last %u ms\n",
                   elapsed_time(), buf->description,
                   (unsigned) buf->len, buf->num_datagrams,
                   elapsed_time() - buf->packet_lifetime);

    ret = esp_mbedtls_net_send(buf->ctx, buf->data, buf->len);
                             ...
                             size_t len)
{
    ...
    if (len > sizeof(buf->data)) {
        esp_mbedtls_printf("  ! buffer size %u too large (max %u)\n",
    ...
    if (sizeof(buf->data) - buf->len < len) {
        if ((ret = ctx_buffer_flush(buf)) <= 0) {
            esp_mbedtls_printf("ctx_buffer_flush failed with -%#04x", (unsigned int) -ret);
            return ret;
        }
    }
                         ...
                         size_t len)
{
    ...
    ret = esp_mbedtls_net_send(ctx, data, len);
    if (ret < 0) {
        esp_mbedtls_printf("net_send returned -%#04x\n", (unsigned int) -ret);
    }
    return ret;
}
...
#if defined(MBEDTLS_TIMING_C)
    if (why == NULL) {
        esp_mbedtls_printf("  %05u dispatch %s %s (%u bytes)\n",
                       elapsed_time(), p->way, p->type, p->len);
    } else {
        esp_mbedtls_printf("  %05u dispatch %s %s (%u bytes): %s\n",
                       elapsed_time(), p->way, p->type, p->len, why);
    }
#else
    if (why == NULL) {
        esp_mbedtls_printf("        dispatch %s %s (%u bytes)\n",
                       p->way, p->type, p->len);
    } else {
        esp_mbedtls_printf("        dispatch %s %s (%u bytes): %s\n",
...
int send_packet(const packet *p, const char *why)
{
    ...
    if (opt.bad_cid != 0 &&
        ...
        if ((ret = dispatch_data(dst, buf, p->len)) <= 0) {
            esp_mbedtls_printf("  ! dispatch returned %d\n", ret);
            return ret;
        }
    }
    ...
    if (opt.bad_ad &&
        ...
        if (p->len <= 13) {
            esp_mbedtls_printf("  ! can't corrupt empty AD record");
        } else {
            ++buf[13];
                   ...
                   mbedtls_net_context *src)
{
    ...
    /* receive packet */
    if ((ret = esp_mbedtls_net_recv(src, cur.buf, sizeof(cur.buf))) <= 0) {
        esp_mbedtls_printf("  ! esp_mbedtls_net_recv returned %d\n", ret);
        return ret;
    }
    ...
    for (delay_idx = 0; delay_idx < delay_list_len; delay_idx++) {
        ...
        if (strcmp(delay_list[delay_idx], cur.type) == 0) {
            ...
            esp_mbedtls_free(delay_list[delay_idx]);
...
int main(int argc, char *argv[])
{
    ...
    if (opt.seed == 0) {
#if defined(MBEDTLS_HAVE_TIME)
        opt.seed = (unsigned int) esp_mbedtls_time(NULL);
#else
        opt.seed = 1;
#endif /* MBEDTLS_HAVE_TIME */
        esp_mbedtls_printf("  . Pseudo-random seed: %u\n", opt.seed);
    }
     ...
     */
    esp_mbedtls_printf("  . Connect to server on UDP/%s/%s ...",
                   opt.server_addr, opt.server_port);
    fflush(stdout);

    if ((ret = esp_mbedtls_net_connect(&server_fd, opt.server_addr, opt.server_port,
     ...
     */
    esp_mbedtls_printf("  . Bind on UDP/%s/%s ...",
                   opt.listen_addr, opt.listen_port);
    fflush(stdout);

    if ((ret = esp_mbedtls_net_bind(&listen_fd, opt.listen_addr, opt.listen_port,
                                MBEDTLS_NET_PROTO_UDP)) != 0) {
...
exit:
    ...
    for (delay_idx = 0; delay_idx < MAX_DELAYED_HS; delay_idx++) {
        esp_mbedtls_free(opt.delay_cli[delay_idx]);
        esp_mbedtls_free(opt.delay_srv[delay_idx]);
    }
