/** \file metatest.c
 *
 *  \brief Test esp_features of the test framework.
 *
...
void * (* volatile calloc_but_the_compiler_does_not_know)(size_t, size_t) = esp_mbedtls_calloc;
void(*volatile free_but_the_compiler_does_not_know)(void *) = esp_mbedtls_free;
...
void test_memory_poison(const char *name)
{
    ...
    if (sscanf(name,
        ...
        esp_mbedtls_fprintf(stderr, "%s: Bad name format: %s\n", __func__, name);
        return;
    }
...
void mutex_lock_not_initialized(const char *name)
{
    ...
    TEST_ASSERT(esp_mbedtls_mutex_lock(&mutex) == 0);
exit:
    ;
#endif
}
...
void mutex_unlock_not_initialized(const char *name)
{
     ...
     * threading implementations (e.g. pthread without our instrumentation)
     * might consider this normal usage. */
    TEST_ASSERT(esp_mbedtls_mutex_unlock(&mutex) == 0);
exit:
    ;
#endif
}
...
{
    esp_mbedtls_fprintf(out, "Usage: %s list|TEST\n", argv0);
    esp_mbedtls_fprintf(out, "Run a meta-test that should cause a test failure.\n");
    esp_mbedtls_fprintf(out, "With 'list', list the available tests and their platform requirement.\n");
}
...
int main(int argc, char *argv[])
{
    ...
    for (const metatest_t *p = metatests; p->name != NULL; p++) {
        if (strcmp(argv[1], p->name) == 0) {
            esp_mbedtls_printf("Running metatest %s...\n", argv[1]);
            ...
            esp_mbedtls_printf("Running metatest %s... done, result=%d\n",
                           argv[1], result);
            esp_mbedtls_exit(result == MBEDTLS_TEST_RESULT_SUCCESS ?
                         MBEDTLS_EXIT_SUCCESS :
                         MBEDTLS_EXIT_FAILURE);
        }
    }

    esp_mbedtls_fprintf(stderr, "%s: FATAL: No such metatest: %s\n",
                    argv0, command);
    esp_mbedtls_exit(MBEDTLS_EXIT_FAILURE);
}
