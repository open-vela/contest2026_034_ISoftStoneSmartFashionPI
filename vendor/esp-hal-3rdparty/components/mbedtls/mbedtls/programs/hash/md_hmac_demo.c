...
int hmac_demo(void)
{
    ...
    const mbedtls_md_info_t *info = esp_mbedtls_md_info_from_type(alg);
    CHK(esp_mbedtls_md_setup(&ctx, info, 1));
    CHK(esp_mbedtls_md_hmac_starts(&ctx, key_bytes, sizeof(key_bytes)));

    /* compute HMAC(key, msg1_part1 | msg1_part2) */
    CHK(esp_mbedtls_md_hmac_update(&ctx, msg1_part1, sizeof(msg1_part1)));
    CHK(esp_mbedtls_md_hmac_update(&ctx, msg1_part2, sizeof(msg1_part2)));
    CHK(esp_mbedtls_md_hmac_finish(&ctx, out));
    print_buf("msg1", out, esp_mbedtls_md_get_size(info));

    /* compute HMAC(key, msg2_part1 | msg2_part2) */
    CHK(esp_mbedtls_md_hmac_reset(&ctx));     // prepare for new operation
    CHK(esp_mbedtls_md_hmac_update(&ctx, msg2_part1, sizeof(msg2_part1)));
    CHK(esp_mbedtls_md_hmac_update(&ctx, msg2_part2, sizeof(msg2_part2)));
    CHK(esp_mbedtls_md_hmac_finish(&ctx, out));
    print_buf("msg2", out, esp_mbedtls_md_get_size(info));

exit:
    esp_mbedtls_md_free(&ctx);
    esp_mbedtls_platform_zeroize(out, sizeof(out));
