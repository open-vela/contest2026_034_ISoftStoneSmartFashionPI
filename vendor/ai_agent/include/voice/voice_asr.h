...
typedef struct voice_asr_ops {
     ...
     * When stream_open is NULL, voice_asr_stream_* falls back to the
     * legacy volc_asr streaming path, so existing backends that only
     * implement recognize() keep working unchanged.
     * handle is backend-private state allocated by stream_open. */
    int (*stream_open)(void **handle);
    int (*stream_send)(void *handle,
                       const unsigned char *pcm, size_t len);
    /* 1 = final utterance ready, 0 = nothing yet, <0 = error. */
    int (*stream_try_recv)(void *handle, char *text_out, size_t text_cap);
    /* Finish and release handle. */
    int (*stream_finish)(void *handle, char *text_out, size_t text_cap);
    /* Release handle without waiting for a result. */
    void (*stream_abort)(void *handle);
} voice_asr_ops_t;

/* Register an ASR backend. Multiple backends can be registered. */
int voice_asr_register(const voice_asr_ops_t *ops);

/* Reset the static backend registry (call before re-registering across a
 * task_create() re-launch). */
...
/* Non-blocking probe: 1=final utterance, 0=no result yet, <0=error.
 * Server-side silence detection.  Call between sends. */
