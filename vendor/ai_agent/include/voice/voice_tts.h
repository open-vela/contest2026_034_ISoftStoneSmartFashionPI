 ...
 * pcm_data/pcm_len: decoded PCM audio (16-bit LE, 24kHz, mono).
 * is_last: 1 if this is the final chunk; a NEGATIVE value is a
 *   rollback signal ("discard everything streamed so far") — used by
 *   the E2E hybrid drain when it truncates and the caller falls back
 *   to classic TTS, so file-backed consumers can rewind cleanly.
...
typedef struct voice_tts_ops {
    ...
    /* Synthesize UTF-8 text into raw PCM (16-bit LE, 24kHz, mono). */
    int (*synthesize)(const char *text,
                      unsigned char *pcm_out,
                      size_t pcm_cap,
                      size_t *pcm_len);

    /* Streaming synthesize — delivers PCM chunks via callback.
     * If NULL, voice_tts_speak_stream falls back to batch synthesize. */
