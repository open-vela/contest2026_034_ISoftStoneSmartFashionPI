...
通用 ASR/TTS 后端抽象，支持多厂商切换。内置火山引擎、小米 MiMo 两个后端。

**后端对比：**

| 后端 | ASR 传输 | TTS 传输 | 适用场景 |
|------|---------|---------|---------|
| `volcengine` | WebSocket 流式 | WebSocket 流式 | 低延迟（需专属凭证） |
| `mimo` | HTTPS REST（批量） | HTTPS REST + SSE | 接入简单（仅需一个 api-key）|

MiMo 使用 OpenAI 兼容 API，接入地址：`https://api.xiaomimimo.com/v1/chat/completions`，推荐用于 ESP32S3（配合 8MB octal PSRAM）。
...
# 方案 B：MiMo（小米）
# 1. 设置 API Key
vela> set_mimo_key <api_key>

# 2. 可选：设置 TTS 声线（默认 冰糖）
vela> set_mimo_voice 茉莉

# 3. 可选：设置 ASR 语言（auto/zh/en，默认 zh）
vela> set_mimo_asr_lang auto

# 4. 切换后端
vela> set_voice_tts mimo    # 或 volcengine
vela> set_voice_asr mimo    # 或 volcengine
```
