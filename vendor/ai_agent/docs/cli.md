...
| 命令 | 说明 |
|------|------|
| `set_volc_key <api_key>` | 设置 Volcengine TTS API Key |
| `set_volc_asr <id> <tok> <cluster>` | 设置 Volcengine ASR 凭证 |
| `set_volc_speaker <id>` | 设置 Volcengine TTS 语音角色 |
| `set_mimo_key <api_key>` | 设置小米 MiMo API Key |
| `set_mimo_voice <name>` | 设置 MiMo TTS 声线（冰糖/茉莉/苏打…）|
| `set_mimo_asr_lang <lang>` | 设置 MiMo ASR 语言（auto/zh/en）|
| `set_voice_tts <name>` | 切换 TTS 后端（volcengine/mimo）|
| `set_voice_asr <name>` | 切换 ASR 后端（volcengine/mimo）|
| `voice_start` | 启动语音通道 |
| `voice_stop` | 停止语音通道 |
