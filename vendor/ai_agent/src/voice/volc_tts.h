...
/* Register the Volcengine WebSocket TTS backend. */
int volc_tts_ws_register(void);

/* Pre-open TTS WebSocket connection (call during LLM processing).
 * Saves ~1.5s TLS handshake time when TTS synthesis is triggered. */
int volc_tts_ws_preopen(void);

/* Cancel a pre-opened connection (call if TTS won't be needed). */
