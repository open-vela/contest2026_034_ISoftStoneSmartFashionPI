...
typedef struct {
    ...
    int pushback; /* one probed byte, -1 = empty */
} asr_tls_ctx_t;
    ...
    const char* host, const char* port)
{
    ...
    if (ctx->net.fd >= 0) {
        /* Short recv timeout during handshake so a stalled server
         * cannot hang the whole voice loop. 5s is enough for normal
         * TLS handshake; 8s was too conservative. */
        struct timeval tv = { .tv_sec = 8, .tv_usec = 0 };

        setsockopt(ctx->net.fd, SOL_SOCKET, SO_RCVTIMEO,
            &tv, sizeof(tv));
        /* Bound send() too — without SO_SNDTIMEO a full TCP send
         * buffer (WiFi stall / server backpressure) blocks forever
         * and freezes the conversation thread. */
    ...
    /* With a blocking socket + SO_RCVTIMEO, WANT_READ/WANT_WRITE is
     * returned only after a full recv timeout, i.e. the server stalled
     * the handshake. Do NOT retry forever (that turns a transient stall
     * into a permanent hang that kills the voice loop); treat it as a
     * timeout and bail so voice_asr_stream_open returns NULL and the
     * conversation thread retries cleanly ("ASR open failed, retry 1s"). */
    while ((ret = mbedtls_ssl_handshake(&ctx->ssl)) != 0) {
        if (ret == MBEDTLS_ERR_SSL_WANT_READ
            || ret == MBEDTLS_ERR_SSL_WANT_WRITE) {
            syslog(LOG_ERR, "[%s] handshake stalled (timeout), aborting\n",
                TAG);
            return -ETIMEDOUT;
        }
    ...
    /* Handshake done — restore the longer timeout for streaming recv.
     * Keep a short send timeout: audio frames are small (≤4KB) and a
     * healthy link sends them in ms; blocking >5s means the link is
     * dead and the stream must be rebuilt. */
    if (ctx->net.fd >= 0) {
...
{
    /* ASR connections are single-use and always discarded once the
     * result is obtained.  Force an abortive (RST) close via
     * SO_LINGER{on, timeout=0} so the network stack immediately
     * reclaims every queued TX buffer instead of parking unacked
     * audio in the FIN retransmit queue.  Without this, a few
     * connect/abort cycles drain the global IOB pool and even new
     * TLS handshakes fail to send (ssl_write/handshake -0x004e).
     * The graceful close_notify is skipped for the same reason (it
     * would enqueue another TLS record and can block on a dead
     * link). Requires CONFIG_NET_SOLINGER=y. */
    if (ctx->net.fd >= 0) {
    ...
    const unsigned char* buf, size_t len)
{
    ...
    while (written < len) {
        ...
        if (ret > 0) {
            ...
            /* WANT_WRITE with a blocking socket means SO_SNDTIMEO
             * expired (send buffer full).  Give the link a bounded
             * total budget, then declare the stream dead instead of
             * retrying forever and freezing the voice thread. */
            struct timespec t1;

            clock_gettime(CLOCK_MONOTONIC, &t1);
            if (t1.tv_sec - t0.tv_sec >= 6) {
                syslog(LOG_ERR, "[%s] ssl_write stalled >6s, "
                    "giving up\n", TAG);
                return -ETIMEDOUT;
            }
        }
    ...
    unsigned char* buf, size_t len)
{
    ...
    /* Consume the byte probed by try_recv, if any */
    if (ctx->pushback >= 0 && len > 0) {
        buf[0] = (unsigned char)ctx->pushback;
        ctx->pushback = -1;
        got = 1;
    }
    ...
    size_t* out_len, int* out_opcode)
{
    ...
    *out_opcode = hdr[0] & 0x0F;
    ...
    if (plen > cap) {
        /* Oversized frame (e.g. long utterances payload): must DRAIN
         * the payload to keep the WS byte stream in sync.  Returning
         * early without consuming it would desync all later frames. */
        syslog(LOG_WARNING, "[%s] WS frame too large: %zu > %zu, drop\n",
            TAG, plen, cap);
        size_t left = plen;
        while (left > 0) {
            size_t n = (left > cap) ? cap : left;
            ret = tls_read_all(ctx, buf, n);
            if (ret != 0) {
                return ret;
            }
            left -= n;
        }
        *out_len = 0; /* caller skips empty frames */
        return 0;
    }
...
/* Reply to a server PING with a masked PONG (control payload
 * is capped at 125 bytes by the WebSocket spec). */
static int ws_send_pong(asr_tls_ctx_t* ctx,
    const unsigned char* payload, size_t plen)
{
    unsigned char hdr[2 + WS_MASK_KEY_LEN];
    unsigned char mask[WS_MASK_KEY_LEN];
    ...
    asr_entropy_func(NULL, mask, WS_MASK_KEY_LEN);
    memcpy(hdr + 2, mask, WS_MASK_KEY_LEN);

    int ret = tls_write_all(ctx, hdr, sizeof(hdr));
    if (ret != 0) {
        return ret;
    }
    ...
    return (plen > 0) ? tls_write_all(ctx, body, plen) : 0;
}
    ...
    const char* cluster)
{
    ...
    /* show_utterances=1: server pushes utterance objects with
     * "definite":true when its VAD detects end of utterance.
     * This is the ONLY mid-stream end-of-speech signal — a
     * sequence<0 final is sent only after the last audio chunk. */
    ...
    cJSON_AddStringToObject(req, "start_silence_time", "1000");
 ...
 * Returns: 1 = got final result (sequence < 0),
 *          2 = server VAD finalized an utterance (definite=true),
 *          0 = intermediate, negative = error
 *
    ...
    char* text_out, size_t text_cap)
{
    ...
    /* Control frames: PING must be answered with PONG; PONG and
     * other empty keepalive frames are not protocol errors. */
    if (opcode == WS_OPCODE_PING) {
        ws_send_pong(ctx, buf, flen);
        free(buf);
        return 0;
    }
    if (opcode == WS_OPCODE_PONG || flen == 0) {
        free(buf);
        return 0;
    }
    ...
    if (cJSON_IsArray(result) && cJSON_GetArraySize(result) > 0) {
        ...
        /* Server VAD end-of-utterance: utterances[].definite == true */
        cJSON* utts = cJSON_GetObjectItem(first, "utterances");
        if (cJSON_IsArray(utts)) {
            int un = cJSON_GetArraySize(utts);
            for (int i = 0; i < un; i++) {
                cJSON* u = cJSON_GetArrayItem(utts, i);
                if (cJSON_IsTrue(cJSON_GetObjectItem(u, "definite"))) {
                    definite = 1;
                    break;
                }
            }
    ...
    return (definite && text_out[0] != '\0') ? 2 : 0;
}
...
struct volc_asr_stream {
    ...
    int got_final; /* 1 if check() already received final result */
};
...
/* Non-blocking check for server response.  Called between sends to
 * detect utterance boundaries (server-side silence detection).
 * Returns: 1 = final utterance got, text_out filled
 *          0 = no result yet / intermediate only
 *         <0 = error */
int volc_asr_stream_try_recv(volc_asr_stream_t* s,
    char* text_out, size_t text_cap)
{
    if (!s || !s->ready || !text_out || text_cap == 0) return -EINVAL;

    int fd = s->tls.net.fd;
    if (fd < 0) return -ENOTCONN;

    /* True non-blocking probe: try to read 1 byte with the socket
     * in O_NONBLOCK mode.  This covers both pending socket data and
     * data already buffered inside mbedtls.  (The old approach ran
     * the whole frame read under O_NONBLOCK, and tls_read_all
     * busy-spun on WANT_READ until the server sent something,
     * stalling the capture loop and dropping audio frames.) */
    unsigned char probe;
    int orig_flags = fcntl(fd, F_GETFL, 0);

    if (orig_flags >= 0) fcntl(fd, F_SETFL, orig_flags | O_NONBLOCK);
    int pr = mbedtls_ssl_read(&s->tls.ssl, &probe, 1);
    if (orig_flags >= 0) fcntl(fd, F_SETFL, orig_flags);

    if (pr == MBEDTLS_ERR_SSL_WANT_READ
        || pr == MBEDTLS_ERR_SSL_WANT_WRITE) {
        return 0; /* no data yet */
    }
    if (pr == 0 || pr == MBEDTLS_ERR_SSL_PEER_CLOSE_NOTIFY) {
        return -ECONNRESET;
    }
    if (pr < 0) {
        syslog(LOG_ERR, "[%s] probe ssl_read: -0x%04x\n", TAG, -pr);
        return -EIO;
    }

    /* Got the first byte of a server message: hand it back to the
     * frame parser and finish the read in blocking mode (bounded
     * by SO_RCVTIMEO on the socket). */
    s->tls.pushback = probe;

    int ret = recv_volc_response(&s->tls, text_out, text_cap);

    /* 1 = protocol final (seq<0); 2 = server VAD closed the
     * utterance (definite=true) — both end this listening turn. */
    if (ret == 1 || ret == 2) {
        s->got_final = 1;
        return 1;
    }
    if (ret == -ECONNRESET || ret == -EIO || ret == -EPROTO) {
        return ret; /* real connection/protocol error */
    }
    return 0; /* intermediate result or transient condition */
}
    ...
    char* text_out, size_t text_cap)
{
    ...
    /* If check() already received the final result, skip recv */
    if (s->got_final) {
        syslog(LOG_INFO, "[%s] stream: final result already received\n",
            TAG);
        /* Still try to send last-chunk to cleanly close the stream */
        send_audio_chunk(&s->tls, (const unsigned char*)"", 0, 1);
        asr_tls_free(&s->tls);
        free(s);
        return (text_out[0] != '\0') ? 0 : -ENODATA;
    }
