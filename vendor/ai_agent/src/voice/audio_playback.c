...
int audio_playback_drain(audio_playback_t* pb)
{
    if (!pb || !pb->player) {
        return -EINVAL;
    }

    /* Estimate playback duration from total bytes written.
     * bytes_per_frame = (bits/8) * channels = 4 for 16-bit stereo.
     * sample_rate = 24000.  Duration = total_written / (sample_rate * bytes_per_frame).
     * Add 200ms safety margin for I2S DMA drain. */
    if (pb->total_written > 0 && pb->bytes_per_frame > 0) {
        uint32_t sample_rate = AGENT_TTS_WS_SAMPLE_RATE;  /* 24000 */
        uint32_t duration_ms = (uint32_t)(pb->total_written * 1000ULL
            / (sample_rate * (uint32_t)pb->bytes_per_frame));
        uint32_t wait_ms = duration_ms + 200;
        if (wait_ms > 30000) wait_ms = 30000;
        usleep(wait_ms * 1000);
    } else {
        /* No data written — short wait for any buffered silence */
        usleep(200 * 1000);
    }
    return 0;
}
