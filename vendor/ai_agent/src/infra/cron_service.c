...
static void cron_fire_job(cron_job_t* job, time_t now)
{
    ...
    if (job->action[0] != '\0') {
        ...
        /* Plain reminder — speak directly for voice (the outbound dispatch
         * is a no-op for voice: normal replies are already spoken in
         * dispatch_response), then push for other channels / UI. */
        if (strcmp(job->channel, AGENT_CHAN_VOICE) == 0) {
            syslog(LOG_INFO, "[%s] Speaking reminder via TTS: %s\n",
                TAG, job->message);
            voice_channel_speak(job->message);
        }
