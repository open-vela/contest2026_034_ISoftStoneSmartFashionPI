...
static int save_json(cJSON *root)
{
    ...
    /* Mirror to SPI Flash so API keys survive reboot.
     * Failure here is non-fatal — next boot uses last good copy. */
    mkdirs(AGENT_PERSIST_CONFIG_DIR);
    int pfd = open(AGENT_PERSIST_CONFIG_FILE,
                   O_WRONLY | O_CREAT | O_TRUNC, 0644);
    if (pfd >= 0) {
        char *pstr = cJSON_PrintUnformatted(root);
        if (pstr) {
            FILE *pf = fdopen(pfd, "w");
            if (pf) { fputs(pstr, pf); fclose(pf); }
            else { close(pfd); }
            free(pstr);
        } else { close(pfd); }
    }
