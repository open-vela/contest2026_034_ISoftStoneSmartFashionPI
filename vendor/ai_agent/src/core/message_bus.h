...
typedef struct {
    ...
    int   flags;         /**< See AGENT_MSG_FLAG_*.  Zero for normal user messages. */
} agent_msg_t;

/* Proactive system prompt (battery reminder etc. queued via
 * voice_channel_inject_prompt): the agent loop must NOT apply the NL
 * fast path to it (its keywords would hijack the prompt — e.g. a
 * low-battery prompt contains 电量/充电 and would be answered by the
 * battery fast path without ever reaching the LLM). */
#define AGENT_MSG_FLAG_PROACTIVE (1u << 0)
