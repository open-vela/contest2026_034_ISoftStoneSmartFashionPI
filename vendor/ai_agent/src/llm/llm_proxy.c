...
static char s_model[32] = AGENT_LLM_DEFAULT_MODEL;
static char s_llm_host[64] = AGENT_LLM_API_HOST;
static char s_llm_path[64] = AGENT_LLM_API_PATH;
...
static char s_vision_model[32] = { 0 };
static char s_vision_host[64] = { 0 };
static char s_vision_api_key[64] = { 0 };

static pthread_mutex_t s_llm_lock = PTHREAD_MUTEX_INITIALIZER;

/* Override of max_completion_tokens / max_tokens applied to every
 * subsequent llm_chat / llm_chat_tools request until explicitly
 * cleared (call llm_set_max_tokens_hint(0)).  Voice / QuickApp
 * agent loops set this at the start of a message and clear it on
 * exit — short responses reduce LLM server-side scheduling latency.
 *
 * No lock needed: single-threaded agent_loop_task producer. */
static int s_max_tokens_hint = 0;

void llm_set_max_tokens_hint(int max_tokens)
{
    s_max_tokens_hint = max_tokens > 0 ? max_tokens : 0;
}

/* Per-message hint: disable deep-thinking mode on reasoning models.
 * Voice / QuickApp loops set this so mimo-v2.5-pro answers directly
 * (reasoning_tokens=0) instead of spending 20s+ on reasoning before
 * a <=40 char reply.  Cleared together with the max_tokens hint. */
static bool s_thinking_disabled_hint = false;

void llm_set_thinking_disabled_hint(bool disabled)
{
    s_thinking_disabled_hint = disabled;
}

/* Add provider-specific "thinking" control to the request body.
 * Only MiMo (api.xiaomimimo.com) documents this parameter, so it is
 * never sent to other hosts. */
static void add_thinking_param(cJSON* body, const char* llm_host)
{
    if (s_thinking_disabled_hint && strstr(llm_host, "xiaomimimo.com")) {
...
static int max_tokens_for_request(int fallback)
{
    return s_max_tokens_hint > 0 ? s_max_tokens_hint : fallback;
}
    ...
    const char* port, const char* api_key, const char* model)
{
    ...
    /* Only persist when the value actually changed — avoids
     * redundant SPI flash writes on every LLM request (the
     * per-request llm_router_apply path will see identical
     * values and skip the expensive save_json call). */
...
/* Warm up the TLS pool for the configured LLM endpoint.  No-op when a
 * live connection is already pooled; must be called from the agent task
 * group (see vela_tls_preconnect).  Only the port-443 HTTPS path uses
 * the pool — plain-HTTP endpoints have nothing to preheat. */
int llm_preconnect(void)
{
    /* Snapshot config under lock.  Copy with the SOURCE sizes — the
     * globals are smaller than llm_http_direct's 128-byte locals. */
    char llm_host[64], llm_port[8];
    pthread_mutex_lock(&s_llm_lock);
    memcpy(llm_host, s_llm_host, sizeof(s_llm_host));
    memcpy(llm_port, s_llm_port, sizeof(s_llm_port));
    pthread_mutex_unlock(&s_llm_lock);

    if (llm_host[0] == '\0') {
        return -1;
    }
    if (strcmp(llm_port, "443") != 0) {
        return -1;
    }

    return vela_tls_preconnect(llm_host, llm_port);
}
    ...
    char* response_buf, size_t buf_size)
{
    ...
    if (is_openai_compat_host(llm_host))
        cJSON_AddNumberToObject(body, "max_completion_tokens",
            max_tokens_for_request(AGENT_LLM_MAX_TOKENS_OPENAI));
    else
        cJSON_AddNumberToObject(body, "max_tokens",
            max_tokens_for_request(AGENT_LLM_MAX_TOKENS));
    ...
    const char* tools_json, llm_response_t* resp)
{
    ...
    if (api_key[0] == '\0') {
        syslog(LOG_ERR, "[llm_proxy] LLM call rejected: api_key is empty. "
            "host='%s' model='%s'. Was llm_router_apply() called?\n",
            llm_host, model);
        return ERROR;
    }
     ...
     * Fast path: use the pre-cached OpenAI-shaped string from
     * tool_registry (built once, invalidated only on tool changes).
     * cJSON_CreateRaw embeds the JSON verbatim in the request body,
     * skipping cJSON_Parse + per-tool cJSON_Duplicate on every call.
     * Slow path (legacy): reshape the Anthropic-format tools_json
     * argument — kept for callers that pass a custom tools list.
     *
     * Callers pass tools_json == NULL to force a tools-less request
     * (e.g. handle_task_complete / force_finish_reply).  Honour that
     * semantic — do not silently inject tools from the registry. */
    if (tools_json) {
        char *openai_pre = tool_registry_get_openai_tools_json();
        if (openai_pre) {
            /* cJSON_CreateRaw strdups the input internally, so the
             * ownership of `openai_pre` never transfers — always free
             * it afterwards regardless of success. */
            cJSON *tools_raw = cJSON_CreateRaw(openai_pre);
            if (tools_raw) {
                cJSON_AddItemToObject(body, "tools", tools_raw);
            }
            free(openai_pre);
        } else {
    ...
    /* Self-check the request body before sending.  The body is built
     * entirely with cJSON (except the raw tools embed below), so it
     * should always parse; if it doesn't we want the malformed bytes in
     * our log rather than a server-side "Invalid JSON in request body". */
    {
        cJSON* probe = cJSON_Parse(post_data);
        if (!probe) {
            syslog(LOG_ERR,
                "[%s] request body failed self-check, dumping: %.800s\n",
                TAG, post_data);
        }
        cJSON_Delete(probe);
    }
    ...
    if (status != 200) {
        syslog(LOG_ERR, "[%s] API error %d: %.500s\n", TAG, status,
            rb.data ? rb.data : "");
        /* Dump the exact request body the server rejected so a 400
         * "Invalid JSON in request body" can be reproduced locally. */
        syslog(LOG_ERR, "[%s] request body rejected (%d): %.800s\n",
            TAG, status, post_data ? post_data : "(null)");
        free(post_data);
        resp_buf_free(&rb);
        return ERROR;
    }
