...
/* ── Task liveness flag ───────────────────────────────────────── */
/* Set at ai_agent_main entry, cleared right before every return so
 * the launcher can wait for teardown to finish before re-launching
 * the agent (runtime UI-mode switch: expression ↔ watch). */
static volatile bool g_agent_running = false;

/* ── Auto-start mode ────────────────────────────────────── */
/* Set when launched with "auto" argv (e.g. from launcher at boot):
 * skip the CLI thread (would fight NSH for stdin) and start
 * wake-word gated voice listening once the network is up. */
static volatile bool g_auto_voice = false;
...
static void* network_watch_task(void* arg)
{
    ...
    if (network_wait_connected(30000) == OK) {
        ...
        /* Auto mode: voice_channel_start_wake() is called externally
         * by the launcher after both agent init and WiFi are confirmed
         * ready. This avoids race conditions where the conversation
         * thread starts before the network is stable. */
        ...
        /* Play no-network alert audio (highest priority) */
...
static void* outbound_dispatch_task(void* arg)
{
    ...
    while (!g_shutdown_requested) {
        ...
        if (strcmp(msg.channel, AGENT_CHAN_FEISHU) == 0) {
            ...
            /* TTS already handled in dispatch_response (agent_loop.c).
             * Voice channel outbound dispatch is a no-op — the response
             * was already spoken before being enqueued. */
#ifdef CONFIG_AI_AGENT_LVGL_UI
...
{
    /* "auto" argv: launched from launcher at boot — no CLI thread,
     * auto wake-word listening after network is up. */
    g_auto_voice = false;
    for (int i = 1; i < argc; i++) {
        if (argv[i] && strcmp(argv[i], "auto") == 0) {
            g_auto_voice = true;
        }
    }
    ...
    /* Reset the voice backend registries up front: on a runtime UI-mode
     * switch the agent is re-launched via task_create() and these statics
     * persist, so without this the launcher's deferred wake-start sees
     * voice_channel_is_ready()==true immediately and starts wake-listening
     * before this instance re-registers its backends. */
     ...
     *
     * Idempotent: on a runtime UI-mode switch the agent is re-launched
     * (fresh ai_agent_main), and blindly re-applying TZ+tzset() made the
     * displayed clock jump back 8h (local 18:38 → UTC 10:38).  Only touch
     * it when the current TZ differs. */
    const char* cur_tz = getenv("TZ");
    if (cur_tz == NULL || strcmp(cur_tz, AGENT_TIMEZONE) != 0) {
    ...
    if (stat("/data", &st) != 0) {
        ...
        syslog(LOG_INFO, "[%s] /data tmpfs mount: %d (errno=%d)\n",
            TAG, mret, errno);
    }

    /* Ensure directory structure exists (use configured AGENT_DATA_DIR) */
    ...
    syslog(LOG_INFO, "[%s] /data dirs ready\n", TAG);

    /* Restore persistent config from SPI Flash (survives reboot).
     * Only config.json is persisted — skills/memory/sessions are
     * rebuilt at runtime on fast tmpfs. */
    {
        struct stat ps;
        syslog(LOG_INFO, "[%s] checking persistent config %s...\n",
            TAG, AGENT_PERSIST_CONFIG_FILE);
        if (stat(AGENT_PERSIST_CONFIG_FILE, &ps) == 0 && ps.st_size > 0) {
            /* Copy SPI Flash → tmpfs */
            char buf[1024];
            int src = open(AGENT_PERSIST_CONFIG_FILE, O_RDONLY);
            if (src >= 0) {
                int dst = open(AGENT_CONFIG_FILE, O_WRONLY | O_CREAT | O_TRUNC, 0644);
                if (dst >= 0) {
                    ssize_t n;
                ...
                close(src);
                syslog(LOG_INFO, "[%s] Restored config from %s\n",
                    TAG, AGENT_PERSIST_CONFIG_FILE);
            }
        } else {
            /* Ensure SPI Flash persist dir exists for later writes */
            mkdir("/mnt/spif/ai_agent", 0755);
            mkdir(AGENT_PERSIST_CONFIG_DIR, 0755);
        }
    }
    ...
    /* IMU fall/upright monitor.  The agent process runs only in
     * emotion-toy mode (watch mode never starts it; a runtime UI-mode
     * switch tears it down — see launcher.c), so mounting here keeps
     * the detector out of wrist-worn scenarios.  No network needed:
     * the monitor detects from the first frame against a fixed upright
     * reference (no boot calibration — see tool_emotion_motion.c). */
    if (tool_emotion_motion_start() == OK)
        BOOT_LOG(&t0, "P5", "motion monitor started (fixed reference)");
    else
        BOOT_LOG(&t0, "P5", "motion monitor unavailable (no IMU)");
#endif
    ...
    if (!g_auto_voice) {
        ...
        /* Auto mode: skip CLI — cli_thread reads stdin and would
         * fight the NSH console for input. */
        BOOT_LOG(&t0, "P6", "CLI skipped (auto mode)");
    }
    ...
    heartbeat_stop();
#ifdef CONFIG_AI_AGENT_EMOTION_TOY
    /* Motion monitor exits on its next poll timeout (<= 500ms), within
     * the "give threads a moment" window below. */
    tool_emotion_motion_stop();
#endif
