...
/**
 * Return the tools list already reshaped into the OpenAI
 * `"tools":[{"type":"function","function":{...}}, ...]` format.
 * The result is cached alongside the Anthropic-shaped tools JSON
 * and rebuilt only when tool_registry_invalidate() is called, so
 * the per-request cost is one strdup instead of a full cJSON parse
 * + per-tool schema Duplicate.
 *
 * @return  Heap-allocated JSON array string, or NULL on failure.
 *          Caller must free() the returned pointer.
 */
char *tool_registry_get_openai_tools_json(void);

/**
 * Return tools JSON filtered by a whitelist of tool names.
 * Only tools whose name appears in `names` are included.
 * names==NULL or count==0 → returns all tools (same as unfiltered).
 *
 * @param names   Whitelist array of tool name strings, or NULL for all.
