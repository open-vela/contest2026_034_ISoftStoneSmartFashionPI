...
/**
 * Peek the current skills-directory hash (no side-effect).
 * Callers cache this alongside a derived artefact (e.g. the
 * system prompt) and invalidate when it changes.
 * Returns 0 before skill_loader_check_changed / refresh has run once.
 */
unsigned int skill_loader_get_hash(void);

/**
 * Check if user text triggers a known builtin skill.  If so, return
 * the full skill content (caller-owns the returned malloc'd copy).
 * Returns NULL when no builtin skill matches.
 *
 * This lets context_builder inject the skill steps directly into the
 * system prompt, eliminating the read_file LLM round-trip.
 */
