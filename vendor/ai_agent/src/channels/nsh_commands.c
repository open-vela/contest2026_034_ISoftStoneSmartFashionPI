...
static void cmd_help(void)
{
    printf(
        ...
        "  set_voice_asr <name>   - Switch ASR backend\n"
                "  set_voice_mode <mode>  - Voice pipeline: classic|e2e\n"
        "  set_volc_e2e_speaker <v> - Set E2E TTS voice\n"
        "  set_mimo_key <api_key> - Set MiMo (Xiaomi) API key\n"
        "  set_mimo_voice <name>  - Set MiMo TTS voice (冰糖/茉莉/苏打...)\n"
        "  set_mimo_asr_lang <l>  - Set MiMo ASR lang (auto/zh/en)\n"
...
#define SHOW_CFG(label, key, mask)                                        \
    ...
    SHOW_CFG("Volc Speaker", AGENT_CFG_KEY_VOLC_SPEAKER, false);
        SHOW_CFG("Voice Mode", AGENT_CFG_KEY_VOICE_MODE, false);
        SHOW_CFG("Volc E2E Speaker", AGENT_CFG_KEY_VOLC_E2E_SPEAKER, false);
...
static void* cli_thread(void* arg)
{
    ...
    while (fgets(line, sizeof(line), stdin) != NULL) {
        ...
        else if (strcmp(cmd, "set_voice_mode") == 0)
            cmd_set_voice_mode(argc, argv);
        else if (strcmp(cmd, "set_volc_e2e_speaker") == 0)
            cmd_set_volc_e2e_speaker(argc, argv);
        else if (strcmp(cmd, "set_mimo_key") == 0)
            cmd_set_mimo_key(argc, argv);
        else if (strcmp(cmd, "set_mimo_voice") == 0)
            cmd_set_mimo_voice(argc, argv);
        else if (strcmp(cmd, "set_mimo_asr_lang") == 0)
            cmd_set_mimo_asr_lang(argc, argv);
